...
**NOTE:** This feature is enabled in Dynamics 365 for Operations, Spring 2017 release. You would also need to install platform update 8 or later. When you install or upgrade to Spring release, the required platform update is included.

How does this feature work?
===========================

As you are aware, Entity store is an operational data warehouse that enables Power users to author reports. Whenever Entity store is refreshed, all available Financial dimensions are included in the Entity store. Let’s consider an example from the **Ledger Activity** aggregate measurement which contains General Ledger journal level details.

With this change, following tables are populated in Entity store whenever it is refreshed. Tables that have changed are highlighted.
...
If the user defines a new Financial dimension with the name, say, **vendor**, with subsequent refresh of the Entity store, following changes will be made

A new table is added to Entity store with the following name

**LedgerActivityMeasure\_Vendor**

The table **LedgerActivityMeasure\_DimensionCombination** contains a new set of fields as follows
...
A business user can create a new report using Financial dimensions using PowerBI desktop. Existing reports that contain Financial dimensions can be refreshed to include additional fields into the report.

Let’s create a PowerBI desktop report using the Ledger Activity measure group. Connect to Entity store database using Power BI desktop tool in a developer environment and select the following tables
...
Using the manage relationships option in Power BI desktop, define relationships with the following table fields
...
Create a matrix report using the **Sales** and **YearName** fields. You should see a report as follows

![](media/d1d0e190896bec3a755b9b586a3cb657.png)

Next let’s add Financial dimension values. Expand the **LedgerActivityMeasure\_DimensionCombination** table in the fields list in PowerBI desktop. You will notice the list of dimension fields expanded into table fields as shown below.

![](media/b5099b0c03fa16f0c2ab70943c7cce8b.png)

Include the **BusinessUnit\_Description** field in the report. Your report should now show Sales by Business unit as follows

![](media/c39e94631896301db6675d36fc9d3886.png)

Notice that you have the **BusinessUnit\_description** as well **BusinessUnit\_Value** fields. The value field enables you to get a numerical value which can be used to sort columns.

Next, you can define a new Financial dimension and refresh Entity store. On completion of refresh, you can right click on the **LedgerActivityMeasure\_DimensionCombination** table and select “**Refresh Data**”. Notice that the newly defined Financial dimension is reflected in the list of fields available for reporting.
...
As we discussed earlier, new tables also got created in Entity store. Each Financial dimension field is also added to a newly created table. Newly created tables contain individual fields for the name, value and description.
...
Open the report created previously. Let us add a Financial dimension table to the report.

Select “**Recent Sources**” option from the Power BI desktop menu. Select the AXDW data connection. Table navigator will be displayed.

Select the table **LedgerActivityMeasure\_LegalEntity** to the report.

**NOTE:** if you are using a developer of Demo environment, you will see this table since **LegalEntity** is a Financial dimension defined in demo data. If you are working with your own data, you will see tables corresponding to Financial dimensions you have defined.

Next, we will relate the newly selected table to existing tables already present within the report. Open the **Manage relationships** dialog in the report.

Join **LedgerActivityMeasure\_DimensionCombination.LegalEntity\_FK** to **LedgerActivityMeasure\_LegalEntity.KEY\_**
...
For a user to see expanded Financial dimensions in entity store, a developer must enable the use of Financial dimensions in Aggregate measurements. It is a best practice to include Financial dimensions when modeling any aggregate measurement that involves Financial data. This can be achieved by creating an aggregate dimension based on “**Financial dimension tables”.** Such an Aggregate dimension, ie. the Aggregate dimension created using “Financial dimension tables” will be expanded at runtime as we saw above.
