...
To log custom telemetry, use the `SysApplicationInsightsTelemetryLogger` class. This class wraps the [!INCLUDE[appinsights](./includes/azure-application-insights-name.md)] telemetry client and provides methods to track events, page views, traces, exceptions, and metrics.

The logger uses the [static constructor pattern](/dynamics365/fin-ops-core/dev-itpro/dev-ref/xpp-static-classes#static-constructors) to ensure a single instance per user session. It also caches the telemetry client to make sure each Application Object Server (AOS) instance uses only one telemetry client.

## Telemetry data contract types

[!INCLUDE[d365foscm](./includes/finops-product-name-long.md)] currently supports the following types of data contracts.
...
To log a custom event to [!INCLUDE[appinsights](./includes/azure-application-insights-name.md)], create an instance of the `SysApplicationInsightsEventTelemetry` class and pass in the necessary payload. Then use the `trackEvent` method on the `SysApplicationInsightsTelemetryLogger` class to send the event.

The following example shows how to send an event when a record is created in the `SysUserLog` table.
...
Out of the box, [!INCLUDE[d365foscm](./includes/finops-product-name-short.md)] use `pageView` entries to log every form that you open in the application.
...
> [!NOTE]
> The previous example comes from the `SysApplicationInsightsGlobalTelemetry` class. This class automatically sends all errors that appear in the Infolog to [!INCLUDE[appinsights](includes/azure-application-insights-name.md)].
...
Interaction with metrics differs from the previous examples. For metrics, you don't need to use a specific data contract. Instead, you can interact directly with the `SysApplicationInsightsTelemetryLogger` class by using the `trackMetric` method. The logger first gets the existing metric instance from [!INCLUDE[appinsights](includes/azure-application-insights-name.md)] and updates the value. By using `trackMetricWithDimensions`, you can add properties that should be used as dimensions. You can slice values based on those dimensions.
