...
1. Schedule enough downtime to complete this operation, because users can't sign in to Finance + Operations (on-premises) while it's occurring.
...
1. Perform an in-place upgrade by following the instructions in [Upgrade Windows Server 2016 to Windows Server 2019](/windows-server/upgrade/upgrade-2016-to-2019).

    > [!NOTE]
    > You receive a warning that states that you must reconfigure your AD FS role after the upgrade is completed. Because you collected the information that's required to re-create the application group, this warning isn't an issue. If you have additional applications, be sure to back up the client IDs so that you can reuse them when you re-create the applications later.

1. After the machine is upgraded, open Server Manager, and complete the configuration of AD FS.
...
#### Create a new AD FS farm or instance and replace your old AD FS farm or instance

1. Create a new AD FS federation farm or instance. For information about how to deploy AD FS, see [Windows Server AD FS Deployment Guide](/windows-server/identity/ad-fs/deployment/windows-server-2012-r2-ad-fs-deployment-guide).
1. Configure AD FS by following the instructions in [Configure AD FS](./setup-deploy-on-premises-pu41.md#configureadfs). However, don't run the **Publish-ADFSApplicationGroup.ps1** script.
...
1. Make a note of the following values: **Active Directory->Client ID for AOS application group** and **Active Directory->Client ID for Financial Reporting application group**.
1. In your new AD FS farm or instance, run the following command.
    ...
    Your new farm or instance is now ready to be used by Finance + Operations (on-premises).

1. If your new farm or instance endpoint differs from the old farm or instance endpoint, be sure to update the endpoint in Microsoft Dynamics Lifecycle Services by selecting the **Update Settings** option and setting the **ADFS OpenID metadata endpoint** field to the new value.

1. If your new farm or instance identifier differs from the old farm or instance identifier, you must update the user info table so that it reflects the new configuration. As of version 2.17.0 of the infrastructure scripts, you can run the following command from one of your AOS nodes to automatically update and regenerate the configuration of each user.
...
To correctly support authentication with the Office add-ins, AD FS on Windows Server 2019 or later requires that you set up Cross Origin Resource Sharing (CORS) headers. You can find this information in [Configure AD FS](./setup-deploy-on-premises-pu41.md#configureadfs). If you're not sure whether you're missing the configuration, run the following script.
...
In Service Fabric standalone clusters, upgrading a node's operating system is a machine-specific operation. (A node can be either a virtual machine \[VM\] or a physical machine.) Two methods exist for doing an in-place upgrade for a node. The first method preserves data and the operating system configuration. The second method doesn't preserve data and the operating system configuration. You can also create new nodes and then add them to the cluster.

All the methods that this section describes keep your cluster up and running, and don't affect the availability of the service, provided that you don't upgrade all the nodes at the same time.
...
> [!IMPORTANT]
> If your cluster has only three seed nodes (**OrchestratorNodeType**), upgrade only one node at a time. Otherwise, the cluster can't keep the system services running, and the cluster isn't available.
>
> If your cluster has only one Business Intelligence (BI)/Management Reporter (MR) node, schedule the node upgrade during a downtime window, because the related services aren't available during the upgrade process.
...
During the operating system upgrade, the process removes all data. Therefore, the node has a fresh operating system installation. You must complete all the configuration steps for each node before you add it back to the cluster.
...
1. After the node is added and the services on it are healthy, repeat the previous steps to provision, configure, and add additional nodes.
1. After you add all the nodes, remove the nodes that run the older operating system by following the instructions in [Remove a node](./onprem-remove-reinstall-aos-node.md#remove-a-node).
