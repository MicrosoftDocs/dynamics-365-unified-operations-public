...
- Central US is no longer being provided as an option for Service Fabric migration. If a customer is planning to leverage dual-write functionality, virtual entities, or any FO apps add-ins that have dependences on Dataverse, please keep in mind that Dataverse is not supported in Central US and there are no plans to support it in Central US for the foreseeable future. We will plan to move your environment to East or West US instead of Central, for continued functionality of such features on self-service capabilities in a supported region.
