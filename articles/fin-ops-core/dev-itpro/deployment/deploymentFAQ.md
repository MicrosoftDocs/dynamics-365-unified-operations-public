...
- Central US is no longer being provided as an option for self-service migrations. If a customer is planning to leverage dual-write functionality, virtual entities, or any Finance and Operations apps add-ins that have dependences on Dataverse, keep in mind that Dataverse is not supported in Central US and there are no plans to support it in Central US for the foreseeable future. We will plan to move your environment to East or West US instead of Central, for continued functionality of such features on self-service capabilities in a supported region.
