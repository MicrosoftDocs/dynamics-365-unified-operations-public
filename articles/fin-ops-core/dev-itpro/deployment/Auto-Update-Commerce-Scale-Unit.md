...
[Autoupdate configurations](../lifecycle-services/configure-service-updates.md) for environments also apply to CSUs. To [pause autoupdates](../lifecycle-services/pause-service-updates.md), CSUs must be on the current release (N) or the previous one (N-1). Releases older than N-1 must be updated to be compliant.
...
All in-store components must run released software that's less than 12 months old to maintain support. Customers are responsible for updating self-hosted components such as components installed in stores or in privately managed datacenters, and ensuring that the installed versions of these components are actively supported.
...
With autoupdate for CSU, updates to in-store components can still be rolled-out in phases using the following controls:
