...
When you deploy a finance and operations apps environment in a geography, you associate multiple Azure resources with it. As part of the migration process, you move these resources from the source geography to the target geography.
...
- You can migrate sandbox and production environments between geographies.
...
- **Third party integrations:** If the migration crosses data resident geographies, the finance and operations apps environment endpoint changes. In this case, you need to change any third-party integrations that use the endpoint. For more information, see [Supported geographies and endpoints](deployment-options-geo.md#supported-geographies-and-endpoints).
- **Finance and operations apps add-ins and micro-services:** Add-in configurations aren't migrated as part of migration process. You must uninstall the add-ins before the migration and then reinstall them after the migration is completed. (For example, dual-write must be reconfigured in the target geography). If you're relocating to a different region within the same geography, you can skip this step.

### Timeline of actions and downtime

- Migrate sandbox environments first and validate them before you trigger a production migration.
...
Migrate and validate sandbox environments before you trigger a production migration.

After you successfully complete sandbox migration and validation, the project team can plan the time for production environment migration and follow these steps, starting with step 2. 
...
| 8 | Customer/Partner | Reconfigure any add-ins, Commerce/point of sale (POS), third-party integrations, and so on. | Consider changes to endpoint URLs. This step doesn't apply if move is for region within same geography. |
