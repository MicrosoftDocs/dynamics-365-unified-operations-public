...
1. In the certificates section, find the certificate of the **RSAT** type, and set the **disabled** option to **false**.
...
1. Rerun the prerequisites script to ensure that the certificate is installed. Perform this step as an administrator.
...
1. Open the **localagent-config.json** file.
1. Under **deploymentOptions**, set **rsatEnabled** to **true**. You must also set **rsatCertificateThumbprint** to the thumbprint of the RSAT certificate that you generated. You can check the thumbprint in the **ConfigTemplate.xml** file.
...
1. Select **Update environment** to start updating your environment.
    ...
    After the update operation completes, the environment is configured to work with RSAT.
