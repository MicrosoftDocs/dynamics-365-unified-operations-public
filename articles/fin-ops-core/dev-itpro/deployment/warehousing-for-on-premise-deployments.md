...
| ActiveDirectoryResource    | Specify the root URL of Finance + Operation (on-premises).<br><br>**Note**: Be sure to include '/namespaces/AXSF'.| 
...
| ConnectionType             | (Optional) Specify whether the connection setting should use a certificate or a client secret to connect to an environment. Valid values are "certificate" and "clientsecret". The default value is "certificate".<br><br>**Note**: Client secrets can't be imported. |
