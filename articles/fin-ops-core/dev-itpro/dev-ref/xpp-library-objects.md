...
Two kinds of classes exist: *application classes* and *system classes*.

- **Application classes** – Implement these classes in X++. You can find them under the **Code > Classes** node in Application Explorer.
- **System classes** – These classes are sometimes known as *kernel classes*. They appear under the **System Documentation > Classes** node in Application Explorer. You can't access the source code for these classes. For a list of the system classes, see [API, class, and table reference](api-reference.md).
...
- **new** method: This method creates an instance of the class. You can call the constructor only by using the **new** keyword. Derived classes can call the **new** method of their constructor by calling the **super** method reference. For more information, see [X++ inheritance](xpp-inheritance.md).
...
Additional methods for a class include the following types:

- Instance methods
- Static methods
- Main methods

You can create methods on many kinds of items. Here are some examples:
...
Use the *substitute application classes* instead of the system classes that they extend.
...
Some of the substitute application classes are associated with a special global variable that represents an instance of the class. For example, the **appl** variable references a pre-instantiated object from the **Application** class. The advantage of the **appl** variable is that the system maintains the object throughout the scope of your session. Your code would be less efficient if it repeatedly used the **new Application()** syntax to obtain an instance of the **Application** class. Don't use the **xApplication** system class. Instead, use the **Application** substitute application class.

You can reference the static members of the **Application** class by using the following standard syntax: **Application::checkForNewBatchJobs()**. However, to reference the instance members of the **Application** class, use that class's **appl** variable, if it exists. This pattern applies to most of the x-system classes. The **Session** substitute application class is one exception, because there's no special global variable for **Session**.
...
Implement classes by using the batch processing system and by extending the **RunBase** and **RunBaseBatch** classes. To remove the **Recurrence** button from the **Batch processing** dialog box, use the **Args::parmEnum** method. Designate a class to run as a server-bound batch method. Server-bound batch methods are more secure than batch methods that aren't server-bound for the following reasons:

- The method runs by using the permissions of the user who submitted the method.
...
1. Override the **RunBaseBatch.runsImpersonated** method to return **true**, as shown in the following example.
...
1. Confirm that the class calls only the following **Info** and **Global** class methods:
    - add
    - Info.copy
    - Info.cut
    - Info.import
...
Two system classes let you manipulate graphics and icons: **Image** and **Imagelist**.

- **Image** – This class lets you load, save, and manipulate individual images. For example, you can capture a screen and save it as an image, crop or rotate an image, or change the color depth.
- **Imagelist** – This class lets you work with a set of images that have common properties, such as the size and transparency color. You can view the image lists that the **ImageListAppl** application classes use.

## Query object model

The query object model contains classes that you use to define and run a query. Use the query objects to define the query data source, the fields that are returned, record ranges, and relations to child data sources. The query classes are more visible when you create a dynamic query in code, but they're also used behind the scenes when you create a static query in Application Explorer.
...
| QueryBuildDataSource | This class defines access to a single data source in the query. If there's more than one data source at the same level in a query, the query produces separate SQL statements and runs them sequentially. If one data source is a child of another data source, the query creates a join between the two data sources. |
...
| QueryBuildRange      | This class defines a subset of records that are returned, based on a single field. A range is translated into a **WHERE** clause in the query SQL statement. If more than one field is used to limit the query (**WHERE** clause), the data source contains more than one range. |
| QueryBuildDynalink   | This class contains information about a relation (limitation) to an external record. When the query runs, this information is converted to additional entries in the **WHERE** clause of the query SQL statement. This class can exist only on the parent data source of a query. Forms use the function when two data sources are synchronized. The child data source then contains one or more DLLs to the parent data source. The function is used even if the two data sources are put in two different forms but are still synchronized. |
...
- Dynamic methods
- Properties – These properties are member functions that set properties. An example is **LeftMargin**.

You can't override system class methods. It's not intended that you use the system classes to design your application objects from scratch. Instead, use them to extend or modify the default functionality in Application Explorer. For example, you can dynamically add extra information to an existing report. Alternatively, you can change the options that are available on a page, based on the user's selection on a previous page.
...
These system classes hold functions that activate whenever you use Application Explorer to create your application. For example, the system uses the **FormDesign** class when you define the layout of your form in the **Designs** node in Application Explorer. These classes also let you create and modify application objects.

### Integration classes

Classes typically implement the integration with the environment. Here are some examples of the classes in this category:
