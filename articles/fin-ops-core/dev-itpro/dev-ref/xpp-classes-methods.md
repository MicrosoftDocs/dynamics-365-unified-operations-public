...
A *class* defines the data and methods of the instances that you later construct from that class. The *class* is an abstraction of an *object* in the problem domain. The instances that you can construct from the *class* are known as *instances* or *objects*. This article uses the term *instance*. The data represents the state of the object and the methods represent the behavior of the object.

*Fields* contain the data for the class. Every instance that you construct from the class declaration has its own copy of the fields. These fields are known as *instance fields*. This article uses the term *field* in most cases.

*Methods* define the behavior of a class. They contain the statements that operate on the data. By default, you declare methods to operate on the instance fields of the class. These methods are known as *instance methods* or *object methods*.
...
Use the **Add new item** dialog in Visual Studio to add a class to your project.

1. In Server Explorer, right-click the project, and then click **Add**.
1. In the **New item** dialog box, select **Installed > Dynamics 365 Items > Code** in the left navigation. Select **Class** and enter a name for the class.
1. Click **Add**.
...
Instance fields are **protected** by default. This default setting means that you can access these fields only within the same class or [a derived class](xpp-inheritance.md). Use the **private**, **protected**, or **public** keywords to change the access level of an instance field.

> [!NOTE]
> Making a member field public isn't a good idea because it exposes the internal workings of the class to its consumers. This exposure creates a strong dependency between the class implementation and its consumers. Always strive to depend only on a contract, not an implementation.

You can assign a value to a field inline along with the declaration of the field itself. This assignment applies to both static and instance fields. The class initializes these values (for instance classes), or the static constructor (**typeNew**) calls them.

The following example shows how to use accessor methods to make the field data public. The field **firstName** is protected, so the example implements accessor (get and set) methods to allow access to the protected field. The field **lastName** is public, so code can get and set the value of the field.
...
A useful attribute is the **SysObsolete** attribute. If you apply the **SysObsolete** attribute to a field, the compiler generates an error or warning on any reference to the field. Whether it's a warning or error depends on the second parameter in the attribute, as shown in the following code:
...
To create an instance of a class, you must instantiate it. Use the **new** keyword to instantiate the class. When the **new** expression is evaluated, it allocates memory on the heap for that state and calls the *constructor*.

- You can define only one **new** method (constructor) in a class.
- If you don't define a constructor, the compiler automatically creates a default constructor with no parameters.
...
To create a clean inheritance model and minimize problems when upgrading code, follow these steps:
...
Use the **finalize** method to explicitly clean up the state of an object. This method is a convention, so it doesn't have special semantics. The managed runtime doesn't make implicit calls to the **finalize** method when it disposes of the object. You must call the method to explicitly clean up. In most cases, you don't need to provide final methods.
...
Call instance methods by using an instance of the class, so you must instantiate the object before you can use the method. The following code shows how to define an instance method and call it from an instance.
...
Use the following syntax to call static methods.
...
A **main** method is a class method that you can run directly from a menu option. The **\_args** parameter lets you transfer data to the method.
...
Each method requires a return type. If a method doesn't return anything, use the **void** keyword as the return type.
...
Apply several modifiers to method declarations. Some modifiers can be combined, such as **final static**. Here are the method modifier keywords:

- **abstract** - Declare the method but don't implement it. Subclasses must override the method. If you try to create an object from a subclass where one or more abstract methods that belong to the parent class aren't overridden, you receive a compiler error. Define abstract methods only in abstract classes.

Classes can also be abstract. Sometimes, a class shouldn't be instantiated because it represents an abstract concept. Only subclasses should be instantiated. Declare base classes of this type as **abstract**. For example, you want to model the concept of an account. Accounts are abstract entities, because only derived classes (savings accounts, and so on) exist in the real world. This example describes a clear case where you should declare the **Account** class as **abstract** and have derived classes for CheckingAccount, SavingsAccount, and so on.

- **display** - The method's return value shows on a page or a report. The value can't be modified on the page or report. Typically, the return value is a calculated value, such as a sum.
- **edit** - The method's return type provides information for a field that is used on a page. The value in the field can be modified.
- **final** - The method can't be overridden in any class that derives from its class.
- **public** - Access **public** methods anywhere that the class is accessible. Subclasses can override these methods. Methods without an access modifier are implicitly public.
- **protected** - Call **protected** methods only from methods in the class and in subclasses that extend the class where the method is declared.
- **private** - Call **private** methods only from methods in the class where the private method is declared.
- **internal** - Call **internal** methods only from methods in the class that are defined in the same model as the class where the method is defined.
...
Use the accessor keywords **public**, **protected**, and **private** to control whether methods in other classes can call the methods on your class. The accessor keywords on methods also interact with the rules for class inheritance.

The accessor keywords that you use with methods include:

- **public** - Methods that you declare as **public** can be called from anywhere that the class is accessible. In addition, a public method can be overridden by a subclass, unless you declare the method as **final**.
- **protected** - Methods that you declare as **protected** can only be called from the following methods:
  - Methods in the class.
  - Methods in a subclass of the class that contains the protected method. You can override protected methods in subclasses.
- **private** - Methods that you declare as **private** can only be called from methods in the class where you declare the private method. You can't override private methods in a subclass. To make maintenance of your code easier, hide implementation details as private methods.
...
When you override a method in a subclass, the overriding method must be at least as accessible as the overridden method. For example, the following compiler rules apply when a protected method is overridden in a subclass:

- You can override a public method in a superclass only with a public method in the subclass.
...
You can initialize parameters in the method declaration parameter list. In this case, the parameter becomes an *optional parameter*. If you don't supply a value in the method call, the default value is used. List all required parameters before the first optional parameter. The following examples show how to create and call a method that has optional parameters.
...
All methods define their own *scope*. Within the scope of the method, the parameters are treated as local variables and are initialized with a value from the expression in the method call. All parameters are passed by value. If you use a variable as a parameter value in the call, that variable doesn't reflect any changes made to the parameter.

## Scope of variables in methods

Scope defines the area in which you can access an item. Methods can access variables that you define in a class. You can only access variables in methods within the current block. Compound statements and local function declarations establish their own nested scopes.

## Local functions

You can declare functions inside a method. These functions are called local functions.

- You can declare any number of local functions in your method.
- Code inside the local function can access variables that are declared earlier in the method that contains the local function.
- Code outside the local function can't access variables that are declared in the local function.
- Only code in the method where the local function is declared can call the local function.
...
The extension method feature adds extension methods to a target class by writing the methods in a separate extension class. From the user's perspective, it's as if the extension method was written on the extended method. The following rules apply:
...
It's perfectly valid to have private or protected static methods in an extension class. These methods typically handle implementation details and aren't exposed as extensions. The following example illustrates an extension class that holds a few extension methods:
...
The extension method technique doesn't affect the source code of the class it extends. Upgrades to the target class never affect any existing extension methods. However, if an upgrade to the target class adds a method that has the same name as your extension method, your extension method becomes unreachable through objects of the target class. Extension methods are easy to use. The extension method technique uses the same dot-delimited syntax that you routinely use to call regular instance methods. Extension methods can access all public artifacts of the target class, but they can't access things that are protected or private.

### Where can you apply extension methods
...
Regardless of the target type, you use an extension *class* to add extension methods to the type. For example, you don't use an extension table to add methods to a table, and there's no such thing as an extension table.

## The this keyword

The **this** keyword is a reference to the instance of the class or table where you use the **this** keyword. The **this** reference is required on instance method calls (if the method called is on a class in the class or any of its superclasses), but it can clarify your code and enhances the behavior of IntelliSense in the code editor. All calls to instance methods must be qualified by either the **this** reference or a variable. You can use the **this** reference to qualify the following information:

- The names of other instance (nonstatic) methods in the same class where you use the **this** reference. Here's an example: `boolColorChanged = this.colorItOrange();`
- The names of methods that the **this** object inherits.
- The names of fields on the table that contains the method where you use the **this** keyword.

You can't use the **this** reference in the following ways:
...
You can nest classes in X++ source code. You can use nested classes only inside forms (that is, classes that extend FormRun) to represent controls, data sources, or data fields.

## Jobs

To quickly and easily run an X++ method, add a `static Main(xArgs args)` method to a class. Set the class as the startup object form for the project in Microsoft Visual Studio. When you run the project, the `Main` method runs.
