...
The Document Reporting Service bundled with finance and operations apps provides user tooling to facilitate two basic user functions, or generic scenarios, in enterprise resource planning (ERP): **document generation** and **business intelligence (BI) and analytics**. These generic scenarios represent the two most frequently used methods for accessing business data.

- **Document generation** – The production of structured documents, where the primary purposes are often printing and bulk email distribution.
...
Using PDFs to interact with documents that the Document Reporting Service renders and bundles with finance and operations apps provides several advantages:

- **Reports appear on the screen more quickly.** Users benefit from improved performance when reports appear on the screen. This performance improvement includes a reduction in the number of progress bars, so that reports are faster.
...
- **Printed output has higher fidelity.** Documents appear in PDF formats to provide consistency with printer output.
- **Local devices can print documents.** You can maintain user identity when you send documents directly from your browser to a printer. You can use the built-in user options on the control toolbar to help secure print jobs when you print documents.
...
1. Open Windows PowerShell, and go to `C:\AOSService\PackagesLocalDirectory\Plugins\AxReportVmRoleStartupTask\`.
1. Run the **UpdateRDLSandboxRule.ps1** command. Here's a list of the parameters:
