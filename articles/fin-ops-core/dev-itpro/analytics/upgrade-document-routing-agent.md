...
The solution for managing the print job queue helps you scale Dynamics 365 finance and operations apps to meet high-volume printing needs. Although the public service endpoints that manage print jobs are backward-compatible, update **all** existing Document Routing Agent (DRA) clients.

If you don't update existing installations of the DRA to the most current version, you might encounter problems such as:
...
IT administrators must perform the following procedures on each domain resource that hosts a DRA.

> [!NOTE]
> When you complete a DRA update, register any printers that connect through the host server. For network printers that are identified by their network paths, if the paths didn't change, updates aren't required.

## Get started

To keep running the DRA as a Microsoft Windows service, you need both the user name and password of the domain account that runs the service. You must have this information after the update finishes. To find the information for the active service account, start the Microsoft Management Console (MMC) Services snap-in and select **Microsoft Dynamics 365 Document Routing Service** in the list.
