...
When you manage your finance and operations instances, you might decide to migrate the current instance to another location. For example, you might migrate your production instance to a new sandbox environment. If you configure the ER framework to store templates in Blob storage, the DocuValue table in the new sandbox environment refers to the instance of Blob storage in the production environment. However, you can't access this instance from the sandbox environment, because the migration process doesn't support the migration of artifacts in Blob storage. Therefore, if you try to run an ER format that uses a template to generate business documents, an exception occurs, and you're notified about the missing template. You're also guided to use the ER cleanup tool to delete and then re-import the ER format configuration that contains the template. Because you might have several ER format configurations, this process can be time consuming.
...
If an ER format template is required to generate outbound documents, such as to process vendor payments including generation of payment advice and control reports, but the required template isn't found in the primary storage location, the following events occur:
...
- Every user who is assigned to the **Electronic reporting developer** or **System administrator** role is notified about the missing template issue through the Action center. The message that appears depends on the value of the **Automatically run the procedure of restoring the broken templates in batch** parameter:
    ...
    - If this parameter is set to **On**, the message notifies you that a missing templates issue has been discovered, and that a new batch process, **Restore broken templates from internal database backup**, has been automatically scheduled. This batch process automatically fixes similar issues for other templates.
...
:::image type="content" source="./media/GER-BackupTemplates-1.png" alt-text="Screenshot of the ER configurations page showing User parameters.":::
...
:::image type="content" source="./media/GER-BackupTemplates-2.png" alt-text="Screenshot of the Vendor payment journal page showing the notification message.":::
...
:::image type="content" source="./media/GER-BackupTemplates-3.png" alt-text="Screenshot of the Batch job page showing the Restore broken templates from internal database backup process.":::

The execution log of the completed **Restore broken templates from internal database backup** batch process includes information about the templates that the process restores from the backup storage location to the primary storage location.

:::image type="content" source="./media/GER-BackupTemplates-4.png" alt-text="Screenshot of the Batch job history page showing the execution log of restored templates.":::
...
:::image type="content" source="./media/GER-BackupTemplates-5.png" alt-text="Screenshot of the Electronic reporting parameters page showing backup storage options.":::

## Manual recovery

Go to **Organization administration** > **Electronic reporting** > **Restore broken templates** to manually restore ER templates from the backup storage location to the primary storage location. Before you start this process, use the **Restore broken templates** page to specify whether to perform the restoration interactively or schedule it as a batch process.
