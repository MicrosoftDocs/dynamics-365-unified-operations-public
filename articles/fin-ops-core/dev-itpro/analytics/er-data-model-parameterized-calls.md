...
However, in version 10.0.15 and later, you can configure data model fields that can be called only when the values of the configured parameters are provided. When a field like this is configured and called from a format component, the required parameters must be provided in a format binding as arguments of such a call. In this case, the values of such arguments can be specified based on format specific values. This allows you to use **dynamic runtime parametrization** for data model calls and support the following data flow.
...
With this new functionality, you can parametrize the call to any data model field of the [*Record*](er-formula-supported-data-types-composite.md#record) or [*Record list*](er-formula-supported-data-types-composite.md#record-list) type. The following data types are supported for the parameters of a data model field:
...
You can specify every parameter of a data model field whose argument can be provided as a single value of the defined data type and the [*list*](er-formula-supported-data-types-composite.md#record-list) of such values.

> [!NOTE]
> The default value for the parameter of a data model field isn't supported. If you add a parameter to a field in a data model, the version of which has been already released and published, you must [rebase](general-electronic-reporting.md#upgrading-a-format-selecting-a-new-version-of-base-format-rebase) all corresponding model mappings and formats to the new version of this model as this data model change isn't backward compatible.

You can configure parametrized data model fields to make model mapping calls format-specific. This configuration can help you reduce the number of model mappings that must be configured for many formats of a single data model. You can also use this technique to improve the execution performance of your formats and reduce the time needed to generate business documents. To learn more about this feature, complete the example in this topic.

## Example: Use parameterized calls of ER data models

The following steps explain how the System administrator or Electronic reporting developer can design an ER solution that contains a data model, a model mapping, and a format ER component that are configured to call a model mapping from a format by providing an argument for the call whose value has been calculated at runtime by using the formula of the running format. 
...
Let's assume that you have an ER solution with a format you can run to generate an electronic document for audit purposes and that contains sales orders and purchase orders related tax transactions with required details (transaction date, tax value, etc.). Starting from the new fiscal year, the structure of this document has changed. You must now submit the extended document that with additional details (names) of all parties (customers and vendors) whose transactions are presented in a generated report. You will need to modify your ER solution to generate a document that's compliant with this new requirement.

### Configure the ER framework

Follow the steps in the topic, [Configure the ER framework](er-quick-start2-customize-report.md#ConfigureFramework) to set up the minimal set of ER parameters. You must complete this setup before you start to use the ER framework to design a new ER solution.

### Design a domain-specific data model

You must create a new ER configuration that contains the required data model component. This data model will later be used as a data source when you design an ER format to generate an audit report. Complete the steps in this section to import the required data model from the provided XML file.
...
Currently, the model is designed to expose only tax transactions with required details.

### Design a model mapping for the configured data model

As a user in the Electronic Reporting Developer role, you must create a new ER configuration that contains a model mapping component for the Sample audit data model. Because this component implements the configured data model for the Finance application, it's Finance-specific. You must configure the model mapping component to specify the application objects that must be used to fill in the configured data model with application data at runtime. To complete this task, be aware of the implementation details of the data structure of the tax business domain in Finance. Complete the steps in this section to import the required model mapping from the provided XML file.
...
Currently, the model mapping is designed to expose tax transactions with required details fetching this information from the **TaxTrans** application table by using the configured `TaxTrans` and `TaxTransFiltered` ER data sources.

### Design a new format

As a user in the Electronic Reporting Functional Consultant role, you must create a new ER configuration that contains a format component. You must configure the format component to populate tax transactions to a generated report with all required details. Complete the steps in this section to import the required format from the provided XML file.
...
The ER format is configured to generate a report as a text file in CSV format.

### Run the imported format

1. On the Action Pane, for the selected **Sample audit format** configuration, select **Run**.
2.IOn the **Electronic report parameters** dialog box, on the **Records to include** tab, select **Filter**.
    1. Specify the conditions to select the tax transactions of the vouchers, **PIV-110000004** and **INV-10000001**.
    2. Select **OK**.
3. Select **OK**.
...
Based on the new requirement, you must submit a document that contains the names of customers and vendors whose transactions are included. To do this, modify the imported ER solution to generate a document that's compliant with this new requirement. Decide how you want to implement the required modifications of the imported ER components.
...
- In your model mapping, configure the binding for the added data model field using available table relations to access the relevant record of the `DirPartyTable` application table and fetch the value of the `Name` field from this record.

This approach will work, but it might cause performance issues as the **TaxTrans** table is the transaction table and therefore may contain a large volume of records. In this case, the number of calls to **DirPartyTable** must be equal to the number of records in the **TaxTrans** table that can cause performance degradation of the SQL database.
...
This approach will work, but it might cause some memory consumption issues. Even when a new [JOIN](er-join-data-sources.md) data source is executed as a single SQL request to application database to prevent database performance issues, the result must be fetched to the memory of the application server. As the number of records and the number of fields in those records will be quite large, this approach might cause very heavy memory consumption or even an out-of-memory runtime exception might be thrown.

You could implement the modification when an executed format will collect in memory, the unique identification codes of customers and vendors for all tax transactions that are placed to a generated report. As only unique codes should be kept, the final set of codes will not be too large to hit memory consumption. Then, this set of codes will be passed to the model mapping as the argument of another call of the data source of the **Model** type. Based on the call, the model mapping will execute a new ER data source that will place a single SQL request to the application database for fetching records from the **DirPartyTable** only for those parties whose codes are presented in the provided set of codes.
...
2. On the **Configurations** page, in the configurations tree in the left pane, select **Sample audit model**.
    1. On the **Versions** FastTab, select version 2  that is in [**Draft**](general-electronic-reporting.md#component-versioning) status.
    2. Expand **Configuration components** FastTab.
...
5. For the selected `Root.Party` field, on the **Node** tab, select **Parameters**.
    1. In the **Parameters** dialog box, on the **Parameters** tab, select **New**.
    2. In the **Name** field, type **PartyRefRecId**.
    3. In the **Type** field, select **Int64**.
    4. Check the **List** field and select **OK** to finish parameters entry.

    > [!NOTE]
    > You configured the `Root.Party` data model field as a field that is called when a value is provided in the **PartyRefRecId** parameter. This value must be in the list of records that contain the field with the `Value` name and the *Int64* data type.
...
8. On the **Versions** FastTab, for version 2, select **Change status** > **Complete**, and select **OK**.
    > [!TIP]
    > Your data model changes were stored in the second revision of the **Sample audit model** data model component that's located in the second version of the **Sample audit model** ER configuration.
...
2. Select **Sample audit model mapping**, and on the **Versions** FastTab, do the following:
    1. Select the 2nd mapping version that is based on the 1st data model version (version 1.2) that is in a **Draft** status.
    2. Select **Rebase**.
    3. In the **Target version** field, leave version 2 of the **Sample audit model** base model.
    4. Select **OK** to rebase and align your model mapping with recent data model changes.
    > [!TIP]
    > The version number of your editable model mapping has changed from 1.2 to 2.2 indicating that the 2nd model version is currently used as a base.
3. Select **Designer**, and on the **Model to datasource mapping** page, select **Designer** for the current model mapping.
4. Add a new data source to access the `DirPartyTable` application table.
...
5. Add a new data source to request **DirPartyTable** records based on the provided list of record identification codes.
    ...
    2. In the **Data sources** pane, select the **Data** data source.
    3. In the **Data source type** pane, select **Functions \> Calculated field**.
    4. In the **Data sources** pane, select **Add**.
        ...
        2. Select **Edit formula**.
            1. On the **Formula designer** page, select **Parameters**.
                ...
                3. In the **Type** field, select *Int64*.
                4. Check the **List** field and then select **OK**.
                > [!TIP]
                > You configured this calculated field to accept at runtime, the argument of a single parameter that's configured as a record list with a single **Value** field of the **Int64** type.
            2. In the **Formula** field, enter the expression, `FILTER(PartyTable, VALUEINLARGE(PartyTable.RecId, DirPartyId, DirPartyId.Value))`.
            > [!TIP]
            > You configured this **DirParty** calculated field to fetch only `DirPartyTable` records whose record identification codes are among codes of the `DirPartyId` list that's provided as an argument when the **DirParty** field is called.
        ...
        3. Select **Save** and then select **OK** to finish a new data source entry.
6. Bind the added data source to the new data model field to use data model for exposing party names.
    ...
    2. In the **Data model** pane, select the `Root.Party.Name` data model field.
        1. In the **Data model** pane, select **Edit**.
        2. On the **Formula designer** page, in the **Data source** pane, complete the following steps:
...
7. Select **Save** and close the **Model mapping designer** page.
8. Close the **Model to datasource mapping** page.
9. In the **Versions** FastTab, for version 2.2, select **Change status \> Complete**, and select **OK**.

### Adjust the imported format

1. On the **Configurations** page, select **Sample audit format**.
2. On the **Versions** FastTab, select the version **1.2** with a status of **Draft**.
    1. Select **Rebase**.
    2. In the **Target version** field, leave version 2 of the **Sample audit model** base model.
    3. Select **OK** to rebase and align your format with the recent data model changes.
3. Select **Designer**.
...
5. Add a new format element to collect record identification codes of parties whose transactions are populated to a generated report.
    ...
    > [!TIP]
    > A **Data source \> Item** element can only be used to perform internal calculations and data transformation in the scope of the running format. Therefore, by adding this format element, you do not change the content of a generated document.
    
6. Add new format elements to populate party names to a generated report.
    1. Select the **Report.Row** sequence element.
        1. Select **Add**.
        2. In the **Add** dialog box, select **Text \> Sequence**.
        3. In the **Component properties** dialog box, in the **Name** field, enter **Party**.
        4. Select **OK**.
    2. Select the **Report.Row.Party** sequence element.
        1. Select **Add**.
        2. In the **Add** dialog box, select **Text \> String**.
        3. In the **Component properties** dialog box, in the **Name** field, enter **Name**.
        4. Select **OK**.
7. On the **Mapping** tab, add a new data source to collect record identification codes of parties whose transactions are populated to a generated report.
    ...
    5. Select **OK**.
8. Add a new binding to collect record identification codes of parties whose transactions are populated to a generated report.
...
9. Add new bindings to populate party names to a generated report.
...
10. Select **Save** and close the **Format designer** page.
11. Close the **Model to datasource mapping** page.
12. On the **Versions** FastTab, for version 2.2, select **Change status** > **Complete**, and select **OK**.
...
2. In the **Electronic report parameters** dialog box, on the **Records to include** tab, select **Filter**.
    ...
    2. Select **OK**.
3. Select **OK**.
4. Review the generated document that contains tax transactions of the selected vouchers and the names of the corresponding customer and vendor.
...
For more details about the execution of the ER solution, trace this execution by using the ER built-in [performance trace](trace-execution-er-troubleshoot-perf.md) parser.
