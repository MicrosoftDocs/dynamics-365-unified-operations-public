...
The **Range** component can be configured as vertically replicated to generate multiple rows in an Excel worksheet. Multiple rows can be generated by the parent **Range** component or by its nested **Range** components. In version 10.0.26 and later, you can force a generated worksheet to keep those rows together on the same page. For doing this, in the ER format designer, set the **Keep rows together** option to **Yes** for the parent **Range** component in the editable ER format. Based on this setting, ER will try to keep the content that is generated by this range on the same page. When the height of this content doesn't fit to the size of the remaining part of the current page, a page break will be added to place this content starting from the beginning of the new page.

> [!NOTE]
> We recommend that you configure the **Keep rows together** option for only ranges that embrace the entire width of a generated document.
>
...
>
> This feature can be used for the **Range** components that reside under the **Page** component. However, the proper calculation of [page footer totals](er-paginate-excel-reports.md#add-data-sources-to-calculate-page-footer-totals) by using [Data collection](er-data-collection-data-sources.md) data sources isn't guaranteed.

To learn how to use this option, follow the example steps provided in the topic, [Design an ER format to keep rows together on the same Excel page](er-keep-excel-rows-together.md).
