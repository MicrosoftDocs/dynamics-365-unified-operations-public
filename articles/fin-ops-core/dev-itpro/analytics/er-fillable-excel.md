...
You can use the **Page** component when you want Excel to follow your ER format and structure paging in a generated outbound document. When an ER format executes components that are under the **Page** component, the required page breaks are automatically added while taking into account the size of the generated content, the page setup of the Excel template, and the paper size that is selected in the Excel template.
...
When the [Range](er-fillable-excel.md#range-component) component is the first component that resides under the **Page** one, and the **Replication direction** property of this range is specified as **No replication**, this range is considered as the page header for pagination based on settings of the current **Page** component. The Excel range that's associated with this format component is repeated at the top of each page that's generated using the settings of the current **Page** component.

> [!NOTE]
> In your Excel template, if you have the configured range [Rows to repeat at top](https://support.microsoft.com/office/repeat-specific-rows-or-columns-on-every-printed-page-0d6dac43-7ee7-4f34-8b08-ffcc8b022409), the address of this Excel range must be equal to the address of the Excel range that's associated with the described above **Range** component for proper pagination. 

When the **Range** component is the last component that resides under the **Page** one, and the **Replication direction** property of this range is specified as **No replication**, this range is considered as the page footer for pagination based on settings of the current **Page** component. The Excel range that's associated with this format component Eis repeated on the bottom of each page that's generated using the settings of the current **Page** component.

> [!NOTE]
> For proper pagination, the Excel ranges that are associated with the **Range** components shouldn't be resized at runtime. Formatting cells of this range using **Wrap text in a cell** and **Autofit row height** Excel [options](https://support.microsoft.com/office/wrap-text-in-a-cell-2a18cff5-ccc1-4bce-95e4-f0d4f3ff4e84) isn't recommended.
...
When the set of nested **Range** components under the **Page** component isn't compliant with the structure described above, a validation [error](er-components-inspections.md#i17) occurs at design time in the ER format designer, informing that it can make problems at runtime.

> [!NOTE]
> To generate proper outputs, don't specify any binding for any **Range** component that resides under the **Page** component, has the **Replication direction** property as set to **No replication**, and is configured to generate page headers or page footers.

For pagination related summing and counting to compute running totals and totals per page, we recommend that you configure the required [Data collection](er-data-collection-data-sources.md) data sources. To learn more how to use the **Page** component to paginate a generated Excel document, complete the procedures in the topic, [Design an ER format to paginate a generated document in Excel format](er-paginate-excel-reports.md).

### <a name="page-component-limitations"></a>Limitations

When you use the **Page** component for Excel pagination, you won't know the final number of pages in a generated document until you complete the pagination. This means you can't calculate the total number of pages by using ER formulas and print out the correct number of pages of a generated document on any page preceding the final one. 

> [!TIP]
> You can do this in an Excel header or footer using the special Excel [formatting](https://docs.microsoft.com/office/vba/excel/concepts/workbooks-and-worksheets/formatting-and-vba-codes-for-headers-and-footers) for headers and footers.

Configured **Page** components aren't taken into consideration when you update an Excel template in the editable format in Microsoft Dynamics 365 Finance version 10.0.22. This functionality is considered for further releases of Finance.

When you configure your Excel template as using [conditional formatting](https://docs.microsoft.com/office/dev/add-ins/excel/excel-add-ins-conditional-formatting), it might not work as expected in some cases.

### Applicability

The **Page** component works only for the [Excel file](er-fillable-excel.md#excel-file-component) format component when this component is configured to use a template in Excel. When you [replace](tasks/er-design-configuration-word-2016-11.md) the Excel template with a Microsoft Word template and run the editable ER format, the **Page** component will be ignored.

The **Page** component works only when the **Enable usage of EPPlus library in Electronic reporting framework** feature is enabled. An exception is thrown at runtime when ER tries to process the **Page** component while this feature is disabled.

> [!NOTE]
> An exception is thrown at runtime when an ER format processes the **Page** component for an Excel template that contains at least one formula that refers to a cell that’s not valid. To prevent runtime errors, fix the Excel template as described in the topic, [How to correct a #REF! error](https://support.microsoft.com/office/how-to-correct-a-ref-error-822c8e46-e610-4d02-bf29-ec4b8c5ff4be).
