...
2. Verify that the configuration provider for sample company, Litware, Inc. is available and marked as active. If you don't see this configuration provider, you must first complete the steps in the procedure, **Create configuration providers and mark them as active**. You are designing a process for parsing incoming bank statements for an application data update. You will receive the incoming bank statements as TXT files that contain IBAN codes. As part of the bank statement import process, you need to validate the correctness of this IBAN codes using the logic that is already available.   
...
2. Select **Repositories** and on the **Localization repositories** page, select **Show filters**.
4. To select the Global repository record, add a filter field, **Name**.
5. In the **Name** field, enter the value **Global** and select the **contains** filter operator.
6. Select **Apply**.
...
8. On the **Configuration repository** page, in the configurations tree, select **Payment model**. 
9. If the **Import** button on the **Versions** FastTab is enabled, select **Import**, and select **Yes**. Otherwise, you have already imported the selected version of the ER configuration **Payment model**.
...
3. Select **Create configuration** to open a new configuration entry dialog box.
...
6. In the **Supports data import** field select **Yes**.
7. Select **Create configuration** to confirm a new configuration entry.
...
3. In the tree, select **Text\Sequence** to add a **Sequence** format component, and in the **Name** field, type **Root**.
4. In the **Special characters** field, select **New line - Windows (CR LF)**. Based on this setting, each line in the parsing file is considered as a separate record. Select **OK**.
4. Select **Add** to open a new component entry dialog box and in the tree, select **Text\Sequence**.
5. In the **Name** field, type **Rows** and in the **Multiplicity** field, select **One many**. Based on this setting, it is expected that at least one line will be presented in the parsing file. Select **OK**.
6. In the tree, select **Root\Rows** and then select **Add Sequence**.
7. In the **Name** field, type **Fields**.
8. In the **Multiplicity** field, select **Exactly one** and then select **OK**.
8. In the tree, select **Root\Rows\Fields**.
9. Select **Add** to open a new component entry dialog and in the tree, select **Text\String**.
10. In the **Name** field, type **IBAN** adn then select **OK**.     
11. Select **Save**.

The configuration is setup so that each line in the parsing file contains the only IBAN code.
...
2. On the **Model to datasource mapping** page, on the Action Pane, select **New**.
    ...
    2. In the **Name** field, type **Mapping to data model** and then select **Save**.
3. Select **Designer**, and on the **Model mapping designer** page, in the **Data source types** tree, select **Dynamics 365 for Operations\Class**.
4. In the **Data sources** section, select **Add root** to add a new data source to call the existing application logic for IBAN codes validation.  
    1. In the **Name** field, type **Check_codes**.
    2. In the **Class** field, type or select **ISO7064** and then select **OK**.
5. In the **Data source types** tree, do the following:
...
6. In the **Data model** tree, do the following:
...
7. Bind components of the configured format to data model fields:
    1. Select **format\Root: Sequence(Root)\Rows: Sequence 1..\* (Rows)**.
    2. Select **Payments**.
    3. Select **Bind**. Based on this setting, each line in the parsing file is considered as a single payment.
    ...
    6. Select **Bind**. Based on this setting, the IBAN field of the data model is filled in by the value from the parsing file.

    ![Model mapping designer page.](../media/design-expressions-app-class-er-02.png)

8. Select the **Validations** tab to add a new [validation](../general-electronic-reporting-formula-designer.md#Validation) rule that displays an error for any line in the parsing file that contains invalid IBAN code.  
    ...
    4. Select **Add data source** and in the **Formula** field, enter the following [expression](../general-electronic-reporting-formula-designer.md#Binding): `Check_codes.verifyMOD1271_36(format.Root.Rows.Fields.IBAN)`.
    5. Select **Save** and then close the page.
    6. Select **Edit message** and on the **Formula designer** page, in the **Formula** field, enter **CONCATENATE("Invalid IBAN code has been found:  ", format.Root.Rows.Fields.IBAN)**.
    7. Select **Save** and then close the page. Based on this setting, the validation condition returns *[FALSE](../er-formula-supported-data-types-primitive.md#boolean)* for any invalid IBAN code by calling the existing method `verifyMOD1271_36` of the application class `ISO7064`. Note that the value of the IBAN code is defined dynamically at run-time as the argument of the calling method based on the content of the parsing TXT file.

    ![Model mapping designer page.](../media/design-expressions-app-class-er-03.png)

9. Select **Save**.
10. Close the **Model mapping designer** page and then close the **Model to datasource mapping** page.
