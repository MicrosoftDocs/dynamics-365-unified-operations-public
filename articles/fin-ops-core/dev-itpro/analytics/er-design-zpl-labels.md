...
Create a new ER configuration that contains a [data model](er-overview-components.md#data-model-component) component for the Warehouse management domain. This data model will later be used as a data source when you design an ER format to generate warehouse location labels.

### Import a new data model configuration

Complete the following steps to import the required data model from the provided XML file.
...
You can view the editable version of the configured model mapping on the **Model mapping designer** page.
...
Complete the following steps to import the required format from the provided XML file.
...
You can view the editable version of the configured format on the **Format designer** page.

![The structure of the ER format on the Format designer page.](./media/er-design-zpl-labels-format.png)

The `model.Location.Label` data source of this format is configured to generate a label containing the following information:

- Warehouse title as a text
- Warehouse title as a barcode
- Location title
- Check digits
...
> [!TIP]
> The label layout is designed to align the location title and the check digits in the center of the label. Because the ZPL II language doesn't support the center alignment for a barcode, the formula of the `model.Location.Warehouse.Alignment` data source is used to calculate the barcode left offset to align this barcode in the center of the label, taking into account the number of characters that the warehouse title contains.

## Prepare your environment for previewing generated labels

The following example uses a printer emulator for rendering ZPL labels to preview on the screen. Complete the following steps to enable this option.
...
Add a printer emulator application for ZPL rendering engine to your Chrome web browser. For example, the [Zpl Printer](https://chrome.google.com/webstore/detail/zpl-printer/phoidlklenidapnijkabnfdgmadlcmjo) emulator. This emulator passes generated labels in ZPL language from a local printer to a web service and returns labels as PDF or PNG files for preview.
...
> [!NOTE]
> Because the printer emulator in this example uses a web service for labels rendering, make sure that your security settings allow you to communicate with this service. Otherwise, your printer emulator will not receive the rendered labels and those labels will not be previewed.
...
2. In the **Find a printer by other options** field, select **Add a local printer or network printer with manual settings**.
3. In the **Choose a printer port** field, select **Create a new port**.
...
3. [Register](install-document-routing-agent.md#register-network-printers) the added local **ZebraPrinter** in DRA.
4. [Activate](install-document-routing-agent.md#administer-network-printers) the local **ZebraPrinter** in your Finance environment.
...
4. On the **File destination** FastTab, select **New**.
    ...
    2. In the **File component name** field, select **Report**.

5. Select **Settings**.
6. In the **Destination settings** dialog box, select the **Printer** tab, set **Enabled**to **Yes**.
...
3. Select **Warehouse location labels**, and on the Action Pane, select **Run**.
    1. On the **Electronic report parameters** dialog, on the **Records to include** tab, select **Filter**.
...
Complete the following steps to learn how you can change the current layout of warehouse location labels. For example, you can change the current layout to add a location profile ID to generated labels.
...
5. Select **Designer**.
    ...
    2. Select the data source, `model.Location.Label`.
...
> [!NOTE]
> You must synchronize the encoding setting of the **Common\File** component of the editable ER format and the appropriate setting of the designed label. The value of the **[Encoding](er-suppress-bom-characters.md)** field of the **Common\File** component shouldn't contradict a ZPL command that's used to control the label's encoding. For example, the `^CI` command. ER doesn't check that these settings are synchronized.
