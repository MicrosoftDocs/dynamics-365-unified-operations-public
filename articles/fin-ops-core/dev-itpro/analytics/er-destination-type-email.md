...
> [!NOTE]
> If no document is generated because the **Enabled** expression for the relevant **File** component returns a **False** Boolean value, no email is sent, even if you configured and enabled an email destination for the component.
...
> [!NOTE]
> If at least one document is generated by a **File** component in a group of components, an email is sent. If no document is generated by grouped components, because the **Enabled** expression for each **File** component returns a **False** Boolean value, no email is sent, even if you configured and enabled an email destination for that group of components.
>
> **Email** is the only destination that you can configure for a group of components. To deliver a document that is emailed based on the email destination setting for a group, add one more destination record, select the component that you want, and then configure another destination for this record.

You can configure multiple groups of components for a single ER format configuration. In this way, you can configure an email destination for every group of components and an email destination for every component.
...
1. Select **Settings**, and then, in the **Destination settings** dialog box, on the **Email** tab, set the **Enabled** option to **Yes**.

:::image type="content" source="./media/ER_Destinations-EnableSingleDestination.png" alt-text="Screenshot of setting the Enabled option to Yes for an email destination.":::

## Configure an email destination

### Email content

Edit the subject and body of the email message.

In the **Subject** field, enter the text for the email subject that appears in the subject field of an electronic message generated at runtime. In the **Body** field, enter the text for the email body that appears in the body field of an electronic message. Set up constant text for the email subject and body, or use ER [formulas](er-formula-language.md) to dynamically create email text at runtime. The configured formula must return a value of the [String](er-formula-supported-data-types-primitive.md#string) type.
...
> [!NOTE]
> Email clients impose layout and style limitations that might require adjustments to the HTML and CSS that you use for the message body. Familiarize yourself with the best practices for creating HTML that the most popular email clients support.
>
...
You can specify the email sender and email recipients. By default, the current user sends the email. To specify a different email sender, configure the **From** field.

> [!NOTE]
> When you configure an email destination, only users with the `ERFormatDestinationSenderEmailConfigure` security privilege, **Configure the sender email address for ER format destinations**, can see the **From** field.
>
> When you offer an email destination for modification at [runtime](electronic-reporting-destinations.md#security-considerations), only users with the `ERFormatDestinationSenderEmailMaintain` security privilege, **Maintain the sender email address for ER format destination**, can see the **From** field.
>
> If you configure the **From** field to use an email address other than the current user's, you must set either the **Send as** or **Send on behalf** permission correctly [set](/microsoft-365/solutions/allow-members-to-send-as-or-send-on-behalf-of-group) in advance. Otherwise, the following exception is thrown at runtime: "Unable to send email as \<from email account\> from the \<current user account\> account, please check the 'Send as' permissions on the \<from email account\>."
...
:::image type="content" source="./media/ER_Destinations-EmailSelectAddressType.png" alt-text="Screenshot of selecting the type of email address.":::
...
Alternatively, you can get email addresses from the contact details of the party that you generate an outbound document for. To use email addresses that aren't fixed, in the **Email source** field, select the [role](../../fin-ops/organization-administration/overview-global-address-book.md#party-roles) of the party for a file destination. The following roles are supported:
...
:::image type="content" source="./media/er_destinations-emaildefineaddresssource.gif" alt-text="Screenshot of configuring an email source account.":::
...
Alternatively, you might have a situation where different parties in the [Global address book](../../fin-ops/organization-administration/overview-global-address-book.md) are registered in different companies ([legal entities](../../fin-ops/organization-administration/organizations-organizational-hierarchies.md#legal-entities)) in such a way that they all use the same account number to fill the configured role. In this case, account numbers for the configured role aren't unique for the whole Finance instance. Therefore, to explicitly select a party, you can't specify only an account number. You must also specify the company that the party is registered in to fill the configured role. Select the **Bind** button (chain symbol) next to the **Company of email source** field in the **Email to** dialog box to open the [Formula designer](general-electronic-reporting-formula-designer.md) page. You can then use this page to configure a formula that returns, at runtime, the code of the company that the desired source must be found in the scope of.
...
- In the **Purpose** field, select the desired purposes. Only email addresses of the selected purposes from contacts of the discovered party are used.
...
> [!NOTE]
> If you select purposes in the **Purpose** field and set the **Primary contact** option to **Yes** at the same time, every email that satisfies at least one configured criterion is used at runtime.
...
:::image type="content" source="./media/ER_Destinations-EmailDefineAddressSource2.png" alt-text="Screenshot of configuring an email address source.":::
...
**Email** is the only previously configured destination that is still available for the selected components. No other previously configured destinations are available, because they're considered unsupported for a group of components. You're notified about these changes as appropriate.

The record that you previously added is considered the header of the group that you created. This header record holds the email destination settings for the group. Other records are group members that use the email destination settings of the group's header record.
...
- If you select a header record, the whole group is ungrouped.
- If you select a member record, and it's the last member record in a group, the whole group is ungrouped.
- If you select a member record that isn't the last member record in a group, that record is excluded from the current group.

The following illustration shows the structure of an ER format that you configured to produce a zipped outbound file that contains a collection letter note and appropriate customer invoices in PDF format.

:::image type="content" source="./media/ER_Destinations-Email-Grouping1.png" alt-text="Screenshot of the structure of an ER format that generates outbound documents.":::
...
:::image type="content" source="./media/ER_Destinations-Email-Grouping2.gif" alt-text="Screenshot of grouping individual components and enabling the Email destination.":::
