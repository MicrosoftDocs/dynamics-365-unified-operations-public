...
> [!NOTE]
> If no document is generated because the **Enabled** expression for the relevant **File** component returns a **False** Boolean value, no email is sent, even if an email destination an email destination is configured and enabled for the component.

You can also [group](#grouping) several **Folder** or **File** components together, and then configure an email destination for all the components in the group. In this case, all outbound documents that components in the group generate **are sent as multiple attachments of a single email**. Based on this destination setting, each generated document is delivered as an attachment of a single email.

> [!NOTE]
> If at least one document is generated by a **File** component in a group of components, an email is sent. If no document is generated by grouped components because the **Enabled** expression for each **File** component returns a **False** Boolean value, no email is sent, even if an email destination is configured and enabled for that group of components.
>
...
:::image type="content" source="./media/ER_Destinations-EnableSingleDestination.png" alt-text="Screenshot of setting the Enabled option to Yes for an email destination." lightbox="./media/ER_Destinations-EnableSingleDestination.png":::
...
You can specify the email sender and email recipients. By default, an email is sent on behalf of the current user. To specify a different email sender, configure the **From** field.
...
:::image type="content" source="./media/ER_Destinations-EmailDefineAddressSource2.png" alt-text="Screenshot of configuring an email address source." lightbox="./media/ER_Destinations-EmailDefineAddressSource2.png":::
...
The record that you previously added is considered the header of the created group. This header record holds the email destination settings for the group. Other records are group members that use the email destination settings of the group's header record.
...
The following illustration shows the structure of an ER format configured to produce a zipped outbound file that contains a collection letter note and appropriate customer invoices in PDF format.

:::image type="content" source="./media/ER_Destinations-Email-Grouping1.png" alt-text="Screenshot of structure of an ER format that generates outbound documents." lightbox="./media/ER_Destinations-Email-Grouping1.png":::
...
:::image type="content" source="./media/ER_Destinations-Email-Grouping2.gif" alt-text="Screenshot of grouping individual components and enabling the Email destination." lightbox="./media/ER_Destinations-Email-Grouping2.gif":::
