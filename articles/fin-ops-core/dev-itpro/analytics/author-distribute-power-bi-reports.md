...
For this example, we'll stage the aggregate models that the Commerce analytical solution consumes in the local Entity Store. The models that the application uses are defined in the RetailCube aggregate measurement. 

1. In the client, open the **Entity Store** page. (Select **System administration** \> **Setup** \> **Entity Store**.) 
1. Select the **RetailCube** aggregate measurement, and then select **Refresh**. 
1. Enter a name for the job that will be run in the background, and then select **OK**.
...
:::image type="content" source="media/Configure-refresh.png" alt-text="Screenshot of Configure refresh dialog box.":::
...
1. On the Power BI **Welcome** page, select **Get data**. 

    Alternatively, when Power BI Desktop starts, you can select **Get Data** \> **SQL Server**. 

    :::image type="content" source="media/Power-BI-Desktop-Get-Data.png" alt-text="Screenshot of Get Data menu in Power BI Desktop.":::

1. In the **SQL Server Database** dialog box, enter.** as the server name and **AxDW** as the database name. Then select the **DirectQuery** option. 

    The following illustration shows the settings that enable Power BI Desktop to access the local Entity Store database.

    :::image type="content" source="media/Connect-to-SQL-Database.png" alt-text="Screenshot of settings for accessing the local Entity Store database.":::
...
1. In the search box, enter **Retail** to filter for entities that are related to the RetailCube aggregate measurement.
1. Select the **RetailCube\_RetailTransDetailsView** table that is shown in the navigator, and then select **Load**. 
...
:::image type="content" source="media/Power-BI-Desktop-Report.png" alt-text="Screenshot of Power BI Desktop report.":::
