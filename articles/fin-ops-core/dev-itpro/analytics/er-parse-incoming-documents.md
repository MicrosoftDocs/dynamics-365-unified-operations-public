...
The following new ER functionality improves the parsing of incoming electronic documents in XML format:

- Use the **CASE** format element as a root element of the ER format to parse incoming electronic documents in XML format. The **FILE** format element is supported as a nested element of the **CASE** element. Therefore, you can configure a single ER format to parse incoming electronic documents that might contain different root XML elements.
- A **Parsing order of nested elements** attribute is available for XML format elements in ER formats. Use this attribute to define a single XML element that you expect in the incoming file. There are two valid sequences of the nested elements:
...
You can design Electronic reporting (ER) formats to parse incoming Microsoft Excel files that represent data in Microsoft Excel workbooks (files in XLSX format). You can then use the content from these files to update application data. This approach is useful if you:

- Design a new model and format and want to test them at run-time. In this case, Excel simulates the actual application data.
...
If you didn't already, to learn how to create the required configurations to import data from an external file, see [ER Create required configurations to import data from an external file](./tasks/er-required-configurations-import-data.md) in the current finance and operations application. To download the file, see the following table.
...
You can design Electronic reporting (ER) formats to parse incoming electronic documents that represent tabular data in plain text (files in CSV format). Then, use the content from these documents to update application data. Use the following approach:

- Begin your format design by adding a new root sequence element to specify that each line in the parsing file is a separate record.

  - In the added sequence element, select the appropriate value, such as **New line - Windows (CR LF)**, in the **Special characters** field in the **Sequence element delimiter** field group.

- Continue your format design by adding a nested sequence element of the added root sequence element to specify that each line in the parsing file is a set of fields.
  ...
  - In the **Quotation application** field, select the value when you expect that some fields of any line that the parser uses this sequence element to parse are enclosed by certain characters. The following options are available:
    ...
    - **All** – Include quotation characters in the parsing line for any field of any data type. If you select this option, define the desired characters in the **Quotation mark** field that the parser uses for fields quotation. For example, the double quotes character.
    - **Text only** – Include quotation characters in the parsing line for any field of the **String** data type. If you select this option, define the desired characters in the **Quotation mark** field that the parser uses for fields quotation.
    - **Derive from parent only** – Use the same **Quotation application** field settings that you define for the parent sequence element. The **Quotation mark** field setting comes from the settings of the parent sequence element as well.
    - **None** – Exclude quotation characters in the parsing line for any field of any data type.

- Complete your format design by adding nested elements for the added sequence element that represents the set of fields of the parsing line. Add the required elements of the **Text** group, such as **String**, **DateTime**, and **Numeric**, to describe the structure of the parsing line as a set of individual fields of different data types.
...
Download the following files to complete the task guide mentioned earlier.
...
Download the following file to complete the task guide mentioned earlier if you didn't watch the task guide, **ER Create required configurations to import data from an external file for electronic reporting**, in the current finance and operations application.
