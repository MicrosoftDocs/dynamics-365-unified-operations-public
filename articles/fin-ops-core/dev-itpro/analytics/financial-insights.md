...
The **Financial analysis** workspace is focused on letting you view and filter the data on existing reports. You can add new visuals to the **Financial analysis** workspace. The **Financial analysis** workspace is available for the current company as well as all companies to show data for all legal entities, regardless of the legal entities that the role has access to.
...
Use the main account type and the main account categories to fill in the appropriate default main accounts on the **Balance sheet** financial statement and the various **Income statement** financial statements in **Financial analysis**.

On the **Main accounts** page, define your main account and assign it to one of the following types:
...
Don't assign any other main account type, such as **Balance sheet** or **Profit and Loss**, to your main accounts. Reporting can't determine the type of main account when you assign other main account types, because they aren't granular enough. The type of main account must be determined to show liabilities and revenue as positive amounts on financial reports.

To appear on the financial statements and to be included in various other visuals, such as KPIs, assign each main account a main account category. The main account categories include a display order. The display order is used specifically on financial statements in **Financial analysis**. After you edit or add a new main account category, you can change the **Display order** value to define the order that the main account categories should be shown in on a financial statement. If you must change the display order for many main account categories, use the Open in Excel feature to quickly edit and publish the changes back to the application.

## Entity store

The data for **Financial analysis** comes from the entity store (**System administration** > **Setup** > **Entity store**). If you open the **CFO overview** or **Financial analysis** workspace and see the following warning message in the visuals, you must update the entities.

:::image type="content" source="./media/Cantdisplay.png" alt-text="Screenshot of a warning message that appears when entities need to be updated.":::

To see data in the **Financial analysis** workspace, update the following entities:
...
You can set up a recurring batch to regularly update the data in the entities. Because each entity is completely rebuilt during an update, select the time and frequency of entity updates carefully. The primary entity used for financial statements is the FinancialReportingTransactionData entity. Therefore, you might decide to update that entity more often.

## Security

Currently, you can't limit the data on embedded Power BI reports to the legal entities that the user has access to. Therefore, control the embedded Power BI reports through duties in the security setup. The duties that you define allow access to data for either all legal entities or only the active company. The following table shows the duties that exist and the roles that they're assigned to. You can remove the duties or assign them to different roles, based on your organization's requirements.
...
The following comparison chart helps differentiate the two options:
...
| **Report on subledger data**                             | Yes. Limited to only vendor, customer.                              | Yes. Vendor, customer, vendor/customer groups, vendor/customer addresses, and more. |
...
In previous releases of **Financial analysis**, you couldn't edit any of the visuals. In future releases, users with the appropriate security can create new visuals, copy existing visuals, and edit visuals. Although the .pbix files that contain the reports are available as resources, don't edit the default reports. The next release includes changes to the data model, default reports, and custom financial statement visual that are used to create the financial statements. To take advantage of new features and changes to the data model in the next release, you must redo any changes that you made to the default reports through Microsoft Power BI Desktop.

## Filtering

Use the **Filter** pane on the left to filter the report. This pane is the same pane that's available through Power BI Desktop. Various levels of filtering exist, and some filtering might not be available depending on what you select on a page (tab) or whether you're using the drill-through capabilities:

- **Report-level filters** – These filters apply to all visuals on all pages (tabs).
- **Page-level filters** – These filters apply to all visuals on the active tab. These filters apply on top of the report-level filters.
- **Visual-level filters** – These filters apply only to the selected visual. These filters apply on top of the page level filters.
...
The **Fiscal calendar** filter is required so that the correct calendar is used for the visual. By default, the report-level filter is set to the active legal entity's fiscal calendar. If you change the filter to a fiscal calendar that has a different start or end date, the beginning balances aren't included. Therefore, your **Balance sheet** financial statements don't show the correct balances. If you select an additional fiscal calendar in the filter, you have an additional set of columns. Each additional set of columns shows the amounts for a different fiscal calendar.
...
Filters are also available for the **Date** and **Fiscal year** fields. Typically, you apply these filters at the page level. By default, the **Date** filter uses a relational date that you can change. You can also remove the relational date filter and use the **Fiscal year** filter instead.

## Currency

All visuals that report on general ledger data show amounts in the accounting currency. Therefore, when you filter on the legal entity, you must be careful to include only legal entities that have the same accounting currency. Otherwise, you aggregate data in different currencies.

All visuals that report on subledger data, such as the **Cash flow forecast** and **Top 10** visuals, show amounts in the system currency. You define the system currency and system exchange rate type on the **System parameters** page.

The **Balance by bank account** visual uses amounts in the bank accounts' currency.

## Dimensions

The default financial statements don't include any financial dimensions but focus only on the main account. Support for financial dimensions will be available in future releases, when the reports become editable. Organizations can then filter on financial dimension values.
...
> [!IMPORTANT]
> If you summarize transactions for multiple vendors or customers in a single voucher by using the financial journals, the data is incorrect. The reporting process can't determine which vendor or customer is related to a specific ledger account in a journal entry, because that information isn't maintained anywhere. Therefore, don't enter multiple vendors, customers, fixed assets, or projects in a single voucher.

## Drill on data

Power BI provides various levels of drilling, each with a different name and functionality. You can also drill on rows and columns. This section discusses the various options by using the **Trial balance** financial statement as an example and showing how you can drill on the rows. The same functionality exists for columns. You just have to change the **Drill on** setting.
...
:::image type="content" source="./media/trial-balance.png" alt-text="Screenshot of the Trial balance statement collapsed to the highest level of the row hierarchy, the main account type.":::

To view the next level of the hierarchy, the main account categories, set the **Drill on** field to **Rows** and then select the **Expand** button (the third button after the **Drill on** field). You now see all the main account categories expanded. Currently, Power BI doesn't let you expand only one row or column but still see all the other rows or columns.

:::image type="content" source="./media/trial-balance2.png" alt-text="Screenshot of the Trial balance statement showing main account categories expanded after drilling down on rows.":::

To expand to the main accounts for all rows, use the **Expand** button. However, to drill down to the main accounts for only one row, first select the **Drill down** button (the single downward-pointing arrow on the right side of the window), and then select the row to drill down on. The following illustration shows the result when the **Sales** row is selected after the **Drill down** button is selected.

:::image type="content" source="./media/trial-balance3.png" alt-text="Screenshot of the Trial balance statement showing the result when the Sales row is selected after the Drill down button is selected.":::
...
:::image type="content" source="./media/trial-balance4.png" alt-text="Screenshot of the Trial balance statement showing the Drill up button drilling up in the context of the Sales category.":::
...
:::image type="content" source="./media/trial-balance5.png" alt-text="Screenshot of the Trial balance statement showing the next level in the hierarchy without the parent level displayed.":::

To see the transaction details behind the summarized balances, select some amounts to drill back into Financial and Operations.
...
If you don't expand far enough on a row, the drill-down doesn't work. For example, if you expand only to the main account category, you can't drill down into the ASE on the balance, because the main account is a required field for filtering in the ASE.

If you expand too far on a row, the additional filters on the financial statements aren't sent to the ASE. Therefore, you might see a difference in your numbers. For example, if you expand to the country or region on the rows of the Income statement by region financial statement, the country or region isn't included as a filter in the ASE.
...
The default financial statements use two hierarchies to drill and expand on the data. One hierarchy is for the rows, and the other hierarchy is for the columns. Both hierarchies are predefined in the design of the financial statement. For most financial statements, the row hierarchy is **Main account type** \> **Main account categories** \> **Main account**. However, some reports include extra fields, such as Country and Region. The extra nodes in the hierarchy come from subledger data for each transaction.

For the columns, the hierarchy focuses on the legal entities and the fiscal periods. For most financial statements, the column hierarchy is **Legal entity** \> **Fiscal calendar** \> **Fiscal year** \> **Quarter** \> **Period**.

Currently, the financial statements don't support the organizational hierarchies, which you use to aggregate data.

## Data limitations

The financial statement visuals have a limit on the number of rows that they can show. Currently, the limit is set to 30,000. If you exceed this limit, the visual shows a warning symbol to notify you about this situation.

:::image type="content" source="./media/data-limit.png" alt-text="Screenshot of a financial statement visual displaying a warning symbol indicating the data row limit has been exceeded.":::

If you exceed the maximum, the totals that appear on the financial statement are incorrect, because the visual doesn't load all the rows.

### Empty rows

Power BI doesn't provide an option to hide and show empty rows. If a row doesn't have any data, the row doesn't appear in the visual.

## Additional resources for Power BI

To enable the embedded reports for the **Financial analysis** workspace in a production environment, you don't need the information in the following resources. Instead, they're helpful for dev boxes and if you want to embed your own Power BI reports.
