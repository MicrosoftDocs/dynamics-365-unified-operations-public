...
This topic describes how to manage the lifecycle of [Electronic reporting](general-electronic-reporting.md) (ER) [configurations](general-electronic-reporting.md#Configuration) for Dynamics 365 Finance.
...
When you finish design of this format, you can change the status of your initial [version](general-electronic-reporting.md#component-versioning) of this ER format configuration from **Draft** to **Completed**. Then you can share the completed version of this ER format configuration [publishing](../../../finance/localizations/rcs-global-repo-upload.md) it to the Global repository. Then, you can access the Global repository from any RCS or Finance cloud instance to import from this repository to this application any ER configuration version that is applicable for this application.
...
Based on the configurations dependency, when you selected this ER format configuration in the Global repository to import it to a newly deployed RCS or Finance instance, the base ER data model configuration will be automatically found in the Global repository and imported along with the selected ER format configuration as the base configuration.

You can also export your ER format configuration version  out of your current RCS or Finance instance to store it locally as an XML file.

![Export the ER format configuration version on the Configuration page.](./media/ger-configuration-lifecycle-img3.png)

In versions of Finance **before version 10.0.29**, when you try to import this ER format configuration version from that XML file or from any repository other than the Global repository to a newly deployed RCS or Finance instance that contains no ER configurations yet, the following exception will be thrown informing you that a base configuration cannot be obtained:
...
In version **10.0.29 and later**, when you try to perform the same configuration import, whenever a base configuration cannot be found neither in the current application instance nor in the currently using source repository (if applicable), the ER framework will try to automatically find the name of the missed base configuration in the Global repository cache and present it in the text of the thrown exception alogn with the globally unique identifier of the missed base configuration.
...
So, you can use the provided name of the missed base configuration to find it and import it manually.

> [!Note]
> Be aware that this new option works only when at least one user has already signed-in to the Global repository either by using the [Configuration repositories](er-download-configurations-global-repo.md#open-configurations-repository) page or by using one of the Global repository [lookup](er-extended-format-lookup.md) fields in the current Finance instance and the Global repository content has been cached.
