...
description: Learn about how to defer the execution of an XML element in an Electronic reporting (ER) format, including limitations and prerequisites.
author: kfend
ms.author: filatovm
ms.topic: how-to
ms.date: 04/08/2026
...
Use the Operations designer of the [Electronic reporting (ER)](general-electronic-reporting.md) framework to [configure](./tasks/er-format-configuration-2016-11.md) the format component of an ER solution that generates outbound documents in XML format. The hierarchical structure of the configured format component consists of format elements of various types. These format elements fill generated documents with the required information at runtime. By default, when you run an ER format, the format elements run in the same order as they appear in the format hierarchy: one by one, from top to bottom. However, at design time, you can change the execution order for any XML elements of the configured format component.

When you turn on the <a name="DeferredXmlElementExecution"></a>**Deferred execution** option for an XML element in the configured format, you defer (postpone) the execution of that element. In this case, the element isn't run until all other elements of its parent run.

To learn more about this feature, complete the example in this article.

## Limitations

The **Deferred execution** option supports only XML elements that are configured for an ER format that generates **outbound** documents in XML format.

The **Deferred execution** option supports only XML elements that reside in one other XML element. Therefore, it doesn't apply to XML elements that reside in other types of format elements (for example, in an **XML sequence** element).
...
If you didn't yet complete the example in the [Defer the execution of sequence elements in ER formats](er-defer-sequence-element.md#Example) article, download the following [configurations](general-electronic-reporting.md#Configuration) of the sample ER solution.
...
Before you begin, also download and save the following configuration of the sample ER solution to your local computer.
...
1. Import the ER format configuration:
    ...
    1. Select **Exchange**, and then select **Load from XML file**.
    1. Select **Browse**, find and select the **Format to learn deferred XML elements.1.1.xml** file, and then select **OK**.
...
1. Review the list of imported ER configurations in the configuration tree.
    ...
    :::image type="content" source="./media/ER-DeferredXml-Configurations.png" alt-text="Screenshot of imported ER configurations on the Configurations page.":::
...
1. On **Localization configurations**, in the **Configuration providers** section, make sure that the [configuration provider](general-electronic-reporting.md#Provider) for the Litware, Inc. (`http://www.litware.com`) sample company is listed and marked as active. If this configuration provider isn't listed, or if it isn't marked as active, follow the steps in the [Create a configuration provider and mark it as active](./tasks/er-configuration-provider-mark-it-active-2016-11.md) article.
    ...
    :::image type="content" source="./media/ER-DeferredXml-ElectronicReportingWorkspace.png" alt-text="Screenshot of Litware, Inc. sample company on the Localization configurations page.":::
...
1. Review the data sources that are configured to access tax transactions:
    ...
    - The **TotalSum** aggregation field of the **Grouped** data source is configured to summarize values of the **\$TaxAmount** field of the **Filtered** data source for all filtered tax transactions of that data source.
        ...
        :::image type="content" source="./media/ER-DeferredXml-GroupByParameters.png" alt-text="Screenshot of TotalSum aggregation field on the Edit 'GroupBy' parameters page.":::
...
1. Review how the configured data sources are bound to the data model, and how they expose accessed data to make it available in an ER format:
    ...
    :::image type="content" source="./media/ER-DeferredXml-ModelMapping.png" alt-text="Screenshot of Model mapping designer page.":::

1. Close the **Model mapping designer** and **Model mappings** pages.

### Review the imported format

1. On **Configurations**, in the configuration tree, select the **Format to learn deferred XML elements** configuration.
1. Select **Designer** to review the format details.
1. Select **Show details**.
1. Review the settings of the ER format components that are configured to generate an outbound document in XML format that includes details of the tax transactions:
    ...
    :::image type="content" source="./media/ER-DeferredXml-Format.png" alt-text="Screenshot of Message XML element and nested XML elements on the Format designer page.":::

1. On the **Mapping** tab, review the following details:

    - The **Report\\Message\\Header** element doesn't need to be bound to a source to generate a single node in an outbound document.
    ...
    - The **Report\\Message\\Summary** element doesn't need to be bound to a data source to generate a single node in an outbound document.
    ...
    :::image type="content" source="./media/ER-DeferredXml-Format2.png" alt-text="Screenshot of Mapping tab on the Format designer page.":::
...
1. Download the file that the web browser offers, and open it for review.
    ...
    :::image type="content" source="./media/ER-DeferredXml-Run.png" alt-text="Screenshot of downloaded file of imported format.":::

The summary node presents the sum of the tax values for the processed transactions. Because the format uses the **model.Data.Summary.Total** binding to return this sum, the sum is calculated by calling the **TotalSum** aggregation of the **Grouped** data source of the *GroupBy* type in the model mapping. To calculate this aggregation, the model mapping iterates over all transactions that you select in the **Filtered** data source. By comparing the execution times of the summary node and the last record node, you can determine that calculation of the sum took 12 milliseconds (ms). By comparing the execution times of the first and last record nodes, you can determine that generation of all record nodes took 9 ms. Therefore, a total of 21 ms was required.

### Modify the format so that the calculation is based on generated output

If the transaction volume is much larger than the volume in the current example, the calculation time might increase and cause performance problems. By changing the setting of the format, you can help prevent these performance problems. Because you access tax values to include them in the generated report, you can reuse this information to calculate tax values. For more information, see [Configure format to do counting and summing](./tasks/er-format-counting-summing-1.md).

1. On the **Format designer** page, on the **Format** tab, select the **Report** file element in the format tree.
1. Set the **Collect output details** option to **Yes**. You can now configure this format by using the content of a generated report as a data source that the built-in ER functions in the [Data collection](er-functions-category-data-collection.md) category can access.
1. On the **Mapping** tab, select the **Report\\Message\\Record** XML element.
...
1. Configure the **Collected data key value** expression as `WsRow`.
    ...
    :::image type="content" source="./media/ER-DeferredXml-Format3.png" alt-text="Screenshot of Record XML element on the Format designer page.":::

1. Select the **Report\\Message\\Record\\TaxAmount** attribute.
1. Configure the **Collected data key name** expression as `SummingAmountKey`.

    :::image type="content" source="./media/ER-DeferredXml-Format4.png" alt-text="Screenshot of TaxAmount attribute on the Format designer page.":::
...
1. Select the **Report\\Message\\Record\\RunningTotal** attribute, and then select **Edit formula**.
1. Configure the `SUMIF(SummingAmountKey, WsColumn, WsRow)` expression by using the built-in [SUMIF](er-functions-datacollection-sumif.md) ER function, and then select **Save**.

    :::image type="content" source="./media/ER-DeferredXml-FormulaDesigner.png" alt-text="Screenshot of SUMIF expression.":::
...
1. Download and review the file that the web browser offers.
    ...
    :::image type="content" source="./media/ER-DeferredXml-Run1.png" alt-text="Screenshot of generated list of tax value with running total.":::

    The last record node contains the running total of tax values that is calculated for all processed transactions by using the generated output as a data source. This data source starts from the beginning of the report and continues through the last tax transaction. The summary node contains the sum of the tax values for all processed transactions that are calculated in the model mapping by using the data source of the *GroupBy* type. Notice that these values are equal. Therefore, you can use output-based summing instead of **GroupBy**. By comparing the execution times of the first record node and the summary node, you can determine that generation of all the record nodes and summing took 11 ms. Therefore, as far as the generation of record nodes and the summing of tax values are concerned, the modified format is approximately two times faster than the original format.

1. Select the **Report\\Message\\Summary\\TotalTaxAmount** attribute, and then select **Edit formula**.
1. Enter the `SUMIF(SummingAmountKey, WsColumn, WsRow)` expression instead of the existing expression.
1. Select **Save**, and then select **Run**.
1. Download and review the file that the web browser offers.

    :::image type="content" source="./media/ER-DeferredXml-Run2.png" alt-text="Screenshot of generated list of tax values using edited formula.":::
...
1. Download and review the file that the web browser offers.
    ...
    :::image type="content" source="./media/ER-DeferredXml-Run3.png" alt-text="Screenshot of downloaded file of tax values for report header.":::

    The sum of tax values in the summary node now equals 0 (zero), because this sum is now calculated based on the generated output. When the first record node is generated, the generated output doesn't yet contain record nodes that have transaction details. You can configure this format to defer the execution of the **Report\\Message\\Summary** element until the **Report\\Message\\Record** element runs for all tax transactions.
...
1. Set the **Deferred execution** option to **Yes**.
    ...
    :::image type="content" source="./media/ER-DeferredXml-Format5.png" alt-text="Screenshot of Deferred execution option of the Summary XML element on the Format designer page.":::

1. Select **Save**, and then select **Run**.
1. Download and review the file that the web browser offers.

    :::image type="content" source="./media/ER-DeferredXml-Run4.png" alt-text="Screenshot of downloaded file of deferred execution.":::

    The **Report\\Message\\Summary** element now runs only after all other items that are nested under its parent element, **Report\\Message**, run. Therefore, it runs after the **Report\\Message\\Record** element runs for all tax transactions of the **model.Data.List** data source. The execution times of the first and last record nodes, and of the header and summary nodes, reveal this fact.
