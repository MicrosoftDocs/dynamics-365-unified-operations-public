...
The selected document type is company-specific and is used when working with Business document management in the company for which you configured the selected document type. When working with Business document management in another company, use the same selected document type if it isn't configured for that company. When you configure a document type, it replaces the one selected in the **SharePoint document type** field.
...
1. Select **New document**, and in the **Title** field, change the title of the editable template if needed. The text names the ER format configuration that the system automatically creates. The draft version of this configuration (**Customer FTI report (GER) Copy**) contains the edited template and is automatically marked to run this ER format for the current user. At the same time, the non-modified original template from the base ER format configuration runs this ER format for any other user.
1. In the **Name** field, change the name of the first revision of the editable template that the system creates automatically.
...
The **New document** option is always available for a template in an ER format configuration provided by current and another provider (Microsoft in this example) that doesn't have any revision. The edited template is stored in a new ER format configuration that the system automatically generates.
...
> [!NOTE]
> This function is available only when you properly configure the **SharePoint document type** parameter. For more information, see [Configure parameters](#configure-parameters).
...
1. Select **Show format** to view the current structure of the format from the base ER format configuration that you must align with the editable template.
...
When you edit the template from an ER format that the current active provider owns, you get the option to revoke the changes you made to the template.
...
You might have signed in to the current instance of the app associated with the Microsoft Entra domain that differs from the Microsoft Entra domain used to deploy this instance. Because the SharePoint service stores templates for making them available for editing by using the Office desktop applications, and it belongs to the same domain, you don't have permissions to access the SharePoint service. To resolve this issue, sign in to the current instance by using the credentials of a user with the correct Microsoft Entra domain.
