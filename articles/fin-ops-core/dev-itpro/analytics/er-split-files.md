...
- You can mark any XML ELEMENT format element as non-breakable. In this way, you keep the nested items of XML nodes that are generated under the format element in a single generated file.
...
If you configure file destinations for a FILE format element that you configure to split the generated output whenever specific limits are exceeded, each piece of generated output is sent to the configured file destination as an individual file. To uniquely name the files that are created by splitting the output, you must configure an ER expression for the FILE format element. If you include an ER data source of the NUMBER SEQUENCE type, the number sequence increments for each piece of the split output.
