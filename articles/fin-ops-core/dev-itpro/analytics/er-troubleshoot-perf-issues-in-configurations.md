...
1. Based on the results of the analysis, use ER configurations to [make changes](#making-changes), or decide to collect more data.

## Troubleshooting

### Analyze execution time

Execution time can depend on unpredictable factors, such as other tasks that run in the same environment and caching that uses data when it's called for the first time. Therefore, you should repeat the execution and measurement several times.

Sometimes, performance issues aren't caused by an ER format configuration that you use for reporting. Instead, X++ code that you use to open that ER format configuration causes the issues.

1. In either the [Action center](#infolog-time) or the [event log](#event-log-time), look at the execution time of the report.
1. Compare the execution time of the report with the total execution time in the scenario.
1. If the execution time of the report is much less than the total execution time, consider the amount of data that the report processes:
...
1. Run multiple tests that involve different amounts of data to determine how the execution time depends on the amount of data.
...
Then, in [Trace parser](#trace-parser), perform a standard bottom-up analysis and answer the following questions:

- What methods consume the most time?
...
Analyze the number of calls. If the number is higher than expected, consider caching the corresponding nodes of the configuration.
...
1. Include only stacks that use **runUnattended**, to filter only the thread that has configuration execution. (Add **runUnattended** to the **IncPats** input box.)
1. Fold all CPU, network, and blocked time.
1. Load [ER presets for PerfView](https://download.microsoft.com/download/2/d/0/2d037b0f-ffd1-4d65-b64f-fcdf51f2c81f/ER_PerfViewPresets.xml).
1. Select **ER** \> **Other preset**.
1. Look at the names:

    - You probably see the platform code that consumes the most time.
    - You can double-tap (or double-click) and go up through **callees**.

        If you find classes that have the prefix "ERExpression," and if they're functions related to formulas, you can guess the function name based on the class name, and you can look at the ER repo to view the attributes.

### Fixes

- If queries consume most of the CPU time, try to reduce the number of queries:

    - [Review the ER trace](#analyze-database-calls) for duplicated queries.
    - Check how many records are fetched, and evaluate how much data should theoretically be fetched.

- If functions consume most of the CPU time, try to find the place in the configuration that consumes the most resources.
- If data collection functions consume most of the CPU time, consider replacing them with *SQL group by* on the model mapping side.

### <a name="collecting-data"></a>Collecting data

Depending on your environment, you can collect available data in several ways:
...
Sometimes, you can reproduce issues only in a production environment. Collect data in the following ways:
...
In addition to the tools that you can use in a production environment, use several tools in a development environment:
...
1. In the **User parameters** dialog box, set the **Show file generation time** option to **Yes**.

#### <a name="event-log-time"></a>Execution time in the event log

1. Open Windows Event Viewer.
1. Under **Applications and Services logs**, open **Microsoft-Dynamics-ElectronicReporting/Operational**.
1. Look for **FormatMapingRun** events where **EventID=2**, because these events contain the information about elapsed time.
...
There are a few limitations to this approach. Because part of ER is implemented in C#, not all the details are available. However, you can view the data usage details. Additionally, long report runs can exceed trace storage limitations.
...
There are a few limitations to this approach. You must have administrative access to the machine. Additionally, you must be an experienced developer, because there's a steep learning curve.

### <a name="making-changes"></a>Making changes

#### <a name="use-caching"></a>Use caching

Although caching reduces the amount of time that it takes to fetch data again, it costs memory. Use caching in cases where the amount of fetched data isn't large. For more information and an example that shows how to use caching, see [Improve the model mapping based on information from the execution trace](trace-execution-er-troubleshoot-perf.md#improve-the-model-mapping-based-on-information-from-the-execution-trace).

#### <a name="reduce-fetched-data"></a>Reduce volume of data fetched

You can reduce memory consumption for caching by limiting the number of fields in the records of an application table that you fetch at runtime. In this case, you fetch only those field values of an application table that you need in your ER model mapping. You don't fetch other fields in that table. Therefore, the volume of memory that is required to cache fetched records is reduced. For more information, see [Improve performance of ER solutions by reducing the number of table fields that are fetched at runtime](er-reduce-fetched-fields-number.md).

#### <a name="cached-parameterized"></a>Use a cached, parameterized calculated field

Sometimes, you need to look up values repeatedly. Examples include account names and account numbers. To help save time, you can create a calculated field that has parameters on the top level, and then add the field to the cache.

Use this approach only when the size of the cached data is small. For more information, see [Improve the performance of ER solutions by adding parameterized CALCULATED FIELD data sources](er-calculated-field-ds-performance.md).

#### <a name="use-join-datasource"></a>Use a JOIN data source

A **JOIN** data source enables one query to fetch several connected records. You don't need a separate query to fetch each connected record. For example, if you have 1,000 lines, and you fetch item data for each line by relation, you have 1,001 queries (= 1,000 + 1). If you use a **JOIN** data source, you use only one query to fetch the same data. For more information, see [Use JOIN data sources in ER model mappings to get data from multiple application tables](er-join-data-sources.md).

#### <a name="filter"></a>Use the FILTER function instead of the WHERE function

The **[FILTER](er-functions-list-filter.md)** function runs conditions on SQL Server, whereas the **WHERE** function fetches all data from the list, one record at a time, and applies the condition for each record. For example, you want to select one record out of 1,000 records. If you use **WHERE**, you fetch all 1,000 records. However, if you use **FILTER**, you fetch exactly one record. **FILTER** can also use indexes on the database side.

#### <a name="collected-data"></a>Use collected data functions or an accumulated data source

If your configuration has a *group by* component that summarizes previously fetched data by report, the component fetches all the data again. By using collected data functions, you enable ER to accumulate data during the first fetch. For more information, see [ER Configure format to do counting and summing](tasks/er-format-counting-summing-2.md).
