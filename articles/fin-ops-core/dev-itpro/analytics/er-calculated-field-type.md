...
This article explains how to design an Electronic reporting (ER) data source by using the **Calculated field** type. This data source can contain an ER expression that, when executed, is controlled by the values of the parameter arguments that you configure in a binding that calls this data source. By configuring parameterized calls of such a data source, you can reuse a single data source in many bindings, which reduces the total number of data sources that you must configure in ER model mappings or ER formats. It also simplifies the configured ER component, which reduces the maintenance costs and the cost of use by other consumers.
...
1. Select **Designer**.  
    ...
    - Fetch the list of tax transactions (**Trans** data source) residing in the **TaxTrans** table:
        ...
        - Calculate the following aggregated values for grouped transactions per tax code:
    ...
    :::image type="content" source="media/er-calculated-field-type-01.png" alt-text="Screenshot of the model mapping designer page showing Tax and Gr data sources.":::
...
1. Select **Designer**. This ER format is designed to do the following tasks:
    ...
    - Present multiple details at each taxation level, with a different number of details in each level.

    :::image type="content" source="media/er-calculated-field-type-02.png" alt-text="Screenshot of the Format designer page.":::

1. Select **Mapping**.
1. Expand the **Model**, **Data,** and **Summary** items.

    The calculated field **Model.Data.Summary.Level** contains the expression that returns the code of the taxation level (**Regular**, **Reduced**, **None,** or **Other**) as a text value for any tax code that you can retrieve from the **Model.Data.Summary** data source at run time.

    :::image type="content" source="media/er-calculated-field-type-03.png" alt-text="Screenshot of the Format designer page showing details of the Data model Model to learn parameterized calls.":::
...
1. Expand the **Model**.**Data2.Summary2** item.
    ...
    :::image type="content" source="media/er-calculated-field-type-04.png" alt-text="Screenshot of the Format designer page showing details of the Model.Data2.Summary2 data source.":::

1. Review the calculated fields **Model**.**Data2.Level1**, **Model**.**Data2.Level2**, and **Model**.**Data2.Level3.** These calculated fields filter the **Model**.**Data2.Summary2** records list and return only records that represent a particular taxation level.
1. Close the **Format designer** page.

## Create a derived format

You can improve the provided format by adding one calculated field to filter the required taxation level instead of using the existing three fields: **Model**.**Data2.Level1**, **Model**.**Data2.Level2**, and **Model**.**Data2.Level3**. You specify the required taxation level in the location where you call this new calculated field.
...
1. In the **Type** field, select **String**.
    ...
    Use only primitive data types to specify the type of the parameter’s argument. Therefore, you can't use **Record list**, **Record**, or **Enum** types for this purpose.

    You can specify up to eight parameters for a single calculated field.

    :::image type="content" source="media/er-calculated-field-type-05.png" alt-text="Screenshot of the parameter data source list.":::

1. Select **OK**.

When you add this parameter, you specify the condition that must be in place to call this calculated field. When you call this calculated field, you need to specify the argument of the **Taxation Level** parameter as a value with **String** format.

   Define parameters only for those calculated fields that reside in a container (either **Record list**, **Record**, or **Container**).

   The configured parameter appears in the list of data sources for this calculated field. Select **Add data source** to add the parameter to the configured expression.

   :::image type="content" source="media/er-calculated-field-type-06.png" alt-text="Screenshot of the data source fields.":::
...
1. Select **Save**.
    ...
    :::image type="content" source="media/er-calculated-field-type-07.png" alt-text="Screenshot of the data source field information.":::
...
:::image type="content" source="media/er-calculated-field-type-08.png" alt-text="Screenshot of the expanded list of calculated field levels.":::
...
1. Select **Yes** to confirm the replacement of the currently used data source, **Level1**, by the new data source, **Levels**, in all nested format elements of the selected format element.
    ...
    The applied binding is built as a call to the parameterized calculated field. By default, the name of the bound format element is used as an argument for the parameterized calculated field under the following conditions:

      - The calculated field is configured to use a single parameter.
      - The data type of this parameter is defined as **String**.

    When the name of the bound format element is blank, the data source name of this element is used in the applied binding.
...
1. Select **Yes** to confirm the replacement of the currently used data source, **Level3**, by the new data source, **Levels**, in all nested format elements under the selected format element.
   ...
   When you specify the argument of the parameterized calculated field for the XML element representing taxation level (for example, **Model.Data2.Levels("Reduced")** as a text value), you don't need to do the same for nested XML attributes - their bindings automatically inherit the value of the argument defined on the parent level (**Model.Data2.Levels.aggregated.Base**, not **Model.Data2.Levels("Reduced").aggregated.Base**).

Recurrent calls to any parameterized calculated field aren't supported.
...
:::image type="content" source="media/er-calculated-field-type-10.png" alt-text="Screenshot of the validation warning notification.":::

## Configure a parameterized calculated field to return a record

When a parameterized calculated field returns a record, you need to support binding of individual fields of this record to format elements. In such cases, there's no parent binding that contains the value of an argument to call a parameterized calculated field. You must define this value in the binding of a single record’s field.
...
1. Select **Add data source**.
1. In the **Formula** field, append **'Taxation Level'))** to the expression you entered in Step 1 to finalize the expression as:  
...
:::image type="content" source="media/er-calculated-field-type-11.png" alt-text="Screenshot of the updated expression.":::

## Remove calculated fields that you don't use
...
1. In the **Versions** FastTab, select the completed version 1.1.1.
1. Select **Exchange**.
...
Run the initial and improved ER formats to make sure that configured parameterized calculated fields work properly.
...
1. Select **Reporting configurations**.
1. Import the configurations from Microsoft Download Center in the following sequence: data model, model mapping, format. Complete the following steps for each ER configuration:

    1. Select **Exchange**.
    1. Select **Load from XML file**.
    1. Select **Browse** to select the required ER configuration in XML format.
    1. Select **OK**.
...
1. Import the exported from RCS completed version 1.1.1 of the **Format to learn parameterized calls (custom)** format:
    ...
    1. Select **Exchange**.
    1. Select **Load from XML file**.
    1. Select **Browse** to select the locally stored **Format to learn parameterized calls (custom)** file in XML format.
    1. Select **OK**.
...
1. In the configuration tree, expand the content of the **Model to learn parameterized calls** item.
1. Select **Format to learn parameterized calls**.
1. Select **Run** on the top-most ribbon.
1. Save the locally generated output.
1. Select the **Format to learn parameterized calls (custom)** item.
1. Select **Run** on the top-most ribbon.
1. Save the generated output locally.
1. Compare the contents of the generated outputs.
