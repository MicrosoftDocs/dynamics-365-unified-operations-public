...
Jobs that are executed in batch can also be used to export data across companies. Prior to platform update 40 for version 10.0.16it was possible to determine if cross-company jobs should be executed in parallel or sequential. The sequential could be enabled in data management framework parameters. After the feature is enabled, a new option will appear when adding an entity to a data project to enable that the job will export data from each of company in a sequential manner. Starting with platform update 40 for version 10.0.16, all cross-company execution will be sequential by default, and the parameter has been removed. The change was made due to performance and other challenges, which made the parallel execution less reliable.
