...
A reference to a data source of the *String* type that you want to format. This argument is required.

`argument 1`: *String*

The first argument, which replaces occurrences of **%1**. This argument is required.

`argument N`: *String*

The *N*th argument, which replaces occurrences of **%2**, **%3**, and so on. These additional arguments are optional.
...
If you don't provide an argument for a parameter, the string shows the parameter as **"%N"**. For *Real* type values, the default string conversion rounds the number to two decimal places.
...
In the Electronic reporting (ER) format that is designed to generate an electronic file for selected customers, select **PaymentModel** as a data source. It controls the process flow. If a selected customer is stopped for the date when the report is processed, an exception is thrown to notify the user. The formula that is designed for this type of processing control can use the following resources:
...
Here's the expression that you can design.
