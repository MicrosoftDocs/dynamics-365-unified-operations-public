...
- You must be an administrator for your Microsoft Entra account. If you aren't the administrator, an administrative user must perform this configuration step for you.
...
1. Select **Register**.
1. Copy the **Application (client) ID** value. You use this value to connect to the PowerBI.com service.

    > [!IMPORTANT] 
    > Be sure to capture the **Application (client) ID** value.

1. Select **Manage** \> **API permissions** \> **Add a permission** \> **Microsoft APIs** \> **Power BI Service**.
...
1. Select **Select Delegated permissions**.
...
1. Select **Add permissions**. You must grant admin consent for the permissions to allow reports to be published to Power BI.
...
1. In the finance and operations client, open the **PowerBI.com configuration** page.
    ...
    :::image type="content" source="./media/D365-PBI-Configuration.png" alt-text="Screenshot of PowerBI.com configuration page.":::
...
To verify the changes and enable PowerBI.com integrations, complete the steps in the next section.
...
1. To validate the PowerBI.com configuration, select **Get started**.
    ...
    > [!NOTE]
    > You might need to refresh the browser to apply the changes.
    >
