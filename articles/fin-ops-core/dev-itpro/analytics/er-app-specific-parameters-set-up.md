...
If you configure application-specific parameters for one version of an ER format and then import a higher version of the same format into the current Finance instance, the existing application-specific parameters won't be applied for the imported version unless you use the feature, **Use application specific parameters from previous versions of ER formats**. For more information, see the section, **Re-use existing parameters** in this topic.

When you select a file for import, the structure of the application-specific parameters in that file is compared with the structure of the corresponding data sources of the **Lookup** type in the ER format that is selected for import. By default, the import is only completed when the structure of each application-specific parameter matches the structure of the corresponding data source in the ER format that is selected for import. If the structures don't match, a warning message states that the import can't be completed. If you force the import, the existing application-specific parameters for the selected ER format will be cleaned up, and you must set them up from the beginning. Starting in Dynamics 365 Finance version 10.0.23, you can change this default behavior. You can enable the **Align ER application specific parameters while importing** feature in the **Feature management** workspace to avoid such warning messages. When this feature is enabled, and you import application-specific parameters with a structure that is different from the structure of the corresponding data sources in the target ER format that is selected for import, the import will succeed in the following cases when compared with the structure of the imported file:

- The structure of the target ER format has been changed by adding new condition columns to any of existing  **Lookup** type data sources. When the import is complete, the application-specific parameters are updated and the values of every added condition column in all imported records of application-specific parameters are initialized by the default value for the [data type](er-formula-supported-data-types-primitive.md) of this column.
- The structure of the target ER format has been changed by removing some condition columns from any of existing **Lookup** type data sources. When the import is complete, the application-specific parameters are updated and values of every removed condition column in all imported records of application-specific parameters are deleted.
- The structure of the target ER format has been changed by adding new **Lookup** type data sources. When the import is complete, the application-specific parameters are appended by the added lookups.
- The structure of the target ER format is changed by removing some of the existing **Lookup** type data sources. When the import is complete, all artifacts that are related to the data sources of the **Lookup** type that were removed from the target ER format are deleted from the imported application-specific parameters.

When the import is completed, in addition to the modifications described above, the state of the imported application-specific parameters is changed to **In progress**. A warning occurs informing you that the automatically adjusted application-specific parameters must be manually edited.
