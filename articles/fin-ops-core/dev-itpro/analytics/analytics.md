...
The term embedded business intelligence (BI) refers to experiences that use highly intuitive and fluid visualizations to provide insights that are relevant to a task, so that the user is more informed and can make better choices. For example, a rental clerk in a car rental organization views the previous rental preferences of a customer when the customer makes a reservation. In this case, the clerk can see the vehicles and colors that the customer selected in the past, and therefore can offer options that are likely to please the customer. Use embedded BI throughout the user interface. At a technical level, building rich visualizations requires a powerful charting framework, and also an efficient way to access aggregated data that enables the display of fluid visualizations. Microsoft Dynamics 365 finance and operations meets both of these requirements, so that application developers can build rich and deep embedded BI–enabled scenarios.

## Where are the perspectives?

Perspectives were designed to present data from a reporting point of view. Perspectives have evolved over the past three releases for analytical scenarios.
...
| Finance and operations apps | Perspectives are a first-class citizen in the data access framework. You can consume them via X++ or C\# code, and also in a model-driven way. |

Perspectives reside within the analytics collection in the Application Explorer. Perspectives underwent a major upgrade and now incorporate the following improvements:
...
- You can model data entities by referencing aggregate models. You can expose data entities to external reporting tools, such as PowerBI, as OData endpoints. You can also consume data entities.
...
- Aggregate data is a first-class citizen within application data access. Its behavior is similar to the behavior of detailed data. For example, you can enrich aggregate data with extended data types (EDTs) and enumerations, and you can help secure them by using application security concepts.
...
You deploy aggregate measurements to Microsoft SQL Server nonclustered columnstore indexes (NCCI). Therefore, they take advantage of the in-memory computing (IMC) engine that is built into Microsoft SQL Server 2016 as Azure DB. Aggregate measurements that have the IMC engine as their destination are referred to as in-memory, near-real-time (IM-NRT) aggregate measurements. These aggregate measurements don't require that a Microsoft SQL Server Analysis Services (SSAS) server be used. Because these models don't involve data updates, the queries that are sent to them reflect the latest state of data in the operational database. That is why it's referred to as near-real-time.

## KPI modeling and customization

In AX 2012 and earlier versions, you had to model KPIs and business indicators by using native SQL Server development tools. Although users could pin a KPI or business indicator to a Role Center by using the **Business Overview** Web Part, they couldn't modify a KPI definition, such as the goal. Users can use a rich client form to modify a KPI definition that a developer built and shipped. Users can also define new KPIs by using the aggregate data that is contained in aggregate measurements. A developer can model a KPI definition in Microsoft Visual Studio and ship it to a customer, either as a project or together with an independent software vendor (ISV) solution. After a KPI is defined, users can customize it at run time.
