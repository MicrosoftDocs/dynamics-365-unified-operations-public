...
1. On the **Configurations** page, import the ER data model configuration.

    1. Select **Exchange**, and then select **Load from XML file**.
    1. Select **Browse**, find and select the **Questionnaires model.version.1.xml** file, and then select **OK**.

1. Import the ER model mapping configuration.

    1. Select **Exchange**, and then select **Load from XML file**.
    1. Select **Browse**, find and select the **Questionnaires mapping.1.1.xml** file, and then select **OK**.

1. Import the ER format configuration.

    1. Select **Exchange**, and then select **Load from XML file**.
    1. Select **Browse**, find and select the **Questionnaires format.1.1.xml** file, and then select **OK**.

1. In the configuration tree, expand **Questionnaires model**.
1. Review the list of imported ER configurations in the configuration tree.

    :::image type="content" source="./media/er-reduce-fetched-fields-number-configurations.png" alt-text="Screenshot of the list of imported ER configurations on the Configurations page.":::
...
1. On the **Model to datasource mapping** page, select **Designer**.
1. On the **Model mapping designer** page, on the Action Pane, select **Group view** to turn on **Group** view.
1. In the **Data model** pane, expand **Questionnaire**.

    Notice that the **Questionnaire** data source has been configured to access the `KMCollection` application table.

1. In the **Data sources** pane, expand **Table records** \> **Questionnaire** \> **Fields**.
    ...
    :::image type="content" source="./media/er-reduce-fetched-fields-number-mapping1.png" alt-text="Screenshot of the provided model mapping on the Model mapping designer page when Group view is turned on.":::

1. On the Action Pane, select **Group view** again to turn off **Group** view, and then select **Show all** \> **Show mapped only**.
    ...
    :::image type="content" source="./media/er-reduce-fetched-fields-number-mapping2.png" alt-text="Screenshot of the provided model mapping on the Model mapping designer page when Group view is turned off.":::
...
1. On the **Configurations** page, expand **Questionnaires model**, and select **Questionnaires mapping**.
...
1. In the **Performance trace result settings** dialog box, select the trace that was generated during the last format run.

    :::image type="content" source="./media/er-reduce-fetched-fields-number-select-trace-1.png" alt-text="Screenshot of selecting the trace in the Performance trace result settings dialog box.":::

1. Select **OK**.
1. On the **Details** FastTab, filter the **Questionnaire** path that points to the **Questionnaire** data source.
1. Review the details of the database query that was generated when the **Questionnaire** data source was called.
    ...
    :::image type="content" source="./media/er-reduce-fetched-fields-number-query1.png" alt-text="Screenshot of the details of the database query on the Model mapping designer page.":::
...
1. In the **Data sources** pane, select **Edit**.
1. In the **Data source properties** dialog box, select **Select fields** to specify the list of fields of the referenced `KMCollection` application table that will be fetched at runtime when the editable **Questionnaire** data source is called.

    :::image type="content" source="./media/er-reduce-fetched-fields-number-select-fields1.png" alt-text="Screenshot of selecting Select fields in the Data source properties dialog box to start to configure the list of fields that will be fetched from the application table by using the editable data source.":::

1. On the **Select fields** page, select **Autofill**.
    ...
    :::image type="content" source="./media/er-reduce-fetched-fields-number-select-fields2.png" alt-text="Screenshot of configuring the list of fields that will be fetched from the application table on the Select fields page.":::

1. Select **Save**, and close the **Select fields** page.
1. Select **OK** to store the changes that you've made to the data source settings.
1. On the Action Pane, select **Show all**.
    ...
    :::image type="content" source="./media/er-reduce-fetched-fields-number-mapping3.png" alt-text="Screenshot of the updated model mapping on the Model mapping designer page.":::

1. Select **Save** to store the changes that you've made to the editable model mapping.
...
1. Review the details of the database query that was generated when the **Questionnaire** data source was called.
    ...
    :::image type="content" source="./media/er-reduce-fetched-fields-number-query2.png" alt-text="Screenshot of the details of the database query for the updated model mapping on the Model mapping designer page.":::
