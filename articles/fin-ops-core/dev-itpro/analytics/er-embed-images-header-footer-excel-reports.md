...
All the following procedures can be done in the **USMF** company. No coding is required. Before you begin, download and save the following file:
...
2.  On the **Legal entities** page, expand the **Report company logo image** FastTab.
3.  In the **Report company logo image** FastTab, select **Change**.
4.  On the **Select image file to upload** dialog box, select **Browse** and point to the downloaded earlier **Company logo.png** file.
5.  Select **Save**.
6.  Close the **Legal entities** page.
...
4. Notice that, in addition to the selected **Free text invoice (Excel)** ER format, other required ER configurations were imported. Make sure that the following ER configurations are available in the configuration tree:
...
For the example that is shown in this section, you want to use the ER configurations that are provided by Microsoft to generate a free text invoice in Excel format, but you must add a customization to show a company logo image in the page footer of a generated invoice.
...
You must configure your custom format so that it meets your new requirement - to show a company logo image in the footer of every report's page.
...
6. Expand and review the following elements:
    ...
    - The **Free text invoice \\ Invoice** element of the **Sheet** type. This element is used to generate a worksheet of a generated Excel workbook.
...
7. Select the **Free text invoice \\ Invoice \\ Footer** element.
    ...
    > [!NOTE]
    > As you have seen earlier, every page footer of the generated invoice contains information about the current page number and the total number of pages in the report. As you can see, the **Free text invoice \ Invoice \ Footer** element contains no child elements. It means that the used Excel template is configured to show paging details in the center of every report's footer.
...
8. Select **Add**, and then select the **Excel \\ Picture** type of the format element that you are adding:
    ...
    > [!NOTE]
    > The **Excel \\ Picture** element will be used to place a company logo image aligning it to the right of the page footer.
...
10. Expand **model.Payment** \> **model.InvoiceBase \> model.InvoiceBase.CompanyInfo**, and select the **model.InvoiceBase.CompanyInfo.Logo** data source field. This data source field of the [Container](er-formula-supported-data-types-composite.md#container) type exposes the company logo image as a media content.
11. Select **Bind**. The **Picture** format element is now bound with the **model.InvoiceBase.CompanyInfo.Logo** data source field, so that a company logo image will be placed at runtime to the footer of a generated invoice.
...
5. In the configuration tree in the left pane, select **Free text invoice (Excel) custom**.
