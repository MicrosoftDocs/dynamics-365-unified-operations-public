...
1. Update the table type from **InMemory** to **TempDB**.
1. Rebuild the report's RDP class.
1. Restore the data source that is linked to the RDP class in the report designer.
1. Redeploy the report.
1. Introduce a **Controller** class to run the report.
1. Update the Output Menu Item so that it points to the **Controller** class instead of the report.
...
To further improve performance, don't submit one large batch. Instead, submit multiple smaller batches for processing at the same time on different servers. You can run many tasks as part of batch jobs. For more information, see [Batch processing overview](../sysadmin/batch-processing-overview.md).
