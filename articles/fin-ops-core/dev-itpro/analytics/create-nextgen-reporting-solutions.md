...
Reports are any visualization of a structured data set. This definition includes transactional data presented in a tabular layout and advanced graphical views of aggregate information. To account for this broad definition, the application offers several tools to produce reports that satisfy complex business requirements. Some common applications of reports in an ERP include:
...
The most difficult task for developers is selecting the *right* Business Intelligence visualization tool for the job, given a customer's requirements. To accomplish this task, it's important to understand the capabilities offered through the tools that are available for creating reports. We offer tooling to support the following basic reporting requirements:

- **Excel Integration** – Allows data management and analysis using Microsoft Excel
- **Embedded Analytics** – Add aggregate data to Workspaces by using native controls like charts and grids
- **Reporting Services** – Create business documents that require precision by using SSRS-based solutions
...
This section walks you through the process of exporting data displayed in an entity details form. It demonstrates how every form can be viewed as a source of data for management and analysis by using the power of Excel.

### Create an analysis report based on all rentals in Fleet Management that are marked as complete

1. Open Microsoft Edge, go to your instance base URL, and sign in.

    1. On the cloud environment, you get the base URL from LCS.
...
1. Select **Add a filter field**, then select **Status** in the drop-down list.
1. Enter **Complete** in the Status filter field, then select **Apply**.
1. Expand the page action bar, select **Open in Office**, then select **Rentals**.

    :::image type="content" source="./media/fmrentals-export.png" alt-text="Screenshot of FMRentals export.":::

    After the data is exported, you can use the power and flexibility of the Excel tools to create reports for presentation or other analysis. The following image is an example of a self-service report authored with Excel.
...
- Flexible source for accessing transactional business data by using external reporting and analytical tools like Excel
...
1. On the toolbar, select **View**, then select **Application Explorer**.
1. Use the **Application Explorer** to search for the **FMReservationsReport** form in the **Fleet Management** module.
1. In the **Application Explorer's** search results, right-click **FMReservationsReport** form, then select **Add to new project**.
1. Select the **finance and operations Project** template, then select **OK** to create the project.
1. In **Solution Explorer**, right-click **FMReservationsReport** menu item, then select **Set as startup object.**
...
The **Reservations by customer** view now visualizes the aggregate data set by using a Pie chart. This chart type simplifies the task of comparing rental activity across customers.

:::image type="content" source="./media/fmcustomer-activity-2.png" alt-text="Screenshot of FMCustomer activity 2.":::

Other functions include:

- Define contextual drill-through navigation by using modeled properties
- Manage drill-through links by using X++ form logic
...
1. Select the **Dynamics 365** template, then select **OK** to create the project.
...
The preceding screenshot shows the FMRentalsByCustomer report design definition as viewed by using the Visual Studio Precision Designer. The Precision Designer offers a free-form design surface with built-in tools that you can use to customize the content and layout of the report. You can also take advantage of embedded VB code to create run-time design manipulations and support user interactions. As an integrated tool, developers can reference AX labels and public APIs to format data in the report body based on AX EDTs. MSDN offers a rich collection of developer documentation related to SSRS formatting capabilities. For a good primer on designing effective SSRS reports, see the article [Reporting Services Reports (SSRS)](/sql/reporting-services/reports/reporting-services-reports-ssrs).

## Customizing the parameter experience

The Reporting Framework offers flexibility through service extensions to facilitate advanced solutions with requirements that can't be addressed by using a modeled solution. Use the VS designer to add basic parameter formatting, grouping, and input validation. X++ based data contract validation is available for more advanced scenarios. Consider adding User Interface (UI) Builder Classes to customize the parameter pane used to prompt for session inputs before running a report. These custom extensions are effective for addressing the following functions:
...
The parameter initialization code sets the default values of the report execution relative to today's date. Use the classes UIBuilder to override the framework's default handling of report parameters. Other extension scenarios supported:
...
> [!NOTE]
> This example demonstrates a UIBuilder extension with an RDP-based report. For a Query based example that includes a UIBuilder extension, see the **FMCustomerList** report.
...
1. Close the browser session and return to the Visual Studio project.
1. On the toolbar, select **DYNAMICS AX**, then select **Import Project…**
1. In the File name window, browse to `C:\FmLab\Lab10-3`, select **FMRentalDetailsReport.axpp**, then select **Open**.
...
1. In the **Solution Explorer**, select the new project, right-click, then select **Set as startup project.**
1. In the **Solution Explorer**, right-click **FMRentalDetails** menu item, then select **Set as startup object.**
1. Select the project in Solution Explorer, right-click, then select **Deploy Reports**
