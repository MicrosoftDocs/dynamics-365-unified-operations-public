...
description: Learn how to install the modern report design templates in the application suite and apply modern design templates.
author: johnmichalak
...
A new set of developer tools is available that takes the form of report designs for several core business documents in the application suite. These report designs have been reimagined so that flexible branding appears in the header and footer of public-facing documents when transactions are generated in the application. The following illustration shows how an earlier design for a sales invoice differs from a modern sales invoice design.

:::image type="content" source="./media/design-comparison.png" alt-text="Screenshot of examples of an earlier sales invoice design and a modern sales invoice design." lightbox="./media/design-comparison.png":::
...
We're maintaining the legacy solutions for two primary reasons:
...
1. Import the model file into your local development environment. To install a model file in a development environment, use the ModelUtil.exe tool and the **-import** directive. Here's an example.

    ```Console
    ModelUtil.exe -import -metadatastorepath=[path of the metadata store] -file=[full path of the file to import]
    ```

1. Navigate to the **\<AOSServiceDrive\>:\\AOSService\\PackagesLocalDirectory\\bin** folder.
...
1. In the list, select the check box for the **ApplicationSuite** package.
...
1. Complete the **New Project** wizard. Accept all default values.
1. In Solution Explorer, select the project, right-click, and then select **Deploy reports** to deploy the build and deploy the reports locally.
...
1. Select **Print management** to open the **Print management setup** page.
1. Expand the tree, and find the settings for the **Sales order confirmation** document.
1. Select **Original &lt;Default&gt;** to begin to modify the default document routing.
1. In the **Report format** list, select **SalesConfirmModern.Report** to enable the modern report design solution.

    :::image type="content" source="./media/UpdatePrintMgtSettings.png" alt-text="Screenshot of Print management settings page that is used to select the modern design.":::

1. Open another page. This step forces a save operation to occur.
1. Post a sales order to view the modern design in the application.
