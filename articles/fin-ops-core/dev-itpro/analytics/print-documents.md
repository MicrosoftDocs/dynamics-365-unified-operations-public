...
> [!NOTE]
    > When printing reports via a network printer using Document Routing Agent, for some reports barcodes are not printed, they appear as plain text (e.g., *LoadNumber*). Workaround is to print reports to screen and send the preview document to print.
