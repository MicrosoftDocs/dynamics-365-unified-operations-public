...
> Note that the **Deferred execution** option is supported  for the only sequence elements that are configured for an ER format using to generate **outbound** documents in text format.

> Note that the **Deferred execution** option is not applicable for a sequence that has been configured as a trimmed one when its maximum length is limited.
...
5.  Import the ER format configuration.
    -   Select **Exchange**.
    -   Select **Load from XML file**.
    -   Select **Browse** to find the **Format to learn deferred sequences.1.1.xml** file.
    -   Select **OK**.
...
1.  Select **Format to learn deferred sequences** configuration.
2.  Select **Designer** to review the format details.
3.  Toggle **Show details** on.

    Review settings of ER format components configured to generate an outbound document in text format and to place to this document details of tax transactions.

    The **Report\\Lines** sequence format element is configured to populate to outbound document a single line that is generated by any of nested sequence elements (**Header**, **Record** and **Summary**).

    ![Electronic reporting format designer page](./media/ER-DeferredSequence-Format.png)

    The **Report\\Lines\\Header** sequence format element is configured to populate to outbound document a single header line to show date and time when the processing starts.

    The **Report \\Lines\\Record** sequence format element is configured to populate to outbound document a single line with details of a single tax transaction that are separated by semicolon.

    ![Electronic reporting format designer page](./media/ER-DeferredSequence-Format1.png)

    The **Report\\Lines\\Summary** sequence format element is configured to populate to outbound document a single summary line with the sum of tax values of the processed tax transactions.

4.  Select the **Mapping** tab.

    -   The **Report\\Lines\\Header** element is bound to no data source to output a single line to an outbound document.
    ...
    -   The **Report\\Lines\\Record** element is bound to **model.Data.List** list to output a single line for every record from the bound list.
    ...
    -   The **RunningTotal** element is a placeholder for running total of the tax values. Currently, this element outputs nothing as neither a binding nor a default value is configured for it.
    ...
    -   The **Report\\Lines\\Summary** element is bound to no data source to output a single line to an outbound document.
    -   The **Prefix3** element outputs **P3** symbols indicating that the added line is the line containing total tax value.
...
Notice that the summary line 22 presents the sum of tax values of the processed transactions. As we have this format configured to return this sum by using the **model.Data.Summary.Total** binding, this sum is calculated by calling the **TotalSum** aggregation of the **Grouped** data source of the *GroupBy* type in using model mapping. To compute this aggregation, model mapping iterates over
all transactions that have been selected in the **Filtered** data source. Comparing the execution time of lines 21 and 22, you can see that it took 10 milliseconds to perform this summing. Comparing the execution time of lines 2 and 21, you can see that It took 7 milliseconds to generate all transactional lines. Totally, it took 17 milliseconds.
...
4.  Select the **Mapping** tab.
5.  Select the **Report\\Line** sequence element.
...
8.  Select the **Report\\Line\\Record\\TaxAmount** numeric element.
...
10.  Select the **Report\\Line\\Record\\RunningTotal** numeric element.
...
17.  Download the offered by a web browser file and open it for review.
     ...
     The line 21 contains the tax value running total that is computed for all processed transactions by using the generated output as a data source - starting from the beginning of the report up to the last tax transaction inclusively. The line 22 contains the sum of tax values for all processed transactions that is computed in model mapping by using the data source of the **GroupBy** type.
Notice that these values equal which means that the output-based summing can be used instead of the **GroupBy** one. Comparing the execution time of lines 2 and 21, you can see that It took 9 milliseconds to generate all transactional lines and perform summing. From the detailed lines generation and tax values summing perspective, the modified format is approximately in two times faster than the original one.

18.  Select the **Report\\Line\\Summary\\TotalTaxAmount** numeric element.
...
23.  Download the offered by a web browser file and open it for review.
     ...
     Now the tax value running total in the last transaction details line equals the sum in the summary line.
...
6.  Download the offered by a web browser file and open it for review.
     ...
     Notice that the sum of tax values in the summary line 2 equals zero. It happens because now this sum is computed based on generated output and at the time when the line 2 is generated, the generated output contains no lines with transaction details yet. You can configure this format to defer the execution of the **Report\\Line\\Summary** sequence element until the **Report\\Line\\Record** sequence element has been executed for all tax transactions.

## Defer execution of the summary sequence to use calculated total
...
5.  Download the offered by a web browser file and open it for review.
     ...
     The execution of the **Report\\Line\\Summary** sequence element is now performed at the end of execution of all other nested items of the **Report\\Line** element that is the parent element of the **Report\\Line\\Summary** one. Therefore, it happens after the execution of the **Report\\Line\\Record** sequence element for all tax transactions of the **model.Data.List** data source. The execution time of lines 1,2,3 and the last line 22 illustrates this.
