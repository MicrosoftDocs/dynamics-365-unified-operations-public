...
Clicking **Generate** button after a recoding file is available with the test case generates test automation files in your working directory. If the test case doesn't already have an Excel test parameter file, then this is created too and appear on the grid under **Parameters File**. Existing Parameter files isn't affected.

You can generate **test execution files** only, without affecting your parameter files by selecting **Generate \> Generate Test Execution files only**.

Selecting **Generate \> Generate Test Execution and Parameter files**, generates both test automation files and a new Excel parameter file in your working directory. This option is useful after uploading a new recording file with changed steps to create a new parameter file with matching steps.
...
Save the Excel files when you're done making edits and make sure then to click **Generate** to create new execution files with the new parameters.
...
If you don’t see the **Pause** column, you're using an older version of the Excel parameters file and need to regenerate it. Select the desired test case, then go to **Generate > Generate Test Execution and Parameter files**. This may override edits you have made to the parameters file, so you should back up the existing Excel file first.
...
Select **Run** to execute the selected test cases. Only test cases with existing automation files can be run. The tool opens and execute these tests with the data you entered in Excel.
...
When a test run is in progress, you can select the **Stop** button on the toolbar to cancel the run. Execution stops after the currently running test case completes. The remaining test cases is marked as **Not Executed** in Azure DevOps.
...
RSAT provides capabilities for managing the readiness of test cases. It also provides a sign out process for test runs. This is configurable in the **Process** tab under Settings.
