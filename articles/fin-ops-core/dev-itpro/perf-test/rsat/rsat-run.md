...
Clicking **Generate** button after a recoding file is available with the test case will generate test automation files in your working directory. If the test case does not already have a Excel test parameter file, then this will be created too and appear on the grid under **Parameters File**. Existing Parameter files will not be affected.
...
Selecting **Generate \> Generate Test Execution and Parameter files**, will generate both test automation files and a new Excel parameter file in your working directory. This option is useful after uploading a new recording file with changed steps to create a new parameter file with matching steps.
...
Save the Excel files when you are done making edits and make sure then to click **Generate** to create new execution files with the new parameters.
