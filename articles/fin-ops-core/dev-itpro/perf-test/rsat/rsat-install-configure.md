...
+ **Hostname** - Hostname of the test environment. For example, myhost.cloudax.dynamics.com. Do not include the https:// or http:/ prefix.
+ **SOAP Hostname** - SOAP Hostname of the test environment. Typically, you need to add a **soap** suffix to the Hostname. For example, if myhost.cloudax.dynamics.com is your Hostname, use myhost**soap**.cloudax.dynamics.com for the SOAP Hostname. 
   + If you are unsure of the SOAP Hostname of your test environment, you can find it in the web.config file of the AOS server, in Infrastructure.SoapServicesUrl.
   + If your test environment is a self-service User Acceptance Test (or higher tier) sandbox environment with no Remote Desktop access (Environment running on the Service Fabric infrastructure), the SOAP Hostname is equal to the Hostname.
...
If your test environment does not allow for Remote Desktop access - For example, a self-service User Acceptance Test (or higher tier) sandbox environment (Environment running on the Service Fabric infrastructure) - follow these instructions to configure the environment to trust the RSAT connection:
1. Create the RSAT authentication certificate using the New button on the RSAT settings dialog as described earlier.
2. Open a support request and provide the support engineer with the following:
   - Your environment ID (located on the environment page in LCS)
   - The .cer file generated by RSAT
   - An acceptable downtime window for the sandbox environment (Downtime is X minutes)
