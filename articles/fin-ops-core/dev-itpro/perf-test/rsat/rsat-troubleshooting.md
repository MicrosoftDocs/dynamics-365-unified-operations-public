...
+ An administrator must create and install the authentication certificate on the same computer where RSAT is installed. If an admin doesn't create the certificate, you see the following error message when you try to run a test case.

```Plaintext
Cannot access finance and operations environment. Verify your settings and make sure the environment is available.
```

+ If you used a previous version of RSAT on the same computer, close it and uninstall it before installing a new version.

## Screen resolution when using Internet Explorer

If you select Internet Explorer as your browser, set your desktop resolution to 100% to run the tests successfully. To change the settings, use Windows **Display settings > Scale and layout**, as shown in the following image:

:::image type="content" source="media/screen-resolution.png" alt-text="Screenshot of Setting screen resolution.":::

## Test playback errors

### SOAP or HTTP request errors

If you're testing against a Standard Acceptance Test Sandbox environment (Tier2) or any other multibox environment, you might receive one of the following errors when you run a test.
...
Assuming that you specified the correct SOAP hostname in the RSAT settings dialog box, run the following PowerShell scripts on your client computer where the test tool is installed.
...
### Can't enumerate AX users error

You might receive the following error when running a test case, or the error details might contain the following messages.
...
:::image type="content" source="media/cannot-enumerate.png" alt-text="Screenshot of Enumerate error message box.":::

To resolve this error, verify the **Admin user name** specified in the RSAT settings dialog box. The **Admin user name** must be the email address of a user that belongs to the System Administrator role on the finance and operations test environment that RSAT connects to. The user account (email address) must also belong to the same tenant as the test environment. For example, if your test environment's tenant is **contoso.com**, the admin user must end with **\@contoso.com**.

### Unsecured fault exception

If a test case inconsistently fails with the following error, this error usually indicates an incomplete configuration of the authentication thumbprints on the AOS virtual machines.
...
Typically, this error happens when the test environment isn't configured to trust the certificate that RSAT uses for authentication. The certificate is identified by the thumbprint specified in your RSAT settings. For example, the thumbprint might be missing in the wif.config file on the AOS virtual machine of the test environment. If you're running against a standard acceptance test environment (Tier 2 or higher), you might not have configured the authentication thumbprint on all of the AOS virtual machines. Make sure you properly add the thumbprint to the **wif.config** file on all of the AOS machines. For more information, see [Configure the test environment to trust the connection](rsat-install-configure.md#configure-the-test-environment-to-trust-the-connection).

You might also see this error when there's a mismatch between the UTC time between the client computer (where RSAT is installed) and the finance and operations environment. This mismatch is a rare case and only happens if your administrator incorrectly configures the UTC time. The client computer and finance and operations environment can be on different time zones. However, the UTC time on both environments must match because the authentication mechanism relies on this comparison.

## Google Chrome Browser

The Google Chrome browser might not work with the Regression suite automation tool due to your Active Directory security settings. In this case, change your RSAT settings to use the new Microsoft Edge or Internet Explorer.

## Validating blank dates

If your test case requires validation that a certain control of type Date/Time is blank, insert the following value into the Excel cell corresponding to this control: "01/01/1900".

## Azure DevOps connectivity

You might see this error when you select the desired Azure DevOps project in RSAT settings: "The structure path \<iteration path\> isn't valid. Verify your settings and try again." To resolve this error, open the project in Azure DevOps and go to the Test Plans. Verify the iteration path defined for each test plan. If the iteration path is similar to what is shown in the error, remove the existing iteration path and add a new one for the test plan and save.
