...
Certificates are often generated by a certificate authority that issues certificates based on the identity of the applicant. You submit a certificate signing request to these providers and in return, they provide a SSL certificate signed using their root certificate and private key. The certificate has a specified life span. If you have a certificate from a CA provider, you can use that certificate instead of creating a self-signed certificate. You can also generate certificates from the Active directory certificate services.

In this section, we generate a self-signed certificates that we need to set up the RSAT system. We'll create a root certificate, an intermediate certificate and a user certificate. After the certificates are created, these certificates are uploaded to Entra and installed on the local box running RSAT 2.8.
...
With the identity and signer in place, the script creates a new self-signed but subordinate certificate that's specifically intended for client authentication. Custom text extensions clarify the usage and
ensure it can't act as a CA.
...
5)  Do the same for the Intermediate one but set \"Is this certificate authority the root?\" = No
 
...
On the computer where you'll run RSAT, install the certificates:
1. Root CA certificates only need installing if self-signed.
...
**Predefined** - Users can't use automated but want to control their policies themselves. If filtering by subject from our rule in RSAT won't work, configuring this is feasible. It may be challenging to make a general policy works with all accounts, but it would be up to users to govern this then. Predefined means RSAT doesn't touch policy entries in the registry at all. This gives an option for configuring the policy not under current users, but under the local machine to share between users.

**Manual** - RSAT removes any policy entries when playing back a test if one is there in advance, but only from the current user. This means if they have a policy from local machine then execution does fallback on this, so that shouldn't be mixed. The manual method gives a window of 10 seconds for manually selecting the policy when certauth callback for the selection of this.  
