...
- Don't use type inference if you want compilation to fail when the return type of a method changes.
...
Well-known data methods, creator methods, and `init` methods usually return records or entities instead of IDs. Use records or entities as method parameters.
...
The code is easier to read when it doesn't include unimportant technical details.
...
When you automate a new domain area, create a base class that holds shortcuts to the most frequently used navigation objects in that area.

For example, for the warehouse management area, create a base class named `AtlWHSTestCase`. It contains shortcuts to `data.whs()`, `data.invent()`, `data.invent().items()`, `data.invent().units()`, and other navigation objects. The shortcuts simplify your test code.
...
Also, introduce shortcuts that many test methods in the same class share.
...
You don't need to create a shortcut for every navigation node. However, consider creating shortcuts for the navigation nodes that you frequently use.
