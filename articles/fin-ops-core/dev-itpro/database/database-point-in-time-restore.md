...
In the event of failure, the restore operation will rollback automatically. Your target environment is restored to the state that it was in before the restore began. A rollback is often required if a customization that is present in the target environment can't complete a database synchronization with the newly restored data.
