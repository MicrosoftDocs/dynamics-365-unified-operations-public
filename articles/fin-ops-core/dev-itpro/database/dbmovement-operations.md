...
|Production| DevTest| Supported in Power Platform admin center | Supported via Power Platform API | [Tutorial - Copy a Lifecycle Services Environment to a unified environment (preview)](/power-platform/admin/unified-experience/tutorial-copy-lifecycle-services-environment-unified-environment) |
...
> [!IMPORTANT]
> Database movement operations are very lengthy operations. The amount of time these operations take varies by data volume, schema complexity, and whether the target of the operation is a flat file, like a .bacpac file in the case of the export operation. In general, these operations can take up to 24 hours to complete, and during the operation there's no way to cancel. In addition, if the target of the operation is an environment like in the production to sandbox scenario, the sandbox environment goes offline at an undetermined time when the database copy operation completes, but before the database synchronization steps begin. This action happens with no warning to end users in the environment. As result, schedule these types of operations during non-business hours.

## Database Movement API

The Database Movement application programming interface (API) integrates several previously mentioned database movement operations into your overall ALM process. By using the API together with your preferred scheduling engine, you can build recurrence into the process, so that it runs daily or on demand.
