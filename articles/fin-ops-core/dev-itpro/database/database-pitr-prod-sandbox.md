...
-  Any **files stored in Azure blob storage aren't copied** from one environment to another. This notably includes **document attachments and custom Microsoft Office templates**. These documents won't be changed and will remain in their current state. 
