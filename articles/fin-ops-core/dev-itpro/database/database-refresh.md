...
> [!IMPORTANT]
> Copying production data during business hours or peak hours could have an impact on the production system. So it's highly recommended practise to do the refresh database operation during off peak hours and limit only one refresh operation at a time from production system.
> [!IMPORTANT]
