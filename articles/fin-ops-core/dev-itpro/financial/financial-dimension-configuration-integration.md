...
> [!NOTE]
> The dimension format types aren't company-specific. The setup is a global setup. You can set up as many of each format type as you need, but only one format type can be active at one time. If no dimension format type is set up, you'll receive an error when you import. You'll also receive errors if the order is incorrect. 
...
For this example, we use the same import file.
...
However, we use the following active ledger dimension format: **Main account - Department - Cost Center - Customer** 
