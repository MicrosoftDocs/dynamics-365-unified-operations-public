...
4. This mapping has a filter on both sides to link customer contacts. Open the mapping details, select the filter button (funnel symbol) next to the **Sales.Contact** entity name, and make sure that the file criteria contain **msdyn_contactfor eq Customer**, **msdyn_sellable eq false**, and **msdyn_contactpersonid ne null**.
