...
A party is a person or an organization that is involved in a business. When you use the party concept, a person or an organization can play more than one role in a business (for example, customer, vendor, or contact). The role is based on the context and purpose. Here are some examples of roles from two fictitious companies, Contoso and Fabrikam:
...
+ **Contact** – A person to contact. For example, if Contoso buys supplies from Fabrikam, employees at Contoso reach out to the contact at Fabrikam.
...
Use the party model to represent medium to complex relationships between organizations and people, especially when a party plays more than one role. Here are some common examples:
...
:::image type="content" source="media/party-gab-image1.png" alt-text="Screenshot of data model for party.":::

> [!TIP]
> When you create a new account record, use the **Party** field to search for the record by name. If you find the record, select it. The system then automatically fills in all the data from the party. You don't have to manually set all the required fields. You can find this behavior on the out-of-box **Account**, **Contact**, and **Vendor** pages.
...
The global address book stores and handles as many postal addresses and electronic addresses as required. For example, Fabrikam has gas stations in 50 locations. Each location has a different postal address, email address, and phone number. All business purchases are billed to the main gas station but shipped directly to the specific gas station that requested the purchase. The global address book stores the main gas station as the billing address for Fabrikam and stores each gas station as a shipping address. You can store the addresses once and then retrieve them as needed for quotations and orders.

Depending on the business context, a person or an organization might play more than one role, and the same postal address and electronic address might be used for all the roles. In this case, a change of address in one role appears in all the other roles. The global address book stores and handles addresses globally.

The following illustration shows the data model for the global address book.

:::image type="content" source="media/party-gab-image2.png" alt-text="Screenshot of data model for the global address book.":::

## Contact

In customer engagement apps, a contact is a person. However, the **Contact** table represents a person, a portal user, a business-to-consumer (B2C) customer, or a vendor. The representation is implicit, and you can't tell the difference unless you examine related transactions. The **Contact** table has a one-to-one (1:1) relationship with the **Account** table. As part of the party and global address book model, dual-write introduces explicit properties for classification and allows N:N relationships between a contact that is a person and an organization (**Account** or **Vendor** entity).
...
The **Contact for Party** table stores and handles many-to-many relationships between **Account** rows and **Contact** rows. It filters out the striped **Contact** rows from unstriped rows and associates only the unstriped **Contact** rows with **Account** or **Vendor** rows.
...
The **Account**, **Contact**, and **Vendor** pages now include an **Addresses** tab. This tab uses a grid to support multiple postal addresses, as shown in the following illustration.

:::image type="content" source="media/party-gab-image4.png" alt-text="Screenshot of grid for postal addresses.":::
...
Use the **New Address** button above the grid to create as many postal addresses as you want.

In customer engagement apps, when a user enters the addresses on the **Summary** tab of the **Accounts** page, the **Address 1** and **Address 2** fields correspond to the **Delivery** and **Invoice** addresses, respectively. However, when a user creates a postal address in finance and operations apps, the first two addresses of the customer record appear in the **Address1** and **Address2** fields, and the user can change the address purpose to **Delivery** and **Invoice**.

:::image type="content" source="media/party-gab-image5.png" alt-text="Screenshot of Summary tab for postal addresses.":::
...
The **Account**, **Contact**, and **Vendor** pages now include an **Electronic Addresses** tab. This tab uses a grid to support multiple electronic addresses, as shown in the following illustration.

:::image type="content" source="media/party-gab-image6.png" alt-text="Screenshot of grid for electronic addresses.":::
...
Use the **New Electronic Address** button above the grid to create as many electronic addresses as you want.

During the lead qualification process, you can provide both a business phone number and a mobile phone number. If **IsMobile=No**, the business phone number is the primary phone number. If **IsMobile=Yes**, the mobile phone number is the secondary phone number.

> [!TIP]
> Use the **Addresses** and **Electronic Addresses** tabs on the **Account** and **Contact** forms to manage postal and electronic addresses. This approach ensures that address data synchronizes to finance and operations apps.

## Setup

1. Open your customer engagement app environment.
1. Install all prerequisite solutions, as described in [Separated Dual-write Application Orchestration package](separated-solutions.md).
1. Install [Dual-write Party and Global Address Book Solutions](https://aka.ms/dual-write-gab).
1. Open the finance and operations app. Go to the Data Management module and select the Dual-write tab. The dual-write administration page opens.
1. Apply both the solutions installed in steps 2 and 3 by using the [Apply Solution]((../../dev-itpro/power-platform/enable-power-platform-integration.md) function.
1. Stop the following maps, because they're no longer required. Instead, run the `Contacts V2 (msdyn_contactforparties)` map.

    + CDS Contacts V2 and Contacts (refers to customer contacts)
    + CDS Contacts V2 and Contacts (refers to vendor contacts)

1. The following entity mappings are updated for party functionality, so you must apply the latest version to these mappings.

    Map | Update to this version | Changes
    ---|---|---
    `CDS Parties (msdyn_parties)`| 1.0.0.2 | This map is new in this release.
    `Contacts V2 (msdyn_contactforparties)`| 1.0.0.6 | This map is new in this release.
    ...
    `CDS Party postal address locations (msdyn_partypostaladdresses)` | 1.0.0.1  | This map is new in this release.
    `CDS postal address history V2 (msdyn_postaladdresses)` | 1.0.0.2 | This map is new in this release.
    `CDS postal address locations (msdyn_postaladdresscollections)` | 1.0.0.0 | This map is new in this release.
    `Party Contacts V3 (msdyn_partyelectronicaddresses)` | 1.0.0.0 | This map is new in this release.
    `Complimentary Closings (msdyn_compliemntaryclosings)` | 1.0.0.0 | This map is new in this release.
    `Decision making roles (msdyn_decisionmakingroles)` | 1.0.0.0 | This map is new in this release.
    `Loyalty levels (msdyn_loyaltylevels)` | 1.0.0.0 | This map is new in this release.
    `Contact person titles (msdyn_salescontactpersontitles)` | 1.0.0.0 | This map is new in this release.
    `Personal character types (msdyn_personalcharactertypes)` | 1.0.0.0 | This map is new in this release.
    `Salutations (msdyn_salutations)` | 1.0.0.0 | This map is new in this release.
    `Employment job functions (msdyn_employmentjobfunctions)` | 1.0.0.0 | This map is new in this release.
    `CDS location roles (msdyn_addressroles)` | 1.0.0.0 | This map is new in this release.

1. Before running the preceding maps, update the integration keys manually as described in the following steps. Then select **Save**.
...
1. In Dataverse, the duplicate detection rules character limits increased from 450 to 700 characters. This limit lets you add one or more keys to the duplicate detection rules. Expand the duplicate detection rule for the **Accounts** table by setting the following fields.
    ...
    :::image type="content" source="media/duplicate-rule-1.PNG" alt-text="Screenshot of duplicate rule for Accounts.":::

1. Expand the duplicate detection rule for the **Contacts** table by setting the following fields.
    ...
    :::image type="content" source="media/duplicate-rule-2.PNG" alt-text="Screenshot of duplicate rule for Contacts.":::

1. If you're an existing dual-write user, follow the instructions in [Upgrade to the party and global address book model](upgrade-party-gab.md) and upgrade your data. **Don't proceed to step 12 without completing this step.** If you're a new dual-write user, proceed to step 12.

1. If you're an existing dual-write user, complete step 11, and then run the maps in the following order. If you're a new dual-write customer, you can proceed directly. If you're shown an error message that states "Project validation failed. Missing destination field...", open the map and select **Refresh Tables**, then run the map.
...
> [!NOTE]
> The `CDS Contacts V2 (contacts)` map is the map that you stopped in step 1. When you try to run other maps, these two maps might appear in the list of dependents. Don't run these maps.
> 
...
> If the supply chain dual write solution is version 2.3.4.203 or later, the plugin owners disable the plugin.
> If the supply chain dual write solution is version 2.3.4.203 or earlier, disable the plugin by contacting support.
> If you uninstalled the party and global address book solution, re-enable the plugin.
>
> The `msdyn_*partynumber` field (a single line text field) that is included in the **Account**, **Contact**, and **Vendor** tables shouldn't be used going forward. The label name has a prefix of **(Deprecated)** for clarity. Instead, use the **msdyn_partyid** field. The field is a lookup to the **msdyn_party** table.
>
...
## Address roles as a multiselect drop-down list
...
:::image type="content" source="media/purpose.png" alt-text="Screenshot of Purpose/Role drop-down list.":::

## Known problems and limitations

+ In finance and operations apps, when you create a customer along with address and save it, the address might not synchronize to the **Address** table. This problem occurs because of a dual-write platform sequencing problem. As a workaround, create the customer first and save it. Then add the address.
+ In finance and operations apps, when a customer record has a primary address and you create a new contact for that customer, the contact record inherits a primary address from the associated customer record. This inheritance happens for vendor contact, too. Dataverse doesn't currently support this behavior. If dual-write is enabled, a customer contacts that is inherited with a primary address from the finance and operations app is synchronized to Dataverse along with its address.
+ In finance and operations apps, you can create a contact record from the **Add Contact** page. When you try to create a new contact from the **View Contact** page, the action fails. This problem is a known problem.

    ![Known problem with Add Contact.](media/party-gab-contact-issue.png)
...
+ You can't enter a forward-dated postal address by using a finance and operations app with dual-write, because Dataverse doesn't support date effectivity. If you enter a future-dated postal address by using a finance and operations app, it synchronizes to Dataverse fully and you see the address on the user interface immediately. Any updates to this record results in an error as it is future-dated and not current in the finance and operations app.
