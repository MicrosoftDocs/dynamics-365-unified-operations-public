...
Maps can be exported and imported into different environment, but automation is available through DevOps . You can find more detail about version control [here](https://docs.microsoft.com/en-us/dynamics365/fin-ops-core/dev-itpro/data-entities/dual-write/app-lifecycle-management#update-entity-maps-and-export-them-to-other-environments-as-a-solution)
### Where can I find example and patterns for filtering Dual write maps (CDS and F&O)?
You can find basic filtering examples [here](https://docs.microsoft.com/en-us/dynamics365/fin-ops-core/dev-itpro/data-entities/dual-write/customizing-mappings#filter-your-data).
More advanced CDS filters can be found [here](https://docs.microsoft.com/en-us/powerapps/developer/common-data-service/webapi/query-data-web-api#filter-results). Nested lookup is currently not supported in dual-write source filter. As of now, only [standard filter operators](https://docs.microsoft.com/en-us/powerapps/developer/common-data-service/webapi/query-data-web-api#standard-filter-operators) directly against entity fields are supported. 
More advanced F&O filters can be found in a combination of [here](https://docs.microsoft.com/en-us/dynamicsax-2012/developer/using-expressions-in-query-ranges) & [here](https://docs.microsoft.com/en-us/dynamics365/fin-ops-core/fin-ops/get-started/advanced-filtering-query-options)
...
### When data flow is paused and then resumed, does it follow the sequence of changes? E.g. customer name changes in F&O from A to B, and then to C. Will CE show all changes, or will it just show A to C?
...
After failover transition is completed, Dual write solution will continue to work as it is, as long as both Finance and Operations apps environment and CDS environment are accessible. Finance and Operations apps environment and CDS environment will adhere to standard failover process.
...
### Can I develop unbounded fields on F&O data entities to flow to CDS via DW?
Yes. Both [computed and virtual fields](https://docs.microsoft.com/en-us/dynamics365/fin-ops-core/dev-itpro/data-entities/data-entity-computed-columns-virtual-fields) can be used, but you should pay special attention to performance overhead from additional X++ logic required for reads/write. Also roundtripping within the same transaction is not allowed, so avoid using virtual fields to transform / calculate additional values through X++ and expect that to go back to CDS within same transaction.
### With CDS offline app, What if it is not possible to update the 'other' side after reconnecting? Would that cause incorrect state between CDS and F&O
Users can access CDS data offline either via using CDS applications or through using Field Service Mobile app. In both options, data will be kept offline, and it can be synced with the server in user's discretion. In case of any issues when the offline data is being synced with the server and updates cannot be performed because other end of Dual write is failing the error can be handled by utilizing conflict management settings in both options. For more details about different conflict management settings, see [CDS conflict management settings](https://docs.microsoft.com/en-us/dynamics365/mobile-app/setup-mobile-offline-for-admin#step-23-set-conflict-detection-for-mobile-offline) and [Resco conflict resolution settings](https://docs.resco.net/wiki/Conflict_resolution).
...
Number sequences for Finance and Operations apps and CDS apps are not connected. In a multi mastered entity scenario, plan for separate number sequence format or range per app, such as:
