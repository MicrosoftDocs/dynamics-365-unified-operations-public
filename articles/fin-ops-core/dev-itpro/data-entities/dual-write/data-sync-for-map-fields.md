...
description: Learn about the dual-write triggering behavior for mapped fields, outlining how dual-write platform core plugins are triggered.
...
Going forward, dual-write platform core plugins are triggered only when the updated field is configured in dual-write map.
...
- If B is updated, the dual-write plugin is invoked and sends both B and C fields as part of payload to finance and operations apps.
- If B is updated and C is calculated based on B, the dual-write plugin is invoked because B is being mapped and sends both B and C fields as part of payload to
