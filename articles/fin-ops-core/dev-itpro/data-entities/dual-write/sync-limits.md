...
- **[Single transaction](#single-transaction)** – All data that the process writes is part of a single transaction.
- **[Multiple transactions](#multiple-transactions)** – The process splits all the data it writes into multiple transactions.
...
| Measure | Limits |
|---|---|
| Number of transactions | Service protection API limits govern the total number of transactions that you can perform per day per tenant. These limits are designed to detect when client applications make extraordinary demands on server resources. For more information, see [Service protection API limits](/powerapps/developer/data-platform/api-limits). |
