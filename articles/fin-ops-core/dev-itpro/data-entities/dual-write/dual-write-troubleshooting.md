...
## <a id="enable-view-trace"></a>Enable and view the plug-in trace sign in Dataverse to view error details

Trace logs can be useful when troubleshooting dual-write live sync problems between Finance & Operations and Dataverse.
The logs provide specific details to the teams who provide technical and engineering support for Dynamics 365.
This article explains how to enable trace logs and how to view them. You manage trace logs in the Dynamics 365 Settings
page and need administrator level privileges to change and view them.
...
1. Select the **Customization** tab and plug-in, and then in the custom work flow activity tracing section change the dropdown to **All**. All activities are traced and a comprehensive set of data is provided to the teams who must review potential issues.
...
1. Select **Plugin Trace Log** in the **Customizations** section of the page.
1. You can find entries in the list of trace logs, based on Type Name and/or Message Name.
1. Open the desired entry to view the full log. The Message Block in the Execution section provides available information for the plug-in.
If available, exception details are provided.
...
1. Open the **DualWriteProjectConfiguration** table by using the Excel add-in. To use the add-in, enable design mode in the finance and operations Excel add-in and add the **DualWriteProjectConfiguration** table to the sheet. For more information, see [View and update entity data with Excel](../../office-integration/use-excel-add-in.md).
1. Set **IsDebugMode** to **Yes** on the project.
1. Run the scenario that generates errors.
1. The verbose logs are stored in the **DualWriteErrorLog** table.
1. To look up data on table browser use the following link: `https://999aos.cloudax.dynamics.com/?mi=SysTableBrowser&tableName=DualWriteErrorLog`, replacing `999` as needed.
1. Update again after [KB 4595434](https://fix.lcs.dynamics.com/Issue/Details?kb=4595434&bugId=527820&dbType=3&qc=98e5dc124ac125c57ad633d885ac612aea3ddb8f4abf9d71ab3aa354f2e06cbe), which is available for platform updates 37 and later. If you have this fix installed, the debug mode captures more logs.  
...
When you open the dual-write page in the Microsoft Edge or Google Chrome browser, the home page doesn't load. You see a blank page or an error such as "Something went wrong."
...
1. You block third-party cookies altogether in Microsoft Edge or Chrome.
...
1. Select **Allow all cookies**. If you don't want to allow all cookies, use the second option.

Second Option:

1. Go to settings by entering `chrome://settings/` in the address bar, and then navigate to **Privacy and Security** > **Cookies and other site data**.
1. If **Block third-party cookies in Incognito** or **Block third-party cookies** is selected, go to **Sites that can always use cookies** and select **Add**.
1. Add your Finance & Operations apps site name - `https://<your_FinOp_instance>.cloudax.dynamics.com`. Make sure you select the checkbox for **All cookies, on this site only**.
...
1. Go to **Workspaces > Data management**, and select the **Dual Write** tile.
1. Select all running mappings, and then select **Stop**.
1. Select **Unlink environment**.
...
1. Find the **Information** form under the forms node.
1. Select the **Information** form and select **Enable security roles**.
1. Change the security setting to **Display to everyone**.
...
1. Inside **Data management**, select **Framework parameters**.
1. On the **Data import/export framework parameters** page, select the **Entity settings** tab, and select **Refresh entity list**. The list might
take more than 30 minutes to refresh, depending on the number of entities involved.
1. Go to **Data management** and select **Data entities** to check that the expected entities are listed. If the expected entities
aren't listed, check that the entities appear in your finance and operations environment and restore the missing entities, if needed.
...
1. Search for entities with problems, and make a note of the target entity, staging table, entity name, and other settings. Delete the entity or entities from the list.
1. Select **New** and re-add the entity or entities by using the data from step 2.
...
1. Open the **Network** tab and type **integ** in the filter text box.
1. Run your scenario and watch the requests being logged.
1. Right-click on the entries and select **Save all as a HAR with content**.
...
1. Run your scenario.
1. Select **save** to export the results as HAR.
