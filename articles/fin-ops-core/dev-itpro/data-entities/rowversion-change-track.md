---
title: Enable row version change tracking for tables and data entities
...
# Enable row version change tracking for tables and data entities
...
> [!NOTE]
> The **SysRowVersionNumber** column (which is also known as the **rowversion** column) is read-only in SQL. Therefore, direct SQL update statements, such as the following example, will break if they try to create or update this column.
>
> ```SQL
> INSERT INTO table2
> SELECT * FROM table1
...
| Data entities where the data sources have range filters aren't supported. | [![Data Sources Ranges.](./dev-itpro/data-entities/media/DataSourceRangesjpg.jpg)](./dev-itpro/data-entities/media/DataSourceRangesjpg.jpg) | Change tracking can't be enabled because the data source %1 in the F&O entity %2 contains Ranges. |
...
| Only normal relation constraints are supported for data sources in a data entity. | [![Data sources Relation Constraints.](./dev-itpro/data-entities/media/DataSourceRelations.jpg)](./dev-itpro/data-entities/media/DataSourceRelations.jpg) | Change tracking can't be enabled because the Relation %1 between the data source %2 and the data source %3 in the F&O entity %4 isn't set to Normal. |
...
This change tracking feature is fully compatible with Dataverse change tracking. For more information, see [Use change tracking to synchronize data with external systems](../power-apps/developer/data-platform/use-change-tracking-synchronize-data-external-systems). However, there are some differences.

> [!NOTE]
> Changes for finance and operations apps will be returned if the last token is within the default value of 10 days, not the value of 90 days that is used for Dataverse tables.
