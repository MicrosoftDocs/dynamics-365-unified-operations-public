...
> [!IMPORTANT]
> Microsoft delivered default configuration templates in Microsoft Dynamics 365 Finance, Enterprise edition July 2017 update. Microsoft Dynamics 365 for Operations platform update 7 includes the Configuration data project feature. You can create and use your own templates in the current product release.
...
1. Set up your system by getting the new data configuration user interface (UI) and set default file extensions.
1. Set up configuration templates for both export and import.
1. Create and run a configuration data project for export.
1. Create and run a configuration data project for import.
...
After you create a configuration data project, it appears in the **Data projects** grid in the **Data management** workspace. For each project, the type of configuration (import or export) and the project category (**Project**, **Configuration**, **Integration**, or **Other**) are shown. Use the selections to the left of the grid to filter by the project you want.
...
Use **Job history** to find more details about the projects that you run. Use the data range filters to filter by the dates when data projects were run. You can view execution details by selecting a job and then using the **Execution details** menu.
...
We updated the **Data management** workspace and the **Template**, **Data export**, and **Data import** pages so that they support configuration management. However, the updated pages are available only in Enhanced view. Standard view isn't updated from previous releases.

#### Change to the new UI for a single user

You can change the system's default view settings to save settings for each user per each legal entity. To switch the view for a single user, the user must select the **Enhanced view** button on each page in the **Data management** workspace.
...
1. Change the **View default** setting from **Standard** to **Enhanced** for all users and legal entities.

### Set file extensions for default data sources

The **Template** and **Import/export** pages let you add entities by selecting a file that was created by using the data management framework. The following default file name extensions are added for the different types of data sources:
...
If you want to use other file name extensions, you must update your data sources so that they have a default file name extension that is used as the target data format.

1. In the **Data management** workspace, select the **Configure data sources** tile, and then select **Data sources**.
1. Add a default file name extension to the appropriate data source.
