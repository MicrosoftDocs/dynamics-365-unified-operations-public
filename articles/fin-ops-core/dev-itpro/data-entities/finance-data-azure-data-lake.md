...
| 410        | Failed to find Application ID to access the Data lake. Application ID provided is incorrect or can't be found      | Application ID provided in the key vault can't be found in Azure active directory. Validate the Application ID provided in the key vault by following the documentation steps [Configure export to Azure Data Lake - Create application](configure-export-data-lake.md#createapplication) |
| 411        | Failed to access data lake with given Application ID and Application secret | Application ID (app-id) and Application secret  (app-secret) provided can't be used to access the storage account. Verify the Application ID and Application secret are valid by following documentation steps [Configure export to Azure Data Lake - Create Application](configure-export-data-lake.md#createapplication) <br> Next validate that the application has required access to the storage account [Configure export to Azure Data Lake - Grant access](configure-export-data-lake.md#grantaccess). You need to contact the systems administrator or the LCS administrator who configured the system | 
| 415        | Failed to access the storage account using the storage name provided in the key vault | Storage account provided in the Key vault can't be found or is invalid. Validate that correct storage account name is entered into the key vault by using steps here: [Configure export to Azure Data Lake - Add secrets](configure-export-data-lake.md#addsecrets) <br> Verify that you have provided the correct secret name for the storage account by using steps here: [Configure export to Azure Data Lake Add secrets](configure-export-data-lake.md#addsecrets) | 
| 420        | Failed to access the key vault or the key vault secrets | Service can't access the key vault or the secrets in the key vault.  Verify that your Azure subscription is not expired <br> <br> Verify that you have created the service principal by following steps here: [Configure export to Azure Data Lake - Create service principal](configure-export-data-lake.md#createServicePrincipal) <br> <br> Verify that the Key vault contains all the required secrets using the steps here: [Configure export to Azure Data Lake - Add secrets](configure-export-data-lake.md#addsecrets) <br> Verify that you have provided the correct Key vault URI in the configuration steps here: [Configure export to Azure Data Lake - Install add-in](configure-export-data-lake.md#installaddin) | 
| 425        | Failed to locate the Azure Tenant ID for the environment | Verify that you have provided the correct Azure tenant ID using the steps here: [Configure export to Azure Data Lake - Install add-in](configure-export-data-lake.md#installaddin)  | 
