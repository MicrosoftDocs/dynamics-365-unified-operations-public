...
| 410        | Failed to find Application ID to access the Data lake. Application ID provided is incorrect or can't be found      | Application ID provided in the key vault can't be found in Azure active directory. Validate the Application ID provided in the key vault by following the documentation steps [Configure export to Azure Data Lake - Create application](https://docs.microsoft.com/en-us/dynamics365/fin-ops-core/dev-itpro/data-entities/configure-export-data-lake#createapplication) |
| 411        | Failed to access data lake with given Application ID and Application secret | Application ID (app-id) and Application secret  (app-secret) provided can't be used to access the storage account. Verify the Application ID and Application secret are valid by following documentation steps [Configure export to Azure Data Lake - Create Application](https://docs.microsoft.com/dynamics365/fin-ops-core/dev-itpro/data-entities/configure-export-data-lake#createapplication) <br> Next validate that the application has required access to the storage account [Configure export to Azure Data Lake - Grant access](https://docs.microsoft.com/en-us/dynamics365/fin-ops-core/dev-itpro/data-entities/configure-export-data-lake#grantaccess). You need to contact the systems administrator or the LCS administrator who configured the system | 
| 415        | Failed to access the storage account using the storage name provided in the key vault | Storage account provided in the Key vault can't be found or is invalid. Validate that correct storage account name is entered into the key vault by using steps here: [Configure export to Azure Data Lake - Add secrets](https://docs.microsoft.com/en-us/dynamics365/fin-ops-core/dev-itpro/data-entities/configure-export-data-lake#addsecrets) <br> Verify that you have provided the correct secret name for the storage account by using steps here: [Configure export to Azure Data Lake Add secrets](https://docs.microsoft.com/en-us/dynamics365/fin-ops-core/dev-itpro/data-entities/configure-export-data-lake#addsecrets) | 
| 420        | Failed to access the key vault or the key vault secrets | Service can't access the key vault or the secrets in the key vault.  Verify that your Azure subscription is not expired <br> <br> Verify that you have created the service principal by following steps here: [Configure export to Azure Data Lake - Create service principal](https://docs.microsoft.com/en-us/dynamics365/fin-ops-core/dev-itpro/data-entities/configure-export-data-lake#createServicePrincipal) <br> <br> Verify that the Key vault contains all the required secrets using the steps here: [Configure export to Azure Data Lake - Add secrets](https://docs.microsoft.com/en-us/dynamics365/fin-ops-core/dev-itpro/data-entities/configure-export-data-lake#addsecrets) <br> Verify that you have provided the correct Key vault URI in the configuration steps here: [Configure export to Azure Data Lake - Install add-in](https://docs.microsoft.com/en-us/dynamics365/fin-ops-core/dev-itpro/data-entities/configure-export-data-lake#installaddin) | 
| 425        | Failed to locate the Azure Tenant ID for the environment | Verify that you have provided the correct Azure tenant ID using the steps here: [Configure export to Azure Data Lake - Install add-in](https://docs.microsoft.com/en-us/dynamics365/fin-ops-core/dev-itpro/data-entities/configure-export-data-lake#installaddin)  | 
| 430        | Failed to Failed to access the environment | Ensure that the environment is available and not in a deleted or inactive state  |
| 435        | Files in the Lake are corrupted or invalid | System has detected corrupted files or an invalid folder structure in your data lake. The system manages files and folders in the lake provided in the configuration. You should not modify files or the folder structure in the data lake yourself. Verify that your users or a process has not modified files or folders in the lake. You can reactivate the tables to see if the issue is resolved. If this does not address the issue, uninstall and re-install the Export to Dat alake add-in  |
