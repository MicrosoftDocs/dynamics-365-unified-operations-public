...
| 439        | System can't find enhanced metadata for the table you have chosen | You need to update the version of Finance and Operations to 10.0.28 (PU52) and manually refresh metadata using the **Manage > Republish metadata** option. You will need to wait a few minutes after this option to verify whether the issues is resolved.      |
| 440        | System can't find enhanced metadata in your environment | You will receive this error if you have chosen the "Enhanced metadata" feature. In this case, the system can't find enhanced metadata. To resolve the issue, ensure that you have installed finance and operations version 10.0.28 (PU52) or later. If you are on the correct version, you can also manually refresh metadata using the **Manage > Republish metadata** option. You will need to wait a few minutes after this option to verify whether the issues is resolved.  |
...
#### Error status codes 8xx indicate an issue with Finance and Operations database 
The Export to Data Lake feature uses Change data capture feature in Finance and Operations database. Error 8xx indicate an issue with Change data capture feature in the finance and operations apps database. This may be the result of a database maintenance operation or another issue impacting database operations. Service management team actively working on resolving this issue and no customer action is required. If the issue persists, you may contact Microsoft support for an update. 

#### Error status codes 9xx indicate an issue with Finance and Operations environment
Error codes 9xx indicate a system configuration issue with Finance and Operations environment. Service management team actively working on resolving this issue and no customer action is required. If the issue persists, you may contact Microsoft support for an update.
                                                                                                                                                                    |
