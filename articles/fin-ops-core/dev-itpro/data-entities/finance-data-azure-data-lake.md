...
| 440        | Enhanced metadata is not available for this table. | You will receive this error if you have chosen the "Enhanced metadata (preview)" feature. In this case, the system can't find enhanced metadata for the table that you have selected. To resolve the issue, ensure that you are on the required Finance and Operations platform versions required by the "Enhanced metadata (preview)" feature. If you are on the correct version, you can also manually refresh metadata using the **Manage > Republish metadata** option. You will need to wait a few minutes after this option to verify whether the issues is resolved.  |
