...
| DMFXsltEnableScript     | This flight applies to PU34 only and for non-production environments. A fix was made in PU34 to prevent scripting in XSLT. However, this resulted in breaking some functionality that was dependant on scripting. As a result, this flight has been turned ON in all production environments by Microsoft as a preventive measure. However, in non-production environments, this must be added by customers if they encounter XSLT failures related to scripting. From PU35 onwards, code change was made to revert the PU34 change and hence, this flight does not apply from PU35 onwards. Even if you had enabled this flight in PU34, upgrading to PU35 will not cause any negative impact due to this flight being ON from PU34.  |
| DMFExecuteSSISInProc     | This flight is OFF by default and it is related to a code fix that was made to run SSIS out of proc to optimize memory utilization of SSIS when running DIXF jobs. However, this change has caused a regression in a scenario where, if the DIXF data project name has a apostrophe (') in it, then the job will fail with an error. If you encounter this issue, removing the (') in the data project name will resolve the failure. However, if for some reason, the name cannot be changed, then this flight can be enabled to overcome this error. Enabling this flight will make SSIS run in-proc as before which could lead to higher memory consumption when running DIXF jobs.  |
