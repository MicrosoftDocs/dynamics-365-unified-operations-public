...
These three tables are all large cross company tables that don't allow record update and have large numbers of inserts daily. Tracking them can result in slow exports. There's never a need to
...
data cleanup job, use a larger number for number of hours it is allowed to run and run it daily. The default of two hours per day is often not enough. Six or eight hours allowed works better. It stops when it 
...
records that were exported for incremental export. These commands can be slowed if there is some process using the entity data at the times they run. Try to schedule exports for times when you aren't running reports against the same entity tables in your destination database.
