...
On the **Compatibility options** tab > **Flat file compatibility options**, the following parameters are available:

- **Enable header for fixed width file**:

  - Select **No** if the exported file doesn't require headers.
  - Select **Yes** if the exported file requires headers.

- **Validate file format with file extension**:

  - Select **No** if you always want to add files to the project when there's a mismatch between the file types.
  - Select **Yes** if you don't want to add files to the project when there's a mismatch between the file types.

On the **Compatibility options** tab > **XML file compatibility options**, the following parameters are available:
...
- **Attribute names in non uppercase for initial mapping (applies to simple entities only)**:
  ...
  - Select **Yes** to show Attribute(column) names in non uppercase letters. Then refresh the entity list.
    ...
    1. Select **Refresh the entity list**. On the **Modify target mapping** page, the data is shown in non uppercase letters.
    1. Create a new data project by adding an entity, or remove the entity and add it back to the data project. The exported file shows attribute names in non uppercase letters.
...
- **Ensure unique execution id**:
  ...
  - Select **No** to use the same execution ID for multiple export requests that you send in parallel.
  - Select **Yes** to use a unique execution ID for every export request.

- **Run data package export API synchronously**:

  - Select **No** to run the `ExportToPackage` API method as an asynchronous call to the exporter.
  - Select **Yes** to run the `ExportToPackage` API method as a synchronous call to the exporter.
