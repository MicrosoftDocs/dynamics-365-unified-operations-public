...
In some cases, you may see the status **Installation failed**. Failure to install is accompanied by an error code and a message. You should correct the reason for failure using the suggestions in the **Resolution** column below. To correct the issue, select the **Abort** option to uninstall the and follow steps to [install the add-in](#installaddin) again.
