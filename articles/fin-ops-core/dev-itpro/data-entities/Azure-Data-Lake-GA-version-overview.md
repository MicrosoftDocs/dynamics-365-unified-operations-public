...
When you enable this preview feature, you can get alerted when something important happens in the Data lake (ex. data is initialized in the lake). The system generates a business event when the initial copy of data is complete. [Business events](/powerapps/developer/data-platform/business-events) is a framework in Dynamics 365 and Dataverse that enables you to easily create intelligent actions and automations. For example, with [Power Automate](https://powerautomate.microsoft.com/), a tool integrated into Power Platform, you can build an automated workflow to perform actions such as auto-trigger a downstream data pipeline. 

We will add more and more business events to the service in the coming months. For more information on business events generated by Export to Data lake service see, [Business events generated by Export to Azure Data Lake service](Azure-Data-Lake-Generates-Biz-events.md).

## Enhanced Metadata (preview)
Along with Data, the Data lake contains metadata that describes the names, data type and size of data. Along with Data files in the lake, you will notice metadata files at a folder level corresponding to data files. When you install Export to Data lake feature and choose data to be added to Data lake, system writes metadata files in addition to data. If you choose the "Enhanced metadata (preview)" option when installing Export to Data Lake, the system adds even more metadata. For more information on metadata and enhanced metadata (preview) feature see [Data and metadata stored in Azure Data Lake](Azure-Data-Lake-Enhanced-Metadata.md)
