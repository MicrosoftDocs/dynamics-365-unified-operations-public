...
When you enable this preview feature, you can get alerted when data is initialized in the lake. The system generates a business event when the initial copy of data is complete. [Business events](https://docs.microsoft.com/en-us/powerapps/developer/data-platform/business-events) is a framework in Dynamics 365 and Dataverse that enables you to easily create intelligent actions and automations. For an example, with [PowerAutomate](https://powerautomate.microsoft.com/en-us/), a tool integrated into Power platform, you can build an automated workflow do perform actions such as auto-trigger a downstream data pipeline. 

We will add more and more Business events to the service in the coming months. For more information on Business events generated by Export to Data lake service see [...] 
