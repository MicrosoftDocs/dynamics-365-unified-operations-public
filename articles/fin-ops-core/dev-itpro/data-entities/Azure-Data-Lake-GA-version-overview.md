...
Data in the lake is stored as CSV files in a folder structure maintained by the system. The folder structure represents how the data is organized in Dynamics. For an example, you will see folders named "Finance", "Supply chain", "Commerce" etc. and within these folders you will see sub folders named "Accounts Receivable", "Accounts Payable" etc. Eventually, you will see folders that contain the actual data for a table. You will see one or more CSV files as well as CDM metadata files that describe the format of the data. 

When data in Finance and Operations gets updated, corresponding data rows in CSV files in the lake are updated. When a new row is added to a table in Finance and operations, or an existing row is deleted, new rows are added or deleted from corresponding CSV files. 

Similarly, when the data structures change in Finance and operations, for an example, when a new field is added to a table, metadata in the lake is updated to reflect the change. System may also re-populate the entire table folder (all the CSV files) in case the data structure change is a major one such as removal of a field.  
