...
- **Existing downstream/consumption pipelines can be preserved.** Using Synapse SQL serverless, a service that enables you to create T-SQL view definitions over data in the lake, you can read data in the lake in the same way as you read data from BYOD. Your data integration pipeline may be re-pointed to consume data in the lake - ie. the T-SQL endpoint provided by Synapse SQL serverless.  This way, you can transition from BYOD without incurring major costs. 
