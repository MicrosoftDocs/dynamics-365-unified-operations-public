...
>
> By enabling preview features, you agree to [Supplemental Terms of Use](../../fin-ops/get-started/public-preview-terms.md).
...
When you choose this option, the number of tables you can choose increases to 350 tables.

In addition, you can also update downstream data warehouses using changed data in the lake. Using the change data folder, you can easily identify the changes made to the data and create near real-time data pipelines.

For more information on near real-time change feeds, see [Change data in Azure Data Lake](azure-data-lake-change-feeds.md).

## Enable enhanced metadata in Data lake (preview)
This preview feature provides enhanced metadata stored in the Finance and Operations application layer (ie. Application Object Server, or AOS) available in the Data lake as Common Data Model metadata files. Enhanced metadata made available by this feature includes:

- Field names and descriptions present in Finance and Operations apps.
...
Metadata is stored in the lake using a machine readable JSON format known as the Common Data Model format. When using this preview feature, Common Data Model metadata files are enriched with enhanced metadata.
...
You need to provision an Azure Data lake in your own subscription. Export to Data lake service copies data into your own lake. You can query the data in your lake using Power BI, Spark, as well as many other tools from Microsoft and others.

Export to Data lake service is an add-on service. The Export to Data lake add-on service is included with your subscription to Dynamics 365 services.
...
You can use the tool provided by the FastTrack solution templates to estimate the cost of storage based on your business volumes in Finance and Operations. For more information, see the [Cost calculaor](https://github.com/microsoft/Dynamics-365-FastTrack-Implementation-Assets/tree/master/Analytics/CostCalculator).

### We are using BYOD service today, why should we transition to Data lake?

Customers use [Bring your own database (BYOD)](../analytics/export-entities-to-your-own-database.md) to extract data from Finance and Operations apps so that they can use that data for reporting or analytics. BYOD requires that customers provision and maintain an Azure SQL database to store data that is exported from Finance and Operations apps.
...
When you choose data, Export to Data lake service makes an initial copy of the data in the lake. If you choose multiple tables, the system makes the initial copy by taking 10 tables at a time. Depending on the size of the data and the number of records in the table, this may take a few minutes or even a few hours. You can see the export progress on screen.

Once the initial copy is done, the system continuously updates the data as changes happen in the Finance and Operations app. When new records are added, modified or deleted, data records in the lake are added, modified, or deleted accordingly.

Using the optional, near real-time change feature (currently in preview), data in the lake is updated within minutes of a change in the Finance and Operations environment.

If you do not use the near real-time update feature, data in the lake is updated within a few hours of a change in the Finance and Operations environment.

Export to Data lake form in a Finance and Operations environment shows the last time stamp the data was updated in the lake. The system also adds additional data fields that help you identify the time the data was updated in the lake. Using these time stamps, your downstream processes can detect and process data as they change in the lake.

### Why can’t I see "Export to Data lake" feature in my environment?

The Export to Data Lake feature is available in the **United States, Canada, United Kingdom, Europe, South East Asia, India, East Asia, Australia, and Japan** regions. If your Finance and Operations environment is in any of those regions, you can enable this feature in your environment by using Microsoft Dynamics Lifecycle Services (LCS). This feature will be available in more regions in the future.
...
You can join the [preview Yammer group](https://www.yammer.com/dynamicsaxfeedbackprograms/#/threads/inGroup?type=in_group&feedId=32768909312&view=all), where you can attend weekly online "office hours" meetings and use the Yammer online forums to stay in contact and ask questions.
