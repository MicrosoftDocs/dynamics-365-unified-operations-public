...
As table data in the data lake updates, you must keep the corresponding fact tables and dimension tables in the data lake updated. Otherwise, your reports don't reflect the latest data.

The easiest way to update fact tables and dimension tables is to periodically create a full copy by using tables. However, this approach can be inefficient. If your tables are large (for example, if they have tens of millions or hundreds of millions of rows), the process of updating a fact table by making a full copy might take hours and might consume lots of compute resources. Therefore, your users might not have the reports in time (that is, they might have to wait hours to see the latest data in reports). Because compute resources are consumed every time that data is reprocessed, you might receive a larger bill from the services that you consumed.
...
The Change feed feature relies on a SQL Server feature named Change Data Capture (CDC). CDC is the native approach to capturing change data in a SQL Server database. This database is the data store behind finance and operations apps. The Change feed feature lets you access the CDC change sign in your data lake.

The following illustration shows how change feeds work in finance and operations apps.

:::image type="content" source="media/Change-feed-overview-picture.png" alt-text="Screenshot of how change feeds work in finance and operations apps.":::
...
1. **Export to Data Lake** microservices capture the changes in the database and write the change log to the customer's data lake. **Change feed** folders in the data lake are organized by table. Each folder contains the change log for a specific table.
1. In addition, in the **Tables** folder, each row that changed also contains several new fields. Each row contains the **LSN** value of the corresponding change record and the **Change Date-Time** value. Although you can use the **LSN** and **Change Date-Time** fields in the table folders to identify whether a row changed, they contain only the latest change. If the same row changed multiple times, only the latest change is shown in the **Tables** folder.
1. Changes are written to the **Change feed** folder in the same sequence (that is, LSN) in which they're committed in the database. Change feed files are written in batches in the data lake, so that their size is optimized for reading. After change feed files are written, they're never updated.
...
When you add a table to a data lake, or when you activate a table that you previously deactivated, the system makes an initial copy of the data in the data lake. At this point, the table's status is **Initializing**. When the initial copy finishes, the system changes the status to **Running**. When the table is in **Running** status, changes in the finance and operations database are reflected in the data lake, and change feeds are added. The **Change feed** folder might be empty if there are no changes to the table since it was initialized. 

To access change folders, open the Azure portal, and find and select the storage account that's associated with your finance and operations environment. You should see the **Change feed** folder in the data lake folder structure. The following illustration shows an example.

:::image type="content" source="media/Change-feed-folders-rootfolder-top-level.png" alt-text="Screenshot of Change feed folder in the data lake folder structure.":::

When you open the **Change feed** folder, you see folders that correspond to the tables that you added to the data lake. You also see CDM metadata files that describe the change folder data. The following illustration shows an example.

:::image type="content" source="media/Change-feed-folders-table-level-with-metadata.png" alt-text="Screenshot of folders and CDM metadata files in the Change feed folder.":::
...
:::image type="content" source="media/Change-feed-folders-examine-metadata.png" alt-text="Screenshot of CDM metadata file opened in a text editor.":::
...
:::image type="content" source="media/Change-feed-folders-change-data-format2.png" alt-text="Screenshot of format of changes in change folders.":::

| Field name                    | Contents |
|-------------------------------|----------|
| Start\_LSN                    | <p>This field identifies the LSN of the transaction that changed source data in the finance and operations database.</p><p>**Note:** The **Start\_LSN** value isn't enclosed in double quotation marks in CSV files. It's a hexadecimal value, as represented in the SQL Server database. Example value: **0X00011E9F00000FB00001**.</p> |
...
| List of fields and values    | The remaining columns provide a list of fields that are present in the table, together with the values. Use the update mask to identify fields that changed as part of the transaction. |
| LastProcessedChange\_DateTime | <p>This field provides the value of the CDC **Change Date-Time** field from the finance and operations database. The date and time is expressed in Coordinated Universal Time (UTC), per ISO 8601.</p><p>Example value: **"2020-08-24T05:26:03.8622647Z"**. Notice that this value is enclosed in double quotation marks. It includes the default seven digits of precision after the second value, and a *Z* that signifies UTC.</p> |
...
## Best practices when using change feeds

Change feeds are a powerful feature that the **Export to Data Lake** feature in finance and operations apps enables. This section describes some best practices to follow when using change feeds.

### Updating near-real-time data marts

If your data warehouse or data marts need to update in near-real time (in other words, they must update within minutes of a data change in finance and operations apps), use change feeds.

However, you need to understand several important concepts:

- Change feeds group change records into files that are around 4 megabytes or 8 megabytes (MB) in size. Microsoft optimized these file sizes to provide the best query response times when Synapse SQL Serverless queries the files. Optimized file sizes and batched writes also reduce the Azure charges you might incur as the data lake updates.
- Change feeds only append change records. The process never updates files in the **Change feed** folder. Although each change record contains the LSN number and date and time of a change, you can also use the date and time stamp of the CSV files to identify changes.
- When you reactivate a table in finance and operations, the process clears the **Change feed** folder and starts change feeds from the next available change. This behavior ensures that the changes are consistent with the **Tables** folder. When you reactivate tables, consider triggering a full refresh of the downstream data pipeline.
...
If your users expect data marts to update daily or several times per day, triggering a full refresh might be an economical alternative, especially if the volume of data is low or moderate. You can also choose to update smaller tables as a periodic full refresh but create near-real-time pipelines for large, frequently updated tables.

### Changing feeds to audit and verify master data updates

The **Change feed** folder is an exact replica of the CDC change logs that the finance and operations database maintains. Changes that you make to master data in finance and operations apps are reflected in CDC. Therefore, by extension, they're also reflected in change feed folders in your data lake.

You can use reports that you build over the **Change feed** folder to audit and verify master data changes in the system.
...
Because tables continue to add changes while they're in **Running** status, change feed folders continue to grow in the data lake. However, the cost of retaining data in the data lake is a fraction of the cost of a SQL database. Therefore, the cost of growing data might not be a major concern.

If you want to reduce the amount of data that is stored in your data lake, you can periodically delete the change log from the data lake. For example, you can run a job that deletes change log files that weren't modified for 90 days or 180 days.
