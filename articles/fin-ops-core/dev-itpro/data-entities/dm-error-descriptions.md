...
This error is caused by invalid mapping generated by the package. Review the mapping configuration and try uploading the package directly to AOS to debug the mapping issue. Ensure that the mapping is generated correctly when uploading it directly to AOS using Data Management.
...
This issue is likely caused by duplicate label usage for entities. If two entities are assigned the same label, the first one encountered during the refresh retains the label as its name, while the second one is renamed to match the underlying entity object. Ensure each entity has a unique label to resolve this error. If the issue persists and an entity is missing, it might be related to the setup of role-based privileges. For more information, see [Security and data entities](security-data-entities.md).

### 9. File upload fails with "Query validation failed - External table isn't in the expected format."

Correct the formatting of the Microsoft Excel file or rename the sheet to remove any special characters. This exception can also occur when attempting to upload Excel sheets labeled as **Confidential**. This behavior is expected, as uploading confidential files results in a permission exception. Change the label to something other than **Confidential** and try uploading the file again. We recommend using a different file format, such as CSV, instead of Excel.

### 10. Import fails with "Number sequence isn't set."

Ensure that the number sequence reference is set before using the autogenerate number feature during import.
...
Verify the data in the input file used for import. Check if null, empty, or duplicate data is provided for columns that are mandatory for the data entity being used. If any rows contain missing or invalid data for those columns, correct the data and retry the import.

### 13. Schema mismatch in AxDb Table and BYOD table.

If there are updates to the schema of an entity, refresh the entity to ensure that the AxDb reflects the updated schema.
...
| DMF1958 | Invalid enum value provided in the input file used for import. | Please check data for enum value and correct it before retrying the import. |
...
| DMF1987 | <p>This error can have two causes:</p><ol><li>A failure occurred because of a table mapping issue.</li><li>A new field/column was added without republishing the entity.</li></ol> | <ol><li>To identify the problematic field, remove all fields from the mapping, except key columns, and then do an export. If the export is successful, incrementally add a few columns at a time and reexport until you find the problematic column.</li><li>After you deploy changes, refresh the entity list, and republish the entity.</li></ol> |
...
| -1071607683 | This is a SSIS execution error code. This indicates constraint related error in the data provided in input file for example: duplicate data, empty or null values, mismatched data type. | Check the data present in file. |

## Known limitations

- **Data management (DMF/DIXF) export to Excel has a 255-character limit:**
  The Data management (DMF) export job truncates data that has the Excel file type if there are more than 255 characters. This limitation is a known SSIS limitation. Data that has more than 255 characters is truncated when you export to the Excel file type. For more information, see [Export long text values](/sql/integration-services/load-data-to-from-excel-with-ssis#export-long-text-values). To export data by using the Data management (DMF) export job, use a different file type.

- **Failure/Slowness in Import/Export Using Excel:**
Projects using Excel files may experience delays while waiting for access to the Excel driver. If access isn't obtained within a 10-minute period, the operation fails. Slow exports or imports using Excel may also cause other scheduled Excel import/export jobs to fail. To avoid this issue, we recommend either not scheduling parallel import/export jobs that use Excel or choosing a different source format.

- **Long String Values Truncated on Import:**
SSIS examines import data to estimate certain properties, such as the maximum length of string values. To optimize performance, SSIS only evaluates the first few rows of the file. If a column contains a much longer value later in the file, that value may be truncated. To address this, locate the row with the long value and either reimport it separately to replace the truncated value or move it to the beginning of the file so SSIS can allocate enough space for the string.

- **Values Truncated or Missing When Importing from Excel:**
The Excel driver used by SSIS infers data types from the first few rows of the file. If subsequent rows contain values that deviate from this schema, the values may be truncated or ignored. To mitigate this, consider using a different file format, such as CSV, which is less prone to these issues.

- **Exporting Large Numbers of Records with Excel:**
There's a known limitation in SSIS when exporting large datasets to Excel. These limitations can lead to performance bottlenecks, file size issues, or row count restrictions depending on the Excel version. We recommend using alternative formats, such as CSV, which are better suited for handling large volumes of data efficiently.

- **Performance Issues in Incremental Export for Certain Entities:**
Incremental exports can encounter performance issues with specific tables, such as DimensionAttributeValueCombination, DimensionAttributeValueSet, InventDim, and InventSum. These tables often have a high volume of records added in short periods. If your incremental export involves these tables, we recommend excluding them from tracking. If tracking other tables is still required, implement a defaultCT query that excludes these problematic tables.

- **Large Import/Export Using Sync Mode on AOS:**
Import/export jobs that can complete within 30 minutes should be run in sync mode on AOS. For jobs expected to take longer, we recommend using batch mode instead of sync mode to ensure stability and performance.



> [!NOTE]
> When doing Data management export to file and downloading to file, an extra period(.) is appended to the end of file name.
> 
