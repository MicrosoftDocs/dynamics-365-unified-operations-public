...
This issue is likely caused by duplicate label usage for entities. If two entities are assigned the same label, the first one encountered during the refresh retains the label as its name, while the second one is renamed to match the underlying entity object. Ensure each entity has a unique label to resolve this error. If the issue persists and an entity is missing, it might be related to the setup of role-based privileges. For more information, see [Security and data entities](/dynamics365/fin-ops-core/dev-itpro/data-entities/security-data-entities.md).
