...
### 6. Error - The number of input columns for OLE DB Destination. Inputs[OLE DB Destination Input] cannot be zero.
...
### 8. Import fails after entity refresh with error The mapping is incorrect for entity and field or Data entities missing and/or renamed after entity refresh or Cannot create a record in Refresh entity list.

This issue is likely caused by duplicate label usage for entities. If two entities are assigned the same label, the first one encountered during the refresh will retain the label as its name, while the second one will be renamed to match the underlying entity object. Ensure each entity has a unique label to resolve this error. If the issue persists and an entity is missing, it may be related to the setup of role-based privileges. Refer to the following guide: https://learn.microsoft.com/en-us/dynamics365/fin-ops-core/dev-itpro/data-entities/security-data-entities

### 9. File upload fails with "Query validation failed - External table is not in the expected format."

Correct the formatting of the Excel file or rename the sheet to remove any special characters. This exception can also occur when attempting to upload Excel sheets labeled as "Confidential." This behavior is expected, as uploading confidential files results in a permission exception. Change the label to something other than "Confidential" and try uploading the file again. We recommend using a different file format, such as CSV, instead of Excel.

### 10. Import fails with "Number sequence is not set."

Ensure that the number sequence reference is set before using the auto-generate number feature during import.
...
Steps to refresh the entity: Go to FnO > Data Management > Framework Parameters > Entity Settings > Refresh Entity List.
...
Projects using Excel files may experience delays while waiting for access to the Excel driver. If access is not obtained within a 10-minute period, the operation will fail. Slow exports or imports using Excel may also cause other scheduled Excel import/export jobs to fail. To avoid this issue, we recommend either not scheduling parallel import/export jobs that use Excel or choosing a different source format.

- **Long String Values Truncated on Import:**
SSIS examines import data to estimate certain properties, such as the maximum length of string values. To optimize performance, SSIS only evaluates the first few rows of the file. If a column contains a much longer value later in the file, that value may be truncated. To address this, locate the row with the long value and either re-import it separately to replace the truncated value or move it to the beginning of the file so SSIS can allocate enough space for the string.
...
There is a known limitation in SSIS when exporting large datasets to Excel. These limitations can lead to performance bottlenecks, file size issues, or row count restrictions depending on the Excel version. We recommend using alternative formats, such as CSV, which are better suited for handling large volumes of data efficiently.
