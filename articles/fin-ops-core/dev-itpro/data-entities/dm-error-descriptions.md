...
This article documents the scenarios when a specific error is seen. These scenarios aren't a complete list of errors and scenarios, however, this list is continuously updated so keep checking back for updates. 
...
When you use recurring imports (enqueue API), if the files are sent to the end point at high frequency. When sequential processing of messages isn't enabled, data management tries to process the files in parallel. When files are processed in parallel, and multiple files have the same record, multiple threads try to update the same record at the same time. You must update the data so that the same records don’t repeat across files. If the same records aren't in the multiple files, the entity is expected to handle such cases. To mitigate the issue, sequentially process the files. If this isn't a data issue and the entity isn't expected to process in parallel, this entity isn't subjected to parallel processing. Enable sequential processing of messages in the recurring job.

## There are fields, which are not mapped to Entity \<EntityName\>

It's a common practice to use the export functionality to generate the entity template file that can be later used for imports. While exporting the template, in fixed width format with 'First row header' set to 'No' (in source data formats set up), the exported template doesn't have the column names. When this file is imported, it results in this error. 

## Data package download - Error downloading data package for job ''. Record for ID - {GUID} not found

One of the scenarios is when the dev environment, points to the database in another environment, such as UAT, and the export job is run from the source environment. The exported file gets uploaded to the blob storage that is associated with the source environment (dev, in this example). This job shows up in the target environment (UAT) since the database is shared. If you try to download the exported file using the **Download file** option, this error displays because the file doesn't exist in the blob storage of the target environment (UAT) from where you are trying to download.

## XML is not in correct format-Exception from HRESULT: 0xC0010009

This message covers all XML formatting issues in the file. For example, the data project has mappings for columns that don't exist in the file that's being used for the operation. This error happens if certain columns were removed from the file and this file is now used. Either fix the mapping in the data project or fix the file to have all the columns as expected.

## Error while uploading a file during export

When running an export on a development environment, an error occurs when the export file can't be uploaded. To resolve this issue, install the latest Azure Storage emulator, restart the virtual machine (VM), and rerun the export job. The storage emulator can be installed from [Azure Storage Emulator](/azure/storage/common/storage-use-emulator).
...
|DMF030|	Error adding or updating OLEDB source properties due to incorrect field mapping.|	The problematic fields can be identified by removing all fields from the mapping except key columns and exporting. If export was successful, add back other fields a few at a time and repeat until you find the problematic field. Don't map any virtual fields if you selected skip staging. Entities with duplicate labels may cause this error. Only one entity can be assigned a particular label as the entity name, if you have more than one entity using the same label it causes an error.|
|DMF040|	Excel query failed. The system can't retrieve the column names or the worksheet name. The system automatically restarts any Data management instances that raise this error frequently.|	Consider changing the file type for the import project to something other than Excel. |
|DMF042	|The Data management service is unreachable and can be due to patching or deployment in progress.	|This is usually a transient failure. Try the import/export job again. If the error occurs consistently, contact support.|
|DMF043	|Package execution failed and when the error pattern doesn't match a known error code.	|Check the execution details for the particular execution ID for the specific error message. Depending on the problem, you may need to contact support.|
...
|DMF1963	|Composite entity import file isn't in XML format. Only XML files are used for composite entity import. Verify the import file isn't empty and is in XML format.|
...
|DMF1973|	Publish entity failed.|	Check details in the error/execution log. If there's insufficient information in the log, contact support.|
|DMF1987	|This can be caused by: 1 - Failure due to a table mapping problem. 2 - Adding a new field/column without republishing the entity. |1 - The problematic field can be identified by removing all fields from the mapping except key columns and doing an export. If this is successful, incrementally add a few columns at a time and re-export until you find the problematic column. 2 - After deploying changes, refresh the entity list and republish the entity.|
|DMF1995|	Sql login timed out. Imports or exports with this error automatically retry.	|For database export, specify a longer than default connection timeout on the connection string. The default timeout is 30 seconds and can be set to be longer.|
|DMF1996	|Deadlock. There are concurrent imports for the same project interfering with each other. |	If using auto generated numbers for import, keep the file size low to avoid locking the staging table. If the error is consistent for a data project, adjust the scheduling of the project to avoid concurrent execution.|
|DMF1997|	AcquireConnectionsError. SSIS was unable to connect to the database. Imports or exports with this error automatically retry.	|For database export, confirm the BYOD(Bring your own database) connection string still works and try again. If this error occurs frequently, contact support.|
...
|DMF2017	|Error reading package executor results.|	View the execution details for logs. If there's not enough information in the log, contact support.|
...
|DMF2051|	Publish entity failed as CCI index isn't supported on the database tier.	|CCI index isn't allowed on some smaller azure database sizes. Turn off the use CCI index option where the connection string is entered, or use a higher DB tier. If use CCI is OFF, the staging table index is used for the destination table key.|
...
|DMF2055|	InvalidXMLCharacterErrorCode. |	Control characters other than tab, aren't allowed in xml files. If exporting data contains a control character like backspace, the export fails.
Remove any control characters from the data or use a different file type for the export. The memo field usually has these values where the data was originally set from email or other automated process. Backspace is the most common character causing this error.|
