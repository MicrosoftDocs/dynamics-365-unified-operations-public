...
3. **Copilot for finance and operations apps:** Adding the Dynamics 365 ERP MCP server as a tool in the Copilot for finance and operations apps agent, enabling it for use with the sidecar chat panel in the client, isn't yet supported. Although you are not blocked from adding the MCP server to the agent, you may experience errors in the execution. Microsoft support doesn't guarantee assistance for resolving errors or issues for these scenarios.
