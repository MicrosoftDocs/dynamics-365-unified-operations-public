...
This approach enables seamless orchestration between finance and operations apps and Microsoft Copilot Studio agents, so you can extend business logic beyond the boundaries of the Dynamics 365 application. Whether the event is workflow-based (for example, approvals) or nonworkflow (for example, confirmations), you can harness it to drive real-time, contextual automation. By using low-code plugins, developers and solution architects can configure these triggers with minimal effort, ensuring scalability and maintainability across business scenarios.
...
In this scenario, you add a trigger capability to an agent. Once the business event conditions are reached (a purchase order is confirmed), the agent triggers and sends an email or Teams message to the manager of that company referring the purchase order is created, sending all relevant data via email.
...
1. Select **Create Trigger** once you define all the options. The trigger is created for this specific purpose.
...
In this step, you add tools so that the agent can communicate through email and Teams with a specific, predefined person.
...
1. Select **Next**, and then select **Add to agent** for this specific tool. The option for your agent to send emails is available on its toolkit. Now you configure this specific tool.
...
1. For this example, keep the inputs **To**, **Subject**, and **Body** set to **Dynamically fill with AI** so the agent understands what information to add for each field. You can customize it by changing the dropdown to **Custom Value**.
...
1. Select **Next**, and then select **Add and configure**. It creates the connection for Teams and you're redirected to the settings option.
...
1. Leave the default values for the remaining options, and select **Save**.
