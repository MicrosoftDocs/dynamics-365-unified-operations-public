...
Every element in a model must have a name that's unique across all models at installation time. However, at installation time, you don't know the names of all the models that your model might be installed together with. To accommodate this situation, include a prefix that's specific to your solution in every element name. By including this prefix when you name elements in your model, you significantly reduce the risk of naming conflicts.

+ If a model contains multiple solutions, each solution in the model can be identified by a different prefix.
+ Choose the prefix carefully to minimize the risk that other models from other parties use the same prefix for their elements.

When you extend functionality in other models, elements that you're extending already contain a prefix. However, don't add your prefix to the extension elements, so that the names don't include multiple successive prefixes. Instead, include your prefix or another term or abbreviation as an infix when you name extension elements.

## Naming extensions

An extension element, such as a table extension, view extension, or form extension, must have a unique name that minimizes the risk of conflicts with extensions in other models. To minimize the risk of conflicts, include a term, abbreviation, or infix that distinguishes the extension from other extensions to the same element in other models.

+ Include either the name of the model where the extension element resides or the prefix that the extension is associated with. For example, a Warehousing module extends the HCMWorker table and uses the **WHS** prefix in the name of all other elements. In this case, the extension might be named **HCMWorker.WHSExtension**. The prefix that is used to name other elements in the module is inserted as an infix in the name. As another example, an extension of the ContactPerson table in the **ContosoCustomizations** model might be named **ContactPerson.ContosoCustomizations** if the extension is intended to contain all extensions to the ContactPerson table from the **ContosoCustomizations** model. The developer tools default to using the model's name as the extension name, since the model name is already required to be unique.
...
Extension classes that augment the logic on tables, classes, or other elements must have a name that's unique across all types in all models. Preferably, the extension class name includes the name of the type that you're extending. However, the name must also include a term, abbreviation, or prefix that distinguishes the class from other types.

+ Start the name of the extension class with the name of the type that you're augmenting, and end the name with the term **\_Extension**.
Therefore, an extension class that augments the ContactPerson table starts with the name **ContactPerson** and ends with **\_Extension**. For example, one extension class might be named **ContactPersonWHS\_Extension**.
+ Include either the name of the model where the extension element resides or the prefix that the extension is associated with. For example, a Warehousing module uses an extension class to augment the ContactPerson table and uses the **WHS** prefix in the name of all other elements. In this case, the extension class might be named **ContactPersonWHS\_Extension**. The prefix that is used to name other elements in the module is inserted as an infix in the name. As another example, an extension class that augments the ContactPerson table in the ApplicationSuite model might be named **ContactPersonApplicationSuite\_Extension** if the extension class is intended to contain all extensions to the ContactPerson table in the ApplicationSuite model.
...
Fields, field groups, indexes, relations, and metadata elements that you add in extensions must have a name that's unique across both the element that you're extending and other extension elements. Therefore, **include a prefix** that minimizes the risk of conflicts across models. In addition, use clear terms and abbreviations so that these artifacts can be easily understood.
...
Variables and methods that you add in extension classes must have a name that's unique across both the type that you're extending and all other extension classes that extend the same type. Create unique and readable names for variables and methods. When you can't create a unique name, add a prefix to minimize the risk of conflicts across models.
...
+ When you can't create a unique and readable name, add a prefix, term, or an abbreviation at the beginning of the member variable or method name. For example, an approving worker class-level member variable might be named **whsApprovingWorker** if **WHS** is one of the prefixes that other elements in the model use. An **approveWork** method might be named **whsApproveWork** if **WHS** is a prefix that other elements in the hosting model use.

+ Avoid generic names, because the risk is high that multiple extensions could be use the same term or that the base functionality would be enhanced with an identical name in a future release. Some examples of names likely to collide are **Approver**, **Delay**, **Group**, **Lookup**, and **Process**.
