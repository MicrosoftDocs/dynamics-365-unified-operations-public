...
The scope of a number sequence defines which organization uses the number sequence. The scope can be **Shared**, **Company**, **Legal entity**, or **Operating unit**. You can combine the **Company** and **Legal entity** scopes with fiscal calendar periods to create even more specific number sequences. You can add new number sequence scopes through extensions.  
...
1. Create an extension class for **NumberSeqScope** class.
   ...
   1. Create a post handler for the **NumberSeqScope::getId** method and add logic for the new scope type, so the corresponding record can be found (or created if it doesn't exist) in the **NumberSequenceScope** table. 
...
1. Create an extension class for the **NumberSequenceDetails** form.
   ...
   1. Create a post handler for the **updateScopeControlValues** method to update the values of the controls in the **Scope** tab.
