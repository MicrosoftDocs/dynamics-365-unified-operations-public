...
- Fixes an issue where the local agent hangs if the file share is unavailable.
- Fixes an issue where the installers can hang during a deployment operation.
...
- Adds a new feature to manage Service Fabric dynamic node tags. This feature requires some additional setup that version 2.20.0 of the infrastructure scripts provides.
...
- Fixes a bug where the local agent can't clean up the workspace directory because the directory contains files with long paths.

### What's new in local agent 3.2.1

- Fixes bugs that prevent the local agent from downloading artifacts correctly from Azure Storage.
- Upgrades the Azure Storage libraries.
...
- This version upgrades the Azure Storage libraries to the latest version. The checkpointing functionality isn't available anymore, but there's now automated retry functionality that you can customize. The product team might bring back checkpoints if the Azure Storage libraries support it again.
- Improved artifact management logic, so the agent doesn't download existing artifacts again.
- Added file hash validation to ensure artifacts in the artifact store match exactly what is in the Lifecycle Services artifact store.
...
- This version introduces a new deployment option to specify that an environment should be configured to work with the Regression suite automation tool (RSAT). For more information, see [Deployment options](../../dev-itpro/deployment/onprem-localagent-options.md).
...
- Local agent 2.7.1 introduces a new deployment option to [Specify that checking the Certificate Revocation List of a certificate should be skipped](../../dev-itpro/deployment/onprem-localagent-options.md).
- This release fixes a bug where the **office365AdfsCompatibility** deploymentOption wasn't set correctly.
...
- Local agent 2.6.0 uses a new Service Fabric SDK and runtime.
- This version fixes a bug where, if refresh state is triggered when the environment is stuck in the Downloading phase, the environment automatically moves to a deployed state without updating the environment. In this situation, the refresh state operation marks the downloading phase as failed.
- The timeout for provisioning an application is increased.

> [!IMPORTANT]
> This version is only compatible with 7.x Service Fabric clusters.

### What's new in local agent 2.5.0

- Local agent 2.5.0 uses new versions of various dependencies. The main changes are Service Fabric and Entity Framework.
- This version also fixes a bug where, if cleanup fails without cleaning up any services, subsequent reattempts always fail during cleanup.

### What's new in local agent 2.4.0

- Local agent 2.4.0 now requires the .NET Framework version 4.7.2 to use the newest changes from Lifecycle Services. To meet the newest requirements, be sure to run the latest infrastructure scripts that are available in Lifecycle Services.
- This version also fixes an issue where the deployment of the AXService fails in slower environments due to a hard-coded timeout.
...
- This version removes the monitoring agent and introduces a new service called LBDTelemetry, that's used to install the ETWManifests.

> [!IMPORTANT]
> This version requires that you download a new local agent configuration file from Lifecycle Services. For more information, see [Troubleshoot on-premises deployments](../../dev-itpro/deployment/troubleshoot-on-prem.md).
