...
1. Make sure that the replication feature is installed and enabled for the source SQL Server instance. To determine whether replication is enabled, run the following SQL script.
    ...
    If the replication components aren't installed, follow the steps in [Install SQL Server replication](../../database-engine/install-windows/install-sql-server-replication?view=sql-server-ver16) to install them.
...
Follow all previous steps to migrate data, but follow this document: [Golden configuration promotion - Copy the sandbox database to production](https://learn.microsoft.com/en-us/dynamics365/fin-ops-core/dev-itpro/database/dbmovement-scenario-goldenconfig#copy-the-sandbox-database-to-production) 
