...
| Get   indexes            | Yes | Customers will be charged based on storage. | 
| Get   lock details       | Yes | Not applicable anymore.<br><br>The platform is responsible for optimizing the database's workload and handling any blocking that may occur.   The platform is also resposible for intermitted connectivity issues and provides retries to avoid any concerns with such actions.   | 
