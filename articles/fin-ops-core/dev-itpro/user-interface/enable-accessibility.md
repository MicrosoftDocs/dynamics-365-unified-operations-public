...
Accessibility is about inclusion. A person with a disability can perform the same task as a person without that disability. Making an accessible control or form should be as fundamental as making it secure, high-performing, or easy-to-understand.

## Keyboard

The bedrock of accessibility is keyboard-only access. When you can use the keyboard to perform all the actions of a form, a nonsighted person or a person with restricted or limited use of their hands can use it. By using a keyboard, you can reach all controls through the tab sequence, direct action (such as **Ctrl+S** for Save), or through some other shortcut key that enables you to move to a control, such as Navigation Pane, App Bar, Message Center, or Message Bar. A simple test is to disconnect your mouse and complete all core and secondary scenarios by using only the keyboard.

## Color

Use color to express state or status of a record or other piece of information. However, don't use color as the only way to communicate state or status. An accompanying symbol, help text, or additional column should include a textual description of the state or status. A simple test is to identify all use of color in your system and ensure that you aren't using color to express a state or status. A common example is to use the color red to indicate "needs attention," or the color green to mean an "OK" status.

## Images

When you display an image, include a label that describes the image. If the image expresses state or status of a record, then accompanying help text or an additional column should include a textual description of the state or status. If the image is symbolic, like a logo, it doesn't require a textual description. If you have an image on a form or grid to convey a status, such as "in progress," ensure that the image has a tooltip that can be read to someone who is using a screen reader.
...
Extensible controls are extensions of the controls that the framework provides, but they require the same usability and accessibility. In addition, widgets or other visually rich controls, such as segmented entry controls or charts, should provide an Accessible Rich Internet Applications (ARIA) tag that is hidden from the sighted user, but provides an introduction to the blind user through a screen reader that describes the use of the control. Every action in an extensible control must be possible by using a keyboard.
...
|---|--------------|----------------------|-------------------|
| Keyboard-only access to all functions | Drill downs, lookups, hyperlinks, data input actions, and shortcut keys.         | All actions can be completed without the use of a mouse.     | |
| Visible focus indicator       |    | It’s always apparent which control has focus.               | |
| Focus order           | Can't “jump to” a nonlogical location.           | Tabbing doesn’t jump to a nonlogical or unexpected portion of the form.  | |
| Focus trapping                | Can't be “trapped” in a control, the focus must be able to move out of control by using the keyboard.            | Tabbing into a control should allow tabbing out or a keyboard equivalent that lets them escape.             | |
...
| User input instructions       | Widgets or other multistep controls must have a usability overview label or help text (can be embedded in ARIA tag).            | When someone uses a screen reader, the control must be introduced and its label or purpose described.              | |
...
| Fine motor control            | Can't require use of the mouse. Must support “sticky keys.”             | User can't be forced to click a moving target (pull right) or any other action that requires a mouse. The same action must also be possible by using the keyboard.             | |
...
| Communication                 | Use of VoIP or other telecommunication device or software, such as Skype, must be accessible and usable by using the keyboard.       | Any use of VoIP or telecommunication is a legal obligation.         | |
...
An extensible control is simply an extension of the framework. Consider the interaction of the extensible control the same as any other framework control. When the control is a visually rich widget that offers mouse actions, the author needs to ensure that equivalent keyboard access functionality is available. A widget can't run in an “accessible mode” that is different from the standard presentation. Instead, the widget must offer similar functionality without the need to activate or toggle state of the control. All uses of color should be based on themed colors so that when the mode is “High Contrast” your color scheme matches the theme change. The World Wide Web Consortium (W3C) website provides guidance for [Supported States and Properties](https://www.w3.org/WAI/PF/aria-1.1/states_and_properties). This site is a helpful resource for ARIA tags and provides the definitions listed in this article.
...
> [!IMPORTANT]
> Your control shouldn't only identify itself by name, but (using a label or ARIA tag) give a brief introduction on how it works. Make sure your descriptive text is supplied via a framework label for localization.

- *aria-describedby* - Identifies the element or elements that describe the object.
...
It might not always be clear to the visually impaired user why the control isn't responsive. Providing a busy message helps in these cases.
...
#### Indicate that the contents are validated or invalid
...
You can learn complex interactions, but you might not always know the current state without trying them out. When you use an *aria-expanded* tag, the control describes its current state. An example is tabbing to a tab or FastTab section of a control.
...
- *aria-live* - Indicates that an element is updated, and describes the types of updates the user agents, assistive technologies, and user can expect from the live region.
