...
Actions are an essential component of any enterprise resource planning (ERP) system. Users can access actions through various mechanisms:
...
In general, actions that users trigger by using right-click context menus or keyboard shortcuts have a corresponding button available elsewhere in the user interface. Users can trigger action controls by using touch or a mouse click. Many system-provided actions can also be triggered by using the keyboard.

## Buttons

Buttons are the foundation of action controls. You can model them inside standard Action Panes or in Toolbars, which are discussed later in this article. You can also add them as stand-alone buttons on the page. For example, the **OK** and **Cancel** buttons at the bottom of a dialog box, or buttons for actions that are specific to an individual field. The following button types are available:

- A **button** is a basic button, for which you must implement the entire functionality in code.
...
In general, buttons take advantage of the same properties as the buttons in previous versions. The following sections discuss a few properties that are related to button visualization, with particular focus on changes from previous versions.

### Button display

The **Button display** property controls what information (including the button label and image) appears on the button. The allowed values for this property depend on where the button is located (for example, inside an Action Pane). Here are some of the location restrictions on the **Button display** property:

- Set buttons inside **Menu** buttons to **Text only** or **Auto**. The **Auto** value is interpreted as **Text only** in this case.
- Set buttons inside **Action Pane** tabs on **Standard Action Panes** to **Text only** or **Auto**. The **Auto** value is interpreted as **Text only** in this case.
- Use **Image only** buttons only for on-canvas buttons that are inline with a field.
...
In previous versions, users often saw images or icons on buttons to help them recognize those buttons. In the current version, the number of images that are used for this purpose is drastically reduced. Fewer images produce a cleaner, more modern user interface. Additionally, there's a desire to indicate processes and tasks by using more common symbols instead of multiple subtly different images. For more details about how images are used on buttons, see the "Button image guidelines" section of [General form guidelines](general-form-guidelines.md).

Two metadata properties define an image for a button: **Image Location** and **Normal Image**. The allowed values for the **Normal Image** property depend on the value of the **Image Location** property. In previous versions, the system heavily used Embedded Resources (kernel resources) to specify button images or icons. However, with the shift to the web, this image format option isn't available. Instead, a new image format (Symbol font) is added, and the expectation is that all buttons that require images use this format (**Image Location** = **Symbol**). The primary reason for this change is that a symbol font is the best performing and most scalable image format. For a list of the full set of symbols that are supported, see [Dynamics Symbol font](symbol-font.md). The following table shows the recommended and preferred method for assigning images to buttons.
...
In general, the **Button Style** property defines how a button appears in the user interface. The framework disregards the **Button Style** property for buttons that you model inside an Action Pane or Toolbar. Instead, the framework renders those buttons by using the style specially designated for buttons in those types of containers. For buttons that you model directly on the form canvas (outside Action Panes), the following button styles are available.
...
The system automatically adds several system-defined buttons to pages. The following table shows the list of system-defined buttons that the system adds to the Action Pane. For more information about how these buttons behave and how to manage them, see [System-defined buttons](system-defined-buttons.md).
...
|[![Popout button.](./media/popoutbutton.png)](./media/popoutbutton.png)|Popout|Pop out the current page into a new dynalinked window.|

### Pinning the Action Pane

The standard Action Pane supports the ability for the user to pin or unpin the Action Pane.

When the Action Pane is pinned open, an Action Pane tab is expanded and pushes the page content below it (for example, it doesn't overlap anything on the page). In this mode, there's a chevron button in the lower right corner of the expanded Action Pane tab to unpin the Action Pane.  

When the Action Pane isn't pinned open, clicking on an Action Pane tab opens it as a flyout on top of the page content. The lower right corner of the Action Pane tab flyout has a pushpin button that you can click to pin the Action Pane open.

### Overflow behavior in the Action Pane

Standard Action Panes include an overflow feature that adds a responsive element to pages and eliminates the need for a horizontal scrollbar in the Action Pane. When the browser width is insufficient to show the entire Action Pane contents, an overflow menu automatically appears in the Action Pane and includes any buttons and Action Pane tabs that didn't fit given the browser width.  Items are added one-by-one starting with the rightmost actions from the Action Pane. Note the system actions on the right side of the Action Pane don't participate in the overflow behavior.  
...
Keyboard shortcuts are another mechanism for triggering some actions. Many actions that had shortcuts in Dynamics AX 2012 continue to have shortcuts in finance and operations. However, because of browser restrictions, the key combination that is used to trigger a particular action might differ.

The following table shows some important keyboard shortcuts that are available. For the full list of current keyboard shortcuts, see [Keyboard shortcuts](../../fin-ops/get-started/shortcut-keys.md). We plan to provide mechanisms so that developers and end users can define shortcuts for other actions.
