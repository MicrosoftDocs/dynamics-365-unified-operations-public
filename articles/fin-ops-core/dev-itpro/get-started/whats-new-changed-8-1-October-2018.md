...
Use shared number sequences to assign customer IDs or vendor IDs. Shared number sequences also let you copy customers or vendors from one legal entity to another legal entity but use the same IDs in both legal entities.

For more information, see [Copy customers by using shared number sequences](/dynamics365/finance/accounts-receivable/copy-customer) and [Copy vendors by using shared number sequences](/dynamics365/finance/accounts-payable/vendor-copy).
...
The **Global transactions** button is now available on the customer page. Use this button to view all transactions for a customer across all legal entities. The **Customer transactions** list page shows transactions only for the legal entities that you have access to, based on your security settings.
...
The **Global transactions** button is now available on the vendor page. Use this button to view all transactions for a vendor across all legal entities. The **Vendor transaction** list page shows transactions only for the legal entities that you have access to, based on your security settings.
...
You can also update due dates and discount dates for open customer transactions. In release 8.1, the experience is improved so that you can add due dates to the **Vendor transaction** list page.
...
Set up the derived dimensions segments and values by using entities.
...
In this release of Finance and Operations, Microsoft made numerous extensibility enhancements to support extensibility through chain of command, delegates, or by providing access to members. In addition, Microsoft made enhancements to enumerations, metadata, and SQL operations. For detailed information, see [Extensibility changes in Dynamics 365 Finance version 8.1](/dynamics365/fin-ops-core/dev-itpro/extensibility/extensibility-changes-81).

## Phantom items

Use the Phantom line type for the lines of a bill of materials (BOM) and multilevel BOM structures. You can also use phantom BOMs for a BOM that has a route network.
...
Standard sales tax functionality in Finance and Operations now fulfills the majority of legislation requirements of United Arab Emirates VAT law. The following country/region-specific features are specific to the United Arab Emirates:
