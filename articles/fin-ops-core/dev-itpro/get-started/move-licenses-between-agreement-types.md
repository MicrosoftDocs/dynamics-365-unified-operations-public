...
When a customer changes the licensing agreement through which they obtained their subscriptions, consider this change a *commercial transaction* that requires working with external parties who sell the subscriptions to Dynamics 365 services. The goal is to maintain uninterrupted access to the service during that time of transition.
...
1. The customer places the order for subscriptions under the new agreement with the Volume Licensing reseller or the CSP.
    ...
    > [!IMPORTANT]
    > Make sure that you purchase the subscriptions against the same Microsoft Entra tenant that you used on the original agreement.

1. The customer activates the subscriptions.
1. In Microsoft 365 Admin Center, the customer verifies that both the new and the existing subscriptions are active for all user and device licenses, as well as for the add-on environment subscriptions.
1. When the new subscriptions are active, the customer requests that the Volume Licensing reseller or the CSP suspend the existing subscriptions. Typically, there's an overlap to help guarantee continuity and avoid disruption of service.
