...
1. Make sure that the grid is bound to the table that represents the entity.
1. Make sure that the form has a menu item that is root-navigable.
1. Make sure that the form can be opened directly via a URL that includes the menu item parameter.
1. Make sure that the filter pane enables the grid to be filtered based on the desired fields.
1. In the designer, create a page for the entity.
1. In the designer, put only a list on the page.
1. In the designer, put the desired fields in the list on the page.
...
1. Make sure that the Master Root Data Source on the form is bound to the table that represents the entity.
1. Make sure that the form has a menu item that is root-navigable.
1. Make sure that the form can be opened directly via a URL that includes the menu item parameter.
1. In the designer, create a page for the entity.
1. In the designer, put the desired fields on the page.
...
1. Make sure that the entity for the list view is the same as the entity for the details view. In other words, the table that is bound to the grid on the form that is used for the list view must be the same table that is the Master Root Data Source on the form that is used for the details view.
1. Make sure that the form that is used for the details view can be filtered on a unique key field by using the filter pane.
1. In the designer, make sure that the list view page is linked to the details view page. Click the list, open the properties, and then set the details view page by using the lookup.

    :::image type="content" source="media/listtodetailsdesigner.png" alt-text="Screenshot of linking the list view page to the details view page.":::
...
1. Make sure that you create a separate details view page for the entity that is being referenced.
1. Make sure that the reference field is added to the page.
1. In the designer, make sure that the reference field is linked to the details view for the referenced entity. For example, in the following illustration, Vehicle-details is the details view page for the referenced entity.

     :::image type="content" source="media/referencepagedesigner.png" alt-text="Screenshot of linking the reference field to the details view for the referenced entity.":::
...
1. Make sure that the form contains a grid that is bound to the table that represents the related entity.
1. Make sure that the table for the related entity is *active joined* to the table for the entity that contains the reference.
1. Create a details view page that contains the desired fields for the entity, and that also contains a list that has the desired fields from the related entity.
...
1. Make sure that the form contains a grid that is bound to the table that represents the related entity.
1. Make sure that the table for the related entity is *active joined* to the table for the entity that contains the reference.
1. Use the form to create a details view page that contains the desired fields for the entity.
1. Use the same form to create a separate list view page that contains only a list that has the desired fields from the related entity.
1. On the details view page, add a PageLinkControl that links to the list view page. Currently, you must use business logic to add the PageLinkControl. The following example show the code that Fleet Management uses.
...
1. Refer to the following code example, which hides the page from the navigation menus, and accesses data on the page using the provided APIs. "My-Hidden-Page" and "My-Field-Id" are the names of the page and control, respectively, and can be found when viewing the corresponding page in the designer.
...
1. Click the **Grid** node and then click **Properties**.
1. The **Control properties** dialog contains a default fetch size of 50 records.
1. Adjust the fetch size as needed.
