...
2. Install the Android debug apk for the finance and operations app on an Android device:
    ...
    - Uninstall the finance and operations app. Ensure that any previous version of the finance and operations app is uninstalled.
...
7. If breakpoints are encountered, then the browser developer tools allow you to control the flow of execution and see the values and parameters being passed. 

8. To change styling at runtime, use the elements tab to help you determine what elements JavaScript should target and how those elements should be styled.
...
7. If breakpoints are encountered, then Visual Studio allows you to control the flow of execution and see the values and parameters being passed. 
...
Version 2.2.8 of the finance and operations mobile app fixes the known issues with the date and time pickers in iOS14. Ensure that you have the latest version of the app if you're experiencing issues running the application on iOS14.  

### The Mobile Client app isn't working on particular devices
...
If Active Directory Federation Services (ADFS) is in use on the domain and the environment is on-premises, then **ADFS must be configured to provide a regular forms-based authentication screen** instead of using Windows Integrated Authentication (WIA). The finance and operations apps for iOS and Android require the regular forms-based authentication screen. ADFS should be configured to only provide WIA for browser clients (use cases). For more information, see [Configure intranet forms based authentication for devices that don't support WIA](/windows-server/identity/ad-fs/operations/configure-intranet-forms-based-authentication-for-devices-that-do-not-support-wia).

### Using multifactor authentication with the finance and operations app
The finance and operations (Mobile Client) app facilitates user authentication with Microsoft Entra ID by presenting the Microsoft Entra sign-in web page within an embedded browser. After a successful sign-in, it will retrieve the user token from the cookies and use that when communicating with the user interaction service that it shares with the web client. Some multifactor authentication mechanisms that involve switching to a different app on the same device can cause the embedded browser to close, causing the sign-in to fail. The workarounds for this scenario include:

- Different device - Use a different device for the multifactor authentication response so the app remains active on the original device.
- Multifactor authentication via phone call - Use a phone call for the multifactor authentication response so an app switch isn't needed.
- Use the "touch and hold" gesture on the authentication notification and then select the **Accept** option. Because the notification acceptance doesn't require an app switch, the sign-in proceeds as usual.

If there are continued problems with MFA authentication, it's helpful to [submit the Microsoft Authenticator app logs](https://github.com/AzureAD/azure-activedirectory-library-for-objc/wiki/Instructions-on-Collecting-Microsoft-Authenticator-Logs) and provide support with the resulting Incident ID.

### Intune support and conditional access
The finance and operations (Mobile Client) app doesn't have Microsoft Intune policies implemented, so it doesn't support Intune. Manually adding the app (following [Add iOS store apps to Microsoft Intune](/mem/intune/apps/store-apps-ios)) is also not supported because the device identifier can't be passed.
...
There are many resources that you can use to figure out how to build or change content for the Mobile Client.
...
- Use the [Dynamics Community forums](https://community.dynamics.com/ax/f/33) by searching for answers and asking questions when needed.
...
- If you rerecord a page with a grid, then you need to rerecord the link to the Details page because otherwise it isn't there.
...
- When adding a lookup field, select a value in a lookup instead of just adding the lookup field. This action ensures that the correct value is selected.
- If you rerecord a lookup, all the references also need to be rerecorded because the GUID for the lookup changes.
- If you want to add a field to a page, you must add all the fields again, because the list is cleared at the beginning of each edit. This functionality is a limitation of task recorder. Reordering isn't possible.
- In the workspace XML, GUIDs are used as references to forms and controls instead of names. GUIDs ensure uniqueness, but come at the cost of maintainability. Those GUIDs are regenerated on each modification, so partial edits are difficult. The use of GUIDs is costly to change, so it's unlikely that changes would be made in the future to use simpler string name references.
- Relationships between form datasources need to be via RecordId instead of string. For example, primary keys of the datasources shouldn't be strings.
- Customers and partners can fork a workspace by creating a copy of it and then make changes as needed.
- There's no checkbox in mobile. You have to manually bind the field to a Yes/No enum in JavaScript.
...
- FastTabs (common on existing forms).
    - Recorded forms shouldn't have FastTabs because the FastTabs expansion state can interfere with playback.
