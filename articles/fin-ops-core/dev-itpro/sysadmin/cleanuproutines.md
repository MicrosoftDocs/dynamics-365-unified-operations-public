...
|------|-------------|
| Calculation of location load | The `WMSLocationLoad` table tracks the weight and volume of items and pallets. The *Summation of load adjustments* job <!--KFM: Do we mean the  *Calculation of location load* job? --> reduces the number of records in the `WMSLocationLoad` table and helps improve performance. |
...
| Dimension inconsistency cleanup | <!--KFM: No longer exists? --> <p>This cleanup routine resolves dimension inconsistencies on inventory transactions that have been financially updated and closed. Inconsistencies might be introduced if the multisite functionality was activated during or before the upgrade process.</p><p>Use this routine only to clean up the transactions that were closed before the multisite functionality was activated.</p><p>**Important:** Don't schedule this routine to run periodically.</p> |
...
| Cost calculation details | <!--KFM: This entry exists, and was in the original table, but wasn't in your draft. I kept it. --> This cleanup routine cleans up cost calculation details. |
...
| Correct item that is not visible in released products form manually | <!--KFM: This text doesn't make sense. Something is wrong here. Please revise. -->This cleanup routine corrects `InventItemLocation` records to solve problems such as released products not displaying and e-commerce catalogs not working after deploying PQU. It corrects `InventItemLocation` records whose `InventDimId` value is *AllBlank* in the `InventTable` table. It inserts a new `InventDim` value that doesn't exist where one record with `InventDimId` is `AllBlank`. |
