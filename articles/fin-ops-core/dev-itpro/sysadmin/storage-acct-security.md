...
### Will a SAS URL that was created previously stop working once the storage account access key is disabled?
 ...
 Yes, previously created SAS URLs will stop working when the account access key is disabled, because seven days are the maximum TimeToLive value (Validity of the URL). Therefore, the SAS URL expires and won't work after seven days.
