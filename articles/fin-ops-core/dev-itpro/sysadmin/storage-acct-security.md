...
The issue could be with the URL truncation due to its variable string size. The variable which is holding the user-delegated SAS URL should have minimun string size 500. If the variable type capacity is less than to 500, then you will get this error. Identify the variable type and its length. If the variable is not of type string or if its capacity is less than to 500, then update the size minimum to 500. If you are using any EDT variable, then update it to "SharedServiceUnitURL" whose string size is 1000 and can accommodate the full URL.
