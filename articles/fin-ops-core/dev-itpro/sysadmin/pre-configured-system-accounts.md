...
> [!IMPORTANT] 
> Don't delete these system accounts. Deleting these accounts disrupts key functionality that Microsoft provides.

| Account | Purpose/use case |
|---|---|
| Axrunner | <p>Monitors the health of the environment and provides alerts as required.</p><p>**Note:** This account is deprecated for self-service environments and is no longer used.</p> |
| DataSyncFrameworkApp | Used by Microsoft Power Platform apps that Microsoft provides to synchronize data from finance and operations virtual tables in Dataverse, based on row version change tracking. |
...
| DynamicsMaintAppUser | Performs deployment and service operations in finance and operations apps. |
...
| PowerPlatformApp | Connects Dual-write and virtual tables for finance and operations apps to Dataverse. |
...
| RetailServiceAccount | Connects Retail services to the finance and operations environment. |
| ScaleUnitManagement | Communicates with the Scale Unit Manager portal. This account is automatically added when you upgrade to version 10.0.23. |
| ScaleUnitPipeline | Maintains and tracks communication between the scale units. This account is automatically added when you upgrade to version 10.0.19. |
| SysHealthServiceUser or Axping (depending on the deployed product version) | Monitors the availability and health of the environment, and provides alerts as required. |
