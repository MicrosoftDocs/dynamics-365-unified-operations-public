...
To standardize on .NET tools for building X++ on Azure DevOps, **MSBuild** with custom X++ targets are used. Your X++ source code repository must contain an X++ project for each package that you need to build. Optionally, you can use a solution file to group the projects, including C# project dependencies, and provide an explicit build order. If the repository doesn't already contain one, you can create a project from **Visual Studio**.

> [!NOTE]
> When using an existing X++ project (rnrproj), make sure that you created it or opened and saved it with Visual Studio tools on Platform update 27 or later.

> [!NOTE]
> Although a package can contain multiple models, a package always must be built in its entirety. As a result, only one project for just one of the models is required to build the whole package. Additionally, the project does not need to, but can, contain any objects.
...
To simplify the usage of the extracted NuGet packages, consider using the **NuGet install** option, and specifying the **-ExcludeVersion** [NuGet command line option](https://docs.microsoft.com/nuget/reference/cli-reference/cli-ref-install#options) so that the extracted package paths can be used in the build regardless of the version of the packages. Use the **NuGet Installer** task and set the **Installation type** to **Install**. Specify the path to the **packages.config** and **nuget.config** files created earlier.
...
To build X++ using **MSBuild**, you must supply several arguments. In the build solution step of a pipeline, you can specify these arguments in the **MSBuild Arguments** field.
...
The following example of **MSBuild** arguments assumes the NuGet packages are installed to **$(Pipeline.Workspace)\NuGets**, the X++ source code is in **$(Build.SourcesDirectory)\Metadata**, and the output of the compiler should go into **$(Build.BinariesDirectory)**:
...
> [!NOTE]
> Due to semantic versioning features in NuGet versions 3.4 and later, make sure that the task installs version 3.3.0 or lower. Currently, deployable package generation does not support semantic versioning.
