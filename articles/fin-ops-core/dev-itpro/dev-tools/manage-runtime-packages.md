...
## Including source controlled third-party in packages

After checking in the package binaries into source control as described, they should be included in deployable packages generated during build automation. There are two options:
1. On a build virtual machine, the standard legacy pipeline will automatically find and include the binaries into the deployable package it generates.
2. Using the [new pipeline](hosted-build-automation.md) or using the new packaging task in the legacy pipeline, review the [create deployable packages](pipeline-create-deployable-package.md) documentation and examples to include source controlled binaries in deployable packages.
