...
The best way for end users to get data out of the system is to use the Excel Add-in. The add-in relies on the OData service to retrieve data and takes advantage of the security that data entities provide. The import and export capabilities in the Data management framework (DMF) and Data import/export framework (DIXF) can also be used; however, DMF and DIXF are often limited to administrators. Additionally, you may consider exploring the available [reporting and analytic features](../analytics/bi-reporting-home-page) in the product.  

When the above options are not possible, you may consider the Export to Excel functionality which can be useful to retrieve data from calculated columns (display methods), columns with formatted values, and data from temporary tables. Because the export process uses the data from the active form to retrieve data, Export to Excel will return the data exactly as it is shown in the grid, including only returning the data matching the current set of filters. 

Export to Excel does have its own set of limitations. First, this mechanism takes more time to export than using the options that go through the OData service, and the time to export can be significant for larger exports. Secondly, exporting data to Excel is a memory intensive process that can result in out of memory errors and server crashes, especially if the organization increases the export row limit beyond the default value. Export to Excel is not intended to be used for auditing purposes. When very large datasets need to be exported from the system, data management is recommended.         

> [!WARNING]
> While organizations can increase the default Export to Excel row limit (of 50k records), it is **strongly recommended to not** to raise this limit above 100k records. Depending on the characteristics of the environment and the particular grid being exported, even that limit may be too high and may cause memory issues.   
