...
Export to Excel does have its own set of limitations. First, this mechanism takes more time to export than using the options that go through the OData service, and the time to export can be significant for larger exports. Secondly, exporting data to Excel is a memory intensive process that can result in out of memory errors and server crashes, especially when the export row limit is increased beyond the default value. Export to Excel is not intended to be used for auditing purposes. When very large datasets need to be exported from the system, data management is recommended.         
