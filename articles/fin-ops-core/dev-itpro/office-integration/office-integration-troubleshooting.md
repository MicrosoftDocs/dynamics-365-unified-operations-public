...
The Export to Excel functionality can be used in situations where the above options are not possible, in particular to retrieve data from calculated columns (display methods), columns with formatted values, and data from temporary tables. This is because the export process uses the data from the active form to retrieve this data. Export to Excel will also return the data exactly as it is shown in the grid, which is useful for filtered datasets. However, this mechanism will take longer to export than using the options that go through the OData service.

> [!WARNING]
> Exporting data to Excel is a memory intensive process. As such, large exports of data to Export should be minimized, as they have the potential to result in out of memory errors. While organizations can adjust the Export to Excel row limit (up to 1 million rows), it is **not recommended** to raise this limit above 100k records. Depending on the characteristics of the environment and the particular grid being export, even that limit may cause memory issues for some customers. 
