...
## Scenario 3: The snapshot for any of the publications failed, and you can track the failure in Replication Monitor
...
1. In the **ReplicationMenu.json** file, view all the menu options that have the same ID sequence. Find the step that you want to rerun, and then update the **Status** value to **0**.

    > [!IMPORTANT]
    > Don't change anything else in the **ReplicationMenu.json** file. Additionally, make sure that the migration app isn't running when you update the file.

1. Open the migration app, and rerun the step.

## Scenario 5: After you create the publication, the replication job fails, and exceptions occur.

After you create the publication, the replication job fails, and the following exceptions occur.

**Exception 1**

> Cannot execute as the database principal because the principal "dbo" doesn't exist, you can't impersonate this type of principal, or you don't have permission. (Source: MSSQLServer, Error number: 15517)  
Get help: `http://help/15517`

**Exception 2**

> The process couldn't execute 'sp\_replcmds' on 'replicationsrv\\MSSQLSERVER2016'. (Source: MSSQL\_REPL, Error number: MSSQL\_REPL20011)  
...
## Scenario 6: The Microsoft Dynamics Lifecycle Services status is "Failed," but the data upgrade trigger is successful in the migration app.
...
1. In the migration app folder, find the **Data** folder.
1. In the **Data** folder, open the **ReplicationMenu.json** file.
1. In the **ReplicationMenu.json** file, view all the menu options that have the same ID sequence. Find the step that you want to rerun, and update the **Status** value to **1**. In this way, you mark the step as completed.
...
## Scenario 9: The replication status for any of the publications shows "Waiting for snapshot to complete" for more than two hours.

**Solution**

In Replication Monitor, select and hold (or right-click) the publication, and then select **Reinitialize Subscription**.

## Scenario 10: You want to resume the data upgrade.

**Solution**

The console app might not have updated the data upgrade status.

To resume the data upgrade, follow these steps:

1. Use the **Help** option to learn the status of the console app. This option lists all the menu options and shows the current state.
...
After you run the **'ds'** option, two types of status are listed: the Lifecycle Services environment status and the data upgrade status.
...
## Scenario 11: After you create the publication, snapshot creation fails with errors

After you create the publication, snapshot creation fails and the following errors are thrown:
...
In Replication Monitor, select and hold (or right-click) the failed publication, and then select **Generate Snapshot**.

## Scenario 12: Data upgrade PreSync and PostSync processes take a long time, and you want to determine which process or job takes more time.

**Solution**

The ReleaseUpgradeDB framework logs the execution of each script in the **ReleaseUpdateScriptsLog** table. When you tune the performance of the data upgrade process, monitor the duration of scripts that run. In the table, you can easily identify the longest-running process or job. Use some of the following queries on the target AxDB to check progress and more.
...
After you create the publication and push the snapshot, the log reader reports an exception on some specific tables.
...
Restart the Log reader agent and Distribution agent (if you're not using immediate updating):
 - You can do this via SQL Server Agent or Replication monitor.
 - Or use T-SQL (example for a push subscription):
