...
| System administration \> Inquiries \> Users - Click Clean up | <p>Delete user logs older than a certain date. This rule has checked for logs over three month old. This log records users logging in and out.</p><p>1. Enter a value in the **History limit (days)** field to define a limit for the deletion. Only log information that is older than the given number of days is deleted. </p><p>2. Click **Select** to open the **Select log cleanup criteria** page, which is a version of the Inquiry page. </p><p>3. Select your cleanup criteria. Select a user or a range of users and, optionally, additional user information, such as date and time. </p><p>4. Click **OK** to return to the **User log cleanup** page. </p><p>5. Click **OK** to perform the cleanup once, or click the **Batch** tab to define parameters to clean up the user log regularly. |
...
| Path | Description |
|------|-------------|
| Organization administration \> Periodic \> Calendar cleanup | Use this page to specify the end date for deleting old working times. All old working hours up to, but not including, this date are deleted.|
...
| Path | Description |
|------|-------------|
| Project management and accounting \> Periodic \> Journals > Delete project journals. <p>On the **Delete project journals** page, click **Select**. In the **Criteria** field, select the journal(s) that you want to delete. Click **OK**.</p> | You can delete project journals from which transactions have been posted. By deleting these project journals, more system resources might be available.
| Project management and accounting \> Periodic \> Quotations > Delete quotations. <p>On the **Delete quotations** page, click **Select**. Click **OK** to transfer the quotations to the **Delete quotations** page. If the query transfers quotations that you don't want to delete, select them in the **Delete quotations** page and press ALT+F9. This will remove the quotations from the list of quotations to be deleted. Click **OK** to delete the quotations that are listed on the **Delete quotations** page. | Deleting unsent project quotations can recover database space. It is possible to create a filter to delete only selected quotations. Note that it's not possible to delete "Sent" quotations. To delete a "Sent" quotation, update the status to **Lost** or **Cancelled**.|

### Human resources

| Path | Description |
|------|-------------|
| Human resources \> Periodic \> Absence \> Delete absence journals. Choose one of the following actions: 1) Click **OK** to delete all absence journals. 2) Click **Select** to select the employees to delete absence journals for. | Use the **Delete absence journals** process to delete all empty journals that haven't been transferred for approval. Exceptions to this rule are empty journals that contain a period that is before a journal that contains planned, or future, absences. If planned absences exist in a future period but you still want to delete the journal, cancel the approval and repeat the delete journal procedure. |
