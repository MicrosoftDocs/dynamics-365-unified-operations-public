...
Virtual entities for Finance and Operations do not exist in CDS by default. A user is required to query the catalog entity to view the entities available in the linked instance of Finance and Operations. From the catalog, the user can select one or more entities and ask CDS to generate the virtual entities. This procedure if explained in subsequent sections.

## Entity fields

When a virtual entity is generated for a Finance and Operations entity, each field in the Finance and Operations entity is attempted to be created in the corresponding virtual entity in CDS. In an ideal case, the total number of fields should be the same in both entities unless there is any mismatch in supported data types between Finance and Operations and CDS as explained below. For data types that are supported, the field properties in CDS are set based on the properties in Finance and Operations. The supported and not supported data
types are discussed next. More information about fields in CDS can be found [here](https://docs.microsoft.com/en-us/powerapps/maker/common-data-service/fields-overview).

| **F&O Data type**               | **Modelled data type in CDS**                                                                                                                                                                                                                                                                                                                                                                                              |
|---------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
...
| UtcDateTime                     | DateTime - DateTimeFormat.DateAndTime, DateTimeBehavior.TimeZoneIndependent Empty date of 1/1/1900 in F&O is surfaced as an null value in CDS                                                                                                                                                                                                                                                                              |
| Date                            | DateTime - DateTimeFormat.DateOnly, DateTimeBehavior.TimeZoneIndependent Empty date of 1/1/1900 in F&O is surfaced as an empty value in CDS                                                                                                                                                                                                                                                                                |
| Enum                            | Picklist F&O Enum is generated as global OptionSets in CDS solution Matching between systems is done by the "External Name" property of values Enum integer value on CDS is not guaranteed stable between systems and should not be relied upon (especially in the case of extensible enums in F&O which do not have a stable ID either) Option set metadata is refreshed when an entity using the option set is refreshed |

Fields of data type *real* and *long* in Finance and Operations are modelled as
data type *decimal* in CDS. Due to the mismatch in precision and scale between
the two data types, the following behavior must be considered.

| **Use case**                            | **Resulting behavior**                                                                                                                                                                                                                                                                                             |
|-----------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| CDS has higher precision                | Should never happen unless the metadata is out of sync                                                                                                                                                                                                                                                             |
| F&O has higher precision                | On read, value is rounded in CDS to the closest precision value If value is edited in CDS, it gets rounded to the closest precision in Finance and Operations On write, writes the value specified in CDS (since F&O supports more precision)                                                                      |
| CDS has higher scale                    | Not applicable                                                                                                                                                                                                                                                                                                     |
| Finance and Operations has higher scale | CDS will show F&O value even if it exceeds 100 billion, though precision losses will occur. For instance, 987,654,100,000,000,000 is displayed in CDS as "987,654,099,999,999,900" If the value of this field is edited in CDS, CDS validation throws an error that it exceeds the max value before sending to F&O |

The following data types in F&O are not supported in CDS. Fields of such a data type in Finance and Operations entity will not be made available in the corresponding virtual entity in CDS. If such fields are used as parameters in OData actions, those actions will not be available for use in the corresponding virtual entity. OData actions are explained in a subsequent section.
...
Data type that are supported in CDS but not in Finance and Operations cannot be supported in virtual entities for Finance and Operations.
...
In CDS, the primary key is always a GUID. The GUID based primary key allows to uniquely identify a record in an entity in CDS.

In order to bridge this implementation gap between Finance and Operations and CDS, the primary key of the virtual entity for Finance and Operations is a GUID (to comply with CDS) which is comprised of the data entity ID in the first 4 bytes, and the rec ID of the root data source in the entity as the last 8 bytes. This design satisfies CDS’s requirements to have a GUID as the entity key and
allows unique identification of the entity record in Finance and Operations using the table Id and rec ID.

## Primary field

In CDS, each entity must have a primary field which must be a single field of type string. The primary field is used in CDS in the following scenarios.
...
Note, if an error is encountered when generating any part of a Finance and Operations virtual entity in CDS, the virtual entity will not be created at all. The above factors are not considered as errors.

### Native entity to native entity relationship

This is the standard CDS functionality where the relationships are resolved using the GUID of the related entity which is the entity key. The GUID identifies the unique entity record in the related entity.

### Virtual entity to virtual entity relationship

The relationships between two Finance and Operations virtual entities are driven by the relation metadata in the Finance and Operations entity. As explained above, these relations are generated as relationships in CDS when the virtual entity is generated. Like native entity behavior in CDS, such relationships use the GUID to identify the unique record of the entity in Finance and Operations.
Semantically, the GUID on the Finance and Operations virtual entity behaves like the GUID on the native CDS entity. The implementation details of the GUID in the Finance and Operations virtual entity is explained in an earlier section.
...
When an entity is created in CDS (native or virtual), five default views are also created for the entity. The following are the types of views that are created.
...
CDS added the primary field of the entity to all these views. Makers can add additional fields to these views as needed.
...
The field groups are used to populate additional fields in the default views in CDS for the Finance and Operations virtual entities. The mapping of the field groups to the default views is given below.
...
OData actions in the Finance and Operations entities are made available as SDK messages in CDS. More information on what SDK messages are and what they enable in CDS can be read here.

Input and output parameters of the following types are supported. If an input or output parameter is of a different type, then the OData action does not show up as the SDK message in CDS.
...
Some examples of OData actions that are supported in Finance and Operations entity but is not supported in the corresponding virtual entity in CDS are listed below.
...
Labels defined on metadata such as entity names and field names in Finance and Operations are retrieved at virtual entity generation time in CDS by passing the list of language locales installed in CDS. Finance and Operations returns each label as a list of locale/value sets, which are then used to construct a label instance in CDS. Only the language packs that existed at the time of entity
generation or refresh will be included and only labels for which Finance and Operations has a translation provided. Any missing translations revert to the label ID, such as \@SYS:DataEntity. After installing a new language pack in CDS,
...
Finance and Operations CRUD business logic on the entity and backing tables is executed when called through the virtual entity in CDS. If any exception is thrown on the Finance and Operations side, then the last message in the error log is returned back to CDS and thrown as an InvalidPluginExecutionException containing the info log message from Finance and Operations. Since the Finance and Operations code runs in the context of the user, the language of the error message will be based on the specified language for the UserInfo record in
Finance and Operations. Any messages written to the info log in Finance and Operations that do not result in an exception are not shown in CDS.

## Calculated/Unmapped fields

Calculated and un-mapped fields in Finance and Operations entities are also available in the corresponding virtual entities in CDS.
