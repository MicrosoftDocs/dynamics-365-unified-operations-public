...
## Authorization
Because finance and operations apps and Microsoft Power Platform maintain separate user security, it's important when operating over a virtual entity record in Dataverse, the appropriate user context is used with the correct permissions to access the data in finance and operations. For more information, see [Authentication and authorization.](authentication-and-authorization.md)

This means that when you create an organization context in a Dataverse plugin that reads a finance and operations virtual entity, that the organization context is associated to the appropriate application or end user that has permissions to the virtual entity in finance and operations. The Dataverse SYSTEM user doesn't have access to all entities in finance and operations, nor do the out of the box service-to-service application have such access. Failure to use a proper user or app-user context may result in error with messages like `Unable to determine the user in CDS`
