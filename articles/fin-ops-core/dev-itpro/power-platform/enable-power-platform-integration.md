...
If a Finance and Operations environment has been manually linked to multiple Power Platform environments, then the process of enabling the Power Platform integration for the environment cannot be automated. The integration must be one-to-one between Finance and Operations and the Power Platform environment for the process enabling the integration to be automated. If, for example, dual-write has been enabled for the Finance and Operations environment with multiple Power Platform environments, there will be metadata showing a link to both environments. 

In cases where there are links to multiple environments, the process to enable the Power Platform integration cannot be automated. To enable the integration for a Finance and Operations environment with one of the existing Power Platform environments, please work with your FastTrack solution architect or Microsoft technical support to identify the environments that should be linked for the integration.
