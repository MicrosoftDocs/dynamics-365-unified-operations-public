...
| **File type**                      | File type used for creating an archive of exported attachments. This file type must be set up as described in [Configure file types](#configure-file-types). Use the **File** value from the list of **File types** or another similar custom file type set up with **File** value in the **Group** property of the **Document type**. | File |
