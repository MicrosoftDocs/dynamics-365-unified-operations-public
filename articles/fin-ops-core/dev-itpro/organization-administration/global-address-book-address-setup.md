...
1. Enter the name of the address format type and a brief description. For example, you might enter **NO** in the **Address format** field and **Norway** in the **Description** field.
1. In the lower pane, select **New** to configure a new address component for the address format type that is selected in the upper pane.
1. In the **Address application object** field, select the first object that must appear when an address in this format is shown. For example, select **Street**.
1. In the **Separator** field, enter the character that is used to separate this object from the next object when an address in this format is shown. For example, enter a comma (**,**).
1. Set the following checkboxes as appropriate:
...
1. Enter the country/region code, a short name for country/region, and the full name. For example, enter **DEU**, **Germany**, and **Federal Republic of Germany**, respectively.
1. In the **Address format** field, select an address format to use for the country/region. The address formats that are defined on the **Address format** tab of the page are available for selection. 
1. In the **Time zone** field, select the time zone of the country/region.
1. Enter the International Organization for Standardization (ISO) code for the country/region.
1. In the **BACEN code**, **IOR facility ID**, and **Code by OKSM** fields, enter values if they are required for your financial, purchasing, or sales transactions. Also select whether the country/region is a member of the customs union.
1. To enable only valid five-digit or nine-digit ZIP/postal codes, select the **Use ZIP + 4 postal code validation rules** option.
...
1. Enter the state/province code and the name of the state/province.
1. Select the time zone for the state/province.
1. Enter the IT state code for the state/province, and any additional codes that are required for transactional information.
1. To define the state/province as the default state/province for the selected country/region, select the **Default state/province** option. The default state/province will be entered by default in state/province fields when a new county or city record is created.
...
1. In the **Country/region** field, select the country/region where the county is located.
1. In the **State or province** field, select the state/province where the county is located.
1. Enter the name of the county and a brief description.
...
1. In the **Country/region** field, select the country/region that you're adding the city information to.
1. Enter the name of the city and a brief description.
1. Select the state/province or county where the city is located.
...
1. Select **New** to create a new line.
1. Enter the name of the district and a brief description.
...
1. Select **New** to create a new line.
1. Enter the ZIP/postal code, and select the corresponding city.
1. Enter the street name that the ZIP/postal code applies to.
1. Enter the lowest and highest street numbers that use the ZIP/postal code, and enter the number types. For example, the ZIP/postal code might apply only to odd-numbered street addresses.
1. Select the state/province, county, district, and time zone for the ZIP/postal code.
