...
A benefit accrual plan includes a set of rules that determine how hours in the plan accrue and are used. The plan also includes a list of earning codes that you match against the earning codes on earnings statement lines. Therefore, the plan balance can be reduced when a specific earning code is used.

The rules for the plan are based on the number of months that a worker is employed with the organization. The months of employment are based on the employment start date or the seniority date. You define rules to control the rate that benefits accrue, and to control the maximum accrual limit, minimum balance, and carry-forward limit.
...
| Field | Description |
|-------|-------------|
| Benefit accrual plan Description | The name and description for the plan. |
| Annual plan start date | The start date for the plan. The plan resets each year on the anniversary of this date. If you leave this field blank, the plan isn't reset. Any fields that show data for the plan year, such as the **Plan year accrued** and **Plan year used** fields on the **Benefit accrual balances** page, include all transactions from the start of the plan instead of all transactions in the plan year. |
| Carry forward balances | If you select this option, you can carry over balances at the end of the plan year to the next plan year. When you create the plan rules, you can limit the number of hours that can be carried forward. |
| Accrual rate basis | The method that this plan uses to accrue benefits for workers who are enrolled in the plan. Use this method together with the value in the **Amount or rate** field to determine the number of hours that are added to the worker's plan. Select one of the following options:<ul><li>**Hourly rate** – Calculate the number of hours added to the plan balance by multiplying the number of eligible hours on the pay statement by the rate in the **Amount or rate** field.</li><li>**Fixed amount by payroll frequency** – Add the number of hours in the **Amount or rate** field to the plan balance on the last day of each pay period that is included in the selected payroll calculation frequency.</li><li>**Fixed amount by date** – Add the number of hours in the **Amount or rate** field to the plan balance on the last day of the pay period that includes the date that is determined based on the value in the **Accrual date** field.</li><li>**Hours of compensatory time** – Calculate the number of hours added to the plan balance by multiplying the number of eligible hours on the pay statement by the value in the **Rate for compensatory time** field on the **Earning codes for plan accrual** FastTab.</li></ul> |
| Frequency | The payroll calculation frequency that includes the pay periods when the number of hours in the **Amount or rate** field is added to the plan balance.<br>**NOTE:** This field is available only when the accrual rate basis is **Fixed amount by payroll frequency**. |
| Accrual date | The day each year when the number of hours in the **Amount or rate** field is added to the plan balance. Select one of the following options:<ul><li>**Employment start date** – This date is on the **Employment history** page.</li><li>**Seniority date** – This date is on the **Worker** page. Verify that the seniority date field always has a value. If you use the seniority date to determine the months of service, and the **Seniority date** field on the **Worker** page is blank, the worker can use hours from the benefit accrual plan, but no hours can accrue in the plan.</li><li>**Annual plan start date**</li><li>**Custom date** – A date that you specify for this plan.</li></ul>**NOTE:** This field is available only when the accrual rate basis is **Fixed amount by date**. |
| Custom accrual date | The month and date each year when the number of hours in the **Amount or rate** field is added to the plan balance.<br>**NOTE:** This field is available only when the accrual rate basis is **Fixed amount by date** and the accrual date is **Custom date**. |

Enter the following information on the **Plan rules** FastTab.
...
| Field                 | Description |
|-----------------------|-------------|
| Months of service     | The minimum number of months of service for the selected rule. The number of months of service is based on the employment start date, the seniority date, or a date that you specify for the worker on the **Benefit accruals** page. **NOTE:**  The employment start date is on the **Employment history** page, and the seniority date is on the **Worker** page. Verify that the **Seniority date** field has a value. If you use the seniority date to determine the months of service, and the **Seniority date** field on the **Worker** page is blank, the worker can use hours from the benefit accrual plan, but no hours accrue to that plan. |
|                       |Include a row that starts at 0 (zero) months of service. Each row ends when the next row starts. The last row starts at the specified number of months and doesn't end. For example, the rows in your plan have 0, 60, and 120 for months of service. The rate and limits in the first row start when a worker is hired and are applied through the end of the worker's eleventh month. The second row starts at 12 months, and the rates and limits are applied through the end of the fifty-ninth month. The third row starts at 60 months, and the rates and limits are applied as long as the worker continues to be employed by your organization. |
...
| Minimum balance       | The number of hours that must be available in the plan before a worker can use hours from the plan. When the available balance is less than this number, the worker can't use any hours from the plan until additional hours accrue, or until the available balance is adjusted. **NOTE:**  To let workers take time off before they have accrued the hours, enter a negative number. If an earnings statement includes hours that reduce the available balance in the plan to less than the minimum balance, the earnings statement lines that include those hours aren't released. |
