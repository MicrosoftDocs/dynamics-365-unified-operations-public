...
Earnings that are automatically generated use settings for the position to identify the pay cycle, legal entity, and whether the earnings should be generated as a salary amount or based on a schedule. During this process, the system automatically generates recurring earnings that are assigned to a specific worker if they're due in the specified pay period and pay cycle.

> [!NOTE]
> When you generate earnings for workers, specify only the pay cycle and pay period to use. Don't specify the type of earning to generate. Recurring earnings, salary, leave, and schedule-based earnings are all created in the same process. Premium earnings, such as overtime premiums, and retroactive earnings are created separately.

To automatically generate worker earnings, follow these steps:

1. Select **Payroll** > **Earnings statement processing** > **Generate earnings**.

    > [!NOTE]
    > To generate earnings from the **Workers** or **Positions** page, select the **Payroll** tab.

1. Specify the following information for the worker positions to generate earnings for.
...
1. Select **OK** to generate earnings statements. You receive a message that states that the earnings statements were generated successfully.

    > [!NOTE]
    > The system creates recurring earnings and the earnings from schedules first, because salary amounts might be affected by other earnings that are automatically generated.

1. To view the earnings statements that the system created or updated, open the **All Earnings statements** list page.
1. Verify that the batch job completed correctly. (Select **Common** > **Inquiries** > **Batch Jobs** > **My batch jobs**. On the **Batch job** tab, select **Log**, and then inspect the log for any errors that occurred.)
...
1. Select **Payroll** > **Earnings statements** > **All Earnings statements**.
1. Open an earnings statement, and then select **Edit**.
1. Select **Add line**, and then enter the following information:

    1. Modify the earnings date if it isn't the end date of the pay period.
    1. Select the position that the worker earned the earnings in.
    1. Select the tax region.
    1. Select the earning code that represents the type of earning. When you select the earning code, the default description, quantity, unit type, and rate are entered from the earning code. Enter a quantity that's appropriate for the unit type. You can also change the rate. However, the rate that you enter must be a flat rate and must not be calculated by the system.

1. If you didn't use the automated process to generate earnings before you began to enter lines manually, you can select **Generate earnings** to use the default values on the remaining lines of the worker's earnings statement. The lines that you entered manually aren't affected. You can also automatically generate earnings statements by using the process that is described earlier in this article. In that case, the lines that you entered manually *are* affected.

    > [!NOTE]
    > You can complete this step regardless of whether the worker is paid a salary or paid hourly.

1. Close the page to save the earnings statement.

## If necessary: Generate premium earnings
...
1. Select **Payroll** > **Periodic** > **Earnings statement processing** > **Generate premium earnings**.
...
1. Specify the following information for the worker positions to generate premium earnings for.

    | Field | Description |
    |-------|-------------|
    | Pay cycle | Select the pay cycle. |
    | Pay period | Select the pay period. The list includes only the pay periods that are available for the pay cycle. The first open pay period is the default pay period, but you can select any open pay period in the list. |
    | Replace premium earnings | Select this check box if you must replace existing premium earning lines. Often, when you select this check box, you also select the **Generate earnings for specified codes only** option in the **Generate** field. |
    | Generate | Select one of the following options:<ul><li>**Generate earnings for all premium codes** – Select this option if there are no dependencies between premium codes.</li><li>**Generate earnings for specified codes only** – Select this option if there are dependencies between premium codes. After you select this option, select the premium codes to generate earnings for.</li></ul> |

1. Select **OK** to generate premium earnings for all earnings statements. You receive a message that states that the premium earnings were generated successfully.
1. Repeat this task for any additional premium codes that you must generate premiums for.
1. To view the earnings statements that were updated, open the **All Earnings statements** list page.
1. Verify that the batch job was completed correctly. (Select **Common** > **Inquiries** > **Batch Jobs** > **My batch jobs**. On the **Batch job** tab, select **Log**, and then inspect the log for any errors that occurred.)

## If necessary: Manually add overtime premiums
...
1. Select **Add line**, and then enter the following information:
    ...
    1. Select the position that the worker earned the earnings in.
    1. Select the tax region.
    1. Select the earning code for the overtime premium. When you select the earning code, the default description, quantity, unit type, and rate are entered from the earning code.

1. Close the page to save the earnings statement.

## If necessary: Generate retroactive earnings
...
1. Select **Payroll** &gt; **Earnings statement processing** &gt; **Retroactive earnings** &gt; **Generate retroactive earnings**.
...
1. Select **OK** to generate retroactive earnings and add them to earnings statements. You receive a message that indicates the number of earnings statements that were updated.
1. To view the earnings statements that were updated, open the **All Earnings statements** list page. For earnings that are based on hours or pieces, the retroactive earnings appear on a separate line, together with the retroactive earning code. For salary-based earnings, the amounts are adjusted, but they don't appear on separate lines. To view all the retroactive earnings that you generated, open the **Review retroactive earnings lines** page. (Select **Payroll** &gt; **Earnings statement processing** &gt; **Retroactive earnings** &gt; **Review retroactive earning lines**.)
1. Verify that the batch job completed correctly. (Select **Common** > **Inquiries** > **Batch Jobs** > **My batch jobs**. On the **Batch job** tab, select **Log**, and then inspect the log for any errors that occurred.)
