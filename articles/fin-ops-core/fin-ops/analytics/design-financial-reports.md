...
For this exercise, you are generating and exploring an existing default report. This report includes all accounts and also contains account properties (attributes) for the accounts. You are drilling into transaction detail, applying dimension filters, changing the currency on the report. First, we update the display order of dimensions for financial reporting. This allows you to choose how the dimensions display not only while designing and viewing financial reports.
...
3. Save the dimension configuration. Next, we generate a report and explore the data in the report.
...
6. Select **Edit**.
    ...
    > [!NOTE]
    > You are prompted to download the click-once report designer and to log in. Use your credentials to log in.

7. Change the base year to 2021 and select **Generate**. When a report is generated from the report designer, it opens in a new browser tab. You can either explorer the report in the new browser tab, or go to your original browser tab and open the report from there by selecting it from the **Financial reports** list.
...
11. Select the same or a different account and **open voucher transactions**. Voucher transactions are filtered to the period, year and account + dimension combination of the account selected. From **Voucher transactions**, you can choose to explore other information about the transaction.
...
## Add additional account properties to a report design
In this exercise, you are modifying an existing default report. You are updating both row definition to include all accounts and you update the column definition to contain account attributes. Once the updates are complete, you generate the newly created report and explore the report. We start from the **Financial reports** list.
...
3. Select **Edit**. **Summary Trial Balance – Default** opens in the report designer.
...
7. With the cursor on row 50, select **Edit**, then **Insert row from dimensions**. Insert row from dimensions allows you to choose what dimensions you'd like to have in your row definition. For this exercise, we're going to build the row definition using Main Account.
...
14. **Save** the row definition. The row definition now contains all of the accounts plus a total row to add all of the accounts together. Next, we update the column definition to include additional account attributes.
...
16. Save the column definition as **Detailed Trial Balance with attributes**. The column definition contains financial data columns, a description column, and calculation columns. We're going to add attribute columns to the column definition to provide additional detail about the accounts.
...
21. Save the column definition. Now that the row definition and column definition have been updated, we need to add them to the report definition.
...
For this exercise, you are modifying an existing default report. You are creating a reporting tree and adding to a report definition to produce a **Cost Center/Divisional Income Statement**. Once the updates are complete, you generate the **Cost Center/Divisional Income Statement** and explore the report using the reporting tree. We start from the **Financial reports** list.

1. Go to **Financial reports** under Inquiries and reports in General ledger.
2. Select the row for the report named **Income Statement – Default**.
3. Select **Edit**. **Income Statement – Default** opens in the report designer.
...
8. **Save** the resulting tree with the name **Cost Centers by Division**. Now that the reporting tree has been created, you modify the reporting tree to contain three new rollup units; Marketing, Operations, and Retail.
...
16. Click unit number 12, **Outlet**, and right-click. Select **Insert Reporting Unit**.
17. Repeat step 11.
...
19. Verify unit 3, **Trade Shows**, is active and right-click.
20. Select **Demote Reporting Unit**. Notice the unit now displays as a child of **Marketing**.
21. Click unit 4, **Marketing Campaign**, and right-click.
...
25. Save the resulting reorganization. Now that we have the reporting tree created and organized, it can be added to the report definition.
...
For this exercise, you are modifying an existing default report. You are adding an organization hierarchy in the report definition to produce a Consolidated Income Statement and Balance Sheet. Once the updates are complete, you generate the consolidated report and explore the report using the reporting tree. We start from the Financial reports list.
...
In this exercise, you are creating a new report. The report is a side-by-side departmental income statement. You use an existing row definition, but create a new report definition and a new column definition that uses dimension filters. We start from the **Financial reports** list.

1. Go to **Financial reports** under **Inquiries and reports** in General ledger.
2. Select **New**. Report designer opens with a blank report definition open. Your first task is creating the column definition.
...
12. Save the column definition as Side by Side Departments. Since we are using an existing row definition, the report definition can now be modified to have to use the newly created column definition and the existing row definition.
