...
A row definition is a report component that specifies the contents of each row on a financial report. You can combine a row definition with column definitions, reporting tree definitions, and report definitions to create a building block group that multiple companies can use.

## Create a row definition

1. In Report designer, in the navigation pane, select **Row definitions**.
1. On the **File** menu, select **New**, and then select **Row definition**. For more information about the content of each cell, see [Modify row definition cells](modify-row-definition-cells-financial-reporting.md).

## Open a row definition

1. In Report designer, in the navigation pane, select **Row definitions**.
1. Select the row definition name to open it.
1. To view any building blocks that are associated with the row definition, right-click the row definition, and then select **Associations**.
...
1. On the **Edit** menu, select **Insert rows from dimensions**.
1. In the **Insert rows from dimensions** dialog box, in the **Dimensions** row, select the cell for the dimension to transfer to the row definition, and then select **All &&&**.
1. To limit the row definition to a specific range of dimension values, enter the starting dimension value in the **Dimension range start** cell, and then enter the ending dimension value in the **Dimension range end** cell. To include all values for the selected dimension, leave these cells empty.
...
1. In the **Starting row code** field, specify the row code for the first dimension value to add to the row definition.
1. In the **Increment each row by** field, specify the gap between consecutive row codes. For example, if the first row code is 100, and the increment value is 30, the first new rows have the codes 100, 130, 160, 190, and 220. Use an increment value that provides enough space to insert new format and formula rows.
1. Select **OK**. For each of the selected dimension values, one line is added to the row definition.
...
1. In Report designer, select **Row definitions**, and then open the row definition to modify.
1. On the **Edit** menu, select **Rounding adjustment**.
1. In the **Rounding adjustments** dialog box, enter the following values:

    - **Rounding adjustment row** – The row code for the row that you want to adjust to balance the balance sheet.
    ...
    > [!NOTE]
    > You must link these row codes to your financial data. In other words, the row must have a dimension value in its **Link to Financial Dimensions** cell. **Don't** reference a description (**DESC**), calculated (**CALC**), or totaled (**TOT**) row.

The amounts in your balance sheet now balance evenly when rounding is turned on.

> [!NOTE]
> The adjustment limit is applied based on the **Rounding precision** option that you specify for the report definition. For example, if you round your report to thousands and enter **2** in the **Adjustment amount limit** field, you receive a warning message when the value in the **Rounding adjustment row** field increases or decreases by more than 2,000.
...
| Task | Steps |
|------|-------|
| **Create a font style** | 1. In Report designer, on the **Format** menu, select **Styles and formatting**. <br> 1. In the **Styles and formatting** dialog box, select **New**, and then enter a unique name for the new style. <br> 1. Make your font selections, and then select **OK**. |
| **Modify a font style** | 1. In Report designer, on the **Format** menu, select **Styles and formatting**. <br> 1. In the **Styles and formatting** dialog box, select a style to modify, and then select **Modify**. <br> 1. Make your font selections, and then select **OK**. |
| **Apply a font style** | 1. In Report designer, in a definition or column definition, or in headers and footers, select one or more cells. <br> 1. In the **Style** list on the toolbar, select a font style. |

### Format row text

The formatting that you specify in the row definition overrides any formatting that you specify in the column definition and the report definition. You can modify the text format by using the controls on the formatting toolbar. These controls are standard Microsoft Windows controls.

1. In Report designer, open the row definition to modify.
1. Select the cells to format. To select multiple cells, hold down the Ctrl key while you select the cell.
1. Select the toolbar button of the format to apply. For example, to indent a row, select the row, and then select **Increase indent** :::image type="icon" source="../../dev-itpro/analytics/media/indent.gif" border="false"::: on the toolbar.
...
1. In Report designer, open the row definition to modify.
1. On the **Format** menu, select **Column width**.
1. In the **Column width** dialog box, enter a value, and then select **OK**. Alternatively, you can drag the right boundary of a column heading cell to change the width of the column.

### Hide columns in the view pane

1. In Report designer, open the row definition to modify.
1. Select the columns to minimize.
1. Right-click, and then select **Hide**.

### Show all hidden columns in the view pane

1. In Report designer, open the row definition to modify.
1. Right-click the minimized column to display, and then select **Unhide**.
