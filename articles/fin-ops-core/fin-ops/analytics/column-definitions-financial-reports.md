...
description: Learn about column definitions. A column definition is a report component that defines the contents of columns on a report.
author: aprilolson
ms.author: aolson
ms.topic: article
ms.date: 06/01/2026
...
1. In Report designer, in the navigation pane, select **Column definitions**.
1. On the **File** menu, select **New**, and then select **Column definition**.
...
1. In Report designer, select **Column definitions**, and then open the column definition to modify.
1. Select the column where you want to insert a new column.
1. On the **Edit** menu, select **Insert column**. The new column appears to the left of the column that you selected.

### Delete a column from a column definition

1. In Report designer, select **Column definitions**, and then open the column definition to modify.
1. Select the column to delete.
1. On the **Edit** menu, select **Delete column**.
...
Use column restrictions to specify how a column definition uses data or calculates information. You can also restrict a report column to a specific unit or for specific dates.

> [!NOTE]
> A **Column restriction** code overrides any conflicting setting that you assign in the row definition.
...
1. In the **Column restrictions** dialog box, select one or more codes in the list, and then select **OK**.
...
| Field                 | Description |
|-----------------------|-------------|
| Column header text    | This text appears in the column header. Type text directly in this field, or select **Insert autoText** to choose an option that updates the column header every time the report is generated. To include multiple autotext codes, select **Insert autoText** again, and then select another code in the list. |
...
| Justification         | Specify how the column header text should be aligned for the column or range of columns that you specify in the **Spread from** and **Spread to** fields. |

### Create a column header

1. In Report designer, open the column definition to modify.
1. Double-click a header cell.
1. In the **Column header** dialog box, enter the column header text. Alternatively, select **Insert autoText**, and choose an option.
...
1. On the **Edit** menu, select **Insert row**. The new row is inserted above the row that you selected in step 2.

> [!NOTE]
> If you have four or more rows of report headers on a report, the headers overlap when you export the report to an Excel worksheet. To view all headers on the report, increase the top margin in the report definition.

### Delete a column header row

1. In Report designer, open the column definition to modify.
1. In the header row, select the cell to delete.
1. On the **Edit** menu, select **Delete row**.
...
> [!NOTE]
> The format of some of the text, such as the long date, depends on your regional settings on the server. To change these settings, select the **Start** button, select **Control Panel**, and then select **Region and Language**. The following table lists the available autotext options for column headers.
...
Conditional spanning headers can span multiple columns that are based on specific period data. For example, if you have a budget report for the fiscal year and want to display the actual budgets of past months together with the projected budgets of future months, use a conditional spanning header to automatically update the report header. Be aware of the following situations when you create a conditional spanning header:

- The process ignores any stop condition (**Spread to** field) that it finds before a start condition (**Spread from** field). For example, column B has the spread condition defined as BASE+1 to BASE, BASE is in column C, and BASE+1 is in column D. In this case, the stop condition in column C is ignored, and the printing of the header starts at column D.
...
After entering information, the user select **OK**. The user then double-clicks the column header cell in column C to open the **Column header** dialog box, and enters the following information.
...
Now, every time that this report is generated, the word "Actual" prints over the columns that contain actual data, and the word "Budget" prints over the columns that contain budget forecasts. Additionally, the number of columns adjusts each month.

## Apply column justification

Use the **Justification** cell to apply justification formatting to a description column in a report. This option affects only the column descriptions, not the actual values.
...
In the column definition, the formatting column detail rows apply special formatting to selected columns. Although some of the **Print control** options and **Column restrictions** options are specific to **FD** columns, most of the options apply to all column types. The formatting that you specify in the column definition overrides the formatting that you specify in the report definition. However, the formatting that you specify in the row definition overrides the formatting that you specify in the column definition. The following rows are considered formatting rows:
...
| ACCT (Account codes) | Display the financial data segment values or dimension values that apply to each row. For account and transaction detail reports, the fully qualified account is printed (for example, **110140-070-0101**). If ranges are specified in the **Link to Financial Dimensions** column in an associated row definition, the range is enclosed in square brackets and is treated as a single value (for example, **[110140:110700]-070-[0101:0200]**). For financial reports and high-level reports that are a combination of several accounts, the financial data link from the row definition is printed (for example, **1100:1200**). |
...
| Option  | Description                                                                                 |
|---------|-----------------------------------------------------------------------------------------------------|
| BASE    | Use the base year that you specify at report time.                                                 |
...
The **Period** cell identifies the fiscal periods that the column should include. The period can be relative to the base period that you specify when generating the report. The following options are available.
...
| \#              | Always use a specific period number. Don't use this option, because it reduces the flexibility of the column definition. |
| \#:\#           | Always use a specific range of periods. Don't use this option, because it reduces the flexibility of the column definition. |
...
- **Currency filter** – Specify a currency filter. The report displays only transactions that are entered in the selected currency.

To determine a company's accounting currency, follow these steps:

1. In Report designer, on the **Company** menu, select **Companies**.
1. In the **Companies** dialog box, select a company, and then select **View**.
...
A user makes the following currency selections in their column definition:
...
The following table shows the report results that can occur for various combinations of the options in **Currency display** and **Currency filter** cells because of the selections that you make. The functional currency is USD.

| Currency Display cell                        | Currency Filter cell | Report result |
|----------------------------------------------|----------------------|---------------|
| Transaction currency          | **YEN**              | **Y6,000** – The result shows only transactions that are entered in JPY. |
| Accounting currency from ledger | **YEN**              |**$60** – The result shows only transactions that are entered in JPY and displays those transactions in USD.<p><strong>Note:</strong> The conversion rate is approximately 100 JPY per USD.</p> |
...
> [!NOTE]
> When you use a base row calculation in a column definition, make sure that each row definition that you use with this column definition contains at least one base row for calculations.
