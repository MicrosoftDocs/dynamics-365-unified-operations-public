...
## If the party type is Legal entity, Organization, or Team, will the global address book synchronize the customer record?

No, the global address book won't synchronize the fields. Because Customer's V3 mapping supports only the **Person** party type, the record doesn't reach Dataverse, and no errors are displayed. A separate party record of the **Person** type must be created.
