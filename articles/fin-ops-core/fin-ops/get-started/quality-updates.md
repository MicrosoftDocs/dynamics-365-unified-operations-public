...
- **Visibility** – We will send notifications through emails and LCS, for upcoming proactive quality updates. In addition, support teams and incident leads will have visibility into where quality updates have been proactively deployed.
...
- **Sandbox sync designation** – Less than 20% of customers today have multiple sandboxes, and keep one sandbox deployed where the version matches production, to help with troubleshooting. In the near future, we will introduce the ability for customers to specify a sandbox environment that should not receive the proactive update deployment together with other sandboxes but should instead receive it later, together with their production environment. Note that if a customer is using a sandbox to test a newer version than their production, that sandbox will receive quality updates to the newer version.
