...
- To add a tile to a workspace, first filter the list on the page so that it shows the data that you want to be summarized or that you want quick access to. [Note that when saved views are enabled, you will need to save a view with these conditions before proceeding.] Then select **Add to workspace**. Select a workspace, and then select **Tile** in the **Presentation** field. After you select **Configure**, a dialog box appears, where you can specify the label to use for the tile in the workspace. You can also specify whether the tile should show a count. After the tile is added to the workspace, you can select it to open the current page from the workspace and view the filtered list that is associated with the tile.
...
- **Unpublished views** – These are template views that have been imported into your system, but have not yet been published. You can publish, export, or delete these views.  
