...
description: Learn about the features that are included in the platform updates for version 10.0.32 of finance and operations apps released in March 2023.
author: twheeloc
ms.author: sericks
ms.topic: whats-new
ms.custom: 
...
| Web client | <p>**Improvements to the grid control**</p><p>Grids are a critical component of enterprise resource planning (ERP) applications. They're used extensively throughout all finance and operations apps, and cater to a range of user scenarios. In this release wave, several improvements have been made to the grid control to help users be more efficient and productive when they interact with a grid. For example, fast data entry is more robust, user efficiency inside the grid from the keyboard has been improved, users can select a range of cells in the grid for copy and paste operations, and the aggregation feature in the grid has been enhanced to show aggregate values for just the selected rows. </p> | [Grid capabilities](../../dev-itpro/get-started/grid-capabilities.md) | Default |
