...
| Module or feature area | Feature name | More information | Enabled by |
|---|---|---|---|
| Platform | Change to query timeout settings | The default timeout settings have been updated so that axOnline interactive workloads that previously had a 30‑minute timeout now use a five‑minute timeout. This platform‑level configuration change is designed to improve system responsiveness and overall stability. Applying it globally ensures consistency and helps prevent performance degradation. The phased production rollout is scheduled to begin on January 20, 2026. <br><br>This change is documented under the Dynamics 365 Finance and Operations 10.0.46 platform updates to maintain transparency and provide clear guidance to customers. The rollout is global and not tied to any specific PU version. | On by default |
...
| System Administration | DMF Telemetry integrates with Application Insights| We’re enhancing the Monitoring and Telemetry feature by introducing **DMF telemetry integration** with Application Insights. This capability is currently behind feature flights and the underlying telemetry is available for preview in early April 2026. For more details, see [DMF telemetry](../../dev-itpro/monitoring-telemetry/monitoring-available-telemetry.md#dmf-telemetry) | Feature Management |
