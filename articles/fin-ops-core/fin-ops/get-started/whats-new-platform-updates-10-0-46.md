...
| Module or feature area | Feature name | More information | Enabled by |
|---|---|---|---|
| Platform | Change to query timeout settings | The default timeout settings have been updated so that axOnline interactive workloads that previously had a 30‑minute timeout now use a five‑minute timeout. This platform‑level configuration change is designed to improve system responsiveness and overall stability. Applying it globally ensures consistency and helps prevent performance degradation. The phased production rollout is scheduled to begin on January 20, 2026. <br><br>This change is documented under the 10.0.46 platform updates to maintain transparency and provide clear guidance to customers. Although the rollout is global and not tied to any specific PU version, intention to include it in the release notes is to help customers prepare and adjust long‑running queries. | On by default |
