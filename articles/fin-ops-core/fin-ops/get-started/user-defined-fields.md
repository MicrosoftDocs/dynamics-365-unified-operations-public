...
### Why can I not enter a value to my custom field? 

If you cannot type a value into the custom field when the page is in Edit mode, that typically is because the table the field was added to is currently read-only. All fields in a table becomes read-only if the backing table is currently configured as read-only on the page.   
