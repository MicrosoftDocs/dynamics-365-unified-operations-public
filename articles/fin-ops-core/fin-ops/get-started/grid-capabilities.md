...
## Typing ahead of the system
In many business scenarios, the ability to quickly enter data into the system is very important. Prior to the introduction of the new grid control, users could change data only in the current row. This meant that after making edits to a row, users could not switch to a different row or create a new row until the system successfully validated any changes in the current row and (in the case of row creation) ran all the logic associated with creating a new row. To reduce the amount of time users spend waiting for these operations to complete and to improve user productivity, the new grid adjusts these actions so that they are asynchronous. Users can create new rows or move to other rows to make changes while previous row validations and row creationg logic are pending. 
...
When you are entering data ahead of the place where the system is processing, you can expect a few changes in the data entry experience: 
-  You will notice a lack of lookup drop-down lists, validation of field values after moving to a different column in the same row, and columns not showing default values initially. This occurs when you are creating or updating ahead of the system, and the standard experience will resume once the system catches up to where you are currently editing. If you made changes to a field that typically has its value defaulted, your changes will overrule the default field value when the server starts processing that row.  
-  If you create a new row with the Down arrow, all columns in the grid will appear as editable and focus will be placed in the first column on the created row by default. This may not be the same column that received initial focus in the legacy grid or the same column that receives focus after selecting a New button. Your organization can customize the system to change the column that receives initial focus when the Down arrow is pressed, if desired (see [Specifying initial focus column when creating new rows with the down arrow](#developer-specifying-initial-focus-column-when-creating-new-rows-with-the-down-arrow) for details). Regardless, you can use personalization to optimize each grid for data entry, specifically by reordering fields so that the first column is the one you want to start in for entering data. You may also want to reorder fields in general for data entry to reduce tab stops and hide any fields that are not needed for data entry in this particular view.
...
## [Developer] Specifying initial focus column when creating new rows with the Down arrow
As discussed in the [Differences when entering data ahead of the system](#differences-when-entering-data-ahead-of-the-system) section, when a user creates a new row with the Down arrow, the default behavior when the "Typing ahead of the system" capability is enabled is to place focus in the first column in the newly created row, which may be a different experience compared to the legacy grid or when a New button is selected. While users and organizations can create saved views that are optimized for data entry (for example, reordering columns so the first column is the one you want to start in for entering data), organizations can also adjust this behavior starting in version 10.0.29 using the **selectedControlOnCreate()** method. 

The **selectedControlOnCreate()** method allows the developer to specify the column that should get initial focus when a new row is created with the Down arrow. As input, this API takes the control ID corresponding to the column that should receive initial focus.  
