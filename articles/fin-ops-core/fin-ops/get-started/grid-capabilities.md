...
## Typing ahead of the system
In many business scenarios, the ability to quickly enter data into the system is very important. Before the new grid control was introduced, users could change data only in the current row. Before they could switch to a different row or create a new row, users were forced to wait for the system to successfully validate any changes in the current row and (in the case of row creation) to run all the logic associated with creating a new row. To reduce the amount of time users spend waiting for these operations to complete and to improve user productivity, the new grid adjusts these actions so that they are asynchronous. Users can create new rows or move to other rows to make changes while previous row validations and row creationg logic are pending. 
