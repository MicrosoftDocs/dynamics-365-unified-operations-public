...
A single column is limited to showing one type of calculated value; however, different columns in the grid may each be configured to show a different type of calculated value.  
...
- Right-click in the column that you want to see a calculated value for. If the **Extended grid aggregation capabilities** feature is enabled, then select **View column totals** and choose the desired calculated value. If that feature is not enabled, then select **Total this column** . This action causes three events to occur:
    ...
    3. The desired calculation is initiated for this column and any other columns that you previously configured to see a calculated value for. The time required to show the calculated value(s) depends on the size of the dataset.

- After the footer is visible, select **Show total** (or **Select calculated value** if the **Extended grid aggregation capabilities** feature is enabled) in the footer area at the bottom of the column that you want to see a calculated value for. If there are no configured columns, this button will be available in the footer in all numeric columns. 

    After at least one column is configured to show a calculated value, the **Show total** (or **Select calculated value**) buttons will be available only on hover or focus. The action of selecting this button just saves your preference for seeing a calculated value in this column, so that the preference is applied during future visits to the page. In the footer, this state is indicated by a dash that appears in the column. (Note that if the dataset is small enough, the calculated value will display immediately.)

If you make a mistake and no longer want to see a calculated value in a particular column, right-click on the column and select **Hide total** (or **View column totals > None** if the **Extended grid aggregation capabilities** feature is enabled) or select the **Hide total** (or **Hide calculated value**) button in the footer in that column. This preference will also be saved for future visits to the page. 

### Calculating aggregate values
When you come to a page with the footer visible and columns already configured for calculated values, those values may or may not be shown in the footer. The behavior is dependent on the size of the dataset on the page. If the dataset is sufficiently small, the calculated values will be shown automatically, along with the number of rows in the dataset. If there are dashes in the footer under the columns you configured, then the dataset is too large for the system to show calculated values immediately, and an explicit action is needed to calculate these values. To do this, click the **Calculate** button in the footer, or right-click on a column you want a total for and select **Total this column** (or **View column totals** and select the calcualted value of your choice if the **Extended grid aggregation capabilities** feature is enabled.

If the calculation takes a long time to complete, you can cancel the operation at any time by selecting the **Cancel** button. Sometimes, the dataset will be too large to calculate aggregate values (a limit imposed by your organization), and you will instead be notified to filter your data more. 

> [!NOTE]
> System administrations can modify the limit for the number of records available for calculating aggregates by adjusting the **Maximum number of local records for each grid** parameter on the **Client performance options** page. The default value is 25,000 records. Administrators should be careful when adjusting this value because a value that is too large can exhaust the available memory on the user's machine. The recommendation is to not exceed 50,000 records.   

Calculated values will update automatically as you update, delete, or create rows in the dataset.
...
- Calculated values for any configured column (e.g. subtotals if the column is configured to show a total)
...
If you sort on a non-grouped column, the grouping remains intact, and the data is sorted inside each group, based on the selected column.
...
When grouping on Date or DateTime fields, you have the option to group by year, month, or day. The group "value" in the corresponding header row will match the format from that field. Additionally, for DateTime and Time fields, you can group by hour, minute, or second.

> [!WARNING]
> Users currently cannot add a grouping column after grouping on a segment of a date or time column.
...
This feature was enabled by default starting in version 10.0.21 and is targeted to become mandatory in October 2022.  
...
This API will be deprecated in the future to allow removal of the legacy grid control, but will be available for at least 12 months after its deprecation is announced.  If any issues require that this API be used, report them to Microsoft.
