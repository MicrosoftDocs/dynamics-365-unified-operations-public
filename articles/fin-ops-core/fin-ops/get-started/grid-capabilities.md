...
When users are entering data ahead of where the server is processing, users can expect a few degradations to the data entry experience, including a lack of lookups, control level validation, and value defaluting. If users need a dropdown to find a value, users are encouraged to wait for the server to catch up to the current row. Additionally, control level validation and value defaulting will occur when the server is processing that row.   
