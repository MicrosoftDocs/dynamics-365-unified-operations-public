...
Client alerts depend on the email subsystem that Microsoft Office integration provides. We recommend that you use the Simple Mail Transfer Protocol (SMTP) provider, so that email distribution doesn't have to rely on a local mail client.
...
:::image type="content" source="./media/Create-alert-rule-form.png" alt-text="Screenshot of Create alert rule dialog box, where the Send email option is set to Yes.":::

> [!NOTE]
> When the **Send email** option is set to **Yes**, alert notifications continue to be delivered from the Action Center.
...
The following image shows the structure of the alert notifications when they're received by email.

:::image type="content" source="./media/Alert-email-templates.png" alt-text="Screenshot of template-based alert notifications for record creation, field changes, and template deletion.":::
