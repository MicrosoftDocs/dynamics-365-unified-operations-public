...
| **Deployment option**              | All |
| **Status**                         | <p>This change is now live across all applicable environments. As a result, all legacy mechanisms that rely on storage account connection strings or shared key–based authentication will no longer work. Customers must use Managed Identity–based authentication and supported platform APIs for all interactions with Finance and Operations–managed storage accounts. Any integrations, customizations, or third‑party solutions that continue to rely on deprecated approaches will fail. This change is enforced on all environments running version 10.0.43 (PU67) and later, with no exceptions. Customers are required to update their implementations immediately to ensure continued functionality. </p><p>For more information, and for updates about this change, see [Finance and operations storage account security updates](../../dev-itpro/sysadmin/storage-acct-security.md).</p> |
