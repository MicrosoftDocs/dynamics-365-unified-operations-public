...
| Web client | Microsoft Graph mail provider | This mail provider replaces the deprecated Exchange provider, which stops working in mid September 2024. For more information, see [Configure or send email](../../dev-itpro/organization-administration/configure-email.md#send-email-with-microsoft-graph). | Admin configuration |
...
| System Administration | Batch Header now has a method BatchHeader::isCurrentBatchTaskBeingCancelled() which you can use in batch classes to immediately return and cancel execution if needed. | |
...
| System Administration | Batch Job History | If the batch job has more than 5,000 batch tasks, the corresponding job history saves only the first 2,500 tasks, preferring tasks with status in the following order: **Error -> Canceled -> Finished -> Not Run**. This measure prevents blocking batch related tables that might occur due to such large jobs. For more information, see [Create a batch job](../sysadmin/create-batch-job.md). | Default |
...
This feature replaces the SHA1 algorithm. 
...
functional updates that you must make to the compiler.
