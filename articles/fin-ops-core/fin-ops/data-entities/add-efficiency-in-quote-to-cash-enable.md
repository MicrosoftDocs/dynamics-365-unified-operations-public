...
1. On the **Dynamics 365 Sales integration** tab, set the fields that are described in the following table. A tooltip is also provided for each field on the page.
    ...
    | **General** | **Copy quotation data to sales orders** | *Integrate Sales Quotation lifecycle with Dynamics 365 Sales* | Choose whether or not to copy Supply Chain Management sales quotation data to sales orders synced from Dynamics 365 Sales. Learn more in [Copy Supply Chain Management sales quotation data to sales orders synced from Sales](add-efficiency-in-quote-to-cash-use.md#copy-quotation-data). |
