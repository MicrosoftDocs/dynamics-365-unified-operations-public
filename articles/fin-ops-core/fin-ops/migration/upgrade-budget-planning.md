...
> [!NOTE]
> The system doesn't set new required fields. If you get stuck on a page, such as the **Budget planning configuration** page, and can't navigate away, close your browser and then reopen it to a different page to enter details in the correct order. Because these required fields aren't yet set, you might encounter problems until you configure everything and set all required fields. This article explains how to set these fields. Here are some of these required fields:
...
1. On **Budget planning configuration**, select the **Columns** tab. As part of the upgrade, the system automatically creates new columns based on your budget plan lines. These columns use dynamic dates, where the time and year are offset from the fiscal year that you define in the Budget planning process.
...
- Budget plan lines for all scenarios in 2017, and Actuals for both 2017 and 2016
   ...
   In this example, the system creates a column named **Jan Scenario 1** for the most recent budget plan transaction data where transactions exist in January. The system creates a similar column for each scenario that has data. After columns exist for all periods in that year, the system creates columns for previous years.

1. Change the column names and descriptions, and any other details, either manually in the client or by doing bulk updates through the Excel add-in that points to the budget plan columns data entity. Set any filters that you previously set for matrix fields within the columns.
...
1. Specify the default account structure for this budget process. Set the **Default account structure** field.
...
A new allocation method lets you distribute from a parent plan to a child plan without going through an intermediate level in the hierarchy. This method is especially useful for customers who previously created financial dimensions just for budget distribution and approvals.
...
In AX 2012, budget planning stored a single plan amount for each value. In Finance, the data model is expanded. There are now accounting currency, transaction currency, and reporting currency amounts for each value. During the upgrade, the system automatically fills in these new columns for existing data.

### Don't convert currency in aggregation
...
### Look back from a budget plan to other modules that contributed to the budget

You can generate budget plans from demand or supply forecasts, project, and other areas. The **Budget plans by dimension set** inquiry includes several options that you can use to run queries to identify the data that was the source for the budget plan.

### Overwrite or append to plan for allocation schedules

If you must distribute amounts from multiple sources, you can specify that the amounts are additive. In this case, the amounts don't overwrite any existing amounts. Instead, they are appended to the existing amounts.
