...
- **Job location** – Specify the job's location, so that candidates can determine whether the position is geographically suitable for them.
...
Hiring managers in Dynamics 365 Finance use a *recruiting request* to create the requirement for job openings. A recruiting request has an **Active** status to be published. After you publish a recruiting request, you can't update it. For more information, see [Recruit job candidates](hr-personnel-recruit.md).
...
- **In progress** – The job ad is saved, and the process of adding hiring team members, adding a hiring template, or gaining approval is in progress.
- **Active** – The job ad is posted to the careers site. After a job ad is posted, you can't change its details.
- **Inactive** – The job ad is deactivated after it's posted to job boards.
...
1. Select **Search** for the **Screening** template. This action displays the **Screening** templates.
1. Select the **Screening** template from the list. You can select multiple templates.
1. View all the questions on the selected templates.
...
When you change the stage to **Approval**, approvers receive notifications in Teams, Outlook, and the Recruiting add-on app. They can use any of these channels to approve or reject the job ad.
...
To post a new job ad by using a recruiting request, follow these steps:
...
1. Choose **+ New**.
1. Select a recruiting request. The app extracts and displays all the information you added as part of the recruiting request in the relevant fields.
1. Fill in the other required fields.
1. Select **Save**.
1. Go to the **Hiring team** tab and add hiring team members.
1. Go to the **Hiring process** tab and select a hiring template.
1. Go to the **Approvals** tab, optionally turn on the **Approval required** tab, and add approvers.
1. Move the business process flow from draft to the next stage.
...
1. Select **Save**.
1. Go to the **Hiring team** tab and add hiring team members.
1. Go to the **Hiring process** tab and pick a hiring template.
1. Go to the **Approvals** tab, optionally turn on the **Approval required** tab, and add approvers.
1. Move the business process flow from draft to the next stage.
