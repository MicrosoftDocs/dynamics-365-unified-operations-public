...
- In General Ledger, go to **Chart of accounts** > **Dimensions** > **Financial dimension configuration for integrating applications** > **Data entities**. Define financial dimension formats for data entities.

> [!NOTE]
> Each legal entity has unique Human Resources parameters, so you must configure them for each entity.
> Coordinate with Ceridian to determine which financial dimensions align with the Dayforce site (location).

> [!IMPORTANT]  
> For customers using **mshr entities**, disable row version change tracking. Contact Microsoft support to enable the `DMFDisableSqlRowVersionCtForCDSVirtualEntity` flight.

## Enable features

Enable features in Feature management to expose Microsoft APIs to the integration and pass data to Dayforce. Some features might already be enabled by default.
...
1. In the **Redirect URI** field, select Web and enter the namespace URL of your FinOps application like `https://<hostname>.operations.dynamics.com`.
1. Select **Register**.
1. When registration completes, the Azure portal displays the app registration's **Overview** pane, which includes its **Application (client) ID**. Note the Application (client) ID.
...
1. Record the secret's value from the **Value** property of the table.
...
1. Select Dynamics 365 apps under the **Resources** tile.
1. Select **Install app**.
1. Select **Dynamics 365 HR Virtual Tables app**.
...
1. Go to tables and search for **Finance and Operations Virtual Data source configuration table**.
1. Select **Microsoft HR Data Source** record and enter the following information.
1. Enter the required information for the data source configuration:
      ...
      - **Microsoft Entra Application ID (Microsoft Entra ID ApplicationId)**: The application (client) ID created for the application registered in the Microsoft Azure portal. You received this information during the [Register the app in Microsoft Azure](#register-the-app-in-microsoft-azure) step.
      ...
      - **Microsoft Entra ID Resource**: Put this value 00000015-0000-0000-c000-000000000000.
...
1. In Dynamics 365 Finance and Operations, go to **System administration** > **Setup** > **Microsoft Entra ID applications**.
...
1. Open the [Maker portal](https://make.powerapps.com/) and select your environment.
1. Select **Available HR Entities** table.
1. Select the table or tables you want to generate in Dataverse.
1. Select **Has Been Generated/refresh**.
...
The change tracking feature in Dataverse detects data that changed since the data was originally extracted or last synced. For more information, see [Use change tracking to synchronize data with external systems](/power-apps/developer/data-platform/use-change-tracking-synchronize-data-external-systems).  
...
1. Enter values in the **User ID**, **User name**, **User email**, and **Role** fields.
1. Use the previously created API Dayforce Connector role.
...
1. Configure an API role for the Dataverse environment.
...
1. Enable the URL for the company.
...
1. Add the Dayforce people connector application. The app ID is **6817703f-e5b3-4eec-b11f-ba6367f1b156**.
1. Give the application the following security roles:
...
1. Add the Dayforce payroll connector that has the client ID **6817703f-e5b3-4eec-b11f-ba6367f1b156** and the user ID **DFAPIConnector**.

## Ready to pay

> [!IMPORTANT]
> Before you can integrate employees into Dayforce, mark them **Ready to pay**. The integration doesn't pick up employees that aren't marked **Ready to pay**.

The Dayforce people connector integration uses Microsoft's ready to pay feature to ensure that a complete, valid employee profile exists before payroll is processed. For more information, see [Ready to pay](hr-compensation-payroll.md).
