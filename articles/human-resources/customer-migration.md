...
Two categories of Microsoft Dynamics 365 Human Resources customers are affected and need to make changes to ensure that they're on the latest finance and operations infrastructure:
...
- Customers who use both Dynamics 365 Human Resources and other finance and operations infrastructure environments that are currently integrated must continue to integrate the two environments after the merge.
- Customers who choose to keep their Dynamics 365 Human Resources environment separate from their existing finance and operations app environment must continue to integrate the environments to make the data available in both environments.
- Customers can continue to use the Data Integrator to copy data between the two environments. Customers who merge data into a single environment after the migration no longer need to integrate the data, because all data is in a single database.
...
The migration experience for each independent software vendor (ISV) solution varies, depending on the integration method of the solution. Microsoft works closely with ISVs to help provide a smooth transition to the finance and operations infrastructure. Many ISV solutions are built on Dataverse by using capabilities of Microsoft Power Platform. These solutions depend on the Dataverse integration between the Dynamics 365 Human Resources environment and the Dataverse environment. The Dataverse integration is being deprecated as part of the infrastructure merge. In the finance and operations infrastructure, new dual-write maps are planned to replace the functionality of the Dataverse integration.

## How is the Data Integrator that moves data between Dynamics 365 Human Resources and other Dynamics 365 apps affected?

After customers move to the finance and operations infrastructure, they must continue to integrate data between the two environments. Customers can continue to use the Data Integrator to perform these data migration tasks. Customers who plan to keep their Dynamics 365 Human Resources environment separate from their existing finance and operations apps environment must continue to integrate the data between the two environments. For customers who merge the two environments into a single environment, integration between the two environments is no longer required.

## How is data moved between Dynamics 365 Human Resources on the finance and operations infrastructure and Dataverse after the migration?

The current Dataverse integration between Dynamics 365 Human Resources and Dataverse is being deprecated as part of the finance and operations infrastructure. There are plans to introduce dual-write maps to map the finance and operations entities to the Dataverse tables. Customers must decide which dual-write maps are required for their implementation. Use virtual tables for integrations and ISV solutions, unless there's a specific business need for the data to be duplicated in Dataverse to run business logic.

## How does the migration effect Dataverse extensions for Dynamics 365 Human Resources?

Customers must migrate to a sandbox environment before they migrate their production environment. When customers migrate their sandbox environment, they must create a copy of their Dataverse environment to connect to the new finance and operations migrated environment. Customers should copy both the data and the solutions for the environment, so that they match the customer's current production environment.

When customers are ready to migrate their production Dynamics 365 Human Resources environment, Microsoft automatically migrates the database and Azure Blob storage. The migrated database and storage point the new environment on the finance and operations infrastructure to the same production Dataverse environment. Before the data can continue to be copied to Dataverse, customers must enable any dual-write maps that are required for their integrations, extensions, and ISV solutions that are built on Dataverse.

## Will Microsoft Power Platform components that I built for Dynamics 365 Human Resources automatically work after the infrastructure migration is completed?

Apps that you create in Power Apps, flows that you create in Power Automate, and other Microsoft Power Platform customizations are like Dataverse extensions. During the migration of customer production environments, there's no impact on Dataverse environments, including any extensions. Apps, flows, and other Microsoft Power Platform customizations should continue to work without requiring additional configuration.

## What happens to Dataverse virtual table entities for Dynamics 365 Human Resources during the migration?

When you migrate your Dynamics 365 Human Resources environment to the finance and operations infrastructure, your environment remains connected to the Dataverse environment that currently connects to Dynamics 365 Human Resources. The Dataverse virtual tables that are generated in the environment continue to work without requiring any additional configuration. You might need to regenerate the virtual tables in the Dataverse environment that connects to your existing finance and operations environment.

## How does an integration that uses the Applicant Tracking System (ATS) API, Payroll API, Dataverse virtual tables for Dynamics 365 Human Resources, or other entities in the Dataverse Web API work after the infrastructure merge is completed?
...
## Our organization configured custom security in Dynamics 365 Human Resources. Does it still apply after the infrastructure migration is completed?

Yes, the data migration includes custom security configurations.

## Are workflows in Dynamics 365 Human Resources automatically migrated?

Yes, the migration includes workflow configurations, workflow history, and current in-process workflows to the merged infrastructure.

## Are saved views in Dynamics 365 Human Resources automatically migrated?

Yes, saved views are migrated to the merged infrastructure.

## Will features that are enabled in Dynamics 365 Human Resources automatically be available after the infrastructure merge?

- For customers who use the **Human Resources** module, manage features that are available only in Dynamics 365 Human Resources through Feature management. Usually, these features aren't enabled by default. Any features that must be enabled by default are documented.
- For customers who use Dynamics 365 Human Resources, features are available in the infrastructure. Any features that aren't available are documented. Every Feature management key that is enabled in the current Dynamics 365 Human Resources environment is enabled in the merged infrastructure. For more information, see [Feature management overview](../fin-ops-core/fin-ops/get-started/feature-management/feature-management-overview.md).

## What happens to BYOD databases during the migration?

The migration process moves import and export configurations for bring your own database (BYOD) to the finance and operations infrastructure.

## Is there an impact on the Azure region when the customer environment is migrated?

The Dynamics 365 Human Resources environment stays in the same Azure region during the migration. If you need to move your environment to a different Azure region, follow the standard steps to migrate to a new region after the migration finishes.

## What happens to Azure data lakes during the migration?

Any export that you currently configure for Azure Data Lake in finance and operations apps maintains the same configuration after the migration.

> [!NOTE]
> You must enable dual-write maps to continue writing data from the Human Resources environment into Dataverse.
...
## What happens to custom integrations to external systems?

Customers must migrate their integrations to the finance and operations environment. Because the endpoint of this environment is different, customers might need to update or change the integrations so that they point to the new environment. This task is primarily a manual process, and the changes that are required depend on the architecture of the integration. Customers should work with their Microsoft partner to determine the best approach for moving integrations.

## What happens to Microsoft Power Platform (Dataverse) extensions when you merge a Dynamics 365 Human Resources environment with an existing finance and operations environment?

If you decide to merge a Dynamics 365 Human Resources environment and an existing finance and operations environment into a single Dataverse instance after the migration, you must combine your Dataverse environments as part of the merge process. This task requires redeploying any apps that you created by using Power Apps, along with any other customizations, in the new environment.

## What happens to documents during the migration?

When you migrate your Dynamics 365 Human Resources environment to the finance and operations infrastructure, the documents remain in the existing document storage and automatically map to the new environment in the finance and operations infrastructure.

In a future release, Microsoft provides new data entities. After that change, customers who choose to merge their Dynamics 365 Human Resources environment with an existing finance and operations environment can export documents and move them into the existing environment.

## After the migration, what happens to the batch jobs that you configured in Dynamics 365 Human Resources?

Batch jobs have to be reconfigured in the finance and operations environment. Customers have to evaluate each batch job and compare it to the current list of batch jobs in the finance and operations environment. Customers should also analyze the overall batch schedule when they merge the two sets of batch jobs, to determine whether changes should be made to the batch schedule, priorities, or batch groups.

## How does the migration affect the LCS project for Dynamics 365 Human Resources?

Customers are required to create a new Microsoft Dynamics Lifecycle Service (LCS) project, and they'll have to select finance and operations apps as the product and **Implementation** as the project type. Additionally, a new field for the subproject type is available when an LCS project is created. Customers have to select the option for Dynamics 365 Human Resources migration to migrate an existing Human Resources LCS project to the finance and operations infrastructure.

The features of finance and operations LCS projects differ from the features of Human Resources LCS projects.

To go live, new customers need to complete the following tasks:
...
## How is the Business process modeler migrated?
