...
This article describes the process for creating a questionnaire. The first step is to design the questionnaire. When you design a questionnaire, you not only write the questions and answers, but also create the structure that enables recording and tabulating answers.
...
- Write questions that appear on the questionnaire, and include answer choices, if applicable.
- Ensure that the questionnaire flows logically, so that respondents remain engaged.
- Arrange questions and answers in the order that you present them to respondents.
- Decide how to evaluate the results, if applicable.
- Decide whether you need additional features. For example, determine whether and how to categorize results, whether a time limit is required, or whether all the questions are mandatory.
...
1. Set up answer groups and answers, if applicable.
1. Set up questions and their association with the answer groups.
1. Set up the questionnaire itself, and attach questions to it.

## Prerequisites

Before you can create questionnaires, answers, and questions, you must have some prerequisites in place. However, not all prerequisites are required for full functionality.
...
You must assign **questionnaire types** when you create a questionnaire. **Questionnaire types** help you manage and classify questionnaires more easily. Use questionnaire types to classify questionnaires and differentiate them from each other. For example, if you have multiple questionnaires to select from, you can filter them by type to help make it easier to find a particular questionnaire. Here are some examples of questionnaire types:
...
The number sequence codes organize questions and answers. Based on these number sequence codes, values are automatically assigned to items.

Define all parameters before you begin to create your data. You can modify the questionnaire parameter settings at any time.
...
- Closed-ended questions require that the respondent select an answer from a list of possible correct answers.

To provide a list of possible answers for closed-ended questions, create answers on the **Answer groups** page.
...
You can use an **Answer group** for more than one question on the same questionnaire, and you can also use it on more than one questionnaire.

> [!NOTE]
> If you modify answer text in answer groups that you already used on completed questionnaires, data can become difficult to evaluate, and questionnaire results might no longer be valid. If you must change an answer group, consider creating a new answer group instead of changing an existing one. You can't delete answer groups that are attached to a question or answer, or that are answered.
...
You can arrange questions in a conditional question hierarchy, so that secondary questions depend on the answer that the respondent selects for the previous question. You can write the questions first and then arrange them into a hierarchy later.
...
- Whether the questionnaire is available to a restricted set of respondents.
- Whether to display a form template as a background behind each page of the questionnaire.
- Whether to require additional notes to clarify the intent of the questionnaire for the respondents.
- Whether to display questions in a fixed order or randomly select them from a pool.
- Whether to ask questions only if specific responses are given for previous answers.

### Set up a questionnaire

Use the **Questionnaires** page to set up a questionnaire. To set up a questionnaire, complete the following tasks in order:

1. Create a questionnaire.
1. Attach questions to the questionnaire by using one of the following methods:
    - If you're using result groups, attach questions to a questionnaire by using result groups. First set up the result groups for the questionnaire, and then add questions to the result groups.
    - If you're not using result groups, attach questions directly to the questionnaire.

1. Set up a conditional question hierarchy, if it's required.
1. Test the questionnaire.

On the **Questionnaires** page, select **Validate** to test whether the questionnaire is assembled correctly. However, it's also a good idea to complete the questionnaire and test it yourself before you distribute it.
...
> [!CAUTION]
> Be careful when you change questionnaires that users already answered. Changes can reduce the accuracy of statistics and therefore make them a poor basis for evaluation. Consider creating a new question instead of changing a question that users already answered.

In a questionnaire, you can't delete the following types of questions:

- Questions that are attached to a questionnaire
- Questions that users already answered and therefore appear in the **Answers** dialog box.

### Result groups

**Result groups** are optional when you attach questions to a questionnaire.

Use a result group to calculate points and categorize the results of a questionnaire. If you use result groups, you can perform the following tasks:
...
- Print a report that shows results for each result group, and also optional points and texts that are based on points that are earned in each result group.

> [!NOTE]
> Before you can set up result groups, complete the following tasks:
...
- Define points for answers in the answer groups that you assign to each question.
- Set up a questionnaire.
...
Set up multiple result groups to evaluate the points that a respondent earns in each category. After a questionnaire is completed, you can view the points that are achieved for each result group.

> [!TIP]
> To evaluate a questionnaire by using points but not separate categories, add all questions to a single result group.

For each result group, you can also set up one or more point-based messages that respondents receive after they complete a questionnaire. The text that is shown can vary, depending on the score that a respondent achieves in a result group. To use point-based messages, define point intervals and a description of each interval. When a respondent achieves a score in a specific interval, the text for that interval is included on the results report.
...
#### Example: Points and texts for result group 3
...
To use point-based messages, set up text intervals for each result group. Each question is assigned two points. Therefore, the maximum point total in each result group is 10.
...
> [!NOTE]
> You can change intervals and texts. However, if a questionnaire is completed, changes might cause differences between previous and new result reports.
...
To use conditional questions to create a question hierarchy in a questionnaire, make the sequence that questions are presented in depend on the answer that a respondent selects for each question. By basing the question sequence on a respondent’s answer, you can modify the questionnaire as the respondent completes it.
