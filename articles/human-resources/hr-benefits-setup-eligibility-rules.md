...
> [!IMPORTANT]
> Every benefit plan must have at least one eligibility rule associated with it. You can associate multiple rules with a benefit.

## Create an eligibility rule

Eligibility rules define which employees can enroll in each benefit plan. After you define eligibility rules, assign them to benefit plans. Then you can process enrollment eligibility to see which employees are eligible for each plan.

During open enrollment, employees can select benefit plans. If they become ineligible for a benefit plan based on eligibility rules after they're already enrolled, they aren't automatically unenrolled. Typically, when a life event occurs that affects plan eligibility, an enrollment period is initiated for the employee to select plans that they're eligible for.

1. In the **Benefits management** workspace, under **Setup**, select **Eligibility rules and options**.

1. On the **Eligibility rules** tab, select **New** to create an eligibility rule. To see plans that are associated with an eligibility rule, select **Attached plans**.
...
1. Specify values for the following fields.
   ...
   | **Enrollment period** | The time period when new hire enrollment is allowed. If you also set this value in parameters, the parameters setting takes precedence over this one. |
   | **Use former employment status** | Specifies whether to use an employee's previous employment status as part of the benefits eligibility rule. For example, you can specify an eligibility rule that waives a coverage waiting period for all employees that transitioned from a **Laid off** status to an **Employed** status within 90 days of their previous employment. |

1. Under **Additional criteria**, select the following options and add information as necessary.
...
1. Under **Additional detail**, you can view the following additional details.
...
You can create [custom fields](hr-developer-custom-fields.md) within Human Resources to track extra information. Add these fields directly to the user interface, and the system dynamically adds a column to the underlying table.  
...
The following tables support custom fields that you can use in eligibility processing:
...
The eligibility process supports the following custom field types:
...
| Field  | Description |
|--------|-------------|
| Name | Name of the criteria that you're creating. |
| Table name | The table name that contains the custom field that you're using for the eligibility rule. |
| Field name | The field that you use for the eligibility rule. |
...
When you assign multiple eligibility rules to a benefit plan, an employee must meet at least one rule to be eligible to enroll in the benefit plan. In the following example, the employee must either meet the requirements of the **Job type** rule or the **Active employees** rule.

:::image type="content" source="media/RulesAssignedToAPlan.png" alt-text="Screenshot of eligibility rules assigned to a plan showing Job type rule and Active employees rule.":::

### Criteria within an eligibility rule

Within a rule, you define the criteria that make up the rule. In the preceding example, the criterion for the **Job type** rule is where Job Type = Directors. Therefore, the employee must be a director to be eligible. This rule has only one criterion within the rule.
...
For example, the **Active employees** rule in the preceding example is made up of the following criteria. In order for the employee to be eligible based on the **Active employees** rule, the employee must be employed in legal entity USMF *and* have a position type of full-time.  

:::image type="content" source="media/CriteriaWithinAnEligibilityRule.png" alt-text="Screenshot of multiple criteria within an eligibility rule.":::

### Multiple conditions within criteria

You can further expand rules to use multiple conditions within a single criterion. The employee must meet at least one condition to be eligible. To build on the preceding example, the **Active employees** rule can be further expanded to include employees that are also part-time employees. As a result, now the employee must be an employee in USMF *and* either a full-time or a part-time employee.  

:::image type="content" source="media/MultipleConditionsWithinCriteria.png" alt-text="Screenshot of multiple conditions within criteria of an eligibility rule.":::

### Eligibility conditions within a custom field criterion

Similar to the preceding example, use custom fields when creating eligibility rules. They work in the same manner. For example, you might want to offer internet reimbursement to the Fargo and Copenhagen employees who are working from home, as the internet costs are higher in those locations. To do this, create two custom fields: **Office location** (picklist) and **Working from home** (check box). Then create a rule called **WFH Employees**. The criterion for the rule is where **Office Location = Fargo** or **Copenhagen** *and*  where **Working from home = Yes**.

Set up the custom eligibility rules as indicated in the following image.

:::image type="content" source="media/EligibilityConditionsWithinACustomFieldCriterion.png" alt-text="Screenshot of eligibility conditions within a custom field criterion.":::

## Configure bundles

Bundles are a set of related benefit plans. Use benefits bundles to group benefit plans that an employee must choose to enroll in certain benefit plans that might depend on other benefit plan enrollments. Examples of when you might want to use a bundle include:

- A health plan bundle that includes high-deductible health insurance with an associated health savings account (HSA).

- A life insurance plan with a mandatory employee life insurance plan bundled with a dependent life plan. This bundle ensures an employee can't select dependent life coverage without signing up for the employee coverage.

1. In the **Benefits management** workspace, under **Setup**, select **Eligibility rules and options**.

1. In the **Bundles** tab, select **New** to create a bundle. To see plans that are associated with a bundle, select **Attached plans**.
...
1. Specify values for the following fields.
   ...
   | **Required**| Indicates that the plan is required to be selected to check out any other plan in the bundle. More than one plan can be marked as **Required**. In that case, all the plans that are marked as **Required** need to be selected to check out any of the plans in the bundle.|
...
Periods define when benefits are in effect and when employees can enroll. Create as many periods as you need to maintain benefits open enrollment and benefit coverage periods. Benefit plan years might not follow a calendar year.

1. In the **Benefits management** workspace, under **Setup**, select **Eligibility rules and options**.

1. In the **Periods** tab, select **New** to create a period. To run a process that attaches all valid active benefit plans to the benefit period, select **Attach plans**. To see plans that are associated with a bundle, select **Attached plans**.
...
1. In the **Periods** tab, select **Flex credit programs**.

1. Select a flex credit program to apply. The fields contain the following information.
   ...
   Ensure that each benefit plan is enrolled in only one flex credit program per benefit period. Otherwise, the system doesn't know which flex credit program to use to grant flex credits and you encounter problems.
...
1. In the **Programs** tab, select **New** to create a program. To make exceptions for employees who don't meet the eligibility rule requirements, select **Eligibility rule override**. To see plans that are associated with a program, select **Attached plans**.
