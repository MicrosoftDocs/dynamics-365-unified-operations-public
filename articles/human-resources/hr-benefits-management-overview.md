...
- Enhanced benefits plans let you create and manage unique benefit plans and support complex benefit rate tables and nested tiers. You can easily create benefit programs, bundles, and autoenrollment rules for an easier employee experience.
...
> [!NOTE]
> You can now customize Benefits management pages. Add custom fields related to coverage rates to the **Coverage option** page for benefit plans. For more information about working with custom fields, see [Custom fields](hr-developer-custom-fields.md).
>
> :::image type="content" source="media/hr-benefits-management-custom-fields.png" alt-text="Screenshot of Benefits management custom fields.":::
...
1. Set up optional benefit information.
1. Set up benefit plans.
1. Set up flex credit programs (optional).
1. Configure required employee information.
1. Configure optional employee information.
1. Process employees to determine eligibility.
1. Employees select plans via employee self-service (optional).
1. Confirm employee plan selections.
1. Life event processing (optional).
1. Rate updates (optional).

## Set up required benefit information

Before you can enroll employees in the plans, set up the following components:

- **Benefit management parameters** – These settings are shared across companies. Set default reason codes, enable the **Benefits annual salary** option, set a default payment frequency for new hires, and enable life events. For more information, see [Set benefits management parameters](hr-benefits-setup-parameters.md).
- **Personal contact eligibility options** – Personal contacts are the individuals who are either dependents or beneficiaries of the plans that you set up. Typically, they're children, spouses, or trust organizations. For more information, see [Configure personal contact eligibility options](hr-benefits-setup-contact-eligibility-options.md).
- **Coverage options** – Set up the types of coverage that are available for a plan. Specifically, define who is covered, or how much coverage is available. For more information, see [Create coverage options](hr-benefits-setup-coverage-options.md).
- **Plan types** – Set up the types of plans that are available when you create a benefit plan. Examples of plan types include **Dental**, **Vision**, and **Savings**. Some important settings on the plan type determine the settings that are available on the benefit plan. For more information, see [Create plan types](hr-benefits-setup-plan-types.md).
- **Eligibility rules** – Use eligibility rules to determine whether an employee is eligible for a plan. Associate at least one eligibility rule with every benefit plan. For more information, see [Configure eligibility rules and options](hr-benefits-setup-eligibility-rules.md).
...
- **Rates** – Rates define how much a benefit costs either the employee or the employer. If money should be allocated back to an employee (for example, a credit toward a gym membership), a negative rate is entered. For more information, see [Configure rates](hr-benefits-setup-rates.md).
...
- **Deductions** – The deductions are basically the header information/codes that will be passed on to the payroll system to identify the deduction for the benefit. You must set up these deductions, because they'll be required on the benefit plan. For more information, see [Configure deductions](hr-benefits-setup-deductions.md).
...
- **Reason codes** – Use reason codes to explain why a benefit might be changing for an employee. For more information, see [Set up reason codes](hr-benefits-setup-reason-codes.md).
...
Use flex credit programs to enroll employees in benefits based on a predetermined number of flex credits. Employees can choose how to allocate their flex credits. For example, if employees are already covered under their spouse's health insurance plan, they don't have to use their credits for health coverage. Therefore, they might want to use their credits for other benefits instead. For more information about flex credit programs, see [Set up flex credit programs](hr-benefits-plans-flex-credit-programs.md).

## Configure required employee information

Before you can enroll employees in benefits, provide the required information for them. 
...
Next, enroll employees in a fixed compensation plan on their start date, or provide an **Annual benefits salary** amount. Before you assign **Fixed compensation** to an employee, assign a **Position**. 

> [!NOTE] 
> The **Fixed compensation start date** can't be before the **Position assignment date**.

Alternatively, if you have an employee who receives supplemental compensation like commissions, you can add a **Benefits annual salary** amount from the employee record. Human Resources uses the **Benefits annual salary** amount when determining coverage amounts, instead of the **Fixed compensation annual** amount. The **Benefits annual salary** must be valid as of the employee's start date or the beginning of the benefit period, whichever is latest. However, a position isn't required to assign the **Benefits annual salary**. To enable the **Benefits annual salary** feature, go to the **Human resources shared parameters** page, on the **Benefits management** tab. This feature is turned off by default.

> [!IMPORTANT]
> If both a **Fixed compensation** and a **Benefits annual salary** amount is entered for an employee, the **Benefits annual salary** is used in determining coverage amounts. In the **Employment details** section of the **Worker** page, you must select a value in the **Benefit pay frequency** field.

## Configure optional employee information
When you create a benefit plan that uses rates that are based on gender or age, enter a birth date and gender for the employee to calculate the benefit cost.

## Process employees to determine eligibility
Before you can enroll employees in plans, run eligibility processing to determine which plans they're eligible for. You can view the results of the eligibility process in the **Process results viewer**. For more information, see [Process enrollment eligibility](hr-benefits-process-enrollment-eligibility.md).

## Employees select plans using **Employee self service** (optional)

When open enrollment occurs, employees are newly hired, or a life event occurs, employees can select or update their benefits by using **Employee self service**. For more information, see [Configure employee self-service](hr-benefits-setup-employee-self-service.md).

## Confirm employee plan selections

You must confirm the benefits that employees select before considering the employees enrolled in them. You can also select benefits on an employee's behalf. To select or confirm benefits, on the **Employee** page, on the **Benefits** tab, select **Worker benefits plans**. To select or confirm benefits for multiple employees, use the **Worker benefit plans bulk update** page.
...
Before you can process life events, you must run open enrollment at least one time during a hiring time frame. In the United States, open enrollment typically occurs one time per year. Outside the United States, open enrollment might occur at the time of hire. Life event processing doesn't require that workers select a benefit plan. However, the workers must be included in open enrollment processing. For more information, see the following articles:
