...
Before you can install the Recruiting app, make sure the following prerequisites are met:
...
- You complete one of the following Microsoft Power Platform integrations:
...
1. Select **Install app**.
1. Search for and install **Dynamics 365 Human Resource recruiting add-on**.
...
> [!NOTE]
> Confirm that all the **HCM Recruiting** flows are active. Otherwise, problems can occur. For example, if the **Portal create candidate** flow isn't active, candidates can't create a profile.

### Recruiting flows

|Name| Description |
|-----|--------|
|Email notification flow  |The primary process for dispatching email notifications utilizes predefined email templates. Different workflows use this process based on the scenario.|
|Job ad approval flow |Notifies approvers through Teams or Outlook for job ad approval.|
|Job ad in app approval flow |Manages in-app approvals and automatically completes the Teams or Outlook workflows if they're already initiated.|
|Publish candidate to Dynamics 365 Finance and Operations |When **Ready to hire** is selected, the candidate's details are forwarded to Dynamics 365 Human Resources.|
...
|When an applicant is deleted| Deletes candidate information in Dynamics 365 Human Resources when an applicant is removed.|
...
|Email lots to candidate and send to plugin| Sends email with options to candidates.|
|Fetch panel members available meeting times slots | Retrieves available Outlook calendar time slots for panel members to schedule interviews.|
