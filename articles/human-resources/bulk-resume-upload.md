...
The Bulk resume upload AI feature in Dynamics 365 Human Resources Recruiting add-on is an AI-powered tool that optimizes and automates the process of uploading, organizing, and analyzing large numbers of resumes simultaneously. Because this feature facilitates more efficient management and processing, it's beneficial to companies and recruiters that receive many applications for open positions.

Recruiters can use the feature to upload multiple resumes simultaneously in various file formats, including PDF, JPG, PNG, or JPEG. Because recruiters don't have to manually upload resumes one at a time, the feature saves significant time.
...
For each candidate profile, the system generates a candidate profile ID. Recruiters can select the ID to open the detailed profile that AI extracted. They can then review and edit the profiles as required.
...
1. Browse to the location, and select the resumes to upload.
    ...
    The system processes the selected resumes. A progress bar shows the number of resumes that are parsed.
...
1. After the system generates a candidate profile, it also generates a **Candidate ID** value inside a batch. Select the candidate ID to open the candidate profile and view the details that AI extracted. 
1. Review the candidate profile that AI generated, and edit the information as required.

### Activate resume upload in careers 
To enable the resume parsing functionality, follow these steps:
...
1. Go to Power Page.
1. In the left-hand navigation panel, select **Setup**.
1. Under **Integrations**, select **Cloud Flows**.
1. Select **Add existing flows**.
1. From the list, select **Retrieve feature control** setting.
1. Assign the **Authenticated users** roles.
1. Select **Add**.
1. The flow appears in the list of added flows.
1. Repeat the same steps for **Resume parse** flow.
1. After adding the flows, select **Sync**.
1. When the sync finishes, the **Upload resume** button appears on the **Create profile** page. 
