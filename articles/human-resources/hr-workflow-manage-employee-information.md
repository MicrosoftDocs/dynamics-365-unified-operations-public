...
This article explains how you can use the workflow capability for Human resources to manage employee information. For example, you can associate a workflow with a position and configure an approval workflow that starts when employees change their record.
...
> [!IMPORTANT]
> When a document, transaction, or master record is submitted for workflow approval, the submitter's primary position determines who the document is routed to next.

### Hierarchy setting in Workflow parameters

1. On the **Workflow parameters** page, go to **Hierarchy routing**.
1. By default, the **Use position hierarchy** option is set to **No**. In this case, the workflow uses the worker's primary position when it navigates the hierarchy. Set the option to **Yes** to make the workflow use the position's parent when it navigates the hierarchy.
...
Grace's company creates workflow routing rules that are based on a **Configurable** hierarchy (matrix/project-based hierarchies). This hierarchy uses Grace's consultant position. If the **Use position hierarchy** option is set to **No**, when a document is routed to Grace for her approval, the workflow looks at her primary position (trainer) to determine where the document should be routed next. In this case, it routes first to Claire and then to Charlie. If the option is set to **Yes**, and the workflow uses a **Configurable** hierarchy, the workflow looks at Grace's consultant position and the reporting relationship to determine where the document should be routed next.
...
1. Create the approval process steps. For the previously described example, the approval process has two steps: one for the line manager approval and one for the HR manager approval.
1. Set the **Completion Policy** field to **All approver** or **Single approver**.
...
1. On **Human resources workflows**, select **New**.
1. In the list of available workflows, select **Identification numbers**.
1. Select **Run** to open the Workflow designer, and then enter your user name and password.
1. Move the **Approve identification number** element from the list of workflow elements to the designer canvas.
1. Connect the approval element to **Start** and **Finish**.
1. Double-tap (or double-click) **Approve element**, select and hold (or right-click), and then select **Properties**.
1. Follow these steps to add work item instructions:

    1. Select **Assignment**, and then select **Hierarchy** under the assignment type.
    1. Under the **Hierarchy** selection, select **Configurable hierarchy**.
    1. Add a stop condition, and close the page.

1. Complete any additional instructions.
1. Select **Save and close**. Activate the new workflow when the dialog box opens, and select **Make active**.
1. Go to **Human Resources** &gt; **Positions** &gt; **Position hierarchy types**.
1. Select **Matrix**.
1. Add the **Worker identification number** workflow to the list.
