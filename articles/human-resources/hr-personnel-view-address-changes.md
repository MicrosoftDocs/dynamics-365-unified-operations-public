...
1. To add an address, select **Add**. To update an existing address, select the address from the list and then select **Edit**.
1. Enter the **Name or description**.
1. Select the **Purpose** dropdown and then select the type of address.
1. Enter the **Country/region**.
1. Enter the **ZIP/postal code**.
1. Enter the **Street**.
1. Enter the **City**, **State**, and **County**. Typically, these fields are automatically set based on the **ZIP/postal code** field.
1. Optionally, select the **Primary** field to indicate a primary address. Only one address can be marked as the primary. If another address is already marked as the primary address, confirm that you want to use this address as the primary.
1. Optionally, select the **Private** field to indicate that the address is private. Only users with explicit permission to view private address information can view this address.
1. Select **OK**.
...
1. Select the worker, and then select **Addresses**.
1. To add an address, select **Add**. To update an existing address, select the address from the list and then select **Edit**.
1. Enter the **Name or description**.
1. Select the **Purpose** dropdown and then select the type of address.
1. Enter the **Country/region**.
...
In some cases, you might want to update an address to change in the future. For example, this change is useful if an employee is moving on the 15th of the following month.

1. Open the **Manage addresses** page by selecting **More options > Advanced** from any address grid.
1. Select **New** to create a new address.
1. Enter the details of the address.
1. Select the **General** FastTab.
1. In the **Effective date** field, select the date the new address is effective.
1. In the **Expiration date** field, optionally select when the address is no longer effective.
1. Close the pages.

## View and monitor address changes

HR personnel can view and monitor address changes from the **Personnel management** workspace. To view the address changes, select the **Personnel management** tile on the **Home** page. The address changes appear on a tile in the upper-right corner. The number above **Address changes** shows how many address changes occurred during the number of days that you specify on the **Human resources parameters** page. 
