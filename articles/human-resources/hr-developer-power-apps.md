...
- To import packages, you must have the **Environment Maker** permission.
- To export or import apps, you must have a Power Apps Plan 2 license or a Power Apps Plan 2 trial license.

## Integration with Microsoft 365, Power Automate

Use the **Integration with Microsoft 365** app to extract team information for signed-in users from Microsoft 365. It references workers in Human Resources to extract employee identification types. Managers can check expiration dates of employee ID types. They can also send an email reminder if the employee ID type is expiring. Power Automate integrates with Power Apps to send this reminder. Confirmation is sent back to Power Apps from Power Automate when the reminder is sent. Identification types include driver's license, passport, and other acceptable forms of ID.
...
The **Power Automate – SQL Connect and execute** template connects to Microsoft SQL Server and enables you to run SQL queries.
