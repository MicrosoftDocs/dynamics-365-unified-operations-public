...
|Point of Sale | Modernized customer create and edit workflows in Store Commerce | Customer creation and editing workflows in Store Commerce have been rebuilt on React with a streamlined single-save experience. Associates can now capture customer information and a primary address in one form and save the complete record in a single action, eliminating the previous multi-step flow where the customer record could be saved without an address. Mandatory address fields are validated inline at the point of entry before the record is committed, ensuring compliance with fiscal requirements for markets where a complete primary address is required on customer records. The edit workflow follows the same consolidated pattern, keeping associates in a single context rather than navigating between separate screens. | Admins |
