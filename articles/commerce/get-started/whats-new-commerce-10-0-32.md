...
| Extensibility  | Enable OpenAPI specification for locally deployed Commerce Scale Units (CSUs) | OpenAPI specification is a widely accepted standard used to provide API descriptions for REST APIs. Commerce Scale Units (CSUs) deployed locally for development will now have OpenAPI specification enabled by default. The API specification can be accessed via the "/swagger" endpoint. | On by default |
