...
| E-commerce  | Prevent zero-dollar authorizations | Businesses can now forcefully block the usage of the APIs that can result in zero-dollar tokenization on E-commerce as these can result in the Carding attacks. To enable this capability, two new configurations are created on the functionality profile, namely **Block zero amount authorization for guest checkouts** and **Block zero amount authorization for signed-in checkouts**. If these are enabled, then the zero-dollar tokenization will be blocked for guest and signed in users.| Admins |
