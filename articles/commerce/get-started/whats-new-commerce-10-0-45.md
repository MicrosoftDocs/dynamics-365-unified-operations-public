...
| E-commerce  | Prevent zero-dollar authorizations | To help prevent carding attacks on Commerce e-commerce, businesses can now block the usage of APIs that attempt to run zero-dollar authorizations that can lead to zero-dollar tokenization. To turn on this capability, on the functionality profile you must enable the new **Block zero amount authorization for guest checkouts** and **Block zero amount authorization for signed-in checkouts** configurations. When these configurations are enabled, zero-dollar authorizations are blocked for guest and signed-in users.| Admins |
| Unified pricing management | Enable custom price attributes for POS and e-commerce | The unified pricing feature provides a simplified approach for defining custom attributes for customers and products in Dynamics 365 Commerce headquarters. You can easily implement these custom attributes with minimal customization using the Commerce Scale Unit (CSU), enabling pricing flexibility in POS and e-commerce store operations. Learn more in [Add custom pricing attributes for Dynamics 365 Commerce Point of Sale](../../supply-chain/unified-pricing-management/upm-support-custom-attributes.md?context=/dynamics365/context/commerce) and [Example custom request handler code](../../supply-chain/unified-pricing-management/upm-custom-request-handler-example.md?context=/dynamics365/context/commerce) | Admins |
