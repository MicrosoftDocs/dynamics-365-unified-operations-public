...
|---|---|---|---|
|  Geolocation |  Changes to geolocation targeting. | Commerce now supports resolving external geolocation providers in the first request. This behavior is turned off by default for all external segmentation providers to prevent unpredictable site performance. Any external provider information isn't resolved on the initial page request, and the default page is shown. To override this default logic and allow geolocation providers to show the targeted version of a page on the first request, file a support ticket with Microsoft. | Contact Microsoft<p><p>For more information, refer to [Device, market, and geolocation targeting](../targeting-overview.md).  |
