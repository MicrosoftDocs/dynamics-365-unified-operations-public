...
description: Learn how to provision Microsoft Teams by using organizational data from Microsoft Dynamics 365 Commerce.
author: ritakimani
ms.date: 01/28/2026
ms.topic: how-to
ms.custom: 
...
Dynamics 365 Commerce offers an easy way to provision Teams if you don't have Teams set up for your retail stores. By taking advantage of well-defined information from Commerce that you want to use in Teams, you can help your store employees get started in Teams. This information includes the organizational hierarchy, store names, employee information, and Microsoft Entra accounts.

The process of provisioning Teams has two main steps:

1. In Teams, create a team for each retail store, and add store workers as members of the appropriate team. If an employee is associated with more than one retail store, team membership reflects that fact. Create a communications team that includes regional managers as members to help publish tasks from Teams.
...
- Make all regional managers communications managers.
- Associate the Azure account of every store manager and worker with that manager's or worker's worker record in Commerce headquarters.
...
1. Go to **System administration \> Inquiries \> Batch jobs**, and find the most recent job that has the description **Teams provision**. Wait until this job finishes running.

> [!TIP]
> If none of your regional managers, store managers, and store workers are associated with a Teams license, you might receive the following error message: "Failed to retrieve applicable SKU categories for the user." To correct the problem, select **Sync teams and members** on the Action Pane.
...
1. Confirm that one team exists for each Commerce retail store.
1. Select a team, and confirm that store workers are added to it as members.
1. In the left navigation pane, select **Users**, and confirm that all store employees in all stores are added as users.

The following illustration shows an example of the **Manage teams** page in the Teams admin center.

:::image type="content" source="media/Teams-FLW-Admin-Teams.png" alt-text="Screenshot of the Manage teams page in the Teams admin center.":::

## Upload a Commerce organizational hierarchy to Teams

Use the Commerce organizational hierarchy in Microsoft Teams to publish tasks to all or selected stores that use the same hierarchy structure.
...
1. When prompted in the Teams PowerShell window, sign in by using the administrator account for your Microsoft Entra tenant.
...
:::image type="content" source="media/Microsoft-teams-verify-org-hierarchy.png" alt-text="Screenshot of an organizational hierarchy in the Select who to publish to dialog box.":::
