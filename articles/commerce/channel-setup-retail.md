...
Until the 10.0.18 release, the transaction id generated for the Point of Sale (POS) was sequential and it comprised of the following: 
	- A fixed part, which is a concatenation of store ID and terminal ID 
	- A sequential part, which is a number sequence 
The final format is {store}-{terminal}-{numberseq}. Given that the transactions can be generated in the offline and online modes there was instances of duplicate transaction id generation, which required a lot of manual data fixing. Thus, with 10.0.19, we have updated the transaction ID format to remove the sequential part and use a 13 digit number generated by calculating the time in milliseconds since 1970. With this change, the new transaction ID format looks like 
{store}-{terminal}-{millisecondsSince1970}. This makes the transaction ID as non-sequential, but this ensures that the transaction IDs are always unique. 

> [!NOTE]
> The transaction IDs are meant for internal system usage so they are not required to be sequential, however, many countries require the receipt IDs to be sequential.

This feature can be enabled from the Feature management workspace. To do so, follow the below steps:
	1. Go to System administration -> Workspaces -> Feature management
	2. Filter by "retail and commerce" for Module
	3. Search for the "Enable new transaction id to avoid duplicate transaction ids" feature name
	4. Enable this. Please note:  
	5. Go to Retail and Commerce -> Retail and Commerce IT -> Distribution schedule
	6. Run 1070 and 1170 jobs to sync the setup to the stores
	7. Once the changes have been sent to the stores, the POS needs to be closed and reopened for it to use the new setup. 

> [!NOTE]
> Once enabled, you will not be able to disable this feature and if it must be disabled, then please contact support.
