...
To fix the problem, we support a new transaction ID format. The new transaction ID format uses a 13-digit number generated by calculating the time in milliseconds since 1970. The new transaction ID format is *{store}-{terminal}-{millisecondsSince1970}*. This makes the transaction ID non-sequential and ensures that transaction IDs are always unique. 
