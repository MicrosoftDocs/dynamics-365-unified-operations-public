...
Specifically, the format is `<store ID>-<terminal ID>-<numbersequence>`. 
...
To fix the problem, a new transaction ID format has been introduced. The new transaction ID format uses a 13 digit number generated by calculating the time in milliseconds since 1970. The new transaction ID format is `<store ID>-<terminal ID>-<millisecondsSince1970>`. This makes the transaction ID non-sequential and ensures that transaction IDs are always unique. 
...
The new transaction ID format feature can be enabled from the **Feature management** workspace in Commerce headquarters. 
...
> [!NOTE]
> After the new transaction ID format feature is enabled, you won't be able to disable this feature. If it must be disabled, contact Microsoft support.
