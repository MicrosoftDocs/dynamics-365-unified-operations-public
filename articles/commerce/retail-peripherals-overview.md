---
title: Connect peripherals to POS in Commerce
...
Each hardware profile supports two cash drawers. This capability enables two active shifts per register to be available at the same time. In the case of a shared shift, or a cash drawer that multiple mobile POS devices use at the same time, each hardware profile allows only one cash drawer. You can connect cash drawers directly to a computer via USB, connect to a network, or connect to a receipt printer via an RJ12 interface. In some cases, you can also connect cash drawers via Bluetooth. Epson and Star are the preferred cash drawer OEMs. For other OEMs, you must create extensions to integrate with them.
...
Receipt printing at the POS is optimized for OPOS. OPOS tends to be faster than printing via Windows. Therefore, use OPOS, especially in environments where 40-column receipts are printed and transaction times must be fast. For most devices, you use OPOS controls. However, some OPOS receipt printers also support Windows drivers. By using a Windows driver, you can access the latest fonts and network one printer for multiple registers. However, there are drawbacks to using Windows drivers. Here are some examples of these drawbacks:

- When you use Windows drivers, the printer renders images before printing occurs. Therefore, printing tends to be slower than it is on printers that use OPOS controls.
- Devices that are connected through the printer ("daisy-chained") might not work correctly when you use Windows drivers. For example, the cash drawer might not open, or the receipt printer might not work as you expect.
...
#### Print receipts on Adyen payment terminals

Starting with Commerce version 10.0.49, if you use the Dynamics 365 Payment Connector for Adyen, you can print receipts from an Adyen payment terminal that has an integrated printer. This capability lets you use receipt formats that you design in Commerce headquarters without requiring a separate receipt printer at the POS register.

To use an Adyen payment terminal as a receipt printer, complete the following setup steps:

1. Enable the feature **Enable Adyen payment terminal printer** from the feature management workspace.
...
- **32-bit system**: `HKEY_LOCAL_MACHINE\SOFTWARE\OLEforRetail\ServiceOPOS`
- **64-bit system**: `HKEY_LOCAL_MACHINE\SOFTWARE\WOW6432Node\OLEforRetail\ServiceOPOS`
...
| Supported device class | Supported interfaces |
| ---------------------- | -------------------- |
| Printer | <ul><li>OPOS</br>**Note**: Enable **Keep Connection Open** to minimize unnecessary OPEN and CLOSE OPOS operations and reduce the likelihood of connection errors.</li><li>Windows driver</li><li>Device</li><li>Network</li></ul> |
| Printer 2 | <ul><li>OPOS</br>**Note**: Enable **Keep Connection Open** to minimize unnecessary OPEN and CLOSE OPOS operations and reduce the likelihood of connection errors.</li><li>Windows driver</li><li>Device</li><li>Network</li></ul> |
| Line display | OPOS</br>**Note**: Enable **Keep Connection Open** to minimize unnecessary OPEN and CLOSE OPOS operations and reduce the likelihood of connection errors. |
| Dual display | Windows driver |
| MSR | <ul><li>OPOS</li><li>UWP (No setup required)</li><li>Keyboard wedge (No setup required)</li></ul> |
| Drawer | <ul><li>OPOS</br>**Note**: Enable **Keep Connection Open** to minimize unnecessary OPEN and CLOSE OPOS operations and reduce the likelihood of connection errors.</li><li>Network <br>**Note**: You can only set up one drawer if **Use shared shift** is configured on the drawer.</li></ul> |
| Drawer 2 | <ul><li>OPOS</li><li>Network </br>**Note**: You can only set up one drawer if **Use shared shift** is configured on the drawer.</li></ul> |
| Scanner | <ul><li>OPOS</li><li>UWP (No setup required)</li><li>Keyboard wedge (No setup required)</li></ul> |
| Scanner 2 | <ul><li>OPOS</li><li>UWP (No setup required)</li><li>Keyboard wedge (No setup required)</li></ul> |
| Scale | OPOS |
| PIN pad | OPOS (Support is provided through customization of the payment connector). |
| Signature capture | OPOS |
| Payment terminal | <ul><li>Custom device support</li><li>Network (For more information, see the payment connector documentation).</li></ul> |
...
| Supported device class | Supported interfaces |
| ---------------------- | -------------------- |
| Printer | <ul><li>OPOS</li><li>Network</li></ul> |
| Printer 2 | <ul><li>OPOS</li><li>Network</li></ul> |
| Line display | OPOS |
| MSR | OPOS |
| Drawer | <ul><li>OPOS</li><li>Network</br>**Note**: You can only set up one drawer per hardware profile if **Use shared shift** is configured on the drawer.</li></ul> |
| Drawer 2 | <ul><li>OPOS</li><li>Network</li></ul> |
| Scanner | OPOS |
| Scanner 2 | OPOS |
| Scale | OPOS |
| PIN pad | OPOS (Support is provided through customization of the payment connector). |
...
> [!NOTE]
> When you share the IIS hardware station, multiple devices can use the hardware station at the same time. For this scenario, use only the devices that are listed in the following table. If you try to share devices that aren't listed (such as barcode scanners and MSRs), errors occur when multiple devices try to claim the same peripheral. A future release will explicitly prevent this configuration.
...
1. Create a hardware station of the **Dedicated** type for the store where the POS register is used. A description is optional.
    ...
    > [!NOTE]
    > You don't have to set any other properties on the hardware station. The register itself provides all other required information, such as the hardware profile.
...
**New for 10.0.8**: The Modern POS for Android app supports Epson network printers and cash drawers connected to those printers through the drawer kick (DK) port. For more information, see [Set up POS hybrid app on Android and iOS](./dev-itpro/hybridapp.md).
...
1. Set the following properties on the shared hardware station:
    - **Host name**: The name of the host computer where the hardware station runs.
...
1. In the **Registry Editor** window, go to `HKEY_LOCAL_MACHINE\System\CurrentControlSet\SecurityProviders\SCHANNEL\Protocols`.
...
Most barcode scanners that function as a keyboard wedge device or have OPOS drivers work with the Store Commerce app. Ensure to test extensively to confirm that all scenarios work as expected.

### MSRs

Most magnetic stripe readers that function as a keyboard wedge device or have OPOS drivers work with the Store Commerce app. Ensure to test extensively to confirm that all scenarios work as expected.
...
- Ensure you close other device driver configuration utilities. If these utilities are open, they might prevent Modern POS or the hardware station from claiming the device.

- If you share the peripheral with multiple POS devices, ensure it belongs to one of the following categories:
