...
- The standard SCM pricing engine supports the pricing calculation based on the "Requested ship date" and "Requested receipt date" along with the current date. However, Retail pricing currently does not support these values. The reason is that for B2C scenarios the customers don't expect the requested delivery date to affect the item price. However, we know that some retailers have both B2B and B2C operations and for B2B operations, it is fairly common to change prices based on the delivery dates. So such retailers can use SCM pricing for their B2B business and Retail pricing for their B2C business. Retail pricing kicks in only if the application user is added as a call center user, so the retailers can assign certain users who will work with the SCM pricing and assign a few that will work with the Retail pricing i.e. these users should be added as a call center users. Additionally, they must turn ON the property "Use today's date for calculating prices" in the Commerce parameters -> pricing and discounts -> Miscellaneous section. This way they can keep the accounts receivable parameter value to Requested ship date or Requested receipt date for SCM pricing, but the Retail pricing will keep using the today's date for pricing calculation. 
