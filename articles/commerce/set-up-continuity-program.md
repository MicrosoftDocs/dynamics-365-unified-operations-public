...
description: Learn how to set up a continuity program for a call center in Microsoft Dynamics 365 Commerce.
author: josaw1
ms.date: 01/29/2026
ms.topic: how-to
...
This article describes how to set up a continuity program for a call center in Microsoft Dynamics 365 Commerce.
...
1. Create a continuity program that specifies details such as the payment schedule, the timing of the shipments, and whether billing is up front. Add a list of products that are included in the continuity program. Each product receives an event ID number that is assigned sequentially, beginning with 1. The event IDs determine the order that products are sent in.
...
After you set up a continuity program as described earlier, you can create a continuity order for a customer. You might also have to perform the following additional maintenance tasks.
...
- **Extend continuity lines** (if necessary) – Extend the number of times that a continuity event can be repeated. The repetition of shipments can then extend beyond the limit that was set in the **Continuity repeat threshold** field in the call center parameters.
- **Perform a continuity update** (if necessary) – Synchronize changes between the continuity program and the continuity parent sales orders.
