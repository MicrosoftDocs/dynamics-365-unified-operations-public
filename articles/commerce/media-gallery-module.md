...
| Zoomed image quality | Numbers from 1-100 representing percentages, selected using a trackbar control. | This property defines the image quality for the zoomed-in images. It can be set to 100% to ensure the zoomed image always uses the highest resolution possible. This is not applicable to PNG files since they use a lossless format. This capability is available as of the Commerce version 10.0.19 release |
...
|Hide master product images for variant| **True** or **False**| If set to **True**, when a variant is selected images of the master product are hidden unless the variant has no images. Products with no variants are not affected by this setting. |
