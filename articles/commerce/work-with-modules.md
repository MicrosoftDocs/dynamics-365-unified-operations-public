...
description: Learn how and when to use modules in Microsoft Dynamics 365 Commerce.
author: phinneyridge
ms.date: 01/30/2026
ms.topic: how-to
...
Modules are logical building blocks that make up your page structure, and they have various purposes and scopes. Some modules are high-level containers, and their only purpose is to hold and organize other modules (child modules). Other modules, such as a simple image placement module, have a specific purpose. Other modules, such as a carousel module, fall somewhere between those two categories.
...
The module developer defines these slots and determines which child modules and how many child modules can go directly inside it. For example, the header slot might support only one module of the **Header Module** type, whereas the body slot might support an unlimited number of modules of any type (except other page container modules).

In the authoring tools, page authors don't need to know in advance which modules can and can't go in each slot. When page authors select a slot and then try to select a module to add to it, they see a filtered view of module types that are supported for that slot.
...
Most day-to-day page and content authoring activities involve content modules, primarily because these modules define the actual content that appears in their parent container modules. Many content modules are available, and you typically add these modules last to a page's hierarchy of nested modules.

:::image type="content" source="../commerce/media/basic-module-nesting.png" alt-text="Screenshot of nesting modules.":::
...
1. In the outline pane on the left or directly in the main canvas, select a container or slot to which you can add a child module.

    > [!NOTE]
    > The module designer defines the list of module types that you can add to a specific module slot. Template authors can refine the allowed module options to help guarantee consistent search engine optimization (SEO) and authoring efficiency for all the pages that are built from a specific template. When you add a module to a slot, the **Add Module** dialog box automatically filters the list so that it shows only modules that are supported in the selected container or slot. The page's template or the container's module definition determines this list of allowed modules.

1. If you're using the outline pane, select the ellipsis (**...**) next to the module name, and then select **Add Module**. If you're using the controls directly within the canvas, select the plus symbol (**+**) in an empty slot or next to the currently selected module, and then select **Add Module**.
...
1. In the outline pane on the left, select the ellipsis (**...**) next to the name of the module to remove, and then select the trash can symbol. Alternately, in the main canvas, you can select the trash can symbol on a selected module's toolbar.
...
1. Select either an upward or downward pointing arrow symbol in the module's toolbar, and then drag the arrow to a new position on the page. The blue line in the canvas and outline indicates where the module can be placed. If a module can't be moved up or down, that arrow symbol is grayed out.
1. Release the module to drop it into the new position.
...
1. If the module can be moved up or down within the container or slot, you'll see options for **Move up** or **Move down**. Select the desired move option to move the module up or down relative to its siblings.
...
1. In the outline pane on the left, expand the tree and select any content module (for example, **Content block**). Alternately, select the module in the main canvas.
1. In the module properties pane on the right, enter properties for any desired module controls.
1. On the command bar, select **Save**. This action also refreshes the preview canvas.

### Edit module text properties

Edit module text properties that aren't read-only directly in the canvas.

To edit module text properties, follow these steps:

1. Select the text control in the canvas, and place your cursor where you want to edit text.
...
You can change module images that aren't read-only directly from the canvas.

To choose a new image for a content module, follow these steps:

1. In the canvas, double-click the image. This action opens the media picker window.
1. Find and select the new image you want to use, and then select **OK**. The new image appears in the canvas.
