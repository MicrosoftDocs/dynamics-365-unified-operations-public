...
**Translations** for shortcuts are key to get them rendered on the POS, without which these shortcuts would not get rendered in POS. If your channel language is different than the system default language, it is critical to define the translation for each shortcut in the expected language. You can define translations by clicking on the 'Translate' for each shortcut. 
