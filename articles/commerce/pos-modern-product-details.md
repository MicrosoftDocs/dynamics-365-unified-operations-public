...
description: Learn about the modernized product details page in Store Commerce POS, including the mobile-first layout, inventory preview, related products, and how to enable these capabilities.
author: anush6121
ms.author: anvenkat
ms.topic: article
ms.date: 05/13/2026
...
- Enable the following feature flag in Commerce headquarters: **Enable modernized product details page**
...
1. Search for **Enable modernized product details page** and select **Enable now**.
...
The product details page uses a sticky vertical layout that keeps primary product information and key actions always visible while associates scroll through details.

- **Sticky layout**: Keeps primary info and actions visible while scrolling.
- **Always-visible key actions**: Keeps **Add to sale** and other primary actions always within reach.
...
Dimension controls (size, color, style) and quantity entry present inline on the page, streamlining variant and quantity selection for quicker, error-free entry.

## Related products carousel

A related products carousel displays on the product details page, enabling quick cross-sell opportunities. Each card in the carousel includes an **Add to sale** button that lets associates add related items to the cart without navigating away.
...
1. In **POS visual profiles**, set **Show nearby store inventory** to **Yes**.
2. Run the **Registers (1090)** job.

## Contextual discount visibility

A quantity discount indicator displays on the product details page when quantity-based discounts are available for the product, helping associates explain pricing benefits to customers.

To enable this feature:

1. In **POS visual profiles**, set **Show quantity discount on modern PDP** to **Yes**.
2. Run the **Registers (1090)** job.
