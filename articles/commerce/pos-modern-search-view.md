...
description: Learn about the modernized search results view in Store Commerce POS, including refreshed product tiles, inventory visibility, discount cues, and how to enable these capabilities.
author: anush6121
ms.author: anvenkat
ms.topic: article
ms.date: 05/13/2026
...
- Enable the following feature flag in Commerce headquarters: **Enable modernized search view results**
...
1. Search for **Enable modernized search view results** and select **Enable now**.
1. Enable any optional per-capability toggles described in the following sections.
1. Run the **Registers (1090)** distribution schedule job to push changes to channels.
1. Restart the Store Commerce app.

## Search results layout

The search results page is rebuilt on React with a refreshed card-based layout that reduces visual clutter and improves scan speed.
...
1. In **POS visual profiles**, set **Show quantity discount captions** to **Yes**.
2. Run the **Registers (1090)** job.

## Inventory visibility

Store and nearby store availability display directly on each product card, allowing associates to confirm stock without navigating to the product details page.

To enable this feature:

1. In **POS visual profiles**, set **Show stock count and availability across stores** to **Yes**.
2. Run the **Registers (1090)** job.
