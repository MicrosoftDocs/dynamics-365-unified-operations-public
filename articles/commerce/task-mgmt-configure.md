...
description: Learn how to configure task management features in Microsoft Dynamics 365 Commerce.
...
> [!NOTE]
> If a nonmanager persona must have task management permissions in the POS, grant permission to the individual. Alternatively, create a new permission group for non-managers and set the **Allow task management** option to **Yes**.

The following illustration shows how to configure task management permissions for store managers.

:::image type="content" source="media/HQ-POS-Tasks-Notifications-User-Permission.png" alt-text="Screenshot of configuring task management permissions for store managers.":::
...
:::image type="content" source="media/HQ-POS-Tasks-Notifications.png" alt-text="Screenshot of the Task management operation on the POS operations page.":::

For more information about how to configure POS notifications, see [Show order notifications in the point of sale (POS)](notifications-pos.md) article.

> [!NOTE]
> When you save your changes, the following warning message is displayed: **Operation parameter will not be enabled in button grid designer for operation ID equal or less than 4000. If you are creating custom operation and want to pass parameter from the button grid designer, then use operation ID greater than 4000.** Select **Close** to close the dialog.
...
:::image type="content" source="media/POS-home-screen-tasks-button-image.png" alt-text="Screenshot of a Tasks tile on a POS home page.":::
