...
- **Catalog type** - Configure this value as **B2B**. You can define B2B catalog-specific properties such as navigation hierarchy, customer hierarchy, and attribute metadata for the catalog. 
...
> [!NOTE]
> This feature is available starting with the Dynamics 365 Commerce version 10.0.27 release. To configure catalog-specific configurations like navigation hierarchy and customer hierarchy in Commerce headquarters, in Commerce headquarters go to the Feature management workspace (**System administration \> Workspaces \> Feature management**), enable the **Enable use of multiple catalogs on retails channels.** feature, and then run the **1110 CDX** job. 
> When you enable this feature, all of the existing catalogs that are used for POS or call center will be marked as **Catalog type = B2C** on the **Catalogs** page. Only existing and new catalogs with **Catalog type = B2C** are applicable for POS stores and call center. 
...
To configure products to add to the catalog, in headquarters go to **Retail and Commerce \> Catalogs and assortments \> All catalogs**, and then on the **Catalogs** tab, select **Add products**. 

Products that were added to a catalog from the root node of the navigation hierarchy by selecting **Add products** on the top ribbon, will inherit their categories if the source navigation hierarchy is also associated to the catalog. Changes to categories that are made on the source navigation hierarchy will be immediately reflected in the catalogs. You must republish the catalogs to update the channels.

Alternatively, you can select a node in the navigation hierarchy and add products directly to a selected category within the catalog. 

When you add products, you will have the option to **Automatically include all variants when only product master is selected.** To prevent the inclusion of all variants, select at least one variant for the product master. 

> [!NOTE]
> If you choose to automatically include all variants in a large product masters selection, you may experience longer processing times. For large selections, we recommend that you select **Include all variants** on the top ribbon on the catalogs form to execute the operation in batch mode. If you only included product master to the catalog and did not include any variants, the variant picker may not be available when you navigate to a product details page. 
...
> [!NOTE]
> If **Catalog type = B2B**, validation will fail if you added POS stores or call center to the catalog. B2B catalogs must only have B2B online channels associated to them. Validation will also fail if no customer hierarchy is associated to a B2B catalog. 
