...
description: Learn how to use a sample CRT extension to enable camera-based product identification in POS, using Azure Custom Vision or any preferred vision model
author: anush6121
ms.author: anvenkat 
ms.topic: how-to 
ms.date: 12/2/2025
...
This sample shows how to enable **camera-based product identification** in the Dynamics 365 Commerce Store Commerce app. It introduces a **Commerce Runtime (CRT) extension** that lets you trigger image-based recognition from **any POS surface area**—including transaction pages, search screens, and inventory workflows.
...
**While the sample integrates with Azure Custom Vision to help you get started quickly, it's fully model-agnostic—train your catalog images with any preferred vision model and configure the API endpoint in the extension to call into it for product recognition.**
...
1. The extension sends the image to your configured model endpoint.
1. The model returns one or more product IDs.
1. If multiple matches are found, the sample displays a list for user selection.
1. The selected item is inserted into the transaction or search flow.
