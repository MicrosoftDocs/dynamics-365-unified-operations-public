...
description: Learn how to copy existing content to another locale within a site in Microsoft Dynamics 365 Commerce site builder.
author: josaw1
ms.date: 01/20/2026
ms.topic: how-to
...
1. In the channel dialog box that appears, select **Add a locale**.
...
> [!NOTE]
> Before you can configure the new locale in site builder, you must add it to the associated online store channel in Commerce headquarters.
...
1. Select **Copy locale**. A notification confirms that the locale copy operation is running.
...
1. Under **Current jobs**, select the job to monitor. The dialog box that appears on the right shows the job details.
...
1. On the channel properties pane, under **Locale-domain mappings**, select the vertical ellipsis next to the locale you want to delete, and then select **Delete**.
