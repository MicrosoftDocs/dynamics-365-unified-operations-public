---
title: Update Commerce headquarters with Microsoft Entra B2C information
description: Learn how to update Microsoft Dynamics 365 Commerce headquarters with Microsoft Entra business-to-consumer (B2C) information.
author: BrianShook
ms.date: 02/20/2026
ms.topic: how-to
...
After you complete the Microsoft Entra B2C provisioning steps, register the Microsoft Entra B2C application in your Dynamics 365 Commerce environment.

To update headquarters with the new Microsoft Entra B2C information, follow these steps:

1. In Commerce headquarters, go to **Commerce Shared Parameters** and select **Identity Providers** in the left menu.
1. Under **Identity Providers**, complete the following steps:
    1. In the **Issuer** box, enter the identity provider issuer string. To find your issuer string, see [Obtain issuer string for headquarters setup](#obtain-issuer-string-for-headquarters-setup).
    1. In the **Name** box, enter a name for your issuer record.
    1. In the **Type** box, enter **Microsoft Entra ID B2C (id_token)**.
1. Under **Relying Parties**, with the preceding B2C identity provider item selected, complete the following steps:
    1. In the **ClientID** box, enter your B2C application ID. You can find this ID in the **Application ID** box of your B2C application's properties page.
...
To get your identity provider issuer string, follow these steps:

1. On the Microsoft Entra B2C page of the Azure portal, go to your **Sign up and sign in** user flow.
...
1. Make sure your application is set to your intended Microsoft Entra B2C application created earlier. Then, select the user flow link that appears under the **Run user flow** header. The link includes ``.../.well-known/openid-configuration?p=<B2CSIGN-INPOLICY>``. (Don't select **Run user flow**.) A new tab opens displaying metadata for the policy to collect the issuer string.
...
To continue the process of setting up a B2C tenant in Commerce, go to [Configure your B2C tenant in Commerce site builder](config-b2c-tenant-site-builder.md).
