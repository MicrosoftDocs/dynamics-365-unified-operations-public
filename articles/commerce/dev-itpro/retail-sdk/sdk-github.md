...
description: Learn how to download Retail software development kit (SDK) samples from GitHub and reference packages from a public feed.
author: josaw1
ms.date: 02/20/2026
ms.topic: how-to
ms.author: josaw
...
| Scenario | Commerce SDK reference | Technology | Documentation |
|---|---|---|---|
| Extend the Store Commerce app for user experience (UX) changes, client logic, workflows, and simple validations. | [Store Commerce app samples](https://github.com/microsoft/Dynamics365Commerce.InStore) | TypeScript, HTML, and CSS | [Run the point of sale (POS) samples](../pos-run-samples.md) |

### Commerce Runtime (CRT)

| Scenario | Commerce SDK reference | Technology | Documentation |
|---|---|---|---|
| Extend CRT to add or change business logic, such as logic for calculating tax, prices, or discounts. | [CRT samples](https://github.com/microsoft/Dynamics365Commerce.ScaleUnit) | C# | [Commerce runtime (CRT) and Retail Server extensibility](../commerce-runtime-extensibility.md) |

### Headless Commerce APIs

| Scenario | Commerce SDK reference | Technology | Documentation |
|---|---|---|---|
| Create a Headless Commerce API extension to expose new Commerce APIs to the client. | [Retail Server samples](https://github.com/microsoft/Dynamics365Commerce.ScaleUnit) | Open Data Protocol (OData) and C# | [Create a new Retail Server extension API (Retail SDK version 10.0.11 and later)](../retail-server-icontroller-extension.md) |

### TypeScript proxy

| Scenario | Commerce SDK reference | Technology | Documentation |
|---|---|---|---|
| Use a TypeScript proxy when you need to consume new Headless Commerce API extensions in the POS or E-Commerce clients. | [CommerceProxyGenerator](https://github.com/microsoft/Dynamics365Commerce.ScaleUnit) | OData and C# | [Create a new Retail Server extension API (Retail SDK version 10.0.11 and later)](../retail-server-icontroller-extension.md) |

### Hardware station

| Scenario | Commerce SDK reference | Technology | Documentation |
|---|---|---|---|
| Add or change logic related to peripherals. | [src\HardwareStationSample samples](https://github.com/microsoft/Dynamics365Commerce.InStore) | C# | [Integrate POS with a new hardware device](../hardware-device-extension.md) |

### Payment connector

| Scenario | Commerce SDK reference | Technology | Documentation |
|---|---|---|---|
| Integrate the POS with a new payment connector. | [src\HardwareStationSample\PaymentDevices](https://github.com/microsoft/Dynamics365Commerce.InStore) | C# | [Create an end-to-end payment integration for a payment terminal](../end-to-end-payment-extension.md) |
...
After you develop extensions (CRT, Retail Server, database scripts, POS, and Hardware station), generate deployment packages to deploy into test, sandbox, and production environments. For more information, see [Create deployable packages](../CSU-packaging.md).
...
Use the packages from [index.json](https://pkgs.dev.azure.com/commerce-partner/Registry/_packaging/dynamics365-commerce/nuget/v3/index.json). Add the package source location in the **nuget.config** file of your extension project file, as shown in the following code example:
...
|---------------------------------------|-----------|
| Microsoft.Dynamics.Commerce.Sdk.ChannelDatabase | You need this package to generate the DB packages with CSU.  |
| Microsoft.Dynamics.Commerce.Sdk.Runtime | This meta package contains all the required packages for implementing the CRT and Retail Server extensions. It includes all the required contracts, messages, requests/responses, and entities. |
| Microsoft.Dynamics.Commerce.Sdk.ScaleUnit | You need this package to generate the CSU package for deployment. |
| Microsoft.Dynamics.Commerce.Sdk.Installers.ScaleUnit|  You need this package to generate the ScaleUnit package for deployment. |
...
| Microsoft.Dynamics.Commerce.Sdk.Installers.HardwareStation| You need this package to generate the Hardware station package for deployment. |
| Microsoft.Dynamics.Commerce.Sdk.Pos| This package contains all POS libraries. |
| Microsoft.Dynamics.Commerce.Sdk.Installers.ModernPos| You need this package to generate the POS extension installer for deployment. |
...
For information about setting up an Azure pipeline, see [Set up a build pipeline for the independent-packaging SDK](../build-pipeline.md).
...
1. Add and commit the changes to the development branch. For example:
...
1. After you complete, test, and validate the development, push the changes to the main branch. For example:
...
1. Merge the changes from the release branch back to main branch if you made any changes in the release branch. For example:
...
1. Create a hotfix branch for the extension from the main branch. For more information, see [Create a release branch after development](#create-a-release-branch-after-development).
1. Release the fix.
1. Merge the changes back to the main branch.
