...
Commerce runtime extensions use the packages or reference libraries from the Retail SDK `\Pkgs` folder but in the Commerce SDK all the required packages are published to this [public feed](https://pkgs.dev.azure.com/commerce-partner/Registry/_packaging/dynamics365-commerce/nuget/v3/index.json) and CRT samples are available for reference in the [GitHub](https://github.com/microsoft/Dynamics365Commerce.ScaleUnit/tree/release/9.33/src/ScaleUnitSample).
...
Previously you had to create a separate Modern POS C# proxy library to consume the API and Commerce Runtime extensions but with the new API extension model, you can directly use the Headless Commerce API.
