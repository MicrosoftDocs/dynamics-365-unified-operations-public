...
> [!NOTE]
> In release 10.0.21, the development VM in LCS will not include Visual Studio 2017 and other dependencies that are required for Commerce development. Install the prerequisites manually or by running MSBuild from the Retail SDK (use Developer command prompt for VS 2019 to run the msbuild command if VS 2017 tools are not installed in the VM,  this will install all the prerequisites but POS build will not work with VS 2019 developer command prompt, it will fail with error saying POS doesn’t support VS 2019 , to fix this issue after the prerequisites installation is completed, open developer command prompt for VS 2017 and run msbuild or  using PowerShell run the checkVS2017Installed.ps1 script RetailSDK\BuildTools to install the Visual studio tools for 2017 and then use the developer command prompt for VS 2017 to install the prerequisites and build the SDK).
