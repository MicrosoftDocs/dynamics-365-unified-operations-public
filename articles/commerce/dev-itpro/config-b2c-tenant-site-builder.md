...
description: Learn how to configure your business-to-consumer (B2C) tenant in Microsoft Dynamics 365 Commerce site builder.
author: BrianShook
ms.date: 02/13/2026
ms.topic: how-to 
...
After you set up your Microsoft Entra B2C tenant, configure the B2C tenant in Commerce site builder. The configuration steps include collecting B2C application information from the Azure portal, entering that B2C application information into site builder, and then associating the B2C application with your site and channel.
...
1. From the **Application (client) ID** reference, collect the application ID of the B2C application created in your B2C tenant. Enter this ID later as the **Client GUID** in site builder.
...
:::image type="content" source="../media/B2CImage_22.png" alt-text="Screenshot of user flow policies on the Microsoft Entra B2C User flows policies page.":::

### Enter your Microsoft Entra B2C tenant application information into Commerce

Enter the details of your Microsoft Entra B2C tenant into Commerce site builder before associating the B2C tenant with your sites.
...
1. In the main window next to **Site authentication profiles**, select **Manage**. (If your tenant appears in the site authentication profiles list, an administrator already added it. Verify that the items in step 6 match the items for your intended B2C setup. You can also create a new profile that uses similar Microsoft Entra B2C tenants or applications to account for minor differences, such as differing user policy IDs).
1. Select **Add site authentication profile**.
1. Enter the following required items in the form, using values from your B2C tenant and application. Fields that aren't required (without an asterisk) can be left blank.

    - **Application Name**: The name for your B2C Application, such as "Fabrikam B2C."
    - **Tenant Name**: The name of your B2C tenant. Use "fabrikam" if the domain appears as "fabrikam.onmicrosoft.com" for the B2C tenant.
    - **Forget Password Policy ID**: The forget password user flow policy ID, such as "B2C_1_PasswordReset."
    - **Signup Signin Policy ID**: The sign-up and sign-in user flow policy ID, such as "B2C_1_signup_signin."
    - **Client GUID**: The B2C application ID, such as "00001111-aaaa-2222-bbbb-3333cccc4444."
    - **Edit Profile Policy ID**: The profile editing user flow policy ID, such as "B2C_1A_ProfileEdit."

1. Select **OK**. You should now see the name of your B2C application appear in the list.
1. Select **Save** to save your changes.

Use the optional **Login custom domain** field only if you're setting up a custom domain for the Microsoft Entra B2C tenant. For more information and considerations regarding the usage of the **Login custom domain** field, see [Additional B2C information](additional-b2c-info.md).

### Associate the B2C application to your site and channel

> [!WARNING]
> - If your site is already associated with a B2C application, changing to a different B2C application removes the current references for users who already signed up in this environment. If you change the B2C application, users can't access any credentials associated with the currently assigned B2C application.
> - Only update the B2C application if you're setting up the channel's B2C application for the first time or if you want users to sign up again by using new credentials for this channel by using the new B2C application. Take caution when associating channels to B2C applications, and name applications clearly. If you don't associate a channel to a B2C application in the following steps, users signing into that channel for your site are entered into the B2C application that shows as **default** in the **Tenant Settings \> B2C Settings** list of B2C applications.
