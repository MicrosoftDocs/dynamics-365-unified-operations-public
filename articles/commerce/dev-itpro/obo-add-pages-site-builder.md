...
description: Learn how to create pages for on behalf of (OBO) functionality in Microsoft Dynamics 365 Commerce site builder.
author: mariash529
ms.date: 08/05/2026
ms.topic: how-to
ms.reviewer: mirao
...
> [!NOTE]
> The steps in this article apply to **Azure AD B2C** only. For **Microsoft Entra External ID (EEID)**, custom sign-in or sign-up pages aren't supported yet. After you configure the OIDC identity provider and add it to the user flow, the B2B account manager sign-in option (for example, "Sign in with StoreManagerB2BSignin") automatically appears in the standard sign-in flow. No extra page customization is required in site builder.

To enable OBO functionality in site builder, complete the following procedures:
...
- [Modify a B2B sign-in page so that it includes a button that the account manager can use to sign-in](#modify-a-b2b-sign-in-page-to-include-an-employee-sign-in-button).

You might also want to complete the following optional procedure:

- [Modify the word "For" in the header](#modify-the-word-for-in-the-header).
...
1. In the **Add a link** dialog box, select the business partner organization that you created, and select **Apply**.
1. Select **Save and publish**.

## Modify a B2B sign-in page to include an employee sign-in button

> [!NOTE]
> This section only applies to built-in policies with an injected contents-based custom user interface (UI). If you're using built-in policies with a generic module custom UI, or with the Azure business-to-consumer (B2C) UI, you can skip this section.
...
1. In the **Main** slot of the new page, select the ellipsis (**...**), and then select **Add module**.
1. In the **Select modules** dialog box, select the **Sign in** module, and then select **OK**. The system names this second sign-in module **Sign in 1**.
...
## Modify the word "For" in the header
