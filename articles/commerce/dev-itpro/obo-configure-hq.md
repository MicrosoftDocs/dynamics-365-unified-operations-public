...
description: Learn how to set up and configure on behalf of (OBO) functionality in Microsoft Dynamics 365 Commerce headquarters.
author: mariash529
ms.date: 08/05/2026
ms.topic: how-to
ms.reviewer: mirao
...
First, add the identity provider that you created in [Create and configure a Microsoft Entra application for account manager sign-in](obo-create-aad-application.md) to **Commerce shared parameters** in Commerce headquarters.
...
1. Under **Identity providers**, select **Add**, and then configure the following fields:
...
1. Under **Relying parties**, select **Add**, and then set the following fields:
    ...
    > [!NOTE]
    > If you're using a Microsoft Entra External ID (EEID) tenant for B2B authentication, the user impersonation scope is created as part of the EEID app registration instead of the B2B app registration. So, also add the **Application (Client) ID** of the app registration that you created in [Create an app registration that exposes the user impersonation scope](obo-create-aad-application.md#create-an-app-registration-that-exposes-the-user-impersonation-scope) as a relying party under the **EEID tenant issuer URL**. Set **Type** to **Confidential** and **User Type** to **Worker** for the relying-party entry.

1. Under **Server resource IDs**, select **Add**, and then configure the following fields:

    1. **Server Resource Id:** Enter `https://<APPLICATIONIDURI>`, where `<APPLICATIONIDURI>` is the ID of the Microsoft Entra B2B application (for example, "api://aaaabbbb-0000-cccc-1111-dddd2222eeee.")
...
> [!NOTE]
> You can skip the following procedure for account managers who already use Microsoft Entra sign-in on POS devices.

To configure account managers in headquarters, follow these steps:

1. Go to **Human Resources** > **Workers** > **Employees**.
1. Select an account to configure (for example, `user@adventure-works.com`).
...
Create a sales group of one or more account managers. For OBO functionality to work, the sales group must include at least one sales representative. Assign commission percentages, if your organization assigns commissions. A commission percentage can equal 0 (zero).
...
1. Find the customer of type **Organization** that the sales group manages (for example, "Contoso B2B"), and then select the customer's name or account number.

    > [!NOTE]
    > You must specify an address book in the **Address book** field for the customer of type **Organization**.

1. On the **Sales order defaults** FastTab, under **Sales group**, enter the sales group ID of the sales group that you created.
1. On the action pane, select **Save**.

Any member of the specified sales group can now work on behalf of any user in the selected customer B2B buyer organization.

In the customer hierarchy that corresponds to this customer organization (**Retail and Commerce** > **Customers** > **Customer hierarchies**), the sales group appears as a read-only value in the **Sales Groups** section.
