...
description: Learn how to merge Microsoft Dynamics 365 Commerce Cloud Scale Unit (CSU) extension packages for deployment.
author: josaw1
ms.date: 02/18/2026
ms.topic: how-to
ms.author: josaw
...
Extensions packages that independent software vendors (ISVs), partners, and customers generate for CSU must be merged into a single package before they're deployed to [Microsoft Dynamics Lifecycle Services (LCS)](https://lcs.dynamics.com/Logon/Index). Currently, LCS doesn't support the deployment of multiple extension packages. The last extension package that's deployed overrides the previous extension deployment.

You can use multiple options to merge CSU extension packages. This article guides you through the process of merging them by generating a NuGet package for ISV CSU extensions.

> [!NOTE]
> You don't need to merge extension packages for on-premises CSU deployments, Store Commerce, or shared Hardware Station. On-premises components support multiple extension deployments.

## Scenario

Both your tax ISV and your payment ISV give you a CSU extension package, and you have your own extension package. You want to deploy all the packages. Because LCS doesn't support the deployment of multiple packages, you must merge all the packages into one package.
...
1. Build your CSU packaging project. The output CSU extension package has the extensions from the ISVs and your extensions.

### Generate a NuGet package for the ISV extensions
...
1. Edit the ISV CSU packaging project file, and add the following build property to [generate the NuGet package on build](/nuget/create-packages/creating-a-package-msbuild#automatically-generate-package-on-build) in addition to generating the CSU extension package.
...
1. Build the CSU packaging project. The output CSU extension package has the extensions from the ISVs and your extensions.
