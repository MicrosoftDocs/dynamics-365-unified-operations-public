...
description: Learn how to configure and use payment processing with nonrecurring payment tokens in Microsoft Dynamics 365 Commerce.
author: shajain
ms.date: 02/18/2026
ms.topic: how-to
...
Payment processing actions in Dynamics 365 Commerce save payment card tokens. Use these tokens for downstream sales order changes, resubmission of authorizations that are close to expiration, and "card on file" caching for future customer orders. In direct configurations, card payment tokens have a "recurring detail" reference that you can use for unscheduled, merchant-initiated transactions. In Commerce version 10.0.40, payment processing can handle payments without saving a recurring customer card token. This ability applies to online stores, call center orders, and point of sale (POS) customer orders that require future shipment.
...
| Recurring | A type of card payment token that you intend to reuse for future transactions, either scheduled or unscheduled. |
| Nonrecurring | A type of card payment token that's nonreusable or absent.
| Card on file | A saved card reference token that's specific to a customer's account and that the customer agrees to save for future use in the Commerce system or payment gateway. |

## Prerequisites

Before you set up an environment to use nonrecurring payment tokens, make sure you meet these prerequisites:

- Enable the following features in Commerce headquarters at **System administration** \> **Workspaces** \> **Feature management**:
...
- For online channels to support nonrecurring tokens, enable the **Enable single payment authorization checkout** setting in Commerce site builder, at **Site settings** \> **Extensions** \> **Card and checkout**.
- For customers who use the Dynamics 365 Payment Connector for Adyen, disable the **Tokenization** field's **Recurring** setting in the Adyen portal, at **Settings** \> **Checkout settings**. This setting exists at the merchant account level and affects all transactions under the merchant account. When the setting is enabled, it forces a recurring detail reference for all transactions against the merchant account. When the setting is disabled, the Dynamics 365 Payment Connector for Adyen sets recurring details as needed for transactional situations. The system mandates a recurring payment token even though the nonrecurring token feature is enabled for the following scenarios:

  - Future orders
  - Installments
  - Unlinked refunds

In these scenarios, a message informs users that a recurring token payment is required and that they can proceed with tokenization after they acknowledge the message. Align training for these processes with your company's compliance policy for saving payment tokens.

> [!WARNING]
> If you enable the **Enable unified payments experience in POS** feature, the nonrecurring token feature doesn't function as expected. This temporary limitation will be fixed in a future release.

## Payment flow updates that use nonrecurring payment tokens
...
When customers pay for goods that they're leaving the store with, the transaction is a "cash and carry" transaction. In this case, the payment is resolved through a captured payment at the end of the transaction. There's no need for the system to use or save a card payment token, so the card isn't tokenized. There's no change from the user experience perspective if the **Enable use of non-recurring tokens in Commerce** feature is enabled or disabled.

#### Customer orders

Place a customer order from the POS, which sets up a sales order that is fulfilled through a shipment to the customer. The customer order has two payment-related concepts, *deposit* and *remaining balance*. If the **Enable use of non-recurring tokens in Commerce** feature is enabled, the cart used for the deposit isn't tokenized because the deposit amount is captured along with order placement. However, whether the card used to pay the remaining balance is tokenized or not depends on the **Requested shipping date** of the order lines. If the requested shipping dates of all the order lines are within a system-defined threshold whose default value is seven (7) days, then no card payment token is saved to place the order. But if the requested shipping date of any of the order lines exceeds a system-defined threshold, then the system enforces the card to be tokenized to avoid payment failure during order fulfillment. To enforce the tokenization, when the cashier tries to take a card authorization for the remaining balance, a dialog is displayed as shown in the following image. The dialog prompts the cashier to follow your organization's compliance standard business procedures to obtain the customer's consent to save their card information for the current order.

:::image type="content" source="./media/Token_prompt.png" alt-text="Screenshot of the dialog prompting the cashier to get customer consent for tokenization.":::

If the customer agrees to saving the card information for the order, the cashier can select **OK** to continue with the authorization. Otherwise, they can select **Cancel**. The cashier can't proceed with the authorization for such orders unless the customer agrees to allow the system to save their card information, or if the requested shipping date is changed and brought within the system-defined threshold. By default, this system-defined threshold is set to seven (7) days and isn't exposed due to any configuration. Retailers can contact the Microsoft support team to update this value to any desired value that meets their business needs. The name of the property that defines this threshold is **Payments.MinOrderProcessingDaysForStoringCardToken**.

Currently, the Commerce system doesn't extend the authorization for a nonrecurring token. If a nonrecurring token expires before an order is invoiced, the invoice action fails unless the retailer obtains a new token by contacting the customer. If your business generally has long lead times for customer orders that result in expired authorizations, to avoid the need to contact the customer to obtain a new authorization you can request that Microsoft support enables the **Payments.AlwaysStoreCardTokenOnOrderBalanceAuthorization** property in your environment. This property enforces all customer order authorizations for the remaining balance to be tokenized, and the cashier sees the dialog indicating that the authorization will be tokenized for remaining balance authorizations. In other words, the cash and carry transactions and customer order deposit aren't tokenized but the remaining balance authorizations are tokenized.
...
When a user edits orders in a call center, if the user manually modifies the order and selects **Complete** and then modifies the existing authorized payment line to reflect the new order total, the payment connector attempts to adjust the existing authorization. The authorization adjustment requests the new authorization total against the preexisting original authorization. If the request is approved, the authorization is adjusted in the Commerce system. If it's declined, the original authorization is considered declined, and a new payment is required to complete the transaction.

> [!NOTE]
> If the order total is modified without manually modifying the order (for example, when the order total increases due to the increase of charges or taxes), the system doesn't adjust the authorization and captures only the amount that's authorized. The only way to adjust the authorization is to manually edit the existing payment line.

### Online storefront payments

For online storefront payments that use credit cards or digital wallets, the payment module processes an online customer's entered payment information. When you enable the **Enable use of non-recurring tokens in Commerce** feature, all online transactions are processed in the Commerce system, but the payment card token isn't saved. The payment module presents the configured payment processor's payment acceptance page in an iFrame element.

When you configure the Dynamics 365 Payment Connector for Adyen for the online store, if you set the **Allow saving payment information in e-commerce** parameter to **True**, the Adyen iFrame element presents a **Save for my next payment** checkbox to the authenticated customer. This mechanism enables the customer's credit card to be saved with Adyen so that it's presented in the Adyen iFrame element at the time of the authenticated user's next checkout. However, this mechanism doesn't save the card on file in the Commerce system. The online storefront's generated sales order processes the payment without a saved payment card token.
...
You can save a payment card token against a customer record for scenarios where a card on file is required and there's an agreement with the customer to save the card for future use.
...
You can use a saved card on file for future sales order references in a call center. For more information, see [Call center payments](#call-center-payments).
...
When you select a record, you see details for the customer account, sales order, and payment status. Users can also view the credit card and authorization response details for more context about a declined authorization. To collect a new payment, select the sales order number to go to the sales order, and then process a new payment line against the order's balance. If previous authorizations exist against the balance, it reflects the amount of any authorized but expired payment lines (or any open and unapplied payment lines).
