...
description: Learn how to create an end-to-end payment integration in Microsoft Dynamics 365 Commerce Store Commerce for a payment terminal that can directly communicate with the payment gateway.
author: Reza-Assadi
ms.date: 02/17/2026
ms.topic: how-to
ms.author: rassadi
...
:::image type="content" source="media/PAYMENTS/PAYMENT-TERMINAL/Overview.jpg" alt-text="Screenshot of the payment connector integration overview.":::
...
- **[Write a payment processor](#write-a-payment-processor):** Use the payment processor to define the merchant properties that are part of the payment integration. The section for this step describes how to implement a new payment processor. It includes information about the interfaces that you should implement and patterns that you should follow.
...
:::image type="content" source="media/PAYMENTS/PAYMENT-TERMINAL/PaymentFlow.jpg" alt-text="Screenshot of the payment flow overview across POS, Hardware Station, and payment connector.":::

### Implement a payment connector
The following section describes how to implement a new payment connector. You can find the examples in the **PaymentDeviceSample** class located under the **SampleExtensions\HardwareStation\Extension.PaymentSample** folder in the Retail software development kit (SDK).

#### Implement the INamedRequestHandler interface
The Hardware Station handles all POS payment-related flows through request/response patterns. The first step in writing a new payment connector is to create a class that implements the **INamedRequestHandler** interface, which is defined in the **Microsoft.Dynamics.Commerce.Runtime.Framework** library.
...
The following table describes all supported request types that a payment connector can implement.

| Request class | Payment flow description |
|---|---|
| OpenPaymentTerminalDeviceRequest | This request is called before a sales transaction is initiated. It's used to establish a connection to the payment terminal. |
| BeginTransactionPaymentTerminalDeviceRequest | This request is called when a new sales transaction is initiated. It's used to handle any initialization on the payment terminal (for example, by initializing the transaction screen). |
...
| EndTransactionPaymentTerminalDeviceRequest | This request is called when the sales transaction is concluded and all payments are captured. |
| ClosePaymentTerminalDeviceRequest | This request is called after the sales transaction is concluded. It's used to close the connection to the payment terminal. |
...
| ExecuteTaskPaymentTerminalDeviceRequest | This extension request can be invoked from the POS through customizations. It enables more payment-related flows. |
| GetTransactionReferencePaymentTerminalDeviceRequest | This request is called to check the correlation ID. It's used for duplicate payment protection. |
...
|---|---|
| token | The unique token value that's generated when the payment terminal is initially locked for the transaction. |
...
| extensionTransactionProperties | The set of extension configuration properties as name/value pairs. |
...
|---|---|
| token | The unique token value that's generated when the payment terminal is initially locked for the transaction. |
| paymentConnectorName | The name of the payment connector that you use as part of the payment flow. Use this variable if you plan to integrate with payment flows that use the **IPaymentProcessor** interface. |
| merchantInformation | The merchant information that you define on the **POS hardware profile** page in the client. |
...
| Variable | Description |
|---|---|
| clientDeviceNumber | The unique POS device number that acquires the lock. |
...
| isOverride | Determines whether this request overrides any existing lock. |
...
|---|---|
| token | The unique token value that's generated when the payment terminal is initially locked for the transaction. |
| paymentConnectorName | The name of the payment connector that you use as part of the payment flow. Use this variable if there's an integration with payment flows that use the **IPaymentProcessor** interface. |
| amount | The amount to authorize. |
| currency | The currency for the amount to authorize. |
| tenderInfo | The card information that the POS sends and retrieves from an external source (if an external source is present). |
...
###### Response
Return the **AuthorizePaymentCardPaymentResponse** response object when handling the **AuthorizePaymentTerminalDeviceRequest** request. The response must contain an instance of the **PaymentInfo** object that has the following required properties.
...
| CardType | The type of card that you used for the payment (for example, **Credit** or **Debit**) by using the **Microsoft.Dynamics.Commerce.HardwareStation.CardPayment.CardType** entity. |
| CashbackAmount | For debit transactions, the cash-back amount that you define on the payment terminal. |
| Errors | The list of errors that occurred during the authorization call. |
...
| TipAmount | The tip amount that the customer selected on the device. |
...
If the payment terminal handles the authorize and capture requests in a single call (that is, if *immediate capture* occurs), and the cashier wants to void the transaction, the payment terminal must support reversal of an immediate capture. When an immediate capture is voided, if the void request fails, the cashier is asked whether they want to locally void the payment. If the cashier selects **Yes**, the tender is voided only in the POS. No call is made to the payment terminal to void the payment. This behavior lets the cashier unblock the POS if it can no longer void the payment on the payment terminal. However, this behavior can cause problems, because a lock lasts for three to five days, until the bank reverses it, but the payment is made for immediate capture. Therefore, duplicate payments can occur.
...
| paymentPropertiesXml | The content of the **PaymentSdkData** object that's returned by the **AuthorizePaymentTerminalDeviceRequest** or **RefundPaymentTerminalDeviceRequest** request. Use this property to support stateful properties between the requests. |
| extensionTransactionProperties | The set of extension configuration properties as name/value pairs. |
...
If the payment terminal requires state from the authorization requests to handle the capture call, store the properties in the **PaymentSdkData** object of the **AuthorizePaymentTerminalDeviceResponse** request, described earlier. Pass these properties through the **paymentPropertiesXml** variable of the **CapturePaymentTerminalDeviceRequest** request.
...
| tenderInfo | The card information that the POS sends and retrieves from an external source (if an external source present). |
| extensionTransactionProperties | The set of extension configuration properties as name/value pairs. |
...
|---|---|
| token | The unique token value that's generated when the payment terminal is initially locked for the transaction. |
| amount | The amount that the POS sets. Use this variable to show the amount on the payment terminal when the card number is retrieved. |
...
|---|---|
| token | The unique token value that's generated when the payment terminal is initially locked for the transaction. |
| task | The unique identifier for the task that runs. |
| extensionTransactionProperties | The set of extension configuration properties as name/value pairs. |
...
| Variable | Description |
|---|---|
| locktoken | The unique lock token that's generated when the payment terminal is initially locked for the transaction. |
| posTerminalId | The POS terminal ID associated with the lock token. |
| extensionTransactionProperties | The EFT terminal ID associated with the transaction and lock token. |
...
|---|---|
| locktoken | The unique lock token that's generated when the payment terminal is initially locked for the transaction. |
| Retail.PaymentSDK.Portable.PaymentTransactionReferenceData TransactionReferenceData | Reference data for the payment transactions in case the correlation ID is out of sync. |
...
| Variable | Description |
|---|---|
| paymentConnectorName | The name of the payment connector that's used as part of the payment flow. Use this variable if there's an integration with payment flows that use the IPaymentProcessor interface. |
| amount | The amount for the gift card cash out request. |
| currencyCode | The currency for the gift card cash out request. |
| tenderinfo | The card information that's sent from the POS and retrieved from an external source (if an external source is present). |
| extensionTransactionProperties | The set of extension configuration properties as name/value pairs. |


#### State in the payment connector
You can host the payment connector as part of the dllhost.exe process when you host it through the in-process Hardware Station inside the POS. Alternatively, you can host the payment connector as a w3wp.exe process when you host it in the Hardware Station that is based on Microsoft Internet Information Services (IIS). In some circumstances, both processes can terminate or stop responding between or during payment flows. Therefore, don't design payment connectors to have state dependencies. Design them to recover if they're terminated at any point during the payment flow–related requests that are described earlier.

### Configure the payment connector in the Hardware Station config
To ensure that the Hardware Station loads the payment connector, set the corresponding assembly reference in the **HardwareStation.Extension.config** file. This file is located in the **Assets** folder in the Retail SDK.
...
To determine the correct payment connector that the POS loads, set the value of the **PaymentTerminalDevice** property in the **Device name** field on the **PIN pad** FastTab of the **POS hardware profile** page in the client, as shown in the following illustration.
...
You usually use payment processors only if you establish a direct connection to a payment gateway. This scenario most often occurs in card-not-present sales transactions or more complex card-present scenarios. Additionally, the payment processor processes the merchant properties that you configure through the **POS hardware profile** page in the client.

> [!NOTE]
> The payment processor is currently required, even if the payment terminal handles all payment requests directly and no merchant properties need to be set through the POS. For more information about implementing the **IPaymentProcessor** interface, see the [Implementing a payment connector and payment device](https://download.microsoft.com/download/e/2/7/e2735c65-1e66-4b8d-8a3c-e6ef3a319137/The%20Guide%20to%20Implementing%20Payment%20Connector%20and%20Payment%20Device_update.pdf) white paper.

### Understanding the merchant properties flows
The following sections describe how the merchant properties are set on the **POS hardware profile** page in the client, and how the POS passes them to the payment connector during payment flows.

#### Set merchant properties on the POS hardware profile page in the client
The following illustration shows how you set the merchant properties through the **POS hardware profile** page in the client. To enable the merchant properties to be set, implement the **IPaymentProcessor** interface that's defined in the **Microsoft.Dynamics.Retail.PaymentSDK** library. Two interface methods are required: **GetMerchantAccountPropertyMetadata** and **ValidateMerchantAccount**.
...
The following illustration shows how the Commerce Scale Unit retrieves the merchant properties from the database and passes them to the payment connector during the **BeginTransactionPaymentTerminalDeviceRequest** request.
...
To handle merchant properties related to payment flows, implement the **IPaymentProcessor** interface defined in the **Microsoft.Dynamics.Retail.PaymentSDK** library. The following example shows how to implement the two required interface methods, **GetMerchantAccountPropertyMetadata** and **ValidateMerchantAccount**. You can leave other interface methods blank (for example, they can return **FeatureNotSupportedException**).
...
The following table shows the required merchant property fields that you set as part of the **GetMerchantAccountPropertyMetadata** method.
...
\* Replace the sample values in this column with unique values for your own payment processor.
