...
Before you can enable the Tap to Pay on iPhone feature, you need to complete onboarding with both Adyen and Microsoft. Contact your Adyen representative to complete the onboarding with Adyen. Then contact Microsoft Support, and ask to have the feature enabled in your environment.
...
The Tap to Pay on iPhone feature works on iPhones that run the Store Commerce app. You must install the app first. For more information, see [Install the app](store-commerce-mobile.md#install-the-app).
...
> [!NOTE]
> Only the cloud architecture for Adyen connector supports Tap To Pay on iPhone. Set the **Terminal architecture** property of the Adyen connector to **Cloud** instead of **Local**.

Next, configure the PIN pad properties in headquarters.
...
1. The Tap to Pay on iPhone feature adds an option named **Accept NFC Payments**. Set this option to **Yes**. If this option doesn't appear on the **PIN pad** FastTab, contact Microsoft Support, and ask to have the Tap to Pay on iPhone feature enabled in your environment.
...
:::image type="content" source="../media/Accept_NFC_Payments.png" alt-text="Screenshot of the configuration of PIN pad properties, including the Accept NFC Payments option, for a hardware profile.":::
...
Use this optional step to test the Tap to Pay on iPhone capability. In some scenarios, the Near Field Communication (NFC)-based tap capability isn't sufficient. For example, the customer might not have an NFC-enabled card, or the customer might want to use a gift card or a wallet.

To support these additional use cases, pair the iPhone that runs the Store Commerce app with a PIN pad device. In headquarters, go to the register that you use with the iPhone. On the Action Pane, on the **Register** tab, in the **Hardware** group, select **Configure IP address**. Then set the details on the **PIN pad** FastTab. Learn about PIN pad onboarding in [Onboard and configure an Adyen payment terminal](adyen-connector-setup.md#onboard-and-configure-an-adyen-payment-terminal).
...
For test environments, set the **Gateway environment** property of the Adyen connector in the hardware profile to **Test**. For live environments, the value of the **Gateway environment** property depends on whether you want to pair the iPhone with an external PIN pad terminal.

- If you don't want to pair an external PIN pad terminal with an iPhone, set the **Gateway environment** property of the Adyen connector to **Custom**. Also set the **Optional Domain** property of the Adyen connector in the hardware profile to the prefix value that you can find in the Adyen customer portal.
    ...
    1. Copy the **Prefix** value that is shown. The following example image shows the **Prefix** property on the **API URLs** page. (The property value is masked in the image.)
        ...
        :::image type="content" source="../media/Prefix.png" alt-text="Screenshot of the Prefix property and masked value on the API URLs page in the Adyen customer portal.":::
...
:::image type="content" source="../media/TTP_ux.png" alt-text="Screenshot of the user's experience of paying by using the Store Commerce app with Tap to Pay on iPhone.":::

## Limitations of the Tap to Pay on iPhone feature

You can't use Tap to Pay on iPhone to authorize the remaining balance of a customer order. In this scenario, the merchant has several options:
