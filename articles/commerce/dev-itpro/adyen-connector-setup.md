...
> [!IMPORTANT]
> Adyen will no longer support [Hosted Payment Pages | Adyen Docs](https://docs.adyen.com/online-payments/classic-integrations/hosted-payment-pages) as of October 2022. When configuring the Dynamics 365 Payment Connector for Adyen, the **Version** "V001" will no longer be supported for Call Center and E-Commerce as it utilizes Hosted Payment Pages from Adyen. Please use **Version** "V002" (with Origin Key) or "V003" (with Client Key). "V003" is recommended by Microsoft and is available in versions 10.0.29 and higher. In addition to Origin Key or Client Key, ensure you set the **Allowed Origins** in the Adyen configuration, adding the full URL (example: 'https://www.adventure-works.com') for your site (for E-Commerce) or Headquarters (for Call Center), depending on the element being configured.
...
1. On the **Payment service account** tab, enter the following information.
    ...
    | Version | Enter the version of the Dynamics 365 Payment Connector for Adyen to use. "V003" should be used for all new implementations, "V002" is available while Origin Key is supported by Adyen.  "V001" is no longer supported.  | Yes | Yes | "V002"/"V003" |
    ...
    | Origin Key (V002) or Client key (V003) | 'Origin Key' required when "V002" is designated for the version. You can obtain this key by following the instructions on the [How to get an origin key](https://docs.adyen.com/user-management/how-to-get-an-origin-key) page on the Adyen website. 'Client Key' required when "V003" is designated for the version. You can obtain this key by following the instructions on the [Migrate to Client key](https://docs.adyen.com/development-resources/client-side-authentication/migrate-from-origin-key-to-client-key#switch-to-using-the-client-key) page on the Adyen website.| Yes | No | *The full Origin or Client key* |
...
1. Enter the following additional information.
    ...
    | Allow saving payment information in e-commerce | (*E-commerce only*) Gives signed-in users the option to save payment details for future online purchases.  | Yes | Yes | True/False |
    | Authorization stale period (days) | (*POS only*) Number of days before an authorization is considered stale and should decline before going to the processor for capture. | Yes | Yes | "7" |
    | Origin Key (V002) or Client key (V003) | (*E-commerce only*) 'Origin Key' required when "V002" is designated for the version. You can obtain this key by following the instructions on the [How to get an origin key](https://docs.adyen.com/user-management/how-to-get-an-origin-key) page on the Adyen website. 'Client Key' required when "V003" is designated for the version. You can obtain this key by following the instructions on the [Migrate to Client key](https://docs.adyen.com/development-resources/client-side-authentication/migrate-from-origin-key-to-client-key#switch-to-using-the-client-key) page on the Adyen website. | Yes | No | *The full Origin or Client key* |
    | EnableRequestProtection | Adds retry logic to "card not present" payment calls, reducing potential for duplicate calls using a correlation ID. When "True", a correlation ID is added to provider requests to prevent duplicates. If "False", calls are sent to the provider without the correlation ID or duplicate protection logic. | No | No | True/False |
    | Non-incremental capture payment methods | Names of the payment method variant or card types used by Adyen to identify card types in authorization responses which do not support incremental capture. Value entered should match the payment method variant/card type string used in Adyen to reference, as noted in [Adyen PaymentMethodVariant](https://docs.adyen.com/development-resources/paymentmethodvariant).  | No | No | "amexcommercial" |
