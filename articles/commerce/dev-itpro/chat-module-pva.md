...
The Commerce chat with Microsoft Copilot Studio feature empowers Dynamics 365 e-commerce customers to use Microsoft Copilot Studio agent capabilities to handle their queries. As of the Dynamics 365 Commerce 10.0.30 release, you can incorporate this feature into e-commerce websites by using the Commerce chat with Microsoft Copilot Studio module that is part of the Commerce module library.
...
After you configure the agent, follow the procedures in the next section to get the bot ID and tenant ID parameter values that you use to configure the Commerce chat experience.

### Find the bot ID of a Microsoft Copilot Studio agent

The Microsoft Copilot Studio user interface doesn't show the bot ID directly. To get it, request a Direct Line token for your agent and read the bot ID from that token.
...
1. In the **Direct Line Speech** pane, under **Get connected**, select **Copy** next to **Token Endpoint** to copy the token endpoint URL.
    ...
    :::image type="content" source="media/copilot-studio-direct-line-speech-token-endpoint.png" alt-text="Screenshot of the Channels tab in Microsoft Copilot Studio with the Direct Line Speech channel selected and the Token Endpoint value highlighted in the Direct Line Speech pane.":::
...
:::image type="content" source="media/copilot-studio-advanced-settings-tenant-id.png" alt-text="Screenshot of the Metadata section on the Advanced settings page in Microsoft Copilot Studio, with the Tenant ID value highlighted.":::

## Configure your e-commerce site 

One recommended way to implement the chat experience for your e-commerce site is to add the Commerce Chat with Microsoft Copilot Studio module to the shared header fragment that you use on your site pages.
...
> [!NOTE]
> You must publish your Microsoft Copilot Studio agent before the chat module can connect to it. If the agent is unpublished, or if it requires end-user authentication, the chat window fails to start a conversation on your site.
...
> [!NOTE]
> Currently, Microsoft Copilot Studio doesn't support Microsoft Entra ID B2C (Microsoft Entra B2C) authentication. It supports only anonymous Retail Cloud Scale Unit (RCSU) calls to get product availability or interact with other anonymous APIs. Calls to authenticated APIs through Microsoft Copilot Studio agents require an explicit customer sign-in.
