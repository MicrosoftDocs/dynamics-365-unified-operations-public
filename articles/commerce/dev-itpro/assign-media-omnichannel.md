...
description: Learn how to assign media to products and categories by using omnichannel media management in Microsoft Dynamics 365 Commerce.
author: phinneyridge
ms.date: 02/12/2026
ms.topic: how-to
...
> [!NOTE]
> The following procedures assume that you already configured omnichannel media management features in your Commerce environments by following the steps in [Omnichannel media management prerequisites and configuration](omnichannel-media-management-overview.md#omnichannel-media-management-prerequisites-and-configuration).
...
1. In the upper right, select **Finish editing** to check in your changes. Although this action doesn't publish your edits, it enables other users in your organization to view staged changes and make more edits before publication.

### Additional media

Use the **Additional media** assignment scope for media assignments that serve a purpose outside a standard product media gallery. **Additional media** items have a **Media purpose** string property that indicates their intended use. Typical **Additional media** purposes include product manuals, spec sheets, media kits, and any other custom media assignment purpose that's outside the primary media gallery. You can set any alphanumeric string as the value of an **Additional media** item's **Media purpose** property. Commerce software development kits (SDKs), APIs, and data actions can access that string for extensibility scenarios.

To assign media to a product's **Additional media** assignment scope, follow these steps:

1. In the site builder **Omnichannel content** workspace, open the **Product media** assignment view. Alternatively, open the same view in headquarters by selecting **Product media assignments** in the **Released products by category** view.
1. In the search field on the left, search for a product by its name or product ID. Then select it.
1. In the upper right, select **Edit product media**.
...
You can assign media in a hierarchical fallback structure across channels and locales. The default assignments for any product should be **Omnichannel** (for channels) and **Neutral** (for locales). These assignments are returned by default whenever a more specific combination of a channel and a locale isn't available. For channel and locale combinations that require different product media assignments than the default **Omnichannel** and **Neutral** assignments, configure specific channel and locale media assignments to override the default assignments. After you create and publish a specific channel and locale media assignment for a product, the inheritance to the omnichannel combination is severed. All media assignments are controlled by the specific channel and locale media assignments.

The following illustration shows the fallback order for channel and locale media assignments.

:::image type="content" source="../media/OMM_2.png" alt-text="Diagram of the fallback order for channel and locale media assignments.":::
...
Product masters typically have one or more product dimensions (color, size, style, or configuration) that determine the complete set of possible variants for the product. Media that you assign directly to the product master serve as the default media for any product variants or dimensions that don't have their own explicit media assignments. When you create media assignments for product dimensions or specific product variants, you sever the inheritance to the product master assignments. The dimensions or the specific product variant media assignments control all media assignments.
...
1. If you're using a hexadecimal color value, select the desired value in the color picker control. Then go to step 11.
...
1. When you finish, at the bottom of the pane, select **Add swatch**.
1. Repeat steps 4 through 11 for each additional dimension value.
1. In the upper right, select **Finish editing** to check in your changes. Although this action doesn't publish your edits, it enables other users in your organization to view staged changes and make additional edits before publication.

### Assign media to product dimensions

For product masters that have multiple dimensions, you typically need to assign media to only some of the dimensions. For example, in the apparel industry, you usually take photos for each color and style of the same master product shirt, but not for each size.
...
1. In the center of the workspace, select **Create assignment matrix**. If someone already created an assignment matrix for this product master, go to step 7.
...
1. To reorder the media in the **Primary media** assignments, use the up and down arrow buttons, or drag the media items to their correct position.
1. When you complete your changes, in the upper right, select **Finish editing** to check in the new set of media assignments. Although this action doesn't publish your edits, it does enable other users in your organization to view staged changes and make additional edits before publication.
...
Commerce currently supports the assignment of a single default image to each channel-specific product category. Use category media assignments for category navigation user experiences. Category media assignments are always channel-specific. Therefore, you don't make default media assignments at the omnichannel level as you can for individual products.
...
1. In the upper right, select **Finish editing** to check in the category media assignment. Although this action doesn't publish your edits, it does enable other users in your organization to view staged changes and make more edits before publication.
