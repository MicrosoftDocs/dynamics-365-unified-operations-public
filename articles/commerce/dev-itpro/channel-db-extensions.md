...
- Editing the values of any fields of tables in AX or CRT schemas from an extension is unsupported by Microsoft and will eventually break the schemas when stricter enforcement improvements are introduced. Any issues that arise from editing schema values via extensions aren't supported by Microsoft.
...
- Don't use any of the CRT, AX, or DBO schema data types in the EXT schema. You must create custom types in the EXT schema and use those intead.
