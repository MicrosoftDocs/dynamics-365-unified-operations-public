...
description: Learn how to add custom notifications in Microsoft Dynamics 365 Commerce point of sale (POS).
author: josaw1
ms.date: 02/17/2026
ms.topic: how-to
...
- Periodically perform some operation in the background. However, this scenario isn't recommended, because it might cause performance problems.

## How custom notifications work

The notification framework in the POS periodically runs for the configured operation in Retail. (You can configure the frequency of the periodic runs.) It calls the notification service in the Commerce runtime (CRT) and real-time service in Retail. When the POS makes the call to the notification service in CRT, CRT runs the business logic and determines whether any notification must be sent to the POS for the operation. The operation ID is passed to the request as a parameter. The CRT notification service checks the operation ID and returns the notification details.

If the notification exists, CRT returns the response to the POS and reports that a notification is available. POS parses this notification and shows it in the user interface (UI). When the POS user selects the notification, the relevant operation in POS runs. Notifications link to an operation. Inside the operation handler, you write the business logic that must be performed.
...
1. In Retail headquarters, go to **Retail and Commerce \> Channel Setup \> POS Setup \> POS \> POS operations**, and create an operation.
    ...
    > [!NOTE]
    > For custom operations, use an operation ID that's greater than 5000.

1. Extend the notification service. When the scheduler for the notification service runs, it calls the extension code for both CRT and the Real time transaction service to get the notification message.
1. Extend the POS. In the POS, write logic inside the POS operation to specify what happens when the POS user selects the notification. For information about how to create a POS operation, see [Add POS operations to POS layouts by using Button grid designer](add-pos-operations.md).
1. In Retail headquarters, configure the notification service with the custom operation. Then, when the scheduler runs the notification service, it includes the custom operation. For information about how to configure notifications in the POS, see [Add POS operations to POS layouts by using Button grid designer](../notifications-pos.md).

    When the POS user selects the notification, the framework calls the correct operation, based on the response that CRT returns. The notification service runs for all the notifications that are configured in Retail headquarters, and it returns the response that includes the operation ID and notification details. Therefore, the POS has the context of the operation ID, and the framework uses this context to call the operation implementation.
...
+ **GetNotificationsExtensionServiceResponse** – In the response, return the notification detail entity (**NotificationDetailCollection**), update **GetNotificationsExtensionServiceResponse**, and return all the notifications. Because the response is a collection, you can return multiple notifications.

## Properties of the notification detail entity

The notification detail entity has the following properties. You can configure some of these properties in the Retail essentials button grid designer and in Commerce headquarters.

| Property               | Data type      | Description                                                                            |
|------------------------|----------------|----------------------------------------------------------------------------------------|
| ActionProperty         | string         | A custom property to send to the POS operation. Configure this property by using the **Operation parameter** field in the button grid designer.         |
...
| LastUpdatedDateTime    | DateTimeOffset | The date and time when the item in the action property was last updated.               |
| LastUpdatedDateTimeStr | string         | The date and time when the item in the action property was last updated, in string format. |
...
1. Rename the project according to the standard naming convention.
1. Inside the project, open the class file named **NotificationExtensionService.cs**.
1. Override the **GetNotificationsExtensionServiceRequest** class to add a custom notification. Return **GetNotificationsExtensionServiceResponse**.
1. Create a new class and override **GetNotificationsExtensionServiceRequest**, or use the sample template. 
1. In the **NotificationExtensionService** class, there's a method named **Process**. The code inside that method checks the operation ID and then, based on the operation ID, creates a notification details object and adds any notifications. Check whether the operation ID is custom operation ID, and then write logic to check whether there are any notifications. If there are, create a notification object that contains the details, and return it together with the response. The POS then parses the response and shows the notification. The following code example is based on the template.

> [!NOTE]
> The action property in the notification detail entity is sent to the POS operation request. Use the action property to pass any custom information from the notification service to the POS. The **ActionProperty** can be configured by using the **Operation parameter** field of the button grid designer, and the **ActionProperty** value should be equal to the input of the **Operation** parameter in the button grid designer.
...
1. After you complete your changes, build the project, and drop the output library into **\\RetailServer\\webroot\\bin\\Ext**.
1. Register the output library in the **CommerceRuntime.Ext.config** file.
1. In the POS, create a new operation that has the same operation ID that you used in the CRT extension. In this example, the operation ID is **5000**. You can use any operation ID that's above 5000.
1. When the POS user selects the notification tile, the POS framework calls the operation handler for the operation ID that is used. Inside the handler, add the required logic to specify what should happen when the POS user selects the notification. For information about how to create a POS operation request, response, and handler, see [Show order notifications in the point of sale (POS)](add-POS-operations.md).
1. Configure the notification scheduler according to the instructions in [Show order notifications in the point of sale (POS)](../notifications-pos.md).
...
1. Debug the CRT code by attaching the CRT project to w3wp.exe. The breakpoint is hit whenever the notification service is called from the POS.
1. When the notification is received in the POS, select it. The notification calls the POS operation handler and runs the custom logic inside it.
