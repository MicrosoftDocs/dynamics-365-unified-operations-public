...
2. Create a new C# class library project that uses the Microsoft .NET Framework version netstandard 2.0 as the target framework, **starting 10.0.26, SDK supports only .NET 6 as the target framework** . Alternatively, use one of the Retail Server samples in the Retail SDK as a template.
