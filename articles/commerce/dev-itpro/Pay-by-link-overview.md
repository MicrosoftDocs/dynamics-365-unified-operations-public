...
If a transaction that has a payment link is voided, then the system tries to expire the active payment link. But cancellation isn't guaranteed. If there are no connection issues and the payment hasn't been made by the payment link, then it is canceled. However, if there are connection issues or the payment has been made, then the payment link isn't canceled even though the transaction is voided. 
