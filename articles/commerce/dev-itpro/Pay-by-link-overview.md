...
The suspended transaction screen has been updated to include a new column "Has payment link”. A value of Yes indicates that this transaction was suspended with an active payment link. The cashier can multi-select the suspended transactions and attempt to void the transactions. For the transactions with payment link, the system will first try to check if the payment link has been used. If unused, then the payment link will be expired and the transaction will be voided, but if the payment link has been used, then an error will be displayed indicating that the payment is made for the payment link. For such cases, the cashier would have to resume the transaction and then either void the payment or complete the transaction. Additionally, when the shift is being closed, if the configuration "Void when closing shift" is enabled in the functionality profile for the store, then the system will follow the same logic as explained above to check the payment link status and expire the unused payment links, before voiding the suspended transactions.

> [!NOTE]
> Suspended transactions with payment links cannot be voided in the offline mode since the payment link status cannot be verified.

With 10.0.44, the PBL payment method supports the scenarios where the payment needs to be "captured”, and the customer is in the store and wants to make a payment. Thus, apart from the above-mentioned scenario, the PBL can also be used for capturing customer order deposit, payment for invoices and customer deposit in POS.  In future releases, the ability to create orders with pending payments will be released which will enable scenarios such that the store associate would be able to create the customer order with customer's shipping details and generate a payment link for the customer. The customer would be able to make the payment from the comfort of their home, whenever they are ready. If the payment is not received within the predefined duration, then the order will be cancelled.
