...
- Enables reuse of extension or independent software vendor (ISV) code developed for the Store Commerce app or Store Commerce for web by using the Commerce software development kit (SDK) with minimal changes.
- Provides an industry-standard developer experience that uses Visual Studio Code and GitHub.
- Combines the benefits of both MPOS and Store Commerce for web.
- Improves performance.
- Simplifies POS and extension upgrades through the Commerce sealed installer framework.
- Supports a dedicated hardware station.
...
The Store Commerce app runs on Windows devices and you can download it from the [Shared asset library in Microsoft Dynamics Lifecycle Services](https://lcs.dynamics.com/V2/SharedAssetLibrary). On the **Shared asset library** page, select **Retail Self-service package** as the asset type, and then find the file that ends with "Store Commerce." Ensure you select the version for the Commerce release that you're using (for example, 10.0.25 or 10.0.26).

### Store Commerce deployment options

Store Commerce supports two types of deployment topologies:

- **In-app** – Deploys all components locally. Supports offline mode and local Hardware Station (HWS).
- **Hybrid** – Store Commerce renders the Cloud POS that deploys in Commerce Scale Unit (CSU) and supports local Hardware Station. However, it doesn't support offline mode.

The hybrid and in-app topologies don't have separate installers. Installation parameters determine the deployment options.

![Store Commerce deployment options.](../media/SC-Deployment.png)

### In-app deployment

For the in-app deployment option, the application content is deployed locally. Store Commerce renders the application content from its local deployment. It doesn't retrieve the Cloud POS UI hosted in the Commerce Scale Unit (CSU).

To update the application content, run the latest version of the Store Commerce installer. Updating the CSU doesn't update the application content, so you can manage the updates at individual registers.

In-app deployment supports offline mode. During installation, pass the `--installoffline` parameter to deploy the offline database. In offline mode, the application can't connect to CSU or Commerce headquarters, and uses the locally deployed CRT.
...
Store Commerce is a shell that connects to Headless Commerce and Commerce headquarters by using CSU in online mode. In hybrid mode, Store Commerce renders the Cloud POS UI that is hosted in the CSU. When you open the Store Commerce app, it prompts for the Cloud POS URL.

![Active Store Commerce dialog box prompting for the Cloud POS URL.](../media/SC-Hybrid.png)

To update Store Commerce, update the CSU to update Store Commerce automatically. Because updates are centrally managed in CSU, you don't have to manage them at individual registers. You must still update the Store Commerce application shell separately by using the installer. For more information about how to update the CSU, see [Apply updates and extensions to Commerce Scale Unit (cloud)](../../fin-ops-core/dev-itpro/deployment/update-retail-channel.md).
...
<td>Yes, but it doesn't support UWP peripherals that are Point of Service devices. For more information about Store Commerce peripheral support, see <a href="../retail-peripherals-overview.md">Commerce peripherals</a>.</td>
<td>Yes</td>
</tr>
<tr>
...
- Store Commerce supports the following operating systems:
...
- The Store Commerce app uses the .NET Desktop Runtime for UI rendering. For Commerce versions 10.0.49 and later, install .NET Desktop Runtime 10. For Commerce versions 10.0.42 through 10.0.48, install .NET Desktop Runtime 8. For Commerce versions 10.0.41 and earlier, install .NET 6.0.16. To develop Store Commerce extensions for Commerce versions 10.0.49 and later, also install the .NET 10 SDK.
- Exclude the following Store Commerce folders when running any external applications or programs such as antivirus applications to prevent the folders from being deleted:
...
- Exclude the following Store Commerce registry entries when running any external applications or programs such as antivirus applications to prevent the registry entries from being deleted:
...
If the **Store Commerce** application type doesn't appear on the drop-down menu, try running the **Initialize** function from the **General** tab of the **Commerce parameters** page (**Retail and Commerce** > **Headquarters setup** > **Parameters** > **Commerce parameters**). Then refresh the page.
...
Download Store Commerce from the [Lifecycle Services Shared asset library](https://lcs.dynamics.com/V2/SharedAssetLibrary). On the **Shared asset library** page, select **Retail Self-service package** as the asset type, and then find the file that ends with **Store Commerce**. After you download the file, follow these steps to install the app.
...
Use the **help** command in PowerShell to find information about all the parameters. In PowerShell, find the Store Commerce installer, and then enter `Store_Commerce Installer_exe_name help install`.

| Parameter | Description |
| --------- | ----------- |
| `--config <path to config file>` | Specifies the path of the configuration file used as part of the installation. |
| `--device <device identifier>` | Specifies the device identifier to use for the Store Commerce application as a default value. If you omit this parameter, you're prompted to input the device identifier during device activation. |
...
| `--enablewebviewdevtools` | Enables developer tools for Store Commerce. If you omit this parameter, developer tools are only enabled if Windows Developer Mode is enabled. |
| `--help` | Shows parameter information. |
| `--inplaceupgradefrommodernpos` | Executes an in-place upgrade from Modern POS. If you omit this parameter, you need to activate Store Commerce after installation. |
...
| `--register <register number>` | Specifies the register number to use for the Store Commerce application as a default value. If you don't specify this parameter, you're prompted to input the register number during device activation. |
| `--retailserverurl <Retail Server URL>` | Specifies the Retail Server URL to use for Store Commerce. If you don't specify this parameter, you're prompted to enter the Retail Server URL during device activation. |
...
| `--sqlservername <SQL server name>` | Specifies the name of the SQL Server instance that Store Commerce uses in offline mode. If you omit this parameter, the installer uses the default instance. |
| `--trustsqlservercertificate` | Trusts the SQL Server certificate when a connection is established to SQL Server. To help avoid security risks, don't use this argument for production deployments. By default, the SQL Server certificate isn't trusted. |
| `--usecommonapplicationdata` | Installs the Store Commerce app for all users on the device by placing a token in a common application data folder. To use the Store Commerce app, add users to the RetailChannelUsers group on the device. If you omit this parameter, the Store Commerce app is only installed for the current user. |
| `--useremoteappcontent` | Download and display the Store Commerce UI from the Commerce Scale Unit (CSU). If you omit this parameter, the local application content that is deployed with Store Commerce is used. For more information, see [Hybrid deployment](#hybrid-deployment). |
| `--verbosity` | Specifies the verbosity of logs (0 - trace, 1 - debug, 2 - informational, 3 - warning, 4 - error, 5 - critical, 6 - silent). When you omit this parameter, the default value of 2 is used. |
...
1. Open PowerShell in administrator mode and go to the folder where you downloaded the Store Commerce installer executable.
1. In PowerShell, find the Store Commerce installer executable and use the `StoreCommerce.Installer.exe uninstall` parameter to uninstall the app. When you run this command, it starts right away and doesn't show a confirmation dialog before uninstalling Store Commerce.
...
1. On the Windows **Start** menu, search for **Store Commerce**, and then open the application. 
    > [!NOTE]
    > Don't run the Store Commerce app with elevated privileges, and don't run it from an account with elevated privileges.
...
1. After activation is complete, sign in to the application by using an employee account.
...
Use the Commerce SDK to customize Store Commerce. You can modify and create the POS user experience, enhance or modify out-of-box functionality, add validations, and add custom features. For more information, see [Point of Sale (POS) extension overview](pos-extension/pos-extension-overview.md), or review the samples on [GitHub](https://github.com/microsoft/Dynamics365Commerce.InStore).

### Hardware station extension

You can extend Store Commerce to integrate it with hardware devices. Use the [sample extension code](https://github.com/microsoft/Dynamics365Commerce.InStore) in GitHub to generate Store Commerce hardware station extension packages. For more information, see [Integrate the POS with a new hardware device](hardware-device-extension.md).

## Microsoft Edge WebView2 control

The Store Commerce app for Windows uses the [Microsoft Edge WebView2 control](/microsoft-edge/webview2/) for rendering. Microsoft preinstalls and automatically updates the WebView2 evergreen runtime on all Windows 11 and eligible Windows 10 devices. The evergreen runtime distribution model doesn't require more effort to manage, but it doesn't let you control the version of WebView2 that is used with a particular release of the Store Commerce app.

Microsoft recommends that you explicitly manage the WebView2 version that is used by a particular update of the Store Commerce app to mitigate the risk of regressions that incompatibilities between the Store Commerce app and the latest WebView2 control cause.  
...
1. Use the standalone evergreen installer to deploy the specific version of the WebView2 runtime you used during user acceptance testing (UAT) for the Store Commerce app.
