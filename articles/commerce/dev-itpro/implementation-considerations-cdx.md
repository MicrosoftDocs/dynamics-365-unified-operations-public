...
This article is for users who implement functionality related to data synchronization (Commerce Data Exchange, [CDX]) in a Microsoft Dynamics 365 Commerce environment. It provides an overview, implementation tips, and guidance that you should consider as you plan your implementation. It covers pages, setup, configuration, best practices, and more.

Proper configuration and synchronization of data is crucial to a correct implementation. Regardless of business requirements, IT infrastructure, and overall preparedness, if data isn't correctly synchronized, the whole environment is effectively useless. Therefore, a top priority is to understand what's required to configure, generate, synchronize, and verify data across the full implementation. This process goes from Commerce headquarters through the Commerce Scale Unit (CSU) to the brick-and-mortar stores that use the Store Commerce app (with or without an offline database) and other in-store components. CDX is the Commerce functionality that replicates and synchronizes data across databases. However, CDX differs from typical data replication functionality because it also allows for filtering. Therefore, CDX helps minimize data sets by generating only data that's specific to the channels that you specify for selection, filtering specific tables from offline databases, and filtering expired records for data that's no longer used such as expired discounts.

Before you review this article, make sure you understand the concepts of a channel (store), registers and devices, and the Store Commerce app offline database. Therefore, review some of the resources at the end of this article, such as the Device management implementation guide and the overview of the Commerce architecture.
...
These features are available in Commerce version 10.0.12 and later.
...
You can configure this feature in the offline profile. It has the following three related settings:

- **Allow manual switch to offline before sign in**: This setting lets Store Commerce app users switch to offline mode before they sign in to the POS. It's helpful in scenarios where timeouts might occur before sign-in is completed, or where atypical response codes from the Commerce Scale Unit (cloud or self-hosted) occur. When you turn on this setting, a Store Commerce app user who's using an offline database can access the **Settings** menu from the POS sign-in page. This menu includes a new option for switching to offline mode. By selecting this option, the user can sign in directly against the offline database instead of first having to sign in via a call to the Commerce Scale Unit.
...
- On the **Scheduler subjob**, set the option to **Yes** to stop the associated table for any job that contains the subjob from being synchronized to offline databases. For example, the channel database SQL table **DIRPARTYTABLE** syncs by three different scheduler jobs. Therefore, if you exclude the **DIRPARTYTABLE** table, you stop its data from syncing to offline databases by all three jobs. Don't exclude this example table (**DIRPARTYTABLE**) as it's critical for the staff-related data it stores in the offline database.

This feature also represents the first step in row-level filtering. In Commerce headquarters, the **Commerce channel schema** page includes a new option named **Filter shared customer data tables**. To open the **Commerce channel schema** page, go to **Retail and Commerce** > **Headquarters setup** > **Commerce scheduler** > **Channel database group**, and then, in the **Commerce channel schema** field, select a value. The default value is **AX7**. By setting the option to **Yes**, you flag all customer data in shared tables. This setting works only for standard Microsoft-created tables (tables that aren't custom-created). When you set this option to **Yes**, you receive a message that states, "This removes customer data from the records in the offline data package only. Customer data is still synchronized to the channel database. All schedule jobs that contain customer data also need to be marked to skip offline synchronization." You can then select either **Yes** or **No**. This message is intended as a reminder that you must also set the **Exclude from offline databases** option to **Yes** for all customer data jobs. By default, the 1010 job is the only customer data job.
...
The **Commerce channel schema** page also includes a **Filter inventory dimensions containing a license plate ID** option, in the same **Optional data filtering** section. When you set this option to **Yes**, inventory dimension records that contain a license plate ID are excluded from offline databases.

> [!IMPORTANT]
> The **Filter shared customer data tables** and **Filter inventory dimensions containing a license plate ID** options apply to offline databases only. They don't filter the data that's synchronized to the Commerce Scale Unit (channel) database. The channel database always receives the full, unfiltered data set. If you need to restrict the data that reaches the channel database, use a different mechanism such as a scheduler job or a subjob configuration.

## Implementation considerations

This section describes configurations that you should consider when you begin to plan your implementation. The features that are described here are related to data management and data configuration. Before you read the guidance provided here, review [Commerce Data Exchange best practices](CDX-Best-Practices.md).

- **Create a Scheduler job calendar**: How often does each job occur? How many times per day does each job occur? Do large, noncritical jobs occur only during offline-hours, when the overall environment isn't heavily used? By creating a calendar (either physical or virtual), you can learn the details about how jobs intersect with other workloads that affect performance (for example, statement posting), hours of operation, batch processing for external data, and any customizations that push or pull data at specified times (or frequently throughout the day, just like a CDX job).

- **Pause offline synchronization**: As a retail organization expands, it should take advantage of this offline profile feature. Growth is good, but manage data generation to help minimize the performance impact on the currently operating business. This feature enables the creation of channels, registers, and databases, but without requiring a massive, performance-affecting amount of data generation long before the registers are ever used.
...
- **Channel database groups**: At a minimum, two channel database groups should exist - one for the initial Commerce Scale Unit (cloud) that you use, and one for any or all offline databases that you use. Large retail enterprises might have multiple offline-focused channel database groups that are separated based on similarity of data in their associated channels (stores).
    ...
    It's also helpful to have a dummy channel database group that you can use to configure new channel databases, registers that have offline support (in this case, offline databases are created), and maybe even new but unused Commerce Scale Units (either cloud or self-hosted). Because this dummy group isn't associated with any distribution schedule jobs, no data generation ever occurs for anything that's associated with it. As time and the implementation progress, re-associate the associated entities (for example, channels [stores] and register offline databases) with the correct database group. An alternative to this approach is to use the **Pause offline synchronization** feature that was described earlier.

### SQL Server versions and licenses

To learn more about the SQL Server versions and how to use them, see [Commerce offline implementation and troubleshooting](implementation-considerations-offline.md).
