...
- **Channel database** – Use this page to create, review, and edit the channel databases that are used in Commerce Scale Units (both Cloud and Self-hosted) and the offline databases that are used with Modern POS. Each database that you create here refers to a single, physical database (in other words, there is a one-to-one \[1:1\] mapping). The channel or offline database is associated with a channel database group. From this page, you can also create full synchronizations of a scheduler job for a selected channel or offline database.
...
The following illustration shows the various pages in Commerce headquarters and how they are related to each another. (For descriptions of these pages, see the previous section.) CDX data generation can occur only if it's fully configured across all these pages. Data synchronization occurs on two pages: download and upload. CDX data generation is required for downloads. Point of sale (POS) data that is generated while offline requires data synchronization upload.

![Commerce Data Exchange association map](./media/CommerceDataExchange-AssociationMap.png)

This following illustration shows the data flows for download and upload. Data packages that are generated through CDX flow downward, from Commerce headquarters to the Commerce Scale Unit and to Modern POS offline databases. Transactional data flows upward from the Modern POS offline databases to Commerce Scale Unit and to Commerce headquarters.
...
- On the **Scheduler subjob**, set the option to **Yes** to stop the associated table for any job that contain the subjob from being synced to offline databases. For example, the DIRPARTYTABLE table is used by three jobs. Therefore, if you exclude this table, you stop its data from being synced to offline databases for all three jobs. (We don't recommend that you exclude this table \[Subjob\]).
