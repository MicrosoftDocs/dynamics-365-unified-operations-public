...
| Offline database compression | In a focused effort to reduce offline database sizes, this feature (available in release 10.0.29 and later) enables automated index compression outside of a Channel (Store) [Store hours](store-hours.md). If, for any reason, store hours are not configured or working properly, the compression will occur at all times. When compression does occur (Based on store hours or not), the POS (Modern POS or Store Commerce) will query the offline database for the non-compressed and non-clustered indexes which are either greater than 100MB in size or at least 1% of the total database size. If any indexes are found meeting these criteria, compression will start for the largest index in the list and then go idle for 10 minutes. After this time, the next index in the list will begin compression and this cycle will repeat until all chosen indexes have been compressed. If no indexes are found, then the compression logic will pause for 30 minutes before checking again. For this feature to function correctly, the **POS offline database compression** feature must be turned on in the **Feature management** workspace followed by running the 1070 distribution schedule job. For more information, see [Feature management overview](../../fin-ops-core/fin-ops/get-started/feature-management/feature-management-overview.md). |
