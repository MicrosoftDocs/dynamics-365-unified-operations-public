...
The following configurations have been released but cause changes to logic that may not be useful for all usage scenarios. These are features that have been tested but have not been thoroughly validated in all scenarios. Maturity has been listed to provide details on confidence of functionality. 

The following will change from month to month, so it is valuable to check back regarding the maturity of a particular feature and whether any new features have been added. To apply any of the following, navigate to **Retail and Commerce > Headquarters setup > Parameters > Commerce parameters**.  From the left-hand menu, select **Configuration parameters**.  In the sub-page that appears, enter the key below into the **Name** field and the default value listed in the description into the **Value** field.

| Feature | Key | Description | Maturity |
|------------------|---------------------|------------------------------|-----------------------------------|
| Delayed download session creation | CDX_ENABLE_DELAYED_OFFLINE_DOWNLOAD_SESSION_CREATION | This key delays the download sessions from being fully created until after the Modern POS device the download sessions are for is fully activated. The default value should be **0** (Zero). | High (Released 10.0.15, well understood, tested thoroughly, in use) |
| Package order enforcement | CDX_ENABLE_DOWNLOAD_SESSION_DEPENDENCY_ENFORCEMENT | This key forces download session application to apply in order.  If session application fails (Which is based on the **Try count times** value, which is by default a value of three), the session will be marked as **Suspended** and session applications will not proceed until the suspended session is retried or cancelled. Using this key, rerunning previously applied sessions (Sessions that are not in state **Available** or in a **Suspended** state) cannot be done. The default value should be **0** (Zero). | Moderate (Released 10.0.18, well understood, tested thoroughly, in use) |
| Rollback on failure | CDX_ENABLE_ROLLBACK_ON_FAILURE | When synchronizing uploaded data from offline databases (Based on the P-job distribution schedule), the system attempts to merge in transactions. If duplicate transaction IDs are found and could cause an error, this feature prevents overwriting the duplicated transaction IDs. The default value should be **1** (One). | High (Released 10.0.13, well understood, tested thoroughly, in use) |
