...
The **Parameters** section of the **Archive credit card transaction data** flyout menu contains the following parameters:
  ...
  - **Use compression** - When this parameter is set to **Yes**, Commerce compresses tokens to save on storage space. For more information, see [Further storage management with token compression](#further-storage-management-with-token-compression) below. This option is available for versions 10.0.31 and higher.
...
Once the **Compress payment tokens** feature is enabled, the **Use compression** property in the **Archive credit card transaction data** parameters section can be set to **Yes** (**Use compression** is available in versions 10.0.31 and higher).
