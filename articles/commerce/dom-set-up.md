...
> [!IMPORTANT]
> - Bing Maps for Enterprise is deprecated and will be retired. Customers with an enterprise license can continue to use Bing Maps for Enterprise until **June 30, 2028**, and customers on the free and basic license for Bing Maps for Enterprise can continue to use Bing Maps for Enterprise until **June 30, 2025**.
...
1. Set the **Enable Azure Maps** option to **Yes**.
1. Enter a valid **Azure Maps key** value, and then select **Save**.
...
For information on how to obtain a key and privacy notice, see [Manage Azure Maps for your organization](dev-itpro/manage-azure-maps.md).
 ...
 > [!NOTE]
 > If you're using Bing Maps, in the left navigation pane, select **Bing Maps**. Set the **Enable Bing Maps** option to **Yes**, and then under **Bing Maps key**, enter a valid key. For more information, see [Manage Bing Maps for your organization](dev-itpro/manage-bing-maps.md).
...
1. On the **General** tab, set the following values:
    ...
    - **Disable road distance calculation** - If this option is **Yes**, aerial distance is calculated for latitude and longitude values of the warehouse and the customer address. Set this option to **No** if you want to use either the Azure Maps API or Bing Maps API to calculate road distance. In this case, either the **Confirm Azure Maps usage for DOM** option or the **Confirm Bing Maps usage for DOM** option is required to be set to **Yes**.
        - **Don't process accepted store orders during order optimization** - Set this option to **Yes** if you don't want DOM to process sales orders that retail stores accept.
    ...
    - **Fulfillment data retention period (in days)** – Specify how long the fulfillment plans that DOM runs generate are kept in the system. The **DOM fulfillment data deletion job setup** batch deletes any fulfillment plan that's older than the number of days that you specify here.
...
1. On the **Solver** tab, set the following values:
    ...
    - **Max auto-fulfillment attempts** – Specify how many times the DOM engine attempts to broker an order line to a location. If the DOM engine can't broker an order line to a location in the specified number of attempts, it flags the order line as an exception. It then skips that line in future runs until the status is manually reset.
    ...
    - **Solver type** – Select a value. Two solver types are released with Commerce: **Production Solver** and **Simplified Solver**. For all machines that run DOM (that is, all servers that are part of the DOMBatch group), **Production Solver** must be selected. The Production Solver requires the special license key that, by default, is licensed and deployed in production environments. In newer Tier 2+ environments, the Production Solver is already enabled.
      ...
      For nonproduction environments, manually deploy this license key. Due to the limitation of nonproduction environments, you must contact Microsoft support to get the latest **DOM license** file. After you obtain the license file, follow these steps:

        1. Start Microsoft Internet Information Services (IIS) Manager, right-click **AOSService website**, and then select **Explore**. A Windows Explorer window opens at **\<AOS service root\>\\webroot**. Make a note of the \<AOS Service root\> path, because you'll use it in the next step.
...
1. On the **Number sequences** tab, assign the required number sequences to the various DOM entities.
    ...
    > [!NOTE]
    > Before you can assign number sequences to the entities, define them on the **Number sequences** page (**Organization administration > Number sequences > Number sequences**).
## Configure fulfillment groups

The DOM feature supports the definition of various types of DOM rules. Organizations can configure multiple rules, depending on their business needs. Define DOM rules for a group of locations or individual locations, and for a specific product category, product, or variant. To create the grouping of locations that you must use for the DOM rules, follow these steps:
...
> [!NOTE]
> - In Commerce version 10.0.12 and higher, enable the **Ability to specify locations as 'Shipping' or 'Pickup' enabled within Fulfillment group** feature in the **Feature Management** workspace.
...
Use fulfillment profiles to group a collection of rules, legal entities, sales order origins, and modes of delivery. Every DOM run is for a specific fulfillment profile. Organizations can define and run rules for a set of legal entities on orders that have specific sales order origins and modes of delivery. If different sets of rules must run for different sets of sales order origins or modes of delivery, define the fulfillment profiles accordingly.
...
1. Set the **Auto apply result** option. If you set this option to **Yes**, the results of the DOM run for the profile automatically apply to the sales order lines. If you set it to **No**, you can only view the results in the fulfillment plan, and they aren't applied to the sales order lines.
1. If you want the DOM profile to run for orders that have every sales order origin, including orders where the sales order origin is undefined, set the **Process orders with empty sales origin** option to **Yes**. To run the profile for only a few sales order origins, define them on the **Sales origins** page.
1. If you want to change how DOM breaks sales lines into different batches, set a value for **Maximum number of order lines per optimization**. For more information, see [Partition sales lines](dom-processing.md#partition-sales-lines).

    > [!NOTE]
    > - In Commerce version 10.0.12 and later, the **Ability to assign Fulfillment group to a Fulfillment Profile** feature must be enabled in the **Feature Management** workspace. This feature lets you specify a list of warehouses that DOM should consider when optimization runs with a fulfillment profile. If you don't specify this list of warehouses, DOM checks all warehouses on legal entities that the profile defines.
...
1. Repeat the previous two steps until you associate all the required rules with the profile.
1. Select **Save**.
...
1. In the **Mode of delivery** field, select the mode of delivery to associate with this profile. You can't associate a mode of delivery with multiple active profiles.
1. Repeat the previous two steps until you associate all the required modes of delivery with the profile.
...
1. In the **Company** field, select the legal entity. The list of companies is limited to the legal entities that you added earlier.
1. In the **Sales origin** field, select the sales origin to associate with this profile. You can't associate a sales origin with multiple active profiles.
1. Repeat the previous two steps until you associate all the required sales origins with the profile.
