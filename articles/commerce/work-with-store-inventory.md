...
When you work with inventory in Microsoft Dynamics 365 Commerce and use any of the Commerce applications that are connected to a Commerce Scale Unit(CSU),it's important that you be aware that the order processing logic in CSU provides limited support for some inventory dimensions and some inventory item types. Commerce applications do not support the full range of item configuration capabilities that is available through the item configuration options in Dynamics 365 Supply Chain Management.

The Commerce applications running on CSU do not support the following product dimensions and item configurations:
...
- **Batch ID** - The POS application provides limited support during statement posting if a batch-controlled items is sold, but users in POS are not able to define the actual batch ID that was sold or picked when using the POS application.
...
>
> If items are configured in a way that the Commerce applications do not support, and appropriate testing isn't done, data failures that aren't easily corrected or may not be able to be corrected at all, may occur.
