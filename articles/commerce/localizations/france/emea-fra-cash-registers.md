...
This version of the cash register functionality for France passed an audit according to the NF 525 certification requirements and is granted a certificate of compliance that has the following category and number:

- **Microsoft Dynamics 365 Commerce, version 10**:

  - Certificate category: B
  - Certificate number: 525/0498-6

An up-to-date certificate is available on the [portal of the certification body](https://certificates.infocert.org/).
...
> [!NOTE]
>
> - To comply with NF 525 certification requirements for updating software versions, Microsoft uses a separate NF 525 specific version. The current NF 525 certified version is Dynamics 365 Commerce version 10.1, which maps to the global major Dynamics 365 Commerce version 10.0. The specific service update version is Dynamics 365 Commerce version 10.0.48, which corresponds to Store Commerce POS build 9.58.x. This certification covers all the supported Store Commerce platforms: Windows, Cloud, Android, and iOS. For more information, see [NF 525 compliance documentation](#nf-525-compliance-documentation).
> - Microsoft ensures the authenticity and security of the Dynamics 365 Commerce software through a rigorous code signing process. Each component is digitally signed with a Microsoft certificate, which guarantees that the software is authored by Microsoft and isn't tampered with. This secure signing process provides customers with confidence in the integrity and origin of the software they use.
> - Microsoft maintains source code for all supported versions of Dynamics 365 Commerce in a secure version control system that is in accordance with the [Microsoft Secure Development Lifecycle](https://www.microsoft.com/securityengineering/sdl). If you need more details about a specific version of Dynamics 365 Commerce, open a support request and a Microsoft engineer can work with you to provide the required information.

### NF 525 compliance documentation

The following table shows the Dynamics 365 Commerce documentation that's related to the NF 525 certification.
...
| Auditor documentation | This documentation describes how to access the data, the files, and the process that the authorities use. | [Fiscal archive for France](emea-fra-fiscal-archive.md) |
...
:::image type="content" source="../media/emea-fra-fiscal-solution.png" alt-text="Screenshot of the high-level design of the fiscal solution for France." lightbox="../media/emea-fra-fiscal-solution.png":::

The high-level, end-to-end process flow for France is as follows:

1. When the checkout process is completed for a sales transaction in POS, POS sends a digital signing request to the Commerce runtime (CRT) via Commerce Scale Unit (CSU). Digital signing of transactions and audit events is implemented by using the [Fiscal registration framework](../dev-itpro/fiscal-integration-for-retail-channel.md) and an [internal](../dev-itpro/fiscal-integration-for-retail-channel.md#fiscal-registration-is-done-internally-in-the-crt) connector.
...
1. When any of the audit events that must be digitally signed occur, the preceding steps, except for the receipt, are repeated for the audit event. The signature of the audit event is saved in the channel DB in a fiscal transaction that is linked to the audit event.
...
If you set the **Audit** option in the POS functionality profile to **Yes**, the POS audit event log registers the following events:
...
> [!NOTE]
> Audit event logs only apply to changes you make in the POS. The logs don't include changes made in Commerce headquarters.

#### Events logged separately in the fiscal archive

The POS audit event log doesn't register the following events. However, the fiscal archive logs them separately with their own signature sequences.

| Event | NF 525 Jet Code | How it's logged |
| ----- | --------------- | --------------- |
| Printing a copy of a receipt | 155 | Logged as part of receipt copies. For more information, see [Digital signing of receipt copies](#digital-signing-of-receipt-copies). |
| Returns | 326 | Logged as regular sales transactions. For more information, see [Digital signing of sales and return transactions](#digital-signing-of-sales-and-return-transactions). |
| Closing a shift | 50 | Logged as part of the shift closing process. For more information, see [Digital signing of closed shifts](#digital-signing-of-closed-shifts). |
| Period tax archiving | 20 | Logged as part of grand totals. For more information, see [Period grand total journal](#period-grand-total-journal). |
| Fiscal year archiving | 30 | Logged as part of grand totals. For more information, see [Period grand total journal](#period-grand-total-journal). |
| Annual or year-end closing | 60 | Logged as part of grand totals. For more information, see [Period grand total journal](#period-grand-total-journal). |

### Digital signing overview

POS digitally signs the following types of records, including transactions and events:

- Sales and return transactions
- Copies of receipts
- Closed shifts and Z reports
- Audit events

The channel database creates and records the signature when you finalize the transaction or register the event. The signed data is a text string made up of several data fields, which vary depending on the type of record. The general signing process includes the following steps:

1. Select the next sequential number for signing, for the same register and type of record.
1. Extract the data fields that you must sign from the record.
...
1. Transform the resulting string using base64url.
1. Store the string used for signing, the sequential number, the signature, the hash algorithm identification, and the thumbprint of the certificate in a fiscal response record that's linked to the transaction or event.
1. Transfer the fiscal response to the enterprise resource planning (ERP) system in Commerce headquarters, along with the transaction or event.
...
When you print a receipt copy, the POS audit event log registers the event. Only receipt copies for signed sales transactions are signed. The signed data for a receipt copy event is a text string that consists of the following data fields:

- The register number that the receipt copy prints from.
...
- A value (**Y** or **N**) that indicates whether the transaction is the first signed receipt copy event for the register.
...
When you close a shift, the POS audit event log registers the event. The data that the system signs for a shift closing event is a text string that consists of the following data fields:
...
> [!NOTE]
> Because of the requirement to maintain a chain of closed shifts for the same register, the system doesn't support closing a shift on a register that's different from the one that the shift was opened on. This feature might be added in later updates.
...
Only the [France-specific](#registration-of-audit-events) audit events are signed. Each register maintains three separate sequences of digitally signed audit events:
...
For an event other than a receipt copy or shift closing event, the signed data is a text string that consists of the following data fields:
...
Receipts for France can include extra information through custom fields:

- **Transaction type**: Add a field to a receipt format layout to identify the type of transaction. For example, a sales receipt should include the text "Sales."
- **Sequential number of signed sales transaction**: Include the sequential number of a signed sales transaction on the receipt. Use this number to link the printed receipt with a digital signature in the database.
- **Extract from digital signature**: Include an extract from the digital signature on the receipt. Use this extract to confirm that the transaction is signed. It consists of a concatenation of the 3rd, 7th, 13th, and 19th symbols of the signature.
- Information about a receipt copy:

  - **Reprint message**: Include a "Copy" caption on a receipt copy.
...
- **Line count**: Include the number of printed item lines on the receipt.
- **Sales totals**: Custom fields for receipt totals exclude nonsales amounts from the total transaction amounts. Nonsales amounts include amounts for the following operations:
...
- **Certification data**: Include the category and number of a certificate of compliance that an authorized body issued per the NF 525 certification requirements.
- **Software version**: Include the version of the software that was certified per the NF 525 certification requirements and that is used to produce receipts.

### Restricting the duration of shifts

You can enforce daily shift closing in POS. A shift can't last longer than the time that you specify in the **Shift closing time** field. POS uses the time of the machine that it's installed on. Several minutes before that time, the operator starts to receive warnings that the shift must close. The value of the **Shift closing interval (minutes)** field determines the number of minutes. If the operator closes and restarts the shift within the warning interval, selling can continue after the shift closing time. Otherwise, the operator can't complete the sales transaction. The operator must either void or suspend the transaction, close the shift, open a new shift, and then recall and complete the transaction.

### X and Z reports

The information included on X and Z reports is based on French requirements:
...
Period grand total journals summarize sales totals per store and fiscal period (for example, per month). Use them as part of the fiscal period closing procedure. Also, an annual journal summarizes sales totals per store and fiscal year. Use it as part of the year closing procedure.

Before you create a period grand total journal, ensure that retail sales data for the store and the fiscal period is uploaded to Commerce headquarters and processed in retail statements.

The **Period grand total journal** page maintains the period grand total journals.

1. To create a new journal, specify a store. If previous journals exist for the store, the next fiscal period after the last closed journal for the store is automatically used as the new journal period. If previous journals don't exist, specify the end date of the journal. In this case, the fiscal period that includes the specified date is used as the journal period.
1. Calculate the journal. Select shifts that you closed during the journal period, and calculate totals for those shifts. You can view the journal's tax totals per sales tax code and the shifts that are included in the journal. You can also select **Functions** > **Clear journal** to clear the calculation results and return the journal to **New** status.
1. After you calculate the journal, close it. You can't modify a closed journal, and you can't create another journal for a previous fiscal period, the same period, or an intersecting period. However, you can cancel the last closed journal for a store. In that case, you can create another journal for the same store and fiscal period.
...
You must create and close period grand total journals for all stores every fiscal period. You can automate journal maintenance by using the following two periodic procedures that can run in batch mode:

- **Create period grand total journals**: This procedure creates and optionally calculates journals for selected stores for a given period. You must select a specific period for the procedure. Therefore, you must manually create a batch job for each period and can't use recurrence. The procedure doesn't close the journals, because it's expected that each journal is manually validated before it's closed.
...
You can also mark a period grand total journal as **Annual** when you create it. An annual journal summarizes period grand total journals for the fiscal periods of a fiscal year. You can create an annual journal for a fiscal year only if a journal for the last fiscal period of the fiscal year is created, calculated, and closed. However, journals don't have to exist for *all* fiscal periods of the fiscal year. For example, if you open a new store in the middle of the year, the first journal corresponds to the fiscal period that the store is opened during. In this case, the first annual journal summarizes journals for fiscal periods from the fiscal period that the store is opened during to the last fiscal period of the fiscal year.

### Fiscal archive

A fiscal archive is an XML file that you can export from a period grand total journal that you close. It includes the totals for the closed period. It also includes detailed data about sales transactions and events. The exported file is digitally signed, and the signature is contained in a separate file. You must keep exported fiscal archives on secured external media for the legal retention period.

Commerce also includes a tool that you can use to verify the integrity of a fiscal archive, and to detect violations of the signature of the archive and of the chains of signed records in the archive. This tool takes the form of a Windows PowerShell script that's available via the Commerce software development kit (SDK). For detailed information about the structure of the fiscal archive format and guidance about how to use the Fiscal archive integrity verification tool, see [Fiscal archive for France](emea-fra-fiscal-archive.md).
...
To use the France-specific functionality, complete these tasks:
...
You must also specify the following settings for France. Run appropriate distribution jobs after you complete the setup.

1. [Enable Commerce features](#enable-features-for-france) for France in the **Feature management** workspace.
1. [Specify various registration numbers](#set-up-the-legal-entity) of the organization, such as the VAT identifier, on the **Legal entities** page. These registration numbers are used when you export the fiscal archive.
...
On the **Legal entities** page, make the following changes. These settings are used in the archive format.

- On the **Bank account information** FastTab, in the **Routing number** field, enter the VAT identifier of the organization.
- On the **Registration numbers** FastTab, in the **NAF code** field, enter the **Nomenclature des Activités Françaises (NAF)** code of the organization.
- On the **Tax registration** FastTab, in the **Tax registration number** field, enter the **Système d'identification du répertoire des établissements (SIRET)** number of the organization.

### Set up VAT per French requirements

Create sales tax codes, sales tax groups, and item sales tax groups. Set up sales tax information for products and services. For more information about how to set up and use sales tax, see [Sales tax overview](/dynamics365/finance/general-ledger/indirect-taxes-overview).

For stores that are located in France, specify sales tax groups and enable the **Prices include sales tax** option.

### Set up POS functionality profiles

To enable printing of Z-reports, set the **Print X/Z report on POS** option to **Yes**. To enable auditing, set the **Audit** option to **Yes**.

To enforce daily shift closing, make the following changes:

- Set the **Enforce daily shift closing** option to **Yes**.
- Set the **Shift closing time** and **Shift closing interval (minutes)** fields.

 > [!NOTE]
 > In France, the daily closing period can't exceed 24 hours unless the daily closing happens before the start of the next daily period. Also, you can't open a new shift until the previous shift is closed.

### Configure custom fields for receipt formats

You can configure the language text and custom fields used in the POS receipt formats. The default company of the user who creates the receipt setup should be the same legal entity where you create the language text setup. Alternatively, create the same language texts in both the user's default company and the legal entity of the store.

On the **Language text** page, add the following records for the labels of the custom fields for receipt layouts. The **Language ID**, **Text ID**, and **Text** values shown in the table are examples. You can change them to meet your requirements. However, the **Text ID** values must be unique and be 900001 or higher.
...
NF 525 compliance requires that receipts aren't printed by default and are only printed by customer request. To comply with this requirement, change the value of the **Print behavior** field to **Prompt User** for every receipt format. You must also configure hardware profiles to support receipt printers and to enable Hardware station. For more information about how to work with POS peripherals, see [Peripherals](../../retail-peripherals-overview.md).
...
- **Lines:** Add the following standard fields:
...
- **Footer:** Add the following fields:
  ...
  - **Tax ID**: This standard field enables a sales tax summary to be printed per sales tax code. Add this field to a new line.
  - **Sales tax basis**: This field prints the receipt's tax basis for cash sales per sales tax code. Prepayments and gift card operations are excluded. Add this field to the same line as the **Tax ID** field.
  - **Sales tax amount**: This field prints the receipt's tax amount for cash sales per sales tax code. Prepayments and gift card operations are excluded. Add this field to the same line as the **Tax ID** field.
  ...
  - **Software Version**: This field prints the version of the software used to produce receipts that were certified per the NF 525 certification requirements (for example, **Microsoft Dynamics 365 Commerce v.10**).
...
1. Download configuration files for the fiscal document provider and the fiscal connector from the Commerce SDK:
    ...
    1. Open **src** > **FiscalIntegration** > **SequentialSignatureFrance** > **Configurations**.
    1. Download the fiscal connector configuration file at **Connector** > **ConnectorMicrosoftSequentialSignatureFRA.xml**.
    1. Download the fiscal document provider configuration file at **DocumentProvider** > **DocumentProviderMicrosoftSequentialSignatureFRA.xml**.
...
You must configure certificates for digital signing of records on the Commerce channel side (sales transactions and audit events) and on the Commerce headquarters side (period grand total journals, Z reports, and fiscal archives). Use a digital certificate stored in Azure Key Vault for signing. For the offline mode of the Store Commerce app, you can also use a digital certificate stored in the local storage of the machine where the Store Commerce app is installed. Use the [User-defined certificate profiles for retail stores](../global/certificate-profiles-for-retail-stores.md) feature to configure certificates stored in Key Vault. It also supports failover to offline mode when Key Vault or Commerce headquarters isn't available. This feature extends the [Manage secrets for retail channels](../../dev-itpro/manage-secrets.md) feature.

> [!NOTE]
> For digital signing, use either a digital certificate issued by an accredited body or a self-signed certificate. Only certificates with RSA-2048-bit or Elliptic Curve Digital Signature Algorithm (ECDSA) 224-bit minimum private keys are acceptable. Commerce supports only RSA-2048-bit or longer keys. If you want to use an ECDSA key, you must implement a customization.

To configure certificates and certificate profiles for digital signing, follow the steps in [Set up certificate profiles](../global/certificate-profiles-for-retail-stores.md#set-up-certificate-profiles).
...
- **Hash algorithm**: Specify one of the cryptographic hash algorithms that Microsoft .NET supports (for example, **SHA256**).
...
Finally, on the **Commerce parameters** page (**Retail and Commerce** > **Headquarters setup** > **Parameters** > **Commerce parameters**), specify the following parameters for digital signing on the Commerce headquarters side:

- **Certificate**: Select a certificate that's stored in Key Vault.
- **Hash function**: Specify one of the cryptographic hash algorithms that Microsoft .NET supports, such as **SHA256**.
- **Encoding**: Specify the encoding of the signed data. Only **UTF-8** is supported.
- **Number sequence**: You can also specify a number sequence to use for automatic numbering of period grand total journals. On the **Number sequences** tab, select a record where the **Reference** field is set to **Period grand total journal**, and then select a number sequence code in it.

> [!NOTE]
> The following hash functions aren't supported: CRC16, CRC32, SHA-1, and MD5. Commerce supports only the SHA256, SHA384, and SHA512 hash functions. If you want to use a different hash function, you must implement a customization.

### Configure the Z report and archive export formats

Depending on your needs, download the ER configurations for the Z report and archive from the following sources:

- If you don't need to customize the ER configurations that Microsoft provides or create your own ER configurations, import the Microsoft-provided configurations from Microsoft Dynamics Lifecycle Services. For more information, see [Import a configuration from Lifecycle Services](../../../fin-ops-core/dev-itpro/analytics/download-electronic-reporting-configuration-lcs.md). Alternatively, you can [download ER configurations from the Global repository of Configuration service](../../../fin-ops-core/dev-itpro/analytics/er-download-configurations-global-repo.md).
- If you need to customize the ER configurations that Microsoft provides or create your own ER configurations, provision a Regulatory Configuration Services (RCS) environment. For more information about how to work with RCS, see [Import ER configurations from RCS](../../../fin-ops-core/dev-itpro/analytics/rcs-download-configurations.md).

Download the following versions (or later versions) of the configurations:
...
> [!NOTE]
> You only need to complete the steps in this section if you're updating an existing environment.

To enable audit events, reinitialize the Commerce extensible enumerations. To enable France-specific data to be transmitted from POS to Commerce headquarters, reinitialize the Commerce scheduler.
...
To enable France-specific functionality, configure extensions for channel components. For more information, see the [deployment guidelines](emea-fra-fi-deployment.md).
...
1. Sign out of POS.
1. Wait for the offline database to fully synchronize.
1. Sign in to POS.
...
1. [Nonsale transaction (expense)](#8-nonsale-transaction-expense)
...
1. In Commerce headquarters, follow these steps:
    ...
    1. On the **Fiscal transactions** FastTab, verify that there's a fiscal transaction that has a successful registration status.
    ...
    1. Verify that the signed transaction sequential number equals the sequential number of the previous signed sales transaction on the same register (if there's any previous signed sales transaction) plus 1. This sequential number should also be printed in the **Sequential number** field of the receipt.
    ...
    1. Verify that the **Digital signature** field of the receipt contains an extract from the digital signature of the transaction, and that it consists of a concatenation of the 3rd, 7th, 13th, and 19th symbols of the signature.
...
1. In Commerce headquarters, follow these steps:
    ...
    1. On the **Fiscal registration results** FastTab, verify that there's a fiscal transaction that has a successful registration status.
    ...
    1. Verify that the signed shift sequential number equals to the sequential number of the previous signed closed shift on the same register (if there's any previous signed closed shift) plus 1.
...
1. Open the **Audit events** page, and select an audit event. Use the **Store number**, **Register number**, **Event type**, **Date**, and **Time** fields to find the event. The **Log string** field contains the description of the event.
1. On the **Fiscal registration results** FastTab, verify that there's a fiscal transaction that has a successful registration status.
1. Verify that the text in the **Fiscal register response** field is in JSON format and contains the following information:
...
1. Verify that the signed event sequential number equals the sequential number of the previous signed event of the same event sequence type on the same register (if there's any previous signed event) plus 1.
...
1. Pay the exact amount.
1. [Validate the transaction](#validate-a-sales-transaction). The transaction isn't digitally signed. No signature is printed on the receipt.
...
1. Pay the exact amount.
1. [Validate the transaction](#validate-a-sales-transaction). The gift card amount is excluded from the total amount of the sales transaction, both on the receipt and in the transaction data that was used to digitally sign it.
...
1. [Validate the transaction](#validate-a-sales-transaction). Validate that the receipt contains a digital signature, and that the total amount of the sales transaction includes only the carryout item.
1. In POS, select **Recall order**, and find the previously created order.
...
1. [Validate the transaction](#validate-a-sales-transaction). The transaction is digitally signed, and the sequence of digital signatures isn't broken.

### 8. Nonsale transaction (expense)
...
1. [Validate the receipt copy audit event](#validate-an-audit-event). The audit event has the **Receipt copy printed** type. Verify that the **Reprint digital signature** field of the receipt copy contains an extract from the digital signature of the audit event, and that it consists of a concatenation of the 3rd, 7th, 13th, and 19th symbols of the signature.
...
1. In Commerce headquarters, follow these steps:
    ...
    1. Ensure that the **Enforce daily shift closing** option is set to **Yes**.
    1. Set the **Shift closing time** field to a time that's 20 minutes after the current time.
...
1. In POS, follow these steps:
    ...
    1. On the **Current transaction** page, add several items, and pay the exact amount. You shouldn't receive any error or warning.
    ...
    1. Try to pay the exact amount. You should receive the following error: "Shift duration exceeds the limit. Suspend the transaction, close the shift and open a new shift." The payment shouldn't be successful.
    1. Suspend the transaction.
    1. Close the shift.
    1. Recall the transaction.
    1. Pay the exact amount. You shouldn't receive any error or warning.
...
1. If the journal is the first journal for the selected store, select the **To date**. You can select any date in a fiscal period, and the **From date** and **To date** fields automatically set to the first and the last day of the fiscal period that the selected date belongs to.

    If the journal isn't the first journal for the store, the **From date** and the **To date** fields automatically set to the first and the last day of the fiscal period that's next to the latest fiscal period that a closed journal exists for.
...
1. Select **Archive** > **Export archive**, specify the name of the export file, and confirm the operation. You must also disable the pop-up blocker of the browser, so that the export can be completed.
