...
> [!NOTE]
> In Commerce 10.0.8 and later, Retail Server is known as Commerce Scale Unit. Because this article applies to multiple previous versions of the app, *Retail Server* is used throughout the article.

## Storing a certificate for digital signing in Azure Key Vault

The digital signature extension uses a certificate that you install in the local certificate storage of the machine where Retail Server is deployed. You must specify the thumbprint of the certificate in the configuration file (see the [SequentialSignatureRegister component](#sequentialsignatureregister-component) section later in this article). Depending on the implementation topology, you might have to store the certificate in [Microsoft Azure Key Vault storage](/azure/key-vault/key-vault-get-started). The localization for France contains a code sample that shows how to override the signing flow and sign sales transactions by using a certificate that is stored in Azure Key Vault storage.

### Prerequisites

Complete the following steps before you can use a certificate that is stored in Azure Key Vault storage:

- Create the Azure Key Vault storage. Deploy the storage in the same geographical region as the Retail Server.
- Upload the certificate to the storage.
- Authorize the Retail Server application to read secrets from the storage.
...
The **DigitalSignatureKeyVaultSample** project contains sample code that uses a certificate that you store in Azure Key Vault. To use the sample in a production environment, you must implement logic so that you can specify the following parameters in the **HashAndSignData** method of the **CertificateSignatureServiceRequestHandler** class:
...
- **Client ID** – An interactive client ID of the Microsoft Entra application that you associate with the Azure Key Vault for authentication purposes. This client needs access to read secrets from the Azure Key Vault.

    ``` csharp
    settings.Add(WellKnownKeyVaultSettings.KeyVaultInteractiveClientId, "Set the client ID here");
    ```

- **Client secret** – A secret key that you associate with the Microsoft Entra application for authentication in the Azure Key Vault.
...
1. Update the **commerceRuntime.ext.config** file by adding the following line to the **composition** section.
...
> [!NOTE]
> The thumbprint of the certificate that you use for digital signing should be specified in the configuration file of the SequentialSignatureRegister assembly, even if the certificate is stored in Azure Key Vault. For more information, see the [SequentialSignatureRegister component](#sequentialsignatureregister-component) section later in this article.

### Using certificate profiles in Commerce channels

In Commerce version 10.0.15 and later, use the [User-defined certificate profiles for retail stores](../global/certificate-profiles-for-retail-stores.md) feature that supports failover to offline when Key Vault or Commerce headquarters aren't available. This feature extends the [Manage secrets for retail channels](../../dev-itpro/manage-secrets.md) feature.
...
1. To read a secret, call `GetUserDefinedSecretCertificateServiceRequest` by using a constructor with `profileId` parameter. This step starts the functionality working with settings from Certificate profiles. Based on the settings, the certificate is retrieved either from Azure Key Vault or local machine storage.
...
Use custom fields to print the following application attributes on receipts<!-- (for more information, see [Cash registers for France](emea-fra-cash-registers.md))-->:

- **Build number** – The software version of the POS application. By default, the value equals the POS build number that Microsoft assigns to the POS application.
- **Certificate category** and **Certificate number** – The category and number of the certificate of compliance that an accredited body issues for the application. By default, the values equal the category and the number of the certificate that is granted to Microsoft:
    ...
    > [!NOTE]
    > The assigned certificate category and number print by default. If you're implementing Commerce, override the certificate category and number.

If you customize the POS application, and your customizations affect the compliance of the application, you might have to request a new certificate of compliance from an accredited body. In this case, override the build number, certificate category, and number. Otherwise, the default values for the certificate category and number print, but you must still specify the POS build number that Microsoft assigned to the POS application.

### Overriding the build number

Specify the software version/build number and publisher in **RetailSDK\\BuildTools\\Customization.settings**.
...
Specify the certificate category and number in **RetailSDK\\SampleExtensions\\CommerceRuntime\\Extensions.ReceiptsFrance\\GetSalesTransactionCustomReceiptFieldService**.
...
> [!NOTE]
> You must also override the certificate category and number if you're implementing Commerce. Use the certificate category and number that the [Specifying application attributes that will be printed on receipts](#specifying-application-attributes-that-will-be-printed-on-receipts) section provides earlier in this article.
...
1. In the **Extensions.CommonFrance\\bin\\Debug** folder, find the **Contoso.Commerce.Runtime.CommonFrance.dll** assembly file.
...
1. Find the extension configuration file for CRT:
    ...
    - **Retail Server:** The file is named **commerceruntime.ext.config**, and you can find it in the **bin\\ext** folder under the IIS Retail Server site location.
...
1. In the **Extensions.ReceiptsFrance\\bin\\Debug** folder, find the **Contoso.Commerce.Runtime.ReceiptsFrance.dll** assembly file.
1. Copy the assembly file to the CRT extensions folder:
...
1. In the **Extensions.SalesPaymentTransExtFrance\\bin\\Debug** folder, find the **Contoso.Commerce.Runtime.SalesPaymentTransExtFrance.dll** assembly file.
1. Copy the assembly file to the CRT extensions folder:
...
1. In the **Extensions.SequentialSignatureFrance\\bin\\Debug** folder, find the **Contoso.Commerce.Runtime.SequentialSignatureFrance.dll** assembly file.
1. Copy the assembly file to the CRT extensions folder:
...
1. Modify the **App.config** file by specifying the thumbprint, store location, and store name for the certificate that should be used to sign sales transactions. The **certificateThumbprint** property is the only mandatory property. The value must be a string that is 40 characters long in uppercase and that doesn't include any delimiters. For more information, see [How to retrieve the thumbprint of a certificate](/dotnet/framework/wcf/feature-details/how-to-retrieve-the-thumbprint-of-a-certificate).
...
1. Copy the files to the CRT extension folder:
...
1. In the **Extensions.XZReportsFrance\\bin\\Debug** folder, find the **Contoso.Commerce.Runtime.XZReportsFrance.dll** assembly file.
1. Copy the assembly file to the CRT extensions folder:
...
1. In the **Extensions.RestrictingShiftDuration\\bin\\Debug** folder, find the **Contoso.Commerce.Runtime.RestrictingShiftDuration.dll** assembly file.
1. Copy the assembly file to the CRT extensions folder:
...
1. In the **RetailServer\\Extensions.SalesTransactionSignatureSample\\bin\\Debug** folder, find the **Contoso.RetailServer.SalesTransactionSignatureSample.dll** assembly file.
1. Copy the assembly file to the **\\bin\\ext** folder under the IIS Retail Server site location.
...
Complete the following procedure to enable the extensions in offline mode for Modern POS.
...
1. Register the proxy change in the extensions configuration file. The file is named **RetailProxy.MPOSOffline.ext.config**, and it's under the local CRT client broker location.
...
1. Include the following existing source code folders in the **Pos.Extensions** project.
    ...
    > [!NOTE]
    > To view all files in the project folder, not just the files that the project includes, select the **Show All Files** button in Solution Explorer. If this button isn't available, make sure that you selected the project. The icons of files and folders that aren't currently part of the project have a dotted outline. Right-click the folder to include in the project, and then select **Include in Project**.

1. Enable the extensions to compile by removing the following folders from the exclude list in **tsconfig.json**:
...
1. Enable the extensions to load by adding the following lines in **extensions.json**:
...
1. Make the following changes in the package configuration files under the **RetailSdk\\Assets** folder:
    ...
    1. In the **commerceruntime.ext.config** and **CommerceRuntime.MPOSOffline.Ext.config** configuration files, add the following lines to the **composition** section:
        ...
        To use a certificate that's stored in Azure Key Vault storage for digital signing, add the following line.
    ...
    1. In the **RetailProxy.MPOSOffline.ext.config** configuration file, add the following lines to the **composition** section.
