...
1. Find and enable the following features:
    ...
    - **Direct fiscal integration from POS registers** – This feature extends the fiscal integration framework by adding the capability to create fiscal connectors that run in point of sale (POS). This type of connector communicates with a fiscal device or service that provides an HTTP application programming interface (API) and doesn't require a dedicated physical machine in the store. For example, this functionality enables fiscal integration for mobile devices without requiring shared hardware station.
    ...
    - **Fiscal Registration State of POS Registers** – When you enable this feature, you can disable the fiscal registration process for specific POS registers. If fiscal registration is disabled for a POS register, sales transactions can't be completed on that register.
...
1. On **Commerce shared parameters**, on the **General** tab, set the **Enable fiscal integration** option to **Yes**.
...
1. On **Commerce parameters**, define the number sequence for the fiscal functional profile number.

> [!NOTE]
> Number sequences are optional. You can generate numbers for all fiscal integration entities either from number sequences or manually.

## Set up a fiscal registration process

The process of setting up the fiscal integration includes the following tasks:

- Configure fiscal connectors that represent fiscal devices or services that you use for fiscal registration purposes, such as fiscal printers.
- Configure document providers that generate fiscal documents that fiscal connectors register in fiscal devices or services.
- Configure the fiscal registration process that defines a sequence of fiscal registration steps and the fiscal connectors and fiscal document providers that the process uses for each step.
...
A fiscal document provider generates fiscal documents that represent transactions and events registered on the POS. It uses a format that supports interaction with a fiscal device or service. For example, a fiscal document provider might generate a representation of a fiscal receipt in an XML format.

To upload configurations for fiscal document providers, follow these steps:
...
> [!TIP]
> By selecting **View**, you can see all functional profiles related to the current fiscal document provider.

> [!NOTE]
> Data mapping is part of a fiscal document provider. To set up different data mappings for the same connector (for example, state-specific regulations), create different fiscal document providers.

### Upload configurations for fiscal connectors

A fiscal connector handles communication with a fiscal device or service. For example, a fiscal connector might send a fiscal receipt that a fiscal document provider created in an XML format to a fiscal printer. For more details about fiscal integration components, see [Fiscal registration process and fiscal integration samples for fiscal devices and services](fiscal-integration-for-retail-channel.md#fiscal-registration-process-and-fiscal-integration-samples-for-fiscal-devices-and-services).

To upload configurations for fiscal connectors, follow these steps:
...
> [!TIP]
> By selecting **View**, you can see all functional and technical profiles related to the current fiscal connector.

For examples of configurations for fiscal connectors and fiscal document providers, see [Fiscal integration samples in the Commerce SDK](fiscal-integration-for-retail-channel.md#fiscal-integration-samples-in-the-commerce-sdk).
...
To restore the default parameters that the fiscal document provider configuration defines, select **Update** on the **Connector functional profiles** page.

> [!NOTE]
> Connector functional profiles are company-specific. If you plan to use the same combination of a fiscal connector and a fiscal document provider for different companies, create a connector functional profile for each company.
...
1. Create a connector technical profile for each fiscal connector by following these steps:
    ...
    1. Select a connector type:
        ...
        - For devices or services that connect to a Hardware station or are present in the local network, select **Local**.
...
You can change parameters on the **Device** and **Settings** tabs in a connector technical profile. To restore the default parameters that the fiscal connector configuration defines, select **Update**. While a new version of an XML configuration is loading, you receive a message that states that the current fiscal connector or fiscal document provider is already in use. This procedure doesn't override manual changes that you previously made in connector functional profiles and connector technical profiles. To apply the default set of parameters from a new configuration, select **Update** on either the **Connector functional profiles** page or the **Connector technical profiles** page.
...
A fiscal connector group combines functional profiles of fiscal connectors that perform identical functions and are used at the same step of a fiscal registration process. For example, if several fiscal printer models can be used in a store, you can combine fiscal connectors for those fiscal printers in a fiscal connector group.
...
A fiscal registration process is defined by the sequence of registration steps and the connector group that you use for each step.
...
- The fiscal document provider predefined the subscription of events and transactions to fiscal registration.
...
- The fiscal document provider uses the data mapping settings from the fiscal document provider configuration to transform transaction and event data, such as taxes and payments, while a fiscal document is generated.
...
Select registers where fiscal registration is prohibited. For example, choose registers where you need to provide only nonfiscal operations such as product catalog search, customer lookup, or transaction draft creation on these devices.
...
Validate the fiscal registration process in the following cases:

- You completed all the settings for a new registration process. These settings include assignment of registration processes to POS functionality profiles and hardware profiles.
- You made changes to an existing fiscal registration process, and those changes might cause a different fiscal connector to be selected at runtime. For example, you changed the connector group for a fiscal registration process step, enabled a connector functional profile in a connector group, or added a new connector functional profile to a connector group.
- You made changes in the assignment of connector technical profiles to hardware profiles.
...
In some cases, a special text must be printed on a fiscal receipt if a discount is applied. Set up fiscal texts for discounts on the **Fiscal connector group** page (**Retail and Commerce \> Channel setup \> Fiscal integration \> Fiscal connector groups**).

- For manual discounts that you apply at the POS, set a fiscal text for the info code or info code group that the **Product discount** info code in the POS functionality profile specifies.
    ...
    > [!NOTE]
    > You can specify a fiscal text for several info codes to support scenarios where info code groups, linked info codes, and triggered info codes are used. In these scenarios, the fiscal receipt contains the fiscal texts from all info codes that are linked to the transaction line where the discount was applied.

- For channel-specific discounts, define a fiscal text for the discount ID.
    ...
    > [!NOTE]
    > If you apply several discounts to the same transaction line, the fiscal receipt contains fiscal texts from all discounts that link to those transaction line.

## Set error handling settings

Set the error handling options that are available in the fiscal integration during the fiscal registration process. For more information about error handling in the fiscal integration, see [Error handling](fiscal-integration-for-retail-channel.md#error-handling).

To set error handling settings, follow these steps:

1. On the **Fiscal registration process** page (**Retail and Commerce \> Channel setup \> Fiscal integration \> Fiscal registration processes**), set the following parameters for each step of the fiscal registration process:

    - **Allow skip** – Enables the **Skip** option in the error handling dialog box.
    - **Allow mark as registered** – Enables the **Mark as registered** option in the error handling dialog box.
    - **Allow postpone** – Enables the **Postpone** option in the error handling dialog box.
    - **Continue on error** – If enabled, the fiscal registration process can continue on the POS register if the fiscal registration of a transaction or event fails. Otherwise, to run the fiscal registration of the next transaction or event, the operator must retry the failed fiscal registration, skip it, or mark the transaction or event as registered. For more information, see [Optional fiscal registration](fiscal-integration-for-retail-channel.md#optional-fiscal-registration).

    > [!NOTE]
    > If you enable the **Continue on error** parameter, the **Allow skip** and **Allow mark as registered** parameters are automatically disabled.
...
1. The **Skip**, **Mark as registered**, and **Postpone** options let operators enter additional information when fiscal registration fails. To make this functionality available, specify the **Skip**, **Mark as registered**, and **Postpone** info codes on a fiscal connector group. The information that operators enter is then saved as an info code transaction that is linked to the fiscal transaction. For more details about info codes, see [Info codes and info code groups](../../info-codes-retail.md).
    ...
    > [!NOTE]
    > One fiscal document and one nonfiscal document can be generated on any step of a fiscal registration process. A fiscal document provider extension identifies every type of transaction or event as related to fiscal or nonfiscal documents. The error handling feature applies only to fiscal documents.
    >
...
1. If the operator must be able to continue to process the current operation (for example, creation or finalization of a transaction) after a health check error occurs, enable the **Allow skip health check error** permission on the **Permission groups** page (**Retail and Commerce \> Employees \> Permission groups**). For more information about the health check procedure, see [Fiscal registration health check](fiscal-integration-for-retail-channel.md#fiscal-registration-health-check).

## Set up fiscal X/Z reports from the POS

To enable fiscal X/Z reports to run from the POS, add new buttons to a POS layout.
...
To enable manual execution of a deferred fiscal registration, add a new button to a POS layout.
...
1. Select **Settings**. If fiscal integration is enabled, the **Fiscal Integration** section shows the following information:
