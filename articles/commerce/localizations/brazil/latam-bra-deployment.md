...
For more information about how to work with Key Vault storage, see [Get started with Azure Key Vault](/azure/key-vault/key-vault-get-started) and [Set up the Azure Key Vault client](../../../finance/localizations/global/setting-up-azure-key-vault-client.md).

You can also use the [User-defined certificate profiles for retail stores](../global/certificate-profiles-for-retail-stores.md) feature that supports failover to offline when Key Vault or Commerce headquarters isn't available. This feature extends the [Manage secrets for retail channels](../../dev-itpro/manage-secrets.md) feature.
...
> [!NOTE]
> For more information, see [Configure and install Retail hardware station](../../dev-itpro/retail-hardware-station-configuration-installation.md).
