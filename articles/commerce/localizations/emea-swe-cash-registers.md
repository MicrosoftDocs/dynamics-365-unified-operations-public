...
2. On the **All stores** page, update store details. Specifically, set the following parameters:
    ...
    > [!NOTE]
    > If you use the control unit integration sample for Sweden, make sure that the length of the **Tax identification number (TIN)** does not exceed 10 chars, as it is limited by the control unit driver.
...
3. Set up POS functionality profiles:
    ...
    > [!NOTE]
    > If you use the control unit integration sample for Sweden, make sure that the length of the receipt number does not exceed 12 chars, as it is limited by the control unit driver.
