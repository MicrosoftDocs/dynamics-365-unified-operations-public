...
description: Learn how to edit and audit online order and asynchronous customer order transactions in Microsoft Dynamics 365 Commerce.
author: josaw1
ms.date: 01/22/2026
ms.topic: how-to
...
Between Commerce versions 10.0.5 and 10.0.6, Microsoft added support for editing cash and carry transactions, such as sales and returns, and cash management transactions, such as float entry and tender removal. In Commerce version 10.0.7, Microsoft added support for editing online order transactions and asynchronous customer order transactions.
...
1. Open the **Store financials** workspace. The **Online order synchronization errors** and **Customer order synchronization errors** tiles provide a prefiltered view of the retail transaction page. Each tile shows the transaction records that failed synchronization for the corresponding order type.
...
1. If the error details indicate that the record needs to be fixed, select **Office Add in**, and then select **Edit selected transaction**. An Excel file that shows the details of the selected transaction is opened.

    - If you're editing an online order transaction, the Excel file contains the following worksheets:
    ...
    - If you're editing an asynchronous customer order transaction, the Excel file contains the following worksheets:
...
1. In the Excel file, in the **Pending order status** field, enter **Editing**, and then publish the change. This value prevents the **Synchronize order** job running in batch mode from skipping this record during processing.
1. In the Excel file, modify the appropriate fields, and then upload the data back into Commerce headquarters by using the publishing functionality of the Dynamics Excel Add-in. After you publish the data, the changes are reflected in the system. During publication, no validation is done for changes that users make.
    > [!NOTE]
    > If you can't find the field that you need to edit, complete the following steps to add the missing field in the worksheet:
    ...
    >   1. When the update is complete, you might need to select **Refresh** to update the values.
...
1. After you make and publish your changes, select **Synchronize order** to immediately start the synchronization process. Alternatively, you can wait for the synchronization process that runs in batch mode to process the transaction.

By default, after the orders sync successfully, they're put in a hold status, based on the hold code that you define in the Commerce parameters. You must remove the hold on the orders before the orders can be processed further for fulfillment or other operations.

## Prevent a transaction from being processed by the order synchronizer

If you already manually created corresponding orders, you might not want the **Synchronize orders** job to process certain transactions. To prevent the **Synchronize orders** job from processing certain transactions, set the **Entry Status** property to **Voided** for those transactions. The **Synchronize orders** job then skips the transactions.
