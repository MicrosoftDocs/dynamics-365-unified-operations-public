...
description: This article provides an overview of the cash management capabilities in Microsoft Dynamics 365 Commerce POS, and provides guidance on setting up the cash traceability capability for two-sided transactions.
...
This article provides an overview of the cash management capabilities in Microsoft Dynamics 365 Commerce point of sale (POS), and provides guidance on setting up the cash traceability capability for two-sided transactions.

> [!NOTE]
> For background information on cash management capabilities in Commerce, see [Shift and cash drawer management](shift-drawer-management.md).
...
Dynamics 365 Commerce provides cash management capabilities in its point of sale (POS) application. Retailers have two cash management options:

- **One-sided cash transactions** - This option enables retailers to reconcile the cash for a store, but doesn't allow them to precisely determine accountability in the event of a cash discrepancy. Once executed, a one-sided cash transaction requires no further action since it is considered completed.
...
1. Select a functionality profile that is linked to the stores for which you want to set up cash traceability functionality.
...
> [!NOTE]
> To use the cash traceability functionality, you must first set up one or more safes for the store. This is because the system expects a safe to be selected for the **Declare start amount** and **Safe drop** operations.
...
1. To add more safes, repeat steps 5 and 6 for each new safe. 
...
- A user who runs a **Declare start amount** operation must enter the source of cash. The user can search for the available safes defined in the store, and then select the safe that the cash is being taken out of so that it can be put into the register. 
...
For example, when a user performs a cash management operation (such as **Tender removal**) for $20 on shift A to be sent to shift B, when the user on shift 2 performs a **Float entry** operation and selects the transaction, the two transactions are systematically reconciled. 
...
Continuing with the example above, if the user on shift B doesn't perform a **Float entry** operation, or doesn't choose the correct transaction during the **Float entry** operation, then the tender removal action remains unreconciled and requires manual reconciliation. 
...
In Dynamics 365 Commerce, cash reconciliation can only be done at the shift level. To manage cash in a safe, the safe must be managed via a shift. 
...
The following example outlines the business process for cash management using a safe.
...
1. The cashier takes the money, performs a **Declare start amount** operation on their register, and selects the transaction created above to reflect the start amount. These actions trigger the systematic reconciliation of the two transactions.

The cashier then executes regular sale transactions throughout the day.
...
1. The cashier hands over the money to store manager, or drops it in the safe by selecting **Safe drop**, entering the amount, and selecting the safe to drop the money into.
...
1. Select a safe, select **Float entry**, and then select the transaction created above. These actions trigger the systematic reconciliation of the two transactions.
1. For the selected safe, select **Bank drop**, and then select the money to be dropped into the bank.
1. Navigate out of the **Manage safe** operation, and then select **Tender declaration** to declare the balance in the safe.
1. Navigate to the **Manage shifts** operation, select the blind-closed shift for the cashier, and then close the shift. The shift on the dedicated register for the safe will then show one unreconciled transaction (**Declare start amount**) performed for the safe. This transaction can't be systematically reconciled because the money is expected to be added from an external source (such as a bank), and so must be manually reconciled. The store manager selects the unreconciled transaction and then closes the shift.
		
> [!IMPORTANT]
> - Any money left in the safe by the end of the day is not automatically carried over to the next day. The store manager must declare the start amount in the safe the next day.
> - For all safe-related actions, the user must use the **Manage safe** operation since that is the only way to select the safe for which the cash transactions are recorded. The **Declare start amount**, **Float entry**, **Tender removal** and **Bank drop** operations must be performed in the context of the **Manage safe** operation. 
...
> - The **Declare start amount** operation is available at two levels within the **Manage safe** operation: the shift level and the safe level. On the dedicated register for the safe, to declare the start amount for a safe the user must run the **Declare start amount** operation within the **Manage safe** operation. The **Declare start amount** operation at the shift level can be used if there is any additional money in the cash drawer associated with this register.
> - To transfer money in either direction between a safe and a shift, the shift must be opened on the register that is sending or receiving the money. In the example used above for [safe management](#safe-management), shift A is required to be open before the money can be transferred from the safe to this shift.
