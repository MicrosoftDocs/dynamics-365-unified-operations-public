...
- **Commerce headquarters and Finance and Operations apps must be on the same version as, or a newer version than, Commerce Scale Unit (both cloud and self-hosted).**
    ...
    For example, if Commerce headquarters and Finance and Operations apps are on version 10.0.23, Commerce Scale Unit must be on version 10.0.21 or earlier (for example, 10.0.20 or 10.0.19).
...
- **Commerce Scale Unit must be on the same version as, or a newer version than, Modern Point of Sale (POS), Hardware Station, and the Commerce software development kit (SDK) and associated local site configurations (such as modules, data actions, and themes).**
    ...
    For example, if Commerce Scale Unit is on version 10.0.23, Modern POS, Hardware Station, and the Commerce storefront must be on version 10.0.23 or earlier (for example, 10.0.22 or 10.0.21).
...
- **Extension packages must be compiled against the same version as, or a newer version than, the target component that the extension applies to.**
    ...
    For example, if the deployed Commerce Scale Unit is on version 10.0.23, the corresponding extension packages must be compiled against version 10.0.23 or earlier (for example, 10.0.22 or 10.0.21).
...
The following table describes the current supported versions of various Commerce components as of **December 10th, 2021**.
...
|---|---|---|---|---|
| Finance and Operations apps | 10.0.23 | 10.0.23 | 10.0.19 | 10.0.19 |
| Commerce Scale Unit (cloud-hosted) | 10.0.23 | 9.33 | 10.0.19 | 9.29 |
| Commerce module library | 10.0.23 | 9.33 | 10.0.19 | 9.29 |
| Commerce Scale Unit (self-hosted) | 10.0.23 | 9.33 | 10.0.15 | 9.25 |
...
For example, customers who are currently on version 10.0.20 can pause updates to versions 10.0.21, 10.0.22, and 10.0.23. However, they must then update to version 10.0.24. In this scenario, after version 10.0.25 becomes available, version 10.0.20 is no longer supported.
