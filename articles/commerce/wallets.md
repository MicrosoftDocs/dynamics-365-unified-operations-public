...
For retail store transactions, additional enhancements have been added to the payment transactions view for payments that are created by using the Adyen connector. The **Approval code** field shows the approval code from the processor, the **Processor payment** field shows the processor payment method, and the **Processor reference** field shows the PSP reference from Adyen. These three fields aren't discrete fields in the database. They will be shown in the user interface only when a specific payment is viewed. A payment processed using the PayPal connector also provides the **Approval code**, **Processor payment** and **Processor reference**. Before the fields can be shown, the point of sale (POS) client must be updated with Commerce version 10.0.20 or later, and the **Enabled** status for the **Enhanced wallet support and payment improvements** feature must be synchronized to the channel.

**Note**: The PayPal connector does *not* provide the values for the three fields for a payment void request. Hence, the three fields will be blank in the user interface.
