...
When publishing product and category media assignments, there are several sequential steps that happen automatically for you in the backgound. First, the underlying content management system (CMS) ensures that all of the product's or category's assigned media item binaries (images, videos, documents, etc.) are actually published.  This is equivalent to navigating to the **Media library** and clicking **Publish** for each media item assigned to a product or category.  Once this step is completed automatically in the CMS,  each individual media item's default CMS URL is now public.  After this, the product (or category) assignments and configured fallback defaults are flattened and saved to the Commerce Headquarters (HQ) SQL database.  Once media assignments are saved to the HQ database, the HQ -> CSU (Commerce Scale Unit) data sync job(s) will pick up the media assignments and push them to CSU database(s) accross various channels.  By default, CSU architecture has a two hour rolling cache for all product and category merchandising data from its own database.  in summary, the time it will take for product and category media assignments to show up for end users after a **Publish** action will vary based on the following factors: 1) the volume of media items and assignments for the publish action, 2) the configured frequency of the HQ omnichannel media batch job in HQ, 3) the configured frequency of the HQ -> CSU data sync jobs in HQ, 4) the rolling default 2-hour cache for CSU merchandising data.
...
7. The assignments are now **Published** and will appear for channel-specific end-users based on the configured HQ merchandising data batch jobs, data sync, and CSU cache schedules.
