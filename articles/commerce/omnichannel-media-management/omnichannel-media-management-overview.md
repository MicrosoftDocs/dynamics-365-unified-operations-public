...
Prior to Commerce version 10.0.35, swatches were centrally managed in Commerce HQ by assigning a color hex value or image to dimension values (example: color = blue).  This central management of swatch assignments does _not_ change with the introduction of the media management features included in 10.0.35+ versions of Commerce; but there is a new capability to optionally assign product specific swatch overrides for display within product pages.  Swatches for category pages, list views, and search results still use the centrally managed swatch values set for product dimension values.  These centrally managed dimension value swatch assignments allow search refiners and filters to behave correctly accross multiple products. But for scenarios where more detailed swatches are wanted within a single product page, product-specific swatch overrides are available.  These product-specific swatches can be used to show things like fabric textures, multi-colored patterns, iconography, or anything else that might go beyond the cross-product dimension-specific swatch values needed for product list refiners.
