...
|Enable forced reselection of shipping address| **True** or **False** |If this optional property is set to **True** (recommended value), then in some edge cases, the customers will be required to re-enter a shipping address during checkout. This helps in scenarios where the customer deletes a saved address which is present on the checkout cart. If this is set to **False**, then in the scenario described, the sales order might not be associated with a shipping address.|
