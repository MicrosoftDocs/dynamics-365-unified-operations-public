...
|Enable forced reselection of shipping address| **True** or **False** | If this property is set to **True**, then customers will be required to reselect their checkout cart's shipping address from their saved addresses if the current address in the checkout cart is no longer in their saved addresses (e.g., if the address was deleted from their account).|
