...
|Enable multiple shipping addresses for an order| **True** or **False**| If this optional property is set to **True**, then a customer can select multiple shipping addresses for an order by selecting a shipping address for each order line.  Since each shipping address requires an individual delivery option so this configuration has a dependency on another configuration named **"Enable multiple delivery options for an order"** available on the Delivery options module. This feature is available as of the Commerce version 10.0.40 release.|
