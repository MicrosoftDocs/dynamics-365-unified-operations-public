...
When the orders are placed in POS or ecommerce, the customers commit to an amount which includes a price and/or a discount. If the customer later calls the call center to modify the order for e.g. to include another item to the order, then, if the promotions on the existing order lines are expired, the customer would not expect the discounts applied on the existing order lines to be removed. However, on the flip side, if there was no discount during the order placement, but now a discount has started, so the customer would want the discount to be applied as otherwise, he could cancel this order and place a new one. Thus, there is a need of preserving the prices and discounts committed to the customer, but the POS and Call center users need the flexibility to recalculate the prices and discounts for the sales order lines, as needed.
