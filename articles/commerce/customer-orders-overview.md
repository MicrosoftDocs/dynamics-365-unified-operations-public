...
The **Use realtime service for order creation with async backup** option is dependent on the **Create customer order in async mode** configuration above. 

The **Use realtime service for order creation with async backup** option can only be enabled if the **Create customer order in async mode** is already enabled. The **Use realtime service for order creation with async backup** option first tries to create the order by using the Retail Transaction Service (RTS). If that attempt fails, the order will be created by using the same batch process described above. Most of the time, the order is created as quickly as it would be created through synchronous order creation. However, with this configuration, POS immediately completes the transaction by using the confirmation number for the order and doesn't wait for the RTS call to be completed. 
