...
#### Pricing impact when editing orders

When orders are placed in POS or on a Commerce e-commerce site, customers commit to an amount which includes a price and possibly a discount. If a customer later contacts the call center to modify the order (for example, to include another item to the order), if the promotions on the existing order lines are expired, the customer would not expect the discounts applied on the existing order lines to be removed. However, if there was no discount in effect when the order was placed, but a discount has started since then, the customer would expect that the discount should now be applied, since the customer could just cancel the existing order and then create a new one where the discount applies. This scenario shows that there is a need to preserve prices and discounts committed to the customer, while at the same time granting POS and call center users the flexibility to recalculate prices and discounts for sales order lines as needed.

When orders are recalled and edited in POS, the prices and discounts of the existing order lines are considered "locked." In other words, the prices and discounts do not change even if some order lines are cancelled or modified, or if new order lines are added. To modify prices and discounts of existing sales lines, the POS user must select the **Recalculate** button, which removes the price lock from the existing order lines. However, until the Commerce version 10.0.21 release this capability was not available in the call center and any modifications to order lines resulted in the recalculation of prices and discounts. With the Commerce version 10.0.21 release, a new feature named **"Prevent unintentional price calculation for commerce orders"** is available in the **Feature management** workspace, and it is enabled by default. With this feature, a new property named **Price locked** is available for all e-commerce orders. For orders placed from any channel, this property (displayed as a check box) will be selected automatically for all the order lines after order capture is complete. The Commerce pricing engine excludes such order lines from all price and discount calculations, so if the order is edited, by default the order lines will be excluded from the pricing and discount calculation. However, call center users will be able to disable this property (clear the check box) for any order line and then select the existing **Recalculate** button to include the existing order lines in the pricing calculations. Even when applying a manual discount to an existing sales line, call center users must disable the price lock property of the sales line and then apply the manual discount. Call center users can also disable the **Price locked** property for the order lines in bulk by selecting the **Remove price lock** button located on the **Sales order \> Sell \> Calculate** tab. Selecting the **Remove price lock** button will remove the price lock from all the order lines except those that are non-editable (in other words, with status "Partially invoiced" or "Invoiced"). After the order edit is complete and the order changes have been submitted, all the order lines become price locked again. Call center users can also disable the **Price locked** property for order lines in bulk by selecting the **Remove price lock** button located on the **Sales order \> Sell \> Calculate** tab. Selecting the **Remove price lock** button removes the price lock from all order lines except the ones which are non-editable (those with the status "Partially invoiced" or "Invoiced").

> [!IMPORTANT]
> If the **"Prevent unintentional price calculation for commerce orders"** feature is turned on, then the trade agreement evaluation setup will be ignored from the pricing workflows. In other words, the trade agreement evaluation dialog boxes will not display the "price related" section. This is because the trade agreement evaluation setup and the price lock feature have similar purposes (to prevent unintentional price changes), but the trade agreement evaluation user experience does not scale well for large order sizes where users need to select one or more order lines for repricing.

> [!NOTE]
> The capability to select one or more order lines for disabling the **Price locked** property is only available when using the call center module. The behavior of POS remains unchanged. In other words, the POS user cannot select order lines for price unlocking, but if the POS user selects the **Recalculate** button, then all existing order lines become "price unlocked."
