...
#### Pricing impact on editing orders
...
When orders are recalled and edited in POS, then the prices and discounts of the existing order lines are considered 'locked' i.e. the price and discounts do not change even if some order lines are cancelled, modified or new order lines are added. To modify the price and discounts of the existing sales lines the POS user must press the "Recalculate" button which removes the price lock from the existing order lines. However, until 10.0.21, this capability was not available in the call center and thus any modifications to the order lines resulted in the recalculation of prices and discounts. With 10.0.21, a new feature named **"Prevent unintentional price calculation for commerce orders"** is available in the Feature Management workspace and it is turned on by default. With this feature, new property named "Price locked" is available for all commerce orders. For commerce orders, placed from any channel, this property (displayed as a checkbox) will be checked automatically for all the order lines after the order capture is complete. The pricing engine will exclude such order lines from all price and discount calculations and thus, if this order is edited, by default, these order lines will be excluded from the pricing and discount calculation.  However, the call center users will be able to unselect this property for any order line and then hit the existing 'Recalculate' button, to include these existing order lines in the pricing calculations. Even for applying a manual discount on an existing sales line, the user must uncheck the price lock property of the sales line and then apply the manual discount. The call center user can also uncheck the 'Price locked' property for the order lines in bulk by pressing a new button named “Remove price lock” under the “Sales order -> Sell -> calculate” tab. Clicking this button will remove the price lock from all the order lines except the ones which are *non-editable i.e. with status Partially invoiced or Invoiced*. After the order edit is complete and the order changes are submitted, all the order lines get price locked again. The call center user can also uncheck the 'Price locked' property for the order lines in bulk by pressing a new button named “Remove price lock” under the “Sales order -> Sell -> calculate” tab. Clicking this button will remove the price lock from all the order lines except the ones which are **non-editable i.e. with status Partially invoiced or Invoiced**.

> [!IMPORTANT]
> Please note that if this feature is turned on, then the **Trade agreement evaluation setup will be ignored from the pricing workflows** i.e. the Trade agreement evaluation dialogs will not show the price related section. This is done because the Trade agreement evaluation and Price lock feature have similar purposes i.e. to prevent unintentional price changes, but the Trade agreement evaluation user experience does not fit well for the large order sizes where the user needs to choose one or more lines for repricing.

> [!NOTE]
> The capability to choose one or more lines for unchecking the 'Price locked' property is only available in the Call center module. The behavior of POS remains as is i.e. the user cannot choose the order lines for price unlocking, rather, if the user presses the Recalculate button, then *all* the existing  order lines become price unlocked.
