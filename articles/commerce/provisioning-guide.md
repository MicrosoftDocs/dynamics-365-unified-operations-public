...
description: Learn how to provision a Microsoft Dynamics 365 Commerce sandbox environment for demo or sandbox usage with built-in demo data.
author: josaw1
ms.date: 01/28/2026
ms.topic: how-to
...
Before you begin, review this article to get an idea of what the process requires.

To successfully provision a Commerce sandbox environment, you must specify some parameters for the environment and Commerce Scale Unit (CSU) that you use when you provision Commerce later. The instructions in this article describe all the steps that are required to complete provisioning and the parameters that you must use.
...
Before you can provision your Commerce environment, make sure that the following prerequisites are in place:

- You have access to the Microsoft Dynamics Lifecycle Services (LCS) portal.
- You're an existing Microsoft Dynamics 365 partner or customer and have an implementation project already created and available to use in LCS.  
- You created a Microsoft Entra security group that you can use as a Commerce system administrator group, and you have its ID available.
- You created a Microsoft Entra security group that you can use as a ratings and reviews moderator group, and you have its ID available. (This security group can be the same as the Commerce system administrator group.)
- You deployed a headquarters instance within LCS. For more information, see [Deploy a new environment](/dynamics365/fin-ops-core/dev-itpro/deployment/deployenvironment-newinfrastructure).

This list isn't exhaustive. If you experience any issues, reach out to your Microsoft partner contact for assistance.

## Provision your Commerce environment

The following procedures explain how to provision a Commerce environment. After you successfully complete the steps, your Commerce environment is ready for configuration. All of the steps described in this section take place in the LCS portal.
...
1. Select **Initialize**.
1. In the warning dialog box, confirm the Commerce Scale Unit initialization by selecting **Yes**. The CSU is now queued for provisioning.
...
1. In the **e-Commerce environment name** field, enter a name. This name appears in some of the URLs that point to your e-Commerce instance.
1. In the **Commerce Scale Unit name** field, select your CSU in the list. The list has only one option.

    The portal sets the **e-Commerce geography** field automatically.
...
1. Select **Initialize**. The **Commerce management** view displays again, where the **e-Commerce** tab is selected. E-Commerce initialization starts.
