...
> [!NOTE]
> It is required that the "Independent sequence" property is checked for all receipt types of the functionality profile, before this feature can be used. Secondly, the system time zone of the device, where the POS is being used, should match the corresponding store time zone. We are working on alleviating these limiations. Please contact Microsoft support prior to using this feature in Production.
