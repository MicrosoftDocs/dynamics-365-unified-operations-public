...
description: Learn how to configure and manage receipt numbers in Microsoft Dynamics 365 Commerce.
...
Retailers generate receipt numbers for various actions in their stores, such as cash and carry transactions, return transactions, customer orders, quotations, and payments. Although retailers define their own receipt formats, some countries/regions have regulations that put restrictions on receipt formats. For example, regulations might limit the number of characters on the receipt, require consecutive receipt numbers, restrict some special characters, or require a reset of receipt numbers at the beginning of each year. Dynamics 365 Commerce helps retailers meet regulatory requirements by making the process of managing receipt numbers flexible.

## Define receipt number formats

Receipt numbering formats are composed of both static content and dynamic content. Static content includes alphabetic characters, numbers, and special characters. Dynamic content includes one or more characters that represent information such as the store number, terminal number, date, month, year, and number sequences that are automatically incremented. Define the formats in the **Receipt numbering** section of the functionality profile. The following table describes the characters that represent the dynamic content.
...
| MM         | The characters **MM** are used for the two-digit month. For example, in January, the format **MM** shows "01" on the receipt. |
...
:::image type="content" source="media/Enable_receipt_reset.png" alt-text="Screenshot of selecting a receipt reset date.":::

After you select a date, it appears in the **Next receipt number reset date** column. The reset date is applicable to all receipt transaction types. Therefore, the receipt number sequence resets for all receipt types.

When the reset date arrives, the receipt number resets for the first transaction of each type. In addition, in the functionality profile, the reset date moves from the **Next receipt number reset date** column to the **Current receipt number reset date** column. This change indicates that if a register isn't used on the reset date, the receipt number resets the next time that the register *is* used. For example, on December 3, 2019, you select **January 1, 2020**, as the reset date. On January 1, when the registers make their first transaction, the receipt numbers reset. However, one register isn't used at all during December and January, but then starts to be used in February. In this case, because a reset action was defined, the receipt number for that register resets when the register makes its first transaction in February.

You can use the **Clear reset date** functionality to clear future reset dates. However, if the reset date occurred in the past, it can't be undone. Therefore, the reset still occurs for all registers where the reset hasn't yet occurred.
...
> - The reset date set in **Functionality profiles** isn't associated with a specific time zone. For example, if you select **January 1, 2020** as the reset date, POS devices in all time zones reset the receipt back to "1" on January 1, 2020 local time.
> - If you want the receipt masks to change together when the reset date arrives, follow the steps in [Make receipt masks change when the reset date arrive](#make-receipt-masks-change-when-the-reset-date-arrives). 
> - Depending on the reset date that you select and the receipt format, you might have duplicate receipt numbers. Although the point of sale (POS) system can handle these situations, they increase the amount of time that is required to process returns, because sales associates must select among the duplicate receipts. Other complications that are related to data cleanup can occur if the duplicate receipts weren't a planned consequence. Therefore, use dynamic date characters (for example, **ddd**, **MM**, **DD**, and **YY**) to help prevent duplicate receipt numbers after a reset.

## Force synchronization of number sequence data at app launch

Organizations that have strict requirements for receipt ID uniqueness can enable automatic synchronization of number sequence data. When enabled, the Commerce Scale Unit (CSU) provides the latest number sequence data whenever POS initializes. This process guarantees that the POS always has the most current number sequence before any transactions are executed.  

To enable synchronization of number sequence data at app launch, follow these steps:

1. In headquarters, go to **Retail and Commerce \> Channel setup \> POS setup \> POS profiles \> Functionality profiles**, and then select the functionality profile used by the stores for which you want to enable the feature.
1. In the **Receipt number synchronization** section, enable the **Retrieve the latest number sequence data** setting. 
1. Go to **Retail and Commerce \> Retail and Commerce ID > Distribution schedule**, and then run the **1070 (Channel configuration)** job.  

> [!NOTE]
> When you enable the **Retrieve the latest number sequence data** setting, the first sign-in after app launch might take longer than usual. The CSU also incurs some extra load.
...
1. When you complete the preceding steps, open POS and start creating new orders. POS now creates orders by using new receipt masks with the receipt ID reset.
