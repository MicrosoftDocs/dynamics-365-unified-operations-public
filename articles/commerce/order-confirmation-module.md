...
description: Learn about order confirmation modules and how to use them in Microsoft Dynamics 365 Commerce.
author: anupamar-ms
ms.date: 01/23/2026
ms.topic: how-to
...
Use the order confirmation module to show order confirmation details after an order. It shows the order confirmation ID, order contact information, and other order details, such as the items that you purchased, payment information, pickup options, and the shipping method.
...
| Contact number | Text | A contact number for order-related questions. |
...
When you create an order confirmation page, add other relevant modules in addition to the order confirmation module. Here are some examples:

- **Recommendations module** – Add the recommendations module to the order confirmation page to suggest other products to the customer.
- **Marketing modules** – Add any marketing module to the order confirmation page to show marketing content.
...
1. Select **Save**, and then select **Preview** to preview the template. The order confirmation module doesn't render because it requires the context of the order confirmation number.
