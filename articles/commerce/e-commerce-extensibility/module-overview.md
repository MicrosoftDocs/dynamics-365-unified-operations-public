...
Microsoft Dynamics 365 Commerce provides a [store starter kit](../starter-kit-overview.md) set of modules to use when building your online e-Commerce pages.  Some CSS for each module can be changed using style presets and/or CSS overrides inside of the site builder without any code changes.  The online SDK can be used to make custom theme changes to change the CSS or even extend a module's view to change the rendered HTML layout. If deeper changes are needed to a module such as business logic then a starter kit module can also be cloned into a new custom module for deeper changes if needed.  More details are provided below as to which option is best for various requirements.
