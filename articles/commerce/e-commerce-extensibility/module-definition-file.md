...
* **"dataActions"** – The data actions node is used to register the data actions that should be run for the module. By default they will run server side, but can also be configured to run client side.
* **"slots"** – Slots are defined only in container modules. They are exposed in the site builder tool. Allow and deny lists can be defined for a slot to allow or disallow specific modules from being accepted in that slot.
