...
You created a new ROPC policy to enable local sign-in. Under **Run user flow** you should see an endpoint URL similar to ```https://<LOGIN_DOMAIN>//<B2C_TENANT>.onmicrosoft.com/v2.0/.well-known/openid-configuration?p=B2C_1_ROPC_Auth```. Take note of the **<LOGIN_DOMAIN>** and **<B2C_TENANT>** values from the URL, because you use this information in your credentials.json file.
...
:::image type="content" source="media/local-sign-in-01.png" alt-text="Screenshot of the Run user flow example in Azure portal.":::
...
After you complete the steps to configure your Microsoft Entra B2C tenant, create a credentials file in your online software development kit (SDK) Node application.
...
- **additionalUsers**: An array of user objects that you can use to configure more users to test with. Each entry in this array should be an object with a name, email address, password, and customer account number. To sign in as one of these users, use the query parameter **mockUser=\<name>**.

### Mock a signed-in B2B user

If you need to mock a signed-in business-to-business (B2B) user, use the **isB2bUser** property for a user credential and set it to **true**, as shown in the following example.
