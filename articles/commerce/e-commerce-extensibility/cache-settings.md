...
The SDK \\src\\settings\\cache.settings.json file holds cache settings for entities returned from data actions calls. These entities, once cached would be returned from cache for all data action requests with the same cache key as long as their TTR(time to refresh) TTL(time to live) are valid. The cache key in for core data actions are implicitly set within the data action itself. For custom data actions, the cacheKey can be defined in the action input by overriding getCacheKey() method.

The following example shows a JavaScript Object Notation (JSON) file that includes a section for the **time to refresh** and **time to live** settings.  Each section contains the data action cache key entity name followed by the time in milliseconds(ms).
...
For example, the **get-simple-products** data action is found in the 'get-simple-products.js' file has the entity name **SimpleProduct**, which can be added to the cache settings file:
