...
description: Learn about regular updates released as part of the Microsoft Dynamics 365 Commerce online software development kit (SDK).
author: samjarawan
ms.date: 05/14/2026
ms.topic: how-to
...
The following table maps the module library versions to Commerce Scale Unit (CSU) versions. Use the same module library versions for the Retail Server proxy and Fabrikam design kit as the ones mapped to the Commerce Scale Unit. The table lists the currently supported versions as of April 2026.

| CSU version | Maximum module library version |
| ----------- | ------------------------------ |
| 10.0.13 | 9.23.x |
| 10.0.14 | 9.24.x |
| 10.0.15 | 9.25.x |
...
In the SDK **/src/settings** directory, there's a file named **app.settings.json**. This file surfaces global settings to Commerce site builder that the module library set of modules uses. It can also contain more settings for custom modules. You can find these settings in site builder at **Site Settings** \> **Extensions**.
...
## Update module library by using a global module library extension

> [!NOTE]
> Only update the module library by using a global module library extension if you have no e-commerce extension customizations.

If you have an environment with no e-commerce extension customizations (in other words, no modules, data actions, or themes) such as a newly deployed environment, you can update the module library by using a preconfigured e-commerce global extension within LCS. Deploying by using one of the globally available customizations overwrites the deployed e-commerce extension package, so make sure you don't have any customizations already uploaded. You must also ensure you're not upgrading to a module library version later than those associated with the deployed Commerce Scale Unit version, as shown in the [Module library versions mapped to Commerce Scale Unit versions](#module-library-versions-mapped-to-commerce-scale-unit-versions) section.
...
1. In the **Pick an e-commerce package** dialog box, select the package to import, and then select **Pick**. It can take 15-30 minutes for the package to be available.
