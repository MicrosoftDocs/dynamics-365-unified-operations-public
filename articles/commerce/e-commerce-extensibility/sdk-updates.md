...
Within the SDK **/src/settings** directory there is a file called **app.settings.json**.  This file surfaces global settings to the site builder tool which are used by the store starter kit set of modules and can contain additional settings for custom modules.  These settings can be found in the site builder tool under the **Site Settings** -> **Extensions** section. 

When upgrading the store starter kit, there may be new settings that are applicable to them that need to be manually added.  The latest app.settings.json file can be found in the [online SDK GitHub settings directory](https://github.com/microsoft/Msdyn365.Commerce.Online/tree/master/src/settings).  This file contains the latest settings for the latest available store starter kit.  If you've not made any additions to the app.settings.json file, you can copy the contents of this file to your version or do a diff and merge the new settings in manually.  If you are updating to an older store starter kit than the latest you can find the specific app.settings.json file under the specific build branch located on the [GitHub branches page](https://github.com/microsoft/Msdyn365.Commerce.Online/branches).  You'll need to manually merge the contents to your app.settings.json file.
