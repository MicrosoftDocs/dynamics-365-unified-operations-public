...
| \_msdyn365___aud_0                          | This cookie stores segment values used by targeting and is only employed if targeting is configured on a page or fragment requested by a site user. It is only dropped when the segment values come from a third-party segmentation provider.      | 7 days |
| \_msdyn365___aud_1                           | This cookie stores segment values used by targeting and is only employed if targeting is configured on a page or fragment requested by a site user. It is only dropped when the segment values come from a third-party segmentation provider.      | 7 days |
| \_msdyn365___aud_2                           | This cookie stores segment values used by targeting and is only employed if targeting is configured on a page or fragment requested by a site user. It is only dropped when the segment values come from a third-party segmentation provider.      | 7 days |
