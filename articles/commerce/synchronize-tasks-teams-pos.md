...
description: Learn how to synchronize task management between Microsoft Teams and Microsoft Dynamics 365 Commerce point of sale (POS).
author: ritakimani
ms.date: 01/29/2026
ms.topic: how-to
...
This article describes how to synchronize task management between Microsoft Teams and Microsoft Dynamics 365 Commerce point of sale (POS).
...
> [!NOTE]
> Before you try to integrate task management with Teams, make sure that you first enable [Dynamics 365 Commerce and Microsoft Teams integration](enable-teams-integration.md).
...
1. On the Action Pane, select **Setup task management**. You should receive a notification that indicates that a batch job named **Teams provision** is being created.
...
1. In the **Drafts** list, select the task list. Then select **Publish**.
...
> [!NOTE]
> After you publish the task list successfully in Teams, the tasks appear in POS. POS managers and cashiers then need to turn on Microsoft Entra authentication in POS. For more information, see the [Enable Microsoft Entra authentication for POS sign-in](aad-pos-logon.md) article.
