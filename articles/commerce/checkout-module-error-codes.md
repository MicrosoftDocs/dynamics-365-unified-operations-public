...
Dynamics 365 Commerce uses standardized error references in online channel checkout errors that it presents to online customers. In Commerce version 10.0.31 and later, error messages in the checkout module include error codes and don't show unnecessary information to the online customer. The error codes refer to errors that are detailed in this article.

Depending on the error that you encounter, the table in this article includes the following details:
...
- Information that you can reference in a support case, if you need additional assistance

## Prerequisites

To enable the checkout module error reference codes, in site builder for your site, go to **Site settings \> Extensions**. In the **Cart and checkout** section, select **Enable Enhanced Online Channel Error Display Messaging**.

## Checkout module error reference codes

Use the following table to get more details about error code references that customers provide or experience in the online store. Scroll to the right to view the **Error Description** column.

| Error code | Dynamics-correlated error code | Error description |
| ---------- | ------------------------------ | ----------------- |
| CCR001     | Microsoft\_Dynamics\_Commerce\_Runtime\_UnableToAuthorizePaymentCardTypeMissingOrNotSupported | The payment can't be authorized. Card Type ID in **TokenizedPaymentCard** is missing, or Card Type ID provided isn't supported. |
| CCR002     | Microsoft\_Dynamics\_Commerce\_Runtime\_UnableToAuthorizePayment | Declined. The payment can't be authorized. |
| CCR003     | Microsoft\_Dynamics\_Commerce\_Runtime\_UnableToAuthorizePaymentCardAdditionalContextRequired | The payment can't be authorized. Additional information required from the customer. |
...
| CCR007     | Microsoft\_Dynamics\_Commerce\_Runtime\_MultipleCustomerAccountPaymentsNotAllowed | A customer account payment is already applied and only one payment is allowed per transaction. |
...
| CCR013     | Microsoft\_Dynamics\_Commerce\_Runtime\_UnableToCancelPayment | Sorry, something went wrong. The payment can't be canceled. Try again. |
...
| CCR025     | Front End Timeout for Checkout API | The front end operation timed out. Confirm if the order is processed in Dynamics 365 Commerce headquarters. |
| CCR026     | Original Authorized Amount Changed | The order amount changed from the original authorization amount processed with the payment gateway. This change might be due to a timed expiration of a coupon, promotion, or sale. |
...
| CCR028     | Microsoft\_Dynamics\_Commerce\_Runtime\_MissingRequiredCartTenderLines | The payment method attempted encountered an error. Contact support to review your account settings or try again with a different payment method. |
