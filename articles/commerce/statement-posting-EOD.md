...
| **Max transaction lines included in aggregation**            | Test values starting at 1000. Depending on the configuration of Headquarters, smaller orders may be more beneficial to performance.                                                                                                                                    | Determines how many lines will be included in each sales order before being split into a new order. The number will not be exact as it splits at the sales order level, but will be close to the set configuration. This is order generation of retail transactions without a named customer. |
| **Maximum number of threads to validate store transactions** | We recommend setting this value to 4 and increasing only if desired performance is not attained. This process will not be able to use more threads than available processors to the batch server. Assigning too many threads here could impact other batch processing. | Controls how many transactions can be validated at the same time for a given store.                                                                                                                                                                                                           |
