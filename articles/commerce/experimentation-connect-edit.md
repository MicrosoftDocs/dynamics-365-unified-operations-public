...
description: Learn how to connect an experiment in Microsoft Dynamics 365 Commerce and make changes to your variations so that they align with your hypothesis.
author: sushma-rao 
ms.date: 01/22/2026
ms.topic: how-to
...
:::image type="content" source="./media/experimentation_connect_edit.svg" alt-text="Screenshot of the experimentation user journey showing the connect and edit step.":::

After you [set up your experiment](experimentation-setup.md) in a partner service, connect the experiment in Dynamics 365 Commerce and edit the experiment variations.

## Connect your experiment
To connect your experiment, launch the **Connect experiment** wizard. The wizard guides you through the steps required to connect your experiment. When you complete the wizard, your experiment is connected and variations are created and ready to be edited.
...
1. To launch the **Connect experiment** wizard, select **Experiments** in the left navigation pane, and then select **Connect**. Alternatively, you can access the wizard from a page or fragment editor by editing it and selecting **Connect experiment** on the command bar.
    ...
    > - You can only experiment on pages with a custom layout. If your page has a preset layout, convert it to a custom layout first. You can do this conversion by going to the page and selecting **Convert to custom layout** on the command bar. For more information on layouts, see [Preset and custom layouts](templates-layouts-overview.md#preset-and-custom-layouts). 

1. If you're connecting to an experiment from the **Experiments** tab in the left navigation pane, select the experiment name from the list that appears.
...
When you exit the **Connect experiment** wizard, you create variations. To edit the variations so that they reflect the choices that you need to verify in the experiment hypothesis, complete the following steps.

1. In editor view, use the variations drop-down menu under the command bar to edit each variation based on your original hypothesis. You might also want to establish a control or base variation by leaving one of the variations unchanged.
1. Select the module to experiment on, select the ellipsis (...), and then select **Add to experiment**.
