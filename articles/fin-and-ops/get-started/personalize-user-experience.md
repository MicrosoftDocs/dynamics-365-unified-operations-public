...
There are many types of personalizations in Finance and Operations. One set of personalizations consists of selections that you make in a list of options on a setup page. Examples include the color theme and time zone. Some personalizations are *implicit* and are related to page usage. For example, Finance and Operations keeps track of the width of grid columns if you adjust them, and the expanded or collapsed state of FastTabs. Other personalizations are *explicit*. Explicit personalizations are changes that a user makes to the appearance of a page by directly changing the way that elements appear or act on that page. For example, the user might add, hide, or reorder elements on the page. Explicit personalizations are often made through an interactive personalization mode.
...
- **Action Panes** – An *Action Pane* appears near the top of most pages. The Action Pane contains buttons for many of the actions that you can perform on the current page. These buttons are often organized on tabs. You can pin a selected tab open, or you can have the tabs collapsed by default. Then, the next time that you open the page, Finance and Operations will restore the pinned state of the Action Pane. If the Action Pane is pinned open, Finance and Operations will also show the last tab that you selected.
...
- Use the **Summary** tool when you want an element to appear in the FastTab summary section. The Summary tool applies only to fields that are in a FastTab section. When you select the **Summary** tool, all fields that have been selected as summary fields are shown in a shaded container. You can interactively add fields to a FastTab summary and remove fields from a FastTab summary by selecting the fields.
