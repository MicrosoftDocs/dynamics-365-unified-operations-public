...
While users could only have a single set of personalizations per form with traditional personalization, **Saved views** expands on personalization in several important ways:  
...
-    Views created for particular page types can also include user-added filters or sorts, which allows users to quickly return to commonly filtered datasets. See the **What pages support views?** FAQ for more details. 
...
Unlike traditional personalization, views are not automatically saved when a user performs explicit personalizations (or filters a list). An explicit action is required to save these changes to a view. This provides user flexibility in creating a view before or after the changes associated with that view have been made and also ensures that view definitions are not unintentionally altered by one-time filters or personalizations. Note that implicit personalizations (e.g. expanding or collapsing a fast tab or changing the width of a grid column) are automatically saved to the current view, even for locked views.  

To ensure the current state of the view is known, once you start making changes to a view by performing an explicit personalization or filtering, an asterisk will appear next to the current view name indicating that you are looking at an unsaved, modified version of that view. 
...
1.	Switch to the view that you want to be the default. 
2.	Click on the view name to open the view selector. 
3.	Select **More** and then **Pin as default**.  

Alternatively, when you create a new view (via the **Save a copy** action), you can make that new view the default view by setting the Pin as default view toggle before saving the view.  

Note that in some cases, the query associated with the default view may not execute when you first navigate to a page. For example, if you navigate through a tile to a page, the tile’s query will be executed regardless of the query associated with the default view. Also, if you navigate to a page whose Classic view already has a defined query, the original query will execute originally in place of the default view’s query. When this happens, you will be alerted by an informational message when the view is loading. Switching views after the page has loaded should allow the view query to execute normally.   

## Managing personal views 
The **Manage my views** dialog gives you basic maintenance capabilities over your personal views and the order of views in the view selector. To open this page, click the view name to open the view selector dropdown, select **More**, and then select **Manage my views**.  

For the list of available views for that page, the following set of actions are available.  
...
-    **Reorder your views**: Use the **Move up** and **Move down** actions to rearrange your views in the order you desire.  
-    **Rename a view**: Use the **Rename** action to change the name of the currently highlighted personal view. This action is disabled for locked views. 
...
## Publishing views
During the publish process, views can be assigned to one or more security roles, meaning any user with that role will be able to access and use that view, though they cannot edit the view. Currently, only system administrators have rights to the Publish action in the view selector dropdown.  

To publish a view, follow these steps: 
1.	Create and save a personal copy of the view you want to publish. 
2.	With that view currently loaded, click on the view name to open the view selector dropdown. 
3.	Click the **More** button and then select **Publish**.  The Publish dialog will open.  
...
5.	Add the desired security role(s) corresponding to the users who will get this view.  
6.	Click **Publish**.
...
1.	Switch to the published view whose parameters you want to update. 
2.	Click **Publish** from the view selector dropdown. 
3.	Click **Yes** if you want to update the existing view (or **No** if you want to publish this under a different name).
4.	Update the name, description, and/or security roles for the view. 
5.	Click **Publish**. 
6.	If you updated the name of the published view, you’ll currently also need to delete the published view with the old name (see the **Managing published views** section). 
...
1.	Switch to the published view you want to modify. 
2.	Save a copy of the published view to create a local draft of the published view. 
3.	Modify the local draft with the desired changes.
4.	Publish the view with the original name. 

## Managing published views
Like managing personal views, the **Manage my views** dialog gives users with publish privileges basic maintenance capabilities over that page’s published views (in addition to their own personal views). To open this page, click the view name to open the view selector dropdown, then select **More**, and then select **Manage my views**.

While all users see a **My views** tab showing their personal views, users with publish privileges also see a **Published** tab that shows all the published views for that page. As there may be several users publishing views, it is important to be able to manage the full list of published views, regardless of whether you were the user who actually published the view.
...
3.	Set the **Enable saved views** toggle to Yes.

Once this feature is enabled, all subsequent user sessions will start with views enabled.  

Note that if personalization is disabled for the environment, views will be enabled even if you follow the above steps, since the views feature is built on top of the personalization subsystem.

### What happens to existing personalizations when views are enabled? 
When views are enabled, any existing personalizations for a user and form are saved into a new view called **My view** that is automatically set as the default view. This is meant to ensure that a consistent user experience before and after views are enabled, except for the view selector control appearing on forms.  

### What pages support views? 
Views are available on most but not all pages in Finance and Operations. Specifically, views are currently available on all full-screen pages except for dashboards and workspaces. Non-full-screen pages (e.g. dialogs, drop dialogs, lookups, enhanced previews) also currently do not support views. View support for additional page types (e.g. workspaces and dialogs) may be considered for a future update.   

### Who is allowed to publish views?
Currently system administrators are the only users who have rights to publish views.  A new security role is planned, however, to give customers more flexibility to who can publish.  

### Why can I not save filters with this view? 
There are a few reasons why a filter may not appear to save with a view: 

1.	The page may not support saving filters as part of the view definition. Note that only pages with large view selectors (see the Switching views section) allow personalizations and query modifications to be saved as a view.  

2.	If the view is the default view and the navigation path to the page includes a query, the view’s query may not be applied initially. The two primary scenarios for this are: (a) If you navigate to a page from a tile, the tile query will execute regardless of the query associated with the default view. (b) If you navigate to a page and that entry point includes a query, the original query will execute originally in place of the default view’s query. You should be alerted with these situations occur by an informational message when the view is loading. You can also confirm by switching to this view after the page loads, as that should allow the view query to execute regardless.  

3.	The page in question may not properly support views, as it may ignore the view query completely. Please report any such instances through the Feedback mechanism. 
