...
- All the DIXF recurring import and export jobs must be fully processed and stopped in the source system prior to initiating the copy. This will ensure there are no orphaned files in the Azure storage as these cannot be processed once the db is restored in the new environment. After the restore, the integration jobs can be resumed.
