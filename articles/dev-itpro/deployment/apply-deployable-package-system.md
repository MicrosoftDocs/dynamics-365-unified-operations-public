...
- **Merged package** – A package that is created by combining one package of each type. For example, one binary update package and one AOT package, or one AOT package one Retail deployable package can be combined. The packages are merged in the Asset library for the project in LCS.
[!NOTE] Binary package and Retail deployable package cannot be included in the same merged package.
