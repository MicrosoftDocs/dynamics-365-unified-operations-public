...
The Unified Operations (Mobile Client) app facilitates user authentication with Azure Active Directory (AAD) by presenting the AAD sign-in web page within an embedded browser. After a successful sign in it will retrieve the user token from the cookies and use that when communicating with the user interaction service that it shares with the web client. Some multi-factor authentication (MFA) mechanisms that involve switching to a different app on the same device will cause the embedded browser to close, so the sign in will fail. 
The workarounds for this are:
1. Different device - Use a different device for the MFA response so the Unified Operations app remains active on the original device.
2. MFA via phone call - Use a phone call for the MFA response so an app switch is not needed.
3. Use the "touch and hold" gesture on the authentication notification and then click the **Accept** option. Because the notification acceptance will not require an app switch, the sign in will proceed as usual.
