...
-   **Notification** – A notification informs the user about events that may or may not be relate to the current user activity. A notification can be caused by a user action or a system event, or it can provide information from the program that might be useful. Typically, a notification doesn't require immediate user action. You notify the user by using the **info()** application programming interface (API).
...
> [!NOTE]
> Because of the way validation message cleanup is handled by the system, usage of the **warning()** or **checkFailed()** API should *always* coincide with setting the return value of validate() to false. Failure to do so can result in warning messages not being displayed to the user.    
