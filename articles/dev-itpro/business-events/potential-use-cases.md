...
| Scheduling batch jobs as a way to process polling logic can put a lot of constranint on the resources. To realize near real time processing of a business use case, it is compelling to schedule the batch to run as fast as it can in F&O which typically ends up being every minute or two. These kind of batch jobs are typically data import/export jobs which are looking for data to be processed but, if data is not available, they process empty cycles which is a wastage of system resources.  | Using business events, the polling use case can be re-designed to be asynchronous which is triggered by the business event. The data that must be processed must be processed only when data is available. The business logic which makes the data available is a good place to trigger the business event which can then be used to start the data processing job/logic. This will save thousands of batch executions from running empty cycles and thus wasting system resources. |
