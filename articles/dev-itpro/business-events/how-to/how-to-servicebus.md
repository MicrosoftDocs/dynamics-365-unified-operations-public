...
Azure Service Bus Endpoint with Dynamics 365 Finance and Operation and how to
consume a Business Event form Azure Service Bus.

Scenario overview
=================

Security best practices recommend storing connection strings outside of
applications application but in a Key Vault drive and giving applications the
...
Exercise 1: Create a new Service Bus Namespace
==============================================
...
3.  Click **Add** to create a new **Service Bus Namespace** then fill in the
    ...
    **resource group** as a container for your lab or use an existing one.
...
Exercise 2: Create a new Service Bus Topic and Subscription
===========================================================
...
For this exercise, you will have to create a Key Vault to store the connection
string you copied above. A key vault is a secure drive used to store keys,
secrets and certificates. Instead of storing the Connection string in F&O it is
more common and secure to store it in a key vault then register a new
application with Azure active directory that will have the right to retrieve the
secret form the key vault on behalf of F&O.
...
3.  Select **Overview** and **copy** the key vault URL **DNS Name** and save it
    for later use.

    <img src="../../media/BEF-Howto-Keyvault-03.png" width="70%">

4.  Then **select** new **BE-key vault \>Secrets\>Generate/Import**, choose a
    new name for your secret and **copy** the service bus **connection string**
    you saved earlier on exercise 2.

    <img src="../../media/BEF-Howto-Keyvault-04.png" width="70%">

5.  Click **Create**

Exercise 4: Register a new Application
======================================
...
used by F&O to retrieve Service Bus Secrets
...
Exercise 5: Configure a Business Events Endpoint in F&O
=======================================================
...
Exercise 6: Consuming a Business Event
======================================

Scenario/business problem

The business scenario is to send an email or a message to a team channel
whenever a customer payment has been posted for USMF. The message needs to
...
2.  Once you activate a Business Event with a new endpoint, F&O sends a test
    ...
    **Navigate** to Azure and select your service bus topic **BE-Topic**

3.  From there jump into your **Service Bus Subscription BE-USMF** that you
    created earlier

4.  **Verify** that the message count of your subscription is showing a value of
...
9.  **Select** trigger called **“When a message is received in a topic
    ...
    **Note:** Auto complete means that once the message is retrieved it is
    deleted from the subscription queue. Peek-lock will authorize concurrent
...
11. Then choose the **RootManageSharedAccessKey** policy for your Logic Apps and
    ...
    **Note**: The send policy can’t be used here as we want to retrieve messages
