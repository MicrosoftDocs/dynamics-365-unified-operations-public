...
> [!NOTE]
> A import or an export job can be run asynchronously by clicking the import or export button. Running in async uses the async framework in Finance and Operations which is different from batch but is a throttled resource. As a result, the job may not start to execute immediately if the async resources are being throttled in the system. The jobs can also be run synchronously by clicking import now or export now. This method of execution starts the job immediately and is useful if asymc or batch does not start due to throttling. The jobs can also be executed in batch by choosing the run in batch option. Batch resources are subjected to throttling and hence the batch job may not start immediately.
