...
- There is a 256 MB limit for the file size that can be handled via export. If there is a large number of records that will be exported, be sure that the resulting file size does not exceed this limit. An alternate way in which handle such scenarios would be to use filters on the entity to export only a subset of data. If this is not feasible, then bring your own database must be considered for the overall solution.
