...
| Area | Entity | Action |
|------|--------|--------|
| System setup | Global address book | The entity no longer exports the records that are created automatically when a company is created. Additionally, starting in Microsoft Dynamics 365 for Finance and Operations, Enterprise edition 7.2.0 with platform update 9, the import no longer accepts those records. |
| GL Shared | Account structures active group | This composite entity exports and imports only the active account structures. If you use entities from any other account structures, the status of the active account structures will be changed to **Draft**, and you must activate those account structures before they can be used. |
| | Advanced rule structures active group | This composite entity is used in combination with the active group entity of account structures. It exports and imports only the active advanced rule structures. If you use entities from any other advanced rule structures, the status of the active advanced rule structures will be changed to **Draft**, and you must activate those advanced rule structures before they can be used. |
...
| Tax | Sales tax parameters | The default value for the marginal base calculation method is **Total** for sales tax parameters. The Ledger Parameters entity doesn't set that value. However, the marginal base that some tax codes use, **Line**, will fail validation. A new entity that is named the Sales tax parameters preset entity was created so that you can import the marginal base calculation method first. You can then import tax codes. |
| Accounts receivable | Customers | The Customers entity was designed to be used for Open Data Protocol (OData) scenarios. For configurations, use the Customer definitions entities and the Customer details entity. The Customer definitions entities let you import the basic information about a customer. In this way, you enable entities that require that a customer have that information earlier in the import process. The Customer details entity contains additional information about a customer that you can add after parameters and reference data have been set up. |
| Inventory management | Warehouse locations | Some warehouse locations require a location profile ID. Location profile IDs require a location format. Currently, the location format information must be manually added before the warehouse location. The entities for the location format and location profile were added in Microsoft Dynamics 365 for Finance and Operations, Enterprise edition 7.2.3. |
...
| | Tracking number groups | The entity automatically filters the Number sequence scope data area by the legal entity. Therefore, a filter isn't required. However, if you must change the legal entity, the legal entity is stored in the table. |
| Retail | POS registers | Apply a filter to Legal entity. This entity was added to the Retail template in Finance and Operations 7.2.3. |
| | Retail store address book | There is no legal entity filter for this entity. Therefore, the export will include records for all legal entities. This entity was added to the Retail template in Finance and Operations 7.2.3. |
| | Retail locator group member | There is no legal entity filter for this entity. Therefore, the export will include records for all legal entities. This entity was added to the Retail template in Finance and Operations 7.2.3. |
| | Retail locator group owner | There is no legal entity filter for this entity. Therefore, the export will include records for all legal entities. This entity was added to the Retail template in Finance and Operations 7.2.3. |
| | Retail devices | There is no legal entity filter for this entity. Therefore, the export will include records for all legal entities. This entity was added to the Retail template in Finance and Operations 7.2.3. |
...
| Area | Entity | Action |
|------|--------|--------|
| System setup | Number sequences | If you use a number sequence for vendors and customers on the parameters pages, and you then copy customers and vendors, you must make sure that the **Allow user to change to a lower number** option on the number sequences is set to **Yes**. The **No** setting will cause the import to reject vendor and customer numbers, because they were created earlier and are lower than the next available number sequence. If you use the Reset to smallest feature, you must change the **Allow user to change to a higher number** setting, because the vendor and customer numbers are higher than the next available number sequence. |
| System setup | Workflow | A workflow requires additional changes before it can be copied. Copies of workflows aren't currently supported. |
| Accounts payable | Vendors | Vendors have many settings that depend on the values that come from other entities. For example, if you update the matching settings so that three-way matching is required, but some vendors are set up for two-way matching, those vendors will fail validation. If you use automatic sequencing for a new vendor, you must unmap the vendor account before you use the Copy into legal entity feature. In addition, if an automatically sequenced vendor has an invoice account, that vendor account isn't transformed and might fail. You might see similar issues in other areas, such as vendors in posting accounts and approved vendor lists by products. |
| Accounts receivable | Customers | Customers have many settings that depend on the values that come from other entities. For example, if you have a default warehouse and site for a customer, you must add sites and warehouses first. Otherwise, the customer will fail validation. The collections contact will also fail if the contact isn't available in the new company. If you use automatic sequencing for new customers, you must unmap the customer account before you use the Copy into legal entity feature. In addition, if an automatically sequenced customer has an invoice account, that invoice account isn't transformed and might fail. You might see similar issues in other areas, such as customers in posting accounts. |
| Budget | Budget cost elements| If you use an annual amount, and there are budget cost elements on the **Earnings basis** tab, an issue occurs when you import budget cost elements. This issue will be addressed in a future release. |
| Fixed assets | Fixed assets depreciation profile manual schedule| The fixed asset depreciation profile must be processed first. Therefore, change the sequence on your data project for the fixed asset depreciation profile from **10** to **15**. The default templates will be updated to this value in Microsoft Dynamics 365 for Finance and Operations, Enterprise edition 7.2.4. |
| General ledger | Intercompany accounting | The Copy into legal entity feature doesn't currently support intercompany accounting. This issue will be addressed in a future release. |
| General ledger | Journal names | Only the journal names for the source legal entity will be copied to the destination companies. If you select the lookup on the **Journal names** page, you will see only the number sequences that are available for that legal entity. However, if you manually enter a number sequence that isn't in that list, the number sequence won't be included in the copy process. |
| General ledger | Ledger allocation rules destination | If you have cross-company allocation rules, you won't see the destination rules for legal entities that don't match the source legal entity. The Copy into legal entity feature will copy the destination records only when the destination is equal to the source. This issue will be addressed in a future release. |
| General ledger | Ledger fiscal calendar year/period | Currently, the process exports all the legal entities instead of just the source legal entity. The copy process works correctly. This issue will be addressed in a future release. |
| General ledger | Ledger parameters | If you turn on the setting for continuous number sequences, but some number sequences that are used on journal names aren't continuous, the imports will fail. You should temporarily turn off that setting in the ledger parameters. In addition, the ledger parameters must be processed first. Therefore, change the sequence on your data project for ledger parameters from **40** to **15**. The default templates were updated to this value in Finance and Operations 7.2.3. |
| Inventory management | Inventory dimension parameters | There is an issue where an error is shown during import, but the correct number of parameters is imported. This issue will be addressed in a future release. |
| Inventory management | Warehouse locations | Some warehouse locations require a location profile ID. Location profile IDs require a location format. Currently, the location format information must be manually added before the warehouse location. The entities for the location format and location profile were added in Finance and Operations 7.2.3. |
| Master planning | Intercompany master plan associations | The Copy into legal entity feature doesn't currently support intercompany master plan associations. This issue will be addressed in a future release. |
...
| Retail | Retail store address book | This entity has a dependency on Retail channel. Therefore, it can't be used. |
| Sales and marketing | Intercompany trading partnerships | The Copy into legal entity feature doesn't currently support intercompany trading partnerships. This issue will be addressed in a future release. |
