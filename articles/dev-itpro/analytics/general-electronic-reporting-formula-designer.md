...
Note that the upper limit for the number of characters in such condition’s text is 32768 characters. Therefore, avoid creation of data sources that may exceed this limitation at run-time causing application to stop execution and throw an
exception. For example, it may happen for a data source configured as follows **WHERE (List1, VALUEIN (List1.ID, List2, List2.ID)** when the **List1** and **List2** lists contain the large volume of records.
...
Consider this option instead of using **WHERE** function as described in the sample above.

##### Example 2

You define the following data sources in your model mapping:

-   **In** (**Table records** type), which refers to the table **Intrastat**;
-   **Port** (**Table records** type), which refers to the table **IntrastatPort**.

When a data source is called being configured as the **FILTER (In, VALUEIN(In.Port, Port, Port.PortId)** expression, the following SQL statement is generated returning filtered records of **Intrastat** table:
...
Note that for **dataAreaId** fields the final SQL statement will be generated by the using **IN** operator.
...
-   **In** (**Table records** type), which refers to the table **Intrastat** and for which the **Cross-company** option is turned on;

When a data source is called being configured as the **FILTER (In, VALUEIN (In.dataAreaId, Le, Le.Value)** expression, the final SQL statement will contain the following condition:
