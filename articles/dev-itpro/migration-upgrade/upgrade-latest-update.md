...
2. If you have executed the Data Upgrade step but not yet Committed the upgrade, then **no actions will be performed** and no environments will be deleted.  You can remain here until you commit or rollback.  Should you decide to rollback and the timer is at zero, then the **new environment** will be deleted.

Only after you commit the upgrade as a success, is the original environment queued for deletion.

#### Commit or rollback

After the data upgrade package is applied, you can review the environment, and your users can perform business validation activities. If this validation is successful, you can mark the whole upgrade as a success by selecting **Commit** on the **Upgrade** menu. You must commit the upgrade to move on to your production environment.  Once you Commit, the original environment is queued for deletion.
