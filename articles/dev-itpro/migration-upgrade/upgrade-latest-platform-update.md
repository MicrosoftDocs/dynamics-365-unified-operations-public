...
[!Important] 
> Tiles on an environments page in LCS show the updates that are applicable to your environment based on the version you have deployed. 

### Import the platform update package
Get the latest platform update package using one of the two options above. Click the tile and after reviewing the KBs included in the platform, click on **Save Package** button to save the package to the project asset library.
