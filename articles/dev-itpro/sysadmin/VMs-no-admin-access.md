...
### Build environments
LCS does not allow installing AOT or license deployable packages on build environments. In this case, remote into the VM and use the **-devinstall** option to install a license deployable package from the command line as described in [this topic](../deployment/install-deployable-package.md). This works as of platform update 17. If you are running on a platform version that is older than platform update 17, and you do not have admin access to your build environment, create a service request of type "Other" to have Microsoft install your license deployable package.
