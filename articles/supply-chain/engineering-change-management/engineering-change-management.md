...
The engineering change request process lets you capture requests for changes from all the relevant departments in your company. It doesn't matter whether you work as an engineer, or in the Manufacturing, Sourcing, Warehouse, or Sales department: anybody can use an engineering change request to request a change. This change might be an idea for a new product, an issue that you discovered while working with an existing product, a suggestion for improving an existing product, or something else.

After someone submits a request for change, a workflow manages the *review and approval* process and identifies who must approve the change (for example, the product owner).

To set up a workflow for engineering change orders or engineering change requests, go to **Engineering change management > Engineering workflows**. Select **New**, select whether the workflow reviews engineering change orders or engineering change requests, and then configure the workflow.
...
| Restriction | A value that indicates whether the information that you added came from an internal or external source. |
...
The **Products** FastTab lets you list each product that the change request affects. Use the buttons on the toolbar to add products to the grid or to remove products.
...
When you review a request for change, search for dependencies. This approach helps you assess the impact of the requested change on open transactions, such as sales orders, production orders, and on-hand inventory. As you review change requests, send notifications to the people responsible for fulfilling the various types of related orders.
...
1. On the Action Pane, on the **Change request** tab, in the **Business impact** group, select one of the following buttons:
    ...
    - **Search** – Scans all open transactions, and then opens the **Business impact to open transactions** dialog box, which lists all transactions that the change affects.
    - **View previous search** – Opens the **Business impact to open transactions** dialog box, which lists the results of the previous search. (A new search isn't done.)
...
1. To work with a transaction in the list, select it, and then select one of the following buttons on the toolbar:
    ...
    - **View transaction** – Opens the selected transaction record.
...
- To view all production orders that an engineering notification is sent for, go to **Production orders > Production orders > All production orders**. Then, on the Action Pane, on the **Production order** tab, in the **Engineering change request** group, select **Engineering notifications** to open the **Engineering notifications** page.
...
As you're reviewing production order notifications on the **Engineering notifications** page, you can follow links to related change orders or production orders by selecting column values or using the related commands on the Action Pane. After you finish evaluating a change, and after you cancel or modify production orders as required, you can mark a notification as resolved. Select the notification, and then, on the Action Pane, select **Resolve**. The notification is removed from all users' views.
...
An engineer who reviews an engineering change request can create an engineering change order directly from the **Engineering change requests** page. On the Action Pane, on the **Change request** tab, in the **Engineering change order** group, select **Copy link and products**.

Select the correct company for the new engineering change order. If the change order results in the engineering product itself being changed (new version, new product, or new variant), assign the change order to the engineering company. If only a local change is needed (**Impact** is set to *None*), assign the change order to a local company and the changes apply to the current product.

## Engineering change orders

Engineering change orders provide a structured process for making changes to engineering products. Propose changes by using a copy of the engineering-relevant data. The real master data isn't affected. For more information about engineering-relevant data, see [Engineering versions and engineering product categories](engineering-versions-product-category.md).
...
After the proposal for changes is completed, a workflow handles the review and approval process. You can set up different workflows, based on priority and severity.
...
You can choose whether to process the engineering change order directly after it's approved, as part of the workflow, or whether to process it later as a manual step. During processing of an engineering change order, engineering data on the actual product is updated.
...
As described in [Engineering companies and data ownership rules](engineering-org-data-ownership-rules.md), the product data that you can edit varies depending on the type of legal entity that you're working in (an engineering company versus an operational company). Data ownership rules also apply to engineering change orders. Therefore, depending on the legal entity where you create an engineering change order, you can make different types of changes. Here are some examples:
...
- For engineering change orders in an *engineering company*, you can make basic changes to the engineering data. For example, you can create new versions of a product, change a product's structure via the BOM, and change engineering attribute values. For each affected product, select one of the following values in the **Impact** field:
    ...
    - **New variant** – Create a new variant based on the selected product version. Its BOM and route information are copied.
...
Engineering change order reports describe the changes made in an engineering change order. They're useful both during and after the review and approval process.

To view an engineering change order report, open the relevant change order. Then, on the Action Pane, on the **Change order** tab, in the **View** group, select **Engineering change order report**.
...
| Environmental health and safety | Specify whether environmental health and safety rules are applicable to the change. If they are, select the applicable rules. |

Use the **Maintain/copy change information** button to copy change information between affected products.
...
- Approve engineering change order product routes
