...
- Bill of material (BOM) for engineering products
...
| Production type | <!-- KFM: Confirm these descriptions. --> This setting only appears when you have enabled [formula change management](manage-formula-changes.md) on your system. Select the type of production this engineering product category applies to. Select one of the following<ul><li>**Planning item** – Use this engineering category to do formula change management for planning items. Planning items use formulas and are similar to formula items, but are only used to produce co- and by-products, not finished products. Formulas are used during process manufacturing.</li><li>**BOM** – Use this engineering category to manage engineering products, which do not use formulas and typically (but not necessarily) include bills of materials (BOM). </li><li>**Formula** – Use this engineering category to do formula change management for finished products. These items will have a formula, but not a BOM. Formulas are used during process manufacturing.</li></ul>. |
| Catch weight | This setting only appears when you have enabled [formula change management](manage-formula-changes.md) on your system. It is only active when **Production type** is set to *Planning item* or *Formula*. Set this to *Yes* if you will use this engineering category to manage items that require catch weight support. <!-- KFM: Confirm this description. Does this only apply to planning items? It seems like the setting is also active for formula items. --> |
