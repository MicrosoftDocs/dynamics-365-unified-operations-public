...
1. **Create return order** is used to process *Quantity-based* claims. When a quantity-based claim is approved, and this setting is set to:
...
1. **Allow unapproved** – Select if return orders can be created before the claim has been approved. Only applicable to quantity-based claims where **Create return order** is set to *Yes*.
...
1. **Claim posting account** – Only enabled where **Claim basis** is set to *Price based*. This ledger account will populate the draft free text credit note's **Main account** when the price-based claim is approved.

### (Optional) Settle approved deductions periodic task
...
> [!NOTE]
> If the **Automatic settlement** parameter on the Settlement tab in **Account receivable \> Setup \> Accounts receivable parameters** is set to *Yes*, the **Settle approved deductions** will not work as the credit will have already been automatically settled.
...
### Create a deduction in the deduction workbench
...
1. Select the **Claim basis**. After the deduction/claim has been *approved* the claim basis will determine the type of credit transaction created. Select one of the following values:
...
1. Select *one* **invoice** against which the deduction will be created.
...
1. **Settle transactions** form opens with selected invoice marked. The **Post** button will only be enabled if the **Create claim journal** was set to *Yes.* Select **Post**. This will reduce the balance on the invoice by the **Claim amount**.

### Create a deduction in customer
