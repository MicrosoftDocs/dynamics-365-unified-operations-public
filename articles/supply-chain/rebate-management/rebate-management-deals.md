...
| Process at posting | A selected option indicates that the claim line is processed at posting time. This option is available only for deal lines where the **Transaction type** field on the **General** tab is set to *Delivered* or *Invoice*. If it's a provision, it will be posted when the delivery note or invoice is generated. |
...
The **Lines** tab of the **Rebate management details** FastTab lets you define calculation lines for the selected deal line. Each calculation line establishes a quantity or value threshold, and a related compensation amount/items. The **Basis** field on the **General** tab defines the denomination of each calculation line (quantity or value).
