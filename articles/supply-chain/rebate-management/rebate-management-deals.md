...
The **Lines** tab of the **Rebate management details** FastTab lets you define calculation lines for the selected deal line. Each calculation line establishes a quantity or value threshold, and a related compensation amount or items. The **Basis** field on the **General** tab specifies whether each calculation line is based on a quantity or a value.
...
1. On the **Items** page, use the buttons on the Action Pane to add items to the grid, or remove items, as required. For each row, set the following fields:
    ...
    - **Multiple** – This field works with the **Quantity** field. For example, on an item line, you set the **Quantity** field to *2* and the **Multiple** field of *100*. If you then have a total sales order quantity of 150, two of the items will be free (two per 100).
