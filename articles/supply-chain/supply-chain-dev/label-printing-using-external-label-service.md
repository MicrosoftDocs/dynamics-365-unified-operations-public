...
:::image type="content" source="../warehousing/media/print-labels-using-dra.png" alt-text="Screenshot of printing labels by using the DRA." lightbox="../warehousing/media/print-labels-using-dra.png#lightbox":::

**Printing labels by using an external service**

:::image type="content" source="../warehousing/media/print-labels-using-external-service.png" alt-text="Screenshot of printing labels by using an external service." lightbox="../warehousing/media/print-labels-using-external-service.png#lightbox":::
...
- You must design your labels using the newer *label layouts* feature instead of the older *document routing label layouts* feature. See the next section for guidelines.

## Use label layouts instead of document routing label layouts

Classic *document routing label layouts* treat the layout body as plain text and apply string escaping, which can produce malformed JSON or XML payloads. Therefore, you must use the newer *label layouts* when you print labels through an external service.

- To view and create layouts that use the new *label layouts* feature, go to **Warehouse management** > **Setup** > **Document routing** > **Label layout**. Use these layouts when you print labels through an external service.
- To see the older *document routing label layouts*, go to **Warehouse management** > **Setup** > **Document routing** > **Document routing layout**. Don't use these layouts to print labels through an external service.

Learn more about how to work with label layouts in [Container label layouts and printing](../warehousing/print-container-labels.md), [License plate label layouts and printing](../warehousing/print-license-plate-labels-using-label-layouts.md), and [Custom label layouts and printing](../warehousing/custom-label-layouts-and-printing.md).

To enable label layouts for license plate labels, follow these steps:

1. Go to **Warehouse management** > **Setup** > **Warehouse management parameters**.
1. Open the **General** tab.
1. On the **License plates** FastTab, set **Use label layouts for license plate labels** to *Yes*.

If you need to migrate existing document routing label layouts, use the [Data entities framework](../../fin-ops-core/dev-itpro/data-entities/copy-configuration.md) to export and then import them by using the following entities:

- **Document routing printer layouts** — to export your existing document routing layout definitions, including the template body content.
- **Label layouts with active versions** — to import and export label layout definitions.
- **Label layout data sources** — to import and export custom data source configurations used by label layouts.

## Overview of required configurations

To enable label printing through an external service, set up the following elements:

- **[External service definition](#service-definition)** – Defines how to call the external API. The definition consists of one or more operations.
- **[External service instance](#service-instance)** – Represents a specific deployment of the service described by the external service definition. Specifies the URL of the service and the authentication secret to use. For each external service definition, you can use one or more instances. For example, you can set up multiple warehouses, each of which has its own on-premises installation of labeling software.
- **[Label printers](#label-printers)** – Defines the label printers that the labeling service manages.
- **[Document routing setup](#document-routing)** – Defines which printer is used for which layout in a specific business process.
...
An external service definition specifies how to call a specific service. This configuration depends on the API definitions that the external service supports. Typically, the external service provider supplies these API definitions.
...
1. Set the following fields for the new service definition:
    ...
    - **External service definition** – Enter a name for the definition, such as *External*.
    - **Description** – Enter a short description of the definition, such as *External*.

1. On the Action Pane, select **Save**.
1. On the **External service operations** FastTab, select **Edit operations** on the toolbar.
1. The **External service operations** page appears. External service operations define how the system formats its HTTP requests to the external service. You must add at least one operation, which the system uses to print labels. On the Action Pane, select **New** to add a new operation, and then follow these steps:
    ...
    - On the **General** FastTab, set the following fields:
        ...
        - **Operation timeout** – Select the timeout period for the operation, in milliseconds (for example, *500*).
...
1. On the **Label print service** tab, set the following fields:
    ...
    - **Print operation** – If you created an operation for printing ZPL-based layouts, select the name of that operation. The operation must already be defined for the current service definition, as described earlier in this procedure.
    - **Variables print operation** – If you created an operation for printing variable-based layouts, select the name of that operation. This operation must already be defined for the current service definition, as described earlier in this procedure.
...
1. Set the following fields for the new service instance:
    ...
    - **External service instance** – Enter a name for the instance, such as *External*.
    - **Description** – Enter a short description of the definition, such as *External*.
...
1. Set the following fields for the new printer:
    ...
    - **Batch print** – Select this checkbox to enable the system to send multiple labels to the selected printer as a single batch job. Clear this checkbox to send one label at a time. Batch jobs print labels all at once, without being interrupted by other print jobs.
    - **Maximum file size** – If you enabled batch printing for the printer, enter the maximum size, in megabytes, for batch files that are sent to it. The value must be more than 0 (zero). Batches that exceed the maximum size are split into smaller batches before they're sent. A large file size lets you include more labels in each batch. However, large batches might take a long time to print and therefore delay other jobs. Alternatively, they might fail if they exceed the memory capacity of your printer. To find the maximum supported value, see your printer's specifications.
    ...
    - **Label print service execution policy** – If the **Connection type** field is set to *Document routing agent/Hybrid*, select one of the following values to specify what the system should do if a request to the external service fails:
        ...
        - *Fallback to the Document Routing Agent* – The system tries to print the label by using the DRA.
...
Part of this JSON request body is generated by the label layout and includes data for a specific label. It's embedded in the external service operation's request body definition, which specifies a printer and other data that isn't specific to the label. Because the printer is provided by the `$label.printer` substitution, this part of the request goes into the external service operation. Because the label file and the variables are specific to each label, this part of the request goes into the label layout.
...
If you want to experiment with printing labels, you can set up a scenario for printing location labels. For more information, see [Custom label layouts printing](../warehousing/custom-label-layouts-and-printing.md). Follow the instructions in this article, and confirm that the scenario is supported.

## <a name="review-log"></a>Review the external service request log

The external service request log contains information about each request that the system sends to an external service. It can help you troubleshoot problems.

The amount of detail that the log records depends on the **Log level** and **Log request bodies** settings that you specify for each external service instance. Learn more in the [External service instance configuration](#service-instance) section earlier in this article.

For each request, the system checks if the external service returns the HTTP success code (200). If the external service returns a different HTTP code, such as 4xx or 5xx, the system reports an error, provided that you enable the relevant logging options.

To review the request log that's generated while labels are printed to the external service, follow these steps:
