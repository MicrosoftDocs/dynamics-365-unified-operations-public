...
You might use this framework, for example, to develop and manage custom integration with external systems, and to process other custom functionality. Microsoft Dynamics 365 Supply Chain Management includes, for example, several out-of-the-box features that use predefined message types and message queues, including: [third-party manufacturing execution system (MES) integration](../production-control/mes-integration.md), [deferred posting](../production-control/deferred-posting.md), and [packing slip posting during container close](../warehousing/packing-containers.md#set-up-the-packing-process). [Warehouse management only mode](../warehousing/warehouse-management-only-mode.md) uses the message processor framework to manage *inbound and outbound shipment orders*.
...
- **Message queue** – The name of the queue that the message will be processed in. The following queues are provided:
    ...
    - *Shipment Orders* - This queue holds messages that support [Warehouse management only mode](warehouse-management-only-mode.md).
