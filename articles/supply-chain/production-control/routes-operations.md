...
- **Working time** – Select this option to schedule the job type according to the [working time calendar](../master-planning/supply-chain-calendars-master-planning.md) that is defined for the operations resource, otherwise the Gregorian calendar is used. Working time can be scheduled either according to the Gregorian calendar or the defined working calendar. If you select this option, scheduling is based on the defined working time calendar. Additionally, the job of the job type is scheduled from midnight on the date that is defined as the job's starting date.
