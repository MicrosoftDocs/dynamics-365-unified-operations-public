...
description: When a worker reports a manufactured item as finished, it's registered as available for further physical processing, and one or more journals are posted.
...
1. On the **Journals** tab, in the **Report as finished journal** section, set the **Posting method** field to one of the following values:
    ...
    - *Immediate* – When a worker reports an item as finished, the system fully processes all related journal postings before it marks the finished item as physically available.
    - *Deferred* – When a worker reports an item as finished, the system marks the finished item as physically available and adds the journal postings to a message queue. The system doesn't wait until the postings are fully processed before it marks the finished item as physically available.

Use the following procedure to configure warehouse-specific overrides for the default posting method that you set on the **Production control parameters** page.
...
Use the following procedure to configure the posting method that the system uses when a worker reports items as finished from the production floor execution interface.
...
The message processor batch job processes the journal postings after the system queues them. To ensure that your journal postings are processed, you must configure this job to run at a regular interval. Use the following procedure to set up the required batch job.
...
Deferred journal postings queue as *message processor messages* that wait until the *message processor* processes them. Set up the message processor to run as a scheduled batch job. To view the deferred posting messages that the message processor has processed or will process, go to **Production control \> Production orders \> Deferred production order posting**.

### Message grid columns and filters

Use the fields at the top of the **Deferred production order posting** page to find specific messages.

The following filters are available:

- **Message type** – Specify the type of messages that appear in the grid.
- **Message state** – Specify the state of the messages that appear in the grid. The following states exist:
    ...
    - *Failed* – The message failed to be processed during the last attempt. The system retries failed messages three times. It then gives up and leaves the message in the *Failed* state. You can't manually cancel or edit a message until after the last of these three attempts.

- **Message content** – This filter does a full-text search of message content. (Message content isn't shown in the grid.) The filter treats most special symbols (such as hyphens) as space characters, and it treats all space characters as Boolean OR operators. For example, if you search for a specific `journalid` value that equals *USMF-123456*, the system finds all messages that contain either "USMF" or "123456." In this case, the list of results is likely to be long. Therefore, it's better to enter just *123456*, because you get more specific results.
...
Find detailed information about a message by selecting it in the grid and then selecting the **Log** or **Raw content** tab under the message grid, where each processing event is shown.
...
You can manually process or cancel a message, depending on its current state. Select the message in the grid, then select **Process** or **Cancel** on the Action Pane.

### Set up business events to deliver alerts for failed processing results

You can set up [business events](../../fin-ops-core/dev-itpro/business-events/home-page.md) that alert you about failed processing results. Go to **System administration \> Setup \> Business events \> Business events catalog**, and activate the business event named *Message processor message processed*.

As part of the activation process, you're prompted to specify whether the event is specific to one legal entity or applies to all legal entities. You're also prompted to provide an **Endpoint name** value. You must define this value first.
