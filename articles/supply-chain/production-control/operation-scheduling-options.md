...
- Start and end dates for the production order and each operation.
- Resources assigned to operations.
...
The scheduling direction is fundamental to the scheduling process. You can schedule a production forward or backward from any date, depending on timing and scheduling requirements.

- **Forward scheduling** – Schedule a production to start as early as possible. The production can start today, tomorrow, or on any specific date in the future. The production is scheduled to start on the earliest possible date and is planned forward in time to the earliest possible end date.
- **Backward scheduling** – Schedule a production to start as late as possible. The schedule is based on the date when the production must be completed and counts backward to the latest possible date that the production can start without missing its target deadline.
...
- **Forward from scheduling date** – Schedule forward from the date that you specify in the **Scheduling date** field. If you don't specify a scheduling date, the scheduling direction is forward from the current date.
- **Backward from delivery date** – Schedule backward from the delivery date that you specify for the production order. If you select this option, but don't specify a delivery date, the delivery date is the current date.
...
- **Backward from scheduling date** – Schedule backward from the date that you specify in the **Scheduling date** field. If you don't specify a scheduling date, the current date is used. Backward from scheduling date is calculated for the production order the last time that a requirement was calculated. If you don't specify a date for the production order, the current system date is used.
- **Backward from futures date** – Schedule backward from the futures date that was calculated for the production order the last time that a requirement was calculated. If you don't specify a futures date for the production order, the current system date is used.
...
When references depend on production orders, they're also known as subproductions. You can schedule subproductions as part of the scheduling of a production order. Select this check box if the scheduling of subproductions should be based on the scheduling of the main production. In relation to the main production, overlying productions are scheduled forward, and underlying productions are scheduled backward. You can view production order references in the **Reference level** field on the **Production orders** page.

### Synchronize references

You can synchronize references with the production order. If you select this option, the system moves and aligns the dates of the subproductions when you make changes to the production order’s schedule. If a production order has one or more subproductions, you might want to schedule the subproductions together with the main production. In this case, you can't start the main production until you complete the related subproductions. Therefore, select this check box if you want the scheduling of subproductions to be based on the start and end times of the selected production. You can select this check box only if you also select the **Schedule references** check box.
...
You can save the current values as default values. Two options are available:
