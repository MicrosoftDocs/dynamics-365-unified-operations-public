...
description: Learn about Croston's method, which is a forecasting algorithm designed for intermittent demand.
...
As with naive forecasting, Croston's method is a fallback option. Demand planning applies this method when the available data doesn't meet the requirements for standard forecasting models. Using Croston's method within a dimension combination indicates that the data you provided might be insufficient.
...
- You use the *Best fit model - version 3 (preview)* (or newer) forecasting algorithm. Croston's method isn't available for other forecast algorithms or as a separate algorithm.
...
The forecast stays stable and realistic, without reacting to random fluctuations or zero-demand periods — giving planners a clearer view of what to expect over time.
...
- Forecasting with a seasonal hint forces the best fit algorithm to not consider Croston's method and results in the selection of one of the other algorithms.
- The method doesn't predict *when* the next demand will occur (only the average interval), nor does it give a probability distribution of demand sizes. On the contrary, it generates a flat line that ensures a stable and realistic forecast.
