...
- Croston's method isn't a forecasting model that you can choose. The system uses it as a fallback for datasets when most data points are zero. [Check the explainability tables](forecast-profiles.md#review-forecast-job-run-history) for your forecast jobs to see which planning objects use Croston's method and consider improving the input data quality where possible.
