...
Before using this feature for the first time, configure settings in the following three areas:

- **[cXML parameters](#cxml-parameters)** – Set up global parameters for the functionality for sending purchase orders.
- **[Vendor setup](#vendor-setup)** – If you want to use commerce eXtensible Markup Language (cXML) by default for all new purchase orders created for any vendor, set the **Send purchase order via cXML** option to *Yes* for that vendor.
...
:::image type="content" source="media/cxml-settings-areas.png" alt-text="Screenshot of areas for setting up cXML features.":::

In addition, set up the [Purchase order request batch job](#po-batch). This batch job sends the confirmed purchase orders.
...
:::image type="content" source="media/cxml-parameters.png" alt-text="Screenshot of the cXML parameters page.":::
...
:::image type="content" source="media/cxml-order-defaults.png" alt-text="Screenshot of default settings for vendor purchase orders.":::
...
> [!NOTE]
> When you confirm a purchase order that can be sent via cXML, the system looks up the vendor that is associated with the purchase order and then finds the first active external catalog that is associated with that vendor. The system then uses the settings from that external catalog to send the purchase order. If multiple external catalogs are set up, the system uses only the first external catalog that it finds, based on the vendor on the purchase order. Therefore, create just one external catalog for each vendor.

:::image type="content" source="media/cxml-supplier-catalog.png" alt-text="Screenshot of external catalog settings.":::
...
If you use the catalog for PunchOut, [set up the message format](set-up-external-catalog-for-punchout.md). The message format establishes the connection to the vendor in the PunchOut transaction from the requisition. When a purchase order is sent, the order properties establish the connection with a vendor.
...
- **STREETLENGTH** – Enter a number that represents the maximum number of characters that the vendor will accept as a street name. If a value is entered here, it overrides the value that is specified on the **cXML parameters** page. The system removes line breaks and spaces to try to fit the standard address in Supply Chain Management into the number of characters that is specified here. Any additional characters are truncated.
...
- **RESPONSETEXT** – Enter any custom text that you expect the vendor to return in the cXML response message after the order is sent. In this way, the system can mark the message as *Acknowledged*. If the response doesn't match standard text or the customer text that you enter here, the request is marked as *Error*.
...
- **CONTENTTYPE** – In a typical catalog setup, you don't have to set this property. If you receive a server 500 error from a vendor's system when you send a purchase order, you can do testing by setting this property to *FALSE*. That value changes a setting in the web request and might enable the message to be sent for some platforms.
...
- **ENABLEMANUALPO** – If this property is set to *TRUE*, when users manually create purchase orders (that is, when they don't start from a requisition), those purchase orders inherit the setting of the **Send purchase order via cXML** option from the vendor. If this property isn't set, or if it's set to *FALSE*, the **Send purchase order via cXML** option isn't set on the purchase order header for manually created purchase orders. For purchase orders that are created from a requisition, the setting of the **Send purchase order via cXML** option is always inherited from the vendor, regardless of the setting of this property. Learn more in the [Set up vendor purchase orders to use cXML](#vendor-setup) section earlier in this article.
- **PUNCHOUTPOONLY** – If this property is set to *TRUE*, only purchase requisition lines that are created from the PunchOut process set the **Send purchase order via cXML** option on the purchase order header. In addition, the purchase requisition line type of all lines on the purchase order must be *External catalog item*. Otherwise, the cXML purchase order can't be created.
- **PUNCHOUTSHIPTO** – If this property is set to *TRUE*, the default address of the legal entity is added to the PunchOut setup request message when the user opens an external catalog. This address is added as the **ShipTo** address. Vendors use the **ShipTo** address to show pricing based on the company location.
- **TRACEPUNCHOUT** – Set this property to *TRUE* if you receive an error message when you try to browse to an external catalog from the requisition. Trace information is then filled in for the **PunchOutSetupRequest** and **PunchOutResponse** messages that are sent between Supply Chain Management and the vendor site. You can view this information on the **cXML cart message log** page, which you can open from the **External catalog setup** page for the vendor catalog that you're having issues with. Set this property to *TRUE* only if you're troubleshooting, because it creates a large performance overhead on the database for each PunchOut. Learn more in the [View the cXML cart message log for external catalog PunchOut](#message-log) section later in this article.
- **TRACESEND** – Set this property to *TRUE* to collect more information about purchase orders sent using cXML. To [view the collected information](#view-vendor-response), go to the **Purchase order request** page. Set this property to *TRUE* only while troubleshooting cXML functionality because it generates a large performance overhead on the database for each purchase order that you send.
...
After you set up all the properties and configure other settings for your external catalog, return to the **General** FastTab of the **External catalogs** page, and set the **Active** option to *Yes*.

### <a name="attach-po-notes"></a>Attach notes to a purchase order

As mentioned in the [Set the order properties](#set-order-properties) section, if you want your delivered cXML to include text from notes attached to the relevant purchase order and/or vendor records, set the **POCOMMENTS** and **VENDCOMMENTS** properties to *TRUE* in the external catalog setup. This section provides more detail about how the system selects and processes these attachments.

To set the types of notes that the system looks for, go to **Procurement and sourcing > Setup > Forms > Form setup**. Then, on the **Purchase order** tab, set the **Include documents of type** field to the type of note that you want to include. Only text notes are included, not document attachments.

:::image type="content" source="media/cxml-form-setup.png" alt-text="Screenshot of the Form setup page.":::

Attachments are included with a purchase order only if their **Type** field matches the value that you select in the **Include documents of type** field, and if their **Restriction** field is set to *External*. To create, view, or edit the attachments for a purchase order, go to **Procurement and sourcing \> All purchase orders**, select or create a purchase order, and then select the **Attachments** button (paperclip symbol) in the upper-right corner.

:::image type="content" source="media/cxml-note-to-vendor.png" alt-text="Screenshot of attached note that is set up to be sent to a vendor.":::
...
To open the log for an external catalog, select the relevant catalog, and then, on the Action Pane, select **cXML cart message log**. The **cXML cart message log** page shows a list of the carts that are returned, the XML that is related to those carts, and the lines that are created on the related purchase requisition.

:::image type="content" source="media/cxml-cart-message-log.png" alt-text="Screenshot of the cXML cart message log page.":::

## Set the extrinsic elements for external catalog PunchOut

When you set the **Punchout protocol type** field to *cXML* for an external catalog, you can add custom extrinsic elements that are inserted at the correct place in the request XML message.
...
1. On the **Message format** FastTab, in the **Extrinsics** section, select **Add** to add a row to the grid for each extrinsic element that you want to include. On each row, set the following fields:
    ...
    - **Name** – Enter a name for the element. This value becomes the value of the **name** attribute for the **Extrinsic** XML element that each row creates. Usually, you work with your vendor to agree on the name for each extrinsic element.
    - **Value** – Select a value for the element. This value becomes the value of the XML element that each row creates. The following values are available:

        - **User name** – Use the name of the user who is doing the PunchOut. This name is the sign-in name for Supply Chain Management.
        - **User email** – Use the email address of the user who is doing the PunchOut. This address is the address that the user sets up on the **Account** tab of the **User options** page.
...
:::image type="content" source="media/cxml-extrinsics.png" alt-text="Screenshot of extrinsic element settings.":::

The user or admin doesn't see the extrinsic elements, because they aren't added until the user does a PunchOut. They're automatically inserted between the **BuyerCookie** and **BrowserFromPost** elements in the cXML setup request message. Therefore, you don't have to set them manually in the XML when you set up the external catalog.

:::image type="content" source="media/cxml-extrinsics-xml.png" alt-text="Screenshot of extrinsic elements added to the XML.":::

## <a name="create-po"></a>Create and process a purchase order

When you create a purchase order for a vendor, the purchase order inherits the setting for the **Send purchase order via cXML** option from that vendor. However, the setting remains available on the **Setup** FastTab in the **Header** view of the purchase order, so that you can change it later as required.

:::image type="content" source="media/cxml-purchase-order.png" alt-text="Screenshot of purchase order set to use cXML.":::

When you create a purchase order from a purchase requisition that came from a PunchOut flow, all the required line details are filled in. You can then manually add purchase order lines or copy them from other purchase orders. Be sure to set all required fields. These required fields include the external reference number, which is the vendor number that used in the cXML message.

:::image type="content" source="media/cxml-line-details.png" alt-text="Screenshot of an example of an external reference number.":::

When you finish filling in all the details for the purchase order, confirm it. The system doesn't send a message unless you confirm the purchase order. To confirm a purchase order, on the Action Pane, on the **Purchase** tab, in the **Actions** group, select **Confirm**.

After you confirm the purchase order, you can view the status of the confirmation through the **Purchase order confirmations** journals. On the Action Pane, on the **Purchase** tab, in the **Journals** group, select **Purchase order confirmations**.

Each purchase order can have many confirmations. Each confirmation has an incremental number. In the following illustration, the purchase order is *00000275*, and the confirmation is *00000275-1*. This numbering reflects standard Supply Chain Management functionality, where changes in a purchase order, and therefore the type of cXML message that should be sent to the vendor, are identified based on the confirmation. As the illustration shows, the **Purchase order confirmations** page also includes **Order send status** and **Order request vendor status** fields. For more information about the various status values that you might see on this page, see the [Monitor purchase order requests](#monitor-po-requests) section later in this article.

:::image type="content" source="media/cxml-po-confirmations.png" alt-text="Screenshot of the Purchase order confirmations page.":::
...
:::image type="content" source="media/cxml-po-request.png" alt-text="Screenshot of the Purchase order request page.":::

If the batch job is set up and running, the document will be sent. You can view the status change after the document is sent. In the following illustration, the **Order send status** field is set to *Sent*. The **Order request vendor status** field is set to *Acknowledged* to indicate that the vendor received the document, and was able to read it and store it in its system. The grid on the **Purchase order request history** tab shows the time when the document was sent. For more information about the various status values that you might see on this page, see the [Monitor purchase order requests](#monitor-po-requests) section.

:::image type="content" source="media/cxml-po-request-2.png" alt-text="Screenshot of status messages on the Purchase order request page.":::

## <a name="po-batch"></a>Schedule the purchase order request batch job

To activate the batch job for sending purchase order requests, go to **Procurement and sourcing > Setup > cXML Management > Purchase order request**. On the Action Pane, on the **Purchase order request** tab, in the **Batch** group, select **Submit job** to open the **Purchase request prepare and send** dialog box. Use this dialog box to set up the recurrence, just as you usually do for batch jobs in Supply Chain Management. Select an interval based on your order volume. Although you can run the batch job every minute, it's probably best to send batches throughout the business day, based on order receipt windows that match your vendors' schedules.

For example, your vendor has a policy that states that all orders that are received by 1 PM are shipped on the same day. In this case, you might run the batch only a few times during the morning to communicate any orders that you have. The remaining orders are sent the next day. This decision is purely a business decision. You can review it and set the parameters for it accordingly.

The process looks for purchase order request documents that have a status of *Waiting*. If you have an order that you must send to a vendor immediately, you can select **Submit job** and set the **Batch processing** option to *No*.

## <a name="monitor-po-requests"></a>Monitor purchase order requests

### View the status of a purchase order

When orders that can be sent via cXML are confirmed, they go into *Waiting* status. As described in the [Create and process a purchase order](#create-po) section, you can view the purchase order status on the **Purchase order request** page. Each purchase order request can have one of several statuses, depending on its parameters and data. This section describes the various status types and the values that they can have. This information can help you manage issues and understand the status of your purchase orders.

:::image type="content" source="media/cxml-monitor-po-request.png" alt-text="Screenshot of purchase order status on the Purchase order request page.":::
...
- **Order send status** – This field can have one of the following values:
    ...
    - **Sent** – The document is sent.
    - **Stopped** – The document is marked as *Stopped* before it's sent. To mark a document as *Stopped*, select **Stop** on the Action Pane of the **Purchase request** page.
    - **Archive** – The document is purged. To purge a document, select **Purge** on the Action Pane of the **Purchase request** page.
...
- **Re-submit count** – The number of times that the record is resubmitted. A high number indicates multiple failures, and therefore might indicate an issue with either your data setup or your vendor's data setup.

### View the XML for a purchase order request message

To view the XML for the purchase order request message, select the **Request XML text** tab at the bottom of the **Purchase order request** page. The information on this tab can help during testing or error validation. To make the information easier to read, you can view it as a formatted message. Copy the contents of the tab to a text file, and then view it in an XML editor.

:::image type="content" source="media/cxml-request-xml-text.png" alt-text="Screenshot of the Request XML text tab.":::
...
:::image type="content" source="media/cxml-response-xml.png" alt-text="Screenshot of the Response XML tab.":::
