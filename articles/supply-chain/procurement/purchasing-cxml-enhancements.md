...
To turn on the feature, go to the [**Featuren management**](../../fin-ops-core/fin-ops/get-started/feature-management/feature-management-overview.md) page and search for the feature named &quot;Purchasing cXML Enhancements&quot;. Highlight the feature and then select **Enable now** to enable feature.
...
- **Delete** - Delete the currently selected property. If you accidently delete a default property from the list, then select **Default properties** to restore all the missing properties.
...
To create, view, or edit attachments for a purchase order, go to **Procurement &amp; Sourcing \> All Purchase Orders** and then select or create a purchase order. Select the purchase order and then select the **Attachments** icon at the right side of the Action Pane.
 ...
 ![](RackMultipart20200803-4-1bg0967_html_a77908fd5e3179af.png) <!--KFM: Get this image. -->
...
To open the log for an external catalog, first choose the right catalog, and then select **cXML cart message log** on the Action Pane. You will then see a list of the carts that have been returned, the XML related to the those carts, and the lines that were created on the related purchase requisition.

![](RackMultipart20200803-4-1bg0967_html_a20bfeeef18c0819.png) <!--KFM: Get or remove this image. -->

## Set external catalogs PunchOut extrinsics

When you set up an external catalog with the **Punchout protocol type** set to _cXML,_ you will be able to set extrinsics to be inserted into the request at the correct position.

 Here are the extrinsics that you can set:

- Username - Setting the value to Username will populate with the username that is executing the PunchOut. This is the login name for Dynamics 365 Finance and Operations.
- User email - This is the email address that the user has setup under the account parameters from the user options.
- Random value - This will add a unique value.
- User - This will be the user email that comes from the user's email that is setup in User Options.
- User full name - Populates user's name from related user contact person.
- Firstname - Populates user's first name from related user contact person.
- Lastname - Populates user's last name from related user contact person.
- Phone number- This populates the primary phone number from the contact person associated with the user accessing the external catalog.

![](RackMultipart20200803-4-1bg0967_html_4dbc04732b589487.png) <!--KFM: Get or remove this image. -->

The user or admin won't see the extrinsics because they are first added when the user performs a PunchOut. They will be inserted between the **\&lt;BuyerCookie\&gt;** and the **\&lt;BrowserFromPost\&gt;** elements in the cXML setup request message, so you don't need to set them manually in the XML when you set up the external catalog.

![](RackMultipart20200803-4-1bg0967_html_336a53fc478ee522.png) <!--KFM: Get or remove this image. -->

![](RackMultipart20200803-4-1bg0967_html_f13a7f450c784dd1.png) <!--KFM: Get or remove this image. -->
...
- **Order request type** will have one of the following values:
  ...
  - **Delete** - If a confirmation has already been sent and acknowledged by the supplier and the purchase order is cancelled, then the confirmation waiting will be marked as _Delete_. Deletes will only be sent they are enabled in the [global cXML Parameters](#cxml-parameters).
- **Request time** - This is the time when the order confirmation was created. You can compare the request time with the order status time to determine time between confirmation and vendor acknowledgement.
...
- **Re-submit time** - This is the time the record was re-submitted.
- **Re-submit count** - This is the number of times the record has been re-submitted. A high number indicates multiple failures, which may indicate a problem with your or your vendor's data setup.
