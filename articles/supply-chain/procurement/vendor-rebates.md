...
- Reduce the administrative burden and errors associated with monitoring promotion performance and processing claims.
...
- On the **Overview** tab, if you have lines with **Item code** set to *table* to specify the item, then the agreement is for that specific item. If you have lines with **Item code** set to *Group* or *All* to specify the items, then the vendor rebate agreement is individually processed per item qualifying for the item code, not across all items qualifying for the item code.

- On the **General** tab, in the **Unit of measure rebate option** field, you can define whether a unit of measure is a condition for the purchase order line to qualify for a rebate claim.

    - *Convert* – A purchase order line qualifies for a vendor rebate per the rebate agreement. You receive a rebate regardless of the unit of measure applied on the line.
...
- On the **General** tab, in the **Calculation date type** field, select the date used to determine whether the purchase occurs in the validity period of the rebate agreement.
...
- In the **Cumulate purchase by** field, you can set the calculation of the rebate claim. The amount can depend on a period (week, month, year, lifetime, or a customized period). The **Invoice** value indicates that a rebate claim is determined every time that a purchase order line is invoiced.
...
- The **Rebate program accrual account** and **Rebate program expense account** fields specify account numbers that receive accrued rebate amounts during the intermediate stage between approval and processing.
...
- In the **Workflow approval status** field, the *Approved* value indicates that the agreement applies to purchase orders that meet the agreement’s conditions.
...
### View details of rebates applied to a purchase order line per the vendor rebate agreement
...
The **Vendor rebate** field in the **Margin estimation** section of the **Price details** page also shows the rebate information.

> [!NOTE]
> On the **Procurement and sourcing parameters** page, on the **Prices** tab, verify that the **Enable price details** option is set to *Yes*. If the option is set to *No*, you can't view the rebates.

## Review and approve claims

Generated rebate claims represent the future payments that can be expected from the vendor. Before a credit note is issued to the vendor, the agreement owner typically wants to review and approve the claims. However, the status of a claim determines whether it's ready for the approval process.

### The status of claims and the effect on the approval process

When a claim is generated, its status is set to *To be calculated* if the rebate is granted on a cumulative basis or *Calculated* if the rebate is granted per invoice. If the status of a claim is *To be calculated*, the claim must go through a calculation process that the Cumulate function handles. Only claims that have a status of *Calculated* can be included in the approval process.

> [!NOTE]
> If the **Approval required** option on a vendor rebate agreement is set to *No*, any claims that are generated have a status of *Approved*. Approval is mandatory for claims that are granted on a cumulative basis.

### Approve claims, and view postings and invoice details

After claims are approved, Accounts payable (A/P) can process them. A credit memo (vendor invoice) for the rebate claim amount is automatically generated. The credit is then added to the vendor balance, and the A/P team can include it in the regular settlement process.
...
6. On the Action Pane, select **Process**, and then select **Filter**. In the **Criteria** field for the **Vendor account** field, select the vendor to process rebate claims for, select other relevant filters, and then select **OK**.
    ...
    The message bars and the status change to *Completed* indicating that the following events have occurred:
...
8. Select the line for the vendor invoice transaction, and then, on the Action Pane, select **Vendor invoice**. If the vendor invoice has been posted, you see the Invoice journal. Otherwise, you see the vendor invoice as a pending vendor invoice that requires manual posting.
...
The process for handling vendor rebates involves multiple manual tracking tasks that are often tedious. By automating these tasks, the vendor rebate management feature helps you move through the following processes:
