...
4. Expand the **General** section.
    ...
    - The **Receipt date** field is used to specify when procured goods and services need to be delivered. You can specify a single receipt date for the order, or the individual order lines can be given unique receipt dates. If the receipt date specified here can't be met for specific products or services because they have longer lead times, then those lines will be created with a later receipt date to accommodate for this.  

5. Expand the **Administration** section. The **Orderer** field can be used to specify who is placing the order. This might be convenient to share with the vendor in case they need to contact that person. The field might be assigned a value automatically if the current user account is associated with a name on the **Users** page.  
...
6. In the **Color** field, enter or select a value. The **Site** and **Warehouse** fields are typically populated with values from the order header, but it's possible to override the fields if some lines need to be delivered to different locations.  
...
1. Select **Totals**.
    ...
    - The **Selection** field allows you to change the basis of how totals are calculated. For example, you could choose **Product receipt quantity** to show totals that relate to the amount of the products that have been received, or **Ordered quantity** to show the amount of product that was ordered.  
