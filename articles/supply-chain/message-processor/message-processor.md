...
To view the list of messages processed by the message processor, go to **System administration > Message processor > Message processor messages**.

### Grid columns and filters on the Message processor messages page

Use the fields at the top of the **Message processor messages** page to find specific messages. Most of these filters match the column headings in the message grid. The following filters and column headings are available:

- **Message type** – The type of message.
- **Message queue** – The name of the queue that the message is processed in. The following queues are available:

    - *Manufacturing Execution 3rd Party* – This queue holds messages that are created as part of the *Manufacturing execution system integration* feature. These messages also appear on the **Manufacturing execution systems integration** page, which is like the **Message processor messages** page but is focused exclusively on that feature. For more information, see [Integrate with third-party manufacturing execution systems](../production-control/mes-integration.md).
    - *Production* – This queue holds messages that are created as part of the *Make finished goods physically available before posting to journals* feature. These messages also appear on the **Deferred production order posting** page, which is like the **Message processor messages** page but is focused exclusively on that feature. For more information, see [Make finished goods physically available before posting to journals](../production-control/deferred-posting.md).
...
To view detailed information about a message, select it in the grid. Then, select the **Log** or **Message content** tab under the message grid, where each processing event is shown.

The text on the **Message content** tab depends on the **Message type** value. Therefore, the text length varies. A typical message content text starts with an opening brace (**\{**) and ends with a closing brace (**\}**). In between are field names (for example, `journalId`), each followed by a colon and a value (for example, *USMF-123456*).
...
Depending on the current state of a message, you can manually process or cancel it. Select the message in the grid, and then select **Process** or **Cancel** on the Action Pane.

If you want to requeue a previously canceled message, select it in the grid. Then, select **queue** on the Action Pane. The system processes the message as usual.

## <a name="processor-batch-job"></a>Schedule message processing by using the message processor batch job

To process a message queue, set up a batch job to run it. Usually, you set up a fixed, regular schedule for processing each queue. However, you can also run any queue on demand. To create and schedule the required batch jobs, follow these steps:

1. Go to **System administration \> Message processor \> Message processor**.
1. In the **Message processor** dialog box, in the **Message queue** field, select the message queue that's associated with the messages that you want to process. The queue that you select depends on the feature or system that generated the messages.
...
You can configure the number of processor tasks dedicated to each message processor queue and set rules for how often the queue cleans processed and canceled messages. Unconfigured queues use a default value that you can override as needed. Follow these steps to customize one or more queues.
...
1. For the new or selected row, make the following settings:
    - **Number of processor tasks** – Specify the number of processor tasks dedicated to the specified queue. The maximum value is *8*. The minimum value depends on the minimum number of batch threads that are configured for your system (typically *2*).
    - **Days before processed message deletion** – Specify the number of days before processed messages are cleaned up (deleted). Set this field to zero (0) to turn off processed message cleanup. For more information, see [Clean up processed and canceled message processor messages](message-processor-cleanup.md).
    - **Days before canceled message deletion** – Specify the number of days before canceled messages are cleaned up (deleted). Set this field to zero (0) to turn off canceled message cleanup. For more information, see [Clean up processed and canceled message processor messages](message-processor-cleanup.md).
