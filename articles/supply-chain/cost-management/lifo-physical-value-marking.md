...
- 7\. Inventory close is performed. Based on the LIFO method, the first financially updated issue is settled against the last financially updated receipt, and so on. In this example, one settlement is created between 5b and 3b. An adjustment of USD 14.00 is made to 3b, and the resulting final cost is USD 30.00.

The following illustration shows the effects of the first in, first out (FIFO) inventory model on this series of transactions when the **Include physical value** option isn't used.
...
- 7\. Inventory close is performed. Based on the LIFO method, the first financially updated issue is settled against the last financially updated receipt, and so on. In this example, one settlement is created between 3b and 5b. An adjustment of USD 14.00 is made to 3b, and the resulting final cost is USD 30.00. Additionally, transaction 6a is adjusted to the receipt transaction cost of 4a. The system won't settle these transactions, because the receipt is updated physically but not financially. Instead, only an adjustment of USD 1.33 is posted to the physical issue transaction, and the resulting adjusted cost will be USD 25.00.
...
You must make sure that the cost of the inventory item is reflected in the margin, or cost of goods sold (COGS), for the sales order invoice. When the purchase order is posted, the inventory is received at a cost of USD 120.00. If this sales order document is marked to the purchase order before the packing slip or invoice is posted, the COGS is USD 120.00, not the current running average cost for the item. If the sales order packing slip or invoice is posted before the marking occurs, the COGS is posted at the running average cost price.
