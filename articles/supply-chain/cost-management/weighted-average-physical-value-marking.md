...
-   6a. â€œWeighted average inventory close transactionâ€ financial issue is created to sum the settlements of all the inventory financial receipts.
-   6b. â€œWeighted average inventory close transactionâ€ financial receipt is created as the offset to 5a.
...
-   7a. â€œWeighted average inventory close transactionâ€ financial issue is created to sum the settlements of all the inventory financial receipts.
-   7b. â€œWeighted average inventory close transactionâ€ financial receipt is created as the offset to 5a.
...
For example, your Customer Service department accepted a rush order from an important customer. Because this is a rush order, you will have to pay more for this item to service your customerâ€™s request. You must be certain the cost of this inventory item is reflected in the margin, or cost of goods sold (COGS), for this sales order invoice. 
...
A receipt transaction is marked to an issue transaction. Then, the valuation method selected for the itemâ€™s item model group will be disregarded and the system will settle these transactions to each other. 
