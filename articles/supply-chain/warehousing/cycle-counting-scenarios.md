...
Each scenario in this article references values and records included in the standard demo data provided for Supply Chain Management. If you want to use the values provided here as you work through the scenarios, be sure to work in an environment where the demo data is installed. Set the legal entity (company) to **USMF** before you begin.
...
1. In the list pane, select the record that is named *Cycle Count*. If no existing record has this name, create it. Confirm or set the following values for the record:
    ...
    - **Directed by:** *System directed* (This value indicates that Supply Chain Management assigns a cycle counting work ID to the worker.)
...
1. Select **OK** to close the dialog box.
1. In the list pane, select the record named *Spot Counting*. If no existing record has this name, create it. Confirm or set the following values for the record:
...
Before the guided cycle counting can occur, you must create some work. This work guides the assigned person through the warehouse, from location to location, to complete the counts set up in the work.
...
After you create the cycle counting work, you do the work by counting items in a warehouse location and then using a mobile device to enter the results in Supply Chain Management. Follow these steps to do the cycle counting work in the Warehouse Management mobile app.
...
1. The system expects you to enter a count of 10 pieces for this item, location, and license plate. Therefore, you're asked to recount. Select the **Qty** field, enter *9* again by using the number pad, and then select **OK** (the check mark button). On the second attempt, your count is accepted.

    > [!NOTE]
    > When the system detects a difference between the expected on-hand quantity and the quantity that you entered, it asks you to count a second time because the **Number of attempts** field is set to *1* for the **Cycle Count Guided** mobile device menu item. You're guided to a specific item number and license plate number because both the **Display item number** option and the **Display license plate** option are set to *Yes* for the mobile device menu item.
...
1. Select **OK** to close the **Create cycle count work by item** dialog box.
    ...
    When the work creation process finishes, you receive an informational message.

### Do cycle counting work for scenario 2

After you create the cycle counting work, follow these steps to do the work in the Warehouse Management mobile app.
...
1. Select the **Qty** field, enter *9*, and then select **OK** (the check mark button).
    ...
    Because the system detects that the specified license plate is already available at another location in Supply Chain Management, that license plate is moved to the current location. Therefore, the system asks you to confirm the movement.
...
One way to create cycle counting work is to use thresholds. A cycle count threshold indicates the quantity or percentage limit of inventory items. The cycle counting work is automatically created when the threshold is reached.

For example, there are 60 items in a location that has a cycle count threshold of 40. During a sales order transaction, 25 items are picked from the location and put in a staging location. Because the new item count, 35, is less than the threshold quantity, the cycle counting work is automatically created for the location.
...
Cycle count plans let you automate the creation of cycle counting work. Set up each cycle count plan with specific item and location queries. When the batch job runs, it creates cycle counting work for all locations that match the item and location criteria, up to the maximum number of counts specified for the plan. When the cycle counting work is created, the counting work line includes information about the location that needs to count. The on-hand inventory that is associated with that location isn't blocked. Therefore, it's available for reservation and outbound processing, even though open counting work exists.
