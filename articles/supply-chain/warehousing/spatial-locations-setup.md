...
1. On the **Warehouse** FastTab, in the **Warehouse coordinates configuration** section, make the following settings:
    ...
    - **Coordinate retrieval strategy** – Select the method that the system uses to retrieve a location's coordinates. Currently, only one setting is available and is selected by default <!-- KFM: Right? -->:
...
If you want to set up the scenario so you can run it, follow these steps to set up the warehouse and wave template:
