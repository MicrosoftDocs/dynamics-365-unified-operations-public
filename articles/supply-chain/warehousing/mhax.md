...
- **data05** – The short pick exception code linked to a *Work exception* record with an **Exception type** of *Short pick*.
- **data06** – The work header's target license plate ID, which can be left blank when set to be autogenerated.
...
You can experiment with the material handling interface by, for example, using [Postman](../../fin-ops-core/dev-itpro/data-entities/third-party-service-test.md#query-odata-by-using-postman).

You can create a *fork* of the [Postman environment and collection examples](https://go.microsoft.com/fwlink/?linkid=2250135). Be sure to select the environment and enter the correct environment variables before you run the `CREATE TOKEN VARIABLE` collection, which makes it possible to run all the collections starting with the name *MHE*.

> [!TIP]
> Turn on the **Enable manual inbound message creation** setting on the **Material handling equipment interface parameters** page if you'd like to simulate inbound messages by creating records directly from the **Inbound queue** page.
