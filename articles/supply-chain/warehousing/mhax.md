...
2. On the **General** tab, set the following fields:
    ...
    - **Enable manual inbound message creation** - When this option is set to *Yes* you can simulate the inbound messages by creation a record directly in the *Inbound queue* page.
...
To ensure that the MH system receives each outbound queue ID only one time, a status field is used to specify whether an event is *Ready* to be sent to the external material handling system or whether it has already been *Sent*.
...
- **data05** – The short pick exception code linked to a *Work exception* record with the *Exception type* equal *Short pick*.
- **data06** – The work header's target license plate ID which can be left blank when enabled to get auto generated.
...
You can easily try out using the material handling interface by for example using [Postman](../../fin-ops-core/dev-itpro/data-entities/third-party-service-test.md#query-odata-by-using-postman).

You can create a *fork* of the [Postman environment and collection examples](https://go.microsoft.com/fwlink/?linkid=2250135). Be sure to select the environment and fill in the correct environment variables before you run the `CREATE TOKEN VARIABLE` collection which will make it possible to run all the collections starting with the name *MHE*.

> [!TIP]
> By enabling the *Material handling equipment interface parameters* parameter **Enable manual inbound message creation** you can simulate the inbound messages by creation a record directly in the *Inbound queue* page.
