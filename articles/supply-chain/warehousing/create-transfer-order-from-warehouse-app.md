...
The transfer order numbers can get looked up by using the [Data inquiry](warehouse-app-data-inquiry.md) capability in the Warehouse Management mobile app. You can for example create a [Detour](warehouse-app-detours.md) mobile device menu item to inquire the same data getting displayed in the rich client **Warehouse app events** page (WHSMobileDeviceQueueMessageCollection) as part of the 'Select order - MobileDeviceQueueMessageCollectionIdentifierId' step. The transfer order number equals the *Identifier* field.
For more information of viewing the data in the rich client, see [Inquire the warehouse app events](#inquire-the-warehouse-app-events).

#### Can I manually select the transfer order number to be used from the Warehouse Management mobile app?

Only autogenerated transfer order numbers via number sequences are supported. See the option to **Select transfer order** above.
