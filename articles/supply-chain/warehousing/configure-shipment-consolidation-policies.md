...
The scenarios in this article show how to set up default and custom shipment consolidation policies.

## Scenario: Configure custom shipment consolidation policies

This scenario provides an example that shows how to set up custom shipment consolidation policies and then test them by using demo data. Custom policies can support complex business requirements where shipment consolidation depends on several conditions. For each example policy later in this scenario, a short description of the business case is included. Set up these example policies in a sequence that ensures a pyramid-like evaluation of the queries. (In other words, the policies that have the most conditions are evaluated as having the highest priority.)

### Make demo data available

This scenario references values and records included in the standard [demo data](../../fin-ops-core/fin-ops/get-started/demo-data.md) that's provided for Supply Chain Management. If you want to use the values that are provided here as you do the exercises, be sure to work in an environment where the demo data is installed, and set the legal entity to *USMF* before you begin.
...
1. Close the page.
1. Open the product that has item number *M9201*. This product is also pre-enabled for WMS processes in the **USMF** demo data.
...
1. Create a transportation mode for use in consolidation queries, and name it *Airways*.
...
1. On the Action Pane, select **Save**.
    ...
    > [!NOTE]
    > When you save the new carrier, the **Mode of delivery** field for the new row in the **Services** grid is automatically set to *Airwa-Air*. When you use the *Airwa-Air* mode of delivery for a sales order, the *Airways* transportation mode is used for related shipments.

#### Create an order pool

1. Go to **Sales and marketing > Setup > Sales orders > Order pools**.
1. Create an order pool that you can use for the consolidation query. This order pool should have the following settings:
...
In this example, you create a *Customer+Mode* policy for the following business case:

- The policy queries for a specific customer account (*US-001*) and a specific mode of delivery (*Airwa-Air*).
- Consolidation with open shipments is turned off.
- Consolidation is done per order ID. (In other words, there are separate shipments per order, warehouse, and so on.)
...
1. Select the **Add** button :::image type="icon" source="media/forward-button.png" border="false"::: to move the field to the **Selected fields** list.
...
> [!NOTE]
> For this business case, order lines for customer *US-001* that use the *Airwa-Air* mode of delivery aren't consolidated across orders. Use this policy first in a sequence, in cases where shipments for all other modes of delivery are consolidated for this customer.

### Create example policy 2

In this example, you create a *Hazardous goods* policy for the following business case:

- The policy queries for a specific filter code (*hazardous*) and a specific mode of delivery (*Airwa-Air*).
- Consolidation with open shipments is turned on.
- Consolidation is done across orders. (In other words, there are separate shipments per account, warehouse, and so on, but only within the item group that is specified in the query.)
...
> [!NOTE]
> For this business case, all order lines where items have a specific filter code (that is, the filter code where the **Code 4** field is set to *Flammable*) consolidated with other items of the same type across orders. If there's an open shipment for the same account, warehouse, and group of items, the new lines are attached to it.

### Create example policy 3

In this example, you create a *Customers' requirements* policy that you can use for the following business case:

- The policy queries for a specific customer account.
- Consolidation with open shipments is turned on.
- Consolidation is done across orders but is based on customer requisitions. (In other words, multiple orders are grouped into shipments, based on the same customer requisition number and warehouse.)
...
> [!NOTE]
> For this business case, all order lines where sales orders have the same customer requisition number are consolidated into one shipment, regardless of the sales order number. (The customer requisition number is used as the customer's purchase order \[PO\] number.) If there's an open shipment for the same account, warehouse, and customer requisition, the new lines are attached to it. Use this policy if the customer sends additional order lines that have the same PO number several times during a day and wants all the lines to be grouped into one shipment. (In other words, there's one bill of lading and one packing slip.)

### Create example policy 4

In this example, you create a *Customers allowing consolidation* policy for the following business case:

- The policy queries a specific order pool to identify customers who accept consolidated shipments.
- Consolidation with open shipments is turned off.
...
> [!NOTE]
> For this business case, all order lines where sales orders belong to the same order pool consolidated into one shipment across sales orders for the same account, warehouse, and transportation mode of delivery. Instead of the order pool, you can use any other field to distinguish a group of customers and use the sales order header by default. Use this approach if the customer, not the warehouse, drives the need for consolidation. (In the earlier consolidation logic, the warehouse drove the need for consolidation.)

### Create example policy 5

In this example, you create a *Warehouses allowing consolidation* policy for the following business case:

- The policy queries a specific order pool to identify warehouses that can consolidate shipments.
- Consolidation with open shipments is turned off.

Typically, you can use default policies to address this business case. However, you can also manually create similar policies by following these steps.
...
After you create all your policies, establish the order in which they're applied. To use a pyramid-like approach, where the policies that have the most conditions are evaluated as having the highest priority, follow these steps:

1. Go to **Warehouse management > Setup > Release to warehouse > Shipment consolidation policies**.
1. Set the **Policy type** field to *Sales orders*.
1. Select each policy that's listed in the left column, and then use the **Move up** and **Move down** buttons on the Action Pane to arrange the policies in the following order:
...
The following scenarios illustrate how you can use the shipment consolidation policies that you created while reading this article. Each scenario walks you through a shipment consolidation process that uses shipment consolidation policies during automated or manual release to the warehouse:
