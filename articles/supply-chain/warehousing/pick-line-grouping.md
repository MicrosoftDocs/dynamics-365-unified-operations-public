...
1. In the **Title** field, enter *Sales group line picking*. This title appears on the mobile device menu.
...
1. Follow these steps to configure the valid work classes for the mobile device menu item:
    ...
    1. On the **Work classes** FastTab, select **New**.
    1. In the **Work class ID** field, select either *Sales* or *SO Pick*, depending on the warehouse that you use. For this scenario, select *SO Pick*.
...
1. In the **Overview** grid, find and select the work template to use with this function. For this scenario, select the *51 Pick to stage* template.
...
> [!IMPORTANT]
> For the pick line grouping functionality to work, the work lines must be sorted by item ID. If lines that have the same items aren't sequenced one after another, they aren't grouped.
...
1. Before you release the orders to the warehouse, make sure that the pick locations have enough inventory for all the items on all the orders. Review the **Location directive** setting to determine which picking locations are used for sales order picking. If you're using the Contoso demo data environment for warehouse *51*, confirm that there is available inventory.
...
1. On the Action Pane, on the **Warehouse** tab, select **Release to warehouse**.
    ...
    You receive a message that states that a shipment and a wave are created, and that the wave is submitted to run in a batch.

    When the wave and all downstream jobs are completed, a work ID is created for work that has six lines. The lines are sorted by item number.

1. Go to **Warehouse management > Work > All work** to view the work that you created. Make a note of the **Work ID** value, because you need it in the next procedure.
...
1. Enter the **Work ID** value that you noted in the previous procedure.

    You see a pick step where all pick lines for item *M9200* are grouped, and you receive an instruction to pick *7* each of item *M9200*.

    > [!IMPORTANT]
    > On the mobile device, the pick work for the three pick work lines is aggregated into one picking step for the user.

1. Confirm the pick step.
1. Go to the work page for the work ID, and notice that all three pick lines for item *M9200* are closed simultaneously.
1. Go back to the mobile device, and continue to pick. Work line 4 for item *M9201* is presented. Because there's only one work line on the order, nothing is left to aggregate.
1. Confirm the pick step.
