...
> - The Skip button in the **System directed** cycle counting mobile device flow will not skip the item that is currently being counted. Instead, the Skip button will skip the whole cycle counting work, and change to the next available cycle counting work one. If another work is not available, it will stay on the same work and show an error message that there is no other work available.
