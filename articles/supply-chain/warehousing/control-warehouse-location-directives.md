...
> [!NOTE]
> Items associated with multiple units of measure (for example: pieces, boxes, or pallets) must have a Unit Sequence Group and the conversion ratios between them defined. This enables, for example, having an order created against one unit but the work associated with it to be created and split in a different unit quantity. Learn more about Unit Sequence Group [here](./unit-measure-stocking-policies.md).
