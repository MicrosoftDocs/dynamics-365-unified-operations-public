...
We **strongly recommend** that you set up one Microsoft Entra ID user for each device. This approach makes it easy to block the Microsoft Entra ID user for just one device if that device is stolen or damaged. (For more information, see [Remove access for a device that uses user-based authentication](warehouse-app-authenticate-user-based.md#revoke).) However, if you prefer, using a single Microsoft Entra ID user for all your devices is possible. Keep in mind, though, that this setup may lead to confusion within the Dynamics F&O portal. All warehouse processes will be associated with the single system user, even though multiple warehouse workers have processed work on multiple warehouse devices. Your choice depends on your licensing strategy and risk tolerance.
