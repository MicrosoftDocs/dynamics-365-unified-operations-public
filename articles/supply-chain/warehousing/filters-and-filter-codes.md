...
By setting up and using product filters, you can automatically organize inventory items in a warehouse and combine filtered items into filter groups. Use filters to put items into categories for handling, purchasing, and selling processes. You might want to group items together or separate them from each other when the way that you handle them is based on weight or handling restrictions. You can also specify which customers or vendors an item can be purchased from or sold to.
...
1. In the **Filter code** field, enter a value.
    ...
    :::image type="content" source="media/Product_Filters10.png" alt-text="Screenshot of setting up a product filter.":::

1. In the **Description** field, enter a name for the code. For example, *Code 2* might represent vendors. You can then create a product filter for a specific vendor or group of vendors. To learn more, see the [Setup vendor filter codes](#vendor-product-filters) section later in this article.

    :::image type="content" source="media/Product_Filters.png" alt-text="Screenshot of a set of product filters.":::

## Set up product filter groups

Use filter groups to organize filter codes. This capability is helpful when a group is used in a query in a location directive and you want to search for the group instead of a series of codes. Each filter group is associated with an item group.

> [!IMPORTANT]
> Not all product filter groups are available for filter codes that are higher than *Code 4* (that is, *Code 5* through *Code 10*). For example, for released products, although all the product filter codes are added, the grouping of filter codes isn't updated. This behavior might be updated in later releases.

To set up filter groups, follow these steps:

1. Go to **Warehouse management > Setup > Product filters > Product filter groups**.
1. On the Action Pane, select **New**.
1. In the **Group 1** and **Group 2** fields, enter the names that you can use to categorize items.
...
1. In the **Code 1** through **Code 10** fields, select the filter codes to include in the group, as required.
    ...
    :::image type="content" source="media/ProdFilterGroup.png" alt-text="Screenshot of item group product filter group configuration.":::
...
When you set up filter codes on an item group, you specify the codes required for products attached to that item group.
...
1. On the **Warehouse** FastTab, in the **Filter required** section, select the check boxes for the filter codes that you must specify for products associated with the item group.
    ...
    :::image type="content" source="media/ItemGroup10.png" alt-text="Screenshot of item groups configuration with filter codes.":::
...
1. In the **Item group filter** section, select the check boxes for the filters that must match for the filter group to be the default filter group for an item.
    ...
    For example, if you select the **Use filter code 1** and **Use filter code 2** check boxes, both filter code 1 and filter code 2 of the item must match the setup of the filter group for the item group before you can select the filter group. When you create a new item, the selected filter group is the default filter group in the **Group 1** and **Group 2** fields on the **Warehouse** FastTab of the **Released product details** page.

> [!IMPORTANT]
> You can enable product filter codes only for items that use warehouse management processes (WMS).

## Specify filter codes for released products

Follow these steps to specify filter codes for released products. For example, use filter codes to group hazardous products that specific vendors purchase.
...
> [!NOTE]
> Customer and vendor filters don't apply to any item set up as generally available.
...
1. In the **Start date/time** field, enter the date and time when the item becomes available.
...
Use this optional procedure to specify items that customers can access, in addition to the items available through the filter setup on the **Generally available items** page. You can set up multiple filters for a single customer.
...
1. In the **Item group** field, select an item group.
1. In the **Code 1** through **Code 10** fields, select the filter codes to use as criteria to limit the items that are available for customers in the selected item group. You must make a selection for every filter code set up for the item group.

## <a name="vendor-product-filters"></a>Set up vendor product filters

Use this optional procedure to specify items that a vendor can access, in addition to the items made available through the filter setup on the **Generally available items** page. You can set up multiple filters for a single vendor.
...
1. In the **Start date/time** and **End date/time** fields, enter start and end dates for the selected item group.
1. In the **Item group** field, select an item group.
1. In the **Code 1** through **Code 10** fields, select the filter codes to use as criteria to limit the items that are available for vendors in the selected item group. You must make a selection for every filter code set up for the item group.

> [!NOTE]
> The setup of vendor product filters applies to released products where warehouse management processes (WMS) are enabled for the associated storage dimension group. The filter codes determine whether the system allows users to purchase a given item from a given vendor when they create purchase order lines. Microsoft Dynamics 365 Supply Chain Management has two methods for handling vendor approval. If one or more released products exist where the **Approved vendor check method** field is set to *Warning only* or *Not allowed*, both vendor approval methods can be enabled for those items. This situation might cause issues when users create purchase order lines.
