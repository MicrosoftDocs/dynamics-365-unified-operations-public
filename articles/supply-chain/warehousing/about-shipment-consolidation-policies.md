...
## Shipment consolidation policies functionality

This section describes the pages, commands, and features that are part of the *Shipment consolidation policies* feature.
...
The values that users select for these fields are used for all newly created shipments, or they're added to existing shipments during consolidation with those shipments. When two shipments have the same value for a field that is selected for consolidation of those shipments, the shipments are consolidated. The same principle applies for all subsequent consolidation fields that are selected. If the values differ, the second shipment is discarded and is selected for a new shipment. The automated consolidation process consists of creating all the unique combinations of values for the shipment consolidation fields and then assigning a shipment to the relevant combination.
...
The list of consolidation fields (that is, fields that are cleared if they have different values) is hard-coded. The list contains all fields that are initialized from a sales order or transfer order line when a new shipment is created. In other words, if a field isn't initialized from a sales order or transfer order line, it's ignored when new data is added to an existing shipment.

### Release to warehouse page

- A field in the lower grid shows the applied consolidation policy.
- A button lets you manually select and/or override the consolidation policy.

### Release to warehouse command on the Outbound load planning workbench page

- The logic is adjusted so that it applied consolidation policies.
- Shipments are consolidated only within a single load.

### Consolidate shipments page

- The search for similar shipments (that is, candidates for consolidation) is changed so that it uses fields selected in the shipment consolidation policy.
- Fields that have different values in different shipments are set to blank.

### Shipment consolidation workbench page

- You can open this page from the **Release to warehouse** menu in the **Warehouse management** module.
- The algorithm analyzes existing shipments that aren't yet shipped. It then proposes consolidation, based on fields selected in the consolidation policies.
...
|---|----|
| Not applicable | Sales or transfer shipments selected for consolidation must have the same consolidation policy as the shipment being created, or they must be assigned to an open shipment when the **Consolidate with existing shipments** option is turned on. |
| The *Release to warehouse* procedure doesn't search among existing shipments to find a shipment for consolidation. Only shipments created by a current instance of the *Release to warehouse* procedure creates are used to find a shipment for consolidation. | If the **Consolidate with existing shipments** option is turned on for a consolidation policy that is currently in use, the *Release to warehouse* procedure searches among existing shipments that were created based on the same consolidation policy to find a shipment for consolidation. Therefore, if you have two policies, a shipment being created based on policy 2 is never consolidated with a shipment that was created based on policy 1. |
...
| You manually select **Consolidate shipments** on the **All shipments** page and then select a target "base" shipment. The filter suggests any existing shipments that have matching values for several key fields. | You manually select **Consolidate shipments** on the **All shipments** page and then select a target "base" shipment. The system suggests other existing shipments by matching the values of several key fields that are configured for the relevant shipment consolidation policies. |
