...
When using the feature **Use existing catch weight tags when reporting production orders as finished** and having the process enabled via a mobile decive menu item, the inventory gets updated based on existing catch weight tag information, and thereby the Warehousing app does not prompt for capturing of the catch weight tag data as part of a production report as finished operation.
