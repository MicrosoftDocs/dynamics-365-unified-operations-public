...
A catch weight tag can either be created via a warehousing app process, get manually created in the form, or via a data entity process. In case a catch weight tag gets associated with an inbound source document line, e.g. purchase order line, the tag will become registered. In case the line is used for outbound processing the tag will be updated as shipped.
