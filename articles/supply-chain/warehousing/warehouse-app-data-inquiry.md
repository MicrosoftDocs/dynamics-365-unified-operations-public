...
By providing barcode scanning capability the warehouse management mobile app offers an easy and accurate way to capture data as part of your warehouse processes. But sometimes barcodes get damaged and unreadable, or the needed data information might not exist as a barcode in your business process flows.
...
One example could be during a purchase order receiving flow, where a purchase order number is required to get recorded for matching up the arrived inventory item quantities. As part of this process, you can easily configure different menu items to obtain a card list view of the relevant purchase order numbers and thereby being able to continue the receiving flow with a quick “select by clicking” approach; but the functionality can get used within all your warehouse management mobile app flows and not only within the following example.
...
This example shows how easy it is to find a purchase order number by scanning one of the item barcodes on the arrived inventory. The logic will simply list all the open purchase orders containing lines for the specific item number making it simple to select the purchase order number for the flowing receiving process.

As additional inspiration you can try to add an inquire detour lookup on the **Scan item** page to search for item numbers within a specific purchase order. This could be relevant in a scenario where an item number barcode cannot get read.
...
4. Under **General** select the following
...
5.	Select **Edit query**
6.	In the **Range** section grid add the following three lines
...
10.	Select **Save** and close the page by selecting **X** (Close) or back in the browser.
...
5.	Select **Edit query**
6.	In the **Range** section grid add the following two lines
...
3. Add the following information:
    - **Name** - Inquire
    - **Description** - Inquire
4. Now with the focus on the newly created menu find and select the first of the three mobile device menu items in the **AVAILABLE MENUS AND MENU ITEMS** list
5. Select the arrow-bottom to move this menu item into the **MENU STRUCTURE** list
6. Do the same for the two remaining menu items
7. Now select the **Main** menu for the demo data
8. Find the just created menu under the *Menus* grouping in the **AVAILABLE MENUS AND MENU ITEMS** list and select it
9. Select the arrow-bottom to move this menu into the **MENU STRUCTURE** list
...
2. Filter in the grid **Step ID** column for *PONum*
3. With the record selected click **Add step configuration** and select the **Menu item** *Purchase Receive* and click **OK**
4. Within the *Mobile device step* for the “Purchase Receive : PONum” Select **Add** in the *Available detours (menu item)*
5. In the list select the previously created **Lookup POs by vendor** menu item
6. Click on **Select fields to send**
7. In the **SEND FROM PURCHASE RECEIVE** we will not enter data because we do not want to pass any values to the detour menu item. But in the **BRING BACK FROM LOOKUP POS BY VENDOR** section select the following:
   - *Copy from Lookup POs by vendor** - Purchase order
   - **Paste in Purchase Receive** - Purchase order
8. Select **OK**
9. Do the same for the two remaining **Lookup POs for today** and **Lookup POs by item** menu items
...
Create several purchase orders with lines to try out your setup and then run the warehouse management mobile app process for the **Purchase Receive**.

You should now see the following page with the three additional menu items:

[![Purchase receiving - purchase order number](media/WMSappPurchaseReceive-PONum-with-detours.png "Purchase receiving - purchase order number")](media/WMSappPurchaseReceive-PONum-with-detours.png)
