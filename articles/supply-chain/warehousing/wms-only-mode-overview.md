...
| Process | Description |
|---|---|
| Return order processing with disposition codes | When you use the inbound shipment orders that are related to an inbound return process, you can't use the process that's used by [sales return orders](sales-returns.md) which supports return reason codes and disposition codes as part of the flow for the **Return order receiving (and put away)** mobile device menu item. For the return order processing using the [Blind return receiving](sales-returns-unannounced.md#blind-return-process) the automated disposition code related to scrapping is not supported for the inbound shipment orders, likewise the [Return details](sales-returns-unannounced.md#return-details-process) capability is not supported related to outbound and inbound shipment orders. |
