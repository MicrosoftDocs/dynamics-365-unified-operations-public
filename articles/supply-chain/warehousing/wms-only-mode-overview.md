...
| Transportation management processes | The transportation management engines that are currently supported for the purchase orders loads aren't supported for the inbound shipment order processes. Note that both for inbound and well as outbound shipment orders it is not possible to assign charges, nor to process direct invoicing, and thereby as well not supported to use the apportionment weight engine to generate freight bills. |
