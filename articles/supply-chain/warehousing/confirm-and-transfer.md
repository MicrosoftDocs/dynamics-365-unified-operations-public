...
Before you can use this feature, it must be enabled on on your system. Administrators can use the [feature management](../../fin-ops-core/fin-ops/get-started/feature-management/feature-management-overview.md) settings to check the feature status and enable it if needed. Here, the feature is listed listed as:
...
To use confirm-and-transfer feature, you must enable it on each of your relevant load templates. In addition, you might choose to prepare your work templates to support the feature, depending on your requirements. If you'd like to work through the example scenario (based on **USMF** demo data) provided later in this topic, then set up your system as described in this section.
...
1. Select **Edit query** on the action pane to open the **Sales** pane.
1. Open the **Sorting** tab on the **Sales** pane.
...
1. Select **OK** to save your settings and close the **Sales** pane.
...
This scenario illustrates an example where the picking process isn't finished yet, but the items picked so far need to be loaded onto a truck and shipped anyway. The user will therefore be able to *split* the unpicked load lines into a new load with all data relationships updated automatically.

> [!NOTE]
> The specific values suggested in this scenario are based on the **USMF** demo data. We recommend you use this demo data while demonstrating or learning the feature. If you aren't using the **USMF** demo data, then substitute your own values as needed.
...
1. Select **New** on the action pane to open the **Create sales order** pane.

1. On the **Create sales order** pane, make the following settings (at minimum):

    - In the **Customer** FastTab, select **US-004**, - **Cave Wholesales**.
    - In the **General** FastTab, set the **Warehouse** to *51*.

1. Select **OK** to create the sales order and close the **Create sales order** pane.
...
1. In the **Load template assignment** pane, select **20' Container** from **Load template ID**
...
1. In the **Release load to warehouse** pane, validate that your **Load ID** is displayed in the **Records to include** FastTab.Expand the section if necessary.
1. Select **OK**.
...
1. Select **Load ID** to open **Load Details**.
...
1. On the **Ship confirm** pane, in the **Load split method during ship confirm** field, select **Split quantity to new load**.
