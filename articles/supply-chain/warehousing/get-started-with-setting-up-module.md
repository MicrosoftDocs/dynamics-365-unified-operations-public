...
1. The **Initialize base data** page opens. Use the settings here to establish initial values for the Warehouse management module. Default values are suggested, but you can edit any of them as needed. If you leave any field blank, then the wizard won't initialize that setting. Make the following settings:
    ...
    - **Receiving** – Define the name for the receiving location type and location profile. The specified receiving location type will also be identified as such on the **Warehouse management parameters** page. <!--KFM: I don't see this on the **Warehouse management parameters** page. Where is it? -->
    ...
    - **Default inventory status ID** – Define the name for the default inventory status. The specified default inventory status ID will also be identified as such on the **Warehouse management parameters** page. <!--KFM: I don't see this on the **Warehouse management parameters** page. Where is it? -->
    - **Default work user ID** – Define the name for the default work user. The specified default user ID will also be identified as such on the **Warehouse management parameters** page. <!--KFM: I don't see this on the **Warehouse management parameters** page. Where is it? -->
...
> [!IMPORTANT]
> After you complete the wizard, you will probably have a new work user that doesn't yet have a *person* assigned to it. A *person* is a record that represents an actual employee and is used for human resource management and other features, while a *work user* is a worker account used to sign in to the Warehouse Management mobile app  <!-- KFM: And other things? Please confirm. -->. To finish setting up your new default work user, you must assign a person to it by  doing the following steps:
>
> 1. Go to **System administration \> Users \> Users**.
> 1. Find the user ID that needs an associated person. <!-- KFM: The user I specified using the wizard doesn't appear here. What happened? Seems like it's on the **Work users** page instead, but how does it decide which work user to assign the user ID to? I think maybe we should refer to [Mobile device user accounts](mobile-device-work-users.md)-->
> 1. Select the hyperlink **User ID**.
> 1. In the **Person** field, select the employee to associated with this user account.
