...
- To receive unannounced sales returns, you're running Supply Chain Management version 10.0.39 or later.
- To use the integration with small parcel shipping (SPS), you're running Supply Chain Management version 10.0.39 or later.
- To print license plate labels from a mobile device, you're running Supply Chain Management version 10.0.40 or later.
...
- **Customer account or Source system** – The source that returns the item. *Customer account* is used for sales return orders and *Source system* is used when using [inbound shipment orders](wms-only-mode-using.md#inbound-shipment-orders) as part of the [Warehouse management only mode](wms-only-mode-overview.md) process.
- **Item number and quantity** – The items that are returned and the quantity that's returned for each item.
...
The *Return details* return process uses a return details record. This record is automatically generated when an order is shipped. It contains all the information required to process a return, such as the return ID, original order ID, shipment ID, order lines, and return-until dates.
...
You can generate a return details record in two ways:
...
In both cases, the system fills in the shipment date and return-until date when it confirms the related shipment.
...
To prepare your system to receive returns without using an RMA order, you must enable the relevant features and complete several related configurations. The following subsections describe the required settings.
...
1. If you're using [Warehouse management only mode](wms-only-mode-overview.md), set up returns for each source system by following these steps:
...
Disposition codes are collections of rules that you can use when receiving items in Supply Chain Management. They help determine the inventory status, the work template, and the location directive for the items, based on the scanned code.

For example, when a warehouse worker uses a mobile device to receive items that are damaged, the worker scans a disposition code for damaged items.
...
- If you receive items that you can't resell and must therefore dispose of, use a disposition code for RMA scrap. This disposition code then returns a *return disposition code*.

Use disposition codes for different processes, such as *purchase order receiving*, *production order report as finished*, and *sales order return receiving*. For the *sales order return receiving* process, if the items are registered by using a mobile device, the use of disposition codes is mandatory.

#### Return disposition codes

To view and set up your return disposition codes, go to **Sales and Marketing** > **Setup** > **Returns** > **Disposition codes**. Define each code by specifying an ID number, an action, and a description. The action defines how items that are assigned to the code should be handled.
...
To set up disposition codes that you can use on a mobile device, go to **Warehouse management** > **Setup** > **Mobile device** > **Disposition codes**. Disposition codes that you create here are available to mobile devices and can support several types of operations, including returns, sales orders, and inventory management. Disposition codes for returns must be linked to a return disposition code defined at **Sales and Marketing** > **Setup** > **Returns** > **Disposition codes**, and that assigns a return action for the code.
...
If you're using [Warehouse management only mode](wms-only-mode-overview.md), you must map local dispositions codes with the *source system disposition codes* used in each relevant [source systems](wms-only-mode-setup.md#source-systems). These codes are used when processing [inbound shipment orders](wms-only-mode-using.md#inbound-shipment-orders).
...
1. Repeat the previous step until you set up all of the disposition codes that you need to map.
...
Before you create the related mobile device menu items, define the required return item receiving policies. These policies enable each menu item to process the return correctly.
...
1. Set the following fields for the new policy:
    ...
    - **Return item receiving policy ID** – Enter a name for the policy, such as *Blind return* or *Return details*.
    ...
    > [!IMPORTANT]
    > If your source system is Dynamics 365 Supply Chain Management (as is the case when you run an [external shared warehouse scenario](wms-only-mode-external-shared-warehouse.md)), set the following values for your blind return policies:
    >
...
To enable workers to process unannounced returns, create a separate mobile device menu item for each type of unannounced return process (*Return details* or *Blind return*) that you support.
...
1. Set the following fields for the new menu item:
    ...
    - **Barcode data policy** – Select the policy to use if multiple fields are filled in based on a single bar code scan. To learn more, see [GS1 bar codes](gs1-barcodes.md).
...
Before you work through the example scenarios, complete all the prerequisites described in the following subsections.

#### Enable sample data

To work through the example scenarios by using the sample records and values that are specified here, use a system where the standard [demo data](/dynamics365/fin-ops-core/dev-itpro/get-started/demo-data) is installed. Additionally, select the **USMF** legal entity before you begin. The example scenarios use the demo data that's associated with the worker/person *Julia Funderburk*.

You can also use these scenarios as guidance for using the feature on a production system. However, in that case, substitute your own values for each setting that's described here.
...
In the blind receiving process, warehouse workers receive return items without matching them to an original sales order or shipment. Warehouse workers just enter the customer account ID, item IDs, and quantities.
...
1. Select **Cancel** to exit the flow and close the menu item.
    ...
    > [!NOTE]
    > You must exit the flow to unlock the license plate. If the license plate is locked, you can't complete mixed license plate receiving in Supply Chain Management, as described in the next section. In some setups, you can finish processing mixed license plates during the *Return item receiving* flow. However, in this scenario, you process the license plate in Supply Chain Management.

### <a name="complete-mixed-license-plate-receiving"></a> Complete mixed license plate receiving

The returned products are now on a mixed license plate for further processing. (Learn more in [Mixed license plate receiving](mixed-license-plate-receiving.md).) Follow these steps to finish receiving the returned items.
...
> [!TIP]
> You can set up your mobile device menu item for processing returns to include a step that lets workers complete mixed license plate receiving directly from the mobile device while receiving the return. Then you don't need to [complete this step](mixed-license-plate-receiving.md) later in the Supply Chain Management web client. To do so, create a [detour](warehouse-app-detours.md) item with the **Activity code** *Complete mixed license plate*. Learn more in [Set up a mobile device menu item for completing mixed license plates](mobile-device-complete-mixed-lp-menu.md).
...
The inventory items are brought to the packing area and are ready to be packed into containers. Follow these steps to create a new container in the system, and to pack and close the container. After you close the container, the system automatically creates a return details record. You can reference this record later when returned items are received back into the warehouse. At this point, you can print a return label and deliver it together with the container.
...
1. The **Return details** page should list a new return details record that shows the order number and line item that you just packed. Make a note of the **Return ID** value for the new record. You need this value later in this scenario.
