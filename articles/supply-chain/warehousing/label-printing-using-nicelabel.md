...
## Prepare for NiceLabel Cloud integration
...
1. The **External service operations** page opens. Select **New** on the Action Pane to add a new operation, which is for the *Print* service operation <!-- KFM: Is this the "variables-based layout" operation we mention earlier? -->. Make the following settings for the new record:
    ...
    - Make the following settings on the **General** FastTab.
        ...
        - **Request body type** – Select *Raw*. <!-- KFM: Maybe explain what this setting means. What about the other option (form-data)? -->
        - **Relative URL** – Enter */Print/v2/Print*. <!-- KFM: Is this always the value we want here, or just an example? -->
    ...
    - On the **HTTP request headers** FastTab, add a row with the following settings:
        - **Key** – Enter *Ocp-Apim-Subscription-Key*. <!-- KFM: Is this always the value we want here, or just an example? -->
...
1. Get a copy of either the primary or the secondary key for the subscription you created earlier on the NiceLabel Developer Portal.
...
## Configuring Supply Chain Management to print to local network printers using the Cloud Trigger API
