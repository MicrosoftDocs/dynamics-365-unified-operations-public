...
- You're running Microsoft Dynamics 365 Supply Chain Management version 10.0.36 or later.
- The feature named *Warehouse management only mode* is turned on in [feature management](../../fin-ops-core/fin-ops/get-started/feature-management/feature-management-overview.md). As of Supply Chain Management version 10.0.47, this feature is mandatory and can't be turned off.
...
1. If you use [Warehouse management only mode with external shared warehouses](wms-only-mode-external-shared-warehouse.md), then on the **Outbound shipment orders** FastTab, you must select an **Outbound shipment processing policy** that has its **Enforce shipment to order matching** option set to *Yes*. Learn more in [Outbound shipment processing policies](outbound-load-handling.md#outbound-shipment-policies).
1. (Optional) If your external source system uses different [inventory statuses](../inventory/inventory-statuses.md) than Supply Chain Management, select **Source system inventory statuses** on the Action Pane to set up the mapping. Learn more in [Source system inventory status mapping](wms-only-mode-exchange-data.md#inventory-status-mapping). 
1. (Optional) Warehouse management processes (WMS) can use an *Owner* inventory dimension to track the ownership of inventory for items shared across multiple source systems. To use the owner inventory dimension, products must have a tracking dimension group where the *Owner* dimension is enabled, and you must also create a *Warehouse inventory owner* configuration that contains a mapping for every *Owner* value that a source system sends in an order line. If a mapping is missing, the order message fails to process. This requirement also applies when the source system sends an empty owner value. In such cases, the configuration must map the empty value from the source system to the appropriate value in the *WOM* Supply Chain Management legal entity system. To create a warehouse inventory owner configuration, select **Warehouse inventory owners** on the Action Pane. Learn more in [Product master and reference data](wms-only-mode-external-shared-warehouse.md#product-master-and-reference-data).
...
As part of the [Source system](#source-systems) creation process, the system automatically creates the following [automated background processes](../../fin-ops-core/dev-itpro/sysadmin/process-automation.md):
...
If you're running warehouses that contain inventory from multiple owners, use the *prevent multiple owners* feature to ensure that all the items stored in a given warehouse location have the same owner. The feature prevents cross-mingling of inventory and allows each owner to have its own segregated storage space. Learn more in [Prevent multiple owners in warehouse locations](prevent-multiple-owners-locations.md).
...
To automatically release outbound shipment orders to the warehouse, set up a batch job to process the orders on a regular basis. Go to **Warehouse management** > **Release to warehouse** > **Automatic release of outbound shipment orders** to set up and schedule the job.
