...
| `"DomainName"` | <p>(Optional) This parameter applies only to the `"UsernamePassword"` connection type. It allows for a simplified sign-in process by specifying the domain part of the Microsoft Entra ID user principal name (UPN). If this field is not set, workers must enter their full UPN to sign in. By providing the domain part here, workers can sign in using just their username. However, entering the full UPN is still an option. |
