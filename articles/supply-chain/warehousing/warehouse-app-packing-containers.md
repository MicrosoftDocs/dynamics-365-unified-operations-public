...
| Identification via GS1 bar code scanning | Yes | You don't need to provide the quantity to pack (`QtyToPack`) as part of a primary input field. Instead, the system will automatically set the quantity to pack to a default value as part of the packing flow. The GS1 bar code setup must therefore have the **Allow overwriting** check box selected for the *QtyToPack* application identifier in the relevant [GS1 policy](gs1-barcodes.md#policies-for-menus) when you are using bar codes that include quantities. |
