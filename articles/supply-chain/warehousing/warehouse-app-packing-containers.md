...
| Identification via GS1 bar code scanning | Yes | Note that the quantity to pack (QtyToPack) is not a value the user must provide as part of a primary input field, the value will automatically get defaulted as part of the packing flow. The GS1 bar code setup must therefore use the setting **Allow overwriting** for the *QtyToPack* application identifier in the used [GS1 policy](gs1-barcodes.md#policies-for-menus) when using quantities in bar codes. |
