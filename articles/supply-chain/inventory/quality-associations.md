...
1. On the header, make the following settings:
    ...
    - **Item code** – Select which products should be included in the rule. Choose one of the following values:
        ...
        - *All* – All products.
    - **Item** – Depending on what you selected for **Item code**, choose the specific product or product group for which this rule should apply. Leave blank if **Item code** is *All*.
...
1. On the **Conditions** FastTab, make the following settings:
    ...
    - **Resource code** – Select the scope of resources to which the rule applies. This option only applies if the **Reference type** is *Route operation*. Choose one of the following values:
        ...
        - *Group* – Apply to a group of resources.
        - *Table* – Apply to a single, selected resource.
    - **Resource** – Depending on what you selected for **Resource code**, choose the specific resource or resource group for which this rule should apply. Leave blank if **Resource code** is *All*.
...
1. On the **Process** FastTab, make the following settings:
    ...
    - **Execution** – Choose one of the following values:
        - *Before* – Automatically create the quality order just before the business event occurs.
...
1. On the **Quality order process** FastTab, make the following settings:
    - **Event blocking** – Chose whether the document should be set on-hold until the associated quality order has been validated and closed. Choose one of the following values:
    ...
    - **Quarantine upon validation failure** – Set to *Yes* to automatically create a quarantine oder if the associated quality order fails validation.
...
1. On the **Specifications** FastTab, make the following settings:
    ...
    - **Expiration** – Select the date on which this quality association should expire.
    - **Worker** – Select the worker who should be assigned by default to the quality orders generated for this quality association.
    - **Quality order priority** – Select the priority that should be assigned by default to the quality order generated for this quality association.
