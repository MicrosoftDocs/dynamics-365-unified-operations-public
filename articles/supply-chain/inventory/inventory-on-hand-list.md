...
1. In the [**Filters** pane](#filters-pane), specify the product and locations that you want to view on-hand information for. Add filters as needed by using the **Add** button. To remove filters, use the **Remove** (**X**) button. Select **Apply** to view the results. The more filters you specify, the more specific the on-hand results are. Therefore, you see fewer rows and/or lower on-hand values.
1. On the Action Pane, select [**Dimensions**](#dimensions) to open the **Dimensions display** dialog box, where you can specify the dimensions that are used to present your on-hand inventory. The more dimensions you specify, the more granular the results are. For example, if you add the **Location** storage dimension, a **Location** column is added to the grid, and the results are expanded. Instead of showing one row per warehouse, they now show one row per warehouse location.
    ...
    > - The on-hand information is easiest to understand when the dimensions that are shown align with the filters that you're using. Use the [examples](#examples) later in this article to understand how different combinations of filters and dimensions lead to different results.
...
| Column | Description |
|---|---|
| Physical inventory | The physical quantity that's available in inventory. |
| Physical reserved | The total quantity that was physically reserved. |
| Available physical | The available (not reserved) quantity that's available in physical inventory.<p>**Available physical** is a calculated field. The value equals the **Physical inventory** value minus the **Physical reserved** value.</p> |
| Available physical on exact dimensions | The available physical quantity for all the dimensions that are shown in the grid. |
| Ordered in total | The total quantity that's included on inbound orders or that has a positive quantity in various inventory journals. |
| On order | The total quantity that's included on outbound orders or that has a negative quantity in various inventory journals. |
| Ordered reserved | The total quantity that's reserved on ordered receipts. The value in this field represents the total quantity of items in outbound transactions that have a status of *Ordered reserved*. Items that are reserved as ordered aren't physically available in inventory. Therefore, they can't be directly picked and delivered. |
...
1. If you want your selection to be the default the next time you open the **On-hand list** page, set the **Save setup** option to *Yes*. If you set this option to *No*, your selection is used only during the current session. Therefore, the next time you open the page, the current default selection is used.
1. Select **OK** to close the dialog box and apply your changes.

## Investigate and act on the on-hand information

The on-hand information on the **On-hand list** page is aggregated based on the filters and dimensions that you specify. In SQL terms, filters control the `WHERE` clause of the on-hand calculation, and dimensions control the `GROUP BY` clause. These clauses control how many records and columns are shown in the on-hand list. Nevertheless, you might still need to further inspect a specific value in the on-hand information.
...
- A check mark in the **Uses the warehouse management process** column indicates WMS-enabled items. They also show helpful warnings. You can learn more about each warning by hovering over it.
...
- The list might include empty rows (such as row 2 in the example that follows this list). These rows indicate that there's currently no on-hand inventory, related reservation, or related order on the dimension, but there was in the past.
...
The **On-hand list** page shows the following final dimensions:

- Item number
- Site
- Warehouse

In the **Filters** pane, set up the following filtering criteria:
