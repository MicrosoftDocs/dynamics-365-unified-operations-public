...
- Values shown in the **Physical inventory** column follow the same aggregation principles of filtering and requested dimensions as non-WMS items. The value is placed on the most-specific storage dimension. In example that follows this list, **Physical inventory** is shown per license plate, except for location *BULK-001* in Warehouse *24* (in row 7), which isn't license-plate enabled.
