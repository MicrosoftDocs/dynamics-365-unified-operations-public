...
Inventory Visibility provides flexibility by letting you set up the _indexes_ to speed up query. These indexes are based on a dimension or a combination of dimensions. An index consists of a *set number*, a *dimension*, and a *hierarchy*, as defined in the following table.
...
| Hierarchy | The hierarchy is used to define the supported dimension combinations in filter&groupby query parameters that can be speeded up. For example, you set up a dimension set that has a hierarchy sequence of `(ColorId, SizeId, StyleId)`. In this case, the system can speed up queries regarding four dimension combinations in groupby. The first combination is empty, the second is `(ColorId)`, the third is `(ColorId, SizeId)`, and the fourth is `(ColorId, SizeId, StyleId)`. The other combinations won't be speeded up. Filters has no restriction of order but should be inside these dimensions if you want it speeded up. For more information, see the example that follows. |
