...
> [!NOTE]
> If existing open work exist for given location, then it will not be possible to change the inventory status. For example, you did purchase receive for an item, but did not do the putaway step, then open work would exist on receiving location and trying to change the status would cause and error. Completing or Cancelling the work would allow you to change the status.
