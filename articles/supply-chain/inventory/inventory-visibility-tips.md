...
- About the application programming interface (API), multiple `SiteID` and `LocationID` values can be specified in each query. The maximum limit is defined as `NumOf(SiteID) * NumOf(LocationID) <= 100`.
