...
- The bulk API has a maximum limit for the records number in each request, it is 512.
...
- The inventory visibility cannot change any data for the data source `fno`, the data flow is one-direction, it means that all quantity change for the data source `fno` should just come from FNO (Supply Chain Management environment). You should not send onhand change request and reservation request for the data source `fno` using API, it is not allowed.
- If you add some new measure in FNO (Supply Chain Management environment), you also should add this new measure in the inventory visiblity, but the quantity change for this new meeasue should only come from FNO (Supply Chain Management environment).
