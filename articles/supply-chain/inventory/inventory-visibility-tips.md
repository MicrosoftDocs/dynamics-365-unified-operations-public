...
- Your [on-hand index configuration](inventory-visibility-power-platform.md#index) must include at least one on-hand index other than partition configuration (set number 0). If you are not interested in querying specific dimension combinations, you can set up an index with only one base dimension `Empty`. Otherwise, queries will fail with the error "No index hierarchy has been set."
