...
The *Quality management for warehouse processes* feature extends the capabilities of the basic quality management feature. It provides the option to create quality orders for the inventory that you send to the quality control location, although quality orders aren't always required. Therefore, it allows for a lightweight quality control process that is based on warehouse work.

## Key benefits

The *Quality management for warehouse processes* feature automatically generates work as part of the receiving process, to move the inventory quantity that is required for quality control to a quality control location. If the quantity that you receive exceeds the quantity that is required for quality control (according to the item sampling setup), the system moves the excess quantity to an inbound location that is defined in the location directive setup. After the quality order is validated, the system automatically generates work to move the quantity for the quality order to a new inbound or return location, based on the validation result and the location directive setup. The automatic generation of work that has only the quantity that must be moved to and from quality control provides an integrated process experience.
...
The following illustration provides an overview of the entities that enable quality orders for warehouse processes.

:::image type="content" source="media/quality-management-entity-diagram.png" alt-text="Quality management entities.":::
...
- **Quality item sampling** – Use this work order type to create work that moves registered inventory to quality control.
- **Quality order** – Use this work order type to create work that moves inventory from quality control to a new location, based on the location directive setup.
...
For example, you define a warehouse where the **Enable quality order for warehouse processes** option is set to *Yes*, and you have two quality associations that are defined for the *Purchase* reference type: one for all items and one for the *Registration* event type. The only difference between the two quality associations is the **Applicable warehouse type** value: it's set to *All* for one quality association and *Quality management for warehouse processes only* for the other. In this case, both quality associations are equally specific, and both are applicable.

The value of the **Test group** field for the quality associations is also a factor. This field defines the test procedure that must be applied. If the **Test group** value is the same for both associations, only one quality order is created, for the quality association where the **Applicable warehouse type** value is *Quality management for warehouse processes only*. If the **Test group** value isn't the same for both associations, two quality orders are created. The first quality order is created for the quality association where the **Applicable warehouse type** value is *Quality management for warehouse processes only*. The second quality order is created for the quality association where the **Applicable warehouse type** value is *All*.

> [!NOTE]
> The *Quality management for warehouse processes only* value is considered more specific than *All* when the criteria for the quality associations for groups 1 and 2 are the same, and when the test group is the same. Two quality orders are created only when the test groups differ.

#### Reference types

When the **Reference type** value is *Purchase*, and the **Applicable warehouse type** value is *Quality management for warehouse processes only*, set the **Event type** field on the **Process** FastTab to *Registration*. *Registration* is the only supported event type for the *Purchase* and *Inbound shipment order* reference types when you're using the *Quality management for warehouse processes* feature.
...
The **Quality processing policy** field on the **Quality order process** FastTab controls whether a quality order is also created when work is created to move an item to the quality control location. Set this field to *Create quality order* or *Create work only*. The default value is *Create quality order*.
...
The creation of quality order work is unrelated to the quality association setup. If a work template exists that has a **Work order type** value of *Quality order*, and if the query criteria are met for that work template, validation of a quality order triggers the creation of quality order work.

#### Referenced item sampling

Each quality association must reference an item sampling. An item sampling defines the quantity that you send for quality control. You can set it up so that it applies only to quality associations where the **Applicable warehouse type** value is *Quality management for warehouse processes only*. If the **Sampling scope** value for an item sampling is *Load* or *Shipment*, or the **Quantity specification** value is *Full license plate*, the item sampling can be referenced only by quality associations where the **Applicable warehouse type** value is *Quality management for warehouse processes only*.

If you define an item sampling that uses the *Quality management for warehouse processes only* applicable warehouse type, you receive an error if you try to reference it from a quality association that doesn't use the *Quality management for warehouse processes* feature.
...
Item sampling controls how often you send items for quality control. The *Quality management for warehouse processes* feature introduces the concept of *item sampling scope*. The system uses the item sampling scope when it evaluates whether and how to create quality orders, quality item sampling work, and quality order work.
...
- **Order** – The source document line is the basis for evaluating whether and how to create quality orders, quality item sampling work, and quality order work.
- **Load** – Loads are the basis for evaluating whether and how to create quality orders and work.
- **Shipment** – Shipments are the basis for evaluating whether and how to create quality orders and work.

> [!NOTE]
> When you set the **Sampling scope** field to *Load* or *Shipment*, the system uses the load entity and shipment entities, if they're available. If they're not available, the system uses the order entity.
...
The value of the **Per nth license plate** field controls how often quality orders are created in relation to the number of items that are registered. For example, a value of *3* sends every third item to quality control, starting with the first item. The value must be more than 0 (zero).

While workers receive items by using the Warehouse Management mobile app, the system validates whether a quality association is set up for each incoming item. If a quality association is set up, the system uses the item sampling record that is configured for that quality association to determine how it creates quality orders, quality item sampling work, and order work.

> [!NOTE]
> When you do receipt registration in the web client (by using the small registration page or the item arrival journal), the system doesn't create quality item sampling work or order work, regardless of the setup. Instead, for items that match a quality association, the referenced item sampling controls the creation of quality orders only.

## Examples of automatic generation of quality orders

The following examples show how the setup of a quality association and an associated item sampling affects the generation of quality orders when you set the **Applicable warehouse type** field to *Quality management for warehouse processes only*.

When the **Quantity specification** value is *Full license plate*, the **Per nth license plate** field controls which license plates the system creates quality item sampling work for. The first license plate always goes to quality control, and then the value of this field specifies that every *n*th license plate after that license plate should also go.
...
When a worker validates one of the quality orders that are shown in the preceding table, the system automatically generates quality order work to move inventory from the quality control location to the location that is defined in the location directive for the *Quality order* work order type. You can set up any location for this purpose, such as a return or storage location, depending on the test result for the quality order. For an example of this setup, see the [example scenario](#example-scenario) at the end of this article.

You can reopen a quality order that you already validated, as long as the quality order work that relates to moving the inventory from the quality control location doesn't have a **Work status** value of *Closed* or *In progress*.

## Process insights when multiple quality associations coexist

You can define and apply more than one quality association to the same source document line. You can set the **Applicable warehouse type** field to *Quality management for warehouse processes only* for some of those quality associations and *All* for others.
...
You now create a purchase order for a quantity of 10 of item A0001 for vendor 104. Then you register a purchase order line that has a quantity of 10 as received on one license plate by using the Warehouse Management mobile app. Here's the result:
...
1. **Validate the second quality order that you created (quantity = 10). The system creates quality order work for a quantity of 4.**

    A change in the inventory blocking quantity triggers the creation of quality order work. Because the sum of quality order quantities was 16, validation of a quantity of 10 causes the remaining quality order quantities to be 6. The inventory blocking quantity is reduced from 10 to 6. The reduced quantity of 4 is allotted to quality order work creation.

1. **Validate the first quality order that you created (quantity = 5). The system creates quality order work for a quantity of 5.**

    A change in the inventory blocking quantity triggers the creation of quality order work. Because the sum of quality order quantities was 6, validation of a quantity of 5 causes the remaining quality order quantities to be 1. The inventory blocking quantity is reduced from 6 to 1. The reduced quantity of 5 is allotted to quality order work creation.

1. **Validate the third quality order that you created (quantity = 1). The system creates quality order work for a quantity of 1.**

    A change in the inventory blocking quantity triggers the creation of quality order work. Because the sum of quality order quantities was 1, validation of a quantity of 1 causes the remaining quality order quantities to be 0 (zero). The inventory blocking is removed (that is, the inventory blocking quantity is reduced from 1 to 0). The reduced quantity of 1 is allotted to quality order work creation.

> [!NOTE]
> The creation of quality order work depends on the inventory blocking quantity that is referenced against one or more quality orders. If the sum of the quality order quantities exceeds the referenced inventory blocking quantity, the order that you validate the quality orders in determines the creation of quality order work.

## Canceling quality item sampling work

You can cancel the work that is created for quality item sampling. To control what occurs when you cancel this work, follow these steps:
...
1. On the **General** tab, on the **Work** FastTab, set the **Unregister receipt when cancelling work** option to one of the following values:
    ...
    - **Yes** – When you cancel quality item sampling work, the associated quality order is deleted, and the inventory is unregistered.
    - **No** – When you cancel quality item sampling work, the associated quality order isn't deleted, and the inventory isn't unregistered.

## Cross-docking

You can set up a quality association that creates item sampling work. However, when cross-docking exists in parallel with a quality association that creates quality item sampling work, if there's only enough quantity to satisfy cross-docking, the system creates only cross-docking work. In cases where the **Enable quality order for warehouse processes** option is set to *Yes* for the receiving warehouse, and the **Applicable warehouse type** field is set to *Quality management for warehouse processes only* for a quality association, the creation of quality item sampling work takes precedence over the creation of cross-docking work. If the quantity exceeds the requirement for cross-docking, the system still creates only item sampling work.

## Destructive testing

You can define a test group that performs destructive testing. In the case of a destructive test, the assumption is that, regardless of the test result, the quantity of the item that is tested is destroyed as part of the test. The way that the *Quality Management for warehouse processes* feature supports destructive testing resembles the way that quality management supports it when the feature isn't turned on. Before the quality order can be validated, the quality controller must specify the pick location for the quantity that is destroyed. You can register picking from the quality order page by selecting **Inventory \> Pick** on the Action Pane. After the pick for the quality order quantity is registered, validation can be completed.

## Example scenario

### Prepare the scenario

To work through this scenario, prepare your system as follows:
...
You must now prepare a basic setup that enables your system to support the *Quality Management for warehouse processes* feature for warehouse 51. (The demo data defines a quality management location named *QMS*. This location is referenced several times in this scenario.) You'll prepare the following elements, as described in the subsections of this section:
...
Create a quality association that uses the new item sampling.
...
You defined a quality association that uses the *Quality management for warehouse processes* feature to trigger the creation of a quality order. You set up the work and location data for warehouse 51 to ensure that specific work is created when purchase registration is done for item M9201. This setup ensures that every third license plate that you register is moved to a quality location (*QMS*), and that a quality order is created for the license plate quantity. Everything else is moved to putaway instead of the quality control location.
...
1. Continue to receive against the line, *5 ea* at a time, until the line is fully received. (A total of four license plates are created.)
...
1. Go to a mobile device or emulator that is running the Warehouse Management mobile app, and sign in to warehouse 51 by using *51* as the user ID and *1* as the password.
1. Go to **Inbound \> Purchase put away**, and put away each license plate from the previous procedure until you close all the work.
...
You'll now continue with the required base setup of the work class, work template, and location directive to enable quality management for warehouse processes. This setup ensures that the required work is created to move the quality order quantity from the quality control location to a designated warehouse location.
...
Warehouse workers can now pick quality order work by using the **QMS Putaway** menu item. Goods that fail quality control can be put in a return location, and goods that pass can be put in the bulk-001 location.
...
1. Select the first quality order for the quantities that you registered.
1. Select **Validate**. The status of the test updates to *Fail*.
...
1. Select the second quality order for the items that you registered.
1. Select **Results** above the lower grid. Update the **Result quantity** value to *5*, and verify that the **Test result** value changes to a check mark.
1. Select **Validate**, and close the page.
1. Back on the **Quality orders** page, select **Validate**, and do the validation. The status updates to *Pass*.
...
1. Go to **Warehouse management \> All work**.
1. Select the work that you just created, and notice that a second quality order work header is created, where the put location is *BULK-001*.
1. Go to a mobile device or emulator that is running the Warehouse Management mobile app, and sign in to warehouse 51 by using *51* as the user ID and *1* as the password.
