...
| Forecast reduction does not consider invoiced/delivered sales orders | When master plan has a demand forecast setup with forecast reduction method by transactions, master planning does reduce the forecast with invoiced/delivered sales orders while planning optimization does not. For example, if forecast is for 100ea, there are 10 ordered (purchase order), and for a sales order 5 delivered and 2 invoiced: then with built-in planning the forecast reduction will be 100-10-2-5=83 ea that should be purchased, while on planning optimization not considering the invoiced/delivered it would be 100-10=90ea. This means that it would suggest to supply for quantities that have already been delivered/invoiced, which may result in oversuppling for a certain period of time. |
