...
| Resource assignment | When working with infinite capacity, the classic built-in master planning will assign all planned orders to the same resource on a given resoruce group. This is improved in Planning Optimization so that the selected resource is random, and is not the same for all production orders. If it is needed to select always the same resource for all planned orders, the resource must be specified in the route. |
