...
| Min./Max coverage code with no values| In built-in classic planning, when Min./Max coverage code is used and no values are used for either min or max, master planning will treat the coverage code as requirement and create one order for each requirement. With planning optimization, one order per day will be created to cover for the full amount of the day.  |
