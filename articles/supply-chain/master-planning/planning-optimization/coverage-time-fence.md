...
A coverage time fence is the number of days after today's date (or, more precisely, the date when you do the planning run) that supply and demand are excluded. To help avoid delays, you must ensure that the coverage time fence is longer than the total lead time. The default system value is 100 days.
...
Every item (product) belongs to a coverage group. If you don't explicitly assign a coverage group to an item, a default coverage group applies. Every item inherits a coverage time fence from its coverage group. However, you can override this time fence for specific items.
...
You can specify a coverage time fence at the master plan level. By using this setting, you define the number of days that the master planning calculation covers for a master plan. This setting overrides any coverage time settings that you define for each relevant item and coverage group.
...
As you set up coverage time fences, consider the following points:
...
- The system doesn't generate requirement transactions for any supply and demand that falls outside the coverage time fence.
- If any approved supply and demand fall outside the coverage time fence, the system doesn't load them into the engine. Therefore, they don't trigger any replenishment, and the system doesn't calculate delays. Nevertheless, don't remove this supply and demand from the system.
- The system ignores variations in safety stock quantities (from minimum keys) if they fall outside the coverage time fence.
- The system ignores intercompany demand if the requested ship date that is calculated isn't inside the coverage time fence. For the deprecated master planning engine, the coverage time fence doesn't limit intercompany demand.
- The system ignores demand forecasts if the budget date isn't inside the coverage time fence. For the deprecated master planning engine, the coverage time fence doesn't limit demand forecasts.
...
## Filter derived requirements by coverage time fence with Planning Optimization

In Supply Chain Management version 10.0.49 and later, the *Filter derived requirements by coverage time fence with Planning Optimization* feature is available in the [Feature management](../../../fin-ops-core/fin-ops/get-started/feature-management/feature-management-overview.md) workspace. In Supply Chain Management version 10.0.48 and earlier, this feature isn't available, and Planning Optimization doesn't filter derived requirements by coverage time fence.

When you turn on the *Filter derived requirements by coverage time fence with Planning Optimization* feature, Planning Optimization doesn't replenish derived requirements that fall outside the coverage time fence that applies to them. Examples include:

- Planned transfer issues, where the replenishing warehouse has different coverage time fence settings.
- Planned BOM component issues, where the raw material has different coverage time fence settings.

When the feature is on, derived demand follows the same coverage time fence boundaries that apply to a sales order. The *coverage time fence boundary* refers to the coverage time fence that applies to the inventory dimensions of the demand. Planning Optimization first checks the item coverage line, then the coverage group, and finally the global coverage time fence.

Examples of derived and internally created requirements include:

- Planned transfer issue
- Planned BOM/formula component issue
- Planned BOM line type service issue
- Safety stock issue
- Expired supply issue
- Kanban component issue

This behavior applies to all coverage codes: *Period*, *Requirement*, *Min/Max*, *Priority*, and *Decoupling point*.
