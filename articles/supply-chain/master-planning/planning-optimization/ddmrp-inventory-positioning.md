...
In the pillow example, you might place your first decoupling point at the *foam billets* for the following reasons:

- It's difficult to source the materials used to make the billet and availability is volatile. This meets the criterion of *external variability*.
- The foam billets can be cut into many different shapes and sizes to create foam inserts for a variety of products that you manufacture, in addition to the pillow. This meets the criterion of *inventory leverage and flexibility*.
...
Based on this analysis, the following illustration shows what the pillow BOM would look like with the decoupling points highlighted with yellow inventory icons.
...
You could decide that the transfer time to move a blanket product between the distribution center and the stores violates your *customer tolerance time* because your customers expect the blanket to be in stock when they visit. In this case, you would set up a decoupling point for the blanket item at each of the three stores and each one would get different buffer levels based on that particular store's lead times, demand patterns, and so on.
...
Items become decoupling points when they belong to a coverage group that is configured with a **Coverage code** of *Decoupling point*. Therefore, the first step in setting up DDMRP is to decide which coverage groups you'll need to implement your DDMRP strategy and then create each of them using the following steps:
...
1. On the **Other** tab, make the following settings in the **Average daily usage** section:
    - **Average daily usage based on** – Select which time periods the average daily usage should be calculated based. Choose one of the following values:
        ...
        - *Blended* – Look at both the past and future usage. Settings for **Past period (days)**, **Forward period (days)**, and blending options all apply. <br>Blended average daily usage (ADU) = ((\[Past weighting\] × \[Past ADU\]) + (\[Forward weighting\] × \[Forward ADU\])) ÷ (\[Past weighting\] + \[Forward weighting\]).
