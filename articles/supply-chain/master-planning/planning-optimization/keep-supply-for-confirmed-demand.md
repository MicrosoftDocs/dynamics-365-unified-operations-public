...
> - This feature doesn't re-evaluate product substitution after requirements are confirmed. For example, planning doesn't substitute a confirmed requirement with an alternate item, even if the alternate item becomes available later.
> - Sales order demands for co-by-products aren't treated as confirmed demand, even if they have a confirmed ship date.
...
1. On the **General** FastTab, set the **Keep supply for confirmed demand** option:
   ...
   - *No* (default) - The system doesn't consider confirmed requirements. All planned orders that aren't approved and fall outside the freezing time fence are replanned during each planning run.

## Set the parameter on the master planning dialog

You can control this feature when you run master planning interactively. The planning run dialog provides the parameter, and the selected master plan automatically sets the default value.

To set the parameter:

1. Go to **Master planning** \> **Master planning** \> **Run** \> **Master planning**.
1. In the **Master planning** dialog box, on the **Parameters** FastTab, set the **Keep supply for confirmed demand** option to *Yes* or *No*. The behavior is identical to what is described in the previous section.
...
1. On the **General** FastTab, set the **Keep received supply outside positive days** option:
   - *No* (default) - Confirmed demand is pegged to on-hand inventory only when the demand falls within the positive days. This setting allows on-hand inventory that was previously backing a confirmed sales line to be reallocated to earlier orders if the supply arrives earlier than required.
...
### Identify confirmed requirements for the example scenario
