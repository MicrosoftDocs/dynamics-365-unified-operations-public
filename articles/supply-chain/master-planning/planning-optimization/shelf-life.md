...
You can track the shelf life of an item only if you track the item at the batch dimension. In other words, you must record the batch reference and the required dates when you receive or manufacture the item, and through every inventory transaction of the item. To manage this option, set up one or more tracking dimension groups to do the required tracking, and then assign the relevant items to these groups as required.
...
1. Follow one of these steps:
    ...
    - On the Action Pane, select **New** to create a new tracking dimension group. Enter a name and description, then select **Save** on the Action Pane.
...
1. To use the shelf-life settings, on the **General** FastTab, set the **Tracking dimension group** field to a tracking dimension group that tracks the batch dimension. You can set this field only when you're first creating a product. You can't change the value for existing products.
...
*Sellable days* functionality ensures that products from a batch that soon expires aren't sent to customers. Moreover, it ensures that when products are sent to a customer, an adequate number of sellable days still remain after delivery.
...
1. For each row, set the following fields:
    ...
    - **Item code** – Select one of the following values to specify the scope of items that are affected:
...
1. Select an existing group in the list pane or create a new one by selecting **New** on the Action Pane.
...
This example shows a basic example of shelf life, where pegging between the supply orders and the demand satisfies the following goals of the system:

- Minimize the sum of delays.
- Maximize the sum of FEFO supply.
- Minimize the replenishment of inventory.

The system uses the following item and master plan settings:
...
The system has the following sales orders for the item:
...
The system has the following supply for the item:
...
- SO2 can be covered by PO1, because PO1 arrives by the requested time and the expiration date is still valid. Therefore, the SO2 requirement is fully covered by PO1.
...
This example shows how the system's attempt to minimize delays can sometimes cause overordering.
...
The system has the following sales order:

- **SO1:** Qty = 2, requested delivery date = today + 3 days

Existing supply and a confirmed purchase order cover this demand:
...
This example shows how shelf life works when you add sellable days for an item.
...
The system has the following sales orders:
...
The system creates a list of pegging candidates based on the supply (FEFO) list and availability dates. Therefore, the on-hand inventory can't fulfill SO1, because that inventory expires before the end of the sellable days that the customer requires (requested receipt date + 5 days). PO1 can cover the SO1 requirement with two units and the SO2 requirement with one unit. Therefore, only SO3 still has uncovered demand for one unit of goods. To cover this requirement, the system creates the following planned purchase order:
...
The system can partially cover this demand with existing supply from the following confirmed purchase orders:
...
SO1 is covered by one unit from PPO1. PO2 covers SO2, because PO2 expires sooner than PPO1.
...
> [!NOTE]
> The system doesn't use PO1, because it arrives too late for S01 and expires before S02 is delivered. PPO1 overorders by one unit to enable the lead time to be 0 (zero), per trade agreement 2.
...
Because the system is configured to allow 10 negative days, it covers the demand of SO1 by using PO1, even though the result is a delay of three days because SO1 creates negative inventory until PO1 arrives. The system doesn't create a planned purchase order, even though the lead time is 0 (zero) and the creation of a planned purchase order would reduce delays.
...
The system can cover this demand with existing supply from the following confirmed purchase orders:

- **PO1:** Receipt date = today, qty = 1, expiration date = today + 1 day
- **PO2:** Receipt date = today + 2 days, qty = 1, expiration date = today + 3 days

However, the system must respect the restriction that shipped items can't be expired at the time of shipment. Therefore, the system can't use both PO2 and PO1 for SO1, because PO1 expires before PO2 arrives. The system creates the following planned purchase order to finish covering the demand for SO1:

- **PPO1:** Receipt date = today, qty = 1, expiration date = today + 10 days

The system can take advantage of the five negative days and use PO2 and PPO1 to cover SO1. However, this approach causes delivery to be delayed until PO2 arrives, and PO1 expires in the meantime. Therefore, the system covers SO1 by using PPO1 and PO1.
