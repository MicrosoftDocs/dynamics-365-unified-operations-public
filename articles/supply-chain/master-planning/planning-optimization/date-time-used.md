...
| Requested issue time | `RequestedIssueTime` | The date of issue that the user requests and is set in Supply Chain Management. This parameter is applicable only for released or approved planned orders. For planned orders, it's blank by default. | **Requested date** (`ReqDateDlvOrig`) |
| Required issue time | `RequiredIssueTime` | The required issue date that Planning Optimization adjusts. If this time falls in the past when Planning Optimization runs, it is moved to the first open day that is no earlier than today. If the time is marked as blocked in the calendar, it is instead adjusted to the first open day before that date. | **Requirement date** (`ReqDate`) and **Requirement time** (`ReqTime`) |
...
| Planned receipt time | `PlannedReceiptTime` | The date when the receipt arrives at the location. | **To date** (`FuturesDate`), **Delayed to time** (`FuturesTime`), and **Delivery date** (`ReqDateDlv`) or **Requested date** (`ReqDateDlvOrig`) if the order isn't released yet. |
| Required availability time | `RequiredAvailabilityTime` | The required availability date that Planning Optimization adjusts. | **Requirement date** (`ReqDate`) and **Requirement time** (`ReqTime`) |
| Expected receipt time | `ExpectedReceiptTime` | The expected receipt date for a released receipt. The user sets the value in Supply Chain Management. THe value isn't adjusted in Planning Optimization. This parameter applies only to released receipts. | **Requested date** (`ReqDateDlvOrig`) |
| Required receipt time | `RequiredReceiptTime` | The required receipt date that Planning Optimization adjusts. | **Requirement date** (`ReqDate`) and **Requirement time** (`ReqTime`) |
| Planned ordering time | `PlannedOrderingTime` | The ordering date that Planning Optimization calculates. | **Order date** (`ReqDateOrder`) and **Order time** (`ReqTimeOrder`) |
...
The plans in the following illustrations are at the day level, but Planning Optimization runs at a more detailed level. For example, because margins can be in hours, the planning ordering time can be January 22, 2021, at 11:35, and so on.
...
:::image type="content" source="media/planning-service-dates-1-small.png" alt-text="Screenshot of simple scenario diagram showing date parameters." lightbox="media/planning-service-dates-1.png":::
...
:::image type="content" source="media/planning-service-dates-2-small.png" alt-text="Screenshot of lead time scenario diagram showing date parameters." lightbox="media/planning-service-dates-2.png":::
...
:::image type="content" source="media/planning-service-dates-3-small.png" alt-text="Screenshot of margin scenario diagram showing date parameters." lightbox="media/planning-service-dates-3.png":::

### Example 4: Delay scenario

One sales order that has requested issue time on January 22 is covered by one purchase order. This example uses the same settings as example 3, but the planning date is moved to January 15. Backward scheduling (red markers) fails because the planned ordering time would have to be before today's date. Therefore, master planning must schedule forward, and delays occur.

The following illustration shows this scenario. (Select the illustration to open a larger version.)

:::image type="content" source="media/planning-service-dates-4-small.png" alt-text="Screenshot of delay scenario diagram showing date parameters." lightbox="media/planning-service-dates-4.png":::
...
:::image type="content" source="media/planning-service-dates-5-small.png" alt-text="Screenshot of transfer scenario diagram showing date parameters." lightbox="media/planning-service-dates-5.png":::
...
:::image type="content" source="media/planning-service-dates-6-small.png" alt-text="Screenshot of lead time with calendars scenario diagram showing date parameters." lightbox="media/planning-service-dates-6.png":::

### Example 7: Delay with calendars scenario

One sales order that has a requested issue time on January 22 is covered by one purchase order. This example uses the same settings as example 6, but the planning date is moved to January 13. Backward scheduling (red markers) fails because the planned ordering time would have to be before today's date. Therefore, master planning must schedule forward, and delays occur.

The following illustration shows this scenario. (Select the illustration to open a larger version.)

:::image type="content" source="media/planning-service-dates-7-small.png" alt-text="Screenshot of delay with calendars scenario diagram showing date parameters." lightbox="media/planning-service-dates-7.png":::
