...
1. In the **Firming** dialog box, on the **Parameters** FastTab, set the following fields. (Many of these fields take their default values from the **Standard update** tab on the **Master planning parameters** page.)
    ...
    - **Group by purchase agreement** – Set this option to *Yes* to group planned purchase orders that have the same vendors with existing purchase agreements and create one purchase order per purchase agreement. The option is automatically selected when **Group by vendor** option is selected. To use **Group by purchase agreement** option, **Find purchase agreement** option must be set to *Yes* on the **Master planning parameters** page.
