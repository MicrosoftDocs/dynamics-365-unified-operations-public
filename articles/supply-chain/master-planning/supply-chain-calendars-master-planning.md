...
This article provides an overview of supply chain calendars and how they affect master planning. It explains the different calendars used in the master planning engine, including how they affect the shipping and receiving dates in planned orders. It also provides recommendations for assigning, using, and updating calendars.

## Definition of a calendar

To define a calendar to use in your organization, go to **Organization administration** > **Setup** > **Calendars** > **Calendars**. Use the buttons on the Action Pane to add, remove, or edit calendars as needed.

To manage the details of a selected calendar, select **Working times** on the Action Pane. Each date entry in a calendar can be open or closed, or it can inherit its open/closed status and working times from the base calendar. The **Control** column on the **Working times** page specifies the value. For each date, the **Column** field shows one of the following values:
...
For any closed dates, the **Closed for pickup** checkbox is automatically selected. For open dates, you can manually select the **Closed for pickup** checkbox. This way, you indicate that work is performed on the date, but shipping isn't performed.
...
The order date of a planned purchase order indicates the date when the order is placed. It's an open date in both the vendor calendar and the coverage group calendar. Sometimes, vendors need a margin of a few days between the date when they receive the purchase order and the date when they can ship the goods. These dates are indicated in the vendor's margin days. However, if the item that's purchased is assigned to a coverage group that has margin days, those margin days override the vendor's margin days.

### Delivery date of a planned purchase order

The receipt date of a purchase indicates the date when you receive the goods. It's an open date in the calendar. To determine which days the purchase orders can be received on, the system considers the following calendars, in order from highest to lowest priority:
...
If a warehouse calendar is set, the shipping date is an open date in that calendar. If no warehouse calendar is set, the coverage group calendar is used.
...
If a coverage calendar is set, the receipt date is an open date in that calendar. If no coverage group calendar is set, the warehouse calendar is used.
...
Note that a coverage group calendar overrides receipt dates in master planning. Therefore, use coverage group calendars cautiously. They're especially useful in cases where replenishment must be done on specific days of the week.
...
Clarify who is responsible for assigning and updating calendars in their appropriate place. In the event of a breakdown or any other unusual change in working days, update the calendars accordingly. Rerun all tasks that depend on calendars, such as master planning and production scheduling, when calendars are updated.
