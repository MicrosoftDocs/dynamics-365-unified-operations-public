...
- **Open** - Indicates that work is performed on the selected day. The calendar is updated according to the working time template.
- **Closed** - Indicates work that isn't performed during the day.
- **Base calendar** - Indicates that the specific date inherits the working times and open/closed from the base calendar. Therefore, when the base calendar is updated, the selected calendar inherits operation times from it.

For any closed dates, the **Closed for pickup** check box is assigned automatically. For open dates, you can manually select the **Closed for pickup** option. This indicates that work is performed on the date, but shipping isn't performed.
...
The coverage group can be assigned in different pages:
    ...
    - On the **Item coverage** page. In the released products details page, select **Item coverage** to go to the item coverage page. In the overview tab you can see different parameters for replenishment depending on the site, warehouse, and product dimensions. The coverage group for each item is inherited from the coverage group on the **Released product details** page. This can be overridden by using **Use specific settings** or **Override group setting** on the **General** tab.
    - On the **Master planning parameters** page. If an item doesn't have a coverage group set in the previous pages, master planning takes the general coverage group set on master planning parameters. This is set up in **Master planning \> Setup \> Master planning parameters** in the **General coverage group** field.

### Calendars for a vendor

#### Vendor working days and shipping days

You can set up calenders that define each vendor's standard operating hours and the days on which they can ship goods. The system uses these calendars when calculating lead times, delivery times, and other dates. Follow these steps to set up your vendor calendars:
...
1. To set the days on which the vendor can ship orders (which might be different from the standard operating hours), expand the **Invoice and delivery** FastTab and choose a calender in the **Ship calendar** field. For more information about this setting and how it's used, see [Calculate requested ship dates for purchase orders (preview)](supplier-requested-confirmed-dates.md).
...
You can assign a calendar to a warehouse to indicate the open dates for receiving and shipping. If no calendar has been assigned to a warehouse, the system assumes that it's open all days.
...
To indicate that a warehouse is open for receiving but pickup isn't possible, you can use the **Closed for pickup policy** within the warehouse calendar. This also applies to customer pickups.
...
The transport calendar is set up in **Sales and marketing \> Setup \> Distribution \> Modes of delivery**. Select a mode of delivery and select the **Transport calendar** button.

A line can be created for each warehouse and mode of delivery, where the calendar is added in the **Transport calendar** column. It specifies the transport calendar that is applied when goods are shipped from the warehouse using the given mode of delivery.

To apply a transport calendar to all shipments using a given mode of delivery, a line can be created without specifying the warehouse.  It affects all shipments using the given mode of delivery, regardless of the warehouse.

If no transport calendar is assigned, it's assumed that all days are open for transport.

### Calendar for a customer

To indicate the dates when a customer can accept deliveries, you can assign a receipt calendar to the customer. If no calendar is assigned to a customer, it's assumed that the customer can receive orders on all days. This affects the receipt date on the sales order lines. If you select a date in the sales order lines that isn't open in the customer calendar, the system indicates that the receipt date isn't valid.

It's only possible to include one calendar per customer. If you need to include a calendar for each different address for a customer, you can create one customer per address and then assign its respective calendar.
...
To indicate the dates in which a legal entity can ship goods, you can set up a shipping calendar under **Organization administration \> Organizations \> Legal entities**. Select the legal entity and add the calendar in the **Foreign trade and logistics** tab in the  **Shipping calendar** field. The shipping calendar acts as a source of defaults for all warehouse calendars in the legal entity.

## How calendars affect dates in planning

### Order date of a planned purchase order

The order date in a planned purchase order indicates the date when the order is placed. It will be an open date both in the vendor calendar and in the coverage group calendar. Sometimes, vendors need a few days of margin between when they receive the purchase order and when they're able to ship the goods. These dates are indicated in the vendor's margin days. However, if the item purchased is assigned to a coverage group with margin days, these margin days override the vendor's margin days.

### Delivery date of a planned purchase order

The receipt date of a purchase indicates the date when you'll receive the goods. It will be an open date in the calendar. The following calendars are taken into account to indicate which days the purchase orders can be received, in order from highest to lowest priority:

1. Vendor's calendar
1. Coverage group calendar
1. Warehouse calendar for the receiving warehouse

The coverage group calendar can be set in different pages and will take priority in the following order:
...
If a warehouse calendar is set, the shipping date will be an open date in the calendar. If no warehouse calendar is set, it takes the coverage group calendar.
...
If a coverage calendar is set, the receipt date is an open date in the calendar. If no coverage group calendar is set, it takes the warehouse calendar.
...
Regarding the coverage group calendar, it's important to consider that that has an overriding behavior regarding receiving dates in master planning. Thus, it's recommended to use it with caution. Particularly, it's useful in the case when replenishment needs to be done specific days in the week.

### Updating SCM related calendars

While it's important that all relevant calendars are assigned in their respective place (vendor, customer, warehouse, mode of delivery, or coverage group), updating them is as important so they reflect the changes. The system defines the production, transfer, purchase, and sales order dates depending on the combination of the assigned calendars.

It's a best practice to clarify who holds the responsibility for assigning and updating the calendars in its corresponding areas. In case of a breakdown or any other unusual change in the working days, it's essential to update the calendars according to it. All tasks that depend on calendars, such as master planning and production scheduling, must be rerun when calendars are updated.
