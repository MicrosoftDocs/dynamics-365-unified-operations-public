...
- **Priority** - It is the strategy that allows you to replenish buffers for the product according to its minimum, reorder point and maximum. Whenever the projected on-hand falls below the reorder point, it will replenish until its maximimum. For replenishing, priority and not date is considered, meaning that it ensures that the most urgent orders are replenished first. More information on [Priority based planning](https://learn.microsoft.com/en-us/dynamics365/supply-chain/master-planning/planning-optimization/priority-based-planning)
- **Decoupling point** - It is the coverage code used to indicate that you would like to setup the product as a decoupling point (buffer) according to the Demand Driven Material Requirements Planning (DDMRP) methodology. DDMRP is a specific planning and execution methodology. To specify that a product is a decoupling point, you must fill in all its dimensions and overwrite its *Coverage code* on its *Item coverage* form under *Use specific settings* and selecting a coverage group that has coverage code *Decoupling point*. More information on the DDMRP methodology on [DDMRP overview](https://learn.microsoft.com/en-us/dynamics365/supply-chain/master-planning/planning-optimization/ddmrp-overview).
