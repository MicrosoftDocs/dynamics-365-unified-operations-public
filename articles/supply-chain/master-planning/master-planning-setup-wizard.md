...
If the calculated number of threads exceeds 75 percent of the available number of threads, it's capped at 75 percent of the number of threads available for each customer. (The number of available threads is determined for each customer.)
...
The bundle size is set to **1**. This value is often the best value, because it helps improve the performance of master planning.
...
- **If you manufacture any of the items:** The firming bundle size is the larger of these two values: **10** and the bundle calculation.
- **If you don't manufacture any of the items:** The firming bundle size is the larger of these two values: **50** and the bundle calculation.

Bundle calculation = (Number of planned orders × (Firming time fence ÷ Coverage time fence) ÷ Number of threads) ÷ 10

### Cache size

The cache size is set to **Maximum**. This value is often the best value because it helps improve the performance of master planning.
...
If you manufacture items, a **Manufacturing setup** page appears later in the wizard.

## Scope of the current plan

On the **Scope of the current plan** page of the wizard, answer questions related to how far in the future various requirements are considered and calculated in master planning. Each question asks whether you want to use a feature and how you want to configure it.

For example, for the forecast plan feature, the wizard asks, "Do you want to use a forecast plan in master planning so that planned orders are suggested to fulfill the forecasted demand?"

The following options are available:

- **No** – Master planning doesn't suggest planned orders to fulfill a forecast. On the **Time fences** tab of the **Master plans** page (**Master planning > Setup > Plans > Master plans**), the wizard sets the **Forecast plan (time fence)** option to **Yes** and sets the number of days to **0**. This setup overrides the time fence that is specified in the coverage group. Because the number of days is set to **0**, the feature isn't used.
- **Yes, as defined in this master plan** – A field becomes available, where you can enter the number of days that master planning suggests planned orders to fulfill the forecasted demand. The wizard sets the **Forecast plan (time fence)** option to **Yes** and sets the number of days to the number of days entered in the **Forecast plan** field on the **Time fences** tab of the **Master plans** page. This setup overrides the values that are set in the coverage groups.
- **Yes, as defined in the coverage** – The wizard sets the **Forecast plan (time fence)** option to **No**. The time fences that are specified in the coverage group are used to specify how long you plan for the forecast.

The remaining questions on this page and their answers follow the same schema:

- **No** – The **Forecast plan (time fence)** option is set to **Yes**, and the number of days is set to **0**.
- **Yes, as defined in this master plan** – The **Forecast plan (time fence)** option is set to **Yes**. The number of days that you enter is used and overrides the values that are set in the coverage groups.
- **Yes, as defined in the coverage group** – The **Forecast plan (time fence)** option is set to **No**.

Learn more in [Job scheduling](/dynamics365/unified-operations/supply-chain/production-control/job-scheduling).

## Scheduling options

The **Scheduling options** page appears only if you answer **Yes** to the "Do you manufacture any of the items planned?" question on the first page of the wizard.
...
- **No** – Operations scheduling is used.
...
The questions on the **Updates of demand and supply** page relate to firming, action messages, and delays.

The master planning setup is updated based on your answers, according to the same schema described in the previous section:

- **No** – The **Time fence** option on the **Master plans** page is set to **Yes**, and the number of days is set to **0**.
- **Yes, as defined in this master plan** – The **Time fence** option is set to **Yes**. The number of days that you enter overrides the values that are set in the coverage groups.
- **Yes, as defined in the coverage group** – The **Time fence** option is set to **No**.

For calculated delays, your answers to the questions in the wizard update the corresponding parameters on the **Calculated delays** tab of the **Master plans** page.
...
You can also modify the parameters in the new configuration. The parameters are separated into parameters that apply to your legal entity and parameters that apply only to the master plan that you selected. To view all the setup parameters that you can change by using the wizard, select **Show all parameters** at the bottom of the page. You can then change the parameters.

When you select **Finish**, the new configuration is applied. If you select **Cancel**, none of the changes are applied.
...
- There are many suppliers. The average lead time for items is one week. A group of items from the same supplier has a lead time of seven weeks.
...
- **Coverage:**
    ...
    Therefore, two coverage groups are created for this example. One coverage group has a coverage time fence of two weeks, and the other has a coverage time fence of eight weeks.

    If **Yes, as defined in this master plan**, is selected as the answer, the number of days entered should exceed the longest lead time of all the items. However, because many items don't have to be planned so far in advance, many planned orders are calculated but never used. Therefore, the master planning runtime increases.
...
- **Scheduling:**
    ...
    Contoso Manufacturing must plan and schedule the individual jobs that it performs on the shop floor. Therefore, it uses job scheduling.
...
- **Sequencing:**
    ...
    A route is defined for the production of each final item. Therefore, master planning schedules the orders in time according to the defined routes.
...
Contoso Retailer is a distribution company in the fashion industry. It uses master planning to calculate when to place purchase orders, based on its forecasted sales. Here are some of its characteristics:

- Contoso Retailer uses a demand forecast to predict sales. Purchase orders are planned according to the forecast.
...
- **Forecast demand:**
    ...
    - **Question:** "Do you want to use a forecast plan in master planning so that planned orders are suggested to fulfill the forecasted demand?"
    - **Answer:** "Yes, as defined in this master plan."

    Contoso includes a demand forecast to predict its sales. Therefore, master planning must recommend planned orders to fulfill the forecast.
...
- **Firming:**
    ...
    Because Contoso Retailer creates purchase orders directly from the planned purchase orders, it's useful if the planned purchase orders are automatically firmed. Because the company runs master planning every day, a firming time fence of one day automatically firms all the orders that are required for the next day.
