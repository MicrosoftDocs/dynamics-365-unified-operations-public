...
An action message is a system-generated suggestion to change an existing planned, approved, or firmed order. Note that action messages are only created for existing planned orders not approved when they are within the freeze time fence. Outside of the freeze time fence, action messages are only created for existing approved planned orders and firmed orders.
