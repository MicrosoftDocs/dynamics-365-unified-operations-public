...
When users self-register, contact records are automatically created for them. Therefore, you can't ensure that a user selects the correct customer account and legal entity. On the other hand, the invitation process lets an admin assign the correct customer account and legal entity to the contact record before an invitation is sent. If you're thinking about customizing the solution so that users can self-register, consider the possible consequences.

## Video

> [!VIDEO https://learn-video.azurefd.net/vod/player?id=5a9d44db-29b0-429b-ad96-22951664353c]

The [Invite customers to register and use your customer portal](https://youtu.be/drGUYHX9QIQ) video (shown earlier) is included in the [finance and operations playlist](https://www.youtube.com/playlist?list=PLcakwueIHoT_SYfIaPGoOhloFoCXiUSyW) available on YouTube.

## Prerequisite setup

Power Apps portals store contacts as records in the **Contacts** table in Microsoft Dataverse. Dual-write then syncs these records to Microsoft Dynamics 365 Supply Chain Management as required.

:::image type="content" source="media/customer-portal-contacts.png" alt-text="Screenshot of system diagram for Customer portal contacts.":::

Before you start to invite new customers, make sure that you enable the **Contact** table mapping in dual-write.
...
1. Set the **Account Number** field to the customer account number that you want the user to have in Supply Chain Management.
...
This section describes the web roles delivered with the Customer portal.
...
The administrator oversees and maintains the website. This person provisions and sets up the Customer portal. The administrator maintains the IT and security aspects of the portal, and makes sure that everything runs smoothly. The administrator might also customize and/or change the portal by adding new functionalities, creating new roles, and more.

#### Customer representative

A customer representative works for a customer company and is responsible for managing the orders that the company places. The customer representative can see all the orders that the company places and the users who placed them. Additionally, the customer representative can see information about the overall account and which contacts can place orders on behalf of that account.

#### Authorized users

Authorized users can place orders and view the status of the orders that they place. However, they can't view the status of orders that other users in their company place.
