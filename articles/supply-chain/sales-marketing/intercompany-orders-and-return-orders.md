...
When you create an intercompany sales order, Microsoft Dynamics 365 Supply Chain Management automatically creates a corresponding purchase order. Similarly, when you create an intercompany purchase order, a corresponding intercompany sales order is automatically created.

This process applies to two-legged orders (transactions between two related companies) and to most three-legged orders (when an intercompany transaction originates with a sales order for an external customer).
...
- When Company A first creates a sales order for an external customer, a purchase order can be created automatically in Company A. This action prompts the automatic creation of an intercompany sales order in Company B. The three-legged order chain consists of a sales order and a purchase order in Company A and an intercompany sales order in Company B.

## About intercompany return orders

You can return intercompany items much like ordinary returns. However, with intercompany items, the return order from the external customer creates an intercompany purchase order. This purchase order leads to the automatic creation of an intercompany sales return order. You can also return an intercompany item without an external customer return order.
