...
description: This article describes how to work with product bundles on sales orders and related documents and how bundle-item prices are allocated to each bundle component. Internal documents (such as picking lists and packing slips) list the component items. However, external documents (such as sales order confirmations and sales invoices), show only the parent item.
author: Henrikan
ms.author: henrikan
...
This article describes how to work with product bundles on sales orders and related documents and how bundle-item prices are allocated to each bundle component. A product bundle comprises a parent item (product bundle item) and multiple component items. The parent item is entered on a sales order line, so that order entry is more efficient. However, it's then exploded into the component items. Internal documents (such as picking lists and packing slips) list the component items. However, external documents (such as sales order confirmations and sales invoices), show only the parent item.
...
Parent items that are flagged as product bundles are treated differently than other BOM items. Here's a list of the differences:

- You explode product bundles included in a sales order by confirming the order. To do so, open the sales order and, on the Action Pane, open the **Sell** tab and select **Confirm sales order**. You must never explode a product bundle item from the **Sales order lines** FastTab (by selecting **Sales order line \> Explode \> BOM lines** from the toolbar) because then the system will treat the item as a BOM rather than a product bundle.
...
- **Component item:** 1 pcs. of item *1000* (the laptop itself), with a base sales price $1,900.00
- **Component item:** 1 pcs. of item *S0021* (insurance), with base sales price $150.00
- **Component item:** 1 pcs. of item *Support* (support), with a base sales price $500.00
...
As the sales order is being picked and packed, the internal documents will only show the components of the product bundle. However, the packing slip and invoice must include a full product bundle; otherwise, they can't be posted. For example, the dialog box shows three component items. If you try to delete one of them, you receive an error message that states that all products in the product bundle must be shipped before they can be invoiced.
...
A partial amount can be shipped and invoiced only if the quantity is reduced for all components of the bundle. For example, a quantity of five of the *Laptop bundle* item is entered on a sales order. After the sales order is confirmed, the three component items are shown on the sales order, and the quantity of each is five. By default, during shipping and invoicing, the quantity is set to five for each component. However, you can adjust the quantity down to three for all three component items. In this case, three full product bundles will be shipped and invoiced. The remaining two product bundle items (a quantity of two of each of the three component items) can be shipped and invoiced later.

### Invoice the sales order

The final step is to invoice the sales order. As with the sales order, the invoice dialog box shows the component items while the printed invoice shows only the parent item.
...
Invoice journal must not include the parent item from the product bundle because any processes that are performed after the invoice is posted are based on that invoice journal. For example, if you create a credit note from the **Sell** tab on the Action Pane, the credit note that is created will include the component items but not the parent item.

## Important limitations

The following important limitations apply when using product bundle items:

- The product bundle feature isn't supported in Microsoft Dynamics 365 Commerce channels, such as online, point of sale (POS), and call centers. The feature also isn't support by the prospect-to-cash solution for Dynamics 365 Supply Chain Management and Dynamics 365 Sales. Items that are configured as product bundles shouldn't be added to orders or transactions that are created in Commerce channels or in the prospect-to-cash solution.
- Product bundle items aren't supported in intercompany orders. In a 3-legged scenario, product bundle items aren't exploded during sales order confirmation on the original sales order, while in a 2-legged scenario, the sales order confirmation process is blocked.
- Product bundle items aren't supported in direct delivery
- Product bundle items aren't supported in delivery schedule
- The sales order line delivery type is always *Stock* for bundle components regardless of the direct delivery setting on the released product.
- Reprinting an invoice or sales order confirmation requires that the related sales order with the product bundle order lines still exists. If the sales order lines have been deleted, then the reprinted invoices and sales order confirmations won't include the parent item, but the component items.
- Miscellaneous charges added to a parent item line aren't allocated to the component item lines when exploded.
