...
| Sales and marketing| Accept payments with payment links using Adyen connector| This feature enables SCM users to generate payment links for customers, which eliminates the need for customers to share sensitive card details verbally over the phone, thus enhancing security and trust. These payment links also enable customers to use modern payment methods like digital wallets that aren't feasible in traditional phone transactions. Moreover, if customers are undecided about their order, SCM users can create the orders on a payment hold. If customers make the payment within a preconfigured duration, then the order is released for fulfillment. Otherwise the order gets canceled. | Enabled by Admins |
