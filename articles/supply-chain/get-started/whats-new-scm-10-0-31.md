...
| Master planning | Source products and materials from multiple vendors using Planning Optimization | This feature lets you source products and materials from multiple vendors. You configure it by defining multiple vendors for an item and setting allocation percentages for each of them. Planning Optimization then automatically suggests a vendor for every planned purchased order, based on the defined allocation percentages and your order history. |
| Production control | Enable use of a numpad in the sign-in page | This feature lets admins add a numpad control to the sign-in page for production floor execution. Workers can then sign in by using the numpad to enter their badge ID or personal number. <!-- KFM: Small doc update may be needed (list on page of PFE FM features, maybe a new config control?) --> |
