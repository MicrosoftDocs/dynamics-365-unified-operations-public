...
| Sales and marketing | Pricing management performance enhancement | Enable this feature to improve performance of pricing calculation, by leveraging rule hash to filter potential pricing rules and speed up matching logic. Hash string will be generated for existing rules when the feature is enabled, and all new pricing rules created after that will have hash generated during creation. The new batch job '**Generate pricing rule hash value**' could be manually scheduled to update hash for all existing pricing rules. This job can be run to generate hash for pricing rules which for some reason are missing hash (delta) or regenerate hash for existing pricing rules. |
