...
| Sales and marketing | Pricing management performance enhancement | Enables Pricing management to make some types of calculations more quickly. This is achieved by using the hash string as new matching logic to speed the pricing rule matching. The new batch job '**Generate pricing rule hash value**' is for updating hash value for all rules. This job can be run for delta or run for all companies. By default, hash value is generated when a pricing rule is created. In rare cases, existing pricing rules will not be applied because of missing the hash value or wrong hash value. Running this batch job can mitigate the issue. |
