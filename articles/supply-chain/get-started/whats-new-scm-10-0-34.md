...
| Sales and marketing | Adjusting reverse match for a settlement process | When this feature is enabled, performing the ‘Reverse match’ action in the deduction workbench in Trade allowances, will correctly bring back the previously matched amount to the open transaction. Without this feature, reversing a match, performed for a deduction transaction against an open transaction, will correctly put the deduction transaction back in status open, if fully unmatched, however the amount for the open transactions will not be updated with the reverse match value. |
