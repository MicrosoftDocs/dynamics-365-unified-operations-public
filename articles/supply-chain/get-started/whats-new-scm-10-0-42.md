...
| Master planning | [Collaborate on supply plans within and across teams](/dynamics365/release-plan/2024wave2/finance-supply-chain/dynamics365-supply-chain-management/collaborate-supply-plans-within-across-teams) | Coming soon | Enabled by default |
...
<!--KFM:  Not sure whether the two planning features should be included here... -->
...
|---|---|
| <p>**Module:** Master planning</p><p>**Feature management name:** *Batch CTP*</p> | Adds a new delivery date control type called *Batch CTP*, which provides confirmed delivery dates for sales order lines when the current dynamic master plan is run. This delivery control makes capable to promise (CTP) functionality available when you are using Planning Optimization. |
| <p>**Module:** Master planning</p><p>**Feature management name:** *(Preview) Improve Planning Optimization performance by merging and queueing plan regeneration jobs*</p> | Makes it faster to run certain master planning scenarios when multiple items are being updated (such as during a net requirements update). It enables merging those plan regeneration jobs for different items into a single job to improve execution time for jobs in the queue. If a job can't be merged, it's added to the queue. |
...
| <p>**Module:** Master planning</p><p>**Feature management name:** *(Preview) Near real-time CTP*</p> | Enables the near-real time capable to promise (CTP) calculation for items that have it set as delivery date control on the sales line. When a sales line with CTP delivery date control is added to an order or edited, the calculation starts automatically. When the field CTP status is set to *Ready*, you will be able to see the confirmed delivery date calculated by the system. |
| <p>**Module:** Procurement and sourcing</p><p>**Feature management name:** *(Preview) Speed up purchase order updates based on incoming vendor emails with Copilot*</p> | With this feature, Copilot automatically scans incoming emails, identifies messages that are related to purchase orders, and classifies them. Copilot then extracts the intent and content of each message and compares it with information that exists in the system. Finally, Copilot identifies order confirmations and change requests and can automatically update purchase orders as needed or mark them as confirmed. <!-- KFM: This probably should be removed. --> |
...
| <p>**Module:** Production control</p><p>**Feature management name:** *Allow to select validation logic for expiration of raw materials: either against planned consumption date or production order delivery date*</p> | Adds a setup option for validating batch expiration dates when reserving a batch of raw material to be used during production. It adds a new value for the **Validate batch expiration on** field to the **Item model groups** page. The setting lets you choose the criteria to use when validating the batch expiration date. You can now choose to use the planned consumption date (the raw material date) or the planned delivery date, as established on the production BOM line or batch order formula line. <!-- KFM: This description doesn't match what I see in the UI --> |
