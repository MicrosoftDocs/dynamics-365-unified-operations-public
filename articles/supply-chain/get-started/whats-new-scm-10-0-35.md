...
| Warehouse management | [Optimize warehouse management implementation and maintenance](/dynamics365/release-plan/2023wave1/finance-operations/dynamics365-supply-chain-management/optimize-warehouse-management-implementation-maintenance) | *Coming soon* | Enabled by default |
...
| Inventory and warehouse management | (Preview) Purge archived inventory transactions | Clears data from archived inventory transactions data after the data has been successfully copied to a Dataverse data lake. Before enabling this feature, make sure the schema of `InventTransArchive` and `InventTransArchiveEntity` tables are identical or some data may be lost during synchronization. |
| Inventory and warehouse management | Archive inventory journals to history tables using archive service <!--KFM: Missing "(Default)" --> | Archives day-to-day transaction tables to history tables. Once archived to history tables, the data from the day-to-day transaction tables will be purged. |
| Sales and marketing | (Preview) Archive sales order from history tables to Dataverse managed data lake and purge | Performs the last step in the archival process, which is archiving from history tables to Dataverse managed data lake. | (Preview) Archive sales orders to history tables using archive service | Performs the first step in the archival process, which is archiving from day-to-day transaction tables to history tables. Once archived to history tables, the data from the day-to-day transaction tables will be purged. |
| Sales and marketing | (Preview) Archive sales orders to history tables | Performs the first step in the archival process, which is archiving from day-to-day transaction tables to history tables. Once archived to history tables, the data from the day-to-day transaction tables will be purged. |
...
| Sales and marketing | Keep existing sorting on intercompany sales lines when updating them | Adds an option that lets users keep existing sorting on intercompany sales line update. |<!--KFM: New feature is enabled by default? -->
| Transportation management | (Preview) Enable receiving with specific goods in transit order on mobile device. | Lets users can pick the specific goods-in-transit order for receiving when multiple goods-in-transit orders exist for the same voyage, container, item number, and purchase order number. |
