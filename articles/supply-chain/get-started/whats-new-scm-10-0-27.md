...
| Product information management | (Preview) Cross-company data sharing for products | This feature enables sharing of data for products using a cross-company data sharing policy based on a master company. <!--KFM: FM description still includes typo. This should probably be on release plan. Really publishing this for 10.0.27? --> |
...
| Sales and marketing | Sales order creation performance enhancement | Currently, sales order creation has a partial uptake of the source document framework, but the data created is not used further in the product. This feature skips the creation of added source document tables such as `SourceDocumentHeader` and `SourceDocumentLine` to both improve performance and save database space. |
