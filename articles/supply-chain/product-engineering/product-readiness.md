...
When you create a new engineering product, the system will check whether there is a readiness check policy <!-- KFM: Do we mean whether a readiness **policy** is set for the category? Is it the policy that controls which readiness checks are generated? BNG yes--> set up on the engineering product category (applicable on released product level, released variant level, and engineering version level). If this is the case, the following things occur:

- Readiness checks are created for the productaccording to the applicable policy. <!-- KFM: According to the applicable policy? BNG yes-->
- The engineering version <!-- KFM: engineering product? BNG no, it is the version, the check of if it is active or not is on the version, all the versions of the product will be invavive--> is set to inactive, which blocks the product from being used. All the versions for the specific product will be set to inactive.
...
- The engineering version <!-- KFM: variant? BNG it is the version what has the active chekmarkl--> is set to inactive, which blocks the product from being used.
...
You can find the open the readiness checks for a product on the **Released products details** page by clicking on **Readiness checks** on the upper menu on the tab Product, under the field group Readiness checks. Or from the **Product version** page, click on **Readiness checks** under the field group Checklist. <!-- KFM: Where can we find it? What is it called? BNG updated -->. 
...
- Select **Check/complete** on the Action Pane to check and complete the readiness check, after which the **Status** will update to *Passed*. <!-- KFM: What happens here? Do we see a new form and/or make some settings or something? BNG, no the check is just marked as "done" -->
- Select **Skip** in the action pane if you prefer to skip a check that isn't mandatory. This can be applicable when there is no data setup which is also not required for the particular product <!-- KFM: I don't understand this example BNG for example, there could be the case where you setup a template product in the release policy, and that template product will default the "default order settings" for the product - how it behaves for warehouse, inventory and purchase. It may be good to have a manual check to review the default order settings. A good way would be to skip it if no changes were made and mark it as complete if changes were made, to keep track to if changes were made or not-->. This can be the case for the cost price calculation for a product that is still in the design phase. If you choose this option, the **Status** will update to *Skipped*.

Depending on how the readiness policy is configured <!-- KFM: Do we mean the readiness policy? BNG yes, before this was on the category, and we moved it into a policy, that is why you see it so often-->, when a readiness check **Status** updates to *Passed*, an extra step may be required to approve the readiness check. In that case, select **Approval** to complete the readiness check. This approval step is always mandatory when the readiness check is skipped.
...
From the readiness checks form, the checklist opens by clicking on **Start checklist** in the action pane <!-- KFM: Where are we starting here? from the readiness checks form-->. When you have finished working through the checklist, the system will validate whether the readiness check is passed based on the settings in the questionnaire and, if so, the **Status** will update to *Passed*. If the readiness check isn't mandatory, you can instead choose to skip it, in which case the **Status** will update to *Skipped*.

Depending on how the readines policy is configured <!-- KFM: Do we mean the readiness policy? BNG yes-->, when a readiness check **Status** updates to *Passed*, an extra step may be required to approve the readiness check. In that case, select **Approval** to complete the readiness check. This approval step is always mandatory when the readiness check is skipped.
...
<!-- KFM: I can't find any of settings described here--it seems like all I can do is select a "readiness policy". Does the system still work like this? BNG readiness policy yes-->
...
Use product readiness policies to manage the readiness checks that apply to a product. The readiness policy is assigned to the engineering category, so all the checks in the readiness policy will apply to the engineering products based on the engineering category. <!-- KFM: How can we manage them? Where is this feature? BNG added more info before-->
...
| Setting | Description |
| --- | --- |
| **Product type** |  Select if the policy will apply to products of type Item or Service. <!-- KFM: What affect does this have? Seems like it can't be changed after saving. BNG indeed, it cannot be changed after saving, it indicates if the policy applies for an Item or a Service--> |
...
| Setting | Description |
| --- | --- |
| **Process area** | Area to which the check is related to. <!-- KFM: Describe how to use this setting. --> |
| **Type** | System check, manual check or Checklist (questionnaire) <!-- KFM: Describe how to use this setting. --> |
| **Name** | Enter a name for a checklist. It will be populated for system and manual checks. <!-- KFM: Describe how to use this setting. --> |
| **Description** | Enter a description for checklist. For system or manual checks it is populated and explains the focus of the check. <!-- KFM: Describe how to use this setting. --> |
...
| **Mandatory** | Select this check box for checks that must be completed by the assigned user (can't be skipped). If is it mandatory, then the check cannot be skipped. <!-- KFM: Describe how to use this setting? --> |
