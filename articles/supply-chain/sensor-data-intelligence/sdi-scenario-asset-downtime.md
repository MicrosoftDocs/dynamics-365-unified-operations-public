...
The *asset downtime* scenario lets you use sensor data to monitor the availability of your assets. If you set up a sensor that sends a signal when a production job on a machine resource produces output, but no sensor signal is received within a specified interval, a notification is shown on the supervisor's dashboard.

## Scenario dependencies

The *asset downtime* scenario has the following dependencies:
...
1. In the **Asset downtime** scenario box, select **Configure** to open the setup wizard for this scenario.
...
1. Select **Next**.
1. On the **asset downtime threshold** page, define how long after the last *part-out* signal the system should send a asset downtime notification. There are two ways to define the threshold:
    ...
    > [!NOTE]
    > Typically, you will use a *part-out* signal to monitor asset downtime. Therefore, you should verify that the threshold for each machine resource is longer than the machine requires to produce each part.
...
After you install your sensors and configure the scenario, you can view asset downtime events in Supply Chain Management. This section describes where and how to view this information.

### View asset downtime data on the Resource status page
...
The following illustration shows an example of asset downtime data on the **Resource status** page during standard operation.

![Asset downtime data on the Resource status page during standard operation.](media/sdi-resource-status-downtime-up.png "Asset downtime data on the Resource status page during standard operation")

The following illustration shows an example of asset downtime data when downtime is detected.

![Asset downtime data on the Resource status page when downtime is detected.](media/sdi-resource-status-downtime-down.png "Asset downtime data on the Resource status page when downtime is detected")

### View asset downtime on the Notifications page
...
The following illustration shows an example of a asset downtime notification.

![Example of a asset downtime notification.](media/sdi-resource-status-downtime-notification.png "Example of a asset downtime notification")
