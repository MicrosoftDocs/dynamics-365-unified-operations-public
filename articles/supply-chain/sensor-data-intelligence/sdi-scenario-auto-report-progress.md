...
description: Learn about the production auto report scenario, which uses a sensor to report progress on a production order.
...
In the *production auto report* scenario IoT‑connected sensors are leveraged to capture production output in real time and automatically post progress at configurable intervals based on unit conversion factors. 

This feature leverages IoT‑connected sensors to capture production output in real time. It uses the sensor signal to automatically post progress on production orders. Workers get live visibility into produced units directly in Production Floor Execution, with support for up to two levels of unit tracking (e.g., pieces per box,boxes per pallet) 
