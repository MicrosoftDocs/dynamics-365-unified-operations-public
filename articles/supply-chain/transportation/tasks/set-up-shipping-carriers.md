...
4. In the **Name** field, type a value.
5. In the **Load template ID** field, select a load template to associate with the service. The load template defines maximum measurements for weight and volume of an entire load. For example, the load template might represent the size of a container or truck. Load templates are also specified for loading building templates, which can then be used in the [load building workbench](load-building-workbench.md) to help you apply load building strategies when creating loads. As a result, you'll be able to select a suitable shipping carrier service for each load.
