...
This article describes how to work with goods-in-transit orders. Only the **Landed cost** module uses this type of order. When an order or voyage is set up to use goods-in-transit processing, you don't have to wait until the goods are received in the warehouse before you can invoice them. Instead, the goods are invoiced when they leave the vendor's warehouse or port of origin, and the financial costs are recognized when the voyage begins. This functionality helps you correctly take ownership of inventory, because goods often become the property of your organization when they leave the shipping port.

When you use goods-in-transit orders, you receive the financially updated items in an interim warehouse known as a *goods-in-transit warehouse*. The goods stay in this warehouse until they can be received at the final destination warehouse (that is, the warehouse defined on the purchase order line). You can't manually remove them. When a goods-in-transit order is invoiced, goods are immediately moved to the goods-in-transit warehouse to signify the change in ownership.

As long as the items are in transit, they aren't available in inventory and can't be picked from inventory for a delivery. However, you can view the goods-in-transit inventory. You can also use the goods for master planning. In this case, use the confirmed delivery date on the purchase order line as the expected date when the inventory is available for consumption. The following sections describe the setup that is required to process inventory and voyages by using the goods-in-transit concept and functionality.
...
When you set the **Goods in transit management** option to *Yes* for the applicable terms of delivery record, the goods are placed in the goods-in-transit warehouse. This action is triggered only if the inventory receipt isn't processed before an invoice is processed. When you set the delivery terms for an order to use goods in transit, users can no longer post a product receipt for the purchase order. If they try, an error occurs. The error message states that they must use the goods-in-transit functionality to proceed.
...
The Landed cost module adds two new types of warehouse: *goods in transit* and *under-delivery*. You can select warehouses of both types as default warehouses. To successfully process goods-in-transit orders, you must configure both the goods-in-transit warehouse and the under-delivery warehouse on the **Warehouses** page. Then, for each default warehouse for use with Landed cost and goods in transit, ensure that the goods-in-transit warehouse and under-delivery warehouse are selected for the available warehouses of each type.

The *goods in transit* warehouse type are associated with your goods-in-transit warehouse. That warehouse is used to process the goods on goods-in-transit orders before they're received at the final destination warehouse.

When the purchase order invoice is posted for a voyage that is enabled for goods-in-transit functionality, inventory is purchased in the receiving warehouse. It's then immediately transferred to the configured goods-in-transit warehouse. This transfer signifies the change in ownership of the goods that aren't yet physically arrived at the destination warehouse.

In general, one goods-in-transit warehouse is enough for each site if Site and Warehouse are the only inventory dimensions used for inventory management. If the Location inventory dimension is also used, ensure to set up a goods-in-transit warehouse for each combination of a site and warehouse, so that the default location can also be specified.
...
Goods can be received by using a mobile device, as long as the mobile device menu and the **Warehouse management** module are set up to receive goods on a goods-in-transit order. This section describes the setup that is associated with goods-in-transit receiving.
...
Landed cost adds a new work order type named *Goods in transit* to the **Location directives** page. Configure this work order type in the same manner as the [purchase order work order types](/dynamicsax-2012/appuser-itpro/create-a-work-template).
...
Landed cost adds a new work order type named *Goods in transit* to the **Work templates** page. Configure this work order type in the same manner as the [purchase order work templates](/dynamicsax-2012/appuser-itpro/create-a-work-template).
...
If you cancel a work header for an over transaction that is already registered, the over transaction is first reduced by the canceled quantity. After the over transaction is reduced to a quantity of 0 (zero), the record is removed, and any additional quantities are unregistered against the principal order quantity.
...
> [!NOTE]
> If sales orders are marked to purchase orders to reserve incoming stock, goods-in-transit orders are split if a purchase order line is marked to multiple sales order lines. Any goods-in-transit order that is split must be manually received one by one.

## Posting rules for landed cost

Landed cost adds two new posting rules that you can configure. Use these posting rules to financially post the direct purchase order invoice amounts and identify ownership of the goods when they leave the point of origin. This process replaces the concept of *goods received not invoiced*, because the goods are invoiced before they're received.

To work with posting profiles, go to **Inventory management** > **Setup** > **Posting** > **Posting**. On the **Purchase Order** tab, you see the following new posting rules:
