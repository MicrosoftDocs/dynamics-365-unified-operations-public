...
| Section | Description |
|---|---|
| Voyage Editor | The **Generate data Manually** and **Keep the auto-generated tracking dimension number while splitting source order line** parameters are introduced since version 10.0.39.<ul>  <li>**Generate data manually** : Instead of generating data from voyage editor form once filter value is changed, the parameter allows generating data manually to improve performance and user experience.</li><li>**Keep the auto-generated tracking dimension number while splitting source order line**: When this parameter is enabled,it will keep the auto-generated tracking dimension number while splitting the source order line. Else, it will generate new tracking dimension number on the split source order line. </li></ul> |
