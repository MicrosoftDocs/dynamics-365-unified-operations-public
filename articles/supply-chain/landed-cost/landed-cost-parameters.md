...
| **Exchange rate type** | The default collection of exchange rates used for multi-currency calculations on a voyage and voyage costs. |
...
| **Automatically&nbsp;update number of cartons** | When set to *Yes*, all cartons will be added together and shown at the voyage and container levels. When set to *No*, the number of cartons is initially set to zero and can be edited manually as needed. |
...
| **Allow zero costs** | Set this to *No* to display an error if a user tries to post a cost estimate for a voyage invoice or purchase order that doesn't include a value for the expected voyage cost. The error message tells the user that a cost of zero can't be allocated and invoice posting will fail. The user can then update the estimate manually (or reconfigure auto cost) and then pull through the updated value or delete the cost if it doesn't apply.</br></br>Set to *Yes* to allow the voyage cost to be empty. In this case, a price of zero will be allocated according to the cost area. The zero-price cost can then have a vendor cost invoice processed against it when the voyage is received.</br></br>We recommend that you configure the estimate on the auto cost record to prevent a zero-price cost from appearing. Although this estimate isn't completely accurate, it should still be more accurate than assuming zero cost. <!-- KFM: Add a link to the topic about auto cost. --> |
| **Adjustment posting date** | When you post an accounts payable voyage cost invoice, the settlements table (inventory adjustments) will also be updated. This field controls the date set by default on the **Select voyage costs** page while in the invoice journal. Choose one of the following values:<ul><li>**Transaction date** – Default to the date of the journal (posting date).</li><li>**Purchase order invoice date** – Default to the financial posting date of the stock (purchase order) invoice.</li><li>**Selected date** – Allow the user to specify a posting date. The user will be able to leave the date blank, but will receive an error on posting the cost invoice if it's still blank at that time. </li></br></ul> |
| **Use purchase invoice voucher** | This setting has an effect when the **Post to charge account in ledger** option is enabled on the **Invoice** tab of the **Accounts payable parameters** page.</br></br>When this is set to *Yes*, cost accrual transactions will use the same voucher number as that used for the purchase invoice.</br></br>When this is set to *No*, the system will take the next available number for the **Costs accrual voucher** sequence set up on the **Number sequences** tab of the **Landed cost parameters** page. |
...
| **Post adjustments as variance** | This setting has an effect when the **Post to charge account in ledger** option is enabled on the **Invoice** tab of the **Accounts payable parameters** page.</br></br>When this is set to *Yes*, the inventory adjustment transactions related to a variance between the estimated costs and the actual costs are posted to a variance account. This overrides the standard functionality by forcing the variance to post to the variance account.</br></br>When this is set to *No*, inventory adjustment transactions related to variances are handled based on the costing method and cost type code configuration. For standard cost, the variance will still post to the variance account. For moving weighted average (WMA), the variance will post either to the variance account or to inventory (if the cost type is set to debit item). |
...
| **Multiple warehouse tolerance** | This parameter controls what will happen when voyage includes several order lines that are destined to be delivered to several different warehouses. These order lines could be spread across one or more purchase orders. Choose whether the system should *Accept* the action, give a *Warning*, or show an *Error*. |
...
| **Section** | **Description** |
| --- | --- |
| **Tolerances** | The **Outside volume tolerance** and **Outside weight tolerance** settings establish thresholds beyond which the system will warn that the goods are overweight or over volume when a user adds them using the **Voyage editor** page. Each is defined as a percentage of the maximum volume and weight (respecitively) set for the relevant shipping container type. We recommend setting this somewhere between 5% and 10% of the volume or weight. |
