...
| Section | Description |
|---|---|
| Voyage Editor | The **Voyage Editor** section was added in Supply Chain Management version 10.0.39. It provides the following new settings:<ul>  <li>**Generate data manually** – When this option is set to *Yes*, data shown on the **Voyage editor** page isn't updated automatically each time you change a filter value. Instead, you can change several filter values before updating the page manually, which improves page performance and user experience.</li><li>**Keep the auto-generated tracking dimension number while splitting source order line** – When this option is set to *Yes*, the system preserves the autogenerated tracking dimension number after splitting a source order line. When this option is set to *No*, the system generates a new tracking dimension number after splitting a source order line. </li><li>**Allow creating voyage in batch** – When this option is set to *Yes*, the system enable use batch mode when user create and update shipping containers. By creating and updating shipping containers in background mode, you can help improve system performance when you work with shipping containers that have many shipping lines. </li></ul> |
