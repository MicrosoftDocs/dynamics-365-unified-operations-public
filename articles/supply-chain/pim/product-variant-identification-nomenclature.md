...
Each product variant has a number and a name. The product variant identification nomenclatures let you select the segments to include in each product variant number or name. Select the following segments on the **Product nomenclature** page:
...
After you define a product variant identification number nomenclature, associate it with a product dimension group. All product masters that reference this product dimension group are assigned product variant numbers according to the nomenclature. However, product variant name nomenclatures can't be associated with product dimension groups. You can also assign a product variant identification nomenclature directly to a product master. In this case, the product variants that belong to the product master are assigned product variant numbers and names according to the nomenclatures.
...
In this case, the product variant number for a red, small, polo T-shirt is TS1234-Red-Small-Polo.

## Nomenclature of constraint-based configurations

For constraint-based configurations, you can create a dedicated nomenclature for the configuration product dimension. On the **Product nomenclature** page, select the following segments:

- Number sequence value
- Text constant
- Attribute value

Each component in a product configuration model can have its own configuration nomenclature. You can only use attributes that belong to the component. You can't use attributes from subcomponents or user requirements.
...
1. Text constant: "AAA"
1. Attribute value: Length

In this case, the configuration ID for wood material that has a length of 78 is WoodAAA78.
...
1. Text constant: "&"
1. Configuration group: Front grill

In this case, the configuration ID for a standard cabinet that has a cloth front grill is M0007&M0021.
...
1. Assign the nomenclature to a product dimension group that has the configuration dimension.
1. Define a configuration nomenclature for the components or BOMs used to configure the product variants.
...
For this example, use a product variant number nomenclature that includes the following segments:
...
Enter the following values for segments:

- Product master number = **M0099**
- Material = **Plastic**
- Length = **12**

In this case, the product variant number is M0099\_PlasticAAA12.
...
In this case, the product variant number is D0123//M0008&M0022.

## Numbering conflicts

In some cases, a product variant number nomenclature that you set up might not produce unique product variant numbers. For example, the product variant numbers aren't unique if one active product dimension isn't included in the nomenclature for a product master that uses the predefined variant configuration technology. The way that you handle conflicts varies, depending on the configuration technology.

### Predefined variants

An error occurs if you try to manually create or automatically generate product variants, and more than one product variant ends up with the same product variant number. To avoid this scenario, use all active product dimensions in the product dimension group. Alternatively, include a number sequence to help guarantee that the product variant numbers are unique.

### Constraint-based configurations

Depending on the nomenclature, the system might try to assign a non-unique product variant number to a configuration. In this case, the system uses the number sequence for the configuration dimension as the product variant number instead, and you receive a warning. To avoid this scenario, include enough attributes in the nomenclature to help guarantee unique product variant numbers. Also, make sure that the **Reuse** option is turned on for the component.
