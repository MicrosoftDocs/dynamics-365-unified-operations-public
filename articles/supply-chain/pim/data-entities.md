...
| Item - barcode association | EcoResProductBarcodeAssociationEntity | This entity is used to export products and bar codes. It allows change tracking, updates, and deletes. To use the entity, the feature *Item-barcode improvements* must be enabled. Its entity key is AssociationID, which creates the association between the barcode and the product. To add support for this key, the new table InventitemBarcodeAssociation will be populated for existing item-barcode data when you turn on the feature. The table is populated using a batch job and if your barcode table has a large number of records, it could significant time to run the batch job. Therefore, we recommend that you plan to enable the feature (and therefore run the batch job) at a time that fits your business needs. |
