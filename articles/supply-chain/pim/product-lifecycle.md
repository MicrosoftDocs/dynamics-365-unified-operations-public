...
1. Use the **Enabled business processes** FastTab to control which of the available business processes can be used with products in the current lifecycle state. For each process that is listed on the FastTab, view and set the following values:
    ...
    - **Policy** – Select one of the following values to control whether and how the business process is permitted for products that are in this lifecycle state:
    ...
    > [!WARNING]
    > Although the **Process area** values are editable, you shouldn't change them. For each row, the value shown in the **Process area** column relates directly to the value shown in the **Process** column. These combinations are generated by the underlying framework and altering them might break system‑driven validations or cause incorrect business rule behavior.
