...
Create the product definition in Supply Chain Management. You can also import it from product lifecycle management (PLM), product data management (PDM), or product information management (PIM) systems. When you use more than one instance of Supply Chain Management, one instance typically acts as the master of the product data for all other instances. A large set of data entities supports this approach and enables the export and import of product definition data from one instance to another.

To support the distribution of product data to many instances, Supply Chain Management lets you use Microsoft Dataverse. You can export the product definitions from an instance of Supply Chain Management to Microsoft Dataverse. You can then use the product definitions to provision other business applications, such as Dynamics 365 Sales, with product data.
...
Choose among three configuration technologies:
...
- The dimension-based configuration is typically used in manufacturing scenarios and lets you use the Configuration dimension in the definition of the bills of materials (BOMs). After you select a specific configuration, the system uses the subset of BOM lines that are valid for that configuration for planning and production. This concept is also known as *global BOM*, because one shared BOM is used for all configurations of a product.
...
*Released products* are products that you release to a specific legal entity. You can release products in bulk to one legal entity or many legal entities at the same time. Because you might need to add various properties and attributes of the products for each legal entity, the **Released product maintenance** workspace helps you monitor and complete the recently released products in each legal entity or in the suborganizations of a legal entity.

### Released product maintenance workspace

You can configure the **Released product maintenance** workspace from the **Configure my workspace** menu item. Select a category hierarchy and category by which to filter the workspace. To adjust the relevant product data in the workspace, you can also define, in days, the time fences for **Recently released products** and **Stopped released products**.

The workspace consists of a summary of tiles and two lists. The **Open cases** list shows product change cases that have products in the selected product category hierarchy that aren't completed and closed. The **Recently released** list shows products that are released within the time fence set in the workspace configuration. For each item in the list, validation is run, and the validation status is shown. This status might indicate that the required configurations for the legal entity aren't completed. From the list, you can directly access the **Released product details**, **Product attribute maintenance**, **Product category maintenance**, **Default order settings**, and **Text translations** pages to complete the required configuration of the product.
