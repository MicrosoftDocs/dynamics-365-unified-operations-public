...
| Manual = No | Manual = Yes | Auto generate | No mapping | Both product numbers and item numbers use the **Item number** number sequence. | Both product numbers and item numbers use the **Product number** number sequence. | Both product numbers and item numbers will follow the product number sequence. This is the recommended approach to import bulk products with the Released product creation V2 data entity. Note this approach must only be used bulk import of items (several rows) and when items are not created through the user interface. If it is needed to bulk import products and the creation of products during the UI, it is recommended to follow the below approach instead. To transition from this approach for the bulk import of products to import and manual creation of products through the UI, you will need manually adjust the Next number in the item number number sequence to match the Next number in the product number number sequence. Then you could switch to the below approach|
| Manual = No | Manual = Yes | No mapping | No mapping | Both product numbers and item numbers use the **Product number** number sequence. | Both product numbers and item numbers use the **Product number** number sequence. | Both product numbers and item numbers will use the product number sequence. However, this configuration won't work if there is more than one item (row) to be imported. Note this approach must be used if you need to both import products using the entities (only one row is possible to be imported at a time) and creating products through the UI.   |
