...
To use the hazardous materials processes, first create a regulation. Regulations define how the printed shipping text is created for an item. They also define the associated modes of delivery.
...
These regulations help determine what information shows when you print the shipping text for an item. You can define as many regulations as you must comply with.
...
The regulation is a shared table and isn't specific to each legal entity. However, modes of delivery are legal entity–specific. Therefore, when you set up a mode of delivery, select the relationship between the regulation, legal entity, and mode of delivery.
...
For reference purposes, list the countries or regions that the regulation is relevant for. However, when shipment reports are run, only the mode of delivery determines the regulation. When you review a warehouse shipment, there's no direct link to the mode of delivery. The shipment can be associated with a shipping carrier and carrier service. When you define a carrier service, you must associate it with a mode of delivery. This relationship is used on shipment reports such as the bill of lading to find the shipping print text for an item.
...
A hazardous material class is typically mapped to the list of classes provided in the regulation that you're conforming to. For example, US regulation CFR 49 lists "class 3" as flammable and combustible liquids. You can set up the classes that are relevant to the materials that you must classify.

Each class is assigned to a material setup in the materials list related to the regulation. Assign a class to each released item as required, to describe the hazardous nature of a product.
...
| Field | Description |
|---|---|
| Class code | Enter a code to identify this class. You define this code for the item. It is used in lookup lists when you assign a hazardous material class to a released item. |
...
For classes that don't have any divisions, create a division where the code is *0*. For example, in the IATA regulation, class-7 radioactive materials have no subdivisions. In this case, you create a *0* division that you can associate with a released product when you assign the class.
...
Hazardous material compatibility groups establish which hazardous material classes and divisions can be shipped together. When operators create warehouse loads or shipments, they can run a compatibility check that issues a warning if the load or shipment includes items that don't all belong to the same compatibility group.
...
| Field | Description |
|---|---|
| Class | Select a hazardous material class that's compatible with all other classes in the group. |
...
| Restrict from air transport | Select this check box to indicate that this class of hazardous material shouldn't be transported by air. |
| Restrict from sea transport | Select this check box to indicate that this class of hazardous material shouldn't be transported by sea. |
