...
| Combination rank | The *combination rank* for a price attribute group lets you define what occurs if multiple rules from the same price component code apply to a given order. If the concurrency mode is based on the *price attribute combination rank*, the rule that has highest rank applies. |
...
>
> You can have any number of price component code records for each of the remaining price components.
...
1. On the **General** FastTab, set the following fields:
    ...
    - **Maintenance mode** – Select one of the following values to specify how you'll create or generate the different combinations of price attribute groups that the price component code supports when you create pricing rules for it. You can edit this field only for new records. (It becomes read-only when you save the record.)
    ...
    - **Price component code group** – Select a price component code group. Set up and maintain your component code groups on the **Price component groups** page (**Pricing management > Setup > Price component codes > Price component groups**).
...
1. If you set the **Maintenance mode** field to *Combined*, use these guidelines to set up the **Price attribute group combination** FastTab:
    ...
    - For each attribute group, set the following fields:
        ...
        - **Header price attribute group** – If you set the **Header type** field to *Group*, specify the attribute group that the price attribute group combination provides so that header attributes can be specified in pricing rules.
        ...
        - **Line price attribute group** – If you set the **Line type** field to *Group*, specify the attribute group that the price attribute group combination provides so that line attributes can be specified in pricing rules.
...
1. If you set the **Maintenance mode** field to *Separate*, use these guidelines to set up the **Price attribute group combination** FastTab:
    ...
    - For each attribute group, set the following fields:
        ...
        - **Line price attribute group** – If you set the **Line type** field to *Group*, specify the attribute group that the price attribute group combination provides so that line attribute can be specified in pricing rules.
        - **Combination rank** – Assign a rank to the price attribute group combination. The rank resolves concurrency if more than one pricing rule applies to the price component code. For more information, see the [Price attribute combination rank](#rank) section.
...
In this situation, the rule that matches the row with the highest **Combination rank** value applies.
...
The following table shows the attribute group combinations and combination ranks that the system automatically generates based on these settings.
