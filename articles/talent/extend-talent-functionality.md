...
> -	Specify the form control from which the application can retrieve its context data. For example, an application might use data about a worker. If you enter the **Worker** page in the **Context** field, the **Worker** page will open when start the application. An entry in the **Context field** is optional. 
> -	Set the size of the dialog on which the PowerApps application will run. The dialogs are designated as “small” or “large” to optimize the user interface of your application for running on a phone a larger device respectively. 
