...
Use areas of responsibility to indicate the work roles, processes, and products that a worker in a position for that job would be responsible for. An example of an area of responsibility for a job titled â€œAccountantâ€ might be â€œFinancial reporting for Product Aâ€.

Positions
----------

Positions are an important element of the lower level of an organization hierarchy. A position is an individual instance of a job. For example, the position, â€œSales manager (East),â€ is just one of the positions that is associated with the job, â€œSales manager.â€ Positions exist in a department and are assigned to workers.
...
1.  Create a custom position hierarchy type called â€œWidgetâ€ to create a hierarchy that includes positions responsible for working on the widget cleaner product.
