...
| Job openings | All created jobs. Deleted jobs aren't exported. The sub-folders contain candidate applications, with sub-folders for candidate application attachments and offer packages, where applicable. If the candidates had themselves applied for the job via the Talent jobs portal using attachments, the candidate applications sub-folder will contain those attachments used at the time of application as well. If the candidate was manually added to the job, to find the associated attachments for the candidates, please use the Candidates folder in the root of the downloaded zip to locate the candidate using the "Auto-number" value found in the candidate json in the job applications sub-folder. The candidate sub-folder will not be present if the candidate was deleted from the job. |
