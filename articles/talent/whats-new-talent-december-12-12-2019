...
## Changes in Core HR
Changes described in this section apply to build number 8.1.2660.
...
With this release, in this unique scenario HcmWorkerPayrollInfo records are no longer created under without worker reference. 

### Message log associated to the action is not being populated when the action fails to complete - (374007)

This release includes a change has made to populate the action message log when an action fails in certain scenarios. 

### CDS Integration batch jobs creation - (388030)

With this change, CDS integration batch jobs are created when the service is enabled.

### Adding additional pick list values to custom fields are not being reflected in CDS after clicking apply on the custom fields form - (379599)

With this change new pick list values entered in Talent are now synched to CDS when you choose to apply your changes.

### Update to CDS for Leave bank transaction entity turns into an insert on Talent side - (352938)
