...
title: Management of customer information for Italy
...
---
# Management of customer information for Italy

[!include [banner](../includes/banner.md)]

## Introduction

This topic describes how customer information (such as the customer's fiscal code) can be handled in the Retail point-of-sale (POS).

You can specify the customer's fiscal code when you create or edit a customer master record in POS. You can also specify it for a sales transaction by copying from the transaction customer or by entering manually. The customer information can then be printed in both regular and fiscal receipts and be used for the purposes of the National lottery. It is also possible to search for a customer in POS by the personal fiscal code.
...
The following needs to be configured before using this functionality:
...
The *Add customer information* operation can be used to add the customer information (such as Fiscal code) to a sales transaction by either copying it from the customer that is specified for the transaction or entering it manually.
...
In addition to the *Add customer information* operation, if the customer information is not specified for a sales transaction, an inquiry of customer information can be triggered automatically after a sales transaction is finalized.

Select the **Enable inquiry of customer information in sales transactions** parameter in the **Tax parameters** field group on the **Functions** fast tab of the **POS Functionality profiles** page to activate the inquiry of customer information.

### Set up receipt formats

You can configure receipt formats, so that the customer's fiscal code is printed in receipts.
...
### Scenario 1: Sale to anonymous customer
...
1. Populate the new customer's attributes. Specify the customer's fiscal code in the **Fiscal code** field. Save the customer and add the customer to the transaction.
1. Register payments for the transaction and finalize the transaction.
...
### Scenario 3: Changing customer information for sale to named customer
...
This topic is a deployment guide that shows how to enable the customer information management in the Microsoft Dynamics 365 for Retail localization for Italy.
