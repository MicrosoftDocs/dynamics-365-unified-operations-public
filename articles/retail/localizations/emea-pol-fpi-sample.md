...
The fiscal integration for Poland includes samples of the integration with the fiscal printers support POSNET THERMAL HD 2.02 protocol. This was developed based on the fiscal integration framework. For details about the fiscal integration functionality, see [Fiscal integration for Retail channel](fiscal-integration-for-retail-channel.md). The full list of printers support POSNET THERMAL HD 2.02 protocol can be found at the site [POSNET](https://www.posnet.com.pl).
...
- Excluding the customer deposit amount.
- Printing fiscal receipt in Customer order scenarios.

## Design

The integration for the fiscal printer is implemented in two extensions:
...
- Hardware station extension - Handles the response from Commerce.HardwareStation.PosnetThermalFVFiscalPrinterSample.FiscalPrinterHandler. Submits commands generated by CRT extension to the fiscal printer by calling the functions of the POSNET driver provided by manufacturer. Handles device errors.
