...
# Show Order attributes in the POS transaction screen using the Attribute control(this feature is available starting from 8.1.3):
...
9. After the designer is started, drag the Attributes panel anywhere in the screen layout designer and adjust the size according to your screen width.
	...
	11. Close the screen layout designer by selecting the Close button (X) in the upper-right corner. When you're prompted, select Yes to save your changes.
...
> [!NOTE]
> The below changes are required only if you are running the application with update 8.1.2 or lower. 
Starting from 8.1.3 use the new Attributes panel to set or update the attribute value in POS. With this new control you **no** need to write any additional code or create UI to set the attribute value in POS. In the attribute control we added the UI to set or update the attribute value. Check the **Show Order attributes in the POS transaction screen using the Attribute control** section in this document for more details.
