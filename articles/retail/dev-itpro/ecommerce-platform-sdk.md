...
5.  Update the path of the RetailStorefrontWebSite from IIS Manager.
    ...
    -  Hence, the physical path of the RetailStorefrontWebSite must be updated to point to â€œC:\\inetpub\\RetailWeb\\Storefrontâ€ to access web storefront on the same ports as before. Another option would be to create a new website and have that point to the inetpub location.
...
You will need to update the â€œretailServerUrlâ€ inside the web.config file of the RetailStorefrontWebSite. The following two fields will need to be updated to use the machine name instead of local host:
