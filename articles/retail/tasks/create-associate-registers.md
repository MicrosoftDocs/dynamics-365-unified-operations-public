...
8. In the EFT POS register number field, type a value.
    * The EFT POS register number is used to inform the payment processor which payment terminal is sending authorization requests. This value is often called the "Terminal ID" or â€œTIDâ€. The TID can generally be found on a sticker on the payment device.  
9. Click Save.
