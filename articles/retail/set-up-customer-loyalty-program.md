...
- Free or discounted shipping is one of the highly motivating factors for customers to buy online. To enable the retailers to set up shipping promotions, with 10.0 release, we have introduced a new type of promotion named “Shipping threshold discount” which the retailer can define the thresholds, which once met, will qualify the customers for discounted or free shipping. For e.g. spend $35 for free 'Two day ship' or Free 'Two day ship' for all loyalty customers. These discounts are only applied to the shipping charges applied to the orders. Since a retailer can set up multiple types of charges e.g. Handling, installation etc. so the retailer needs to specify which charge is considered shipping charges. This configuration is named “Shipping charge code” and is available on the Customer orders tab under “Retail parameters” form. This discount honors all the existing standard discount capabilities i.e. the retailer can restrict these discounts with the coupons such that only the customers with these coupons can get these discounts. Also, these discounts leverage the Price groups capability to determine the eligibility of such discounts e.g. the retailer can choose to run these promotions only in the online channels and/or across channels for such certain customer groups such as loyalty customers. Once the order lines with the specified delivery mode meets the defined threshold, then the shipping discount gets applied and reduces the shipping charge as per the discount set up. 

> [!NOTE]
> Unlike other periodic discounts such as Quantity,Simple, Mix and Match, & Threshold discounts, the shipping discount does not create discount lines, rather directly edit the shipping charge.
