...
- Free or discounted shipping is one of the highly motivating factors for customers to buy online. To enable the retailers to set up shipping promotions, with 10.0 release, we have introduced a new type of promotion named “Shipping threshold discount” which the retailer can define the thresholds, which once met, will qualify the customers for discounted or free shipping. For e.g. spend $35 for free 'Two day ship' or Free 'Two day ship' for all loyalty customers. These discounts are only applied to the shipping charges applied to the orders. Since a retailer can set up multiple types of charges e.g. Handling, installation etc. so the retailer needs to specify which charge is considered shipping charges. This configuration is named “Shipping charge code” and is available on the Customer orders tab under “Retail parameters” form. These discounts, like the other periodic discounts, are applicable for the price groups associated to the discount. 
    Once the order lines with the specified delivery mode meets the defined threshold, then the shipping discount gets applied and reduces the shipping charge as per the discount set up. It is important to note that, unlike other discounts, the shipping discount does not create discount lines, rather directly edit the applied shipping charge.
