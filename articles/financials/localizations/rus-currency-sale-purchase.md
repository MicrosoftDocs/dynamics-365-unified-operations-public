...
- Register transactions for a sale, purchase, or transfer of currency that is done by using the **Money transfers in transit** ledger account.
- Automatically calculate the profit/loss amounts that are caused by the currency conversion for a sale or purchase of currency. (The calculation considers the currency exchange rate of the Central Bank and the exchange rate of the legal entity's bank on the date of the currency transfer.) Also automatically generate transactions for profit/loss from the currency conversion.
...
- [Set up a bank to use for currency conversion transactions](#set-up-a-bank-to-use-for-currency-conversion-transactions)
...
Use the **Currency revaluation accounts** page to set up the calculation of profit/loss amounts for currency conversion.
...
4. Define the following revenue and income codes for currency conversion: **Revenue code (currency conversion)** and **Expense code (currency conversion)**.

### Set up a bank to use for currency conversion transactions

Use the **Banks** page to associate a vendor account with the bank that should be used for currency sale or purchase transactions.
...
3. In the **Transaction extended type** field, select one of the following currency transfer types: **Currency sale**, **Currency purchase**, or **Currency transfer**.
...
Use the **Methods of payment** page to set up a method of payment for the sale, purchase, or transfer of currency.

1. Go to **Accounts payable \> Payment setup \> Methods of payment**.
2. Create a method of payment.
3. On the **General** FastTab, in the **Account type** field, select **Bank**.
4. In the **Payment account** field, select the bank account to transfer currency from for this method of payment.
5. In the **Bank transaction type** field, select the bank transaction type for the sale, purchase, or transfer of currency.
...
Use the **Bank accounts** page to set up specific information for the sale or purchase of currency.
...
9. In the **Method of payment** field, select the method of payment that you created earlier for the sale of currency.
10. On the **Bank** tab, in the **Bank transaction type** field, validate the value that is automatically filled in based on the method of payment, or select the transaction type for the currency sale transaction.
...
13. In the **Generate payments** dialog box, select the **Export format** option, and then, in the **Export format** field, select **Currency transfer order**. Alternatively, select the **Method of payment** option, and then, in the **Method of payment** field, select the method of payment for the currency sale.
...
16. In the **Bank currency transfer** dialog box, enter the required information.
    ...
    | Field              | Description |
    |--------------------|-------------|
    | Worker             | Select the worker who is responsible for the currency sale. |
    | Transit account    | Select the bank account where rubles should be received for the currency that is sold (the result of the currency conversion). The template that will be used to print a currency sales order is taken from this bank account. |
    | Currency           | Select the currency that should be received in the transit bank account after the foreign currency sale. |
...
### Create transactions for the bank commission and the currency that is received as a result of the foreign currency sale

After you post the payment order for a currency sale, you can register the bank commission and the currency that is received as a result of the currency conversion. (In this case, Russian rubles are received.)
...
3. Select **Lines**, create a journal line for the bank commission withdrawal, and enter the following information.
    ...
    |---------|-----------------------|-------------|
    | List    | Account               | Select the vendor account that you used for the currency sale transaction in the previous procedure. |
...
4. Create a journal line for the received currency, and enter the following information.

    | Tab     | Field                                              | Description |
    |---------|----------------------------------------------------|-------------|
    | List    | Account                                            | Select the vendor account that you used for the currency sale transaction in the previous procedure. |
    |         | Credit                                             | Enter the amount of currency that is received. |
    |         | Currency                                           | Select the accounting currency, which is the currency that is received as a result of the foreign currency sale. |
    |         | Offset account type                                | Select **Bank**. |
    |         | Offset account                                     | Select the bank account where the accounting currency is received. |
    | Bank    | Bank transaction type                              | Select the transaction type that has the **Currency sale** extended type. |
    | Payment | Currency (under **Currency conversion**)           | Select the currency that was sold. This currency must differ from the accounting currency. |
...
### Post the settlement of currency sale transactions

You post the settlement of currency sale transactions to generate profit/loss transactions from them and from the currency exchange difference transactions.

1. Go to **Cash and bank management \> Setup \> Bank groups**.
2. Select a bank, and then select **Functions \> Settle open transactions** to settle currency sale transactions.
...
5. Review the transactions that will be settled:
    ...
    - On the **Overview** tab, the **Transit account** and **Bank account - inflow** fields are specific to the currency sale:

        - For positive transactions, the value of the **Transit account** field equals the transit bank account that was selected for the currency sale transaction in the **Transit account** field in the **Bank currency transfer** dialog box.
    ...
    - On the **General** tab, the fields in the **Currency conversion** section are specific to the currency sale.
...
7. Select **Post**. The following transactions are generated:
    ...
    - Unrealized currency exchange difference (if, for the foreign currency that is sold, the currency exchange rate on the date of the currency sale differs from the currency exchange rate on the accounting currency receipt date)
    - Currency sale profit or loss (if, for the foreign currency that is sold, the bank exchange rate differs from the Central Bank exchange rate on the date of the accounting currency receipt transaction)
...
### Create a transaction of rubles movement for a currency purchase
...
3. Select **Lines**, create a journal line, and enter the following information.
    ...
    |         | Offset account type   | Select **Bank**. |
    |         | Offset account        | Select the bank account that rubles are moved from for the currency purchase. |
    |         | Method of payment     | Select the method of payment that you created earlier for the purchase of currency. |
    | Bank    | Bank transaction type | Validate the value that is automatically filled in based on the method of payment, or select the transaction type for the currency purchase transaction. |
    | Payment | Posting profile       | Verify that the value equals the profile that is used to post to the **Money transfers in transit** ledger account. |

4. Select **Generate payments**.
5. In the **Generate payments** dialog box, select the **Method of payment** option, and then, in the **Method of payment** field, select the method of payment for the currency purchase.
...
8. In the **Bank currency transfer** dialog box, enter the required information.
    ...
    | Field              | Description |
    |--------------------|-------------|
    | Worker             | Select the worker who is responsible for the currency purchase. |
    | Transit account    | Select the bank account where the foreign currency that is purchased should be received (the result of the currency conversion). |
    | Currency           | Select the currency that should be received in the transit bank account after the foreign currency purchase. |
...
9. Select **OK**.
    ...
    During generation of the currency purchase order, the following information is validated:

    - The currency of the transit bank account must differ from the accounting currency.
    - The currency of the transit bank account must equal the currency that is selected in the **Bank currency transfer** dialog box.
    - The currency of the payment journal line must be the accounting currency.

10. Select **Print \> Currency purchase order** to print the currency purchase order. (The template is taken from the transit bank account that is selected in the **Bank currency transfer** dialog box.)
11. Validate and post the journal.

### Create transactions for the bank commission and the currency that is received as a result of the foreign currency purchase

After you post the payment order for a currency purchase, you can register the bank commission and the foreign currency that is received as a result of the currency conversion.
...
3. Select **Lines**, create a journal line for the bank commission withdrawal, and enter the following information.
    ...
    | Tab     | Field                 | Description |
    |---------|-----------------------|-------------|
    | List    | Account               | Select the vendor account that you used for the currency purchase transaction in the previous procedure. |
    |         | Credit                | Enter the amount of the bank commission. |
    |         | Currency              | Select the currency of the bank commission. |
    |         | Offset account type   | Select **Ledger**. |
    |         | Offset account        | Select the ledger account for bank commission losses. |
...
4. Create a journal line for the received currency, and enter the following information.
    ...
    |         | Currency                                           | Select the code for the currency that is purchased. |
    |         | Offset account type                                | Select **Bank**. |
...
5. Post the journal.
    ...
    - The transaction currency must differ from the company's accounting currency.
...
### Post the settlement of currency purchase transactions

You post the settlement of currency purchase transactions to generate profit/loss transactions from them.

1. Go to **Cash and bank management \> Setup \> Bank groups**.
2. Select a bank, and then select **Functions \> Settle open transactions** to settle currency purchase transactions.
...
4. Select **Post**. The transaction for profit or loss from the currency purchase is generated (if, for the foreign currency that is purchased, the bank exchange rate differs from the Central Bank exchange rate on the date of the foreign currency receipt).
...
### Create a currency movement for a currency transfer
...
3. Select **Lines**, create a journal line, and enter the following information.
    ...
    |         | Debit                 | Enter the transaction amount for the currency transfer. |
    |         | Currency              | Enter the code for the currency that is transferred. |
    |         | Offset account type   | Select **Bank**. |
    |         | Offset account        | Select the bank account that the currency transfer is generated from. The bank of this bank account must be associated with the vendor account that you selected in the **Account** field. |
    |         | Method of payment     | Select the method of payment that you created earlier for the transfer of currency transfer. |
    | Bank    | Bank transaction type | Validate the value that is automatically filled in based on the method of payment, or select the transaction type for the currency transfer transaction. |
    | Payment | Posting profile       | Verify that the value equals the profile that is used to post to the **Money transfers in transit** ledger account. |

4. Validate and post the journal.

### Create transactions for the bank commission and the result of the currency transfer
...
3. Select **Lines**, create a journal line for the bank commission withdrawal, and enter the following information.
    ...
    | Tab     | Field                 | Description |
    |---------|-----------------------|-------------|
    | List    | Account               | Select the vendor account that is related to the bank account that is used for the currency transfer. |
    |         | Credit                | Enter the amount of the bank commission. |
    |         | Currency              | Select the currency of the bank commission. |
    |         | Offset account type   | Select **Ledger**. |
    |         | Offset account        | Select the ledger account for bank commission losses. |
...
4. Create a journal line for receiving the transferred currency, and enter the following information.

    | Tab     | Field                 | Description |
    |---------|-----------------------|-------------|
    | List    | Account               | Select the vendor account that is related to the bank account that is used for the currency transfer. |
    |         | Credit                | Enter the amount of the currency transfer. |
...
### Post the settlement of currency transfer transactions

You post the settlement of currency purchase transactions to generate currency exchange difference transactions.
...
4. Select **Post**. The exchange rate difference transaction is generated. Ledger accounts for the currency exchange difference are taken from the **Realized gain** and **Realized loss** accounts that are specified in the setup of currency parameters.
5. You can review the transactions that are generated. On the **Bank groups** page, select **Transactions**.
