...
Here's an example of a statistical dimension that you assigned to a Cost accounting ledger.
...
A statistical dimension member represents an entity that you want to register nonmonetary measures for. Use these measures as an allocation base or just to report nonmonetary values.

Create statistical dimension members manually. Alternatively, import them from a file by using the Data management import/export tool.

A statistical dimension member automatically becomes a predefined allocation base. Use it as an allocation base in policies or as input in other types of allocation bases.
...
Statistical measures can come from many kinds of sources. Dynamics 365 Finance is a great source for extracting statistical measures. Use a statistical measure provider template to easily configure the statistical measures you want to extract.
...
> [!NOTE]
> You can use all tables that contain financial dimensions as sources for statistical measures.

**Example: Count of employees per cost center**

The count of employees per cost center is a statistical measure that you can use for various purposes that provide managerial insight:
...
Here's an example of employees in the HcmEmployment table.
...
When you create a **Statistical measure provider template** record, decide which function to use:
...
For example, set up a statistical measure provider template as follows.
...
In this example, if you want a count of all full-time employees (FTEs), add a range in the **Worker type** field. In the **Criteria** field, select **Employee** to limit the output range as follows.
...
Before you can get statistical measures into Cost accounting, you must establish the relation between the statistical measure provider template and the statistical dimension member. Create this relation per Cost accounting ledger and version. The relation consists of a data connector and a data provider. You can have several data connectors and data providers per statistical dimension member.

> [!NOTE]
> In this example, you create a relation only for the **Actual version**.

Go to **Cost accounting ledger** > **Actual version** > **Manage** > **Statistical measures** to establish the relation. For this scenario, select the **Dynamics 365 Finance – Statistical measures** data connector, because you want to extract data from Finance.
...
After the source data for the statistical measure is processed, Cost accounting creates the following statistical entries.
...
If you assign the FTEs predefined dimension member allocation basis as an allocation base in a cost distribution rule, the cost distributes by using the following allocation factor.
...
A production cost center, CC010 (Packaging), is responsible for packaging the products before they're shipped to customers. Add the direct labor cost to the products through the bill of materials (BOM) and route. You must also allocate the indirect cost of running the cost center to the produced products. Often, the best statistical measure for such an allocation is the number of registered production hours per product within the given period.

The ProdRouteTrans table holds all production labor transactions per legal entity DataAreadID.

Here's an example of the ProdRouteTrans table.
...
The system recognizes that ProdRouteTrans is a table where each record belongs to a separate legal entity. Therefore, you're asked to select the legal entity that transactions should be imported from.
...
You can record the consumption of electricity in Microsoft Excel by using the predefined format of the data entity. Here's an example.

| Accounting date | Dimension member name1 | Dimension member name2 | Dimension member name5 | Magnitude  | Source identifier |
|----------|---------------|--------------|---------------|------------|----------|
| 31-01-2017      |       |          |            | 2,450.00   | Electricity       |
| 31-01-2017      |     |        |                        | 4,100.00   | Electricity       |
| 31-01-2017      | CC003                  |                        |                        | 15,000.00  | Electricity       |

When you've imported your data via Data management, the data is stored in a Cost accounting staging table. Therefore, the imported data can be used in multiple Cost accounting ledgers. A reload of data isn't required.
...
> [!NOTE]
> When you define the data provider configuration, you must specify which cost object dimensions to match against the imported transactions. After the system processes the source data for the statistical measure, it creates the following statistical entries in Cost accounting.
...
If you assign the Electricity predefined dimension member allocation basis as an allocation base in a cost distribution rule, the system distributes the cost by using the following allocation factor.
