...
Cost accounting lets you collect data from various sources, such as the general ledger, subledgers, budgets, and statistical information. You can then analyze, summarize, and evaluate cost data, so that management can make the best possible decisions for price updates, budgets, cost control, and so on. The source data that is used for cost analysis is treated independently in Cost accounting. Therefore, updates in Cost accounting don’t affect the source data. However, when you collect cost data from various sources, and especially when you import the main accounts from General ledger as cost elements, there's data redundancy, because the same data exists in both General ledger and Cost accounting. This redundancy is required, because you use financial management for external reporting and Cost accounting for internal reporting.
...
Cost entries result from a transfer via data connectors from general ledger entries, cost allocations, and posted cost entries in cost journals.
...
Use cost elements as a function to track and categorize costs. There are two types of cost elements: primary and secondary.
...
Secondary cost elements represent the internal cost flow because these costs are only used in Cost accounting. They're used in cost roll-up rules to aggregate costs into meaningful buckets used by overhead calculation.

## Cost classification

Cost classification groups cost according to their shared characteristics. For example, costs can be grouped by elements, traceability, and behavior.
...
- **Fixed cost** - A fixed cost is a cost that doesn't vary in the short term, regardless of changes in activity level. For example, a fixed cost can be a basic operating expense of a business, such as rent, that isn't affected even if the activity level increases or decreases.
...
Distribute cost from one cost object to one or more other cost objects by applying a relevant allocation base. Cost distribution and cost allocation differ in that cost distribution always occurs at the level of the primary cost element of the original cost and involves no reciprocal processing.

## Cost allocation

Is used to allocate the balance of a cost object to other cost objects by applying an allocation base. Finance supports the reciprocal allocation method. In the reciprocal allocation method, the mutual services that auxiliary cost objects exchange is fully recognized. The system automatically determines the order to perform the allocations in and iterate over it. The balance of a cost object is allocated by a single allocation base. Allocations across cost object dimensions and their respective members are supported. The allocation order is controlled by the cost control unit.

## Cost allocation policy

A cost allocation policy defines the amounts and quantities that you must allocate. Allocation rules include allocation source rules, which determine the costs that are allocated, and allocation target rules, which determine where the costs are allocated. For example, all costs for facility services are an allocation source that you can allocate to various departments in an organization (that is, to allocation targets).

## Cost roll-up

The purpose of cost roll-up rules is to aggregate costs into meaningful buckets. The level of aggregation is user-defined and involves the assignment of a secondary cost element. If cost roll-up isn't used, every element of a cost is allocated from one cost object to another

## Cost rate policy

Use the cost rate to calculate price per cost object. To understand the elements of the price, define cost rate policies. There are two types of cost rate: historical cost rate and planned cost rate. A historical cost rate is a calculated rate that is used as a multiplier for the allocation base of a cost object. The rate is calculated based on the cost allocations in the previous period. A planned rate is a user-defined rate.

## Dimension hierarchy

There are two dimension hierarchies: categorization hierarchy and classification hierarchy.
Use the dimension categorization hierarchy type for reporting purposes. It supports only the cost element dimensions.
Use the dimension classification hierarchy type to define policies and for reporting purposes. It supports all dimensions, such as cost objects, cost elements, and statistical dimensions.
...
Overhead costs refer to the ongoing expenses of operating a business. They're costs that you can't link directly to specific business activities. Here are some examples of overhead costs:
...
Define rates per cost object and cost element. Two types of rates exist: fiscal period and user-specified. The overhead calculation determines fiscal period rates. A user-specific rate is user-defined and can allocate cost between cost objects at a predetermined rate in the overhead calculation.

## Published

If you set this field to Yes, a user assigned one of the following roles can view the report in the Cost control workspace:
