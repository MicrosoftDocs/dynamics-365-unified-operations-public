...
Sometimes you won’t be able to settle transactions because another activity is currently processing the document.  This results in an error when you try to settle the transaction because that transaction is in use.  You can resolve this using the **Marked transaction detail** page to find transactions that are marked for settlement and identify any other processes that are accessing them.

Transactions are marked for settlement when vendor invoices are being paid or customers pay their open invoices.  Occasionally, these invoices may already be marked for settlement, so users can’t select the invoice for payment.  These invoices could be marked by another customer payment journal, sales order, vendor payment journal or purchase order in the current legal entity or in another legal entity.  

If this problem is encountered when entering a customer payment open the **Customer marked transaction details** page (**Accounts receivable > Periodic tasks > Customer marked transaction details**). You can enter any of the following selection parameters, **Customer account**, **Voucher**, **Date**, or **Invoice** to quickly identify where a transaction is blocked. If you don’t choose any selection parameters, the system will show all blocked documents from the current company, or any other company you select. Once the transaction has been identified it can be selected, and you can click the **Unmark selected transactions** function to remove the transaction that’s been blocked for settlement. This will remove the selected transaction from any journal that includes it, without removing the document from the other location. It will only remove the marking information from that journal.
