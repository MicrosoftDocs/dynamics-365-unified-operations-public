...
19. Close the page.
> [!NOTE]
> To be able to post a postdated check (PDC) to a bank account when session date is greater or equal maturity date, you must enable the feature **Maturity date validation of posting payment journal with postdated checks to bank account** which allows to post payment journals for vendors or customers with postdated checks, when session date is greater or equal to maturity date.
> 
> When setting **Method of payment** (Accounts payable > Payment setup > Methods of payment) do not fill in **Bridging account**. In this case the offset account is filled in with the bank account which set up in the **Method of payment**.
>  
> When the feature is enabled and session date is less then maturity date the following error message is displayed **Maturity date must be less or equal to the session date if offset account type is Bank.** when posting a payment journal. If the feature is not enabled, a user can post payment journal with PDC when session date is less then maturity date.    
