...
The **Vendor invoice automation** workspace shows information that's related to vendor invoice processing. It includes a **My work** view and an **Analytics - All companies** page. The **My work** view shows summary tiles, vendor transaction grids, and related vendor information. The **Analytics - All companies** page uses the capabilities of Power BI to show visualizations that are related to vendor invoices.

## Set up the workspace to show Power BI content

Complete this setup before data can be shown in Power BI visualizations in the **Vendor invoice automation** workspace.

1. In the **Feature management** workspace, filter the list to find the **Vendor invoice automation** feature.
1. Select **Enable now**.
1. To ensure that you can process invoices from beginning to end without requiring manual intervention, set up a vendor invoice workflow. To set up a workflow, go to **Accounts payable \> Setup \> Accounts payable workflows**.
1. Go to **Accounts payable \> Setup \> Accounts payable parameters**, and select the **Vendor invoice automation** tab. For more information, see [Setup options for vendor invoice automation](vnd-invoice-set-up-options.md).
1. Set the **Automatically submit imported invoices to workflow** option to **Yes**.
1. If product receipts should automatically be matched, set the **Automatically match product receipts to invoices lines** option to **Yes**.
1. Review the remaining optional settings, and configure them according to your organization's requirements.
1. Go to **System administration \> Setup \> System parameters**, and set the **System currency** and **System exchange rate** fields.
1. Go to **General ledger \> Setup \> Ledger**, and set the **Accounting currency** and **Exchange rate type** fields.
1. Go to **General ledger \> Currencies \> Currency exchange rates**, and enter the exchange rates between the transaction currency and the accounting currency, and between the accounting currency and the system currency.
1. Go to **System administration \> Setup \> Entity store**, and look for **Vendor invoice automation measure**. Select **Refresh**.

To view the information that displays in the workspace, you must have the Accounts payable manager or Accounts payable clerk security role.

## My work view

### Company selection

When you turn on the **Vendor invoice automation** feature, a **Company** field appears at the top of the workspace. The selection in the **Company** field affects all the information displayed in the workspace. By default, the view shows information for the company that you signed in to. By selecting a different company in the **Company** field, you can show information for that company on the workspace. You can then select a tile in the workspace to go the related page in the selected company.
...
When the feature is turned on, you also see three tiles grouped together on the workspace in a section called **Journals**. The tiles are titled **Journals**, **Journals - Assigned to me**, and **Invoice pool**.
...
The record you create is based on the company filter, not the company that you're signed in to. For example, you're signed in to the **UMSF** company, but the company filter is set to **GBSI**. In this case, when you select **New** and then select a record type in the list, you create the record in the GBSI company.
...
The **Open purchase orders** grid shows all purchase orders that aren't yet fully invoiced. Use the **Invoice now** button to create a vendor invoice for a purchase order. Use the **Prepayment invoice now** button to create a prepayment invoice for a purchase order.

The **Product receipts** grid shows purchase receipt transactions that aren't yet fully invoiced. Use the **Invoice now** button to create a vendor invoice for a purchase order.

The **Pending vendor invoices** grid shows all vendor invoices that haven't been submitted to the workflow system. Use the **Search** field and the company filter to search for a specific vendor invoice. Use the **Edit** button to edit a transaction that appears in the grid.

On the **Find purchase order** grid, use the **Search** field to search for a specific purchase order.
...
When you set the **Automatically submit imported invoices to workflow** option to **Yes** on the **Accounts payable parameters** page, you can view automation analytics. The **Analytics - All companies** page provides important metrics, such as vendor invoices that are in approval by approver and by company. This page contains five report pages. One page provides an overview, and the other pages provide details about Accounts payable automation metrics.
