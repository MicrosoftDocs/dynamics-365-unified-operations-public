...
description: Learn about centralized payments for Accounts payable, including examples that show how posting for centralized payments is handled in various scenarios.
author: twheeloc
ms.author: shpandey
ms.topic: article
ms.date: 08/20/2026
...
Organizations that include multiple legal entities can create and manage payments by using a single legal entity that handles all payments. Therefore, you don't have to enter the same payments in multiple legal entities. This article provides examples that show how posting for centralized payments is handled in various scenarios.

In a centralized payments organization, there are many legal entities for operations, and each operating legal entity manages its own vendor invoices. You generate payments for all the operating legal entities from a single legal entity, which is known as the legal entity of the payment. During the settlement process, the applicable due-to and due-from transactions are generated. You can specify which legal entity in the organization receives the realized gain or realized loss transactions, and how cash discount transactions that are related to a cross-company payment are handled. On the centralized payment journal line, set the **Account type** to Vendor. Set the **Offset account type** to Bank or Ledger. The bank account should be in the current company.
...
- The legal entities are Fabrikam, Fabrikam East, and Fabrikam West. You make payments from Fabrikam.
- Set the **Post cash discount** field on the **Intercompany accounting** page to **Legal entity of the invoice**.
- Set the **Post currency exchange gain or loss** field on the **Intercompany accounting** page to **Legal entity of the payment**.
- Set up the vendor Fourth Coffee as a vendor in each legal entity. You identify the vendors from the various legal entities as the same vendor because they share the same global address book ID.
...
Fabrikam East has an open invoice for vendor account 100, Fourth Coffee. Fabrikam enters and posts a payment to Fabrikam vendor account 3004, Fourth Coffee. The payment settles the open invoice.
