...
##	Set up a channel with shared mailbox
A shared mailbox can be used by a group of people, like a support team, to receive and send email from the same email address. Select a shared mailbox to add or remove members, set up automatic replies, manage aliases, and more. In invoice capture, follow these steps to create a channel which uses the shared mailbox to receive invoice emails from the supplier:

1.	Click **+ New** to create a channel. 
2.	Enter a **Channel name**, set **Use manage channel** to **No**.
3.	Save and note the **Channel id** from the browser url.
4.	In Power Apps, select **Flow**.
5.	Click **+ New flow**, select **Automated cloud flow** from the dropdown list.
6.	Enter a name for the flow. Select the **When a new email arrives in a shared mailbox** trigger. 
7.	Click **Create** to create the power automated flow.
8.	Provide the shared mailbox address in the **Original mailbox address**. Set the **Only with attachments** and **Include attachments** to **Yes**. 
9.	Enter additional criteria to address business requirements.
10.	Click **+** to insert a new step. Choose **Add an action**, select **Microsoft Dataverse** and **Perform an unbound** actions.
11.	Click **Rename** to rename the step.
...
15.	Select **Attachment content** for **FileContent**. Select **...** > **Peek code**. Copy **Item/FileContent** which looks like "@items('xxx')?['contentBytes']" and wrap with string function like "string(items('xxx')?['contentBytes'])" and paste the final value to **Attachment content** field.
