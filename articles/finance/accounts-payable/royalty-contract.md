...
A royalty contract records an agreement with an asset or intellectual property owner. It specifies the negotiated terms and conditions under which the licensor qualifies for a monetary reward when the licensee uses its property to obtain revenue.

Register royalty contracts on the **Royalty contracts** page. To open the **Royalty contracts** page, select **Accounts payable > Broker and royalties > Royalty agreements**.

:::image type="content" source="./media/royalty-contract-management-royalty-agreements-page.png" alt-text="Screenshot of the Royalty agreements page.":::

The **Selection** tab in the lower part of the page shows the products that qualify for a royalty fee.

On the **General** tab in the upper part of the page, several fields provide more details about the agreement's conditions as they're negotiated with the licensor:

- The **Cumulate sales by** field specifies the period that a royalty amount is calculated for, based on cumulative sales. For example, the period might be a month. Alternatively, to calculate the royalty amount every time that a sales order line is invoiced, select **Invoice**.
- If you set the **Approval required** option to **Yes**, a royalty program owner must approve the claims before a royalty can be turned into an invoice that is payable to the licensor.
- The **Accrual account** and **Expense account** fields must specify account numbers that receive accrued amounts during the intermediate stage between approval and processing.

Set up royalty rates on the **Royalty amounts** tab in the lower part of the page. To set up the rates as tiers, add a line for each tier, and set the **From value** and **To value** fields.

> [!IMPORTANT]
> To make an agreement valid, select **Validation** on the Action Pane. The **Validated** option on the **General** tab in the upper part of the page is set to **Yes**.
...
:::image type="content" source="./media/royalty-contract-management-price-details.png" alt-text="Screenshot of the Royalty FastTab on the Price details page.":::
...
Royalty claims that you generate represent future payments to the licensor. The contract owner cumulates the claims for the relevant period and then creates an interim liability for the licensor by approving those claims.
...
On the **Royalty claims** page, the **Starting royalty amount** field specifies the fee amount that, after it's approved and processed, is paid to the vendor as a royalty.
...
When you generate a claim on a time basis, its status is set to **To be calculated**. This status is used because the royalty is granted on that time basis. The claim isn't included, together with other claims, in the cumulative calculation until the end of the period.

If there are multiple sales orders for the same vendor, you must recalculate the claims so that any cumulative effect is considered. On the Action Pane, select **Cumulate**.

When you run the **Cumulate** action, the system posts an accrual journal for the claim amounts. To view the details of the posting, find the claim in the list of royalty claims, and then, on the Action Pane, select **Royalty transactions** to see and access the accrual journal.
...
The following events occur, and the claim's status changes to **Completed**:
...
> [!NOTE]
> You specify the account number for royalty fees for the procurement category that you use on the purchase invoice line for the royalty. Set that procurement category, in turn, on the **Broker and royalty** tab of the **Accounts payable parameters** page.
