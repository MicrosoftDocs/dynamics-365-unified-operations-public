...
- Parameters for processing automation background tasks:
  ...
  - Validating invoice matching for manual invoices that were automatically matched to product receipt lines. Different business processes use this framework to define how often the selected process runs. The available frequencies for the **Match product receipt to invoice lines** and **Submit vendor invoices to workflow** background processes include **Hour** and **Daily**.
...
To achieve touchless automation from the import process through vendor invoice posting, set up a Vendor invoice workflow. To set up a workflow, go to **Accounts payable** > **Setup** > **Accounts payable workflows**. To ensure that the invoice can be processed from start to finish without manual intervention, include an automated posting task in your workflow configuration.

## Parameters for automatically applying prepayments in imported invoices

- **Automatically apply prepayment for imported invoices** – When you set this option to **Yes**, the system automatically looks up existing prepayments for a corresponding purchase order when you import vendor invoices. If it finds prepayments that it can apply, it adds one extra line to apply the prepayments in the vendor invoices that you're importing.
...
- **Automatically submit imported invoices to workflow**:
  ...
  - If you set this option to **No**, you must manually submit invoices to the workflow system.

- **Check match product receipt status before workflow submission**:

  - If you set this option to **Yes**, the system can't automatically submit the imported invoice to the workflow system until the matched product receipt quantity equals the invoice quantity.
  - If you set this option to **No**, the system submits the invoice to the workflow system after the number of attempts is reached, even if the product receipt quantity doesn't equal the invoice quantity.

    The **Check match product receipt status before workflow submission** option is available only if the **Enable invoice matching validation** option is selected. When you select this option, the **Automatically match product receipts to invoice lines** option is automatically set to **Yes**.

- **Automatically match product receipts to invoice lines** – If you set this option to **Yes**, background processing can be used to automatically match posted product receipts to invoice lines that a three-way matching policy is defined for. That process runs either until the matched product receipt quantity equals the invoice quantity, or until the value of the **Number of times to attempt automatic matching** field is reached. The process can run until the invoice is submitted to the workflow system.

    The **Automatically match product receipts to invoice lines** option is available only if the **Enable invoice matching validation** option is selected. If you set the **Match product receipts to invoice lines prior to automatically matching** option to **Yes**, you can't set this option to **No**. To set this option to **No**, you must first set the **Match product receipts to invoice lines prior to automatically matching** option to **No**.
...
- For manually created invoices, the **Include in automated processing** option is disabled by default. If you enable the **Automatically match product receipts to invoice lines** option, the **Automated receipt match status** field is initially set to **Not applicable**. You can manually enable this option to include an invoice in the vendor invoice automation process after invoice entry is completed. The value of the **Automated receipt match status** field changes to **Not yet run**. The **Include manually created invoices in the invoice automation** feature controls this functionality.
...
- If the **Automatically match product receipts to invoice lines** option is enabled, the invoice is excluded from automation under these conditions:
    ...
    1. The **Automatically submit imported invoices to workflow** option is enabled.
    1. The invoice isn't approved after it's successfully submitted to the workflow system.
