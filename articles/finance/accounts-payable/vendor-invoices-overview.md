...
When an invoice is in workflow, the approver can update Invoice quantities so that they match the value in the Product receipt quantity to match field. To do so, on the **Feature management** page, turn on the **Update the invoice quantities to match product receipt quantities in workflow** parameter.  If an approver in the workflow process has removed all the matches from all the product receipts from the invoice line, the invoice line will be deleted. When the **Feature management** parameter is not enabled, invoice quantities are not updated for invoices in workflow.
