...
To streamline the process, you can automatically derive invoice lines from the purchase order or product receipt. To enable automatic creation of invoice lines, follow these steps:

## Enable the settings

1. Go to **Accounts payable \> Setup \> Accounts payable parameters**.
1. On the **Vendor invoice automation** tab, under **Automatic line creation for imported invoices**, set the **Automatically create invoice lines** option to **Yes**.
1. In the **Choose default quantity for automatic invoice lines creation** field, select the quantity that should be used to automatically generate invoice lines:
...
1. Set the **Allow invoice to be imported with no derived lines** parameter. This parameter determines whether invoice import is blocked if there are no derived invoice lines.
...
- **HeaderOnlyImport** – Set this field to **Yes** to generate lines for invoice headers.
...
> [!NOTE]
> If you enter multiple product receipt numbers, several pending vendor invoices that have the same invoice number are created. You must manually consolidate these invoices before you can process the invoice further. In future releases, the invoices are automatically consolidated.
...
- **PurchIdRange** – The **Purchase order number** that sets the prepayment amount.
- **VENDORINVOICETYPE** – Set to **"VendorAdvance"** to indicate a prepayment invoice.
- **IMPORTEDAMOUNT** – The **Amount** on the prepayment invoice.

The import of a prepayment invoice isn't accepted when the prepayment invoice is already generated without a settlement under the purchase order.
...
- **Currency code derivation and validation** – The system automatically derives the currency code from the purchase order if you don't provide a currency code in the vendor invoice header entity. The system raises a validation error when the imported invoice has a different currency code from the one specified on the purchase order.
