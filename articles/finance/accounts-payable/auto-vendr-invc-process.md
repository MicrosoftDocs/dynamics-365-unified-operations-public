...
> [!NOTE]
>In versions 10.0.45/10.0.46, the override actual tax with imported tax functionality is controlled by a feature in feature management and not a toggle in AP parameters.  To enable this functionality, navigate to feature management and enable the following features:  Apply the imported sales tax for invoice register and invoice journal, Apply the imported sales tax for pending vendor invoice.  In releases 10.0.47 and later, the feature is available by default, and users must configure the feature in AP parameters as listed above.  However, if the feature was previously enabled in versions 10.0.45/10.046,  these toggles will be automatically enabled.  
