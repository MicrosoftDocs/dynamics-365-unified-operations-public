...
## Vendor invoice automation – Tracking the Invoice received date value

The **Invoice received date** value indicates the date when the company received the invoice from the vendor. It provides a starting point for tracking the invoice's progress through the automation processes. The **Invoice received date** value can be included in the imported data for a vendor invoice. For invoices that were manually created, you can specify the date. If no **Invoice received date** value is entered, the current date is used by default.

## Vendor invoice automation – Tracking the Imported invoice amount and Imported sales tax amount values
