...
If no accounts are defined in the currency revaluation posting profile the currency revaluation adjustment will be posted to the defined accounts in the currency revaluation accounts. If these accounts are not defined, it will post to the defined accounts on ledger page. |
...
|Tax calculation|	Tax calculation data model|	The following fields are extended for the latest tax calculation data model: - Is Prepayment Journal Voucher - Direct Delivery - Intercompany Direct Delivery These header fields are enabled in the **List code applicability** table.|
