...
| General ledger | Add a date filter when viewing unsettled ledger transactions | This functionality allows you to enter a date when you are only viewing unsettled transactions on the **Transactions for <main account>** page.  When you enable this feature, there is a button to show only unsettled transactions. If you select this button, you can define a date on which to filter those transactions. If you open the **Transactions for <main account>** page from the **Trial balance** report, the date defaults with the last day of the trial balance. To remove the date filter, select the button again and remove the date. | Feature management | 
