...
|Tax | Tax exempt number validation logic update for the Create customer dialog |When the feature is enabled, tax exempt number validation will not occur on the 'Create customer' dialog. This means you no longer need to enter a value in the Tax exempt number field during customer creation. Instead, after customer account is created, you can add a tax registration number in the Registration IDs section and select it on the customer account page. | Feature management |
