...
The following table lists the features that are turned on by default in 10.0.21. These features can be turned off manually in [Feature management](../../fin-ops-core/fin-ops/get-started/feature-management/feature-management-overview.md).

| Feature area | Feature&nbsp;name&nbsp;in feature&nbsp;management            | More information                                             |
| ------------ | ------------------------------------------------------------ | ------------------------------------------------------------ |
| Tax          | (Saudi Arabia) Enable tax calculation for full project invoice amount | This feature enables tax calculation based on the full project invoice amount before deducting the retention amount on the project invoice. There is no tax calculated on the retention project invoice.   |
| Tax          | Enable multi-batch processing for the GSTR report                | The feature enables multi-batch processing for the GSTR report and improves the performance related to the report generation. |
| Tax          | Generation of “GST transaction ID” at export invoice posting | This feature enables the allotment of the GST transaction ID when the invoice is posted for export transactions through a sales order. |
| Tax          | (India) Enable changing tax rate type in purchase invoice   | In India, when a vendor invoice is received from the supplier, the tax rate is often different from the tax rate in the purchase order. This feature provides the flexibility to change to the tax rate type on the vendor invoice. |
| Tax          | Enable rounding rules for withholding tax                    | This feature enables the rounding rules that are set for the witholding tax code under **Tax** > **Indirect taxes** > **Withholding tax** > **Withholding tax codes** in the **Rounding form** field. When this feature is enabled, the withholding tax amount is rounded following the setup of: **Normal**, **Downward**, and **Rounding-up**, provided the transaction currency and tax currency are the same. |
| Tax          | Sales tax conversion                                         | This feature enables dual currency support for the tax domain. After you enable this feature, you can specify the routing of sales tax amount conversions under different currencies and enable auto-balancing for sales tax payments in the reporting currency. |
| Tax          | Enable inquiry form for posted sales tax                     | This feature enables the **Inquiry** page for posted sales tax which improves inquiry performance and search transactions. |
| Tax          | Calculate origin amount for sales tax specification by ledger transaction report | This feature enables calculating the origin amount in the **Sales tax specification by ledger transaction** report when it's grouped by sales tax code and the **Total** parameter is **Yes**. |
| Tax          | (India) Enable overriding TCS/TDS group on purchase invoice          | This feature allows you to override the TCS/TDS group value on a vendor invoice that's created from a purchase order. |
| Tax          | (India) GTE calculation validation                           | This feature enables the validation for GTE calculation to prevent empty tax components or zero tax amount. |
| Tax          | Non deductible % field in Posted sales tax                   | When this feature is enabled, the **Non deductible %** field is populated in sales tax transactions from the corresponding sales tax codes. The field is located on the **Posted sales tax** page. |
| Tax          | (India) Enable withholding setup for currency validation to prevent the withholding payment from failing due to currency variations | This feature ensures that the currency is setup for withholding tax codes and withholding tax authority in the accounting currency. All withholding tax transactions must be posted and settled with tax the authority in the accounting currency only. |
...
| Tax          | (India) Enable credit/debit note against an export invoice      | This feature lets you issue a credit or debit note against a posted export invoice. The credit or debit note can be issued by using the **General journal**, a **Free text invoice**, and an **Export sales order**. The separate number sequence for exporting a credit or debit note must be set up on the **GST Reference number Sequence Group** page. The shipping bill number and date can be recorded in the credit or debit note for reference. |
| Tax          | Sales tax rate on invoice date in vendor invoice journals    | Enable sales tax rate determination on the invoice date in the **Vendor invoice** journal, **Invoice register**, and **Invoice approval** journals when the **Calculation date type** field is set to **Invoice date** in the General ledger parameters. |
...
| Tax          | Sales tax rate on the invoice date in a purchase order credit note | Enable the sales tax rate determination based on the original invoice date for a purchase order credit note when the **Calculation date type** is set to **Document date** in the General ledger parameters. |
| Tax          | Conditional tax settlement for Ledger accruals               | This feature allows you to post a conditional tax settlement correctly with its payment tax code when Ledger accruals are used. |
| Tax          | Enable creating tax component with pre-defined rules         | This feature allows you to create a tax component with pre-defined rules to support VAT behaviors such as non-deductible and reverse charge for purchases and sales. |
