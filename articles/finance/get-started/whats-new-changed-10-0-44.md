...
| Fixed assets |Enable scheduled start date/time on recurrence of proposals|Add a “Scheduled start date/time and time zone” fields to the Define recurrence batch—visible when enabled. So that the system automatically schedules the batch via scheduler logic, logs the event in Batch History, shows a confirmation, and lets users modify or clear the schedule up to five minutes before execution (reverting to manual start if cleared).| |
