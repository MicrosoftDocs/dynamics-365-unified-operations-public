...
| General ledger | General ledger snapshots | A new button for Trial balance snapshots is added to the **Trial balance** list page. A trial balance snapshot calculates the balances of the selected financial dimensions. These balances can be exported to Excel by using an Open Data Protocol (OData) entity. Each trial balance snapshot has an external tracking ID used to make the snapshot available to external systems for reporting purposes. | On by default |
