...
|----|----|----|
| Cash management | Bank to ledger reconciliation report improvement | During bank reconciliation, if a bank transaction has a correction, there will be two bank transaction lines on the **Bank to ledger reconcilation** report. One line is the orginal bank transaction and the other line is the correction. The two lines from the bank transaction will now match the two lines from the general ledger transaction. | 
