...
| Globalization | (ER) Keep rows of a single section together on the same Excel page | With this feature, you can tune an ER format with an Excel template to improve the look and readability of generated reports by keeping rows of a single section together on the same page. For more information, see [Row handling](../../fin-ops-core/dev-itpro/analytics/er-fillable-excel.md#row-handling). | Parameter |
