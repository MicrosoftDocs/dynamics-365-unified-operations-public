...
| General Ledger | Navigate to the original document from Documents pending accounting | This change allows simple navigation back to the original document that is found in the Documents pending accounting page | Default |
| General Ledger | Default dimension sharing enabled for Master company sharing  | This change allows default dimension data to be included in the master company sharing functionality. This is enabled only for dimensions that are defined as global values. | Default |
