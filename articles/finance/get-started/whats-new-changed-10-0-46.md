...
| Cash and bank management | Display payee name for customer/vendor payment information | This feature enables the retrieval and display of the payee name defined on the customer and vendor bank account details. The payee name is visible in customer and vendor payment journals, payment proposals, and outbound payment files. The payee name can be different from the name of the customer or vendor defined on the master data, and should align with banking requirements, ensuring accurate identification of payment recipients. | Feature management |
...
|Subscription billing| Reduce chattiness in SubBillDeferralRecognitionProcessing and SubBillDeferralRecognitionProcessingBatch| This change reduces a significant amount of chattiness in the batch process. This change is controlled by SubBillDeferralRecogProcChattinessReductionFlight |On by default|
|Subscription billing  | Fix generate invoice with sales order performance problems| This change introduces a posting sales order batch job for billing schedule invoicing batch processing for the sales order posting process. This change enables multithreaded posting resulting in parallelism and faster execution time. This change is controlled by SubBillGenerateInvoiceMultiThreadedPostingFlight| On by default|
|Subscription billing |[Performance improvements for deferral recognition batch (preview)](../accounts-receivable/perf-def-recognition.md) | This feature enhances the subscription billing deferral recognition batch process by processing deferral data in parallel, allowing each run to complete more efficiently. Key performance improvements are introduced in Subscription Billing deferral processing. These enhancements address critical business challenges observed in high-volume deferral scenarios and improve overall reliability and scalability of the batch process.| Feature management|
...
| Module or feature area | Feature name | More information | Enabled by |
|---|---|---|---|
| Cash and bank management | Bank account lifecycle management | As of Dynamics 365 Finance version 10.0.46, this feature isn't in preview. The bank account lifecycle management feature enables approval workflow for bank account opening, modification, and closing. It adds extra signer information in the bank account data model and provides bank account change history for auditing purposes. | Feature management |
| Cash and bank management | Modern bank reconciliation | When you enable the feature, sales tax direction is considered when the general ledger voucher is generated during the bank reconciliation process. | Feature management |
 |Credit and collections |Customer interest note creation – top picking optimization |This feature uses top picking to improve performance when creating customer interest notes. This improvement helps avoid long-running transactions and ensures faster and more efficient processing when handling large transaction volumes. | Feature management |
| General ledger | Account reconciliation agent (Production ready preview) | The Account reconciliation agent functionality is improved to provide suggested actions for **In ledger not in subledger** and **In subledger not in ledger** exception types.| Feature management |
| General ledger |User ID added to the **Ledger settlement inquiry** page for full visibility. |The user ID that did the ledger settlement now displays in the **Ledger settlement inquiry** page. | Default |
| General ledger |Performance improvements in Ledger settlements when the **Enable advanced awareness option** feature isn't enabled. | The **Ledger settlements** page is updated to improve performance when doing ledger settlements. You might need to update customizations to work with the new page. | Default |
 |Subscription billing  | Billing schedule  |The discount group setup in the trade agreement for item prices automatically applies to the billing schedule when you create it for the corresponding customer record.  | Feature management |
 |Subscription billing  |Subscription billing deferral COGS adjustment-enhancement (preview) | A dedicated dashboard monitors and provides visibility into errors that occurred during the adjustment process. This dashboard offers actionable insights and a user-friendly interface, so users can conveniently reprocess failed adjustments through a rerun batch job—reducing manual effort and enhancing the overall reliability of the deferral.  | Feature management |
...
Services and view the planned regulatory updates by using the issue search tool. Issue search lets you search by country or region, type of feature, and release.
