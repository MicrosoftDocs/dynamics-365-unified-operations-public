...
| General ledger | Update balances button added to dimension set balance calculation in Trial balance |In the Trial balance, the **Update balances** button is now available when the **Performance enhancement for general ledger dimension set balance calculation** feature is enabled in Feature management. You can manually start a batch job to update the dimension set balances instead of waiting for the automatic process to update the balances. For more information, see [General ledger account balances](/general-ledger/general-ledger-account-balances). | Feature management |
