...
|General ledger|	Post foreign currency realized gains/losses for ledger settlements|	In version 10.0.36, this feature released with the ability to post foreign currency realized gains and realized losses for ledger settlements when the reporting currency values of the debits and credits differ. In 10.0.38, new capabilities are being added which will post foreign currency realized gains and realized losses for ledger settlements when the accounting currency values of the debits and credits differ. This feature also changes the determination of whether a ledger settlement is fully settled. It determines whether ledger settlement is fully settled based on the transaction currency. The exception to this behavior is if ledger settlement occurs where the transaction currencies differ for the marked transactions. Then, the logic will use the accounting currency values to determine whether the marked transactions are balanced. This feature does not support partial settlement.|	Feature management |
| Tax calculation | [Integration with project transactions](/dynamics365/release-plan/2023wave2/finance-supply-chain/dynamics365-finance/tax-calculation-service-integration-project-operations) | Tax calculation integration is now extended with the following transactions of Project management and accounting in the Finance and Operations: Project invoice proposal; Journals (Hour/Expense/Item/Fee); Project quotations; Project Operations integration journal | Parameter |
