...
> [!NOTE]
> When you use the **Transaction currency** attribute, if you don't specify a currency filter, the system includes transactions in all the currencies that you have transactions for. We recommend that you specify a currency filter to prevent the row from showing an aggregate amount that isn't meaningful when the report is generated.
...
- You can set the exchange rate type on the **Main accounts** page within General ledger; there's an option for **Exchange rate type** on the **Financial reporting** FastTab. 
