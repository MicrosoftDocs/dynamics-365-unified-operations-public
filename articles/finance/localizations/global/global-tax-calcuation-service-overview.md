...
Import and set up your Tax Calculation configuration by using the version that matches your Microsoft Dynamics 365 Finance or Microsoft Dynamics 365 Supply Chain Management version.
...
1. In the **Globalization Studio** workspace in Finance, view and import taxable document model configurations and model mapping configurations. If you need to extend configurations for an advanced scenario, see [Add data fields in tax configurations](tax-service-add-data-fields-tax-configurations.md).
1. In the **Globalization Studio** workspace in Finance, create or maintain tax features. Use tax features to maintain tax rates and tax applicability rules.
1. After you finalize the tax feature setup, change the **Feature version status** to _Completed_.
1. In **Tax calculation parameters**, select which tax feature setup version to use for a specific legal entity.
1. In Finance and Supply Chain Management, operate transactions as usual. When Tax Calculation is needed, the client collects information from the transaction, such as the sales order or purchase order, and packages the information as payload. The client then sends a request to calculate the tax.
1. The tax calculation service receives the tax calculation request from the client and completes the calculation. It returns the tax result to the client.
1. The Dynamics 365 client receives the tax result and presents the tax calculation result on a sales tax page.

## Supported transactions

You can enable tax calculation by transaction.

The following table lists the transactions supported in each version.
...
> [!NOTE]
> For any country or region that Microsoft doesn't localize, you can enable and run Tax Calculation by using other global features.
