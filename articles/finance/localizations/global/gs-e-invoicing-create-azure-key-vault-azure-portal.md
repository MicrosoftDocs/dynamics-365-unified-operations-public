...
1. Create a key vault in the tenant that you use for the Dynamics 365 Finance environment. For more information, see [Create a key vault using the Azure portal](/azure/key-vault/general/quick-create-portal).

    Next, set up the access policy to grant the Electronic Invoicing service the correct level of secure access to the secret that you created.
...
1. In the **Secret permissions** field, select the **Get** and **List** operations.
    ...
    :::image type="content" source="../media/add-access-policy-page.png" alt-text="Screenshot of the Add access policy page showing secret permissions set for the Get and List operations.":::
...
1. On the **Overview** page, copy the value of the Domain Name System (DNS) name for the key vault. Use this value during the setup of Electronic invoicing parameters in Finance and refer to it as the **Key Vault URI** value. For more information about how to set up Electronic invoicing parameters, see [Configure Electronic invoicing parameters](gs-e-invoicing-set-up-parameters.md).
