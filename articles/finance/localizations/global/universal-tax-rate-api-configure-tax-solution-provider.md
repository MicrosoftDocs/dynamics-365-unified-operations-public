...
1. If the feature setup defines new tax codes, the **Maintain mandatory fields for new tax codes** page appears. For each tax code, select a settlement period and a ledger posting group. To bulk update the mandatory fields, select all tax codes and then select **Bulk update**.
1. Select **Confirm**. A log file is downloaded in your browser. This file contains the information for the imported and updated tax codes, tax groups, and item tax groups for the current legal entity.
1. In the **Business process** field, select the processes that access the external tax service for tax calculation.

    > [!NOTE]
    > To learn about the supported business processes that you should select, contact your tax solution provider.

1. Select **Enable address validation**, and select the country/region keys that your tax rate provider supports.

    > [!NOTE]
    > To ensure an accurate location-based tax calculation, your tax rate provider validates and corrects the addresses in the master data. After you enable address validation, a **Validate** button is available when you add or maintain the addresses in the master data.
    > 
