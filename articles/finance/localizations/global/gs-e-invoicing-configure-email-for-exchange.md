...
1. For Internet Message Access Protocol (IMAP) access, select the **IMAP.AccessAsApp** permission.
    ...
    :::image type="content" source="../media/e-invoicing-configure-email-for-exchange2.png" alt-text="Screenshot of the IMAP.AccessAsApp permissions being added for the app registration.":::

1. Select **Add permissions**.
1. To access Exchange mailboxes through IMAP, your Azure Active Directory (Azure AD) application must get tenant admin consent for each tenant. For more information, see [Introduction to permissions and consent](/azure/active-directory/develop/v2-permissions-and-consent).
    ...
    :::image type="content" source="../media/e-invoicing-configure-email-for-exchange1.png" alt-text="Screenshot of admin consent being granted for the selected permissions.":::

1. Select **Overview**.
1. Copy the **Application (client) ID** and **Directory (tenant) ID** values.
1. Select **Certificates & secrets**.
1. On the **Client secrets** tab, select **New client secret**, and notice the value in the **Expiration** field. When a secret expires, it can't be used anymore, and you must create a new secret.
1. After you create a secret, copy the **Value** and save it in the Azure Key Vault that's associated with the Electronic Invoicing Service service environment.
...
1. Save the **Application (client) ID** as a secret in the same key vault, and add a link to this secret in the Electronic Invoicing Service service environment in RCS.
...
After a tenant admin consents to your Azure AD application, they must register the application's service principal in Exchange by running a series of PowerShell snippets through Exchange Online PowerShell. Some of these snippets use values that you copied in the previous procedure:
...
1. Install the **ExchangeOnlineManagement** module.
    ...
    > [!NOTE]
    > You only need to install the **ExchangeOnlineManagement** module once on the Exchange administrator's machine.
...
1. On the **Data channel** tab, in the **Parameters** section, set the following required fields.
    ...
    | Field              | Description |
    |--------------------|-------------|
    | Data channel       | Enter a unique name to identify the data channel. The name can have a maximum of 10 characters. It's referenced in applicability rules and in the Dynamics 365 Finance application during the communication process. |
    ...
    | User name secret   | Enter the name of the Key Vault secret that contains the ID of the email user account. You must create this secret in Key Vault and set it up in your service environment. |
    ...
    | Archive folder     | Specify the folder where processed emails should be stored. If you don't set a value, the service automatically creates a folder. |
    | Error folder       | Specify the folder that emails should be moved to if the processing fails. If you don't set a value, the service automatically creates a folder. |
    ...
    | Subject filter     | Enter a string to filter by subject. Only emails where the subject matches the filter are processed. This field is optional. A simple mask such as **\*smth\*.ext** is supported. Here, each asterisk (\*) represents zero or more occurrences of any character. |
    ...
    | Processing mode    | <p>Select one of the following options to specify whether you can process all the attachments in an email together, or whether you should process each attachment separately:</p><ul><li><b>By attachment</b> – A new electronic document is created for each attachment in the email. For example, if one email includes several files that contain e-invoice data, each file is considered a new e-invoice.</li><li><b>By email</b> – One attachment is considered a base attachment, and one electronic invoice is created. Other attachments can be used as supporting files.</li></ul> |

1. In the **Attachments filter** section, add the file filtering information. Only attachments that satisfy the defined filter are processed. For example, **\*.xml** filters for attachments that have the .xml file name extension. You use the name of the attachment in Dynamics 365 Finance or Dynamics 365 Supply Chain Management during setup.
