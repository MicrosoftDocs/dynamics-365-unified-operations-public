...
Electronic invoicing feature versions follow a lifecycle that has up to two statuses:
...
Specify an **Effective from** date for any new version of an Electronic invoicing feature. By using this date, you can define a default version that the deployment uses or a version that it overwrites.
...
Use the **Deploy** command to deploy a version of an Electronic invoicing feature to the Microsoft Dynamics 365 Finance or Dynamics 365 Supply Chain Management environment.
...
Select **Cancel deployment** to remove a specific version of an Electronic invoicing feature from the service, if you deployed it there. The **Deployed to the service** column indicates whether a version is deployed to the service.

## Rebase Electronic invoicing features

When one Electronic invoicing feature is derived from another feature, select **Rebase** to update the derived feature with the changes that you made in the original (parent) feature.
...
1. Select **OK**.
1. Review the feature components, and make any required changes.
...
When you create a new version of an Electronic invoicing feature, the system copies the latest feature version. To use an earlier version of the feature as the base for a new version, select the version, and then select **Get this version**. The system creates a new draft version of the feature that's a copy of the selected version.
