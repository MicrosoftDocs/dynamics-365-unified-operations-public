...
8. Select **OK** to start the import.
    ...
    > [!IMPORTANT]
    > Currently, there is a size limit of 2 Gb for address import. The state can't be imported if it contains an XML file that exceeds the 2-Gb size limit.
...
The process of importing addresses can take time. For example, if the address data size is about 800 M for state 01, the import might take one to two hours. We recommend that you use the batch job mode by enabling the **Batch processing** option.
