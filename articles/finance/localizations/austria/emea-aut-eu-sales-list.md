...
The Austrian EU sales list report includes the following fields:
...
>[!NOTE]
>The .xml file for the EU sales list report shows the company's tax registration number.
...
> [!NOTE]
> If you set the **VAT exempt number export** field in the **Intrastat** section of the **Foreign trade and logistics** FastTab (that is, it isn't blank), the value is used instead of the VAT ID that you created in step 6 in the .xlsx file for the EU sales list report.
...
1. On the **EU sales list** tab, set the **Report cash discount** option to **Yes** if a cash discount should be included in the value when a transaction is included on the EU sales list.
1. On the **Electronic reporting** FastTab, in the **File format mapping** field, select **EU Sales list (AT)**.
...
1. In the **Authority identification** field, enter a two-digit code in the range from **01** through **98**.
...
1. Transfer transactions as required.
...
1. Go to **Tax** > **Indirect taxes** > **Sales tax** > **Sales tax authorities**.
1. In the **Sales tax authority** field, select **AT**.
1. In the **Authority identification** field, enter **55**.
...
1. Select **OK**, and review the report in XML format that is generated. The following tables show the values in the example report.
    ...
    **EU sales list lines**
    ...
    | SOLEI   |              | 1              | The **SOLEI** field is available only for service transactions and has a value of **1**. For triangular transactions, there's a **DREIECK** field that has a value of **1**. |

1. Review the report in Microsoft Excel format that is generated.

    :::image type="content" source="../media/EUSL-aut.png" alt-text="Screenshot of the EU sales list report for Austria in Microsoft Excel format.":::
