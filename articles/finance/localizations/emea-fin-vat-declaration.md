...
This topic describes how to set up and generate a value-added tax (VAT) declaration for Finland in the official TXT format and how to preview the VAT declaration in Microsoft Excel.
...
| 304 | Tax on the import of goods from outside the EU. | Vero tavaroiden maahantuonnista EU:n ulkopuolelta | ImportsOfGoods</br>ImportsOfGoodsUseTax – when *Use tax* option is used |
...
| 317 | The amount of VAT relief. | Alarajahuojennuksen määrä | Not applicable, calculated automatically. |
| 318 | Tax on the purchases of construction services and scrap metal (reverse charge). | Vero rakentamispalvelun ja metalliromun ostoista | PurchasesReverseCharge</br>PurchasesReverseChargeUseTax – when *Use tax* option is used |
...
The following tasks prepare your Microsoft Dynamics 365 Finance environment to generate an electronic file for VAT declaration for Finland and preview VAT amounts in Excel format.
...
To automatically generate a VAT declaration, associate sales tax codes in Finance to the lookup results in the ER configuration.
...
Follow these steps to define which sales tax codes in your Finance environment generate which filed ID on the VAT declaration for Finland.
...
5. On the **Sales tax** tab, on the **Tax options** FastTab, in the **VAT statement format mapping** field, select the **VAT declaration Excel (FI)** ER format.
   ...
   This format is printed when you run the **Report sales tax for settlement period** report. The format is also printed when you select **Print** on the **Sales tax payments** page.
...
To generate a VAT declaration, configure the tax registration number of your organization (field \"010\" - Asiakkaan y-tunnus tai henkilötunnus).
...
Complete the following steps to define the VAT registration number that is used by **Electronic messaging** during generation of VAT declaration for Finland.
...
3. Select **OK** and on the next page, set the following fields.
...
Sales tax payment transaction results in the [Settle and post sales tax](../general-ledger/tasks/create-sales-tax-payment.md) job procedure that settles sales tax balances on the sales tax accounts, and offsets them to the sales tax settlement account for a given period. When the **Settle and post sales tax** job procedure is completed for an interval of the sales tax settlement period, you can generate a VAT declaration in Excel from **Sales tax payments** page.
...
## Generate the electronic file for VAT declaration from electronic messages
...
5. On the **Messages** FastTab, select **Collect data**, and then select **OK**. The sales tax payments that were generated earlier as a result of the [Settle and post sales tax](../general-ledger/tasks/create-sales-tax-payment.md) job procedure are are added to the message.
...
11. Select **Attachments** in the upper-right corner of the page, and then select **Open** to open the file.
...
2. In the list, find and select the feature, **Cross-company queries for the populate records actions**, and then select **Enable now**.
...
6. For each new line, select **Edit query**, and specify a related settlement period for the legal entity that is specified in the **Company** field on the line.
   ...
   When the setup is complete, the **Collect data** function on the **Electronic messages** page collects sales tax payments from all legal entities that you defined.
