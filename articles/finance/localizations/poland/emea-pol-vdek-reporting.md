...
The JPK-V7 reporting process is predefined by the data entities that the **PL JPK_V7 EM setup.zip** package delivers. The following illustration shows an overview of the process.

:::image type="content" source="../media/vdek-em-processing.jpg" alt-text="Screenshot of JPK_V7M reporting process flow.":::
...
:::image type="content" source="../media/create-em.jpg" alt-text="Screenshot of Run processing dialog.":::

1. A new electronic message is created. Enter a description, and specify the start and end dates of the period that you want to generate the JPK-V7 report for.
...
1. On the **Message additional fields** FastTab, specify any additional values that are required for the declaration part of the JPK-V7 report.

:::image type="content" source="../media/message-additional-fields.jpg" alt-text="Screenshot of Message additional fields FastTab.":::

1. In the **CelZlozenia** additional field, specify whether this submission is an original report or a correction. Two values are allowed:
...
1. In the **Wersja schematu** additional field, specify which version of the XML Schema Definition (XSD) schema must be used to generate the report. Two values are allowed:

    - **1** for JPK-V7(1).
    - **2** for JPK-V7(2). In the **PL JPK-V7 EM setup v.7 ID584502.zip** package, this value is set as the default value.
    - **3** for JPK-V7(3). In the **PL JPK-V7 EM setup v.9 ID1074347.zip** package, this value is set as the default value. Version 3 of JPK-V7 schema must be used from February 1, 2026.
...
1. In the **Sklad pliku** additional field, specify which content of JPK-V7 must be included in the report. Two values are allowed:
...
1. In the **Wariant JPK_VAT** additional field, define a variant of the XSD schema of the XML file that must be generated. Two values are allowed.
...
1. You can also specify manual values for the following additional fields that are related to elements of the declaration.
    ...
    | P_54_Powód - in JPK-V7M(1) | P_55, P_56, P_57, or P_58, depending on the user selection. | The refund to the bank account that article 87, section 6a (**P_55**), 6 (**P_56**), 2 (**P_57**), or 5a (**P_58**) of the VAT Act refers to. | Zwrot na rachunek VAT, o którym mowa w art. 87 ust. 6a (P_55) lub 6 (P_56) lub 2 (P_57) lub 5a (P_58) ustawy |
    ...
    | P_60     | P_60 | A non-negative integer that has a maximum of 14 digits. The amount of refund that you must credit against future tax liabilities. | Wysokość zwrotu do zaliczenia na poczet przyszłych zobowiązań podatkowych |
    | P_61     | P_61 | A string (1..240) that you must use if **P_60** is used. The type of future tax liability. | Rodzaj przyszłego zobowiązania podatkowego |
    ...
    1. Select **New** \> **Note**, and then enter information in the **Description** and **Notes** fields.
...
1. To run report generation in a batch, specify parameters on the **Run in the background** FastTab. When the report is generated, the process attaches it to the electronic message as a file.
1. Select **OK**.

:::image type="content" source="../media/generate-vdek.jpg" alt-text="Screenshot of using the Run processing dialog box to generate the JPK-V7 report in Excel format.":::

1. To view the file, select the electronic message, and then select the **Attachments** button (paper clip symbol) in the upper-right corner of the page.
1. On the **Attachments for Message** page, select the attachment, and then, on the Action Pane, select **Open**.
...
:::image type="content" source="../media/generate-xml-vdek.jpg" alt-text="Screenshot of using the Run processing dialog to generate an XML file in JPK-V7 format.":::

After you select **OK** in the dialog, you see the following declaration text:

- **English:** "When you generate the VAT declaration you confirm information in the report is true and complete. Your consent is recorded in the report. Incomplete payment or non-payment of VAT due to the Tax Authority, this declaration is the basis for the issuance of a writ of execution in accordance with the provisions of the enforcement proceedings in the administration. A false or incomplete declaration may result in prosecution in accordance with regulations of fiscal penal code."
...
By selecting **OK** on the declaration page, you give your consent to the declaration. The JPK_V7 is generated only if you consent to the declaration.

The action log related to the electronic message log information about the user who generated the JPK_V7 and performed other actions with the electronic message.

When you generate an XML file for the JPK-V7 report, you attach it to the electronic message. To view the file, select the electronic message, and select the **Attachments** button (paper clip symbol) in the upper-right corner of the page. On the **Attachments for Message** page, select the attachment, and then, on the Action Pane, select **Open**.

## Change the status of the electronic message

When you finish working with a report, you can change the status to **Zgłoszone JPK-V7M** or **Zgłoszone JPK-V7K** (**Reported JPK-V7M/Reported JPK-V7K**). You can't delete electronic messages that are in this status. You can change the status back to **Wygenerowane JPK-V7M** or **Wygenerowane JPK-V7K** as required.
...
If you generate the JPK-V7 report in XML format for a period that is shorter than a full month or a full quarter, you receive a warning that it can't be used for submission to the authority. The report also contains the following information that indicates that it isn't for a full month:

- The **\<Miesiac\>** tag contains information about the date interval that report is generated for.
- The **\<P_\*\>** tag of the **Deklaracja** part of the report contains calculated values that represent the aggregation of amounts from the **SprzedazWiersz** and **ZakupWiersz** parts of the report. The values in this tag are rounded to a two-digit decimal value, not an integer value.
