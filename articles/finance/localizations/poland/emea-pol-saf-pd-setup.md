...
2. In the configuration tree, select **General ledger data model mapping** configuration and set **Default for model mapping** option to **Yes**. Ensure that no other model mapping conguration under the **Standard Audit File (SAF-T)** model is not set as **Default for model mapping**.
...
11. Click **Export** button on the Action pane to export the application-specific parameters as an XML file and close the **Application specific parameters** page.
...
15. Click **Import** button on the Action pane and select the XML files saved on the step 10.
