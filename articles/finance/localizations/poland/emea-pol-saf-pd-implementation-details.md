...
Use the **Journalize transactions** periodic task before you report the JPK_KR_PD. For more information, see [Journalize posted journal entries](../../general-ledger/tasks/journalize-posted-journal-entries.md).

When you run the **Journalize transactions** periodic task for detailed records of accounting entries, the **D_1** field represents the number from journalized journal. If you don't run the **Journalize transactions** periodic task for some records of accounting entries, the **Voucher** value is reported in **D_1** field of this record.
...
For the records of accounting entries that correspond to sales operations, this field represents the **Sales date** value if it's filled in. If the **Sales date** isn't used, it represents the **Document date** or **Accounting date**.

For the records of accounting entries that correspond to purchase operations, this field represents the **Date of vendor VAT register** value if it's filled in. If the **Date of vendor VAT register** isn't used, it represents the **Document date** or **Accounting date**.
