...
> [!IMPORTANT]
> As of September 2024, you can import all electronic invoicing Globalization features only into the **Globalization studio** workspace, because Regulatory Configuration Service (RCS) is decommissioned. For more information about migration to Globalization Studio, see [Regulatory Configuration Service merge to the Globalization Studio workspace](../global/workspace/merge-rcs-to-gsw.md).

## Country/region-specific configuration for the French Chorus Pro submission (FR) electronic invoicing feature

You need to complete some steps to configure the **French Chorus Pro submission (FR)** electronic invoicing feature. The configuration publishes some parameters with default values. Review and update these values so that they better reflect your business operations.
...
- Your organization must be registered to operate with Chorus Pro. Microsoft provides integration with Chorus pro in OAuth2 Mode via an application programming interface (API). For detailed information about Chorus Pro registration and application activation, see the [official documentation](https://communaute.chorus-pro.gouv.fr/documentation/help-for-api-developers-in-oauth2-mode/).
    ...
    1. Copy and save the client ID of the OAuth credentials and the secret key. You'll use this information in later steps.
    ...
    1. Copy the user ID of the technical account and the password. You'll use this information in later steps.
...
Some of the parameters for the **French Chorus Pro submission (FR)** electronic invoicing feature are published with default values. Before you deploy the electronic invoicing feature to the service environment, add a feature that is based on the Microsoft-provided feature, and complete the common parameters on the **Feature parameters** tab. Review and update the default values as required, so that they better reflect your business operations.

For France, there are at least three interactions with Chorus Pro in the pipeline: first to submit the invoice, then to fetch the status, and finally to get the details of the submitted invoice. Each interaction requires common parameters, such as Chorus Pro connection and authentication details. You reuse these common parameters in the feature setup for all document types. Chorus Pro provides the values when a company is onboarded.

> [!NOTE]
> The configuration of common parameters is simplified. You no longer have to go to each action and feature setup, and repeatedly specify the common connection parameters. The **Feature parameters** tab is available only as of version 10.0.41 or later.

> [!IMPORTANT]
> Microsoft provides the **French Chorus Pro submission (FR)** feature. Before you can use it, you must complete additional configuration, as described in this article. For information about how to configure invoicing features and apply changes, see [Configure feature setups](../global/gs-e-invoicing-feature-setup.md). For example, in addition to the connection parameters, you can filter specific legal entities so that they're processed in applicability rules. By default, the feature applies to all legal entities that have a primary address in France. Additionally, **out of the box**, the feature is configured to connect to the **sandbox environment of the PISTE service**.
...
> [!NOTE]
> To track the status of submitted documents in Chorus Pro, enable two additional features in the **Feature management** workspace: **Extended document identification in submission log** and **Execute update actions for submitted documents**.
...
1. Select a legal entity, and then, on the **Addresses** FastTab, add a valid French primary address for the legal entity. Make sure that the following mandatory address elements are defined:
...
When you submit work invoices, use the **Project manager** party. Before you use it, set up the new party type as a customer account. You can then use it when you post invoices.

Follow these steps to enable the project manager for a project invoice.

1. Go to **Project management and accounting** > **Projects** > **Project contracts**.
1. On the **Funding sources** tab, in the **Project manager** field, select a customer account. Then select **Project contracts** > **Project contracts** > **Project contracts**.
...
1. On the **Chorus Pro** tab, add the following information:
    ...
    > [!NOTE]
    > Use Azure Key Vault storage to store the secrets. For more information, see [Set up the Azure Key Vault client](../global/setting-up-azure-key-vault-client.md).

1. Enable the **SIRET and service code validation during invoice posting** parameter.
1. In the **Report format** field, select the configurable business document to use when a submission report is printed.
...
1. For each table name, set the **Document context** and **Electronic document model mapping** fields as specified in [Set up Electronic document parameters](../global/gs-e-invoicing-set-up-parameters.md#set-up-electronic-document-parameters).
1. In the **Customer Invoice journal** table name field, select **Response types**.
1. Create a new response type that has the same name that was defined for the related variable in the corresponding feature setups.
...
1. Create another new response type that has the same name that was defined for the related variable in the corresponding feature setups.

    1. In the **Submission status** field, select **Pending update actions execution**.
    2. In the **Data entity name** field, select **Sales invoice Chorus Pro entity**.
    3. In the **Configuration** field, select **Chorus Pro response invoice data import format (FR)**.

    :::image type="content" source="../media/emea-fr-einvoice-response.jpg" alt-text="Screenshot of the response type setup.":::

### Define electronic invoice frameworks

1. Go to **Accounts receivable** > **Setup** > **Electronic invoice frameworks**.
1. In the **Type** field, enter the electronic invoicing framework.

   If you don't configure an electronic invoice framework, the system sends all invoices as **A1 - simple invoice**. Use the following codes for the invoice framework to attribute work invoices: A4, A7, A8, A9, A10, A12, A13, A14, and A22. For more information, see [Invoicing framework and transmission modes](https://communaute.chorus-pro.gouv.fr/documentation/submit-works-invoices-for-suppliers-contracting-party-subcontracting-co-contracting/?lang=en#1530527446538-8a6bf25f-3ff8).

1. If you enable **SIRET and service code validation during invoice posting**, set up the following options to have more granular control over the validations that run in the invoice posting process:

   1. Go to **Accounts receivable** > **Setup** > **Electronic invoice frameworks**.
   1. Add the required setup lines to allow users to enable or disable validation per document and party type.
 
      > [!NOTE]
      > Where relevant (Project invoice proposal, Free text invoice, Sales order), the system runs **only enabled validations** after selecting the Invoicing framework and during posting when **SIRET and Service code validation during invoice posting** is enabled. When validation for the Service code is enabled, it validates **only** when the MOE/MOA SIRET structure requires it, not indiscriminately.
...
1. Switch to the **Header** view, and select the following information:

    - Electronic invoice framework type
    - Invoice account service code

        > [!NOTE]
        > If you enabled the **SIRET and service code validation during invoice posting** parameter in the previous section, you get the list of available service codes from Chorus Pro.
...
1. Open an existing invoice proposal, or create a new one.
1. Switch to the **Header** view, and select the following information:
...
To import documents from Chorus Pro, go to **Organization administration** > **Periodic** > **Electronic documents** > **Receive electronic documents**. After the import process finishes, you can find the received documents in the **Pending vendor invoice** workspace. You can add more attributes to these documents as needed. Some of the attributes you can add include:
