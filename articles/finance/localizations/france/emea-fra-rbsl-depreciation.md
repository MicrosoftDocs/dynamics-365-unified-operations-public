...
When you create a **Fixed asset group/value model** setup, you specify the number of years and the number of months in the service life of the fixed asset. By default, use these values for some fixed asset value models, and use them to calculate the RB/SL factor for the fixed asset. For each fixed asset, calculate the reducing balance percentage based on the service life of the asset and the appropriate RB/SL factor: Reducing balance % = RB/SL factor ÷ Service life × 100%. Additionally, calculate and compare the reducing balance and straight line remaining depreciation amounts, and use the larger amount as the RB/SL depreciation amount. For example, a fixed asset that has an acquisition price of EUR 1,000.00 is put into service on February 10, 2016, and is depreciated over five years by using the RB/SL method. Here's the calculation for the reducing balance percentage: Reducing balance % = 1.75 ÷ 5 × 100% = 35% The following table shows the depreciation amounts and the book value for the asset for each year.
