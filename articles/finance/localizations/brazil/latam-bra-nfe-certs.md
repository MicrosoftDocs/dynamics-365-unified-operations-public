...
The process that you use to install certificates varies, depending on the state where you issue the NF-e fiscal document. Use the correct certificate installation process so that Dynamics 365 Finance can connect with the state web services.

The Brazilian government updated the root certification authority (CA) that it uses to issue the certificate required to establish a Secure Sockets Layer (SSL) connection on the server side. The current version of the root CA is AC Raiz v5. By default, Microsoft Windows deploys the previous version (AC Raiz v2).

## Solutions

- Manually install root CA v5 (AC Raiz v5) – You must install the root certificate in the Trusted Root Certification Authorities store. Although the Brazilian government created a new root certificate (v5), Windows Security hasn't yet added this certificate to the Trusted Root Certification Authorities store. This means you must open an incident in Windows Support and then manually install the certificate. Use this workaround until the Windows Security group reaches an agreement with the Brazilian government.
- Install a NF-e HTTP proxy server – The tax authorities for some states, such as Ceará (CE), Goias (GO), Pernambuco (PE), and Rio de Janeiro (RJ), among others, haven't enabled the most secure and market-standard cipher suites in their web servers. However, the Windows virtual machines (VMs) that are used to run Finance follow the Microsoft security policy and allow only connections that use TLS 1.2 with more secure cipher suites. Therefore, the system can't connect directly to these web services. To route communications to those states, you must install a proxy HTTP server in your Microsoft Azure subscription. For more information about the supported cipher suites, see [Encryption in finance and operations apps](../../../fin-ops-core/dev-itpro/sysadmin/encryption.md).

> [!NOTE]
> To verify that your endpoints comply with the supported ciphers described in the article, [Encryption in finance and operations apps](../../../fin-ops-core/dev-itpro/sysadmin/encryption.md), perform a test by using a TLS cipher and scanner tool. Test your endpoint by using [SSLLABS](https://www.ssllabs.com/ssltest/analyze.html).

You must install Brazilian certificates through Azure Key Vault. Key Vault lets you store encrypted keys, certificates, and secrets (such as authentication keys, storage account keys, data encryption keys, .pxf files, and passwords) by using keys that are protected by hardware security modules (HSMs).
...
The following table lists each tax authority and the solution that you should use to install certificates for it. If your test of TLS cipher reveals that the tax authority doesn't use the more secure cipher suites, install the NF-e HTTP proxy server.
