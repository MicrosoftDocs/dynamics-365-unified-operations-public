...
This difference between the incoming and outgoing amounts is known as the *ICMS-ST adjustment*, because the correction can be either positive (+) or negative (-), and you must report it in SPED Fiscal or other related tax statements.

- Taxpayers need to determine the ICMS-ST tax amount to collect for the products subject to ICMS-ST.
- Taxpayers must compare the ICMS-ST calculated amount with the purchase ICMS-ST tax amount.
- Taxpayers need to register the adjustment as positive or negative, depending on the result.
- Taxpayers need to determine the ICMS-ST to complement or to restore.
...
SP state introduced a new tax statement, CAT 1219. This tax statement requires that companies submit an ICMS-ST tax declaration monthly for tax complement and restitution of ICMS-ST amounts.

The statement file is generated based on version 1.1.0 of the guidance layout document.

### Scope

The following tables provide information about the records that the CAT 1218 statement generates.
...
| 07         | COD_VER   | The code for the version of the layout. You manually fill in this field when running the reports.                                  |
| 08         | COD_FIN   | The code for the purpose of the file. You manually fill in this field when running the report. The following values are available: |

- **00** – Regular file delivery
- **01** – File delivery that the tax authority requires
- **02** – File delivery for replacement of a previously uploaded file

#### Record 0150 – Participant table

Only users who have transactions in the reported booking period are included in this record. There's one record per user.
...
Only items that have transactions in the reported booking period are included in this record. There's one record per item.
...
This record reports the total amount (initial inventory position and final inventory position) of record 1100 records. There's one record per item. The ICMS-ST tax assessment collects the information from the **Presumed taxed balances** functionality.
...
| 04         | ICMS_TOT_INI | The accumulated initial value of the total ICMS that the taxpayer supports and calculates for the item at the beginning of the period |
| 05         | QTD_FIM      | The final item quantity at the end the period                                                                                                |
| 06         | ICSM_TOT_FIM | The accumulated final value of the total ICMS that the taxpayer supports and calculates for the item at the end of the period         |

#### Record 1100 – Balance details record

This record reports details of electronic fiscal documents. The system collects this information from the data stored on the **Outgoing** and **Incoming** tabs of the **Presumed taxes** page.
...
| 09         | ICMS_TOT  | The total value of ICMS that the taxpayer supports in incoming fiscal documents                                                              |
...
Through Decree 54308/2018, RS state established the procedure for calculating and reporting the complement or restitution of ICMS-ST tax amounts for contributors (retailers and non-retailers) that perform activities involving goods subject to the tax substitution regime (ICMS-ST).

The state defined two processes: one for retail companies and the other for non-retail companies.

### Scope

The state considers the ICMS-ST amount calculated for complement and restitution as a sub-assessment of ICMS-ST. Report this amount in SPED Fiscal ICMS-ST in record 1900 and related adjustment records, such as records 1921 and 1923.
...
Through SEF Portaria No. 396/2018, Santa Catarina state established the procedure for calculating and reporting the complement or restitution of ICMS-ST tax amounts for contributors (retailers and non-retailers) that perform activities that involve goods that are subject to the Tax substitution regime (ICMS-ST).
...
The following records are out of scope: 2112 (outgoing fiscal documents that the Fiscal printer ECF generates) and 2131.

### Setup

1. Go to **Organization administration** \> **Setup** \> **Brazilian parameters**.
1. On the **Fiscal document** tab, in the **ICMS-ST** section, set the following options to **Yes**:
 ...
 :::image type="content" source="../media/brazil-icms-01.png" alt-text="Screenshot of the Brazilian parameters page, ICMS-ST section.":::

1. Go to **Fiscal books** \> **Setup** \> **Fiscal books parameters per state**, and then select the appropriated state (**SP**, **SC**, or **RS**).

### Presumed tax

On the **Presumed tax** FastTab of the **Fiscal books parameters per state** page, set the following parameters:

- **Presumed tax calculation algorithm** – Select the method that is used to calculate the presumed ICMS-ST tax when outgoing fiscal documents to final and non-final consumers are generated and posted in Dynamics 365 Finance. This parameter fills in the related tags when the NF-e fiscal document model.

  - **Last purchase** – Use information from the latest incoming fiscal documents that you posted during the period.

- **SPED Presumed tax calculation algorithm** – Select the method that is used to calculate the presumed ICMS-ST from ICMS-ST tax assessments that you create in the **Fiscal books** module:

  - **Average** – Calculate the average amount from incoming fiscal documents. Select this method for SP or SC state.
  - **Last purchase** – Use information from the latest incoming fiscal documents that you posted during the period. Select this method for RS state.

  :::image type="content" source="../media/brazil-icms-02.png" alt-text="Screenshot of the Fiscal books parameters per state page, Presumed tax FastTab.":::

### ICMS-ST sub-tax assessment in RS state

For RS state, the amounts and details about the calculation of ICMS-ST compensations and restitution appear in SPED Fiscal in record 1900 and other related records. To automatically create the adjustment transactions that Decree 54308/2018 establishes, you need to complete additional setup.
...
- **Sub tax assessment indicator** – Select the type of indicator that identifies the sub-tax assessment.
- **Sub tax assessment description** – Enter a description of the sub-tax assessment that appears in the **03 DESCR_COMPL_OUT_APUR** field in record 1900.
- **Journal name for restitution/complement adjustment** – Select the journal that creates and posts the compensation and restitution tax assessment.
...
- **Complement (E210-E220)** – Select the related adjustment code for this type of adjustment. When this adjustment is created, the ICMS-ST tax assessment updates.
- **Restitution (E111)** – Select the related adjustment code for this type of adjustment. When this adjustment is created, the ICMS tax assessment updates because only credits are allowed in the ICMS tax assessment.
- **Consolidate record 1923** – Set this option to **Yes** to consolidate or report on record 1923 per fiscal document (no items appear). The **08 COD_ITEM** field remains blank.

  :::image type="content" source="../media/brazil-icms-03.png" alt-text="Screenshot of the Fiscal books parameters per state page, Sub assessment ICMS-ST parameters FastTab.":::

## Repro steps

### ICMS-ST tax assessment

Before you generate a statement for each state, create a fiscal booking period for the month, create the ICMS-ST tax assessment for the state, and then calculate any balances that apply in that state. If you want to create a credit adjustment related to inventory position, mark **Yes** for the **Credit inventory adjustment** checkbox in ICMS-ST tax assessment form before to confirm.

> [!NOTE]
> The credit adjustment related to inventory position is only applicable for RS and SC states.

:::image type="content" source="../media/brazil-icms-04.png" alt-text="Screenshot of the ICMS-ST tax assessment page.":::

1. Go to **Fiscal books** \> **Common** \> **Tax assessment** \> **ICMS-ST**.
1. On **ICMS-ST**, on the Action Pane, on the **General** tab, in the **View** group, select **Presumed taxes balance**.

 :::image type="content" source="../media/brazil-icms-05.png" alt-text="Screenshot of the ICMS-ST page, General tab on the Action Pane.":::
...
- Amount per inventory unit and totals
  ...
  :::image type="content" source="../media/brazil-icms-06.png" alt-text="Screenshot of the Presumed taxes balance page.":::
...
Here's how the function for the opening balance calculation works:

1. Validate whether a presumed tax balance for the previous period exists. If no balance exists, the calculation starts. If a balance exists, the procedure considers the values from the previous period.
1. Select all items in on-hand inventory that have any transactions before the current period.
1. Calculate the quantity for every selected item on the first day of the period.
1. Find the last purchase for every item that has a ICMS-ST tax transaction.
1. By using the tax amounts from the last purchase and converting the purchase unit to the inventory unit, calculate ICMS and ICMS-ST amounts for the on-hand inventory quantity.

Here's how the function for the closing balance calculation works. This function is applicable only to SP state.

1. Delete old presumed tax balances for the period except those that are used for the next period. These amounts are updated, and all other amounts are recalculated.
1. Select all items in on-hand inventory that have any transactions during theperiod.
1. Search for the presumed tax for every item.
1. Calculate the balance in the previous periods to get the opening balance quantity. If there's no balance in previous periods, the opening balance quantity is 0 (zero).
1. Select all incoming fiscal documents in the period, summarize ICMS and ICMS-ST tax amounts, and summarize item quantities.
1. Calculate the average amount per unit by summarizing the totals of ICMS and ICMS-ST amounts in all incoming documents and dividing them by the summarized quantity in those documents.
1. Select all outgoing fiscal documents, and summarize their quantities. Then calculate ICMS and ICMS-ST tax amounts by using the average amount that was calculated in step 6.
1. Calculate closing balance amounts and quantities by using the values from the previous periods, and the sums of incoming and outgoing documents from the current period. These sums were calculated in steps 5 and 7, respectively.

#### Presumed tax calculation

After you calculate the balances for the first time, open the **Presumed taxes** page, and select **Calculate presumed tax**.

:::image type="content" source="../media/brazil-icms-07.png" alt-text="Screenshot of the Presumed taxes page.":::

The page includes fields that are required for reporting purposes. Each tab shows the related fiscal documents that are applicable for compensation and restitution according to the criteria that state law establishes.
...
:::image type="content" source="../media/brazil-icms-08.png" alt-text="Screenshot of the total amounts shown on Presumed taxes page.":::

Here's how the average algorithm for presumed tax calculation works:

1. Select all lines of all outgoing fiscal documents in the current period.
1. For every item from these lines, select all lines of all incoming fiscal documents that have the same item code in the current period. Then calculate the average ICMS and ICMS-ST tax amounts.
1. If no incoming documents exist in the period, consider the corresponding record in the presumed tax balances.
...
> [!NOTE]
> After you select **Generate adjustments**, you can't revert to the initial state. To report record E220 or E111, you must manually reverse the adjustment transaction that is created during execution of the Generate adjustments function. The other adjustments that are related to debit, credit, credit for inventory, and credit reversal are created only for SPED Fiscal purposes. You won't see these transactions, because they're saved in a temporary table.

### Generate a statement

#### SP state

1. Go to **Fiscal books** \> **Common** \> **Booking period**. On the **Tax statements** tab, select **CAT 4218 SP**. You can also find this statement in the ICMS-ST tax assessment for SP state.
1. In the **File name** field, enter the full path where you want to save the text file. Include the name of the file.
1. In the **Layout version** field, select the version.
1. In the **File type** field, select the type of file:

 - Regular
 - Specific intimation
 - Substitute

Validate the generated text file by using the tax authority application. After the file is approved, send the information to the tax authorities.

#### RS state

1. Go to **Fiscal books** \> **Common** \> **Booking period**. On the **Tax statements** tab, select **SPED fiscal**. You can also find this statement in the ICMS-ST tax assessment for RS state.
1. Set the parameters that are usually set to generate the SPED Fiscal statement. To generate record 1900 and other related records, set the parameters that are required to generate the SPED Fiscal statement. To set up those parameters, follow these steps:
1. Create and sync the related booking period.
1. Sync inventory. If applicable, take the credit from the inventory opening balance.
1. Create or update the ICMS-ST tax assessment for RS state.
1. Calculate presumed tax balances (if applicable).
1. Calculate presumed tax.
1. Generate adjustments.
1. Complete the remaining steps that are related to the ICMS-ST tax assessment.
1. Generate the SPED Fiscal statement in the usual way.
...
2. On the **Setup parameters** FastTab, add the internet address and the request header address, so that the related XML file can be submitted to the government tax authorities.
 ...
 :::image type="content" source="../media/brazil-icms-09.png" alt-text="Screenshot of the Tax statements parameters page, Setup parameters FastTab.":::
...
Submit the file by using web services. A certificate is required. After you receive the return message together with the result of government validation, save the response in the same location where the file was generated.
