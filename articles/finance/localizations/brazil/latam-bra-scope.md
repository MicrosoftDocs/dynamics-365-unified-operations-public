...
The software helps organizations run their business operations while complying with country or region-specific laws, regulations, and common business practices for handling their daily activities. This software includes features and functionality that are designed to address specific federal tax, accounting, financial, or statutory reporting laws or regulations that commonly affect businesses in Brazil.
...
The localization scope for Microsoft Dynamics available in Brazil is limited to tax calculation, accounting transactions, issuing and receiving fiscal documents, and issuing fiscal receipts in the following four scenarios: procure to pay, quote to cash, commerce, and regulatory or statutory reporting.

For a list of features that Microsoft delivers and supports as part of the Brazilian localization for Microsoft Dynamics, see the [Brazilian localization features](#brazilian-localization-features) section of this article. Details about each of the features can be found in Help in Microsoft Dynamics, and in white papers that are published on the [Microsoft Dynamics Localization Portal](/dynamics/s-e/).

## Market availability

Microsoft's goal is to deliver regulatory features in time for installation. To implement the tax, accounting, financial, or regulatory/statutory reporting requirements that typically affect a majority of businesses in Brazil, Microsoft aims to release tax and regulatory updates that contain required changes before the effective date or other date mandated by the applicable government authority at either the federal or state level (but only for the states that are identified in the [Brazilian localization strategy](#brazilian-localization-strategy) section of this article). This schedule ensures that channel partners have enough time to update their customer solutions. Internally, Microsoft calls this date the *market required date* (MRD).

Microsoft strives to meet its established MRDs and dates that the applicable government authority mandates. However, various factors can affect the timely delivery of tax and regulatory updates. Here are some examples:

- Legislative or regulatory changes that the government makes, but doesn't give enough notice about before the date when the law takes effect
...
If it's not feasible to meet these dates, Microsoft uses commercially reasonable efforts to develop and release tax and regulatory updates as soon as possible.

Any dates that Microsoft publishes are for planning purposes only and are subject to change at any time without notice.

Microsoft makes no representations, warranties, or guarantees about the timeliness or completeness of any tax or regulatory update that it provides. To the maximum extent permitted under applicable law, Microsoft disclaims all implied warranties and conditions, such as implied warranties or conditions of merchantability and fitness for a particular purpose.
...
- Configurable default taxes based on operations defined or specified per CFOP group
...
- Posting fiscal documents for receiving:
  ...
  - From vendors that aren't ICMS payers or contributors (using models 1, 1-A, and 55)
...
- Customer returns with a fiscal document that the customer issues
- Electronic fiscal document access key

### Inventory

- Issue and receive fiscal documents for transfers and returns of inventory items between fiscal establishments
- Issue and receive fiscal documents for remittance and returns of inventory items from a third party
...
- Support for the following records for companies defined as Profile A:
  ...
  - Block 1990: 1900-1910-1920-1921-1923-1926-1990 only for Rio Grande do Sul state
...
- Requirements from states or regions other than the states or regions that are described in the [Brazilian localization strategy](#brazilian-localization-strategy) section of this article
...
- **Commerce industry:**
  ...
  - **Retailers:** You can't use POS in the following businesses in states where compliance with the PAF-ECF law "ATO COTEPE/ICMS N° 46" of 2014 is mandatory:
...
The Brazilian localization doesn't support the following tax reporting requirements for fiscal books:

- A printable version of fiscal books, in accordance with *AJUSTE SINIEF 2* from April 3, 2009, which makes the requirement for printed books obsolete
- SPED Fiscal that has specific requirements from the states or regions that are described in the [Brazilian localization strategy](#brazilian-localization-strategy) section of this article
- *Simples Nacional* or *Super Simples* taxation mode
- *Regime especial* taxation mode (see the [Notes](#notes) section of this article)
- Requirements from states or regions other than the states or regions that are described in the [Brazilian localization strategy](#brazilian-localization-strategy) section
...
The [Microsoft Dynamics Localization Portal](/dynamics/s-e/) provides information about localization features and documents that Microsoft has released, and also localization features and documents that are planned for release.
