...
1. Sales order 1 for customer C1 has the following sales order lines:
    ...
    1. Sales order line 1.2, which has 20 units of item B

1. Sales order 2 for customer C2 has the following sales order lines:

    1. Sales order line 2.1, which has 200 units of item C
    1. Sales order line 2.2, which has 100 units of item D
...
1. You create load 1 that has the following load lines:
    ...
    1. Load line 1.2 for sales order line 1.2. This load line has 10 units of item B.
    1. Load line 1.3 for sales order line 2.1. This load line has 100 units of item C.

1. You create load 2 that has the following load lines:

    1. Load line 2.1 for sales order line 1.1. This load line has the remaining 20 units of item A.
    1. Load line 2.2 for sales order line 2.1. This load line has 50 units of item C.
...
1. The following fiscal documents are generated for load 1:
    ...
    1. A fiscal document for customer C2 that has a fiscal document line for 100 units of item C.

1. The following fiscal documents are generated for load 2:

    1. A fiscal document for customer C1 that has a fiscal document line for 20 units of item A.
    1. A fiscal document for customer C2 that has a fiscal document line for 50 units of item C.
...
|---|---|
| Country/region | The primary address for the legal entity must be in Brazil. |
| Related setup tasks | <ul><li>Create a fiscal establishment group and a fiscal establishment. </li><li>Set up a fiscal document type, and then assign it to customers, vendors, or inventory items.</li><li>Set up a transportation management shipping carrier that has the same vendor that you specified in the **Vendor account** field in the **Carrier** form in the **Sales and Marketing** module.</li></ul> |
| Related task | Create an outbound load for a shipment. |
...
1. Select **Accounts receivable** > **Periodic** > **Sales update** > **Loads**.
1. In the **Loads** form, select a load that has a confirmed shipment, and verify the identification code, status, direction, site, warehouse, and shipment of the load.
...
If you don't have access to the pages that you need to complete this task, contact your system admin, and provide the information in the following table.

| Category                      | Prerequisite |
|-------------------------------|--------------|
| Configuration keys            | Select **System administration** > **Setup** > **Licensing** > **License configuration**. Expand the **Trade** license key, and then select the **Warehouse and Transportation management** configuration key. |
