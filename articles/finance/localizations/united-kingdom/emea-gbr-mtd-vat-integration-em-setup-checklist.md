...
| Name                 | Description |
|----------------------|-------------|
| Dynamics 365 Finance | The HMRC web application for Microsoft Dynamics 365 Finance. Use this web application for VAT return submission. |
| Sandbox HMRC         | The HMRC sandbox for Finance. Use this web application for testing of the interaction with sandbox of HMRC. |
...
The following table shows other request parameters that you must define for these web services.
...
Both the **UK MTD VAT TEST** processing and the **UK MTD VAT returns** processing use the following other fields.

> [!NOTE]
> You can't change any of these fields.
...
| Tax registration number | The VAT registration number of the company that's submitting the VAT return. |
...
|----------------|-------------|---------------------------------------|
| Populated      | The record is filled in from sales tax payments. | Yes |
| Excluded       | The record is excluded from report generation. | Yes |
| To be reported | The line is included in the VAT return for submission. | No |
| Reported       | The record is included in a VAT declaration file. | No |
| Sent           | The record is sent to HMRC in a VAT declaration. | No |
| Submitted      | The record is submitted to HMRC. | No |
...
|-----------------------------------|-------------|---------------------------------------|
| New obligation request            | A new electronic message is created to request retrieval of VAT obligations. | Yes |
| Retrieved VAT obligation          | The VAT obligation request is sent. | Yes |
| Error VAT obligations retrieving  | A technical error occurred during retrieval of VAT obligations. | Yes |
| Completed VAT obligations request | VAT obligations are successfully retrieved. | Yes |
| Error VAT obligations importing   | A technical error occurred during import of VAT obligations. | Yes |
| New VAT return                    | This sales tax payment is included for further submission. | Yes |
| Ready to generate VAT return      | The message is ready to generate a VAT return. | Yes |
| Generated VAT return              | The VAT return in JavaScript Object Notation (JSON) format is generated and attached to the message. | No |
| Error VAT return generation       | A technical error occurred when the Electronic reporting (ER) report was generated. | Yes|
| Sent VAT return                   | The VAT return is sent to HMRC in JSON format. | No |
| Error VAT return submission       | An error occurred during submission of the VAT return. |Yes |
| Completed VAT return              | The VAT return is completed. | No |
| Error VAT return import response  | A technical error occurred during import of the VAT return response. | Yes |
| New liabilities request           | A new electronic message is created to request liabilities. | Yes |
| Error VAT liabilities request     | A technical error occurred during the request for VAT liabilities. | Yes |
| Completed VAT liabilities request | VAT liabilities are successfully retrieved. | Yes |
| New payments request              | A new electronic message is created to request VAT payment information. | Yes |
| Error VAT payments request        | A technical error occurred during the request for VAT payments. | Yes |
| Completed VAT payments request    | The request for VAT payments is completed. | Yes |

## Action populates records
...
| Retrieve VAT obligations            | <p>Prepare the request to retrieve VAT obligations.</p><ul><li>**From statuses:** Error VAT obligations retrieving, New obligation request</li><li>**To statuses:** Error VAT obligations retrieving, Retrieved VAT obligation</li><li>**Format mapping:** "MTD VAT interoperation (UK)"</li><li>**Executable class:** "RetrieveVATObligations"</li><li>**Web service:** "HMRC GET"</li><li>**File name:** "response.json.txt"</li></ul> | Message execution level |
| Test retrieves VAT obligations       | <p>Retrieve VAT obligations from the HMRC sandbox.</p><ul><li>**From statuses:** Error VAT obligations retrieving, New obligation request</li><li>**To statuses:** Error VAT obligations retrieving, Retrieved VAT obligation</li><li>**Format mapping:** "MTD VAT interoperation (UK)"</li><li>**Executable class:** "TestRetrieveVATObligations"</li><li>**Web service:** "HMRC sandbox GET"</li><li>**File name:** "response.json.txt"</li></ul> | Message execution level |
...
| Submit VAT return                   | <p>Submit the VAT return in JSON format to HMRC.</p><ul><li>**From statuses:** Error VAT return submission, Generated VAT return</li><li>**To statuses:** Error VAT return submission, Sent VAT return</li><li>**Message item type:** "VAT return"</li><li>**Format mapping:** "MTD VAT interoperation (UK)"</li><li>**Executable class:** "SubmitVATReturn"</li><li>**Web service:** "HMRC POST"</li><li>**File name:** "response.json"</li></ul> | Message execution level |
| Test submits VAT return              | <p>Test submission of the VAT return to the HMRC sandbox.</p><ul><li>**From statuses:** Error VAT return submission, Generated VAT return</li><li>**To statuses:** Error VAT return submission, Sent VAT return</li><li>**Message item type:** "VAT return"</li><li>**Format mapping:** "MTD VAT interoperation (UK)"</li><li>**Executable class:** "TestSubmitVATReturn"</li><li>**Web service:** "HMRC sandbox POST"</li><li>**File name:** "response.json"</li></ul> | Message execution level |
...
| Test retrieves VAT obligations       | Yes | Obligation |
...
| Test submits VAT return              | Yes | Submission |
