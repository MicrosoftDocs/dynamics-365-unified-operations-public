...
After you finish downloading the ER configurations, complete the following steps listed in the remainder of this article.
...
:::image type="content" source="../media/apac-nzl-gst-declaration.JPG" alt-text="Screenshot of the GST101A declaration form.":::
...
1. Select **Add** again, and then follow these steps to include the final record line:
   ...
   By adding this last record (NA), you define the following rule: When the tax code and name that is passed as an argument doesn't satisfy any of the previous rules, the transactions are not included in the GST return form. Although this rule isn't used when generating the report, the rule does help to avoid errors in report generation when there is a missing rule configuration.

1. In the **State** field, select **Completed**, and then select **Save**.
1. Close the **Application specific parameters** page.

:::image type="content" source="../media/apac-nzl-gst-declaration-setup.JPG" alt-text="Screenshot of the GST101A declaration setup page.":::
...
To avoid issues when the report is generated, create all mappings where the sales tax codes are posted. For example, if the second row in the table above wasn't included in the configuration, there might be errors when the report is generated because there's a credit note (sales invoice) with a sales tax code that's equal to GST15% posted in the related period. Go to **Tax > 88Inquire > **Posted sales tax** to review all posted sales tax codes and those that aren't included in this mapping of the configuration.
