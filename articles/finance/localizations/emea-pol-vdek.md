...
| P_52     | P_52 | A non-negative integer that has a maximum of 14 digits. The amount that was spent on the purchase of cash registers and that must be deducted in the specified period and returned in the given settlement period, or that increases the amount of input tax that must be transferred to the next settlement period. If **P_48** is more than or equal to **P_38**, or if the amount of benefits for the purchase of a cash register from the surplus tax on accrued, then in **P_52** takes the remaining amount of benefits for the purchase of cash registers, taxpayer reimbursement, or deduction from the tax payable for subsequent billing periods. | Kwota wydana na zakup kas rejestrujących, do odliczenia w danym okresie rozliczeniowym przysługująca do zwrotu w danym okresie rozliczeniowym lub powiększająca wysokość podatku naliczonego do przeniesienia na następny okres rozliczeniowy |
