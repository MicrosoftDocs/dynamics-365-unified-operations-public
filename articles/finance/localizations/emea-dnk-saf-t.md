...
| **Header Comment** | Specify your comment to SAF-T. Value specified in this parameter will be reported in `HeaderComment` element of SAF-T under `AuditFile > Header` node. It can contain any further generic comments on the audit file. Maximum length is 256 characters.  |
| **Tax Accounting Basis** | Value specified in this parameter will be reported in `TaxAccountingBasis` element of SAF-T under `AuditFile > Header` node. Select one of the following values from the list: <li>Invoice Accounting <li>Cash Accounting <li>Delivery <li>Other |
| **Tax Entity** | Value specified in this parameter will be reported in `TaxEntity` element of SAF-T under `AuditFile > Header` node. Select one of the following values from the list: <li>Company <li>Division <li>Branch reference |
