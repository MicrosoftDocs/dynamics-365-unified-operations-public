...
description: Learn about the strategy and scope for tax, finance, and accounting laws and regulations in Mexico, including outlines on terminology and abbreviations.
author: AdamTrukawka
ms.author: atrukawk
...
| Area | Item | Dynamics AX 2009 | Dynamics AX 2012 RTM-R2 | Dynamics AX 2012 R3 | Finance |
|------|------|------------------|-------------------------|---------------------|---------|
| Master data | Legal entity tax identifiers: RFC (Mexican tax ID – Registro Federal de Contribuyentes), CURP (Unique Population Registry Code – Clave Única de Registro de Población), Company type, State inscription, Tax regime | Yes | Yes | Yes | Yes |
| | Customer tax identifiers: RFC, CURP, Company type, State inscription | RFC only | Yes | Yes | Yes |
| | Vendor tax identifiers: RFC, CURP, Company type, State inscription | RFC only | Yes | Yes | Yes |
| | Vendor tax identifiers for the DIOT declaration: Type of vendor, Type of operation, Tax registration ID for foreign, Country/region code, Nationality | No | Yes | Yes | Yes |
| | Bank tax identifiers: RFC | Yes | Yes | Yes | Yes |
| | Tax types: VAT (IVA), ISR, IEPS | No | Yes | Yes | Yes |
| Taxes (Calculation by Core functionality) | VAT regular and conditional tax | Regular | Yes | Yes | Yes |
| | IEPS | Yes | Yes | Yes | Yes |
| | ISR regular | Yes | Yes | Yes | Yes |
| Procure (Receive goods and service invoice) | Input UUID of the CFDI document: Purchase invoice, Vendor invoice, Invoice register, General journal | Yes | Yes | Yes | Yes |
| | Input series and invoice number: Purchase invoice, Vendor invoice, Invoice register, General journal | Yes | Yes | Yes | Yes |
| | Input of related information for the DIOT declaration: Purchase invoice, Vendor invoice, Invoice register, General journal, Project – Expense, Project – Journal Fee | No | Yes | Yes | Yes |
| CFD | Support for CFD layout 2.0 | Yes | Yes | Yes | No |
| | Support for CFD layout 2.2 | Yes | Yes | Yes | No |
| | CFD messages/events: Issue | Yes | Yes | Yes | No |
| | Printable CFD version in PDF format | Yes | Yes | Yes | No |
| | XML viewer for issued CFD invoices | Yes | Yes | Yes | No |
| | Automatic sending of the CFD document by email to customers during the posting process | Yes | Yes | Yes | No |
| | Generation of a monthly report | Yes | Yes | Yes | No |
| CFDI | Support for CFDI layout 3.0 in customer invoices | Yes | Yes | Yes | Yes |
| | Support for CFDI layout 3.2 in customer invoices | Yes | Yes | Yes | Yes |
| | Support for CFDI layout 3.3 in customer invoices | Yes | Yes | Yes | Yes |
| | Support for CFDI layout 3.3 in customer payments with payment complement 1.0 | Yes | Yes | Yes | Yes |
| | Support for CFDI layout 3.3 in customer advance payments: CFDI advance payment, CFDI invoice and settlement, CFDI advance payment reverse | Yes | Yes | Yes | Yes |
| | Support for CFDI layout 3.3 in packing slip and transfer orders between sites (TRASLADOS) | No | No | Yes | Yes |
| | Support for CFDI layout 3.3 in foreign customer invoices and foreign complement 1.1 | No | No | Yes | Yes |
| | Support for Global CFDI layout 3.3 in customer receipts (Commerce) | No | No | Yes | Yes |
| | Support for CFDI withholding layout 1.0 | Yes | Yes | Yes | Yes |
| | CFDI messages/events: Issue, Cancel, Manual Cancel | Yes | Yes | Yes | Yes |
| | Printable CFDI version in PDF format that includes a bidimensional bar code | Yes | Yes | Yes | Yes |
| | XML viewer for issued CFDI invoices | Yes | Yes | Yes | Yes |
| | Automatic sending of CFD documents by email to customers during the posting process | Yes | Yes | Yes | Yes |
| PAC | Microsoft only provides a code example for PAC integration by using web server certificate authentication. | Yes | Yes | Yes | Yes |
| DIOT | Generation of the DIOT declaration as a text file | No | Yes | Yes | Yes |
| Adjustment inflation | Generation and registration of the Adjustment inflation process | No | Yes | Yes | Yes |
| ISR declaration | Generation of the ISR declaration report | No | Yes | Yes | Yes |
| VAT reports | Generation of VAT reports: Sales, Purchase | No | Yes | Yes | Yes |
| Electronic ledger accounting statement | Generation of electronic ledger accounting XML files: Chart of Account, Trial Balance, Ledger entries with related documents (folios), Auxiliary ledger account | Yes | Yes | Yes | Yes |
