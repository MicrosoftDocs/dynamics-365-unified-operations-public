...
Set up the types of tax-responsible entities that the company operates with. These entities can include customers, vendors, employees, and even the company itself.
 
## Prerequisites

Before you can set up taxpayer types for organizations in Latin America, make sure the following prerequisites are met:
...
- Tax ID types are configured.
- Document class letters are configured.
...
1. In the **Settings** section, set the options as required to complete the configuration according to the characteristics of the taxpayer type.
    ...
    | Internal taxpayer                       | Set this option to **Yes** if the taxpayer is internal and doesn't represent a real entity. No transactions that entities of this taxpayer type make are entered on tax reports. |
...
1. In the **Document types** section, complete the grid with **Tax Id types** that are associated with the taxpayer type according to the country or region's legislation.

    > [!NOTE]
    > This configuration and the tax ID type configuration in the country or region address book let you filter or directly assign the tax ID type in the customer or vendor record.
