...
- **Server token** – The secret token that is used to authorize the web application when requests are made to any application-restricted endpoint. (Server token is deprecated on HMRC side and not in use any longer.)
...
1. Follow the steps described in [Create a test user](https://developer.service.hmrc.gov.uk/api-test-user) section of HMRC portal.
2. Test user generated in [Create a test user](https://developer.service.hmrc.gov.uk/api-test-user) section contains information about **UserID**, **Password**, and **Making Tax Digital Income Tax ID** fields, and their respective values.
...
4. Store somewhere **UserID**, **Password** values related to the create test user. You will need these values during next steps of the authorization process.
...
6. On the **Sign in** page, enter the **UserID** and **Password** values from the created test user. The next page shows the authorization code that HMRC granted. 
