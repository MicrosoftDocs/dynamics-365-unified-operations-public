...
3. Select **Enable** to activate the feature. For more information about feature management and the available options, see [Feature management overview](../../fin-ops-core/fin-ops/get-started/feature-management/feature-management-overview.md) article.
...
3. Enter or update ZIP, Street, City, District, State and County
   ...
   > [!NOTE]
   > Please note that States should have State codes assigned in Organization administration > Setup > Addresses > Address setup > State/province tab.
...
3. Expand Contact information fast tab in Customers form to add Primary phone and Primary email
   ...
   > [!NOTE] 
   > Customer contact details might not be necessary for some versions of Dynamics 365 Finance.
...
2. Check the eInvoice checkbox for a customer
   ...
   > [!NOTE]
   > If this option is set to Yes, the system will be marking invoices posted for this customer account as ready for electronic invoice processing.
...
3. Use Run processing function to create electronic invoices for all relevant posted documents and send them to IRP
  ...
  > [!NOTE]
  > You can use Run in the background option to set up a batch processing to send all appropriate newly posted documents to IRP based on predefined time interval. 
...
6. Use Run processing function to cancel all electronic invoices selected for cancellation
	...
	> [!NOTE]
	> You can use Run in the background option to set up a batch processing to send all appropriate newly posted documents to IRP based on predefined time interval. Once operation is completed you can see information about the number of processed documents in the action log pane.
...
The E-Invoice page, Overview tab shows statuses of electronic invoice. Here are the possible statuses the electronic invoice can get:
...
After the E-Way bill for the invoice is entered, the E-Way bill status will be displayed as **Created** on the **E-Invoice** page.
...
### Where can I find detailed information about what went wrong for an invoice which was not successfully registered?
...
| Issue | Solution |
|-------|----------|
| Unable to post project invoice. When the “Validate before posting” checkbox is turned on and user is trying to post project invoice proposal, the system is throwing a validation error "Document Details: Document number cannot be empty."| Go to Tax - Setup - Tax configuration - Tax setup. Select the company and click Parameters. Set Tax document posting mode to "Synchronous".
Action: Generate SalesInvoice/ProjInvoice/CustomsShippingBillJour/TransferOrder EM |
| Cannot find 'StateCode_IN' field in 'LogisticsAddressState' table during the e-invoice generation in EM | Disable the feature “Minimize memory consumption by storing datasets at ER reports runtime” in feature management.  |
| Error generating token request | Make sure the GER configurations, e-Invoice parameters or KeyVault setup does not have missing setup. If the error contains "Function DigitalCertificateManager::findCertificateBySubject has been incorrectly called" then the Certificate is set up incorrectly. Download a certificate from IRP again and add it to the Azure KeyVault as a secret by copying entire file content. Example of a proper setup: <br> ![Item setup page.](media/apac-ind-einvoicesetup.png) |
...
| 2150: Duplicate IRN | This shouldn't happen in production. The issue appears when user is using the same credentials on different environments and the same invoice numbers are sent to the IRP. Go to Tax - Setup - GST reference number sequence group and set up a unique number sequence for the AR GST invoice. |
| Other types of errors. Example: "The field HSN Code must be a string with a minimum length of 6 and a maximum length of 8." or "Recipient PIN code should be 999999 for Direct Export" | Read the error code and correct the master data. If something is wrong in the customer data or company data, it can be corrected. Correct it, then select an EM with an error, delete the corresponding EM item and run the processing again. It will create a new EM with a fixed JSON. If something is wrong in the transaction data (invoice lines or tax transactions), it can't be fixed easily. User should revert invoice, fix the master data and try to create a new invoice. |
