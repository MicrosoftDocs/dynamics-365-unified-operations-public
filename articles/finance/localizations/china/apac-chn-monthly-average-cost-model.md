...
You can perform inventory costing by using the following standard models:
...
In addition to the standard inventory costing methods, companies in China widely use the monthly average cost inventory model to evaluate inventory cost. The monthly average cost inventory model is based on the weighted average principle. This model is like the weighted average date model, except that the value is calculated by month instead of by date. The monthly average cost model is an alternative to the weighted average date and standard cost models. By using the monthly average cost model, you calculate and adjust the average costs for each month; therefore, the cost figures are closer to the actual prices than when using the standard cost model. The following table lists specific differences between the weighted average date model and the monthly average cost model.
