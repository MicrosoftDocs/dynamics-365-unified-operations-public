...
For each depreciation method, calculate the depreciation amount based on calendar days in each period, as required by the Hungarian Company taxation law. The corresponding depreciation profiles disable depreciation conventions.

## Depreciation method: Straight line (Hungary)

When you set up a fixed asset depreciation profile and select **Straight line (Hungary)** in the **Depreciation method** field, you depreciate assets based on the total service life of the asset and the actual number of days in each period.
...
When you set up a fixed asset depreciation profile and select **Multiplication Factor** in the **Depreciation method** field, you depreciate assets that you assign the depreciation profile to based on individual percentages for each year. Set up these percentages on the **Manual schedules** FastTab. The sum of the percentages must be 100.
...
When you set up a fixed asset depreciation profile and select **Factor (Hungary)** in the **Depreciation method** field, you depreciate assets assigned to the depreciation profile by using a single percentage that you enter in the **Factor** field.
...
The yearly depreciation amount is based on the factor percentage: Depreciation amount = Acquisition cost × Factor (percentage). The following table shows the calculation results for the Factor (Hungary) depreciation method when you set the period frequency to **Yearly**.
...
If you set the period frequency to a unit that's smaller than a **Yearly** (for example, **Half-Yearly**, **Quarterly**, or **Monthly**), the yearly depreciation amount is distributed to each period. The distribution is based on the total number of days for the year (365 or 366) and the length of the period. The following table shows the calculation results for the Factor (Hungary) depreciation method when you set the period frequency to **Half-Yearly**.
...
When you set up a fixed asset depreciation profile and select **Sum of years’ digits** in the **Depreciation method** field, you depreciate assets based on the acquisition price and a percentage that decreases each year. The percentage depends on the number of service life years.
