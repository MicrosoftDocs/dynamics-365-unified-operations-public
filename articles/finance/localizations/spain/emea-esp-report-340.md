...
description: Learn how to set up and generate Report 340 for Spain in Microsoft Dynamics 365 Finance.
author: liza-golub
ms.author: egolub
ms.topic: how-to
ms.custom: 
...
1. On the **Setup** FastTab, select **Add**, and then set the following fields to set up the sales tax codes that should be included in the VAT book.
    ...
    | Non-Deductible VAT      | Select the checkbox to activate non-deductible VAT for the sales tax code. If the VAT amount is non-deductible, purchasers aren't allowed to deduct it.             |
    | Reverse Charge          | Select the checkbox to activate reverse charges for the sales tax code. Reverse charges are part of the VAT law. In some cases, goods or services are purchased from a foreign company. When reverse charges are activated, the VAT on these goods and services is payable by the recipient company, not by the foreign seller company. |

1. Select **Spanish VAT reports** to open the **Spanish VAT reports** page.
1. Select **Create new** to create a report.
1. In the **Spanish VAT list** dialog, set the following fields.
...
1. On the **General** tab, set the following fields.
    ...
    | Field                                | Description          |
    |--------------------------------------|---------------------------------------------------------------------|
    | Presentation type                  | Select the type of media to use for the exported file:<br>- **Telematic**: Upload the report to the tax authorities' website, or submit the report by using the free software provided by the tax authorities.<br>- **CD-R**: Send the report to the tax authorities on a CD-ROM. |
...
1. On the **Spanish VAT reports** page, select **Totals** to open the **Totals** page. On this page, you can view the following values:
...
1. Select **Output \> Export to ASCII file** to open the **Export to ASCII file** dialog.
1. In the **File name** field, enter a name for the file, and then select **OK**.
1. Select **Output \> Print** to open **Spanish VAT register book** dialog.
1. In the **Format mapping** field, select the **VAT register book (ES)** format that you downloaded earlier, and then select **OK**.
...
1. Set up sales tax. Learn more in [Sales tax overview](../../general-ledger/indirect-taxes-overview.md).
...
1. Go to **Tax \> Setup \> Sales tax \> Spanish VAT books**.
1. Select **New** to create a Spanish VAT book.
1. On the **Setup** FastTab, add the following sales tax codes:
    - EUP21
    - EUP-21
    - EUS
    - VAT 10
...
1. Select **Create new**.
1. In the **Spanish VAT list** dialog, set the following fields, and then select **OK**:
    - **VAT book:** ESP
