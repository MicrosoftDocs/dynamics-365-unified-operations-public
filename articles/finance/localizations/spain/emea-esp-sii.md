...
description: Learn how to set up and use Microsoft Dynamics 365 Finance to work with the Suministro Inmediato de Información del IVA (SII) system of Spain.
author: ankviklis
ms.author: ankviklis
ms.topic: how-to
ms.date: 07/11/2025
...
According to R.D. 596/2016 in Spain, a value-added tax (VAT) management system that's based on the SII system allows for a two-way, automated relationship between the Spanish Tax Agency (La Agencia Estatal de Administración Tributaria [AEAT]) and the taxpayer. Taxpayers who are subject to SII, and others who voluntarily adopt it, must send details of their billing records within four days through online filing on the AEAT website. Learn more in [Immediate Supply of Information on VAT (SII) official website](https://sede.agenciatributaria.gob.es/Sede/iva/suministro-inmediato-informacion.html).

Dynamics 365 Finance supports the full cycle of processing, including the format generation and submission of the following reports to the SII system of Spain:
...
To prepare Finance to work with the SII system, you must import the following ER configurations.
...
Electronic message functionality is provided to maintain the different processes that are used in electronic reporting for different document types. Learn more in [Electronic messaging](../../general-ledger/electronic-messaging.md).
...
1. Before the data entities are used for the first time to import the data from the package, sync the mapping of the source data and target data. In the list for the package, select a data entity, and then, on the Action Pane, select **Modify target mapping**.
1. Above the grid for the package, select **Generate mapping** to create a mapping from scratch, and then save the mapping.
1. Repeat steps 8 and 9 for every data entity in the package before you start the import. For more information about data management, see [Data management overview](../../../fin-ops-core/dev-itpro/data-entities/data-entities-data-packages.md).
1. Import data from the **ES SII setup.zip** file into the selected company.
1. In the **Data management** workspace, select **Import**.
1. Specify a **Group name**.
1. Select **Add file**.
1. In the drop-down dialog, set the **Source data format** field to **Package**, and then set the **Source data format** field to **Package**.
...
After the data entities are imported, you can see two types of electronic message processing: **SII** and **CollectionInCash**. This processing contains almost all the setup required for your legal entity.
...
| OperacionesIntracomunitarias      | **Intra-community operations:** Message items of this type are created for existing vendor invoices that meet specific intra-community criteria. Learn more in [Algorithm to define the TipoOperacion (Intra-community operation type) additional field](#algorithm-to-define-the-tipooperacion-intra-community-operation-type-additional-field). | SII              |
...
1. If you set the **Use advanced certificate store** option to **Yes**, go to **System administration \> Setup \> Key Vault parameters** to set up the Key Vault parameters.
1. Go to **Tax** \> **Setup** \> **Parameters** \> **Electronic messages** \> **Web service settings**.
1. Enter the following information to define the internet address for web services.
...
1. Set up number sequences for electronic messages.
1. Set up security roles for electronic message processing.

## Set up executable class parameters

Three executable classes are included in the two types of electronic messages processing (**SII** and **CollectionInCash**) that are used to work with the SII system and imported into the system by using a package of data entities.
...
1. On the Action Pane, select **Parameters**, and then, in the **Add new electronic message items** dialog that appears, set the following values for the parameters of the executable class.
    ...
    If you're using the [Tax Calculation](../global/global-tax-calcuation-service-overview.md) service, and the [Support multiple VAT registration numbers](../global/emea-multiple-vat-registration-numbers.md) feature is enabled in the **Feature management** workspace, an additional **Multiple Tax ID parameters** group is visible in the **Add new electronic message items** dialog. Set the following values for the parameters in this group.
...
1. Expand the **Records to include** FastTab and specify additional criteria to be applied to the datasources of Finance from which invoices will be populated in EM items. The following datasources are available:
...
1. On the Action Pane, select **Parameters**, and then, in the dialog that appears, set the following values for the parameters of the executable class.
...
1. If the invoice type isn't defined in step 1, check whether the value is **F4** (summarized invoice).
1. If the invoice type isn't defined in step 2, define it according to the setup of additional field values.
...
1. In the **Invoice type** column, select the code that is applicable to the SII system.
1. In the **SII code description** column, add a description. This description won't be used in reporting, but it might help users select the appropriate financial reason.
...
1. In Dynamics 365 Finance, go to **Tax \> Setup \> Electronic messages \> Additional fields**.
1. In the list of additional fields on the left, select **TipoFactura**.
1. On the **Value** FastTab, specify criteria:
...
1. In Dynamics 365 Finance, go to **Tax \> Setup \> Electronic messages \> Additional fields**.
1. In the list of additional fields on the left, select **TipoOperacion**.
1. On the **Value** FastTab, specify criteria:
    - **Account type**: Select **Customer** or **Vendor**.
...
| 14              | Invoice with VAT pending accrual on work certifications with Public Administration recipients for the first half of 201.      |
...
The following algorithm is used to search for the special scheme code for the register consists of three stages:

1. Search among records where **Account code** is set to **Table** for the customer or vendor.
1. If the record isn't found in stage 1, search among records where **Account code** is set to **Group** for the customer or vendor.
1. If the record isn't found in stage 2, search among records where **Account code** is set to **All**.
...
1. If no record is found in substage 1, search for a record where the value of the **Sales tax group** matches the sales tax group that was passed as a parameter, but the **Item sales tax group** field is blank. (A blank field represents "any.")
1. If no record is found in substage 2, search for a record where the **Sales tax group** field is blank, but the value of the **Item sales tax group** field matches the item sales tax group that was passed as a parameter.
1. If no record is found in substage 3, search for a record where both the **Sales tax group** field and the **Item sales tax group** fields are blank.
...
1. In the left pane, expand the **Invoices Communication Model** node.
1. Select the format configurations **SII Invoice Issued Format (ES)** with a version higher than 108.54 or **SII Invoice Received Format (ES)** with a version higher than 108.47.
1. Select **Configurations** \> **Application specific parameters** \> **Setup**
1. Select **ExemptionCauseTypeLookUp**
1. On the **Conditions** tab, select **Add**.
1. In the **Lookup result** field, select the applicable exemption type.
1. In the **Tax exempt code** field, enter the tax exempt code for each exemption type.
...
- The **Intra-community** checkbox is selected for the sales tax group (**Tax \> Sales tax \> Sales tax group**) that is used in the sales tax transaction that is related to the invoice. Verify that the [Intra-community VAT for Spain](emea-esp-intra-community-vat.md) feature is set up and used in your system.
...
1. On the Action Pane, select **Parameters**, and then set up the **TipoOperacion** value for the **Intra-community operation ID** additional field.
...
1. For each sales tax code that is granted tax exemptions, on the **Setup** tab, select a **Sales tax code**.
1. In the **Exempt code** column, enter the **Exemption code type**. The code type must be the same code that you entered in the **ExemptionCauseType** lookup in the [Set up Exempt code types on Issued and Received Invoice formats](#exempt) section.
1. Select **Save**.
...
## <a name="multiple-vat"></a>Report to the SII system of Spain for multiple VAT registrations
...
1. On the **Tax calculation parameters** page, on the **Multiple VAT registrations** tab, select the **VAT declaration** checkbox.
1. Define the VAT registration number of the company from the name that you will be reporting to SII system of Spain in the **Company Tax Id** field in the [**SIIGenerateItems executable class**](#siigenerateitems) parameters.
1. Specify the **Transfer order history** data source on **Records to include** FastTab of the [**SIIGenerateItems executable class**](#siigenerateitems) parameters.
...
The following table describes the manual user actions that are included in **SII** processing to help you manage the reporting of specific messages and EM items via their status change.
...
1. In the dialog, in the **Processing** field, select **SII**.
...
1. In the dialog, in the **Processing** field, select **SII**.
1. If you want to run all the possible actions for **SII** processing, clear the **Choose action** checkbox. If you want to run only a specific action, select the **Choose action** checkbox, and then, in the **Action** field, select the action to run.
1. Define the parameters of the batch processing on the **Run in the background** FastTab in the **Run processing** dialog. For more information about batch processing, see [Batch processing overview](../../../fin-ops-core/dev-itpro/sysadmin/batch-processing-overview.md).
...
1. In the **Update status** dialog, select **SII** processing. Then, in the **Action** field, select **ExcludeMessageItem**.
1. In the **New status** field, select **Excluido**.
1. On the **Records to include** FastTab, define additional criteria to specify which invoices must be excluded from further processing.
...
1. In the **Fiscal year** field, enter the year to report collections in cash for.
1. Set the **Minimum amount of payments in cash** field, and then select **OK** to generate the report.
...
1. In Dynamics 365 Finance, go to **Tax \> Inquiries and reports \> Electronic messages \> Electronic message items**.
1. On the Action Pane, select **Run processing**.
1. In the dialog, in the **Processing** field, select **CollectionInCash**.
1. If you want to run all possible actions for **CollectionInCash** processing, clear the **Choose action** checkbox. If you want to run only a specific action, select the **Choose action** checkbox, and then, in the **Action** field, select the action to run.
...
1. In the **Update status** dialog, select **CollectionInCash** processing. Then, in the **Action** field, select **Exclude message item**.
1. In the **New status** field, select **Excluido**.
1. On the **Records to include** FastTab, define additional criteria to specify which EM items must be excluded from further processing.
