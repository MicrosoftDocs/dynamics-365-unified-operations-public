...
You can enable the GST/SST features only if the country/region of the legal entity's address is Malaysia.

### GST/SST registration number

The GST/SST registration number is printed on your tax invoices and on some reports. You maintain it in the address information for your legal entity. You must first define a registration type on the **Tax registration type** page. Make sure that the country/region code is **MYS**, and that the **Primary for country** check box is selected. Then, in the legal entity's primary address, add the tax registration number for GST/SST. Select the tax registration type that you defined, and then enter your GST/SST registration number.
