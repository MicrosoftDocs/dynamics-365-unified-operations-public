...
# Make payments to self-employed persons - RF-1321
...
1. In the list of data package files, find **NO RF-1321 EM setup v.N** package (where "N" is the version of the package), and download it to your computer. We recommend that you download the latest available version of the package.
...
1. In the **Data management** workspace, select **Import**, and then, on the **Import** FastTab, set the **Group name** field.
...
1. Find and select the **NO RF-1321 EM setup v.N** setup file that you previously downloaded.
...
1. Before the data entities are used for the first time to import the data from the package, sync the mapping of the source data and the target data. In the list for the package, select a data entity, and then, on the Action Pane, select **Modify target mapping**. 
1. Above the grid for the package, select **Generate mapping** to create a mapping from scratch. Then save the mapping.
1. Repeat steps 11 and 12 for every data entity in the package before you start the import.
1. You can now import data from the **NO RF-1321 EM setup v.N** setup file into the selected company. On the Action Pane, select **Import** to start the import.
...
To work with the Electronic messages functionality, you must define the related number sequences. Follow these steps.
...
To specify which ER format is used in **RF-1321** processing, follow these steps.
...
1. Select the **Generating RF-1321** executable class (which is set to call **EMRF1321PayToSelfEmployedVendorController_NO**), and then, on the Action Pane, select **Parameters**.
1. In the **RF-1321 Payments to self-employed vendors without fixed place of business** dialog box, select **Posting profile** is necessary and select **OK**.
...
1. Expand **Bank account information** FastTab.
1. In the **Routing number** specify the organization number that is reported in `<organisasjonsnummer>` field.
...
1. In the **Run processing** dialog box, select **OK**.
1. A new electronic message is created. Enter a description, and specify the start and end dates of the period that you want to generate the RF-1321 report for.

### Generate an XML file in RF-1321 format

1. On the **Electronic messages** page, on the **Messages** FastTab, select **Generate report**.
1. In the **Run processing** dialog box, in the **Action** field, select **Generate file**.
1. To run report generation in a batch, enable the **Batch processing** ckeckbox on the **Run in the background** FastTab and specify the batch parameters.
1. In the interactive mode (the **Batch processing** ckeckbox is **Off**), the report parameters are available on the next report dialog. You can select a **Posting profile** or specify other filters using **Records to include**.
