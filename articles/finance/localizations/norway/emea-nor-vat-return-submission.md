...
description: Learn how to submit a VAT return to the Altinn web service of Norway in Microsoft Dynamics 365 Finance.
author: liza-golub
ms.author: egolub
ms.topic: how-to
ms.custom: 
...
:::image type="content" source="../media/emea-nor-vat-return-em-processing.png" alt-text="Screenshot of the EM processing schema delivered with the NO VAT return – Altinn setup file.":::
...
1. [Collect sales tax payments](#collect-sales-tax-payments).
1. [Mark the message as ready to generate the VAT return](#ready-to-generate).
1. [Preview the VAT return in Microsoft Excel](#preview-vat-return).
1. [Generate the VAT return](#generate-vat-return).
1. [Validate the VAT return in the Tax Administration web service](#validate-vat-return).
1. [Submit the VAT return](#submit-vat-return).
1. [Download attachments](#download-attachments).
...
1. Select **OK**. The new electronic message is created.
1. In the **From date** and **To date** fields for the message, define the period that you want to submit the VAT return for.
1. Optional: In the **Description** field, enter a description. This text won't be included on the report.

:::image type="content" source="../media/emea-nor-vat-return-create-message.png" alt-text="Screenshot of creating a message.":::
...
1. Select **OK**.
1. The sales tax payment lines are posted during the period that was specified for the message that is listed on the **Message items** FastTab in relation to the selected message.

:::image type="content" source="../media/emea-nor-vat-return-collect-sales-tax-payments.png" alt-text="Screenshot of collecting sales tax payments.":::
...
1. On the **Message additional fields** FastTab, select the **NO VAT Note for VAT return** additional field.
1. In the **Field value** column, select a value in the lookup field.

To add a free-text note that is limited to 4,000 characters, follow these steps:

1. On the **Messages** FastTab, select the electronic message to specify a note for.
1. On the **Message additional fields** FastTab, select the **NO VAT Note for VAT return** additional field.
1. In the **Field value** column, select **fritekstnotat** in the lookup field.
1. Select **Attachments**, and then, on the Action Pane, select **New** \> **Note**.
1. In the **Note** field, enter your note. This note will be included on the VAT return in XML format when it's generated.
...
1. On the **Message additional fields** FastTab, select the **NO VAT Payment ID** additional field.
1. In the **Field value** column, specify the KID number.
...
1. Select **OK**. The status of the message is updated to **NO VAT Ready to generate VAT return**, and the **Generate report** button becomes available.

:::image type="content" source="../media/emea-nor-vat-return-ready-to-generate.png" alt-text="Screenshot of marking a message as ready to generate a VAT return.":::
...
1. In Dynamics 365 Finance, go to **Tax** \> **Inquiries and reports** \> **Electronic messages** \> **Electronic messages**, and select the message that was created in the **NO VAT return** EM processing.
1. On the **Message** tab, select **Generate report**.
1. In the **Run processing** dialog box, select **NO VAT Preview VAT return in Excel**.
1. For reporting periods that contain a large volume of tax transactions, we recommend that you run the report in batch mode. On the **Run in the background** FastTab, select the **Batch processing** checkbox. Then set the **Task description** field and other parameters for the batch.
1. Select **OK**. A new file is attached to the message, but the status of the message isn't changed.
1. Select **Attachments**, and then select **VAT return preview.xls**.
1. On the Action Pane, select **Open** to preview the VAT return in Excel format.
...
1. In the **Run processing** dialog box, select **NO VAT Generate VAT return**.
1. For reporting periods that contain a large volume of tax transactions, we recommend that you run the report in batch mode. On the **Run in the background** FastTab, select the **Batch processing** checkbox. Then set the **Task description** field and other parameters for the batch.
1. Select **OK**.

    :::image type="content" source="../media/emea-nor-vat-return-generate-vat-return.png" alt-text="Screenshot of generating a VAT return.":::
...
1. Select **Attachments**, and then select **mvamelding.xml**.
1. On the Action Pane, select **Open** to preview the VAT return in XML format.
...
1. To review the information about any errors that were identified during the business validation that the Tax Administration performs, on the **Action log** FastTab, select the line for **NO VAT Import validation response**.
1. Select **Attachments**, and then select the attached HTML file.
1. On the Action Pane, select **Open**. The file is opened in a browser window, and you can review the content in a table. After you apply corrections, you can go back to the [Generate the VAT return](#generate-vat-return) step to regenerate the XML file.
...
1. Select **OK**. The VAT return submission is complete when the electronic message has a status of **NO VAT SUCCESSFUL VAT return submission to the Tax Administr**.
...
1. In Dynamics 365 Finance, go to **Tax** \> **Inquiries and reports** \> **Electronic messages** \> **Electronic messages**, and select the message that was created in the **NO VAT return** EM processing.
1. On the **Messages** FastTab, select **Send report**.
1. In the **Run processing** dialog box, select **NO VAT Download receipt** or **NO VAT Download payment information**.
1. Select **OK**. The `betalingsinformasjon.xml` ("NO VAT Download payment information") or `kvittering.pdf` ("NO VAT Download receipt") file is attached to the electronic message.
1. Select **Attachments**, and then select **betalingsinformasjon.xml** or **kvittering.pdf**.
1. On the Action Pane, select **Open**.
