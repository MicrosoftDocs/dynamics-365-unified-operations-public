...
A payment identification (ID) is a unique identifier for customer payments that you settle electronically. You can divide the payment ID into different parts, such as the customer account number, invoice number, prefix, suffix, and external reference. When you receive a payment from a customer, the payment ID identifies the payment transaction for a sales invoice that the bank sends.
...
1. Select the **Delete payment ID** check box to delete the payment ID information from all documents.
    ...
    > [!NOTE]
    > Use this option to remove or update Payment IDs for documents that have a Payment ID assigned. You can delete the Payment ID from specific types of documents.  
