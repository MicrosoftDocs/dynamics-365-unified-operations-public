...
description: Learn how to set up Intrastat parameters and transfer transactions to Intrastat in Microsoft Dynamics 365 Finance.
author: liza-golub
ms.author: egolub
ms.topic: how-to
ms.custom: 
...
1. In the **Name** field, enter the name of the commodity. For example, enter "Other commodity".
...
To assign a commodity code to a product hierarchy and released product, follow these steps:
...
1. In the list, select the released product that you want to assign to the commodity code, and then select **Edit**. 
...
1. In the **Statistical amount** field, select an option. For example, select **Empty**, which indicates that the statistical value to report for transactions with a transaction code of "21" is always zero.  
1. Go to **Tax** > **Setup** > **Foreign trade** > **Foreign trade parameters**.
1. In the **Transaction code** field, enter or select a value. 
1. In the **Credit note** field, enter or select a value. This value also identifies the physical return. The credit note for the physical return transfers in the Intrastat journal in the opposite direction. For example, the return of arrival transfers as dispatch. Otherwise, the credit note is considered a correction and transfers with the same direction and opposite sign. For example, the correction of arrival transfers as an arrival with a negative amount and the active flag is set to **Correction**.  
1. Expand the **Transfer** FastTab and in the **Items with commodity code** field, select **Yes**. Select this option to transfer only the transactions with a commodity code assigned. Transactions without a commodity code don't transfer to Intrastat.  
...
1. Expand the **Foreign trade** FastTab, and then select **Edit**.
1. In the **Intrastat invoice value** field, select **Yes**. The amount is transferred to the **Invoice charges** field and is summarized with the amount transferred in the **Invoice amount** field. If you need to transfer the amount of changes to the **Statistical charges** field and summarize by using the **Statistical amount** field, in the **Intrastat statistical value** field, select **Yes**.  
...
1. Expand the **Line details** FastTab. On the **Foreign trade** tab, you see the transport is filled automatically from the selected mode of delivery.  
...
1. Select the **Filter** button near the bottom of the pane. In the list, mark the row with **Date**.
