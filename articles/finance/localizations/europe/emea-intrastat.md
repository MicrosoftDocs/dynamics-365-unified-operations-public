...
| Intrastat \> Exchange rate | Select the exchange rate type for Intrastat reporting. <br> The **Exchange rate type** parameter is globally available as of 10.0.46 version of Finance. <br> When [Multiple VAT registration numbers](../global/emea-multiple-vat-registration-numbers.md) is **enabled**: <li> If the **Exchange rate type** parameter is **not specified**, the system applies the standard algorithm: it first recalculates from the Transaction currency to the Accounting currency, and then to the Intrastat currency using the default exchange rate.</li> <li> If an **Exchange rate type** parameter is **specified**, the system uses this **Exchange rate type** to directly calculate amounts to the Intrastat currency.</li> When [Multiple VAT registration numbers](../global/emea-multiple-vat-registration-numbers.md) is **disabled**: <li> If the **Exchange rate type** parameter is **not specified**, the system directly calculates the Intrastat amounts using default exchange rate to the Accounting currency.</li><li> If an **Exchange rate type** parameter is **specified**, the system uses this **Exchange rate type** to directly calculate amounts in Intrastat to the Accounting currency.</li> |
...
For general information about multiple VAT registration numbers, see [Multiple VAT registration numbers](../global/emea-multiple-vat-registration-numbers.md).
