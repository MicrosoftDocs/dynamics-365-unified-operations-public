...
This article explains the level of coverage that exists for electronic invoicing in Europe.

Council Directive 2010/45/EU regulates the implementation and adoption of European Union-wide electronic invoicing and affects all EU member states. Companies that want to benefit from electronic invoicing must submit sales order invoices, free text invoices, project invoices, sales order credit notes, and project invoice credit notes as .xml files to the government or other trading parties that mandate use of electronic invoicing. These .xml files must comply with certain standards. The country/region-specific requirements and their implementation may differ across EU member states but commonly they use Universal Business Language ([UBL](https://www.oasis-open.org/committees/tc_home.php?wg_abbrev=ubl)) in different versions with customizations as well as [PEPPOL](https://docs.peppol.eu/poacc/billing/3.0/) specifications and access points for validation and transportation. The primary advantage of UBL is that business documents can be standardized for different purposes. Because UBL is a flexible, international standard that supports many business requirements, these business documents can be exchanged across national and regional borders.
...
Electronic invoicing is based on [Electronic reporting (ER)](../../../fin-ops-core/dev-itpro/analytics/general-electronic-reporting.md). An **Invoice model** data model, invoice model mapping, and several country/region-specific ER format configurations are created for the following countries/regions:
...
The electronic invoices and credit notes that you generate include required information, such as a European Article Numbering (EAN) number, contact person, dimension account number, and address information for the customer. Validation rules are applied when you generate invoices so you can verify that the correct information is entered. The set of required information might differ from country/region to country/region. Because the requirements, as well as supported countries/regions and formats, are subject to change, always go to the Shared asset library on Microsoft Dynamics Lifecycle Services (LCS) and view the most up-to-date list of available files that have an asset type of **GER configuration**.

## Electronic invoice configuration

The setup and specifics of electronic invoices depend on the country or region that you implement them for. For more information about how to set up and use customer electronic invoices, see the related country or region-specific topics:
...
> [!NOTE]
> Although these Task guides were created for Danish-specific e-invoice format *OIOUBL*, they are applicable for other supported countries or regions with minor deviations.
