...
description: Learn how to set up and post fixed asset preacquisitions with Microsoft Dynamics 365 Finance.
...
> [!NOTE]
> Legal entities that have their primary address in Hungary or the Czech Republic are the only ones that can use the functionality for posting fixed asset preacquisitions. A preacquisition of a fixed asset isn't depreciable, and it doesn't affect the acquisition costs or the net book value of the fixed asset. When you post a preacquisition, the status of the fixed asset changes to **Acquired**. A fixed asset that has the **Acquired** status isn't depreciable. By contrast, when you post an acquisition, the status changes to **Open**, and the fixed asset is depreciable.

## Set up preacquisitions

Before you can post a preacquisition, complete the following steps:
...
1. On the **Fixed assets** page, create a new journal, and enter all required information.
...
1. Select **Post** > **Post** to post the preacquisition transaction. On the **Books** page, the status of the fixed asset is now **Acquired**.
