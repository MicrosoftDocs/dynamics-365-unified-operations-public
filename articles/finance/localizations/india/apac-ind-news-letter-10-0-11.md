...
This article summarizes the new features and critical bug fixes released in Dynamics 365 Finance version 10.0.11 for India GST localization.

## New features

### GST tax settlement as per new settlement rules (vide Circular No. 98/17)

Based on the amended GST law, maintain the following sequence.
...
:::image type="content" source="../media/GST-tax-settlement-new-rule-1-10-0-11.PNG" alt-text="Screenshot of Setoff rules for sales tax hierarchies page.":::

The tax settlement hierarchy now includes a new tax settlement change. You can define the tax set off hierarchy for a tax component with any  tax settlement priority. Previously, the system settled each tax component's recoverable amount against the tax payable amount of its own tax component and then with other tax components. Going forward, you can define the set off priority of a tax component with any other tax component. For example, set up a settlement priority of the input tax credit for CGST against IGST first, and then with CGST. Similarly, SGST can be set off against IGST first, and then SGST.

## Data entity for stock transfer order transaction

To support large stock transfer orders, you can now upload stock transfer transactions through the data entity by using data management.  
...
- Tax information fields don't update automatically when a user copies lines from an original free text invoice.
...
- Removed and inactive addresses appear under **Location** on the **Tax information** page for invoices that
...
- You can delete the source details in the GST number sequence group even when the details are attached to a transaction.
- Charges aren't automatically updated in the **Assessable value** field on the sales order line when charges are applied using the **Auto charges** functionality. If the **Assessable value** check box is marked when setting up auto charge,
  charges aren't automatically updated on the sales order line to include charge amount in the assessable value of goods.  
- Importing the **General journal** lines with tax by using the data import/export feature seems successful, but when verifying
  the tax document there's no tax information for the imported **General journal** transactions.
