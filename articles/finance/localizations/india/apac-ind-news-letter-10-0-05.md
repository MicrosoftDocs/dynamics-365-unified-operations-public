...
description: Learn about new or changed functionality for India GST features released in Dynamics 365 Finance version 10.0.05, including outlines on new configurations.
author: prabhatb
...
This article summarizes the new features and critical bug fixes released in Dynamics 365 Finance version 10.0.05 for India GST localization.

## New configuration

The Shared Asset Library in Lifecycle Services offers the following configurations for version 10.0.5. For more information about the released configuration details, see [India Goods and Services Tax (GST) overview](apac-ind-gst.md).

- Taxable Document.version.82.xml
- Taxable Document (India).version.82.143.xml
- Tax (India GST).version.82.143.263.xml

 These configurations also include the following features or fixes:

- Resolved the issue of zero tax base for tax exempt transactions.
- Support for CGST and SGST for intra-state stock transfer orders between warehouses with different GST registrations.
- Support for VAT for non-GST items.
- Enable tax rate type.

In most taxation systems, there's a concept of tax rate type, similar to standard tax rate, reduced tax rate, and super reduced tax rate. In India GST, there are five slabs.
...
:::image type="content" source="../media/GST-tax-rate-type-1-10-0-05.png" alt-text="Screenshot of Released product details showing Tax rate type.":::

Use the Tax rate type to determine the tax rate which dramatically reduces the number of tax setup data records.

:::image type="content" source="../media/GST-tax-rate-type-2-10-0-05.png" alt-text="Screenshot of Tax rate type lookup.":::

For Retail customers whose terminals aren't updated to 10.0.5, use the following configurations. The only difference is that these configurations don't support tax rate types.
...
This feature was introduced in version 10.0.2, but more details are now available. If you're using Microsoft's standard configuration, or you extended only the tax document, you can skip model mapping to improve the tax calculation and posting performance.

:::image type="content" source="../media/GST-skip-model-mapping-3-10-0-05.PNG" alt-text="Screenshot of Tax configuration showing Skip model mapping option.":::

### Support calculate/adjust tax in accounting currency

This feature was introduced in version 10.0.3, but more details are now available. You can adjust the actual tax amount in the accounting currency for import/export orders to mitigate the difference for customs duty.
...
:::image type="content" source="../media/GST-skip-model-mapping-4-10-0-05.PNG" alt-text="Screenshot of Tax setup parameters showing Calculation method field group.":::

:::image type="content" source="../media/GST-skip-model-mapping-5-10-0-05.png" alt-text="Screenshot of Tax document totals.":::
...
1. In the **Reference** field, select the financial dimension to link to the site inventory dimension.
1. Select **Sites** to open the **Sites** page. The financial dimension selected in the **Reference** field is displayed
  on the **Financial dimensions** FastTab, where you can also select a financial dimension value for the dimension.
1. Select a site, and on the **Financial dimensions** FastTab, select a financial dimension value. The specified financial
  dimension value is associated with the selected site.
1. Repeat step 4 for each site.
1. When you have finished with the sites, go to **Inventory management** > **Outbound Orders** > **Transfer Order** > **Stock transfer order** and create a new stock transfer order.
1. Select a stock order line, and on the **Financial dimension** tab, select **Line details** > **Default financial dimension**.
...
:::image type="content" source="../media/GST-skip-model-mapping-6-10-0-05.png" alt-text="Screenshot of Tax document posting types.":::

:::image type="content" source="../media/GST-Reverse-charge-posting-7-10-0-05.PNG" alt-text="Screenshot of Tax document Details pane.":::

1. Identify the reverse charge liability in a separate account such as **Tax Payable (Reverse charge)**. This should be defined as a **Ledger account** instead of **Tax account** because reverse charge liability can't be directly settled with input tax credit. Instead, it must first be paid separately from the **Cash Ledger account**.
1. Set the following field information:
