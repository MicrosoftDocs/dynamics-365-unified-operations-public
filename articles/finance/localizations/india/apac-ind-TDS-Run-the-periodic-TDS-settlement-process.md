...
1. Go to **Tax \> Declarations \> Withholding tax \> Withholding tax payment**.
    ...
    :::image type="content" source="../media/apac-ind-TDS-47.png" alt-text="Screenshot of the Withholding tax payment dialog box.":::

1. In the **Withholding tax payment** dialog box, in the **Tax type** field, select **TDS**.
1. In the **Tax Account Number (TAN)** field, select the Tax Account Number (TAN) to run the settlement process for.
1. In the **Withholding tax component group** field, select the TDS component group to run the settlement process for.
1. In the **Settlement period** field, select the TDS settlement period to run the settlement process for.
...
1. In the **From date** field, select the start date to run the TDS settlement process from.
...
1. In the **Transaction date** field, select the date to post the TDS settlement transaction.
1. In the **Withholding tax payment version** field, select the TDS settlement version:
...
1. Select the **Update** check box to run the TDS settlement process and post the amounts to the ledger accounts. If this check box is cleared, the settlement process won't be run, and the financial entries won't be generated.
1. Select **OK** to run the TDS settlement process and generate the withholding tax payment report. The status of TDS transactions that are included in the settlement process is shown as **Settled** on the **Settlement** page (go to **Accounts payable \> Payments \> Vendor payment journal**, select **Lines**, select **Functions**, and then select **Settlement**).
