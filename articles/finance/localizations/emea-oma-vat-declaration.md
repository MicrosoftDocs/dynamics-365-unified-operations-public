...
- In the **Feature management** workspace, enable the following features:
	...
	- **Category hierarchy for Sales and purchase tax report**. This feature is optional if you want to use commodity code as goods and service description in VAT books.
...
In the **Electronic reporting** workspace, import the Electronic Reporting format, **VAT Return Filing Excel (OM)** from the repository.
 
> [!NOTE]
> The format above is based on **Tax declaration model** and uses the **Tax declaration model mapping**. These additional configurations are automatically imported.

After you finish downloading the ER configurations from Lifecycle Services (LCS) or the global repository, complete the remaining procedures in this topic.

 
### Set up application-specific parameters

The VAT declaration form includes a set of boxes (lines) that correspond to specific parts of the VAT return process. Each box should include information about the base, adjustment, and VAT amounts. To include the requirements established by the form, configure each box with the information that is automatically provided from the sales tax transactions generated from sales, purchases, or other operations where VAT tax is posted through the sales tax code configuration.

The Application-specific parameters option lets you establish the criteria of how the tax transactions will be collected and calculated in each box of the declaration form when the report is generated depending on the configuration of the sales tax code. The available criteria are:
...
**Box: 1a - Supplies of goods / services taxed at 5%**. As per legal definition, the value in this box should be the reported total value of standard rate supplies of goods and services in the Sultanate, including deemed supplies. Report the VAT-exclusive value only.

In Finance, and depending on the tax configuration, you can have a specific sales tax group code implemented that represents and calculates the operations at a standard sales rate. In this example, it's necessary to configure **Box 1a** as follows:
...
2. Select the current version, and on the **Lookups** FastTab, select the **VATBoxesLookup** lookup. This lookup identifies the list of boxes in the VAT form that are required by tax authority. 
...
4. In the **Sales tax group** column, select the sales tax group that is used to calculate the related line of VAT return form. For example, VAT_DOM.
5. In the **Item sales tax group** column, select **Not blank**. Box 1a will be identified by Sales tax group only.
6. In the **Tax code** column, select **Not blank**. Box 1a is going to be identified by Sales tax group only.
7. In the **Transaction classifier** column, select the tax transaction classification where the sales tax group code is used.
8. Repeat steps 3-5 for all VAT return form boxes and the combination of sales tax code and tax transaction types configured in your legal entity.
9. Select **Add**, and then follow these steps to include the final record line:
...
10. In the **State** field, select **Completed**, and then select **Save**. 
11. Close the **Application specific parameters** page.

The following table represent an example of how you need to configure these parameters to establish the configuration between the different boxes in the declaration form and sales tax code configuration implemented in Finance.
...
To avoid issues when the report is generated, create all mappings where the sales tax codes and sales tax group are posted. For example, if the line of **box 1a**  has SalesCreditNote omitted in this configuration, and tax transactions are posted by using sales tax group VAT_DOM, you will encounter issues when the report is generated. We recommend that you use the **Tax > Inquire > Posted sales tax** menu to review all posted sales tax transactions and those that aren't included in this mapping of the configuration.

The following table provides the available values in the **Transaction classifier** column. This information helps you to understand how the tax transactions are classified and assigned to the related sales tax code.
...
To generate the VAT return form report in Microsoft Excel, define an ER format on the **General ledger parameters** page.
