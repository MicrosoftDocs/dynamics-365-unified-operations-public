...
description: Learn how to work with partial fixed asset disassembly or liquidation for Russia in Microsoft Dynamics 365 Finance.
author: evgenypopov
...
1. On the **Partial dismantlement** page, set up the accounts for writing off the balance value, balance depreciation, net book value, and profit/loss.
...
1. Select the line for an item, and then select **Add to remains**.
1. in the **Add to remains** dialog, in the **Add** field, enter the remaining quantity.
...
1. In the lower pane, select **Calculating prices**. The balance value depreciation and depreciated cost are calculated for every component that is being written off.
1. In the **Calculating prices** dialog, in the **Calculating prices** field, select the method that is used to calculate prices:
...
1. If the cost must be rounded to an integer value, set the **Round off market value** option to **Yes**.
1. Select **OK**. The cost of the item is calculated, when the item return to inventory  You can manually update the market value as you require.
...
1. In the lower pane of the **Componentry** page, on the **Inventory dimensions** tab, select inventory dimensions that are used to post the item to inventory.
1. To create the write-off disassembly transaction, select **Fixed assets (Russia) \> Journals \> FA journal**.
1. Select **New** to create a journal.
1. Select **Lines** and then **New** to create lines to record the write-off disassembly transactions.
1. In the **Add to journal** dialog, in the **Transaction type** field, select **Partial dismantlement**.
1. In the **Transaction date** field, enter the transaction date.
1. Select **OK**. The partial fixed asset disassembly lines are created in the journal, according to the depreciation profile settings.

    If there no transaction details on the journal line, you must enter them manually.

1. On the Action Pane, select **Functions \> Show invisible** to show profit/loss lines.
1. On the Action Pane, select **Post \> Post**. Transactions are created in the ledger and fixed asset accounts, and the item is posted to inventory.

    The partial dismantlement transaction value is shown in the **Acquisition adjustment of partial dismantlement** and **Depreciation adjustment of partial dismantlement** fields in the **Balance by FA** dialog (**Fixed assets (Russia) \> Value models \> Balance**).
