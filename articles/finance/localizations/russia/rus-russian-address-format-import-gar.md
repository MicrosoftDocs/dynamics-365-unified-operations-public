...
description: Learn how to import addresses in the GAR format for Russia in Microsoft Dynamics 365 Finance.
author: evgenypopov
...
The GAR format provided by the Federal Tax Service (FTS) contains information about addresses in administrative-territorial and municipal divisions. Current GAR functionality enables the import of addresses in the municipal division only.
...
1. Import configurations for the GAR MUN hierarchy file:
    - GAR hierarchy import MUN\_HIERARCHY(RU)
1. Import configurations for GAR address files:
...
1. Import configurations for GAR address parameters files:
...
Learn more in [Download ER configurations](../../../fin-ops-core/dev-itpro/analytics/er-download-configurations-global-repo.md).
...
1. Select **Import from GAR**.
1. In the **Import from GAR** dialog, on the **Parameters** FastTab, select **Browse** to select the zip archive.
1. If you're importing a full database, set the **Full import** option to **Yes**. If you're importing a delta file, set it to **No**.
1. If you intend to import houses and steads, set the **Import Houses and Steads** option to **Yes**.
1. If you intend to import rooms, set the **Import Rooms and Apartments** option to **Yes**.
...
1. Create a new zip file that contains the folders that correspond to the required states and the metadata files.
