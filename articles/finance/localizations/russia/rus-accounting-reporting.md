...
description: Learn how to set up accounting reporting for Russia in Microsoft Dynamics 365 Finance.
author: evgenypopov
...
1. Upload the Data management package settings.
    ...
    1. Download the package, **RU Accounting reporting 5.08 (2019).zip**. 
    1. In Dynamics 365 Finance, in the **Data management** workspace, select **Import**.
    1. In the **Job details** section, enter any name for the job.
    1. In the **Data source format** field, select **Package**.
    1. In the **Upload data file** field, select **Upload**, and then select the **RU Accounting reporting 5.08 (2019).zip** file that you downloaded earlier.
    1. After the data entities are uploaded, select **Import**.

1. In Dynamics 365 Finance, go to **General ledger \> Financial reports setup \> Financial reports**, and validate the financial reports that are imported. (All the data that is imported is presented only in the Russian language.)
...
1. Set up financial report cell operations for the financial reports that are imported. Set up all required cells. These required cells include cells for totals.
...
1. Set up the organization codes that are used in accounting reporting and any additional information about the company.

    1. Go to **Tax \> Setup \> Electronic messages \> Additional fields**.
    1. In the left pane, select the field that is named **EconomicActivityTypeCode (ОКВЭД)**. Then, on the **Value** FastTab, select **Add**. In the **Field value** field, enter the code for legal entity's economic activity type.
    1. Select the field that is named **OrganizationalFormCode (ОКОПФ)**. Then, on the **Value** FastTab, select **Add**. In the **Field value** field, enter the code for the legal entity's organizational form.
    1. Select the field that is named **OwneshipFormCode (ОКФС)**. Then, on the **Value** FastTab, select **Add**. In **Field value**, enter the code for the legal entity's ownership form.
    1. Select the field that is named **AuditorCompanyName (Наименование аудиторской организации)**. Then, on the **Value** FastTab, select **Add**. In **Field value**, enter the name of the auditor company in case of mandatory audit of the reporting.
    1. Select that field that is named **AuditorStateRegistrationNumber (ОГРН аудиторской организации)**. Then, on the **Value** FastTab, select **Add**. In **Field value**, enter the state registration number of the auditor company.
    1. Select the field that is named **AuditorTaxRegistrationNumber (ИНН аудиторской организации)**. Then, on the **Value** FastTab, select **Add**. In **Field value**, enter tax registration number of the auditor company.
    1. Go to **Tax \> Setup \> Electronic messages \> Electronic message processing**.
    1. On the **Message additional fields** FastTab, enter the following information.
        ...
        | AuditorTaxRegistrationNumber | Select the value that you entered in step 1. |
...
1. Set up the ER format that is run when accounting reporting is generated in electronic format.
    ...
    1. In the left pane, select the action that is named **Generate BUHOTCH 5.08**, and then expand the **General** FastTab.
    1. Validate that in the **Format mapping** field, the configuration name **Accounting reporting format 5.08 (RU)** is selected. Validate that **Show dialog** is set to **Yes**.
...
1. On the **Messages** FastTab, select **New**. Then, in the **Run processing** dialog, select **OK**.
1. Select the message line that was created. Enter a description, and specify the start date and end date for the report. The end date is treated as the base date for financial reports.
...
1. On the **Messages** FastTab, select **Update status**. Then, in the **Run processing** dialog, select **OK**.
...
1. To review the report that is generated, select the **Attachments** button (the paper clip symbol) in the upper-right corner of the page. Then select **Open** to open the file.
