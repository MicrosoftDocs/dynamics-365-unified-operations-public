...
description: Learn how to use translation to transfer accounting transactions from one company to another in Russia in Microsoft Dynamics 365 Finance.
author: evgenypopov
...
1. Set up the currencies and exchange rates.
1. Create fiscal periods.
...
1. Create a line.
1. In the **Rule group** field, enter a unique code for the translation rule group.
1. In the **Company** field, enter the code for the source company.
1. In the **To company** field, select the code for the target company.

    > [!NOTE]  
    > The value of the target company's standard currency is automatically updated in the **To currency** field.

1. In the **Description** field, enter a description of the translation rule group.
1. On the **Translation methods** FastTab, select **Methods initialization** to create a list of translation methods.
1. Select **Add**, and then, in the **Method** field, select **Ledger translation**.

    :::image type="content" source="../media/translation-groups01.jpg" alt-text="Screenshot of the Translation groups page.":::

1. On the **Ledger parameters** FastTab, in the **Default currency group** field, select the currency group that should be used by default for the account translation rules.
1. In the **Default dimension group** field, select the dimension group that should be used by default for the account translation rules.
1. In the **Default priorities** field, enter the priority that should be used by default for the account translation rules.
1. Set the **New ledger vouchers** option to **Yes** to create new ledger voucher numbers when you do translations. If the **New ledger vouchers** option is set to **No**, ledger account transactions are translated to the ledger account of the target company by using the voucher number of the source company.
1. In the **Number sequence code** field, select the number sequence for the vouchers.

    > [!NOTE]  
    > The **Number sequence code** field is available only if you set the **New ledger vouchers** option to **Yes**.

1. In the **Translation difference** section, you define the level that the translation difference should be checked to and the account where translation difference amounts should be put. The translation difference is the imbalance that occurs in the target company's primary and secondary currencies because the valuation of the corresponding entries is done at different rates.
1. In the **Check translation difference** field, select one of the following validation methods for the translation difference:
...
1. In the **Ledger account** field, select the main account number that should reflect the translation differences for the target company's default and secondary currencies.
...
1. In the **Check imbalance** field, select the validate method for the imbalance:
...
1. In the **Ledger account** field, select the account number that should be used to accumulate the imbalance amounts in the target company's transaction currency.
...
1. In the left pane, in the **Rule group** field, select the rule group that you're setting up the currency conversion rule for.
    ...
    :::image type="content" source="../media/currency-groups02.jpg" alt-text="Screenshot of the Currency conversion rules page.":::

1. Create a currency group.
1. In the **Currency group** field, enter a unique code for the currency group.
1. In the **Description** field, enter a description of the currency group.
1. On the **Currency conversion rules** FastTab, select **Add** to create a line.
1. In the **Currency** field, select the code for the currency in the source company that the conversion method is being set up for.
1. In the **Rate method** field, select the currency conversion method that should be used when transactions are translated from the source company to the target company:
...
1. In the **Inflation index** field, enter a unique code for the inflation index.
1. In the **Description** field, enter a description of the inflation index.
1. On the **Rules for dimensions** FastTab, select **Add** to create a line.
1. In the **From date** field, select the start date when the index becomes valid.
...
1. In the **Exchange rate** field, enter the index value that should be used during the index validity period.

    :::image type="content" source="../media/inflation-indexes03.jpg" alt-text="Screenshot of the Rules for dimensions FastTab showing the Exchange rate field.":::
...
1. In the left pane, in the **Rule group** field, select the translation group that you're setting up dimension conversion rules for.
    ...
    :::image type="content" source="../media/dimension-groups04.jpg" alt-text="Screenshot of the Rules for dimensions FastTab.":::

1. Select **New** to create a dimension group.
1. In the **Dimension group** field, enter a unique code for the dimension group.
1. In the **To company** field, enter the code for the target company that should be used for transactions during translation.
1. In the **Description** field, enter a description of the dimension group.
1. On the **Rules for dimensions** FastTab, select **Add** to create a line.
1. In the **Dimension** field, select the dimension in the source company that the conversion rule is being configured for.
1. In the **From dimension code** and **To dimension code** fields, define the range of dimension values.
1. In the **Conversion method** field, select the method that is used to convert the dimension of the translated transaction from the source company:
...
1. In the **To dimension** field, select the type of dimension in the target company that the values should be transferred to.
1. In the **To dimension code** field, select the code for the dimension in the target company that the transaction dimension in the source company should be moved to during translation.
...
1. On the **Overview** tab, in the **To account** field, select the ledger account from the target company's chart of accounts that transactions in the source company should be translated to. After you save your changes to the line, the name of the target company is automatically selected in the **Account name** field.
1. In the **Main account** field, select the ledger account in the source company that should be used to translate transactions to the target company.
1. In the **Offset account** field, select the offset account in the source company that should be used to translate transactions that have the specified correspondence. Leave this field blank to select all transactions, regardless of the offset account.
1. In the **Crediting** field, select the type of transactions to translate:
...
1. In the **Currency group** field, select the code for the currency conversion group. If a default currency group is configured for the translation rule group that is selected in the **Rule group** field, the value is automatically entered. However, you can change the value as you require.
1. In the **Dimension group** field, select the code for the dimension conversion group. If a default dimension group is configured for the translation rule group that is selected in the **Rule group** field, the value is automatically entered. However, you can change the value as you require.
1. In the **Priority** field, enter the priority of the rule. If a default priority is selected for the translation rule group that is selected in the **Rule group** field, the value is automatically entered. However, you can change the value as you require.
    ...
    :::image type="content" source="../media/translation-ledger-account-rules05.jpg" alt-text="Screenshot of the Translation ledger account rules lines.":::

1. On the **General** tab, in the first **Posting type** field, select a voucher type to translate only transactions that have that voucher type. For example, if you select **Petty cash**, only transactions in the bank module are translated.
1. In the second **Posting type** field, select a voucher type to translate only transactions that have that voucher type for the offset account.
1. In the **Posting layer** field, select a posting layer to translate only transactions that have that posting layer.
1. In the **Only reversing entry** field, select **Yes** to translate only reversing transactions, or select **No** to translate only regular transactions. Leave this field blank to translate both regular transactions and reversing transactions.
1. In the **To posting layer** field, select the posting layer that the translated transactions should be assigned to in the target company.

    :::image type="content" source="../media/translation-ledger-account-rules06.jpg" alt-text="Screenshot of the Translation ledger account rules fields.":::

1. Select **Initialize** to automatically create a line that has an account translation rule for all the ledger accounts in the target company. In the **Initialize** dialog, you can select **Filter** to filter the required accounts.
1. Select **OK**. The system creates a line that has a rule for each selected ledger account.
...
1. In the **Rule group** field, select the translation rule group that should be used by default when the translation session is started.
1. On the **Number sequences** tab, in the **Number sequence code** field, select a number sequence code for the **Translation session** reference.
1. Close the page.
...
1. Select translation dates.
1. Apply translation rules to all operations in the specified period.
1. Show the transaction conversion results on a **Preview** page.
1. Save the translation results to the target company.

To do a single translation, follow these steps to use the **Translation** wizard.

1. In Dynamics 365 Finance, go to **General ledger** \> **Periodic tasks** \> **Translation sessions**.
1. In the upper part of the page, in the **Rule group** field, select a translation group.
1. Select **Translation wizard** to open the **Translation** wizard.
1. On the **Welcome** page, select **Next**.

    :::image type="content" source="../media/welcome07.jpg" alt-text="Screenshot of the Welcome page of the Translation wizard.":::

1. On the **Consistency check** page, select **Consistency check** to validate the conversion parameters.

    :::image type="content" source="../media/consistency-check08.jpg" alt-text="Screenshot of the Consistency check translation setup page.":::

1. In the **Consistency check** dialog, specify the details, and then select **OK**.
    ...
    :::image type="content" source="../media/consistency-check09.jpg" alt-text="Screenshot of the Consistency check page.":::

1. Select **Next**.
1. On the **Selection** page, in the **From date** and **To date** fields, define the period that the translation should be done for.
1. Select **Inquiry** to specify additional transaction selection criteria.

    :::image type="content" source="../media/selection10.jpg" alt-text="Screenshot of the Selection page.":::

1. Select **Next**.
1. On the **Transaction processing** page, select **Next** to run the data conversion.

    :::image type="content" source="../media/translation-processing11.jpg" alt-text="Screenshot of the Transaction processing page.":::

1. Select **Preview** to view the conversion results.
1. Select **Next** two times to import the translation data into the target company.
1. After the translation is completed, select **Finish**.
...
1. Go to the target company, and verify that the transactions have been successfully transferred. The date of the transactions that are transferred to the target company will be the date when the transactions were posted in the source company.
...
1. Select **Batch translation** to open the **Batch translation** dialog.
1. On the **Parameters** FastTab, in the **From date** and **To date** fields, define the period that the batch translation should be done for.

    :::image type="content" source="../media/batch-translation12.jpg" alt-text="Screenshot of the Batch translation page.":::

1. On the **Run in the background** FastTab, select **Recurrence** to open the **Define recurrence** dialog, where you can set up the recurrence of the batch translation.

    :::image type="content" source="../media/define-recurrence13.jpg" alt-text="Screenshot of the Define recurrence dialog.":::

1. In the **Batch translation** dialog, select **OK** to do the batch translation.

## Cancel a processed translation
...
1. Select **Cancellation**. The **Export status** and **Import status** fields are automatically updated to **Canceled**.
