...
1. On the **Product dimensions** tab, in the **Warehouse** field, select the warehouse where the item is located.
1. In the **Inventory profile** field, select the inventory profile for the item.
1. In the **Batch number** field, select the batch number of the item.
1. In the **Serial number** field, select the serial number of the item.
1. Close the page.
1. On the **Working clothes/Special rigging/NVFA issue journal** page, select **Validate** to validate the journal.
1. Select **FA journals** \> **FA journal (putting into operation)** to put the asset into operation.

    –or–

    Select **FA journals** \> **FA journal (depreciation)** to depreciate the asset.

1. Select **Close** to post the journal.
...
1. Double-click an existing working clothes or special rigging item.
1. Select **Componentry** \> **Componentry**.
1. Select the fixed asset item or component that is required, and then select **Add to remains**.
1. In the **Quantity** field, enter the remaining quantity of the item.
1. Select **OK** to add the working clothes or special rigging item to item remainders. The lines that are marked in the upper pane of the **Componentry** page are moved to the lower pane.
1. In the lower pane, select **Calculating prices** to calculate the balance value, depreciation, and depreciated cost for each item that is returned. The calculations are based on the price calculation method that is selected.
1. Close the page.
1. Go to **Fixed assets (Russia)** \> **Journals** \> **FA journal**.
1. Create a fixed asset journal.
...
1. In the **Transaction type** field, select **Disposal (dismantlement)**.
