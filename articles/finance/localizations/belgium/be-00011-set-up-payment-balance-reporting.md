...
Use the following procedures to set up BLWI information for Belgium. These procedures use the USSI demo data company.

This functionality is available for legal entities that have their primary address in Belgium. Before you can complete this procedure, you must set up the registration ID for Belgium and enter the registration number that you use to create the BLWI declaration.
...
1. In the **BLWI** field, select **Yes**. Activate this parameter before you post customer or vendor transactions so that the system transfers the transactions to the payment balance.  
1. In the **Email** field, enter a value.
1. In the **Name** field, enter a value.
1. In the **Telephone** field, enter a value.
1. In the **Fax** field, enter a value.
1. In the **Check BLWI code on journals** field, select an option. Select the rule for checking that a payment purpose code is specified in documents. The available options are None, Warning, and Error. If a transaction has a customer or vendor that's located in a foreign country or region (that is, the country or region of the customer or vendor differs from the country or region of the legal entity), the transaction doesn't have an assigned payment purpose code, and the rule is set to **Warning** or **Error**, either a warning or error message is shown during posting. This validation is applied for all customer or vendor transactions except payment transactions.  
...
1. Select **New**.
1. In the **Survey code** field, enter a value.
1. In the **Month/Quarter** field, select an option.
1. In the **Calculation type** field, select an option. If you select the **Turnover in the Calculation type** option, the customer and vendor transactions that you post during the reporting period are transferred to the payment balance, and the amount that you report is the total amount of the posted transaction. If you select the **Balance** option, customer and vendor transactions that are open (not fully settled or closed) at the end of the reporting period are transferred to the payment balance, and the amount that you report is the open (unsettled) amount of the posted transaction as of the end of the reporting period.  
1. In the **Description** field, enter a value.
1. In the **Summary country/region** field, enter a value.
1. Select **Add**.
1. In the **Central bank purpose code** field, enter or select a value.
1. Select the **Include payment** checkbox. By default, payment transactions aren't transferred to the payment balance survey. You must first activate the **Include payment** field for purpose codes.
