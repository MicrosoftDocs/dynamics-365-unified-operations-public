...
description: In Japan, lump-sum assets, low-value assets, and deferred assets are depreciated in equal amounts in each year of the service life.
author: EricWangChen
ms.author: wangchen
ms.topic: article
ms.date: 12/08/2025
...
No. The depreciation amount doesn't change, depending upon when it's calculated during the month. For example, you acquire a lump-sum fixed asset:
...
When you use the equally divided depreciation method, the depreciation amount for this fixed asset is JPY 4,166.67 for each month, regardless of whether you calculate it at the beginning of the month or in the middle of the month.

## How do you calculate equally divided depreciation for a low-value asset that you acquire midyear?

If you purchase a low-value fixed asset in the middle of a fiscal year, Dynamics 365 Finance treats the current fiscal year as the first fiscal year for the depreciation calculation. For example, you acquire a low-value fixed asset in the middle of a fiscal year:
...
The following table shows the depreciation calculation for the fiscal year from April 1, 2012, to March 31, 2013.
...
## How do you calculate equally divided depreciation for a lump-sum asset acquired midyear?
...
## How is depreciation calculated using the equally divided depreciation method if the asset calendar changes during the asset life cycle?

When Finance calculates depreciation by using the equally divided depreciation method, it bases the calculation on the fiscal calendar that you select in the **Calendar** field on the **Books** page. If you change the asset calendar during the asset life cycle, the system calculates the number of fiscal periods, such as months or quarters, in a fiscal year based on the current calendar. For example, you acquire a lump-sum fixed asset:

- Fiscal calendar = April 1 to March 31
- Acquisition date of the lump-sum fixed asset = April 1, 2012
- Number of years to equally divide the depreciation amount = 3 years

If you change the fiscal calendar in April 2013, during the second fiscal year, from April 1 through March 31 to January 1 through December 31, the system calculates the number of depreciation periods for the fiscal years as shown in the following table.
...
No. When you depreciate a fixed asset by using the equally divided depreciation method, you equally divide the depreciation amount among the fiscal periods within the useful life of the fixed asset. Therefore, you can't change the depreciation method during the life cycle of the fixed asset.

## Can I change the depreciation period for an equally divided depreciation profile?

No. If you select an equally divided depreciation profile on the **Books** page, you can't change the value in the **Number of years to equally divide depreciation amounts** field on the **Depreciation profiles** page for the depreciation profile.
