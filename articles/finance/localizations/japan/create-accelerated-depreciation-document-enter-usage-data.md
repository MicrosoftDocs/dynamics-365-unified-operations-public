...
For Japan, you declare accelerated depreciation on a per document basis. Each document can contain multiple fixed assets. You must calculate an overuse rate on the accelerated depreciation document and multiply it by the ordinary depreciation amount to get the amount for the accelerated depreciation. The details of the accelerated depreciation declaration are listed on the document. You can only use a confirmed accelerated depreciation document at accelerated depreciation proposal for posting. 

The following procedures walk you through how to create a fixed asset with an accelerated depreciation profile, create an accelerated depreciation document and assign the fixed asset to it, enter overuse hours for the fixed asset, and calculate the overuse rate on the document. The procedures use the demo data company JPMF.
...
1. In the **From date** field, enter a date. The **From date** and **To date** values define the declaration period during which to apply and declare the accelerated depreciation. 
1. In the **To date** field, enter a date.
1. Expand the **Fixed assets and working hours detail** section. Enter working hours by fixed assets so that the data is accurate and complete.  
1. Select **Add by query**.
1. In the **Criteria** field, filter to find the fixed assets.
1. Select **OK**.
1. In the **Planned** field, enter a number. Besides planned hours, you can also enter reserved or actual hours if they apply.  
1. Select **Save**.
1. Select **Calculate daily average hour**.
1. Confirm that the overuse rate is calculated and updated. If you already calculated the overuse rate, you can skip the calculate step and directly enter the overuse rate and confirm the document for proposal purpose.  
