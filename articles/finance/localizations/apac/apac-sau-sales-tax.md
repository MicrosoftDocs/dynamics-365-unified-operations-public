...
The following table lists the typical tasks that you complete to set up sales tax. It includes information about considerations that pertain specifically to sales tax in Saudi Arabia.
...
You can create reporting codes that are required for analysis, such as details of legislation return and data disclosure. The following illustration shows examples of reporting codes.

:::image type="content" source="../media/apac-sau-sales-tax-reporting-codes.png" alt-text="Screenshot of examples of sales tax reporting codes.":::

### Sales tax codes, sales tax groups, and item sales tax groups

Set up a default sales tax group on the customer or vendor, and set up a default item sales tax group on items. When you create a sales order or purchase order, the system transfers the default sales tax group from the customer or vendor to the header and lines, and the default item sales tax group from the items to the lines. However, you can change groups both in the header and on lines.

The same sales tax code might be included in two groups (a sales tax group and an item sales tax group) on a line. In this case, when you post the invoice, the system creates a sales tax transaction that uses this sales tax code, and it calculates sales tax. If there's no intersection of sales tax codes in the groups on a line, a sales tax transaction isn't created.
...
:::image type="content" source="../media/apac-sau-sales-tax-groups-diagram.png" alt-text="Screenshot of sales tax group setup diagram.":::
...
- Calculation (Use the default settings at first.)
...
For import operations from other countries or regions, where VAT is paid at customs, you might select the **Use tax** check box in the sales tax group. Then, when you post an invoice that uses this setting, the system creates a sales tax transaction that uses the **Use tax** direction, but sales tax isn't included in the vendor liability. Sales transactions that use the **Use tax** direction aren't considered when sales taxes are settled and posted (see Settle and post sales tax). Additionally, depending on ledger posting setting (see Ledger posting groups), the system creates ledger transactions for this sales tax (see the illustration that follows). This setting lets you analyze purchases from other countries or regions by VAT rate.
...
When you print an invoice, if your language differs from the language that you specify for the current legal entity, two invoices are generated. One invoice is in Arabic, and the other invoice is in the language that you select on the **User options** page.
...
:::image type="content" source="../media/apac-sau-sales-tax-diagram.png" alt-text="Screenshot of sales tax payment by reporting code.":::

### Report setup

Consider the following information before you generate the **Sales tax payment by code** report:

- You can create reporting codes in details that are required for analysis (for example, details of legislation return and data disclosure).
- To include sales tax transactions on the report, you must select reporting codes in the corresponding fields on the **Report setup** and **Report setup – Credit note** FastTabs of the **Sales tax code** page.
    ...
    :::image type="content" source="../media/apac-sau-sales-tax-codes.png" alt-text="Screenshot of an example of the setup of reporting codes for a sales tax code.":::
...
1. Select the report parameters. These parameters include the report layout.

The following illustration shows an example of the **Sales tax payment by code** report.

:::image type="content" source="../media/apac-sau-report-example.png" alt-text="Screenshot of an example of the Sales tax payment by code report.":::
