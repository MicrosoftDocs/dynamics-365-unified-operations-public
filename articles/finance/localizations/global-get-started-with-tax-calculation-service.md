...
title: Get started with the tax calculation
...
## Set up the tax calculation feature in RCS
...
After you complete the setup in RCS, as described in the previous section, you will have a published version of the tax feature. Follow these steps to set up the tax calculation in Finance.

The setup in this section is done by legal entity. You must configure it for each legal entity that you want to enable the tax calculation service in Finance.
...
2. On the **General** tab, set the following fields:
    ...
    - **Enable tax calculation service** – Select this check box to enable the tax calculation add-in for the legal entity. If the tax calculation service isn't enabled for the current legal entity, the legal entity will continue to use the existing tax engine to determine and calculate tax.
    ...
    - **Business Process** – Select the business processes to enable for the tax calculation service.
...
4. On the **Error handling** tab, define the expected error handling method for the legal entity. Three options are available for each result code from the tax calculation service:

    - No
    - Warning
    - Error

5. Save the tax calculation setup.
6. Repeat steps 1 through 5 for each additional legal entity.

## Transaction processing

After you've completed all the setup procedures, you can use the tax calculation service to determine and calculate tax in Finance. The steps to process transactions remain the same. The following transactions are supported in Finance version 10.0.18:
