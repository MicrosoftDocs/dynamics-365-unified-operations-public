...
| Require same currency in settlement | Specify whether transactions can be settled only if they're in the same currency. |
...
| Field | Description |
| ------- | ------------- |
| Separate exchange difference invoice for each invoice | Specify whether to create a separate exchange difference invoice for each invoice. |
| Include only fully settled invoices | Specify whether to include only fully settled invoices in the exchange difference invoice update. |
| Group by dimension | Specify whether to group invoices by financial dimension. |
| Group by tax group | Specify whether to group invoices by tax group. |
| Group by profit and loss | Specify whether to group invoices by profit and loss. |
| Default invoice prefix | Specify whether to assign the default serial prefix for the invoice number automatically to exchange difference invoices. |
| Copy description to voucher transaction | Specify whether to copy the description to the voucher transaction. |
| Copy description to journal line | Specify whether to copy the description to the journal line. |
...
Before you create an exchange difference invoice, define the realized gain and realized loss accounts on the **Currency revaluation posting profile** page in Accounts payable and Accounts receivable.
...
### Use exchange difference invoicing for selected vendor or customer groups

Some organizations require different exchange difference processes for domestic and foreign vendors or customers. For example, domestic vendors might require exchange difference invoices, whereas foreign vendors should have realized exchange rate gains or losses posted automatically during settlement, without creating an exchange difference invoice.

You can support this scenario through standard configuration. There's no additional parameter to enable or disable exchange difference invoicing for individual vendors, customers, or their groups.

Finance determines whether to process a settled transaction by exchange difference invoicing based on the realized gain or realized loss account that is used during settlement. The system selects that main account according to the standard main account lookup hierarchy:

1. **Currency revaluation posting profile**
1. **Currency revaluation accounts**
1. **Ledger** (**Accounts for currency revaluation** FastTab)

Exchange difference invoicing evaluates the realized gain or realized loss account that you used for the settlement transaction.

- If the settlement uses a realized gain or realized loss account that you configured through the **Currency revaluation posting profile**, the transaction is eligible for exchange difference invoicing.
- If no matching account exists on the **Currency revaluation posting profile** page, the application continues the standard account lookup process. When you select a realized gain or realized loss account from the **Currency revaluation accounts** page or the **Ledger** page, the exchange difference is recognized during settlement, and no exchange difference invoice is created.

To apply different exchange difference processes, configure realized gain and realized loss accounts following these steps:

1. Go to **General ledger** > **Currencies** > **Currency revaluation posting profile**.
1. Configure realized gain and realized loss accounts for the vendor or customer groups that should use exchange difference invoicing.
1. Leave vendor or customer groups that shouldn't use exchange difference invoicing without a corresponding record on the **Currency revaluation posting profile** page. For those groups, the application uses the next applicable account in the standard lookup hierarchy.

> [!IMPORTANT]
> If both currency-based rules and vendor or customer group-based rules can apply to the same transaction, set the **Lowest level of defaulting** field to **Account**. This setting ensures that the vendor or customer group configuration takes precedence over general currency rules.

With this configuration, you can use different exchange difference processes in the same legal entity:

| Vendor or customer group | Behavior |
| --------------------------- | ---------- |
| Group configured on the **Currency revaluation posting profile** page | The transaction is eligible for exchange difference invoicing. |
| Group without a corresponding record on the **Currency revaluation posting profile** page | Realized exchange gains or losses are posted automatically during settlement. No exchange difference invoice is created. |

Learn more about configuring realized gain and realized loss accounts in [Currency revaluation posting profiles](../../general-ledger/currency-revalue-posting-profile.md).
...
The reports are implemented as Electronic reporting (ER) configurations. Import the latest versions of the following ER configurations:

- **Exchange difference invoice (TR) (Excel)** – This configuration provides an Excel report that you can use to print exchange difference invoicing.
...
Before you can generate the reports, select the relevant formats in the **Exchange difference invoice** and the **Exchange difference invoice voucher** fields on the **General** tab of the **Exchange difference invoicing parameters** page.
...
The exchange difference invoicing feature retrieves settled invoice and payment transactions for customers and vendors that you specify during the process. When you select **New** on the Action Pane to create exchange difference invoice records, you see two options:
...
The **Date** field determines which settled transactions are considered for customers and vendors. Use it to ensure that no exchange differences are created from payment transactions that are dated after the specified invoice date.

> [!NOTE]
> The system uses the values set on the **Invoice** tab of the **Exchange difference invoicing parameters** page as default values when you create a new exchange difference invoice.
>
...
After you create exchange difference invoices, various functions become available for them on the **Exchange difference invoicing** page. The following table describes the buttons on the **Invoice** tab of the Action Pane.
...
> [!NOTE]
> After the vendor or customer exchange difference invoice is created, you can't reverse settled transactions for the customer or vendor.
...
Use the **Date** field to manually change the posting date of exchange difference invoices. To calculate the tax amount for the exchange difference invoice, select a sales tax group and an item sales tax group.

Use the **Delete** button to remove a selected record in the **Exchange difference invoice** section. When you delete an exchange difference invoice, you also delete the invoice and payment transactions that led to its creation. If you need to re-create a deleted exchange difference invoice, select **New** on the **Exchange difference invoicing** page, and then select either **Exchange difference invoices for vendors** or **Exchange difference invoices for customers**. The relevant exchange difference invoice is then listed again, based on the selected parameters.

In some cases, you might need to exclude the generated exchange difference invoice before posting it to the ledger. To exclude an exchange difference invoice, select **Exclude**. You can view information about excluded records on the **Excluded records** tab. To include a record again, select **Include**.
...
According to legal requirements, if the exchange difference amount is a debit balance, you must create an exchange difference invoice for the relevant customer or vendor. In this case, the **Invoice serial** field is mandatory, and you must select a serial prefix in it. If the exchange difference amount is a credit balance, you must enter the document number of the exchange difference invoice that you receive from the relevant customer or vendor in the **Invoice serial** field.

> [!NOTE]
> - If you set the **Default invoice prefix** option to **No**, the **Invoice serial** field is mandatory, and you must manually select a serial prefix in it.
> - If you set the **Default invoice prefix** option to **Yes**, the **Invoice serial** field is automatically set, based on the **Default prefix** value that you select on the **Preprinted serial numbers** page. Learn more in the article [Use serial numbering](emea-tur-serial-numbering.md).

When you generate the **Exchange adjustment statement** report from the **Document** group on the **Invoice** tab of the Action Pane, the **Reconciliation statement sent** field in the **Exchange difference invoice** section is marked. You can then approve the relevant exchange difference invoice by using the **Approve** button. As required, use the **Reset Approval** button to reset approval.

If you set the **Require invoice approval** option to **Yes** on the **General** tab of the **Exchange difference invoicing parameters** page, the **Post** button becomes available only after the exchange difference invoice is approved. Use this button to post the invoice to the ledger. If you set the **Require invoice approval** option to **No**, you can post the exchange difference invoice to the ledger without approval.

After you post exchange difference invoices, the **Foreign currency revaluation** transaction type on the **General** tab of customer or vendor transactions indicates them.

In the **Exchange difference invoice line transactions** section, use the **Delete** button to remove invoice transactions during the reconciliation process between vendors and customers. After you delete an invoice, the exchange difference amount that is shown in the **Exchange difference amount** field in the **Exchange difference invoice** section automatically updates to reflect the deletion. Select **Transactions** to open a detailed view of all the voucher transactions that are associated with the relevant invoice. Select **Move** to transfer an exchange difference invoice to another invoice transaction within the same customer or vendor account. This button helps efficiently manage and organize multiple invoice transactions.

When you reconcile invoices, payments, and collections with customers and vendors, timing differences in money transfers might cause exchange differences. You must review the value of the **Exchange difference amount** field and manually adjust it as required. Initially, the **Calculated exchange difference amount** and **Adjusted exchange difference amount** values are the same. However, you can make adjustments in the **Settled transactions** section. You must manually enter the reconciliation amount from customers and vendors.
...
1. In Dynamics 365 Finance, go to **Accounts payable** or **Accounts receivable** > **Periodic tasks** > **Exchange difference invoicing**.
1. Select an exchange difference invoice that you posted.
...
1. In Dynamics 365 Finance, go to **Accounts payable** or **Accounts receivable** > **Periodic tasks** > **Exchange difference invoicing**.
1. Select an exchange difference invoice that you didn't post.
1. On the **Action** pane, on the **Invoice** tab, in the **Document** group, select **Exchange adjustment statement**.

When you print the **Exchange adjustment statement** report, the **Reconciliation statement sent** field is marked, and the **Approve** button becomes available in the **Exchange difference invoice** section. If you select **Approve**, the **Approved** and **Approved date** fields are set.

> [!NOTE]
> If you set the **Require invoice approval** option to **Yes** on the **Exchange difference invoicing parameters** page, follow these steps:
>
