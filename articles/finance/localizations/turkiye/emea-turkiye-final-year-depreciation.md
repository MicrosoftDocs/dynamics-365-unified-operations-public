...
In Türkiye, the remaining depreciation amount in the final year must be distributed equally across all 12 months.  
...
When one of these methods is selected in a depreciation profile, the system automatically distributes the final year’s depreciation evenly across each month.  
...
When Prorate depreciation is set to **No**; 

- The system calculates a **full year of depreciation for the acquisition year**, even if the asset is acquired later in the year.  
- Depreciation is distributed evenly across all 12 months.  
- If earlier periods are open, depreciation for those months is included as **cumulative depreciation** in the acquisition month’s depreciation run.  
...
When **Summarize depreciation** is enabled, the system posts all depreciation amounts—both prior-period cumulative depreciation and the acquisition-month depreciation—in one summarized proposal. This ensures consistent postings without creating entries in closed periods.

When Prorate depreciation is set to **Yes**; 
...
If earlier months of the acquisition year are closed (for example, after e-Ledger submission), the system cannot post depreciation for those months. Any unposted depreciation is included in the acquisition month depreciation proposal.
...
The **Reducing balance (Türkiye)** depreciation method ensures that any remaining depreciation amount is automatically adjusted and distributed evenly in the final year.  

### Set up the depreciation profile  

Follow these steps to create a depreciation profile with the **Reducing balance (Türkiye)** depreciation method:

1. Go to **Fixed assets > Setup > Depreciation profiles**.  
2. Select **New** or edit an existing profile.  
3. In the **Method** field, select **Reducing balance (Türkiye)**.  
4. Enter the **Percentage** for the depreciation rate.  
5. In the **Depreciation year** field, choose **Calendar** or **Fiscal**.  
6. In the **Period frequency** field, select **Monthly**.
7. Set the **Full depreciation** option is **Yes**.
8. Save the profile.
...
Follow these steps to configure depreciation parameters in a specific asset book:

1. Go to **Fixed assets > Fixed assets > Fixed assets**.  
2. Select the fixed asset, and then select **Books**.  
3. Select the required asset book.  
4. In the **Depreciation convention** field, select **Full month**.   

### Scenario – Reducing balance (Türkiye)

This scenario demonstrates that how depreciation is calculated when the **Prorate depreciation** option is set to **No**. 
In this case, the system calculates a full year of depreciation for the acquisition year, even if the asset is acquired later in the year.

When the first depreciation proposal is generated in the acquisition month, the system can include cumulative depreciation for earlier unposted months of the year.  
...
When **Prorate depreciation** option is set to **No**, the system always calculates the full annual depreciation amount for the acquisition year.  
In the final year, the remaining net book value is distributed evenly across all 12 months.
...
> [!IMPORTANT]  
> For legal entities in Türkiye, you should not use the standard **Reducing balance** depreciation method together with **Prorate depreciation** option.  
...
The **Straight line service life (Türkiye)** depreciation method ensures that any remaining depreciation amount is automatically distributed equally across all 12 months in the final year when prorated depreciation is used.  

### Set up the depreciation profile  

Follow these steps to create a depreciation profile with the **Straight line service life (Türkiye)** depreciation method:

1. Go to **Fixed assets > Setup > Depreciation profiles**.  
2. Select **New** or edit an existing profile.  
3. In the **Method** field, select **Straight line service life (Türkiye)**.  
4. In the **Period frequency** field, select **Monthly**.
5. Save the profile.

For details about creating or editing depreciation profiles, see [Set up and create depreciation profiles](../../../finance/fixed-assets/tasks/set-up-depreciation-profiles.md).  
...
1. Go to **Fixed assets > Fixed assets > Fixed assets**.  
2. Select the fixed asset, and click **Books**.  
3. Select the required asset book.
4. In the **Service life** field, enter the useful life in years.  
5. In the **Depreciation convention** field, select **Full month**.  
