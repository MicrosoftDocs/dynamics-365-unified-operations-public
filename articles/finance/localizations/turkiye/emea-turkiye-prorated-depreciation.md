...
This section provides an overview of prorated depreciation in Türkiye and explains how the system calculates the first year and final year depreciation amounts when an asset is placed in service partway through the year in Microsoft Dynamics 365 Finance.  

Prorated depreciation is a calculation method that begins depreciation based on the month an asset is placed in service. When the **Prorate depreciation** option is enabled, the system calculates depreciation only for the months the asset is in service during the acquisition year.

If you use one of the Türkiye-specific depreciation methods, any depreciation amount that cannot be recognized in the acquisition year is added to the final year and distributed evenly across all 12 months. This ensures that both the first year and final year depreciation are calculated proportionally.

This behavior is commonly used for assets where depreciation must begin from the month of acquisition.
Organizations can enable prorated depreciation if they need depreciation to start from the acquisition month rather than applying a full annual amount. 

Finance supports this through the **Prorate depreciation** option in fixed asset books.  
This option automates the proportional calculation for both the first and the final year of depreciation.  

When the **Prorate depreciation** option is set to **Yes**, the system automatically adjusts the depreciation start date based on the asset’s placed in service date.  
...
This ensures that depreciation begins in the correct month and prevents the system from calculating a full year of depreciation for the acquisition year.

The **Prorate depreciation** option can be set either:  
...
2. Select a fixed asset and open the **Books** page.  
3. On the **Depreciation** FastTab, set **Prorate depreciation** to **Yes**.  

### Configure prorated depreciation for an asset group  

1. Go to **Fixed assets > Setup > Asset groups**.  
2. Select an asset group and open the **Asset group books** page.  
3. In the **General** section, set **Prorate depreciation** to **Yes**.  

> [!IMPORTANT]
> For legal entities in Türkiye, you should use the Türkiye-specific **Straight line service life (Türkiye)** and **Reducing balance (Türkiye)** depreciation methods together with the **Full month** depreciation convention.
> Standard depreciation methods do not support prorated depreciation correctly in the acquisition year or the final year.
...
The scenario demonstrates that how prorated depreciation works when it is applied together with the **Straight line service life (Türkiye)** depreciation method.
...
|------|----------------------|---------------------|--------------------------|----------------|-------|
| 2024 | 1,500.00 TRY         | **1,500.00 x 3 = 4,500.00 TRY (October - November - December)**  | 4,500.00 TRY             | 85,500.00 TRY  | Prorated depreciation (3 months) |
...
This section explains how prorated depreciation interacts with the **Reducing balance (Türkiye)** depreciation method, and how remaining first year depreciation is carried forward and allocated across the final year.

The scenario demonstrates that how prorated depreciation works when it is applied together with the **Reducing balance (Türkiye)** depreciation method.
...
| Year | Monthly depreciation amount | Annual depreciation amount | Accumulated depreciation amount | Net book value | Notes |
|------|----------------------|---------------------|--------------------------|----------------|-------|
| 2024 | 3,000.00 TRY         | **3,000.00 x 3 = 9,000.00 TRY (October - November - December)**    | 9,000.00 TRY             | 81,000.00 TRY  | Prorated depreciation (3 months) |
...
The **Prorate depreciation** option in **Fixed asset books** should not be confused with the **Calculate prorated depreciation** parameter in **Fixed asset parameters**.  
...
> [!IMPORTANT]  
> For legal entities in Türkiye, you must use the **Prorate depreciation** parameter instead of the standard **Calculate prorated depreciation** parameter.  
> The standard parameter adjusts only the first year calculation method and does not change the depreciation start date or final year allocation.  
