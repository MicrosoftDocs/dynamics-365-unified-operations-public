...
> [!NOTE]
> Billing schedules with projects is available in Dynamics 365 Finance version 10.0.32 and later.
...
2. Set up on the **Billing schedule lines**. 

On the **General** fastTab you can filter the billing schedules to be included by the **Start date** or **End date**. Leave the **From date** field blank to select all billing schedule lines until the **To date**. 
 - Set **Exclude zero consumption** to **Yes** to not include usage type billing schedule lines with zero consumption, which have 0.00 net amount, in the sales order lines. 
 - Set this to **No** to include the 0.00 net amount line on the sales order. 
...
-  Select the **Posting option**:  
   -   **Create sales order** -  creates the sales order(s) for the billing schedule lines.
   -   **Show Posting invoice or Create invoice proposal page** - creates the sales order and opens the **Posting invoice** page. You can review and manually post the invoice or save it and post it at a later time. When a project exists for any of the lines, a project invoice proposal is created for the sales order.  
...
Specify the date to create the invoice in the **Invoice date**. The default date is the session date for Microsoft Dynanmics 365 Finance. When the **Posting option** is **Create sales order**, the invoice date is not applicable and can't be edited. 

Set **Don't print child items** to **Yes** if you want to suppress the child items when using revenue split on the billing schedule lines. All child items are hidden from the sales invoice and only the parent item will appear on the invoice. If the net amount of the (hidden) child items has a non-zero sum, the net amount of the parent line shows a summary of the child lines. 

Select **No** to print the parent and child items when using revenue split. All child items appear below the parent item on the sales invoice. The net amount for each child item appears on the invoice. This can only be used when all (parent and child) items in a revenue split are included on the invoice. 
...
The process for determining the top items for invoices created from the **Generate invoice** page is:
...
-   The Revenue Allocation button for billing schedules on any page isn't available
-   The unbilled revenue feature can't be used
-   The Unbilled revenue button on the **Billing schedules** page isn't available.
-   On the **Generate invoice**, the **Consolidate by Item** and the **Split by item group** options can't work together (**Split by item group** is **Yes**, **Consolidate by item** is automatically set to **No** and can't be changed to **Yes**). 
