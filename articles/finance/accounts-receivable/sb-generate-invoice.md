...
- **Consolidate by customer** – Set this option to **Yes** to combine billing schedules lines if the customer ID, document ID, customer reference, project ID (if it's used), and currency ID are the same. Terms of payment, payment method, or payment schedule.
