...
Feature management enables the Revenue recognition feature by default. If your organization doesn't use this feature, you can turn it off in the **Feature management** workspace.
Revenue recognition, including bundle functionality, isn't supported in Commerce channels (e-commerce, POS, and call center). Items that are configured for revenue recognition shouldn't be added to orders or transactions that were created in Commerce channels.
...
A new journal type is available for revenue recognition. You need this journal type for two scenarios.
...
Configure revenue recognition settings on the **Revenue recognition** tab of the **General ledger parameters** page (**Revenue recognition \> Setup \> General ledger parameters**). The following settings are available:
...
- **Post invoice corrections to Accounts receivable when reallocating** – When you reallocate for invoices that are already posted, you must correct the accounting entry for the posted invoice. Use this option to specify how the correction is done.

  - Set this option to **No** to limit posting of the correcting transaction to General ledger. When you set this option to **No**, no additional documents are created in Accounts receivable for the internal accounting correction. When the invoice is paid, the settlement process uses the old accounting entry to post any cash discounts, or any realized gains or losses.
...
:::image type="content" source="./media/revenue-recognition-setup-info.png" alt-text="Screenshot of revenue recognition setup information." lightbox="./media/revenue-recognition-setup-info.png":::

## Revenue schedules

Create a revenue schedule for each occurrence when you can defer revenue. For example, if your organization offers support over six-month, 12-month, 18-month, and 24-month periods, create a revenue schedule for each period. The setup of the revenue schedule determines how the revenue price is allocated across the number of periods that you select. It also determines the default dates that are entered for the revenue schedule when you post the invoice.
...
Create revenue schedules on the **Revenue schedules** page (**Revenue recognition \> Setup \> Revenue schedules**).

:::image type="content" source="./media/revenue-recognition-revenue-schedules.png" alt-text="Screenshot of the Revenue schedules page." lightbox="./media/revenue-recognition-revenue-schedules.png":::

Enter descriptive values in the **Revenue schedule** and **Description** fields. Use the following additional settings to create the revenue schedule when you post the invoice.

- **Occurrences** – Enter the number of months or occurrences for the revenue deferral.
- **Automatic hold** – Select this checkbox if all lines of the revenue schedule should automatically be put on hold when the invoice is posted. The hold must be manually removed from each line of the schedule before the line's deferred revenue can be recognized.
- **Automatic contract terms** – Select this checkbox if the contract start and end dates should automatically be set. These dates are automatically set only for released products of the **Post contract support** revenue type. The contract start date is automatically set to the sales order line's requested ship date, and the contract end date is automatically set to the start date plus the number of months or occurrences that is defined in the setup of the revenue schedule. For example, the product on the sales order line is for a one-year warranty. The default revenue schedule is **12M** (12 months), and the **Automatic contract terms** checkbox is selected for this revenue schedule. If the sales order line has a requested ship date of December 16, 2025, the default contract start date is December 16, 2025, and the default contract end date is December 15, 2026.
- **Recognition basis** – The recognition basis determines how the revenue price is allocated across the occurrences.
  ...
  - **Monthly** – The amount is allocated equally across the number of months that you define in the occurrences.
  ...
  - **Fiscal period by days** – The amount is allocated based on the actual days in each fiscal period.
    ...
    - The results of **Monthly by days** and **Fiscal period by days** are the same when the fiscal periods follow calendar months. The only exception is when the recognition convention is set to **End of month/period**, and the **Contract start date** and **End date** fields are left blank on a sales order line.
...
:::image type="content" source="./media/revenue-schedule-details-2nd-scrn.png" alt-text="Screenshot of the Revenue schedule details page." lightbox="./media/revenue-schedule-details-2nd-scrn.png":::

## Inventory setup

You can recognize revenue for released products on sales orders, but not with sales categories on the sales orders or with free text invoices, if the document doesn't include any items. The selections that you make when you set up released products determine how the item's revenue is recognized. For example, the selections determine whether the revenue price is allocated, and whether the revenue is deferred by using a revenue schedule.

Start the setup on the **Revenue recognition** FastTab of the **Item group** page (**Revenue recognition \> Setup \> Inventory and product setup \> Item group**). This page includes several setup fields. These fields set default values only for new released products that you create in the system. As you create new products, the values that you set here are entered by default for the item group. You can override the default values for released products on the **Released products** page (**Revenue recognition \> Setup \> Inventory and product setup \> Released products**). The default values that you set for the released products are then carried forward to the sales order.
...
After you finish configuring the settings for the released product, manually define the revenue price by entering the fair value price or the median price (if you're using median price method) on the **Revenue prices** page (go to **Revenue recognition \> Setup \> Inventory setup \> Released products**, and then, on the Action Pane, on the **Sell** tab, in the **Revenue recognition** group, select **Revenue prices**).

:::image type="content" source="./media/revenue-recognition-revenue-prices.png" alt-text="Screenshot of the Revenue prices page." lightbox="./media/revenue-recognition-revenue-prices.png":::

The revenue price that you manually define on this page is used to determine the revenue price allocation on each sales order, based on the criteria that you define. Each criterion is matched to the sales order line to determine the revenue price that should be used in the allocation process.

- **Item code** and **Item relation** – Define a revenue price for an individual product or a group of products. If you select **Table** in the **Item code** field, select the released product in the **Item relation** field. If you select **Group** in the **Item code** field, select the item group in the **Item relation** field.
- **Account code** and **Account/Group number** – Define a revenue price for all customers, an individual customer, or a group of customers. If you select **All** in the **Account code** field, the price is used for all customers. If you select **Table** in the **Account code** field, select the customer in the **Account/Group number** field. If you select **Group** in the **Account code** field, select the customer group in the **Account/Group number** field.
- **Currency** – Enter a separate revenue price for each currency that you enter a sales order in. For example, if you currently sell in U.S. dollars, Canadian dollars, and euros, you must define a revenue price in all three currencies. The revenue price isn't translated from one currency, such as the accounting currency, to any other transaction currencies that you're using.
...
On the **Setup basis** page, add a record for each item group that the item is supporting. When the revenue allocation occurs, the revenue price is distributed across the essential and nonessential parts for the PCS item.

### Posting profiles

Three additional posting types support the ability to defer revenue. Set up these posting types on the **Sales order** tab of the **Posting** page (**Revenue recognition \> Setup \> Inventory and product setup \> Posting**).
...
- **Partial invoice revenue clearing** – Enter the main account for the clearing account that's used either when the sales order is partially invoiced or when reallocation occurs. The balance in this account returns to 0 (zero) when the sales orders are fully invoiced.
...
Set up the components for revenue recognition, as if they're products that you can sell individually on a sales order. For example, make sure that you define the correct revenue price for each component, and that you set the price basis for PCS items.
...
- On the **Engineer** FastTab, in the **Production type** field, set the item as a BOM item.
- On the **General** FastTab, in the **Bundle** field, mark the item as a bundle item.
...
:::image type="content" source="./media/revenue-recognition-bom-scheduleds.jpg" alt-text="Screenshot of Released products BOM schedules." lightbox="./media/revenue-recognition-bom-scheduleds.jpg":::

If the bundle parent item and bundle components are set to allocate, the bundle revenue price is distributed to the components, based on their revenue contribution percentages.
