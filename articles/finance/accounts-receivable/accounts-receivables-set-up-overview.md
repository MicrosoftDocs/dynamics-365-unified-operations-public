...
> [!NOTE]
> You can't access some pages in the following table unless you enter data or parameter settings in other pages.
...
| | Customer groups | Create and maintain groups of customers who share key parameters. These parameters include terms of payment, settle periods, inventory posting ledger accounts, sales tax group, and default account setup. |
...
| | Payment days | Define the payment days that are used to calculate due dates for payments that you receive from customers or make to vendors. |
...
| | Collections team | Set up a collections team that represents the workers that you can set up as agents. If no team exists, the system automatically sets up a team called Collections in the Accounts receivable parameters. |
| | Customer aging snapshot | Create aging snapshots for customers. An aging snapshot contains the calculated aged balances for a group of customers at one point in time. This step requires an aging period definition to be set up. |
| | Customer contacts and email settings | Set up contacts for customers with their email addresses. These addresses appear in the Collections page and are used to create emails that go to customers. Also set up a default Collections contact for each customer that appears first in the Collections page. |
| | Customer pools | Set up customer pools, which are queries that define a group of customer accounts that you can display and manage for collections or aging processes. |
| | Customer posting profile | Set up the profiles that control the posting of customer transactions to the general ledger. |
| | Customer reason codes | Set up customer reason codes. |
| | Customer write-off reason codes | Set up customer write-off reason codes that are used for write-off transactions. |
...
| | NSF information. | Set up NSF information on the bank account that the system uses when a payment is marked as an NSF transaction on the Collections page. |
| | Salesperson information | Set up the email address for salespersons. These addresses appear on the Collections page and you can use them to send email to a salesperson from that page. |
