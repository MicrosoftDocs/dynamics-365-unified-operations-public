...
4. Select the **Start date** and type the **Number of periods**. The **End date** will be updated based on the number of periods specified. If the **Billing end date** is updated, the **Number of periods** will update to 0. 
5. Click **OK**. 
...
26. Select the **Item number**. If the item that is added is a parent item in a revenue split, the lines are automatically updated with the child items. You can only edit the parent item in a revenue split, and all child items are updated accordingly. 
27. Select the **Item type**. 
28. Update the start and end dates. 
29. Before the invoices are created, the **Billing frequency** can be changed. After the first invoice for the billing schedule is created, the billing frequency cannot be updated.
...
1. Select the **Schedule number** of the billing schedule that you want to edit. 
2. On the FastTab, select the line you want to delete, and select **Remove**. 
3. Select **Save**. 
...
|**Terminate**|This is only available when the selected lines have a status of **Active**. For revenue split items, only the parent item can be terminated. |
...
|**Escalation and discount**| Not available for items that are part of a revenue split, except for parent items where the revenue split uses the **Zero amount** allocation method. |
...
|**Item group**|Displays the item group for the line item. |
...
|**Account tab**|
|**Main account**|Displays the main account that is created on the sales line. The default value is from the sales order. This field can be empty. |
...
|**Termination type**|Displays the termination type. The default value is from the **Termination type** in the header. |
