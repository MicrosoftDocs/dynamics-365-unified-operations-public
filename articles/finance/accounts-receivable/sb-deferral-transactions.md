...
> [!NOTE]
> When you create a sales order from a billing schedule, all values on the **Deferrals** or **Create credit note** page are read-only, and you can't edit the deferral parameters. To edit the values, use the **Modify schedule** page.
...
1. Select the line, and then select **Deferrals**.
1. On the **Transaction deferral** page, set the **Deferred** option to **Yes**. Edit other fields as needed.
1. To preview the deferral schedule, select **Preview**.
...
After you post the transaction, open the **All deferral schedules** page to view the deferral schedule.
...
1. Create a sales order transaction.
1. In the **Sales order lines** section, select the item that you want to create a credit note for.
1. Select **Sales order line** \> **New**, and then select **Credit note**.
1. Select **Deferrals**.
1. On the **Sales order transaction deferral** page, set the **Adjust existing schedule** option to **Yes** for the item.
1. In the **Adjusted schedule** field, select the deferral schedule that you want to apply the credit note to.
1. Update the **Recalculate date** and **End date** fields as you require.
1. Select **OK**.
...
1. Use the **Pending vendor invoices** page to enter a vendor invoice.
    ...
    When you enter a line, the system automatically sets the deferral for the item or procurement category that you select. This deferral is based on the deferral setup on the **Deferral defaults** page. You can edit or add deferrals at the distribution level.

1. On the purchasing line, select **Financials \> Distribute amounts**.
1. For each distribution amount, select **Deferrals**. When you post the invoice, the system creates a deferral schedule for each distribution that a deferral is set for.

### Tax

In some cases, the tax amount can be fully or partially non-recoverable. If the tax amount of a deferrable line contains a non-recoverable amount, the non-recoverable tax amount is included in the deferrable schedule amount when you post the invoice.

### Variance

If the amount on the vendor invoice line differs from the amount on the purchase order line, the system creates variance distributions. If the line is deferred, the system sets the ledger account for these distributions to the deferral account, and includes the amounts in the deferrable schedule amount when the invoice is posted. These distributions are named **Price variance** and **Cost variance**.

### General journals and invoice journals

Use the **Deferrals** page to enter the deferral parameters for a journal voucher line. You can also preview the deferral schedule for straight line deferrals. When you enter a line, the system automatically sets the deferral based on the setup of deferral accounts on the **Deferral defaults** page. You can manually change the deferral options for each line. When you post the journal voucher, the system creates the deferral schedule.

#### Post and transfer

To post the journal voucher, use the **Post and transfer** command. The deferral options follow the line that the voucher applies to. For vouchers that don't have errors, the system creates a deferral schedule if it's deferred. For a voucher that has an error and are transferred, the system also transfers any deferral options.
...
Use the **Deferrals** page to enter the deferral parameters for a free text invoice line distribution. When you enter a distribution, the system automatically sets the deferral based on the deferral settings on the **Deferral defaults** page.
...
| Field | Description |
| ------- | ------------- |
| Deferred | <p>Specify whether the line is a deferral:</p><ul><li>**Yes** – The line is a deferral.</li><li>**No** – The line isn't a deferral.</li></ul><p>When you set this option to **Yes**, the **Adjust existing schedule** option isn't available. For items and charges that you set up as deferred, this option is set to **Yes**. For items and charges that aren't set up as deferred by default, set this option to **Yes**.</p> |
| Adjust existing schedule | <p>Specify whether the line is an adjustment to an existing deferral schedule:</p><ul><li>**Yes** – The new sales line is an adjustment to an existing deferral schedule. In this case, you can select a deferral schedule in the **Adjusted schedule** field.</li><li>**No** – The new sales line isn't an adjustment to an existing deferral schedule.</li></ul><p>When you set this option to **Yes**, the **Deferred** option isn't available.</p> |
| Adjusted schedule | <p>Select the deferral schedule that the line is adjusting. All values on the page are then updated with the values from the originating deferral schedule. You can't edit those values.</p><p>This field is available only when the **Adjust existing schedule** option is set to **Yes**.</p> |
...
| Recognition account | <p>Specify the account that is used when a deferral is recognized. This account must differ from the deferral account.</p><p>When you initially set the **Deferred** option to **Yes**, the system copies the dimension values that are used in the deferral account to the recognition account. If the dimension in the deferral account doesn't exist for the recognition account, it's ignored.</p> |
...
| Deferral account | <p>Specify the consumption deferral account number.</p><p>For standard products, two deferral schedules are created:</p><ul><li>**Standard sales** – A revenue schedule that has a credit balance. In this case, select the consumption deferral account.</li><li>**Consumption** – A consumption (cost of goods sold \[COGS\]) expense schedule that has a debit balance. In this case, select the consumption recognition account.</li></ul><p>When you post the invoice, the consumption amount is posted to the specified consumption deferral account. These fields aren't available for service items.</p> |
...
| Equal per period | <p>Specify whether the number of days in every period is equal or varies by period:</p><ul><li>**Yes** – Every period has the same number of days.</li><li>**No** – Periods don't have the same number of days.</li></ul><p>When you set this option to **No**, the number of days in a period is considered when the amount in each period for a deferral schedule is calculated.</p><p>The default value is from the **Revenue and expense deferrals parameters** page.</p> |
...
| Description | <p>Specify whether you want to separate events per unit:</p><ul><li>**Yes** – Separate the event lines so that there's one line per quantity.<p>For example, there are three event lines, and the invoice has a quantity of four. In this case, the resulting deferral schedule has 12 lines.</p></li><li>**No** – Don't separate the event lines.</li></ul> |
...
| Quantity | <p>Specify the allocation quantity. The default value for all lines is **0** (zero). If you update the quantity, the total quantity of all lines must be equal to or less than the quantity that is specified for the line item on the sales order.</p><p>This field is available only when the **Allocation type** field is set to **Variable quantity**.</p><p>If you change the quantity of the line item on the sales order so that it's less than the original quantity, and you create the invoice, fewer event-based lines are created. The total allocated quantity doesn't exceed the quantity that is specified on the originating sales order or billing schedule.</p> |
