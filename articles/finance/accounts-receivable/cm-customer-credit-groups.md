---
title: Customer credit groups
description: Learn about customer credit groups, including a step-by-step process that outlines how to set up customer credit groups.
author: JodiChristiansen
ms.author: twheeloc
ms.topic: article
ms.date: 08/05/2026
ms.custom:
ms.reviewer: twheeloc
audience: Application User
ms.search.region: Global
ms.search.validFrom:
ms.search.form:
ms.dyn365.ops.version: 
---

# Customer credit groups

[!include [banner](../includes/banner.md)]

You can define groups of customers who share a credit limit. The system also considers the individual credit limit that you define on the customer invoice account.

Members of a customer credit group can be selected from different legal entities. When you add a customer to the list of customers in the customer credit group, the expiration date of the credit limit for each customer is changed to the expiration date that is assigned to the group.

You can set up customer credit groups on the **Customer credit groups** page (**Credit management \> Customer credit groups \> Customer credit groups**).

1. In the **Group number** and **Description** fields, enter an identifier and description for the group.
1. In the **Credit limit** and **Currency** fields, enter the credit limit and currency that the system should use when it checks the credit limit for any member of the group.
1. In the **Credit limit to date** field, enter the date when the credit limit expires. Customer credit groups must have an expiration date.

After you finish setting up a customer credit group, add customers to it by specifying their legal entity and customer account ID. When you add a new customer to a customer credit group, the system searches for the same customer account across all legal entities and prompts you to add it to the customer credit group.

Use the **Aged balances** menu to view the details of the aging balance for all invoice customers in a customer credit group. The **Aging balance** page shows a summary of the aged balances for the invoice customer accounts.

[!INCLUDE[footer-include](../../includes/footer-banner.md)]
