...
1. On the **Recognition processing** page, set the **Recognition action** field to **Create recognition journal**.
1. In the **Cutoff date** field, select the cutoff date. The processing includes only lines where the end date is earlier than or the same as the selected cutoff date.
1. Select **Filter**, and add data filters so that the list shows only the range of records that you want.
1. Select **View preview** to view the lines.
1. In the list of lines, select any lines that you don't want to process, and then select **Remove**.
1. Select whether you want to summarize the recognition journal entry.
1. In the **Transaction date** section, you can override the transaction date with a specific date to process the transaction. You can specify the transaction date for closed periods.
1. To do the processing as part of a batch, select **Batch**. In the **Batch processing** dialog box, set the parameters for the batch, and then select **OK** to return to the **Recognition processing** page. The revenue recognition is processed later, when the batch is processed.
1. Select **Process**. If you didn't add the transaction to a batch, the system immediately processes all lines. Otherwise, the system processes the lines when the batch is processed.
...
1. On the **Modify schedule** page, edit the options that you want to change. Depending on the deferral schedule, you won't be able to edit all the options.
1. Select **Preview** to review the changes to the deferral schedule.
1. Select **OK**.
...
If you use a template, the skipped periods are ignored, and the template is used only to calculate the end date.
...
1. On the **Modify Account** page, select the account that you want to change (deferral, short-term, or recognition).
1. In the **New account** field, select the new account.
1. Select whether changes to the account create adjustment journal entries.
1. If you set the option to **Yes** in the previous step, select the journal name in the **Journal name** field, and specify the transaction date in the **Transaction date** field.
1. Select **OK**.
...
1. On the **Place hold** page, select whether you want to transfer the balance from the deferral account or the hold account.
1. If you selected to transfer the balance, select the journal name in the **Journal name** field, select the hold account in the **On hold account** field, and specify the transaction date in the **Transaction date** field.
1. Select **OK**.
...
1. In the **Journal name** field, select the journal name.
1. In the **Transaction date** field, specify the transaction date.
1. Select **OK**.
...
1. On the **Cancel unrecognized amount** page, select whether you want to create cancellation entries.
1. If you select to create cancellation entries, select the journal name in the **Journal name** field, select the cancellation account in the **Cancellation account** field, and specify the transaction date in the **Transaction date** field.
1. Select **OK**.
...
1. On the **Cancel complete schedule** page, select whether you want to create cancellation entries.
1. If you selected to create cancellation entries, select the journal name in the **Journal name** field, select the account in the **Cancellation account** field, and specify the transaction date in the **Transaction date** field.
1. Select **OK**.
...
1. On the **Recognition processing** page, set the **Recognition action** field to **Reverse recognition journal**.
1. In the **Cutoff date** field, select the cutoff date. The processing includes only lines where the end date is earlier than or the same as the specified cutoff date.
1. Select **Filter**, and add data filters so that the list shows only the range of records that you want.
1. Select **View preview** to view the lines.
1. In the list of lines, select any lines that you don't want to process, and then select **Remove**.
1. The **Name** and **Description** fields are automatically updated with the default journal name and description. You can change the values as you require. You can also select whether you want to summarize the recognition journal entry.
1. In the **Transaction date** section, you can override the transaction date with a specific date to process the transaction. You can specify the transaction date for closed periods.
1. To do the processing as part of a batch, select **Batch**. In the **Batch processing** dialog box, set the parameters for the batch, and then select **OK** to return to the **Recognition processing** page. The reverse revenue recognition is processed later, when the batch is processed.
1. Select **OK**. If you didn't add the transaction to a batch, all lines are immediately processed. Otherwise, the lines are processed when the batch is processed.
...
> [!NOTE]
> When you create a credit note from the **Sales order transaction deferral** page and set the **Adjust existing schedule** option to **Yes**, the credit note is automatically applied to the schedule when you post the credit note.

1. On the **All deferral schedules** page, select the deferral schedule.
1. In the **Credit adjustments** list, select a line, and then select **Apply**.
1. On the **Apply credit note (deferrals)** page, specify the recalculation date in the **Recalculation date** field and the end date in the **End date** field.
1. In the **Header** list, select the **Sales order** that has credit notes.
1. In the **Lines** list, select the line.
1. Select **OK**.
1. Select **Yes**.
...
1. On the **All deferral schedules** page, select the deferral schedule.
1. In the **Credit adjustments** list, select the invoice, and then select **Unapply**.
1. Select **Yes**.
