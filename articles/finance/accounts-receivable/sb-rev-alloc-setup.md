...
| Option | Description |
| -------- | ------------- |
| Amount | The standalone selling price is an amount that you specify that's more than 0 (zero). The amount is converted between the functional and originating currencies as required. |
...
| Allocate residual amount | <p>The origin of the standalone selling price is calculated as *Total contract value of parent item* – *Total standalone selling price of child items*:</p><ul><li>*Total contract value of parent item* is the net or billed amount.</li><li>*Total standalone selling price of child items* is the sum of the extended or contract standalone selling price of all child items, except the child item that uses this standalone selling price origin.</li></ul><p>If the calculated amount is a negative value, the amount is set to 0 (zero).</p><p>**Note:** You can select this option for only one child item in the revenue split.</p> |
| None | The origin of the standalone selling price is based on the child items. This option applies to items that you define as parent items in a revenue split template. If you select the **Revenue split** checkbox, this option is automatically selected, and you can't change the setting. |
...
| Percent of parent standard price | The origin of the standalone selling price is a percentage of the standard price of the parent item. You can change the default value. This option is available only for child items in a revenue split template. It's the default option for child items. When you change the option for a child item from **Percent of parent standard price** to **Percent of parent invoice price**, or from **Percent of parent invoice price** to **Percent of parent standard price**, the calculated values are also updated. |
...
Use the **Item standalone selling price** page to specify the origin and standalone selling price of an item. The values that you specify on this page are used as the default values on the **Revenue allocation** page in multiple element revenue allocation and recurring contract billing.
...
1. Follow one of these steps, depending on the value that you selected in the **Standalone selling price origin** field:
...
1. For each item that you want to add to the list, select **Add**.
1. In the **Item code** field, select the item code. In the **Item relation** field, select the item relation. For items where the **Revenue split** checkbox is cleared, the selected combination of an item code and an item relation must be unique.
1. Change the default value of the **Standalone selling price origin**, **Standalone selling price**, or **Percent** field as you require.
1. For each parent item that you want to add to the list, select **Add**.
1. In the **Item code** field, select the item code. In the **Item relation** field, select the item relation.
1. Select the **Revenue split** checkbox.
1. In the **Item relation** field, select the item relation. The list is updated with the parent item and all child items.
1. For the child item, change the default value of the **Standalone selling price origin**, **Percent of parent standard price**, **Percent of parent invoice price**, or **Allocate residual amount** field as you require.
1. To add several items at one time, select **Add items**.
1. Update the query to select the range of items that you want to add.
1. Select **OK**, review the list of items that you added, and select **OK**.
