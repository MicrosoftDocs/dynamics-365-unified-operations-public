...
Use the **Recurring contract billing parameters** page to set up the default values for billing schedules that you create in Recurring contract billing. Go to **Subscription billing** > **Recurring contract billing** > **Setup** > **Recurring contract billing parameters**. All billing schedules that you create initially use these default values. However, you can change the values for each billing schedule as needed.
...
2. In the **Termination type** field, select how the final invoice is calculated when a billing schedule is terminated:
    ...
    - **Adjust schedule** – Cut off the billing schedule on the termination date, change the status of the schedule to **Last billing**, and adjust the associated deferral schedule by reversing the amount that no longer must be recognized. If the billing start date is after the termination date, remove the remaining billing periods.
    - **Bill remaining** – Add any remaining amounts in the billing schedule to the termination period, change the status of the schedule to **Last billing**, and update the deferral schedule. If the billing start date is after the termination date, add the total amount of all remaining billing periods to the billing period, and remove the remaining billing periods.
    - **No adjustment** – Terminate the billing schedule on the termination date. Don't make any adjustments to the billing schedule.
...
4. Set the **Align to month** option to **Yes** to align the billing schedule detail lines with the end of a month when you create or update a billing schedule. Set it to **No** to use incremental dates.
...
12. Set the fields for sales order creation:
    ...
    - Consolidate invoices by period, customer, or item. Set any combination of **Yes** and **No** values. Split invoices by item group.
    - The following posting options are available for invoices:

        - **Create sales order** – Create only the sales order.
        - **Show Posting invoice or Create invoice proposal page** – Invoice the sales order, and open a page where you can manually post the sales order invoice. If you're using a project on the billing schedule, open the **Invoice proposal** page, and you can manually post it.
...
13. Set the fields for support and renewals:
    ...
    - In the **Default support and renewal start date** field, specify the date to use as the default start date for support and renewals:
        ...
        - **Beginning of next month** – Use the first of the next month as the start date. If the transaction date is the first, use the first of the current month.
...
15. Set the **Track price change** option to **Yes** to record the price changes on billing schedule lines. If you manually change a billing schedule line from **Standard** to **Flat**, and enter a new price, the system tracks audit information on the billing schedule line. Set the option to **No** if you don't want to track these changes.
16. Set the **Default new line to active when header status is on hold** option to **Yes** if new billing schedule lines should have a status of **Active** when you add them to a billing schedule header that has a status of **On hold**. Set the option to **No** if new billing schedule lines should have a status of **On hold** in this situation.
17. Specify whether the system filters records on the **Generate invoice** page by start date or end date by default.
...
Use the **Sequence number** tab on the **Recurring contract billing parameters** page to set the default value for billing schedule numbers. Set up the default values on the **Number sequences** page (**Organization administration** > **Number sequences** > **Number sequences**).

## Set up billing schedule groups

Use the **Billing schedule group** page to create a billing schedule group for Recurring contract billing. Go to **Subscription billing** > **Recurring contract billing** > **Setup** > **Billing schedule group**. When you create a new billing schedule and apply a billing schedule group to it, the values you define on the **Billing schedule group** page become the default values for the billing schedule. You can change any of the default values for the specific billing schedule that you create. Set up multiple billing schedule groups, and then assign one of them as the default billing schedule group on the **Recurring contract billing parameters** page.
...
4. In the **Billing frequency** field, specify how often the billing schedule bills to a customer: **One-time**, **Daily**, **Monthly**, **Quarterly**, **Semiannually**, or **Annually**.
...
6. In the **Pricing method** field, select the default pricing method for items on the billing schedule:
    ...
    - **Standard** – Calculate the unit price based on the total quantity that you enter, and use the standard pricing from the **Released products** page in Product information management.
    - **Flat** – Apply a flat price that you enter on the billing schedule line.
...
8. Set the **Invoice separately** option to **Yes** to create a combination of consolidated invoices and separate invoices for the same customer. For example, a customer has five billing schedules. Consolidate three of them on a single invoice, but each of the other two requires a separate invoice. Set the option to **No** to create only one consolidated invoice for the customer.
...
Use the **Item group setup** page to define item groups for support and renewal proration, alignment dates for renewal items, and the top billing functionality. Go to **Subscription billing** > **Recurring contract billing** > **Setup** > **Item group setup**.
...
- **Adjust renewal dates** – Select whether the item group determines the alignment date for a renewal item. Select the checkbox to update the renewal start and end dates based on the specified alignment date.
- **Number of top lines** – Specify the most expensive items to charge on an invoice for the specific item group for the top billing functionality. This number must be zero or a positive value. A zero value indicates that the top billing feature isn't used. If used, this number typically ranges from zero to six items.
