...
7. The default currency for the fee is the currency of the legal entity. This currency code can be different than the invoice currency. You must set up a collection letter sequence for every currency that you want to create a collection letter. The system only creates collection letters for transactions in the currencies that are specified in a collection letter sequence, even if no fee is specified.
