...
Beginning with **Dynamics 365 Finance version 10.0.46**, the fixed assets acquisition proposal creates **Inventory sold** transactions when a fixed asset is acquired through the **Fixed assets acquisition proposal journal** in scenarios where the *same fixed asset ID* is linked to more than one inventory item.
