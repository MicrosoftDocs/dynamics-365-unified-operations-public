...
description: Learn about the method that is used in Fixed assets to calculate depreciation after an asset is split by using the reduce balance method.
author: moaamer
ms.author: moaamer
ms.topic: article
ms.date: 05/27/2026
...
The amount to depreciate for the original asset is $1,822.25. This amount equals the NBV before the split transaction is posted ($9,111.25), plus the acquisition adjustment that the system generates during posting of the split transaction (-$8,000), plus the depreciation adjustment that the system generates during the split transaction ($711). Therefore, the depreciation for the second year is (1,822.25 × 18 percent) ÷ 12 = $27.33.
