...
-	Asset book fields: Map fields such as acquisition method and depreciation. 
   ...
   > [!NOTE]
   > The Copy and Map options enable Assign field values to define so you can define the constant value or mapping values.
...
3.	Launch the wizard and follow the steps on the screen.
4.	The Asset transfer status reflects the progress of the transfer process. 
o	Started: A transfer is created.
o	Assets generated: On the Review journal details step, the destination assets are created but no journal exists yet.
o	Journal created: The wizard is completed and the journal is successfully generated.
o	Journal posted: The journal is posted.
o	Assets generated: All generated journals are deleted before posting.
Select assets
...
[!NOTE] Confirm that there are mappings between legal entities to avoid errors.
...
o	Acquisition and depreciation value: Records the acquisition of the destination asset by using the source asset’s original acquisition and accumulated depreciation amounts separately.
o	Net book value: Records the destination asset acquisition as a single amount equal to the source asset’s net book value.
o	Transfer date: This value reflects the posting date for the transaction, the acquisition date for the destination asset, and the date the source asset is removed from the source company’s books.
