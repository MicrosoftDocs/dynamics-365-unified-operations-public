...
Before running the data upgrade, understand the two options available for upgrading depreciation book journal lines to transaction vouchers, and the number sequence that's used for the voucher series.
...
- **Number sequence code** – The code of the number sequence.
  ...
  - Default value: No default, update to the number sequence code.
...
*// A new number sequence is created with these parameters.*
...
*// Voucher allocation goes row-by-row for existing number sequences.*
...
*// The default system-defined number sequence is used if a number sequence code with the specified scope isn't found.*
...
When you use the system-generated number sequence approach (option 1), the upgrade uses set-based processing to allocate the voucher numbers as specified in the upgrade script parameters. The upgrade also creates a new number sequence with specified parameters after the allocation.
