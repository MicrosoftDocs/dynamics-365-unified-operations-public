...
- Acquisition is often used as the transaction type for the derived books. Use this transaction type when the book and the derived book should apply to the fixed asset from the time of the acquisition of the fixed asset.
...
> [!WARNING]
> Depreciation that is posted in the derived book is the same amount that was posted for the primary book. If the depreciation methods differ between the books, you shouldn't use the derived process to generate depreciation transactions.

In Microsoft Dynamics 365 Finance version 10.0.41, the **Consider split transaction in derived books posting** feature automates the process of splitting transactions from the main book to designated derived books based on asset book configurations. This feature ensures accurate recording across primary and secondary ledgers without manual input. It also helps maintain consistency. After you enable the feature, the system automatically posts split transactions according to the predefined settings in the asset book. Therefore, no manual intervention is required, and derived books remain updated in real time.
...
1. On VM 1, select the **Derived books** tab. Select VM 2 in the **Book** field, and **Acquisition** in the **Transaction type** field.
...
> [!NOTE]
> If you don't use derived books, you must post the acquisition of the fixed asset both for book VM 1 and book VM 2.
