...
For example, if there are 4,000 fixed assets and two books are associated with each asset, one book is posted to the current layer, and the other book is posted to the tax layer. If you acquire 4,000 fixed assets through batch processing, the batch job that creates one fixed asset acquisition journal contains 4,000 asset books.

> [!NOTE]
> The journal that is created continues to be used until the batch job is completed.
>
> The derived books aren't included in the maximum number of books per journal.

You can use  batch processing to run depreciation for the same set of acquired assets. For example, a batch job creates two depreciation journals, each of which consists of 2,000 asset books. The first journal contains books that are associated with the fixed assets that are numbered 1 through 2,000. The second journal will then contain books that are associated with the fixed assets that are numbered 2,001 through 4,000.

The batch processing job excludes closed books. For example, in a batch job for depreciation, 10 of the first 2,000 books are closed. In this case, the first journal contains books that are associated with the fixed assets that are numbered 1 through 2,011. The second journal then contains books that are associated with the fixed assets that are numbered 2,012 through 4,000.
...
For example, there are 5,001 fixed asset IDs, three books are associated with each fixed asset ID, and each asset book is posted to the same posting layer. You run depreciation for three consecutive months, without summarizing. The depreciation journal is created through a batch job, and the system creates seven journals that have 667 fixed asset IDs and three books for each fixed asset ID. The result is 2,001 books. Therefore, in three months, there are 6,003 journal lines to maintain the same asset IDs in the same journal. The system also creates one journal that has 332 fixed asset IDs and three books for each fixed asset ID. In three months, there are 2,988 lines.
