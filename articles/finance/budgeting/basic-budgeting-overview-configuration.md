...
Functionality that was introduced in Microsoft Dynamics 365 Finance version 10.0.7 (January 2020) lets you add functionality that provides additional flexibility. To enable this functionality, go to the **Feature management** workspace and select **Budget register entries for quantity only** and/or **feature2**.

The **Budget register entries for quantity only** feature enables the ability to post a budget register entry with quantity-only amounts. For example, you could post a budget entry of 32 quantity with a price of zero, resulting in an amount of zero. You may then use this quantity within the context of a financial reporting report to determine a price per quantity. Note that no inquiries or reports were updated as part of this feature; the only updated functionality was the ability to post an amount that results in zero. 

The **Budget register entries defaulting of amount type** feature enables the defaulting of amount type within a budget register entry to have some intelligence and not always default as expense. The budget register entry line now will default to expense when the main account is of type expense, will default to revenue with the main account type is of expense, and will default to expense for all other types. 
