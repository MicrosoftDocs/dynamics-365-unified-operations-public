...
> [!NOTE]
> If your financial dimensions include any wild characters, such as underscore (`_`), when defining budget control rules, user should use asterisk (`*`) for the criteria definition to ensure the accurate expression is found. For example, if you have Department as Department_1, when defining budget control rule, you should use expression Deparment is like Deparment*1. The asterisk (`*`) will match any number of characters, for example, *Department_1* as well as *Deparment_A1* or *DeparmentA_1*.
