...
description: Learn about classification problems and accuracy in machine learning (ML) models. Enhance your understanding of accuracy in ML prediction results.
...
Most discussions about the confusion matrix focus on binary classifiers, as in the preceding example. This case is a special case where you can consider other metrics, such as sensitivity and recall.

Next, consider a classification problem for a finance scenario that has three states. The model predicts whether a customer invoice is paid on time, late, or very late. For example, out of 100 test invoices, 50 are paid on time, 35 are paid late, and 15 are paid very late. In this case, a model might produce a confusion matrix that resembles the following illustration.

![Model 1.](media/payment-prediction-matrix.png)

A confusion matrix provides more information than a simple accuracy metric. However, it's still relatively easy to understand. A confusion matrix tells you whether you have a balanced dataset where the output classes have similar counts. For the multi-class scenario, it tells you how far off a prediction might be when the output classes are ordinal, as in the preceding example about customer payments.
...
However, for a more thorough understanding, consider several challenges that are associated with accuracy. The usefulness of the metric depends on the context of the problem. A question that often arises in relation to model performance is, "How good is the model?" However, the answer to this question isn't necessarily straightforward. Consider the following confusion matrix (model 2).
...
To begin to address this question, consider the accuracy of a naïve guess. For a classification problem, a simple guess always predicts the most common class. For model 1, that guess is "on time," and it produces an accuracy of 0.50. The guess for model 2 is also "on time," and it produces an accuracy of 0.80. Because model 1 improves on the naïve guess by 0.73 – 0.50 = 0.23, whereas model 2 improves on the naïve guess by 0.83 – 0.80 = 0.03, model 1 is a better model, even though it has lower accuracy. The calculation reveals that effective assessment of a model's quality requires more context than the accuracy value.
...
Accuracy is an important tool for communicating with domain experts who aren't familiar with statistics. However, to make the information useful, provide additional context together with the accuracy value.

For the payment prediction scenario, set a target for the ML model that includes factors in different payment behaviors. The target is that the model should improve upon a naïve guess by reducing the number of incorrect answers by at least 50 percent. In other words, you want a target accuracy that splits the difference between the accuracy of a naïve guess and 100 percent.
...
The final consideration in this article is a more advanced measure of classification ML performance that's known as F1 accuracy.

Before defining F1 accuracy, introduce two additional metrics: precision and recall. Precision indicates how many of the total number of predictions that are specified as positive are correctly assigned. This metric is also known as the positive predictive value. Recall is the total number of the actual positive cases that the model predicted correctly. This metric is also known as sensitivity.

[![True results vs. false results.](./media/tn-fn.png)](./media/tn-fn.png)

In the confusion matrix in the preceding illustration, calculate these metrics in the following way:
...
Let's look at a concrete example. Earlier in this article, you saw an example of a model that predicted whether an animal was a dog or a cat. The illustration is repeated here.
...
Although F1 accuracy isn't as easy to understand, it adds nuance to the basic accuracy number. It can also help with unbalanced datasets, as the following discussion shows.
...
To simplify the calculations, sample arrays were built to match the actual and predicted values. These arrays used sklearn's metrics library in Python to calculate the values. Here's the result.
...
For more detail about how this calculation works, here's the sklearn.metrics classification report for model 1. The three states, "On time," "Late," and "Very late," are represented by the rows that are labeled 1, 2, and 3, respectively. The macro average is just the average of the "f1-score" column.
