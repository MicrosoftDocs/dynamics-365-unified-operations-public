...
You can enter intercompany transactions as either a single line or multiple lines on various financial journals, such as the general journal, the vendor invoice journal, and vendor and customer payment journals. When you enter an intercompany transaction on a single line, the system correctly enters default financial dimension values on the Due to and Due from ledger accounts. However, when you enter an intercompany transaction on multiple lines, the system doesn't correctly enter default financial dimension values. Why does the defaulting behavior work differently?
...
• You can summarize lines for both the originating company and the destination company.
...
USMF is the originating company. The voucher contains two destination companies, DEMF and USSI. Enter the following lines into the general journal and post them.
...
During posting, you can easily determine the financial dimension values for the destination company's Due to or Due from account. Because there's only one originating company, you create a balancing "due to" or "due from" entry for each destination company line. Therefore, you always take the dimensions from the destination companies' lines, as shown in the following sections.
