...
The sales tax framework supports many types of indirect taxes, such as sales tax, value-added tax (VAT), goods and services tax (GST), unit-based fees, and withholding tax. The system calculates and documents these taxes during purchase and sales transactions. Periodically, you must report and pay these taxes to tax authorities.

The following diagram shows the entities of the tax setup and how they're related.

:::image type="content" source="./media/taxoverview1-300x209.jpg" alt-text="Screenshot of diagram showing overview of tax setup entities.":::

For every sales tax that a company must account for, you must define a sales tax code. A sales tax code stores the tax rates and calculation rules for the sales tax.

Link every sales tax code to a sales tax settlement period. Sales tax settlement periods define the intervals at which sales tax must be reported and paid to the sales tax authority. Assign every sales tax settlement period to a sales tax authority. A sales tax authority represents the entity that sales tax is reported and paid to. It also defines the layout for the sales tax report. You can relate sales tax authorities to vendor accounts. For more information, see [Set up sales tax settlement periods](tasks/set-up-sales-tax-settlement-periods.md).

Link every sales tax code to a ledger posting group. A ledger posting group specifies the main accounts that amounts for the sales tax codes post to.

You can also define optional sales tax reporting codes. Assign these codes on sales tax codes for the various amount types that the system calculates for the sales tax code. The **Sales tax payment by code** report shows totals per sales tax reporting code for a given sales tax settlement period and interval.
...
|-----------------------------------------------------------------|----------------------------------------------------------------|
| Create main accounts                                  | Required. Before you can set up the sales tax functionality, create the main accounts that the company uses to pay and record taxes.  |
...
| Set up sales tax settlement periods                            | Required. Sales tax settlement periods contain information about when and how often sales tax must be reported and paid. They're related to a sales tax authority.                                                                                                                                                       |
| Set up sales tax reporting codes                               | Optional. Assign sales tax reporting codes to sales tax codes to report amounts for multiple sales tax codes under one sales tax reporting code. For more information, see [Set up sales tax reporting codes](tasks/set-up-sales-tax-reporting-codes.md).                                         |
| Set up sales tax codes                                         | Required. Sales tax codes contain the tax rates and calculation rules for each sales tax. Sales tax codes relate to a sales tax settlement period and a ledger posting group. For more information, see [Set up sales tax codes](tasks/set-up-sales-tax-codes.md).                                |
...
| Set up sales tax parameters on the application parameter pages | Required. Different areas, such as General ledger, Accounts receivable, and Accounts payable, must set up parameters for correct calculation of indirect taxes. Although most of these parameters have default values, modify them to fit each company's requirements.                                          |

## Sales tax on transactions

On every transaction (sales or purchase document lines, or journals), enter a sales tax group and an item sales tax group to calculate sales tax. Specify default groups in master data (for example, customer, vendor, item, and procurement category), but you can manually change the groups on a transaction if you need to. Both groups contain a list of sales tax codes, and the intersection of the two lists of sales tax codes determines the list of applicable sales tax codes for the transaction.
...
You must report and pay sales tax to tax authorities at regulated intervals (monthly, quarterly). You can settle tax accounts for the interval and offset the balances to the tax settlement account, as specified in the ledger posting groups. You can access this functionality on the **Settle and post sales tax** page. You must specify the sales tax settlement period that sales tax should be settled for.

After you pay the sales tax, balance the sales tax settlement account against the bank account. If the sales tax authority that you specify on the sales tax settlement period is related to a vendor account, post the sales tax balance as an open vendor invoice and include it in the regular payment proposal.
...
You typically settle and pay 2,500 to the tax authority when you post the invoice in June, even though you didn't receive the payment from the customer.

However, if you're using a conditional sales tax, you settle with the tax authority when you receive the payment from the customer on July 30.
