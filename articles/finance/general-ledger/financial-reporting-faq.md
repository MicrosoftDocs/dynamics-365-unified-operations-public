...
## What should I do if my beginning/opening balances in the multi-company consolidation report do not match the Trial Balance after running Year-End Close (YEC) without transferring dimensions?
This occurs because Financial Reporting (FR) does not support YEC without transferring balance sheet dimensions, which is also the recommended best practice. If YEC is run without transferring these dimensions, the beginning balance (BB) will be posted to a different dimension than the previous year’s balances, leading to discrepancies.
...
Financial Reporting is unable to access archived data; other data explorers must be utilized for this purpose. Reports generated for archived years will not return any data, although previously generated reports remain viewable without the ability to drill down into transaction details. It’s advisable to export or print these reports with transaction details before archiving. Regarding currency translation, it will disregard the opening amount carried forward from archived years, affecting translated retained earnings. This limitation is not a concern if currency translation is not used or if the opening amount is already zero.
