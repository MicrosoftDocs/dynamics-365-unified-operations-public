...
After you select the **Enable advanced awareness options** parameter, ledger transactions that have been settled are no longer be included in the opening balance of the next fiscal year when the general ledger year-end close is run. The exclusion of ledger transactions that are settled might present a challenge for customers at year-end close if a reporting currency is defined for the ledger.
