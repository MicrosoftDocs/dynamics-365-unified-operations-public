...
This article explains the options in the **Origin** field on the **Sales tax codes** page and how soles tax is calculated based on the selected option for a sales tax code.
...
The **Percentage of net amount calculation** is the default value in the **Origin** field. The sales tax is calculated as a percentage of the purchase or sales amount, excluding any other sales taxes.
...
If you select **Percentage of gross amount**, the sales tax is calculated as a percentage of the gross sales amount. The gross amount is the line net amount plus all taxes and fees for the line except the one tax with Origin = Percentage of gross amount.
...
> [!NOTE]
> Only one tax code with Origin = Percentage of gross amount can be used for a transaction. Otherwise, you will receive an error that the sales tax can't be calculated. 
...
> [!NOTE]                                                              
> Multi-level tax on tax calculations isn't possible. A tax can't be calculated based on a tax that's already calculated based on another tax. Multiple single level tax on tax codes can be calculated on a transaction. 

## Amount per unit
When you select **Amount per unit** in the **Origin** field, sales tax is calculated as a fixed amount per unit multiplied by the quantity entered on the document line. A unit has to be selected in the **Unit** field. The amount per unit is specified on the **Sales tax code values** page.

### Example
Sales tax code is set up as: USD 1.20 per unit = box on a sales invoice line. 25 boxes of an item are sold and the sales tax is calculated as 25 x 1.20 = 30.00.

> [!NOTE]
> If the transaction is entered ina different unit than the unit specified on the sales tax code, it's automatically converted based on the unit conversions that are set up on the **Unit conversions** page. 
...
#### Example 2
SALESTAX: Origin = Percentage of net amount. The **Calculate before sales tax** option isn't selected for the DUTY calculation. Net amount: 10.00 DUTY: 1 x 5.00 = 5.00 SALESTAX: 10.00 x 25% = 2.50 Total sales tax: 5.00 + 2.50 = 7.50 Total amount: 10.00 + 7.50 = 17.50
...
### Example
The tax rate is 20%. The invoice line shows a quantity of two items at 329.00 each. The return cost price of each item is 318.00. Net amount: (2 x 329.00) = 658.00 Sales tax: (658.00 - (2 x 318.00))  x 20% = 4.40
