...
For each sales tax code that you create on the **Sales tax codes** page, select the method of calculation to apply to the tax base amount in the **Origin** field.
...
If the net amount is 10.00, then DUTY 1 is 1.00 (10.00 &times; 10%) and DUTY 2 = 2.00 (10.00 &times; 20%). The amounts are as follows: Gross amount: Net amount + DUTY 1 amount + DUTY 2 amount (10.00 + 1.00 + 2.00) = 13.00. SALESTAX = 13.00 &times; 25% = 3.25. Total DUTIES and SALESTAX: 1.00 + 2.00 + 3.25 = 6.25. Total amount: 10.00 + 6.25 = 16.25.

> [!NOTE]
> You can use only one tax code where **Origin** = **Percentage of gross amount** for a transaction. Otherwise, you receive an error message that states that the sales tax can't be calculated.

## Percentage of sales tax

When you select **Percentage of sales tax** in the **Origin** field, the system calculates sales tax as a percentage of the sales tax that you select in the **Sales tax on sales tax** field. The system calculates the sales tax that you select in the **Sales tax on sales tax** field first. The system then calculates the second sales tax based on the first sales tax amount.
...
> [!NOTE]
> If you enter the transaction in a unit other than the unit that you specify on the sales tax code, the system automatically converts the unit based on the unit conversions that you set up on the **Unit conversions** page.
...
Set the document or journal to **Amounts include sales tax** = **No**. Transaction line amount: 10.00. Tax rate: 25%. Sales tax: (Transaction line amount &times; tax rate) &divide; (100 – tax rate) (10.00 &times; 25%) &divide; (100% – 25%) = 3.33. Tax base amount (origin amount): Transaction line amount = 10.00.
