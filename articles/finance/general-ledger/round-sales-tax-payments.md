...
Periodically, you need to report and pay sales tax to tax authorities. To complete this action, run the Settle and post sales tax process on the **Sales tax** page. Settle sales tax for a period against the sales tax accounts, and post the sales tax balance to the Sales tax settlement account. You can round the sales tax balance that you post to the Sales tax settlement account as required by tax authorities by setting up a rounding rule on the **Sales tax** page.

The rounding difference is posted to the Sales tax rounding account that you select in the Accounts for automatic transactions field in the General ledger.

The following example illustrates how the rounding rule on Sales tax authority works.
...
1. Select **Tax** > **Indirect taxes** > **Sales tax** > **Sales tax authorities**.
1. On the **General** FastTab, in the **Rounding form** field, select **Normal**.
1. In the **Round-off** field, enter 1.00.
1. When it's time to pay the sales taxes to the tax authority, go to **Tax** > **Declarations** > **Sales tax** > **Settle and post sale tax**. On the sales tax settlement account, you can see that the tax liability amount of **98,765.43** is rounded to **98,765**.
...
> [!NOTE]
> If you set the round-off value as 0.00, then:
>
...
| Rounding | Calculation process |
|----------|---------------------|
| round(1.015, 0.01) = 1.02 | 1. round(1.015 / 0.01, 0) = round(101.5, 0) = 102<br>2. 102 * 0.01 = 1.02 |
| round(1.014, 0.01) = 1.01 | 1. round(1.014 / 0.01, 0) = round(101.4, 0) = 101<br>2. 101 * 0.01 = 1.01 |
| round(1.011, 0.02) = 1.02 | 1. round(1.011 / 0.02, 0) = round(50.55, 0) = 51<br>2. 51 * 0.02 = 1.02 |
| round(1.009, 0.02) = 1.00 | 1. round(1.009 / 0.02, 0) = round(50.45, 0) = 50<br>2. 50 * 0.02 = 1.00 |

> [!NOTE]
> If you select Own advantage, the rounding always gives the legal entity the advantage.
