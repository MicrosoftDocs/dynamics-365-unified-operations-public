...
Asynchronous transfer of subledger batches requires that the batch server be set up, online, and working since batch tasks are created for immediate execution on the batch server. When the feature  **Subledger transfer to General Ledger performance optimization** is enabled the **Process automation** system batch job named **Process automation polling system job** must be enabled as mentioned here: [Process automation](../../fin-ops-core/dev-itpro/sysadmin/process-automation.md).
