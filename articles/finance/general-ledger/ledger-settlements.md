...
You can exclude settled transactions from inquiries and reports. By excluding these transactions, you can more easily analyze the open ledger transactions that make up the balance of the ledger account.
...
> [!NOTE]
> Beginning in Dynamics 365 Finance version 10.0.40, the **Awareness between ledger settlement** feature, along with its associated features **Automate ledger settlement process** and **Post foreign currency realized gains/losses for ledger settlements**, can be found on the **General Ledger parameters**, specifically under the **Ledger settlements** tab. You manage these features through parameters titled **Enable advanced awareness options**, **Enable process automation for ledger settlement**, and **Enable post currency realized gains/losses for ledger settlements** respectively.

## Set up ledger settlement

Select the main accounts for ledger settlement. Choose these main accounts in one of two ways:

1. Go to **General ledger** > **Ledger setup** > **General ledger parameters**.
1. On the **Ledger settlements** tab, select the charts of accounts that you want to select the main accounts from.
1. Select the main accounts for ledger settlement. Because charts of accounts are global, all companies that you assign the selected charts of accounts to have the same main accounts selected for ledger settlement.

–or–

1. Go to **General ledger** > **Periodic tasks** > **Ledger settlements**.
1. Select **Ledger settlement accounts**.
1. In the dialog box, select the charts of accounts and main accounts for ledger settlement. This dialog box is a shortcut. Any main accounts that you add here also appear on the **General ledger parameters** page.

You can use the same basic procedures to remove main accounts from ledger settlement at any time. Removal of a main account has no effect on previous ledger settlements. However, the main account and transactions no longer appear on the **Ledger settlements** page.
...
1. Set the filters at the top of the page:
    ...
    - Select a date range. Alternatively, select a date interval code to automatically fill in the date range. Don't settle ledger transactions that cross fiscal years. When the **Awareness between ledger settlement and year-end close** feature is enabled, you must complete ledger settlement within a fiscal year.
    - When the **Awareness between ledger settlement and year-end close** feature is enabled, you must settle ledger transactions for transactions in a single main account and the main account is a required filter.
...
1. Select **Display transactions** to show all the transactions that match the filters that you set and the list of accounts that you specified when you set up the chart of accounts list in the previous section.
...
1. Select lines for settlement. The value in the **Selected amount** field at the top of the page increases or decreases to reflect the total amount on the selected lines.
1. When you finish selecting transactions, select **Mark selected**. For each selected transaction, a check mark appears in the **Marked** column. Additionally, the value in the **Marked amount** field above the grid increases or decreases to reflect the total amount on the marked lines.
1. When the value in the **Marked amount** field is **0** (zero), select **Settle marked transactions**. The status of the marked transactions is updated to **Settled**.

    > [!IMPORTANT]
    > All transactions that you mark for settlement for the active legal entity are settled, even if they aren't currently shown on the **Ledger settlement** page because you applied a filter.
...
1. In the **Status** filter, select **Settled**.
1. Select lines for reversal.
1. Select **Reverse marked transactions**. The status of all transactions that have the same settlement ID is updated to **Not settled**.
...
1. In the **Status** filter, select **Settled**.
1. Highlight one line.
1. Select **View settlements**.
1. Select the lines for reversal.
1. Select **Reverse marked transactions**. The status of all transactions that have the same settlement ID is updated to **Not settled**.
1. Select the back arrow to go back to the **Ledger settlements** page. The **Not settled** transactions have a checkbox in the **History** column as a visual indicator that they were settled and unsettled.

    > [!IMPORTANT]
    > All transactions that have the same settlement ID are reversed, even if you don't mark them. For example, you marked and settled four lines. All four lines have the same settlement ID. If you mark one of those four lines and then select **Reverse marked transactions**, all four lines are reversed.

## Unmark for selected users

Select **Unmark for selected users** to unmark ledger settled transactions for all legal entities by user ID. For example, an accounting manager can unmark transactions for a user who left for vacation before they finished the settlement, or for a user who left the organization. This action enables another user to mark those transactions for settlement.
...
Starting with Finance release 10.0.44, you can review missing settlement accounts by using the **Review missing settlement accounts** page.
...
1. Select the fiscal year. The page displays ledger accounts that have ledger settlements but are missing from the **Ledger settlement** page in General ledger parameters.
1. On the **Review missing settlement accounts** page, select **Add to Ledger settlement accounts** to add the missing accounts to the **Ledger settlements** page in General ledger parameters.
1. If there are no missing accounts, the **No missing ledger settlement accounts found** message is displayed at the top of the page.
...
## Post foreign currency realized gains and losses for ledger settlements

In Finance version 10.0.36, the **Post foreign currency realized gains and losses for ledger settlements** feature is available in the **Feature management** workspace. This feature calculates and posts foreign currency realized gains and losses for settlements from the **Ledger settlements** page when the reporting currency values of the debits and credits differ. In Finance version 10.0.38, support is added for calculating and posting foreign currency realized gains and losses for settlement when the accounting currency values of the debits and credits differ. Before you can enable the **Post foreign currency realized gains and losses for ledger settlements** feature, you must enable the **Awareness between ledger settlement and year-end close** feature.

> [!NOTE]
> Beginning in version 10.0.40, the **Awareness between ledger settlement** feature, along with its associated features **Automate ledger settlement process** and **Post foreign currency realized gains and losses for ledger settlements**, can be found on the **General Ledger parameters**, specifically under the **Ledger settlements** tab. You manage these features through parameters titled **Enable advanced awareness options**, **Enable process automation for ledger settlement**, and **Enable post currency realized gains and losses for ledger settlements** respectively. When upgrading to version 10.0.40 or later, the new General ledger parameters are set to **Yes** if the feature was enabled in Feature management before the upgrade.

This feature also improves the usability of the ledger settlement process by simplifying the marking process. You no longer have to select and mark vouchers for ledger settlement in two steps. Instead, you can mark the voucher in the grid on the **Ledger settlements** page. The **Marked debits**, **Marked credits**, and **Difference** fields are shown to provide a clear indication of the totals for your work in progress. You must complete your work by settling the marked vouchers before you close the **Ledger settlements** page. Starting in version 10.0.38, you can view the **Marked debits**, **Marked credits**, and **Difference** fields in both the accounting and reporting currencies from the **Ledger settlements** page.

### Set up realized gain and realized loss account information

After you enable the feature, you must specify which ledger settlement accounts process realized gains and losses during settlement. Set the **Calculate realized gains and losses** option for each main account that's ledger settled.

1. Go to **General ledger** > **Ledger setup** > **General ledger parameters**.
1. On the **Ledger settlements** tab, select **Ledger settlement accounts**.
...
- For accounts of the **Balance sheet**, **Asset**, **Liability**, **Equity**, or **CA Common** type, the system posts a gain when the adjustment value is more than 0 (zero), and posts a loss when the adjustment value is less than or equal to 0 (zero).
- For accounts of the **Profit and loss**, **Revenue**, or **Expense** type, the system posts a loss when the adjustment value is greater more 0 (zero), and posts a gain when the adjustment value is less than or equal to 0 (zero).
