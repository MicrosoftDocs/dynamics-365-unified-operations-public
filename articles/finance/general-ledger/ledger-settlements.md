...
This feature will calculate and post foreign currency realized gains and losses for settlements performed from the **Ledger settlements** page when the reporting currency values of the debits and credits differ. To enable the **Post foreign currency realized gains/losses for ledger settlements** feature, the **Awareness between ledger settlement and year-end close** feature must be enabled.
...
After you enable the feature, each organization must determine which ledger settlement accounts will process realized gains and losses during settlement. 
...
By default, the option is set to **No**. When the option is set to **Yes**, the system will post foreign currency exchange rate realized gains and losses during the ledger settlement process. Foreign currency exchange rate realized gains and losses will be generated when the Amount in reporting currency value of the debits does not equal the Amount in reporting currency value of the credits. When the option is set to **No**, realized gains and losses will not be generated by the ledger settlement process. We recommend setting this option to **No** for your Accounts receivable and Accounts payable summary accounts, since foreign currency exchange rate gains and losses are typically realized in the subledgers. Additional consideration should be given to your **Cash** accounts that may be revalued in the **Cash and bank management** module.
...
In order to post a realized gain or loss, the number sequence information for the type **Ledger settlements** in the **General ledger parameters** page must be specified. (**General ledger > Ledger setup > General ledger parameters > Number sequences**)

### Post a realized gain or realized loss
During the ledger settlement process, the system will post realized gain and realized losses when the Amounts in reporting currency of the marked debits and marked credits differ. To determine whether the system should compute a gain or a loss, it takes into account the account type and the direction of the change. The system is responsible for calculating the adjustment value, which is derived from the formula: Adjustment Amount = 0 - (Debits - Credits). This resulting amount represents the adjustment required to balance the debits and credits to zero.The system will determine the gain or loss, based on the **Main account type** of the account that is being settled. For accounts of type **Balance sheet**, **Asset**, **Liability**, **Equity** or **CA Common**, a gain will be posted when the adjustment value is greater than zero and a loss will be posted when the adjustment value is less than or equal to zero. For accounts of type **Profit and loss**, **Revenue** or **Expense**, a loss will be posted when the adjustment value is greater than zero and a gain will be posted when the adjustment value is less than or equal to zero. 

A voucher that contains the accounting for the realized gain or realized loss information will be created, as of the settlement date. The financial dimensions for the accounting entries will be based on the voucher that is being settled. The system uses the **Settlement typical balance** setting to determine which voucher to retrieve the default dimensions from.  If multiple vouchers are being settled, the system will prorate the calculated realized gain or realized loss amount to the financial dimensions of the voucher, based on the transaction currency values. The transaction type for the posted voucher will be **Ledger settlement**.

### Reverse a settlement that contains a realized gain or realized loss
When a ledger settlement is reversed, any realized gain or realized loss vouchers that were recognized at the time of settlement will be fully reversed. The transaction type for the posted reversal voucher will be **Ledger settlement reversal**.  You cannot reverse a reversed ledger settlement. Instead, you can re-settle the vouchers. 

### Unrealized gains and losses
The foreign currency revaluation process in general ledger creates unrealized gains and losses transactions. If unrealized gain and losses exist for transactions that are ledger settled, they will be resolved the next time the foreign currency process is run. We recommend that unrealized gains and loss transactions from prior foreign currency revaluation runs are not marked for ledger settlement and that you perform foreign currency revaluations in general ledger, after ledger settlement.  The foreign currency revaluation routine will resolve the unrealized gain or unrealized loss. 
