...
The **Inventory** tab in the **Inventory posting profiles** page is used to control how inventory transactions that are not created from sales orders, purchase orders, or production orders, will post to the general ledger. In general, inventory transactions automatically post the physical update and financial update to the inventory simultaneously. One exception are transfer orders that separate physical and financial updates when you ship and receive the inventory. 

Examples of inventory transactions include the following:

| Transaction type | Receipt or Issue | P/F/Both | Description |
|-------------------------|-------------------------|-------------------------|-------------------------|
| Movement | Both | Both | A movement journal is an inventory journal that allows you to add or remove inventory. This journal functions the same as the Inventory adjustment journal. One key difference is that you specify the main account that offsets the entry to.</br>This journal is typically used to enter beginning balances and for one-off adjustments that need to be expensed. For example, removing inventory for a sales sample.</br>The physical and financial updates are made simultaneously when the journal is posted.</br>Positive quantities on the journal lines represent receipts and negative quantities represent issues. |
