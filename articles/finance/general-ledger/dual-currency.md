...
In addition, several modules are enhanced to track, report, and use the reporting currency in various processes. The affected modules include:
...
The posting logic changed for all transactions that generate an accounting entry to general ledger. Here's how the reporting currency was previously calculated:

Transaction currency amount > Accounting currency amount > Reporting currency amount

For example, you enter a transaction in the Canadian dollar (CAD) currency. The CAD amount translates to the accounting currency, which is the US dollar (USD). The USD amount then translates to the reporting currency, which is the euro (EUR). Therefore, the exchange rates must exist between CAD and USD, and between USD and EUR.
...
Various reports and inquiries now show both reporting currency amounts and accounting currency amounts. Not every report and inquiry is updated. For example, reports that show amounts only in the transaction currency didn't change.
...
The financial journals, such as the general journal and vendor invoice journal, now include more information about the reporting currency. The totals for the voucher and journal show the reporting currency. You can see the reporting currency's exchange rate on the **General** tab of the journal lines. You can override the reporting currency's exchange rate when you enter transactions.

## Vendor invoices, sales orders, and sales agreements

Vendor invoices, sales orders, and sales agreements have been updated to include a fixed exchange rate for the reporting currency. A fixed exchange rate can be defined for both the accounting currency and reporting currency when the transaction currency is different. When the accounting currency and reporting currency are the same, the fixed exchange rate is kept in sync by using the accounting currency’s fixed rate as the reporting currency’s fixed rate. The reporting currency fixed exchange rate can't be changed for this configuration. When the accounting currency and reporting currency differ, a fixed exchange rate can be defined for both the accounting currency and reporting currency during transaction entry. If the reporting currency hasn't been defined on the ledger, the **Reporting currency fixed exchange rate** field isn't enabled, and no reporting currency amount is calculated.
...
- Many reports and inquiries in General ledger already had the reporting currency, but a few didn't. One example is the **Trial balance** list page. This list page now includes columns for both the accounting currency and the reporting currency. The columns for the reporting currency are hidden if the accounting currency and the reporting currency are the same, or if no reporting currency was defined on the ledger.
...
- The aging process is updated so that an organization can view the aging buckets in either the accounting currency or the reporting currency.
...
Previously, the **Cash and bank management** module didn't track any reporting currency amounts for transactions that you posted against each bank account. After an upgrade, the system automatically tracks reporting currency amounts for any new transactions that you post. However, you must add reporting currency amounts to previously posted transactions in the subledger.

- **Upgrade consideration:** Because bank account transactions didn't previously track reporting currency amounts in the subledger, a wizard is available so that you can add reporting currency amounts to bank account transactions. This wizard doesn't post to the general ledger again. Instead, it takes the reporting currency amounts from the general ledger and updates them in the subledger tables.

  - To start the wizard, select **Cash and bank management** > **Setup** > **Add reporting currency amounts to bank account transactions**.
  - The wizard shows transactions for all bank accounts in the current company that have a reporting currency amount of 0 (zero). It only includes transactions that you posted before the upgrade.
  - For each bank account transaction, the wizard finds the corresponding accounting entries in the general ledger and enters a default reporting currency amount. Although you can edit the amounts, don't edit them. Otherwise, you might not be able to reconcile the bank account balances to the general ledger. The only time that you should edit an amount is if the reporting currency amount can't be found in the general ledger. In that case, the wizard shows an amount of 0 (zero) for the reporting currency.
  - If you select **Cancel** in the wizard, the system saves the bank account transactions and any changes to the reporting currency amounts until the next time that you run the wizard. However, the system doesn't update the reporting currency amounts on the bank account transactions. That update occurs only when you select **Finish** in the wizard. You can run the wizard multiple times. Therefore, you can fix any incorrect reporting currency amounts if you make a mistake.

- No processes in Cash and bank management changed, but various reports and inquiries were updated so that they show reporting currency amounts. The cash flow forecasting reports are an exception. They weren't updated to include the reporting currency amounts.

### Fixed assets

Previously, the **Fixed assets** module didn't track any reporting currency amounts for transactions that you posted against each fixed asset book. After an upgrade, the system automatically tracks reporting currency amounts for any new transactions. However, you must add reporting currency amounts to previously posted transactions in the subledger.

In addition, major changes were made to the depreciation process. These changes require user action after an upgrade. It's important that you read and understand the following changes, even if you aren't yet using Fixed assets.
...
- The way that the depreciation process determines the reporting currency amount has changed. The following scenario compares how depreciation previously determined the reporting currency amount and how it determines the reporting currency amount now.
    ...
    **Depreciation scenario**
    ...
    An asset is acquired and posted with the following amounts. The reporting currency amount is posted in the general ledger, but it isn't stored in the Fixed asset subledger tables.
...
- **Upgrade consideration:** Because fixed asset book transactions didn't previously track the reporting currency, a wizard was added so that you can add reporting currency amounts to fixed asset book transactions. This wizard doesn't post to the general ledger again. Because the way that the depreciation proposal calculates reporting currency amounts changed, you must run and complete the wizard for every company before an organization can enter any depreciation transactions.

  - To start the wizard, select **Fixed assets** \> **Setup** \> **Add reporting currency amounts to fixed asset books**.
  - The wizard shows transactions for all fixed asset books in the current company that have a reporting currency amount of 0 (zero). Only transactions that you posted before the upgrade are included.
  - The wizard doesn't pull the reporting currency amounts from the general ledger. As was described in the previous scenario, the reporting currency amounts that were originally posted to the general ledger incorrectly used the spot rate. Those amounts shouldn't appear in the fixed asset subledger, because the next depreciation calculation will use the incorrect amounts. Instead, the wizard finds the exchange rate on the date of the first acquisition. It then uses that exchange rate to recommend the reporting currency amount that should be posted in the subledger. For example, here's what the wizard might show for the previous scenario.
 ...
 | BUIL-00001  | 200\_SLLT | Depreciation   | 6/3/2025 | USD      | 50,000  | 50,000  | 2.5       |  25,000                   |
  ...
  - When the reporting currency amounts are updated in the subledger, you **must** set the **Have you updated all the reporting currency amounts on the fixed asset book transactions?** option to **Yes** and then select **Finish**. Depreciation calculations can't be completed until you set the **Have you updated all the reporting currency amounts on the fixed asset book transactions?** option to **Yes**. After that option is set to **Yes**, the wizard is no longer available.
...
- **Upgrade consideration:** New setup fields were added to fixed assets and are used in the depreciation calculation. You might have to update these fields before the next depreciation calculation.
...
- Because the depreciation proposal now calculates amounts in both the accounting currency and the reporting currency, the **Fixed asset journal** is updated so that it shows depreciation amounts in the reporting currency. For depreciation transactions, the transaction currency is always the accounting currency. Therefore, those amounts continue to appear in the **Debit** and **Credit** columns. Two new columns, **Debit in reporting currency** and **Credit in reporting currency**, are added to the grid.
  ...
  - If you enter a deprecation transaction type in the Fixed asset journal, the reporting currency amounts appear in the new columns. You can change those amounts.
  - If the accounting currency and the reporting currencies on the ledger are the same, the amounts stay in sync. If you change the **Credit** amount, the **Credit in reporting currency** amount automatically changes so that it matches.
  - If you enter any other transaction type in the Fixed asset journal, the **Debit in reporting currency** and **Credit in reporting currency** amounts never show, either before or after posting. The accounting currency and reporting currency amounts are still available on the voucher that posts to the general ledger.

### Consolidations

In Dynamics 365 Finance, feature management provides enhanced flexibility for consolidation and dual currency. To enable this functionality, go to the **Feature management** workspace and select **Enable dual currency functionality in General ledger consolidation**.

In General ledger consolidation, there's an option to consolidate either the accounting or reporting currency amounts from the source companies. If the accounting or reporting currency is the same as the accounting or reporting currency in the consolidation company, the amounts are copied directly rather than translated.
