...
If you don't import a data entity package, you can manually set up the EM functionality. In this case, set up the following elements:
...
Set up number sequences for both messages and message items. The system uses the number sequences to automatically number messages and message items. The numbers that you assign are unique identifiers for the messages and message items in the system. To set up number sequences for electronic messaging, go to **General ledger** > **Ledger setup** > **General ledger parameters**.

## <a id="types"></a>Message item types

Message item types identify the types of records that are used in electronic messages. Set up message item types by going to **Tax** > **Setup** > **Electronic messages** > **Message item types**.

## <a id="item"></a>Message item statuses

Message item statuses identify the statuses that apply to message items in the processing that you're setting up. Set up message item statuses by going to **Tax** > **Setup** > **Electronic messages** > **Message item statuses**.
...
Set up the message statuses that should be available in message processing. Set up message statuses by going to **Tax** > **Setup** > **Electronic messages** > **Message statuses**.

The following table describes the fields on the **Message statuses** page.

| Field name          | Description |
|---------------------|-------------|
| Message status      | Enter a unique name for the message status. Message statuses characterize the state of an electronic message at every moment. The name that you enter is shown on the **Electronic messages** page and in a log that is related to electronic messages. |
...
The EM functionality collects records from transactional tables in Microsoft Dynamics 365 Finance as message items. Use this feature to prepare the records for reporting and then report them. However, transactional tables sometimes don't provide enough information to fill in records in a way that meets the reporting requirements. To fill in all the information that must be reported for a record, set up additional fields. You can associate additional fields with messages and message items. Set up additional fields by going to **Tax** > **Setup** > **Electronic messages** > **Additional fields**.
...
| Counter     | Set this option to **Yes** if the additional field should contain a number sequence in an electronic message. The system automatically fills in the value of the additional field when an action of the **Electronic reporting export** type runs. |
...
On the **Values** FastTab, you can predefine values that an additional field can have. These values are then available for users to select. Therefore, they don't have to manually fill in during processing. The following table describes the fields.
...
By default, combinations of criteria that you define by the **Account/Group number**, **Account code**, **Effective**, and **Expiration** fields don't influence the selection of values for additional fields. However, an executable class can use these combinations to implement specific logic that calculates values for additional fields.
...
| Field                 | Description |
|-----------------------|-------------|
| Executable class      | Enter the name that you use during the setup of a message processing action that this class is called in relation to. |
| Description           | Enter a description of the executable class. |
| Executable class name | Select an X++ executable class. |
| Execution level       | This field is automatically set, because the value is predefined for the selected executable class. This field limits the level that the related evaluation runs on. |
...
Some executable classes have mandatory parameters that you must define before the executable class runs for the first time. To define these parameters, on the Action Pane, select **Parameters**. In the dialog box that appears, set the fields, and then select **OK**. It's important that you select **OK**. Otherwise, the parameters aren't saved to the database, and the executable class isn't called correctly.
...
On the **Datasources setup** FastTab, add a line for every data source that the process uses, and set the following fields.

| Field                  | Description |
|------------------------|-------------|
| Name                   | Enter a name for the data source. |
| Message item type      | Select the type of message item to use when you create records for the data source. |
| Account type           | Select the type of account to associate with records from the data source. |
| Master table name      | Select the table that is a data source. |
| Document number field  | Select the field that the document number comes from in the selected master table. The value of this field is used as the value of the **Document number** field for the message item. |
| Document date field    | Select the field that the document date comes from in the selected master table. The value of this field is used as the value of the **Message item date** field for the message item. |
| Document account field | Select the field that the document account comes from in the selected master table. The value of this field is used as the value of the **Account number** field for the message item. |
| Company                | This field is available when the **Cross-company queries for the populate records actions** feature is turned on in the **Feature management** workspace. Use this feature to set up cross-company data sources for the populate records actions. You can fetch data from multiple companies. |
| User query             | <p>If you set up a query by selecting **Edit query** above the grid, and you specify the criteria that must be applied to the selected master table that data is filled in from, this checkbox is automatically selected. Otherwise, all the records are filled in from the selected master table source.</p><p>When the **Cross-company queries for the populate records actions** feature is turned on in the **Feature management** workspace, and records must be collected from several companies, add a line for each additional legal entity that must be included in reporting. For each new line, select **Edit query**, and specify a related criterion that is specific to the legal entity that is specified in the **Company** field on the line. When you've finished, the **Datasources setup** grid contains lines for all the legal entities that must be included in reporting.</p> |
...
1. Find and select the **Cross-company queries for the populate records actions** feature in the list.
1. Select **Enable now**.

To set up the [populate records actions](#populate) for multiple companies whose data you want to include in reporting, follow these steps:

1. Go to **Tax** > **Setup** > **Electronic messages** > **Populate records actions**.

    When you enable the **Cross-company queries for the populate records actions** feature, the **Datasources setup** grid on the **Populate records action** page includes a **Company** field. For existing records that you created during the general setup of the [populate records actions](#populate), this field shows the identifier of the current legal entity.

1. In the **Datasources setup** grid, add a line for each subsidiary legal entity that you want to include in reporting, and set the following fields.

    | Field name             | Value |
    |------------------------|-------|
    | Name                   | Enter a text value that helps you understand where this record comes from. For example, enter **Data source name - Subsidiary 1**. |
...
1. For each new line, select **Edit query**, and specify related criteria for the legal entity that you specify in the **Company** field on the line.

## <a id="applications"></a>Web applications

Use web application settings to set up a web application so that it supports Open Authorization (OAuth) 2.0. OAuth is the open standard that lets users grant "secure delegated access" to the application on their behalf, without sharing their access credentials. You can also go through the authorization process by getting an authorization code and access token. Set up web application settings by going to **Tax** > **Setup** > **Electronic messages** > **Web applications**.
...
When you store an access token to a web application in the system's database in encrypted format, you can use it for requests to a web service. For security purposes, limit access to the token to the security roles that are allowed to address those requests. If anyone outside the security group tries to address a request, they receive an error that states that they aren't allowed to interoperate by using the selected web application. To set up the security roles that have access to the access token, use the **Security roles** FastTab on the **Web applications** page. If you don't define security roles for a web application, only a system admin can interoperate by using the web application.
...
Use web service settings to set up direct data transmission to a web service. To set up web service settings, go to **Tax** \> **Setup** \> **Electronic messages** \> **Web service settings**.
...
| Certificate                    | Select an Azure Key Vault certificate that you previously set up. |
| Web application                | Select a web application that you previously set up. |
...
| Request headers                | Specify request headers. A request header is an HTTP header that you can use in an HTTP request. It isn't related to the content of the message. |
...
| File name to send                         | Enter the name of the attachment to an electronic message that this action sends. If multiple attachments have the same original file name, the newest one is sent. If no attachment is found that has the specified original file name, the request is sent without content. This field is available only for actions of the **Web service** type. |
| File name                                 | Specify the name of the file that the action results in. This file can be the response from the web server or the report that is generated. This field is available only for actions of the **Web service** and **Electronic reporting export message** types. |
...
- **Create message** – Use this action type to let users manually create messages on the **Electronic message** page. You can't set up an initial status for an action of this type.
- **Populate records** – This action type must already be set up. Associate it with a populate records action to enable the action to be included in processing. It's assumed that this action type is used for the first action in message processing (when no electronic message was created in advance) or for an action that adds message items to a message that was previously created by the **Create message** action type. Therefore, for actions of this type, you can set up a result status only for message items. You can set up an initial status only for messages.
- **Message execution level** – Use this action type to set up an executable class that the system evaluates at the message level.
- **Message item execution level** – Use this action type to set up an executable class that the system evaluates at the message item level.
- **Electronic reporting export** – Use this action type for actions that generate a report based on an exporting ER configuration at the message item level.
- **Electronic reporting export message** – Use this action type for actions that generate a report based on an exporting ER configuration at the message level (for example, when a message doesn't have any message items).
...
The following table shows the result statuses that you must set up for different action types and response types.
...
Use the **Message item additional fields** FastTab to add predefined additional fields that are related to message items. You must add additional fields for each type of message item that the fields are related to. You can specify a default value that is assigned to the additional field during processing.

Use the **Message additional fields** FastTab to add predefined additional fields that are related to messages. You can specify a default value that is assigned to the additional field during processing.

Use the **Security roles** FastTab to set up the security roles that are predefined in the system for specific processing. Users who have a specific role sees only the processing that is defined for that role.
