...
Inventory subledger transactions create two entries in the general ledger through the physical posting and the financial posting. For more information, see [Physical and financial updates](../../supply-chain/cost-management/physical-financial-updates.md).
...
You receive a quantity of 5 on the first purchase order line. You receive the full quantity for the second line and no quantity on the third line of the purchase order. There's now a second inventory transaction related to the first purchase order line. The transaction for the second purchase order line is updated to **Received**, and the third transaction remains the same. The following diagram shows the relationship with the additional InventTrans record for purchase order line 1.
...
In this example, a voucher is posted to the general ledger; this is the physical voucher. The item model group is configured to post physical inventory, and all items use the same item model group. The inventory posting profile is using a single set of main accounts. A single voucher is created, and the InventTrans record will link both InventTrans 1 and InventTrans 2 to the same voucher.
...
The purchase order line 3 in the example is a serialized item. Ten serial numbers are registered for the purchase order during the receiving process. The inventory transaction is split into 10 inventory transactions. The following diagram shows the relationship and additional inventory transactions, each with their own serial number related to purchase order line 3.
...
In the preceding example, if you receive each serial number on a single product receipt, you create one additional voucher. The physical voucher field links to each serial number. The same link is true for the financial update when you invoice the purchase order.
...
generation for a sales or purchase order. Updates to the reference document aren't allowed after the financial date is populated. |
...
Each inventory transaction has a status that's displayed in either the **Receipt** or the **Issue** field. The field that's used depends on the type of inventory transactions. Receipts are transactions that increase the inventory. For example, a purchase order with a positive quantity or a sales order return with a negative quantity. Issues are inventory transactions that decrease the inventory. For example, a sales order with a positive quantity or a purchase order return with a negative quantity.
...
| Reserved physical | This status indicates that the inventory is allocated or reserved against a specific order. When you reserve inventory, it isn't physically available for other orders.                                 |
...
For more information about the inventory transactions, see [Inventory transactions detail](../../supply-chain/inventory/inventory-transactions-details.md).
...
1. Select the tab for the type of transaction. For example, select **Purchase order**.
1. Select the radio button for the posting type. For example, select **Purchase expenditure for expense**.
1. Select **New**.
1. In the **Item code** field, select an option for **Table**, **Group**, **All**, or **Category**. For example, to configure a posting profile for a specific item
    group, select **Group**.
     ...
     - If you selected **All** in step 5, the **Item relation** field is blank.
     - If you selected **Category** in step 5, the **Item relation** field is blank. Use the **Category relation** field to select the category the posting profile applies to.

1. In the **Account code** field, select an option for **Table**, **Group**, or **All**. For example, to apply the posting profile to all vendors, select **All**.
     ...
     - If you selected **All** in step 5, the **Account relation** field is blank.

1. To associate a particular tax group with the selected posting type, select a **Sales tax group**. If this field is blank, the posting type applies to all existing tax groups. Posting that's specified for tax groups only applies to sales and purchases transactions.
1. Specify the account number to post the account type to the **Main account** field. If you didn't create an account number for use as the accounting type, you can create a new account. You create a new account in the **Main account details** page in General ledger.
