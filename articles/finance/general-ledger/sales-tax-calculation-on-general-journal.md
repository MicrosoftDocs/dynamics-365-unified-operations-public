...
- Determine the sales tax amount that is stored in a temporary sales tax table.
...
If a voucher has a journal line where the account type is **Project**, all the journal lines in the voucher follow the same tax direction. The following illustration shows the rule. The following points show the possible tax directions for project accounts.

• If the sales tax code is use tax, the sales tax direction is Use Tax.

• If the sales tax code is exempt tax, the sales tax direction is Tax Free Purchase.

• If the sales tax code is intracom VAT, the sales tax direction is Sales Tax Payable.

• If the sales tax code is reverse charge, the sales tax direction is Sales Tax Payable.

Otherwise, the sales tax direction is Sales Tax Receivable.
...
If a voucher has a journal line where the account type is **Vendor**, all the journal lines in the voucher apply the same tax direction. The following points show the possible tax directions for vendor accounts.
...
If the sales tax code is exempt tax, the sales tax direction is Tax Free Sale. Otherwise, the sales tax direction is Sales Tax Payable.
...
Otherwise, if the journal amount is debit (positive), the sales tax direction is Sales Tax Receivable; if the journal amount is credit (negative), the sales tax direction is Sales Tax Payable.
...
1. Add a legal entity for which you want to override the **Sales tax direction**, and select **Sales tax**.
1. Select the sales tax direction:
