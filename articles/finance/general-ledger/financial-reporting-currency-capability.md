...
> [!Note]
> If no currency filter is specified when using Transaction currency, the system pulls all different currencies available. The filter defaults to “all currencies”. Be sure to specify a currency to avoid a meaningless aggregate number.
