...
In Microsoft Dynamics 365 Finance version 10.0.40, the  **Automate ledger settlements process** feature is enabled on the **General Ledger parameters** page, under the **Ledger settlements** page. This feature is managed through the **Enable process automation for ledger settlement** parameter. You can enable the **Enable process automation for ledger settlement** parameter only if the **Enable advanced awareness options** parameter has been enabled.
...
Starting in Finance version 10.0.40, enabling the **Enable post currency realized gains/losses for ledger settlements** parameter under **General ledger parameters** page allows recognition of realized gains and realized losses in the reporting currency during automatic settlement.

Ledger settlement automation defines the following details:

- When ledger settlements run
...
The remaining settings on the page are generic and define the occurrence pattern for this version of the ledger settlements. For example, if an occurrence is for Monday, you can define it so that it runs weekly, and you can select Monday as the day of the week when it runs. You can also enter an early schedule time, such as 2:00 AM, so that the process automation completes before the start of the next business day. Schedule the process automation so that it runs outside your normal working hours. In this way, you help reduce the impact on your accounting staff during the workday.
...
The next page in the wizard is the **Ledger settlements match criteria** page. Use this page to define the main accounts that the process automation occurrence includes, and the criteria that determine matching debits and credits.

### Account selection

The main accounts that you previously defined as ledger settlement accounts for the legal entity appear. (Define main accounts as ledger settlement accounts at **General ledger \> Ledger setup \> General ledger parameters \> Ledger settlements**.)

### Date

Select this option if the automatic ledger settlements process should consider the dates of the ledger debit and credit transactions. When you set this option to **Yes**, enter the **Allowed date difference**.

## Allowed date difference

Enter the number of days variance for the debit and credit transactions dates. The system calculates the variance as the number of days between the debit and credit transaction dates, searching before and after the selected transaction date. A value of 0 specifies that the transaction dates of the debit and credit transactions must match. This field is available when the **Date** field is selected.
...
- **Error** – The automation ran, but an error occurred. You can view the errors by selecting the **View results** button.
- **Completed** – The automation successfully ran. You can view the settlement results on the **Ledger settlements** page.

To view the matching results, go to **General ledger \> Periodic tasks \> Ledger settlements**. The **Automation rule** field on the **Ledger settlements** page shows the name of the automated ledger settlement scheduled task that was used to settle the transaction. Unsuccessful settlement attempts don't update the **Automation rule** value. If you manually reverse a transaction that was previously successfully settled by the automation process, the **Automation rule** value is removed.
...
The Automate ledger settlements feature runs the automation for transactions that are dated from the first day of the fiscal year to the current date when the occurrence runs. For fiscal years that end on December 31, you might have to adjust the execution date of your occurrence to ensure that it's run in December. For example, automation is set to run on the first day of every month. This automation will be run on December 1, 2025, and is scheduled to run on January 1, 2026. It's recommended to change the January 1, 2026 occurrence to December 31, 2025. This change will ensure that the transactions that are dated December 2 through 31 will be considered for automatic settlement.
...
The automation process includes all transactions that meet the criteria, except transactions that you manually mark for settlement on the **Ledger settlements** page.

## Reversal of transactions that the automation process settles

You can reverse a settlement that was made by the automated ledger settlement process. When a transaction that was settled by the automation process is reversed, the **Automation rule** value is removed.
