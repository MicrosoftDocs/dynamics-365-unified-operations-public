...
When you build processing, it's helpful to first define the processing actions and statuses that you set up. The following illustration shows the processing for this example.
...
1. Create the following message statuses:
...
1. On the line for the **New** status, select the **Allow delete** checkbox to let users delete messages that have this status.

## Create additional fields

1. Go to **Tax** > **Setup** > **Electronic messages** > **Additional fields**.
1. Add an additional field and its values.
1. Set the **User edit** option to **Yes** to let users edit the field.

![Additional fields](media/additional-fields.png)

## Create message processing actions

For this example, create the following message processing actions:
...
1. Create an action named **Create message**. On the **General** FastTab, in the **Action type** field, select **Create message**.
1. Create an action named **Update to Prepared**, and set the following fields:
...
1. Create an action named **Generate report**, and set the following fields:
...
1. Optional: To let users regenerate a report several times, create an action named **Update to initial status**, and set the following fields:
...
For this example, set up all the actions so that they run separately. The assumption is that you initialize every action.

1. Go to **Tax** > **Setup** > **Electronic messages** > **Electronic message processing**.
1. Add a record for your processing, and add all previously defined actions and an additional field.
1. (Optional) On the **Security roles** FastTab, define security roles for your processing to limit access to specific reporting.
1. Go to **Tax** > **Inquiries and reports** > **Electronic messages** > **Electronic messages**.
1. Select **New** to create a message. At this point, you can add dates and a description. You can also update the value of the additional field as you require.
...
1. To update the message status, select **Update status**. In the **New status** field, select **Prepared**, and then select **OK**.

    ![Updating the message status](media/update-status.png)

    The message status is updated to **Prepared**.

1. Generate the report by selecting **Generate report**.

    The report is generated, and the message status and action log are updated.

1. To view the generated report, select the **Attachment** button (paper clip symbol) in the upper-right corner of the page.
