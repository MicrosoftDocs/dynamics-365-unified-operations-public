...
Because of the flexibility of financial journals, you can enter a single voucher that represents one transaction but has multiple customers, vendors, fixed assets, projects, or bank accounts. Microsoft refers to this functionality as One voucher. The One voucher scenarios don't include transactions that include only ledger accounts. Those transactions go to general ledger, not to subledgers such as Accounts receivable, Fixed assets, and Bank.

Two categories of One voucher examples exist:

- The voucher contains multiple transactions that you enter as a single transaction. Here are some of the possible examples:
...
- The voucher contains one transaction that affects multiple nonledger account types. Here are some of the possible examples:
...
- **Transaction reversal** – If more than one subledger account type exists on a voucher, when a single subledger transaction is reversed, an incorrect accounting entry might be posted for the reversal in the general ledger. For example, if you acquire multiple assets in a single voucher and then reverse the acquisition of one of the assets, the general ledger accounting is incorrect for the reversal.
- **Reporting and inquiries** – If you include more than one subledger account type (for example, **Vendor** and **Customer**) on a voucher, reports/inquiries will show only the first account value that's found.
    ...
    :::image type="content" source="./media/Journal-voucher1.png" alt-text="Screenshot of a multiline voucher that contains four projects representing the invoice lines." lightbox="./media/Journal-voucher1.png":::

    Three of the four projects are posted to the same main account (601500). If you open the Accounting source explorer to view details about posted transactions for that main account, you notice that the project ID for all three lines is **000057**. This behavior is a known limitation of one voucher. The details don't correctly link each line to the appropriate project on the journal. Instead, the first account value that's found always shows on reports and in inquiries.

    :::image type="content" source="./media/Project2.png" alt-text="Screenshot that shows the details of the posted transactions for main account 601500." lightbox="./media/Project2.png":::
...
:::image type="content" source="./media/GL-parameters3.png" alt-text="Screenshot that shows the Allow multiple transactions within one voucher option on the General ledger parameters page." lightbox="./media/GL-parameters3.png":::
...
- **One voucher number only** – Every line that you add to the journal is included in the same voucher. The lines contain more than one customer, vendor, bank, fixed asset, or project.
- **In connection with balance** – Enter a multiline voucher where there's no offset account. The lines contain more than one customer, vendor, bank, fixed asset, or project.
...
| Correcting the accounting entry of a posted document | An organization might have to correct the Accounts receivable or Accounts payable ledger account for a posted invoice. Because the invoice is correct, it shouldn't be reversed. | Yes/No | If a correction must be made to the Accounts receivable or Accounts payable ledger account, an adjustment can be made directly to the ledger account. This approach requires that the adjustment be made during a "down time," so that the ledger account can temporarily allow for manual entry. One downside of this approach is that the vendor/customer to ledger reconciliation reports show a difference going in and out. The net amount is 0 (zero). |
...
As functional gaps are filled, Microsoft will communicate the new features to be used instead of One voucher. For some business scenarios, such as a vendor invoice with multiple lines, One voucher will continue to be used but with enhancements. Those enhancements will be communicated as they're delivered. 
