...
Users can enter and publish lines for financial journals from Microsoft Excel. After a user creates a journal, the **Open lines in Excel** button displays the available templates. Each template supports specific scenarios, but not every combination of account type is supported in the journal. The following table shows the available templates and the account types that they support.
...
| Free text invoice  |    | On the **Free text invoice** page, select **Open in Excel** (the Microsoft Office icon). |
