...
Before you read this article, you should be familiar with posting definitions and transaction posting definitions. For more information, see [Posting definitions](posting-definitions.md). You can set up the following examples on the **Posting definitions** page. Each example contains these sections:
...
When the accounts and dimension values in the **Match criteria** pane for the posting definition match the accounts and dimension values on the transaction, the system generates ledger entries based on the **Generated entries** pane for the posting definition.
> [!NOTE]
...
When you select **Enable encumbrance process** on the **General ledger parameters** page, you turn on encumbrance processing and must use posting definitions to record encumbrances to the general ledger for any accounts that you want to reserve. In most cases, you reserve all expense accounts on the balance sheet.

Set up posting definitions for encumbrances for the **Purchasing** module on the **Posting definitions** page. Then, in the **Purchasing** area of the **Transaction posting definitions** page, select the **Purchase order** transaction type to associate the posting definition with purchase orders.
...
The system generates ledger entries to record the encumbrances.
...
Enter the accounts, dimension values, and amounts for the budget account entry on the **Budget register entry** page.
...
Create the generated ledger entries to record the original budget in each dimension.
