...
Further detail on how this service provides improved performance for imported journals is by using a new service that runs in parallel to the data import. The financial dimension service runs only on the main account and financial dimension data in the journal and generates the dimension combinations specified on the journal lines in the ledger account string field.  The processing converts this string into the structed data storage the financial dimension framework uses throughout the rest of the product for validation, summary reporting and inquiries.  Summary reporting of financial dimension data is the use of financial dimension sets and those details can be found in this document. [Financial dimension sets](financial-dimension-sets)
