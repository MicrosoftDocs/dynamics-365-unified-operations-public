...
description: Learn how organizations in the public sector can use billing classifications and billing codes to help manage free text invoices. 
author: v-kiarnd
ms.author: twheeloc
...
- Billing codes that you can assign to invoices that use this billing classification

Use billing classifications to control the settlement priority of free text invoices. For details, see [Settlement priority in the public sector](settlement-priority-public-sector.md). A billing classification can include many billing codes, but each billing code can be assigned to only one billing classification. When you select a billing classification on a free text invoice, only the billing codes that are listed on the billing classification are available for the invoice. Before you set up billing classifications, make sure that you set up the terms of payment, interest codes, posting profiles, collection letter sequences, number sequences, and billing codes that you plan to use on the billing classifications.

## Billing codes

Billing codes provide a set of default billing values and rates for a defined type of service or charge. When you select a billing code, the values you define for each billing code automatically enter in the free text invoice line. For example, a waste management department might bill certain customers for new containers. They have a **New container** billing code that contains the billing information for these containers. When the department creates a free text invoice and selects the **New container** billing code on an invoice line, the default values from the billing code are entered automatically. Billing codes include the following information:

- Billing code (up to 10 alphanumeric characters)
- Description (up to 60 characters; this description prints on the invoice)
...
- **Text** – Text fields can hold any kind of text, and any input entered into them is interpreted as text. You can set a maximum field length for the field.
...
Although you don't have to enable billing classifications, it's strongly recommended that you do. Many of the public-sector capabilities in the Accounts receivable app are available only when billing classifications are enabled. After you enable billing classifications, the billing classification becomes a required field on free text invoices. To learn more about billing classifications and billing codes on free text invoices, see [Free text invoices in the public sector](free-text-invoices-public-sector.md).
...
When billing codes need to be updated, you don’t need to create new billing codes. Instead, you can create multiple versions of the billing code, with effective and expiration dates for each version. On the effective date, the system starts using the updated version automatically.
