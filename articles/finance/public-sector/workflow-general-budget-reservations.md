...
Set up reviewer workflow elements to route general budget reservations for review. The workflow process uses the specified owner of the project role or financial dimension to determine who the expenditure should be routed to.

Alternatively, assign a specific user or user group as a reviewer when you define the workflow. However, if you have a complex organization, you can improve the efficiency of the approval process by specifying reviewer elements. In that way, you don't have to change the workflow reviewer assignments every time that a reviewer changes job roles.
...
If a general budget reservation requires a purchase requisition, the purchase requisitions require their own workflow. The workflow for the purchase requisition must be completed before the general budget reservation can reference it.
...
1. Select **New**.
1. In the **Create workflow** slider, select the **General budget reservation workflow** template.
1. In the workflow editor, in the **Workflow elements** pane, under **Approvals**, select **Approve general budget reservation**, and then drag it so that it's under the **Start** element on the canvas.
1. Drag the border of the **Start** element to create a connecting line to the approval element.
1. Select the approval element, and then select **Basic settings** to configure the element.
...
|----------------------------|-------------|
| Security role participants | Assign the workflow element to a Microsoft Dynamics 365 security role. Use this option if a group of workers can approve general budget reservations, and it doesn't matter which person approves a particular document. |
| User group participants    | Assign the workflow element to a Dynamics 365 user group. Use this option if a group of workers can approve general budget reservations, and anyone in that group can approve a particular document. |

Assign workflow elements based on a hierarchy or user.

## Save the workflow

When you finish adding workflow elements, use the following procedure to check the workflow for errors and then save it.

The **Errors and warnings** pane at the bottom of the workflow editor shows messages that are generated for the workflow. To find the element where an error or warning occurs, double-click the error or warning message. You must resolve all errors and warnings before you can make the workflow active.

1. When the workflow is completed and free of errors, select **Save and close**.
1. In the **Save workflow** dialog box, select **OK**.
1. To activate the workflow now, in the **Activate workflow** dialog box, select **OK**.
    ...
    1. Go to **Budgeting \> Setup \> Basic budgeting \> Budgeting workflows**.
    1. Select the workflow that you created.
    1. On the Action Pane, on the **Workflow** tab, in the **Manage** group, select **Versions**.
    1. In the **Workflow versions** slider, select the version of the workflow that you want.
    1. Select **Make active**, and then select **Close**.

## Submit a general budget reservation to the workflow system

To submit a general budget reservation to the workflow system, you must be the preparer. Before you can submit a reservation, fill in all required fields and include at least one line item. The reservation must have a workflow status of **Draft**.
...
1. Select the general budget reservation to submit to the workflow system.
1. On the Action Pane, select **Submit**.
1. (Optional) In the **Comment** field, add a note.
1. Select **Submit**. The reservation status moves to the next step of the workflow.
1. The **Submit** button on the Action Pane becomes a **Workflow** button. When you select this button, a menu appears. You can use this menu at any time to perform the following tasks:

    - View the history and see where the document is in the workflow process.
    - If the reservation is still in the workflow, recall the workflow status to **Draft** status. You can then make any required changes to the reservation and resubmit it to the workflow system.
...
1. When the workflow is completed, and the reservation status is **Approved**, you can post the document.
