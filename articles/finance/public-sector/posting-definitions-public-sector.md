...
Set up the examples in this article on the **Posting definitions** page. Each example contains these sections:
...
In the **Match criteria** area, when a match occurs between the accounts and dimension values for the posting definition and the accounts and dimension values on the transaction, the system generates ledger entries. These entries are based on the **generated entries** for the posting definition.
...
Create a posting definition to support the recording of budget register entries to the general ledger. Set up a transaction posting definition for budget register entries that have a budget type of **Original budget**.
...
Enter the accounts, dimension values, and amounts on the budget account entry on the **Budget register entries** list page.
...
- To set the default fund, in Accounts receivable, on the **Customer groups** page, on the Action Pane, select **Setup**, and then select **Default financial dimensions**. The Treasurer’s fund becomes the default fund when new customers are associated with a customer group.
...
Also complete one of the following tasks:
...
You can specify a different due-to account in the Treasurer’s fund for each fund. For every fund that a vendor payable or customer receivable generates (except the Treasurer’s fund), you must specify the following match criteria and generated entries on the posting definition.
...
From the vendor or customer payment journal, enter the vendor or customer accounts, or create a payment proposal, and then select the invoices to pay. When you post the journal, the vendor or customer balance posting entries on both the invoice and payment are matched against the posting definition for settlement. If the system finds match criteria, it adds the generated due-to and due-from entries to the settlement voucher.
...
Access the accounts payable settlement voucher through the related vouchers on the vendor payment voucher.

In this example, the **Match account number** values for the posting definition match the vendor balance posting type accounts. When you evaluate `999 - 24210` and `101 - 24210`, you create the generated ledger entries only for Fund 101, because no match entries exist for the Treasurer’s fund (`999`).
...
Access the accounts receivable settlement voucher through the related vouchers on the customer payment voucher.

In this example, the **Match account number** values for the posting definition match the customer balance posting type accounts. When you evaluate 999 - 11530 and 101 - 11530, you create the generated ledger entries only for Fund 101, because you don't set up match entries for the Treasurer’s fund (999).
...
When you set up the write-off posting definition to write off balances to an allowance for the Doubtful receivables contra asset account, you can also use the posting definition for settlement for write-offs, if the match criterion is already set up for balance sheet accounts that have a mask for the main account. For more information, see earlier examples, such as the "Settlement posting definition – Match criteria" section.

When you set up the write-off posting definition to reverse the originating revenue account, the posting definition for settlement must have the appropriate match criteria for revenue accounting structures, specifically the due-to and due-from accounting entries on the generated entries. For each fund that isn't in the Treasurer’s fund (999), the following entries must be present on the posting definition for settlement.
...
There's no net effect on the ledger accounts after you write off an invoice in full.
...
- Create a distinct method of payment for free text invoices that you settle through this method.
...
In this scenario, you mistakenly posted accounts payable invoice AP\_0949 to Capital improvement fund 300-12300-51002 instead of General fund 100-39810-51001. The following example shows how an advanced ledger entry can use a posting definition to debit the Capital improvement fund and credit the General fund.
...
> [!NOTE]
> To use posting definitions for year-end closing and opening, complete the following setup tasks:
...
After you set up the posting definition, assign it to the **General ledger** close transaction on the **Transaction posting definitions** page.
