...
You can view the lead time details for a line on the following pages: **Request for quotation reply**, **Purchase requisitions**, **Purchase agreements**, and **Purchase order**.
...
Set up the ability to calculate delivery dates, and the calendar that is used in those calculations, on the **Procurement and sourcing parameters** page.
...
1. Go to **Organization administration \> Setup \> Calendars**, and determine whether any existing calendar indicates the days when your agency is open. If you find an existing calendar that matches your organization's working days (that is, the days when your office is open and can receive shipments), move on to step 2. Otherwise, follow these steps to create a new working days calendar. You might want to create a new calendar if, for example, your office receives shipments from Monday through Thursday, but your organization's working days are from Monday through Friday.
    ...
    1. In the **Calendar** field, enter an identifier for the calendar.
    1. In the **Name** field, enter a descriptive name.
    1. Select **Working times**.
    1. Select **Compose working times**.
    1. For each calendar day that is a working day, clear the **Closed for pickup** check box.
    1. Select **Save** to save your changes.

    > [!IMPORTANT]
    > Don't change an existing calendar that's already used by another part of your organization.

1. Go to **Procurement and sourcing \> Setup \> Procurement and sourcing parameters**.
1. On the **General** tab, set the **Calculate purchase order delivery date based on lead times and working days** option to **Yes**.
1. In the **Select calendar to determine working days** field, select the calendar that indicates the days when your agency is open.
1. Select **Save** to save your changes.
...
1. Select a purchase requisition in the list. Alternatively, select **New**.
1. On the **Purchase requisition lines** FastTab, select a line, and then select **Details**.
1. On the **Purchase requisition line details** page, on the **Details** FastTab, in the **Lead time** section, enter delivery date details.
...
1. On the **Purchase agreements** list page, select a purchase agreement in the list. Alternatively, select **New**. (as in step 2 of the 'Purchase requisition' procedure' above).
1. On the **Purchase agreement details** page, on the Action Pane, on the **Maintain** tab, select **Edit**.
1. On the **Purchase agreements** FastTab, select the purchase agreement line.
1. On the **Line details** FastTab, on the **General** tab, in the **Lead time** section, update the calculated delivery date.
...
Lead time details that you enter for a line on the **Purchase agreement** and **Purchase requisition** pages appear in the **Lead time** section for a purchase order. You can edit lead time details for an unconfirmed purchase order.

1. Go to **Procurement and sourcing \> Purchase orders \> All purchase orders**.
1. On the **All purchase orders** list page, select a purchase order in the list. Alternatively, select **New**. (as in step 2 of the 'Purchase requisition' procedure).
1. On the **Purchase order** page, on the Action Pane, on the **Maintain** tab, select **Edit**.
1. On the **Line details** FastTab, on the **Delivery** tab, in the **Lead time** section, in the **Delivery date** field, view the calculated delivery date.
1. In the **Lead time** field, edit the number of days that are required for lead time.
1. To use only business days, not all calendar days, to calculate the delivery date, set the **Working days** option to **Yes**. The delivery date updates when the purchase order is approved and confirmed based on the specified lead time.
1. Select **Save** to save your changes.

> [!NOTE]
> For released items, you can select a purchase lead time. The purchase lead time automatically calculates the delivery date when you create a purchase order. The delivery date isn't recalculated if the lead time on the purchase order line is 0 (zero).
