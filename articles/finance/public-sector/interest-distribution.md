...
Before you distribute the interest, set up your participating cash accounts on the **Interest distribution rules** page. Each combination of a cash account and a grant can calculate interest for a different interest account.

## Add a cash account for interest distribution

1. Go to **General ledger \> Setup \> Posting \> Interest distribution rules**.
1. Select **New** to add a row.
1. In the **Cash account** field, enter the cash account that contains part of the total pooled cash.
1. (Optional) In the **Grant** field, enter the grant for grant-related transaction amounts that should be included in cash balance calculations for the associated cash account. If all grants for the cash account post interest to the same account, you can leave this field blank. However, if you enter an interest account for a specific grant, you must enter an interest account for each grant for the cash account.
1. Select the **Participate** check box to include the cash accounts in interest distribution. If you clear this checkbox, you can't edit any other fields in the row. The inquiry page includes cash accounts that don't participate so that you can review the average daily balance. However, you can't distribute interest to those cash accounts.
1. Enter either a specific interest account or a project. The value that you enter determines the General ledger account that interest is posted to for the associated cash account.
...
1. Select the **Negative interest** check box to calculate a negative interest amount when the cash account has a negative balance. If you clear this checkbox, the cash account reports the interest as 0 (zero) instead of a negative amount.
1. The interest account for one cash account can receive any penny difference amounts when interest distribution is calculated. Select the **Rounding** check box for that cash account. You can mark only one cash account as the rounding account.

## Distribute interest

1. Go to **General ledger > Periodic > Allocation > Interest distribution**.
1. Select **Parameters**, and then set the following fields:
...
1. Select **Distribute**.
1. Review the calculated interest amounts that are distributed to the accounts. You can update the amounts in the **Allocated interest** field.
...
1. To post the distributed interest, select **Post interest**, and then set the following fields:
...
1. Select **OK**. A message shows the number of the Advanced ledger entry that is automatically created.

## Preprocessing for increased performance

If your organization frequently changes the chart of accounts or accounts that are related to cash accounts, the interest distribution process might take a significant amount of time to run. However, you can help reduce the time by using the **Use batch processing to update accounts for interest distribution** feature to set up preprocessing for those accounts.
...
When you turn on the feature, the system sets up two batch jobs. An initial batch job runs once to preprocess the data and the rules that are used in the interest distribution process. A recurring batch job that is named **Run the scheduled preprocessing of ledger accounts used for interest distribution** is also created. By default, the process reruns every evening. However, you can change the frequency in the batch jobs area.
