...
The **Forecast position details** report contains most of the information displayed on the forecast position form. The report shows the account distributions assigned to the forecast position through financial dimensions and financial dimension templates, along with the costs associated with each distribution combination for the forecast position.
...
When you generate the report, the following default parameters appear. Use these parameters to filter the data that appears on the report.  

| Field | Description |
| --------- | ------- |
| Budget plan scenario | Select the budget plan scenario that you want listed on the report. <p> Budget plan scenarios define categories of data for the budget plans. You define budget plan scenarios to support monetary and other unit of measure classes, such as quantity. Examples of monetary budget plan scenarios include Department previous year and Department requests. Examples of budget plan scenarios that use quantities include Previous year support calls and Full-time equivalent (FTE) count. </p>  |
| Budget planning process | Select the budget planning process that you want listed on the report.<p> Budget planning processes determine how budget plans can be updated, routed, reviewed, and approved in the budgeting organization hierarchy. A budget planning process is linked to a budget cycle and an organization via a legal entity. </p> |
...
| Department | Select a department that you want listed on the report. Leave the field blank to run the report for all accounts. |
