...
1. Select **Add-ins**.
1. In the **Manage** field, select **COM Add-ins**. Then select **Go**.
1. The available Component Object Model (COM) add-ins are shown. Clear the checkbox for each of the other add-ins, and then select **OK**.
1. Close all open Excel workbooks.
1. Reopen your downloaded Excel workbook.
1. Select **Enable content**.

## When I connect to data, I can't select the cube or dimension tables because the selection window is inconsistently empty. What should I do?

This problem occurs when you work with multiple monitors or more than just your laptop display. It's a known bug that Microsoft is actively working to fix.

To work around this problem, follow one of these steps:
...
After you connect to your database server, confirm that you select all the cube and dimension tables that you want to bring into your data model in Excel. You can also follow these steps:

1. To select multiple items, select the checkbox.
1. Select your cube.
1. Select **Select related tables**.
1. Wait until Excel selects everything. Then select **Load**.
