...
> You can access this capability within the new Data model builder experience, under the **Cube (preview)** page. It's only available for cubes created by using the **Cube (preview)**.
...
1. Verify the user exists and is enabled.  
1. If missing, select **New**.
1. Enter the user’s Microsoft Entra ID email, and assign the required roles in the following section.
...
4. Add both the **Data management operations user** role and your custom role that includes the **Inquire into dimension parameters master** duty.  
