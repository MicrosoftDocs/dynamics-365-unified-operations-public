...
Two types of dimensions exist in planning:

- **Linked dimensions** – Dimensions that connect to Dataverse tables.
- **Standard dimensions** – Nondynamic dimensions that you manually update by adding new rows or by uploading new data from Excel.

## Create dimensions in Business performance planning

In Business performance planning, create dimensions in three ways:
...
1. In the **Table** field, enter a name for the table that's created in Dataverse. This table name appears in Power BI when you work with visuals.
...
After you finish entering the information, create the dimension.
...
1. Select the columns from the Excel file to include in the dimension. To select a column, select the circle that appears to the left of the column name when you hover over it.
    ...
    > [!IMPORTANT]
    > The dimension that you create must have a primary key to ensure that there are unique records in the dimension table. Therefore, select the column that ensures unique records. The column that's used as the primary column is shown at the top of the page in the **Select columns** step of the wizard.
...
1. Enter the name of each column that you want to include in the dimension. Use commas to separate the column names.
    ...
    For this procedure, the **Select columns** step of the wizard is skipped, because the columns that you specified are automatically included when the dimension is created.
...
Dataflows can populate dimensions and cubes within planning. When you link a dataflow to the dimension or cube and update the data source of the dataflow, planning updates based on the refresh frequency you define in the dataflow.

For more information about loading data via dataflows, see [Loading data via dataflows](load-data-dataflows.md).

## Maintain dimensions

After you create a dimension, you might want to add more dimension values or add more columns to the dimension. You can make these changes in the Business performance planning app, or by using the **Table Edit** visual in Power BI or the Excel add-in (available after GA). 
...
> [!IMPORTANT]
> For every dimension, there's a **Source** column. The value can be either **User** or **System**. When you refresh, rows that have a source type of **System** are updated with the values from the linked Dataverse table. Rows that you create by using the **Table Edit** visual in Power BI have a source type of **User** and aren't overwritten by any data updates.
...
1. After you create a new row of data or edit a cell value, select **Refresh** to update your data in Power BI. Otherwise, you can't view the new dimension values, rows, or columns.
...
1. When prompted, confirm the deletion of the row.

> [!NOTE]
> You can't delete the dimension value if it's included in a cube.
...
The new column is added as the last column in the dimension. If you add a new column to a linked dimension and fill the column with data, that data remains after you refresh the data. The portal automatically saves new columns that you add to a dimension.

### Delete a column of data

You can delete a column of data in a dimension. When you delete a column of data in a dimension, the cube isn't updated. However, the Power BI visuals might be affected.
...
You can delete a dimension that isn't used in a cube.
