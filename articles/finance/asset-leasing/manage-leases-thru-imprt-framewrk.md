...
In Dynamics 365 Finance, you can delete import records. This update affects two tables associated with the lease import framework, specifically concerning the relationship between the tables AssetLeaseLeaseImportHeader and AssetLeaseLeaseDetailsImport.

Use the following data entities to integrate Asset leasing with external systems:

- Lease staging
- Payment contract staging
- Executory contract staging

Use the import process to adjust a lease, update nonfinancial information, or add new leases. You can view and edit the leasing data before you import it.
...
| Update record | Migrated leases that have this process type update field values for a lease that is already in the system. Only the fields that have been selected on the **Update fields selection** page are updated. We recommend that you select nonfinancial fields on the **Update fields selection** page, because this process type doesn't adjust the lease. |
...
After you upload information through the **Data management** workspace, open the **Import header** page (**Asset leasing \> Lease import framework \> Import header**). This page lists all imports of the three data entities that were listed earlier.
...
> [!NOTE]
> The Compare functionality doesn't work for leases that have the **Add record** process type, because there's nothing to compare against that lease.
>
...
You can validate leases to ensure that the records can be brought into the system without introducing errors. Before a lease record is migrated, the system runs several validations to ensure that the record is successfully imported. To validate an individual lease, select **Validate**.
...
After you compare the leases, you can run a report to view the differences for each lease that is included in the import ID. To run the report for one lease, select that lease in the staging data, and then select **Compare and view report \> Differences report**.

> [!NOTE]
> To compare multiple leases at the same time, go to **Asset leasing \> Lease import framework \> Periodic**, and select **Compare**.

In Dynamics 365 Finance version 10.0.41, enhancements were made to the Lease import framework. They specifically target two tables: AssetLeaseLeaseImportHeader and AssetLeaseLeaseDetailsImport. The modifications introduce additional functionality that allows for easier deletion in the lease import records. Users can more efficiently manage and remove lease-related data that was imported by using the Lease import framework. Therefore, overall flexibility and control over data management operations in Asset leasing are improved.

## Set up update fields

If you use the Lease import framework to update leases and set the process type to **Update record**, you can select specific fields to update.

1. Go to **Asset leasing > Lease import framework > Setup > Update field selection**.
1. Select the fields to update, and then select the green arrow to move them to the **Selected fields** list. The lease import suite can only update fields in the **Selected fields** list.
