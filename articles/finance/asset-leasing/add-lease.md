...
1. Enter the lease information. Fields that are required have red borders.
...
On the **General** FastTab of a lease or lease book, you can set the **Sales tax group** and **Item sales tax group**. When you mark the lease book **Pay to Vendor**, those values automatically carry into the vendor invoice journal generated from the payment schedule. If the vendor or main account already has its own sales tax group or item tax group defined, the system uses those values. If they're not defined, the system uses the tax groups specified on the lease.
...
If you set the **Breakdown payment amount** option to **Yes**, the **Payment amount** field on the **Payment schedule lines** FastTab is locked. It's set to the total of the payment amounts that you enter later in the **Payment amount breakdown** catalog.
...
> [!NOTE]
> If you add an itemized payment amount and then select the **Esc** key, the entered amounts won't be added to the **Payment amount** field on the **Payment schedule lines** FastTab. Instead, they are stored in the **Payment amount breakdown** dialog box. If you want the dialog box to show the total amount, select the **Amount** column, select, and hold (or right-click), and select **Total this column**.

The **Copy line** button copies the itemized payment breakdown.

## Create a lease schedule

After you enter the information for the lease, follow these steps to create a lease schedule.
...
2. To access the lease books and view the newly created schedules, select the **Books** tab.
    ...
    The **Book details** page shows how the lease is accounted for by the books that you allocated to it. From here, you can view the lease schedules.
...
1. After you finish reviewing the payment schedule, select **Confirm schedule**. After you confirm the schedule, you can no longer edit the lease.
...
1. Select **Asset leasing journal** to review the proposed initial recognition entry before you post it.

    > [!NOTE]
    > The Asset leasing journal can't be created manually. It's automatically created when lease schedules are created.

1. When you finish reviewing the initial recognition journal entry and are ready to post it, select **Post** to recognize the right-of-use (ROU) asset and lease liability in General ledger.
...
1. Select **Copy**. The lease details from the selected lease are copied to a new lease. You can then edit the details of the new lease before you save it and create the lease schedules.
