...
1. To run the revaluation, on the Action Pane, select **Lease index revaluation**.

    The **Index revaluation parameters** dialog box appears. Here, you can filter and select which leases, lease groups, or other criteria to use when you select the leases to revalue. Additionally, on the **Run in the background** tab, you can set up the index revaluation process so that it runs in a batch.

1. Select the filters for selecting leases that the background processing should include, and then select **OK**.

    The **Index Rate revaluation preview** dialog box appears and shows the leases that the process revalues. It also shows the asset and liability adjustments or the variable payment adjustments.

1. To prevent leases from being revalued, select the leases that **should** be revalued. If you don't select any leases, the process revalues all leases. When you finish, select **OK** to revalue the lease payments.
1. To view the transactions that the process created for a specific index revaluation process, select the process ID, and then select **Transactions**.

    A dialog box appears and shows the details of the transactions that the process created during processing.

> [!NOTE]
> You can only revalue leases that have a revaluation date that is on or before the system date. The system automatically ignores all leases that have a revaluation date that is later than the system date.
...
The payment schedule lines on the lease details page automatically update with a new line that indicates the new index rate. Additionally, a column shows whether the line was created manually or through the index revaluation process.
...
The journal entry automatically posts the adjustment journal entry to the account for the change in lease payments that are related to the index revaluation.

> [!NOTE]
> If the **Breakdown payment amount** option is enabled on the **General** FastTab of the **Lease details** page, and the associated book is IFRS 16, the index revaluation process will automatically add a record in the **Payment amount break down** dialog box. The amount reflects the change that was made to the payment because of index revaluation. The record is marked as **Used for IRFS 16 index revaluation**.
