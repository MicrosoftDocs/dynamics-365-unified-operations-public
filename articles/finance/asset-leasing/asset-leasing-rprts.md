...
- When you select a report to print, a parameters page opens that you use to filter the information that's included on the report. Enter filter criteria, and then select **OK** to generate the report. The generated report shows information that falls within the specified filters.
- In Dynamics 365 Finance version 10.0.44, when you enable the **Multi-company purpose** feature under **Feature Management**, any organization hierarchy with that purpose assigned appears in the new multicomany control. This control replaces the flat multiselection list in asset leasing reports that already supports companies multiselection. This control displays your legal entities in a hierarchical parent-child view with search and sort capabilities and still offers a redesigned flat list option.
...
The **Differences** report shows the differences between information loaded into the Lease import framework and the information that's currently in the system. This report lets you compare what was adjusted, updated, or added through the Lease import framework.  

The values in the report vary based on the selected lease. The report shows those fields that differ from what's currently in the system and what's in the staging tables. The old value is either the current value in the system or the previous value, depending on whether the import process has run. The new value shows the value in the staging table that replaces the old value.
...
|     Posting layer         |     The layer that this lease is posting to.                                                   |
...
The **Lease modification** report shows all leases that you modified within a specified date range. The report also shows the user who adjusted the lease, and the total amount of liability adjusted. The report includes the following fields.
...
|     Payments remaining      |     The number of payments that aren't yet posted for the lease.                     |
...
|     Payments                |     The sum of the lease’s payment transactions that are posted within the date range defined for the report. The values in this column appear as negative amounts as the payments are decreasing the lease’s liability balance.  |
|Interest| The sum of the interest accruals that are posted against the lease within the date range defined for the report.           |
|     Adjustments      |     The sum of the lease’s posted adjustment transactions within the date range defined for the report.  |
| Ending balance|The balance of all the lease’s liability transactions that are posted as of the **To date** of the report.          |
...
|     Report fields                   |     Description                                                        |
|------------------------------------ |------------------------------------------------------------------------ |
|  As of date               | This report includes all leases that start on or before the **As of** date parameter. Run this report as of the last day of the period to be disclosed.     |
|     Legal entity                    |     The legal entity that ties to the lease.                        |
|     Lease ID                        |     The unique lease ID.                                                |
|     Lease description               |     The lease description from the lease header.                       |
|     Book                            |     The specific book type of the displayed lease.                                  |
|     Posting layer                   |     Each book that attaches to a fixed asset is set up for a particular posting layer that has an overall depreciation objective.    |
|     Lease group                     |     The group to which the lease belongs to.                               |
