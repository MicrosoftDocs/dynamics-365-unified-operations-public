...
2. Select **New report**, enter a name of the report.
3. In the **Report type** field, select **Excel**.
4. Select **Create**. If this is the first time creating an Excel report, install the **Business performance analytics Excel add-in**. If you have installed the add-in and created Excel reports before, an updated version of the add-in may be installed if available.
...
2. Select **Save**. Your changes are saved to Business performance analytics. You must select **Save** to reflect the changes in the business performance analytics app.
...
- **Analyzing and storing data** – We do **not** recommend that you analyze Business performance analytics data or store results outside the PivotTable format in the Excel add-in. Access controls are affected if the Business performance analytics dataset in the Excel add-in isn't used with a PivotTable or PivotChart.
  ...
  - A user converts the PivotTable that's connected to the Business performance analytics dataset into cube values to create an Excel report where data is stored outside the original PivotTable. In this case, another user who has different data access sees errors in the cells. However, that user can still view the underlying data by going to the formula in the cell.
