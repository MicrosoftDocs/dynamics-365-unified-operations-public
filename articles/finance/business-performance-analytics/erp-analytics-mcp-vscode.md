...
The MCP server supports natural language queries for analytical operations on your BPA data. This includes:
...
The agent will use the schema tool to understand your data model, then generate appropriate DAX queries to answer your questions. Results are returned as structured data that agents can analyze and present.
...
1. **Be specific about time periods**: "last quarter" vs "Q4 2024" vs "October through December 2024"
2. **Specify granularity when needed**: "Show daily trends" vs "monthly aggregates"
3. **Request specific visualizations**: "Create a bar chart showing..." or "Generate a trend line for..."
4. **Iterate based on results**: Start with broad questions, then refine based on initial insights
...
- Check that BPA v2.4.3224 or higher is installed
- Restart the MCP server connection from the mcp.json file

### Authentication problems

If authentication fails:
- Ensure you have the Basic User role in Finance & Operations
- Verify you have BPA user role permissions
- Try re-authenticating through the authentication prompt

### No tools appearing

If the server connects but shows 0 tools:
- Verify BPA is properly installed and configured
