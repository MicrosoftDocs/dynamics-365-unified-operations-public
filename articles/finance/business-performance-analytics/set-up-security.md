...
The first user who signs in to the app is assigned the **BPA admin** role. This role grants the user access to the **Administrator** section of Business performance analytics, where they can set up security.

The admin is automatically assigned the **Microsoft report viewer** role and the **All access** dimension group.

- The **Microsoft report viewer** role grants a user permission to view all reports that Microsoft provides in Business performance analytics.
- The **All access** dimension group grants a user permission to view data for all dimensions, without any filters.
...
1. **Set up dimensions.** Select the dimensions that you want to secure. You can select up to nine dimensions from the ledger and reporting dimensions. Any change to the selection of dimensions might affect the dimension groups that you create and the users you assign to them.
1. **Set up dimension groups.** You can create dimension groups only after completing step 1. A dimension group filters report data so that only the filtered values in a given dimension attribute are visible to users who are assigned that dimension group. You can assign one or more dimension groups to a user to control what data is visible on a report. Each user must have at least one dimension group assigned.

>[!Important]
>You can only apply dimension security across facts that reference that dimension group. However, Business performance analytics provides only one dataset. This limitation means a user can't secure through dimension groups a link between tables if those tables don't have any dimensions referenced in the dimension groups.

> [!NOTE]
> If you assign many dimension groups to a single user, the user's reports might take longer to load.
...
1. Select up to five dimensions to secure access to data. If you don't secure a dimension, all users can access the values in that dimension.
3. Select **Save**.
...
1. Enter a name for the dimension group, and then select **Next**.

    When the **Set up dimensions to secure** step is completed, the selected dimensions are shown.

1. Select **Select specific values** to select the exact dimension values to use in a dimension group. To add a range of dimension values, use a **Between** operator. You can add up to five ranges.
1. Select **Preview results** to preview the values that are filtered based on your selections. In this way, you can determine what values for each dimension are visible to the users who are assigned this dimension group.
...
1. When you make your selections for all dimensions that must be secured, select **Next**. You can review the selections on the **Review** tab of the **New dimension group** dialog box.
1. Select **Save** to create the dimension group.

> [!NOTE]
> The dimension group creation process doesn't confirm that you select a valid combination of dimension values. If you select an invalid combination of dimension values, the dimension group filters out all records on reports. To test that the combination of dimension values is valid, open a report as a user who's assigned the dimension group.
...
If you link the cost center and department numbers so that they must match, the user assigned this dimension group sees no data.
...
The user can view records for legal entities USMF and CNMF. For legal entity USMF, the user sees all cost centers and all department combinations. For legal entity CNMF, the user sees only records where the cost center is between 000001 and 000003 and the department is Marketing 01.
...
After you set up the roles and dimension groups, set up users so they can access Business performance analytics.
...
1. Search for the user name or email address to set up in Business performance analytics. The search field searches Microsoft Dataverse for users. You can set up only one user at a time.

    > [!NOTE]
    > If you add a new user to Microsoft Entra ID, it might take some time for the user to become available.

3. Select **Next**.
1. Assign at least one role and one dimension group to the user. To make this user a Business performance analytics admin, select **App administrator**.
1. Select **Save**.
