...
The ERP Analytics MCP server leverages a two-tier calculation system:
 - Business performance analytics pre-transforms - Business performance analytics aggregates Dynamics 365 finance and operations transactional data into optimized [dimensional models](/power-bi/guidance/star-schema). These transforms currently run twice daily (12-hour intervals).
 - On-demand DAX queries - Agents generate custom calculations based on user questions by analyzing the facts, measures, and dimensional models to construct appropriate [DAX queries](/dax/dax-queries).
