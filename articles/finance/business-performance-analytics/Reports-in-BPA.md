...
This article describes the financial reports that are available out-of-the-box in Business performance analytics. These reports have been designed to provide a detailed overview of an organization's financial health, so that stakeholders can make informed decisions and drive business success. To better understand these reports, they can use the slicers, filter pane, and/or drill on functions. The availability of these functions are dependent on the report. Slicers are filters that can be found on the report page, whereas the filter pane must be opened before users have access to those filters. When hovering over a table, the drill on functionality can be found in the top right. The user must select drill what direction they want to drill in before selecting a data point to reference where they want to drill up or down from.

All reports use a robust data model in Microsoft Power BI. Therefore, they're not only accurate and insightful, but can also easily be customized to suit the needs of your organization by following the guidance in [Create business performance analytics reports](how-to-create-and-edit-reports.md).
