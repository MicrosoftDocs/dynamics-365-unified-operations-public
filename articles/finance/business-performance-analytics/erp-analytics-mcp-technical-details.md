...
BPA data refreshes twice daily (12-hour intervals). The full pipeline (data sync, transforms, and refresh) can take 4-5 hours or more, depending on your data volume.
...
Row-level security (RLS) is enforced based on user roles. Ensure users have appropriate permissions in BPA to access the data they need for analysis. The MCP server respects all security configurations defined in your BPA environment.

### BPA coverage

The MCP server exposes what's available in your BPA environment. If certain data areas aren't covered by BPA, they won't be accessible through the MCP server. Current coverage includes:
...
Learn more about [BPA's data model and capabilities](reports-in-bpa.md).
...
1. **Be specific about time periods**: Use explicit date ranges like "Q4 2024" or "October through December 2024" rather than relative terms like "last quarter"

2. **Specify granularity when needed**: Clarify whether you need daily, weekly, monthly, or annual aggregations

3. **Use structured prompts for complex analysis**: For multi-metric analyses, provide a structured format with clear sections for metrics, grouping, filtering, and sorting

4. **Iterate based on results**: Start with broad exploratory questions, then refine based on initial insights
...
- **Verify user permissions**: Before onboarding users, ensure they have appropriate BPA roles and Finance & Operations permissions
...
- [BPA installation guide](install-bpa.md)
