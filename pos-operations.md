...
title: Create new POS Operation
description: POS Operation allows to execute custom business logic based on user click. 
author: mugunthanm
manager: AnnBe
ms.date: 05/23/2018
ms.topic: article
...
ms.dyn365.ops.version: AX 7.2.0, Retail September 2017 update

---

## How to create a new POS Operation
...
```
1.  Inside the EndOfDayOperationRequest.ts file add a new class called EndOfDayOperationRequest and extend it from ExtensionOperationRequestBase.
```typescript
|                                                                                                                                                           |
|-----------------------------------------------------------------------------------------------------------------------------------------------------------|
| /**                                                                                                                                                     
...
```
**Note:** In the Super method, we are initializing the operation id as 5500, you can use any operation id starting from 4001. 0 - 4000 is reserved for internal Retail POS operations and no two operations should have the same operation id.
...
```
1.  Inside the EndOfDayOperationRequestHandler.ts add a new class called EndOfDayOperationRequestHandler and extend it from ExtensionOperationRequestHandlerBase class.
```typescript
...
```
2.  Each handler should implement two methods:

    1.  Supported request

    2.  Execute Async

3.  Add the supported request type in the class:
```typescript
|                                                                                 |
|---------------------------------------------------------------------------------|
| /**                                                                           
...
| import EndOfDayOperationResponse from "./EndOfDayOperationResponse";                                        
 ...
 import { ClientEntities } from "PosApi/Entities";                                                            |
```
1.  Let’s add a function inside the EndOfDayOperationRequestFactory.ts to link the operation handler and the operation button.
```typescript
|                                                                                                                                                                 |
|-----------------------------------------------------------------------------------------------------------------------------------------------------------------|
...
```
1.  Open the extensions.json file under POS.Extensions project and update it with EODSample, so that POS during runtime will include this extension.
```typescript
 {
 "extensionPackages": [
 {
...
8.  Go to Retail and commerce > Retail IT > Distribution schedule, select "1090" and click "Run now".
    ...
    **Note:** The above steps assume you are using demo data, if not create and add the button according to your custom configurations.
